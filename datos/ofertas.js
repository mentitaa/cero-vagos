/* Generado por el motor de Cero Vagos. No editar a mano. */
window.CERO_VAGOS = {
 "generado": "2026-09-26T10:07:34",
 "total": 423,
 "stats": {
  "total_procesadas": 17496,
  "aprobadas_vigentes": 423,
  "tasa_aprobacion": 2.4,
  "sin_sueldo": 12919,
  "pct_sin_sueldo": 74,
  "sueldo_mediano": 1300,
  "por_fuente": {
   "Laborum": 234,
   "Bumeran": 189
  },
  "por_departamento": {
   "Lima": 381,
   "Callao": 17,
   "Arequipa": 6,
   "Cusco": 5,
   "La Libertad": 3,
   "Piura": 3,
   "Ica": 2,
   "Áncash": 2,
   "Ayacucho": 1,
   "Junín": 1,
   "Lambayeque": 1,
   "Puno": 1
  }
 },
 "ofertas": [
  {
   "id": 1,
   "puesto": "UPN San Juan de Lurigancho: Gestor de Tiempo Real",
   "empresa": "Laureate",
   "cat": "Atención al Cliente",
   "min": 2400,
   "max": 2400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 0,
   "vence": "2026-10-26",
   "restan": 30,
   "score": 97,
   "resumen": "En UPN contribuimos a la transformación del Perú a través de una educación inclusiva, accesible y de calidad que transforma vidas.",
   "funciones": [
    "Serás responsable de supervisar y administrar en tiempo real la carga y el flujo de los canales de Contact Center (llamadas, chat, SMS, email, web), asegurando el cumplimiento de la planificación diaria, los niveles de contactabilidad y los resultados operativos del equipo de asesores.",
    "Asegurar el cumplimiento de la planificación diaria y semanal de las campañas vigentes",
    "Monitorear el logueo/deslogueo de agentes y la operatividad de sus recursos",
    "Controlar el desborde de los canales y garantizar la cobertura de asesores",
    "Generar reportes post mortem diarios, semanales y mensuales",
    "Coordinar breaks y alertar/escalar cortes o interrupciones en la gestión",
    "Balancear la carga operacional monitoreando horarios de transferencia (VDN)"
   ],
   "requisitos": [
    "Estudios universitarios o técnicos completos o en curso en Ingeniería de Sistemas, Telecomunicaciones, Informática, Computación o afines",
    "Manejo indispensable de Genesys (reportería y control de resultados)",
    "Manejo de Microsoft Office a nivel intermedio",
    "Deseable: manejo de SQL y Excel",
    "Experiencia mínima de 3 años en el puesto (Controller para Call Center)"
   ],
   "beneficios": [
    "Horario: 45 horas semanales.",
    "Modalidad: presencial.",
    "Lugar de trabajo: UPN San Juan de Lurigancho.",
    "Sueldo Fijo S/2,400",
    "Bono por indicadores: S/1,200",
    "Ingreso a planilla completa desde el primer día",
    "Seguro Vida Ley desde el primer día",
    "EPS cubierta al 50% + seguro oncológico al 100%",
    "Beneficios educativos en instituciones Red Laureate (UPN, UPC, Cibertec)",
    "Descuentos corporativos",
    "Línea de carrera",
    "Excelente clima laboral"
   ],
   "url": "https://www.laborum.pe/job/laureate/upn-san-juan-de-lurigancho-gestor-de-tiempo-real-gtr/6ab73114ba83cb6e704f8372"
  },
  {
   "id": 2,
   "puesto": "Anfitriona, azafata, operador, cajera, seguridad",
   "empresa": "PLANET GAME S.A.C.",
   "cat": "Atención al Cliente",
   "min": 1250,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 0,
   "vence": "2026-10-26",
   "restan": 30,
   "score": 94,
   "resumen": "RESUMEN: ¡ÚNETE AL EQUIPO DE PLANET GAME S.A.C.!",
   "funciones": [
    "AZAFATA DE SALA",
    "Atender cordialmente a los clientes, ofrecer las degustaciones y servicios establecidos por la empresa, mantener ordenada su zona de trabajo y estar permanentemente atenta a las necesidades de los clientes.",
    "Recibir y despedir cordialmente a nuestros clientes, brindar orientación al ingreso, informar sobre promociones y actividades de la sala y contribuir a brindar una excelente primera impresión.",
    "OPERADOR DE SALA",
    "Supervisar el correcto funcionamiento de las máquinas, brindar asistencia básica a los clientes, reportar incidencias y apoyar en las diferentes actividades operativas de la sala.",
    "Realizar las operaciones de caja de manera responsable y ordenada, brindar una atención rápida y cordial, cumplir los procedimientos de control establecidos y mantener correctamente registrada la información correspondiente a su turno.",
    "OPERARIO DE LIMPIEZA",
    "Mantener permanentemente limpias y ordenadas las diferentes áreas de la sala, incluyendo servicios higiénicos, zonas de atención, máquinas, mobiliario, pisos y demás espacios asignados.",
    "PERSONAL DE SEGURIDAD",
    "Velar por la seguridad y el orden dentro de la sala, permanecer atento al ingreso y salida de clientes, informar cualquier situación irregular y cumplir los protocolos de seguridad establecidos por la empresa.",
    "PUESTOS DISPONIBLES",
    "Azafatas de sala: S/ 1,250 + S/ 150 de bono mensual"
   ],
   "requisitos": [
    "Secundaria completa",
    "Experiencia mínima de 3 meses en atención al cliente, servicios, caja, seguridad, limpieza o puestos similares, según el cargo al que se postule",
    "Buena presencia y trato cordial con los clientes",
    "Responsabilidad, puntualidad y compromiso con el trabajo",
    "Facilidad para trabajar en equipo",
    "Buena comunicación y vocación de servicio",
    "Capacidad para seguir procedimientos e indicaciones",
    "Disponibilidad para trabajar en horarios rotativos",
    "Disponibilidad para laborar en turnos de mañana, tarde, noche y amanecida",
    "Disponibilidad para trabajar en nuestras diferentes salas de máquinas tragamonedas"
   ],
   "beneficios": [
    "Sueldo mensual de S/ 1,250",
    "Bono mensual de S/ 150",
    "Jornada laboral de 8 horas",
    "Ingreso a planilla completa desde el primer día",
    "Todos los beneficios laborales de acuerdo con ley",
    "Trabajo estable en una empresa del rubro de entretenimiento",
    "Capacitación para el correcto desempeño de tus funciones",
    "Buen ambiente laboral",
    "Oportunidad de aprendizaje y desarrollo",
    "Posibilidades de crecimiento y línea de carrera dentro de Planet Game"
   ],
   "url": "https://www.laborum.pe/job/planet-game-s-a-c/anfitriona-azafata-operador-cajera-seguridad-salas/6ab738aeba83cb6e704f99ad"
  },
  {
   "id": 3,
   "puesto": "Técnico 1 MHFC",
   "empresa": "CICSA PERU S.A.C.",
   "cat": "Tecnología",
   "min": 1250,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 0,
   "vence": "2026-10-26",
   "restan": 30,
   "score": 92,
   "resumen": "RESUMEN: Se parte de uno de los conglomerados más grandes e importantes de todo Latinoamérica.",
   "funciones": [
    "Cumplir con los programas de mantenimiento preventivo y correctivo de red HFC.",
    "Realizar un mantenimiento de red a la planta Interna (HUBs) cumpliendo con los parámetros de operación óptimos para su funcionamiento.",
    "Lograr el rendimiento de producción diaria establecido por la compañía en todas las actividades de mantenimiento que se le asigne.",
    "Otras funciones que indique su jefe inmediato"
   ],
   "requisitos": [
    "Estudios técnicos en carrera de telecomunicaciones, u otros relacionadas",
    "Experiencia mínima de 6 meses en posiciones similares.",
    "Conocimiento de redes HFC, DOCSIS 3.0, 3.1, FTTH.",
    "Conocimiento de equipos pasivos y activos de redes de televisión por cable.",
    "Redes de Datos, Telefonía, Cableados, Electricidad, Office , Networkig.",
    "Portadoras de Video Audio CH de forward y Retorno",
    "Conocimiento de MS Office (Excel, Office).",
    "Licencia de conducir (A1)"
   ],
   "beneficios": [
    "Sueldo 1250 Soles",
    "Ingreso a planilla de régimen general desde el 1er día.",
    "Seguro Essalud cubierto al 100%, SCRT, Seguro vida ley.",
    "Línea de carrera.-Requerimientos- Educación mínima: Técnico2 años de experienciaEdad: entre 22 y 46 añosLicencias de conducir: A-I Disponibilidad de viajar: SiPalabras clave: tecnologo, tecnico, tech, technician, technology, tecnologia, technologist"
   ],
   "url": "https://www.laborum.pe/job/cicsa-peru-s-a-c/tecnico-1-mhfc-telecomunicaciones/6ab7536aba83cb6e704fdd2e"
  },
  {
   "id": 4,
   "puesto": "Auxiliar de Producción, Ate",
   "empresa": "ilender Perú",
   "cat": "Ingeniería",
   "min": 1415,
   "max": 1415,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 0,
   "vence": "2026-10-26",
   "restan": 30,
   "score": 91,
   "resumen": "RESUMEN: GANA HASTA S/ 1,840 AL MES*· TE OFRECEMOS:",
   "funciones": [
    "Realizar el pesaje de productos e insumos.",
    "Recepcionar y verificar insumos.",
    "Apoyar en las actividades operativas del área.",
    "Cumplir con las funciones asignadas por el supervisor inmediato.",
    "¿Por qué trabajar en ilender?",
    "Porque formarás parte de una empresa líder en su sector, donde valoramos el esfuerzo, promovemos el aprendizaje continuo y brindamos oportunidades reales de crecimiento profesional en un entorno de trabajo colaborativo y seguro.",
    "Vacantes disponibles para personas con o sin discapacidad.",
    "En ilender promovemos la igualdad de oportunidades y valoramos la diversidad, fomentando un ambiente de trabajo inclusivo y respetuoso para todos."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 6 meses en puestos similares.",
    "Disponibilidad para trabajar en horarios rotativos.",
    "Responsabilidad, compromiso y disposición para el trabajo en equipo."
   ],
   "beneficios": [
    "Desayuno gratuito",
    "Almuerzo cubierto al 80%",
    "Planilla completa desde el primer día",
    "Pagos mensuales",
    "Asignación familiar (según corresponda)"
   ],
   "url": "https://www.laborum.pe/job/ilender-peru/auxiliar-de-produccion-ate/6ab76216ba83cb6e704ffe83"
  },
  {
   "id": 5,
   "puesto": "Asesores(as) de Ventas Call center San Miguel",
   "empresa": "MONTALVO INSTITUTE CORP SAC",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 0,
   "vence": "2026-10-26",
   "restan": 30,
   "score": 88,
   "resumen": "RESUMEN: En Montalvo Institute, institución líder en formación en cosmetología y belleza, buscamos Asesoras de Ventas Telefónicos para formar parte de nuestro equipo comercial.",
   "funciones": [
    "Contactar a interesados y brindar información sobre nuestras especialidades.",
    "Identificar necesidades del cliente y ofrecer la mejor opción.",
    "Realizar seguimiento a prospectos para concretar ventas.",
    "Registrar información en el sistema (CRM).",
    "Cumplir metas e indicadores comerciales."
   ],
   "requisitos": [
    "Experiencia en ventas telefónicas, por call center o atención al cliente.",
    "Buena comunicación y orientación a resultados.",
    "Disponibilidad para laborar presencialmente en la sede San Miguel.",
    "Horario disponible",
    "(Descanso rotativo sábado o domingo.)",
    "Lunes a Viernes: 8:00 pm - 5:00 p.m.",
    "Sábado: 8:00 a.m. a 6:30 pm",
    "Domingo: 8:00 am - 3:30 pm (Se otorga bono Dominical S/.20)"
   ],
   "beneficios": [
    "Sueldo fijo S/1300 + Comisiones y/o bonos por cumplimiento de ventas",
    "Planilla completa desde el primer día (Asignación Familiar, CTS, Gratificación, Vacaciones, Vida Ley, etc.)",
    "Comisiones + incentivos",
    "Bono por asistencia dominical",
    "Línea de carrera",
    "Capacitaciones Constantes"
   ],
   "url": "https://www.laborum.pe/job/montalvo-institute-corp-sac/asesores-as-de-ventas-call-center-san-miguel-sueldo-fijo/6ab740c2ba83cb6e704fad35"
  },
  {
   "id": 6,
   "puesto": "Atención al cliente",
   "empresa": "PLANET GAME S.A.C.",
   "cat": "Otros",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 0,
   "vence": "2026-10-26",
   "restan": 30,
   "score": 86,
   "resumen": "Planet Game S.A.C., empresa peruana dedicada al rubro del entretenimiento, busca personas dinámicas, responsables y con actitud de servicio para formar parte de nuestro equipo.",
   "funciones": [
    "Promocionar nuestros juegos y servicios.",
    "Atender y orientar cordialmente a los visitantes.",
    "Realizar cobros y operaciones de caja.",
    "Apoyar en las diferentes áreas del establecimiento.",
    "Mantener el orden y la correcta presentación del área de trabajo.",
    "Contribuir a que nuestros visitantes disfruten una experiencia divertida y agradable."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 3 meses en atención al cliente o manejo de caja.",
    "Buena comunicación, carisma y trato cordial.",
    "Capacidad para realizar distintas funciones y trabajar en equipo.",
    "Responsabilidad, puntualidad y compromiso.",
    "Disponibilidad para trabajar en Villa El Salvador."
   ],
   "beneficios": [
    "Planilla completa desde el primer día.",
    "Todos los beneficios de ley.-Requerimientos- Educación mínima: Técnico años de experienciaEdad: entre 23 y 37 añosPalabras clave: polivalente, multiusos, versatil, multifuncion, plurifuncional, multifuntion, polifacetico, care"
   ],
   "url": "https://www.laborum.pe/job/planet-game-s-a-c/multifuncional-parque-zonal-huascar-villa-el-salvador/6ab7377dba83cb6e704f9621"
  },
  {
   "id": 7,
   "puesto": "Atención al cliente",
   "empresa": "MONTALVO INSTITUTE CORP SAC",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 0,
   "vence": "2026-10-26",
   "restan": 30,
   "score": 83,
   "resumen": "RESUMEN: Montalvo Instituto, institución líder en educación en cosmetología y belleza a nivel nacional, se encuentra en la búsqueda de Asesores(as) de Ventas para nuestra sede Carabayllo.",
   "funciones": [
    "Brindar información sobre cursos y especialidades a los interesados.",
    "Asesorar a los clientes según sus necesidades.",
    "Gestión de matrículas y seguimiento a potenciales alumnos.",
    "Cumplimiento de metas comerciales.",
    "Apoyo en procesos de cobranzas y atención al cliente."
   ],
   "requisitos": [
    "Experiencia en ventas telefónicas o presenciales, atención al cliente.",
    "Disponibilidad para laborar de manera presencial en la sede de Huancayo.",
    "Buena comunicación y orientación a resultados.",
    "(1 día de descanso rotativo: sábado o domingo)",
    "Lunes a viernes: 10:00a.m. 7:00 p.m.",
    "Sábado: 9:30 a.m. 6:30 p.m.",
    "Domingo: 8:00 a.m. 3:30 p.m. (Se otorga bono Dominical S/.20)",
    "Sueldo fijo S/ 1,300 + bono por cumplimiento de ventas + incentivos",
    "A partir del 4to mes (Incremento por Permanencia a S/.1,400 fijo)",
    "Planilla completa (regimen general) desde el primer día (Asignación Familiar, CTS, Gratificación, Vacaciones, Vida Ley, etc.)",
    "Bono Dominical"
   ],
   "beneficios": [
    "Línea de Carrera",
    "Capacitaciones constantes",
    "Si buscas estabilidad laboral y desarrollo en el área comercial, ¡esta es tu oportunidad!"
   ],
   "url": "https://www.laborum.pe/job/montalvo-institute-corp-sac/asesores-as-de-ventas-sede-carabayllo-sueldo-fijo-comisiones/6ab72618ba83cb6e704f64ef"
  },
  {
   "id": 8,
   "puesto": "Asistente Administrativo de Compras",
   "empresa": "ManpowerGroup Perú",
   "cat": "Administración",
   "min": 2300,
   "max": 2300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 1,
   "vence": "2026-12-24",
   "restan": 89,
   "score": 97,
   "resumen": "En Manpower Perú buscamos al mejor talento para asumir un nuevo reto laboral como Asistente Administrativo de Compras ¡La oportunidad de tener un futuro mejor, está aquí!",
   "funciones": [
    "Gestionar la homologación y rehomologación de proveedores mediante la solicitud y revisión de documentación, la verificación de los requisitos establecidos y el seguimiento a la subsanación de observaciones.",
    "Realizar la búsqueda y apoyar el desarrollo de proveedores para mantener una base actualizada y ampliar las alternativas de abastecimiento en las distintas categorías de compras.",
    "Mantener actualizado el registro de evaluaciones, documentos pendientes y vencimientos de proveedores; reportar resultados y observaciones, y elevar las solicitudes de excepción a la jefatura para su evaluación y aprobación.",
    "Crear y actualizar códigos de materiales y servicios en SAP, validando la información recibida y verificando previamente la existencia de códigos equivalentes para evitar duplicidades.",
    "Revisar y depurar, en la etapa inicial, el maestro de materiales y servicios, identificando duplicados, descripciones inconsistentes, clasificaciones incorrectas y códigos potencialmente obsoletos; ejecutar los ajustes aprobados considerando su uso y sus dependencias en SAP.",
    "Apoyar en la definición de estándares de descripción, clasificación, unidades de medida y campos obligatorios para materiales y servicios, y aplicarlos en la creación y actualización de códigos, previa validación de la jefatura.",
    "Revisar los procesos de homologación y gestión de códigos para identificar demoras y reprocesos, proponer mejoras que agilicen la atención y apoyar su implementación bajo la supervisión de la jefatura.",
    "Elaborar y mantener actualizados los controles e indicadores de solicitudes pendientes, tiempos de atención, avance de la depuración del maestro y estado de los procesos de homologación y rehomologación.",
    "Realizar otras funciones afines al puesto que le sean asignadas por su jefe inmediato."
   ],
   "requisitos": [
    "Egresado técnico o universitario, bachiller o titulado de Administración, Negocios Internacionales, Ingeniería Industrial, Logística o carreras afines.",
    "Experiencia mínima de 1 año en funciones relacionadas con datos maestros SAP y gestión de proveedores, con participación directa en homologación",
    "Experiencia práctica en creación y mantenimiento de códigos de materiales y servicios en SAP, módulo MM (indispensable).",
    "Microsoft Office a nivel intermedio e inglés básico para lectura (deseables).",
    "Organización, atención al detalle, seguimiento oportuno, comunicación clara e iniciativa para proponer mejoras.",
    "Disponibilidad para laborar de lunes a viernes de 8:00 a. m. a 6:15 p. m. en La Molina."
   ],
   "beneficios": [
    "Pago mensual: S/2,300 en planilla del régimen general de Manpower desde el primer día.",
    "Modalidad presencial.",
    "Contrato de 3 meses (sujeto a renovación).",
    "Únete a una empresa líder a nivel global.",
    "Beneficios corporativos, como descuentos especiales y acceso a actividades.",
    "Capacitaciones constantes.",
    "En Manpower Perú promovemos la inclusión e inserción laboral sin distinción por razón de discapacidad, género, orientación, identidad y expresión de género, lugar de origen o generación, Por ello ¡Todos los talentos son bienvenidos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-administrativo-de-compras-la-molina-sector-automotriz-manpowergroup-peru-1118456091.html"
  },
  {
   "id": 9,
   "puesto": "Cocinero Cevichero– Fríos y Calientes",
   "empresa": "HVA INVERSIONES",
   "cat": "Gastronomía",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 1,
   "vence": "2026-10-25",
   "restan": 29,
   "score": 97,
   "resumen": "Preparar mise en place para platos de la carta.",
   "funciones": [
    "Preparar mise en place para platos de la carta.",
    "Elaborar platos calientes y fríos: ceviches, tiraditos, arroces, sopas, entradas, jaleas, chicharrones y más.",
    "Garantizar la producción en tiempo y forma durante el servicio.",
    "Mantener seguridad e higiene en manipulación, almacenamiento y emplatado de alimentos.",
    "Presentar los platos según los estándares del restaurante.",
    "Mantener orden y limpieza del área de trabajo.",
    "Lunes a Domingo",
    "Turno mañana: 8:00 a.m. 6:00 p.m. -Requerimientos- Educación mínima: Técnico2 años de experienciaEdad: entre 25 y 35 añosPalabras clave: chef, cocinero"
   ],
   "requisitos": [
    "Experiencia mínima de 2 años en restaurantes.",
    "Experiencia en cocina caliente y fría (ceviches, tiraditos, sopas, arroces, entradas marinas y frituras).",
    "Dominio en preparación de chicharrones y frituras (para chicharronero).",
    "Estudios técnicos truncos o completos en gastronomía o afines (deseable).",
    "Disponibilidad para laborar en Cercado de Lima (Plaza de Armas).",
    "Disponibilidad para laborar tiempo completo.",
    "Actitud proactiva, responsabilidad y trabajo en equipo."
   ],
   "beneficios": [
    "Sueldo desde S/. 2000.00 2400.00 según evaluación y experiencia",
    "Ingreso a planilla y asignación familiar.",
    "Pagos quincenales.",
    "Alimentación cubierta.",
    "Pago doble por feriados laborados.",
    "Comisiones individuales y semanales.",
    "30% de descuento en consumos (en nuestros locales).",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Bonos por metas mensuales.",
    "Un día de descanso a la semana."
   ],
   "url": "https://www.laborum.pe/job/hva-inversiones/cocinero-cevichero-frios-y-calientes-centro-de-lima-plaza-de/6ab5fe8aba83cb6e704eca9f"
  },
  {
   "id": 10,
   "puesto": "Auxiliar de Centro de Distribución",
   "empresa": "ManpowerGroup Perú",
   "cat": "Logística",
   "min": 1100,
   "max": 1100,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 1,
   "vence": "2026-12-24",
   "restan": 89,
   "score": 95,
   "resumen": "En Manpower Perú buscamos al mejor talento para asumir un nuevo reto laboral como Auxiliar de Centro de Distribución - Part time . ¡La oportunidad de tener un futuro mejor, está aquí!",
   "funciones": [
    "G estionar y atender pedidos diarios mediante SAP , asegurando su correcta facturación y despacho.",
    "Validar información de pedidos y códigos SCOP a través de la plataforma OSINERGMIN (PVO).",
    "Controlar el ingreso y salida de distribuidores y unidades de transporte, verificando documentación, implementos de seguridad y cumplimiento de requisitos normativos.",
    "Supervisar la recepción, carga, descarga e inventario de cilindros llenos y vacíos, garantizando la exactitud del stock.",
    "Emitir y registrar documentos de transporte y facturación (facturas electrónicas, guías de remisión y transferencias) en SAP.",
    "Monitorear la disponibilidad de stock y coordinar solicitudes de abastecimiento con planta.",
    "Mantener actualizados los registros operativos y reportes de control en OneDrive y otros sistemas internos.",
    "Realizar inventarios físicos y controlar la capacidad de almacenamiento del centro de distribución.",
    "Gestionar el archivo y envío de documentación operativa al área de Back Office.",
    "Coordinar la atención de auditorías e inspecciones de entidades fiscalizadoras.",
    "Controlar los materiales de oficina y el estado de los equipos de seguridad (extintores, entre otros).",
    "Velar por el cumplimiento de los procedimientos de seguridad y operaciones del Centro de Distribución."
   ],
   "requisitos": [
    "Educación Superior Técnica en las carreras de Administración o afines.",
    "Experiencia mínima de 1 año en posiciones afines.",
    "Manejo de Excel nivel intermedio .",
    "Manejo de Microsoft Office a Nivel Intermedio.",
    "Manejo de SAP y transacciones SAP .",
    "Disponibilidad de trabajar en el siguiente horario de Lunes a Sábado de 15:00 p.m. a 19:45 p.m.",
    "Disponibilidad para laborar en Villa el Salvador"
   ],
   "beneficios": [
    "Salario bruto mensual S/1100",
    "Planilla general de Manpower (Asignación familiar, gratificaciones, CTS, seguro de salud).",
    "Pagos Mensuales.",
    "Disfruta de beneficios corporativos como descuentos especiales, acceso a actividades.",
    "En Manpower Perú, promovemos la inclusión e inserción laboral sin distinción por razón de discapacidad, género, orientación, identidad y expresión de género, lugar de origen o generación. Por ello ¡Todos los talentos son bienvenidos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-de-centro-de-distribucion-medio-tiempo-villa-el-salvador-manpowergroup-peru-1118456094.html"
  },
  {
   "id": 11,
   "puesto": "Operario de Producción",
   "empresa": "Multimoldes S.A.C.",
   "cat": "Ingeniería",
   "min": 1350,
   "max": 1350,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 1,
   "vence": "2026-10-25",
   "restan": 29,
   "score": 95,
   "resumen": "RESUMEN: OPERARIO DE PRODUCCIÓN CON O SIN EXPERIENCIA",
   "funciones": [
    "Apoyar en la operación y abastecimiento de las máquinas de producción.",
    "Realizar labores de producción, revisión y acondicionamiento del producto.",
    "Mantener el orden y limpieza de su área de trabajo.",
    "Aprender progresivamente el funcionamiento de las máquinas y procesos productivos."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Con o sin experiencia como Operario de Producción en empresas industriales.",
    "Ganas de aprender y desarrollarse en el manejo de procesos y máquinas industriales.",
    "Disponibilidad para trabajar en San Juan de Miraflores.",
    "Disponibilidad para trabajar en horarios rotativos (una semana de turno día y la siguiente de noche).",
    "Disponibilidad para realizar horas extras.",
    "Responsabilidad, puntualidad y compromiso con el trabajo.",
    "Disponibilidad para ingreso inmediato."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,350",
    "S/ 100 de movilidad fija",
    "S/ 100 de bono de asistencia",
    "S/ 200 de bono de productividad",
    "Bonificación nocturna, según corresponda.",
    "Pago de horas extras, según corresponda.",
    "Ingreso a planilla con los beneficios de ley.",
    "Capacitaciones y formación técnica a través de SENATI, de acuerdo con los programas establecidos por la empresa."
   ],
   "url": "https://www.laborum.pe/job/multimoldes-s-a-c/operario-de-produccion-sin-exp-en-plantas-industriales-san/6ab5cf4aba83cb6e704e3ddd"
  },
  {
   "id": 12,
   "puesto": "Operario de Almacén",
   "empresa": "Grupo BASA-FACUSA-EISAC.",
   "cat": "Logística",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 1,
   "vence": "2026-10-25",
   "restan": 29,
   "score": 94,
   "resumen": "Verificar que la producción ingrese a los almacenes de Distribución en los masters correctos empacado y en el pallet correcto y bien ubicado.",
   "funciones": [
    "Verificar que la producción ingrese a los almacenes de Distribución en los masters correctos empacado y en el pallet correcto y bien ubicado.",
    "Participar en los inventarios y otras labores extras cuando se requiera.",
    "Despachar los productos que sea solicitada por el área.",
    "Garantizar la limpieza y el orden en el área de trabajo.",
    "Realizar cualquier otra actividad inherente al área que sea designada por su supervisor."
   ],
   "requisitos": [
    "Secundaria Completa (indispensable).",
    "No contar con antecedentes penales, judiciales o policiales (indispensable) - La Empresa realizará la respectiva verificación.",
    "Con experiencia de 01 año en puestos similares.",
    "Disponibilidad para trabajar en turnos rotativos ( lunes a Sábado)",
    "Vivir en zonas aledañas a Santa Anita, El Agustino, San Luis, Ate (deseable)."
   ],
   "beneficios": [
    "Ingreso a la planilla desde el primer día de trabajo (en la misma planilla de la empresa)",
    "Sueldo ofrecido: S/ 1,200.00 + S/100.00 bono de productividad + S/50.00 de movilidad",
    "Todos los beneficios de acuerdo a ley (Gratificación, Vacaciones, CTS, Asignación Familiar, Bonos, Utilidades y pago de horas extras)",
    "Seguro Vida Ley",
    "Patrocinio a las carreras de SENATI",
    "Convenios educativos con UPN - IDAT - ZEGEL - USIL - ESAN",
    "Descuentos en los productos de la empresa. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 21 y 40 añosPalabras clave: operator, operario, operator, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/grupo-basa-facusa-eisac/operario-de-almacen-despacho/6ab5d1bbba83cb6e704e457d"
  },
  {
   "id": 13,
   "puesto": "Operario de Producción",
   "empresa": "Wellco Peruana SA",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 1,
   "vence": "2026-10-25",
   "restan": 29,
   "score": 94,
   "resumen": "RESUMEN: ¿BUSCAS TRABAJO CON TODOS LOS BENEFICIOS DE LEY? ¡Esta es tu oportunidad!",
   "funciones": [
    "Operar máquinas siguiendo normas de calidad",
    "Cumplir metas de producción asignadas",
    "Mantener limpio tu puesto de trabajo",
    "Reportar fallas o riesgos al supervisor",
    "Cumplir normas de seguridad y reglamento interno",
    "Si te encuentras interesado, envía tu CV al WhatsApp: 946 362 912",
    "¡No dejes pasar esta oportunidad de crecer con nosotros!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 añosPalabras clave: operator, operario, operator, production, produce, fabricacion, manufactoring, elaboracion, making, manufactura, produccion"
   ],
   "requisitos": [
    "Mayor de 18 años",
    "Con o sin experiencia (¡te capacitamos!)",
    "Disponibilidad inmediata",
    "Deseable residir en Lurín o Zona Sur"
   ],
   "beneficios": [
    "HORARIO FIJO (LV 7am5:30pm)",
    "Salario base S/ 1,130 + producción al destajo ( aprox mensual S/1450 a S/1600)",
    "Pagos QUINCENALES.",
    "Ingreso a planilla desde el PRIMER DÍA",
    "CTS + Gratificaciones + Vacaciones al 100%",
    "Essalud + SCTR + Seguro Vida Ley",
    "Alimentación cubierta al 100%",
    "Pagos quincenales",
    "Movilidad de Acercamiento desde paradero Acho (Km 40)"
   ],
   "url": "https://www.laborum.pe/job/wellco-peruana-sa/operario-de-produccion-rubro-calzado-ingreso-directo-en/6ab5d227ba83cb6e704e4709"
  },
  {
   "id": 14,
   "puesto": "Operario de Producción",
   "empresa": "Grupo BASA-FACUSA-EISAC.",
   "cat": "Ingeniería",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 1,
   "vence": "2026-10-25",
   "restan": 29,
   "score": 94,
   "resumen": "Realizar la correcta inspección en la fuente del producto moldeado y control de las cantidades producidas.",
   "funciones": [
    "Realizar la correcta inspección en la fuente del producto moldeado y control de las cantidades producidas.",
    "Correcto rebabado de productos.",
    "Realizar el control de su producción, tiempo de ciclo y pesaje de producto.",
    "Realizar limpieza de tolva y lavado de cámara en cambios de color y cambio de molde.",
    "Garantizar la limpieza y el orden en el área de trabajo.",
    "Realizar cualquier otra actividad inherente al área que sea designada por su supervisor."
   ],
   "requisitos": [
    "Secundaria Completa (indispensable).",
    "No contar con antecedentes penales, judiciales o policiales (indispensable) - La Empresa realizará la respectiva verificación.",
    "Con experiencia de 01 año en puestos similares.",
    "Disponibilidad para trabajar en turnos rotativos ( lunes a Sábado)",
    "Vivir en zonas aledañas a Santa Anita, El Agustino, San Luis, Ate (deseable)."
   ],
   "beneficios": [
    "Ingreso a la planilla desde el primer día de trabajo (en la misma planilla de la empresa)",
    "Sueldo ofrecido: S/ 1,200.00 + S/100.00 bono de productividad + S/50.00 movilidad.",
    "Todos los beneficios de acuerdo a ley (Gratificación, Vacaciones, CTS, Asignación Familiar, Bonos, Utilidades y pago de horas extras)",
    "Seguro Vida Ley",
    "Patrocinio a las carreras de SENATI",
    "Convenios educativos con UPN - IDAT - ZEGEL - USIL - ESAN",
    "Descuentos en los productos de la empresa. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 21 y 42 añosPalabras clave: operator, operario, operator, production, produce, fabricacion, manufactoring, elaboracion, making, manufactura, produccion"
   ],
   "url": "https://www.laborum.pe/job/grupo-basa-facusa-eisac/operario-de-produccion/6ab5d1caba83cb6e704e45a1"
  },
  {
   "id": 15,
   "puesto": "Operario de limpieza",
   "empresa": "PLANET GAME S.A.C.",
   "cat": "Recursos Humanos",
   "min": 1150,
   "max": 1150,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 1,
   "vence": "2026-10-25",
   "restan": 29,
   "score": 93,
   "resumen": "RESUMEN: ¡ÚNETE AL EQUIPO DE PLANET GAME S.A.C.! Huaral",
   "funciones": [
    "Brindar una atención cordial y eficiente a nuestros clientes",
    "Mantener el orden y la correcta presentación de la sala",
    "Cumplir los procedimientos establecidos para cada puesto",
    "¡Postula y forma parte de la familia Planet Game!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 44 añosPalabras clave: operator, operario, operator, aseo, higiene, cleaning, limpieza, ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide"
   ],
   "requisitos": [
    "Secundaria completa",
    "Experiencia mínima de 3 meses en atención al cliente o puestos similares",
    "Buena actitud, responsabilidad y vocación de servicio",
    "Disponibilidad para trabajar en horarios rotativos: mañana, tarde, noche y amanecida",
    "Disponibilidad para laborar en nuestras salas de tragamonedas"
   ],
   "beneficios": [
    "Sueldo mensual de S/ 1,150",
    "Bono mensual de S/ 150",
    "Jornada laboral de 8 horas",
    "Ingreso a planilla completa desde el primer día",
    "Todos los beneficios de ley",
    "Oportunidad de crecimiento dentro de la empresa"
   ],
   "url": "https://www.laborum.pe/job/planet-game-s-a-c/operario-de-limpieza-huaral-salas-tragamonedas-auxiliar-de/6ab5e526ba83cb6e704e82ff"
  },
  {
   "id": 16,
   "puesto": "Técnico mecánico de mantenimiento",
   "empresa": "Multimoldes S.A.C.",
   "cat": "Ingeniería",
   "min": 1900,
   "max": 1900,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 1,
   "vence": "2026-10-25",
   "restan": 29,
   "score": 92,
   "resumen": "MULTIMOLDES SAC, empresa líder con más de 25 años en el mercado, dedicada a la producción y comercialización de envases de papel y artículos escolares, ubicada en San Juan de Miraflores, se encuentra en búsqueda de un:",
   "funciones": [
    "Supervisar el funcionamiento continuo y correcto de las máquinas de planta.",
    "Realizar mantenimiento correctivo cuando se requiera.",
    "Cambio de rodamientos, retenedores, ejes, piñones y empaques, así como lubricación de transmisiones.",
    "Soporte en mantenimiento de infraestructura (iluminación, tableros eléctricos, etc.)."
   ],
   "requisitos": [
    "Técnico Mecánico de Mantenimiento industrial egresado con mínimo 1 año y medio de experiencia (incluidas prácticas preprofesionales).",
    "Disponibilidad para trabajar en turnos rotativos.",
    "Residir en Lima Sur: Chorrillos, San Juan de Miraflores, Villa El Salvador o zonas aledañas."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,900 + pago de horas extras.",
    "Movilidad: S/ 200.00.",
    "Bono de asistencia: S/ 100.00",
    "Bono de productividad: S/ 200.00",
    "Ingreso a planilla con todos los beneficios de ley desde el primer día (CTS, gratificaciones, ESSALUD, etc.)."
   ],
   "url": "https://www.laborum.pe/job/multimoldes-s-a-c/tecnico-mecanico-de-mantenimiento-exp-en-plantas/6ab5cfbcba83cb6e704e3f4e"
  },
  {
   "id": 17,
   "puesto": "Asesor",
   "empresa": "Integra Retail",
   "cat": "Ventas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 1,
   "vence": "2026-10-25",
   "restan": 29,
   "score": 92,
   "resumen": "RESUMEN: ¡ÚNETE A GRUPO CARSA COMO ASESOR(A) DE VENTAS!",
   "funciones": [
    "Asesorar y recomendar nuestros productos, brindando una excelente experiencia de compra.",
    "Brindar orientación sobre garantías, reparaciones y entrega de productos.",
    "Realizar actividades de campo para la captación de nuevos clientes, generación de referidos y ampliación de cartera."
   ],
   "requisitos": [
    "Experiencia mínima de 3 meses en ventas (deseable experiencia en electrodomésticos, productos financieros o rubros similares).",
    "Experiencia en atención al cliente y cumplimiento de objetivos comerciales.",
    "Experiencia en trabajo de campo o captación de clientes (deseable, no indispensable).",
    "Personas dinámicas, proactivas y con alta orientación a resultados.",
    "Excelente comunicación y capacidad para generar relaciones con clientes.",
    "Disponibilidad para trabajar en horario comercial Full Time."
   ],
   "beneficios": [
    "Sueldo básico de S/ 1,200.",
    "Comisiones ilimitadas.",
    "Premios y bonos por cumplimiento de objetivos.",
    "Ingreso directo a planilla desde el primer día.",
    "Todos los beneficios de ley.",
    "Pago mensual de comisiones.",
    "Capacitaciones constantes.",
    "Excelente clima laboral.",
    "Oportunidades reales de crecimiento y línea de carrera dentro del grupo.",
    "¡Esta es tu oportunidad para crecer profesionalmente!",
    "Si te apasionan las ventas, disfrutas cumplir metas y quieres generar ingresos sin límites, ¡queremos conocerte!",
    "¡Postula ahora y forma parte de Grupo CARSA!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: labour, gremial, sindical, laboral, cambaceo, sales"
   ],
   "url": "https://www.laborum.pe/job/integra-retail/todo-huaral-oportunidad-laboral-buscamos-asesores-de-ventas/6ab5e701ba83cb6e704e89d0"
  },
  {
   "id": 18,
   "puesto": "Administrador",
   "empresa": "Confidencial",
   "cat": "Administración",
   "min": 3800,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 1,
   "vence": "2027-01-23",
   "restan": 119,
   "score": 90,
   "resumen": "Empresa dedicada al procesamiento de minerales ubicada en Chala nos encontramos en la búsqueda de 01 Administrador para integrarse a nuestro Equipo - Régimen 20x10",
   "funciones": [
    "Supervisar la gestión administrativa.",
    "Diseñar e implementar políticas y procedimientos internos de las áreas que se encuentran bajo su responsabilidad, así como de controles internos financieros de la organización.",
    "Coordinar el abastecimiento de recursos e insumos.",
    "Supervisar las condiciones de bienestar del personal.",
    "Administrar bienes, instalaciones y servicios necesarios para la Operación.",
    "Supervisar los procesos de compras de abastecimiento, aprueba requerimientos de bienes y servicios.",
    "Otras funciones relacionadas con el área Administrativa."
   ],
   "requisitos": [
    "Titulado en la carrera de Administración, Contabilidad, carreras afines",
    "Experiencia mínima de 4 años (indispensable), deseable en Plantas.",
    "Capacidad analítica, organización y liderazgo.",
    "Residir en Lima o Arequipa."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día de trabajo, con todos los beneficios de ley.",
    "Alojamiento, alimentación y movilidad cubiertos."
   ],
   "url": "https://www.bumeran.com.pe/empleos/administrador-1118456081.html"
  },
  {
   "id": 19,
   "puesto": "Representante de Servicios B2B Ingresos Hasta S",
   "empresa": "Confidencial",
   "cat": "Tecnología",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Arequipa",
   "fuente": "Bumeran",
   "dias": 2,
   "vence": "2027-01-22",
   "restan": 118,
   "score": 95,
   "resumen": "Prospectar y captar nuevas empresas dentro de la zona asignada.",
   "funciones": [
    "Prospectar y captar nuevas empresas dentro de la zona asignada.",
    "Contactar clientes potenciales mediante llamadas, correos y visitas comerciales.",
    "Realizar visitas agendadas y visitas en frío para presentar los servicios de la empresa.",
    "Coordinar y ejecutar demostraciones gratuitas del servicio para potenciales clientes.",
    "Elaborar propuestas comerciales y negociar condiciones con clientes.",
    "Realizar cierres de venta y cumplir con los objetivos comerciales establecidos.",
    "Gestionar y fidelizar clientes, generando oportunidades de venta adicional (cross selling y upselling).",
    "Registrar y reportar la gestión comercial en el sistema correspondiente."
   ],
   "requisitos": [
    "Experiencia mínima de 2 años en ventas comerciales o ventas B2B.",
    "Estudios técnicos o universitarios en Administración, Ingeniería Comercial, Marketing, Ingeniería Industrial o carreras afines (completo o incompleto).",
    "Experiencia en prospección de clientes, ventas en frío y cierre de negocios.",
    "Conocimientos en técnicas de ventas, negociación y gestión comercial.",
    "Manejo de Microsoft Office a nivel intermedio.",
    "Perfil orientado a resultados, con habilidades de comunicación, negociación y cierre comercial.",
    "Disponibilidad para viajes ocasionales dentro de la zona sur del país."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,500.00",
    "Bono de movilidad: S/ 200.00",
    "Bono quincenal: S/ 300.00",
    "Bono por sobreproducción: S/ 200.00",
    "Comisiones promedio: S/ 1,950.00",
    "Ingreso promedio mensual estimado: S/ 4,150.00",
    "Ingreso a planilla desde el primer día bajo régimen REMYPE, con beneficios de ley.",
    "Herramientas de trabajo proporcionadas por la empresa",
    "Camisas corporativas",
    "Tarjetas de presentación",
    "Celular corporativo",
    "Viáticos cubiertos para viajes comerciales."
   ],
   "url": "https://www.bumeran.com.pe/empleos/representante-de-servicios-b2b-ingresos-hasta-s-4000-1118456048.html"
  },
  {
   "id": 20,
   "puesto": "Vendedora El Polo",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ventas",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 2,
   "vence": "2026-12-23",
   "restan": 88,
   "score": 94,
   "resumen": "Responsable de brindar una adecuada atención al cliente de acuerdo a los estándares de la compañía.",
   "funciones": [
    "Responsable de brindar una adecuada atención al cliente de acuerdo a los estándares de la compañía.",
    "Alcanzar los objetivos de ventas establecidos por la empresa.",
    "Apoyar con los stocks y hacer inventarios.",
    "Encargado de la exhibición y orden de productos en el lugar de trabajo.",
    "Apoyo en preparación de pedidos web."
   ],
   "requisitos": [
    "Solo mujeres entre 22 y 40 años de cualquier nacionalidad.",
    "Contar con una experiencia mínima de 1 año en venta retail de importantes marcas ubicadas en Centros Comerciales o boutique puerta a calle.",
    "Disponibilidad para trabajar en horarios retail, 48 horas a la semana, se trabajan 6 días y se descana 1. Los descansos son los martes. El horario de trabajo es de 12 pm a 9 pm con 1 hora de refrigerio (solo los domingos son de 11 am a 7 pm).",
    "Excelente trato, servicio al cliente y muy buena actitud."
   ],
   "beneficios": [
    "Grato ambiente laboral y posibilidad de hacer línea de carrera.",
    "Los 3 primeros meses mediante recibos de honorarios y desde el 4to mes en planilla pyme (1/2 gratificación en julio + 1/2 gratificación en diciembre + 1/2 cts anual + 15 días de vacaciones al año + Essalud).",
    "Sueldo básico (S/.1400) + comisiones individuales de S/.400 mensuales.",
    "Descuentos en productos de la marca."
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedora-el-polo-adn-retail-s.a.c.-1118455960.html"
  },
  {
   "id": 21,
   "puesto": "Asistente Contable",
   "empresa": "HVA INVERSIONES",
   "cat": "Contabilidad",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 2,
   "vence": "2026-10-24",
   "restan": 28,
   "score": 94,
   "resumen": "Registrar y revisar comprobantes de compras y ventas.",
   "funciones": [
    "Registrar y revisar comprobantes de compras y ventas.",
    "Apoyar en el registro de asientos y operaciones contables.",
    "Realizar conciliaciones bancarias y seguimiento de diferencias.",
    "Revisar y organizar la documentación sustentatoria de las operaciones.",
    "Apoyar en la revisión y control de cajas, ingresos, egresos y rendiciones de nuestros locales.",
    "Realizar seguimiento de detracciones, retenciones y percepciones.",
    "Apoyar en la revisión de información registrada en SIRE.",
    "Elaborar reportes y brindar soporte en el análisis de información contable.",
    "Apoyar en los procesos de cierre contable mensual.",
    "Mantener organizado el archivo físico y digital del área.",
    "Identificar inconsistencias en la información y comunicarlas oportunamente.",
    "Otras funciones relacionadas al área contable."
   ],
   "requisitos": [
    "Técnico, egresado de Contabilidad.",
    "Experiencia mínima de 1 año como Asistente o Auxiliar Contable.",
    "Experiencia en registro y revisión de compras y ventas.",
    "Análisis de estados financieros",
    "Elaboración de Estado de Ganancias y Pérdidas",
    "Declaración de planilla en el Plame",
    "Realizar las altas y bajas del personal",
    "Conocimiento de conciliaciones bancarias.",
    "Conocimiento de detracciones, retenciones y percepciones.",
    "Manejo de SUNAT y SIRE.",
    "Manejo de Excel a nivel intermedio.",
    "Deseable experiencia en empresas del rubro gastronómico, retail o consumo masivo."
   ],
   "beneficios": [
    "Sueldo desde S/. 1,400.00 1,800.00 según evaluación y experiencia.",
    "Ingreso a planilla y asignación familiar.",
    "Pagos quincenales.",
    "Alimentación cubierta.",
    "30% de descuento en consumos (en nuestros locales).",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Un día de descanso a la semana."
   ],
   "url": "https://www.laborum.pe/job/hva-inversiones/asistente-contable-cercado-de-lima/6ab4994cba83cb6e704d71d3"
  },
  {
   "id": 22,
   "puesto": "Jefe Dpto Administración de Personal",
   "empresa": "Servicios Postales del Perú S.A.",
   "cat": "Ingeniería",
   "min": 3655,
   "max": 3655,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 2,
   "vence": "2026-12-23",
   "restan": 88,
   "score": 92,
   "resumen": "SERPOST S.A es una empresa pública de derecho privado del sector transporte y comunicaciones y tiene por actividad principal la prestación de servicios postales con cobertura local, nacional e internacional.",
   "funciones": [
    "A) Planear, organizar y dirigir los procesos de reclutamiento, selección, contratación, inducción y desarrollo de personal, teniendo en cuenta las normas emitidas al respecto.",
    "B) Elaborar, coordinar y ejecutar evaluaciones de desempeño para el personal de la Empresa.",
    "C) Planear, elaborar y dar cumplimiento a las líneas de carrera y planes de sucesión de la Empresa.",
    "D) Asesorar, coordinar y atender los requerimientos laborales vinculados a derechos y obligaciones de los colaboradores de la Empresa, de acuerdo con las disposiciones laborales vigentes.",
    "E) Representar a la Empresa, por delegación del Gerente General, ante la autoridad administrativa de trabajo y/o judiciales.",
    "F) Dirigir y controlar los procesos de pago de las planillas y sus derivados.",
    "G) Supervisar que los movimientos de personal referentes a incorporación, rotación, traslados y ceses se realicen de acuerdo con los dispositivos legales vigentes y normatividad emitida al respecto.",
    "H) Supervisar el estricto cumplimiento del Rol Anual de Vacaciones del personal de la Empresa.",
    "I) Supervisar que los legajos de personal se encuentren debidamente actualizados.",
    "J) Administrar el Cuadro de Asignación de Personal - CAP.",
    "K) Proponer actualizaciones de las directivas internas referentes a recursos humanos.",
    "L) Mantener información actualizada del sistema de remuneraciones."
   ],
   "requisitos": [
    "Educación Formal",
    "Bachiller en Psicología, Gestión de Recursos Humanos, Ingeniería Industrial, Contabilidad, Administración u otras carreras afines a las funciones del puesto. (Indispensable)",
    "Maestría en Recursos Humanos, Desarrollo del Talento, Contabilidad o relacionado al puesto. (Deseable)",
    "Título universitario, Especializaciones en Legislación Laboral, SST, Gestión del Desempeño, Recursos Humanos u otras especializaciones afines a las funciones del puesto. (Deseable)",
    "Cursos relacionados al puesto o al giro de la empresa. (Deseable)",
    "Los grados académicos serán validados en las plataformas digitales de la Superintendencia Nacional de Educación Superior Universitaria (SUNEDU) y el Ministerio de Educación (MINEDU), en cuanto corresponda.",
    "Se entiende por \"puestos equivalentes”, a aquellos puestos que tienen asignadas funciones iguales o similares y que para su desempeño se exijan requisitos de estudio, experiencia y competencias laborales iguales o similares, sin embargo; tienen nomenclaturas distintas.",
    "La experiencia indispensable señalada como requisito en los perfiles descritos deberá corresponder a funciones directamente asociadas al área funcional donde se ejerce el cargo.",
    "5 años mínimo de experiencia laboral en general, 3 años mínimo en puestos equivalentes (3) o 4 años como coordinador, administrador o supervisor en empresas públicas o privadas.",
    "Función General",
    "Ejecutar y supervisar las acciones relacionadas a la gestión de los recursos humanos."
   ],
   "beneficios": [
    "Planilla completa con TODOS los BENEFICIOS de LEY desde el 1er día (Gratificación, CTS, vacaciones, seguro vida ley, essalud, entre otros)",
    "Horario fijo (Mañana de L a V 8am a 5:35pm)",
    "Sueldo: S/3,655.00",
    "IMPORTANTE ADJUNTAR CV DOCUMENTADO"
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-dpto-administracion-de-personal-servicios-postales-del-peru-s.a.-1118455947.html"
  },
  {
   "id": 23,
   "puesto": "Mozo de Salón",
   "empresa": "KINGDOM  HOTEL",
   "cat": "Gastronomía",
   "min": 1130,
   "max": 1135,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 2,
   "vence": "2026-11-23",
   "restan": 58,
   "score": 92,
   "resumen": "Brindar una atención excepcional al cliente en las áreas de restaurante, buffet y eventos.",
   "funciones": [
    "Brindar una atención excepcional al cliente en las áreas de restaurante, buffet y eventos.",
    "Mantener la limpieza y el orden del área de trabajo asignada.",
    "Asegurar la disponibilidad de insumos necesarios para la preparación de bebidas.",
    "Realizar el montaje y desmontaje de mesas según los estándares establecidos.",
    "Preparar y organizar la cubertería y la vajilla.",
    "Gestionar el sistema de punto de venta (POS) para el registro de pedidos y cobros."
   ],
   "requisitos": [
    "Experiencia previa de al menos 1 año en puestos similares.",
    "Capacidad demostrada para trabajar eficazmente en equipo.",
    "Disponibilidad inmediata para cubrir turnos rotativos (mañana y tarde).",
    "Actitud responsable, puntual y con gran atención a la pulcritud y presentación personal."
   ],
   "beneficios": [
    "Alimentación incluida.",
    "Oportunidades de desarrollo profesional y línea de carrera.",
    "Acceso a capacitaciones y charlas de formación continua.",
    "Todos los beneficios laborales correspondientes por ley."
   ],
   "url": "https://www.bumeran.com.pe/empleos/mozo-de-salon-kingdom-hotel-1118455961.html"
  },
  {
   "id": 24,
   "puesto": "Marketing Digital: Especialista en Automatización, Funnels de Venta y Lead Gen",
   "empresa": "Confidencial",
   "cat": "Ventas",
   "min": 2000,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 2,
   "vence": "2026-12-23",
   "restan": 88,
   "score": 90,
   "resumen": "Construir y optimizar funnels automatizados de punta a punta.",
   "funciones": [
    "Construir y optimizar funnels automatizados de punta a punta.",
    "Levantar la información del negocio y del proceso de ventas de cada cliente.",
    "Crear plantillas de email, formularios, secuencias y workflows de automatización.",
    "Redactar copys para páginas web y contenido de campañas.",
    "Optimizar la conversión de los sitios web (botones, estructura, llamados a la acción).",
    "Configurar y entrenar agentes conversacionales de IA (base de conocimiento, prompts, calendarios, plantillas de WhatsApp y email).",
    "Coordinar el avance de varios proyectos a la vez y mantener contacto con el cliente informando avances y entregas.",
    "Monitorear campañas (KPIs, leads, costo por lead, interacción), reportar resultados y tomar acciones para mejorar el desempeño."
   ],
   "requisitos": [
    "Buscamos una persona marketera y ejecutora, experta en la construcción de funnels automatizados. Alguien dinámico, multitasking, con criterio propio e inteligente, que disfrute moverse por todo el mundo del marketing digital y que aprenda rápido.",
    "Este NO es un puesto para quien busca una sola función, cumplir un horario y hacer lo mismo todos los días. Es para alguien a quien le gusta construir: tomar un proyecto, coordinarlo en lo esencial y remangarse para ejecutarlo de principio a fin.",
    "Formación en marketing (carrera completa o últimos ciclos).",
    "Experiencia comprobable en construcción de funnels, automatizaciones y email marketing (secuencias, plantillas, formularios, workflows).",
    "Experiencia coordinando proyectos.",
    "Criterio analítico para leer campañas y tomar decisiones basadas en datos.",
    "Inglés avanzado.",
    "Idealmente, mínimo 2 años de experiencia en funciones similares.",
    "Valoramos mucho (deseables)",
    "Experiencia previa en GoHighLevel.",
    "Edición de videos.",
    "Nociones de configuración de agentes conversacionales o IA aplicada al marketing."
   ],
   "beneficios": [
    "Modalidad remota (con posibilidad de volverse híbrida más adelante).",
    "Abierto a postulantes de todo Latinoamérica.",
    "Contamos con una plataforma interna de capacitación (TH Academy) con procesos y procedimientos estandarizados para que domines nuestras herramientas rápidamente.",
    "¿Te interesa?",
    "Si eres una persona que construye, que aprende rápido y que quiere crecer en el mundo del marketing digital y la automatización, queremos conocerte. Postula ahora."
   ],
   "url": "https://www.bumeran.com.pe/empleos/especialista-en-automatizacion-y-funnels-de-marketing-1118454354.html"
  },
  {
   "id": 25,
   "puesto": "Azafata",
   "empresa": "GLADCON",
   "cat": "Gastronomía",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 2,
   "vence": "2026-10-24",
   "restan": 28,
   "score": 89,
   "resumen": "RESUMEN: En Gladcon Group, somos un grupo corporativo peruano líder en el sector de juego y entretenimiento, con más de 30 años de experiencia y más de 24 salas tragamonedas y casinos distribuidos en más de 11 provincias del país.",
   "funciones": [
    "Recibir y dar la bienvenida a los clientes del casino, gestionar reservas y asignar las mesas de manera ordenada, para que el servicio sea lo esperado.",
    "Tomar pedidos con precisión, conocer a detalle los platos que se ofrecen en el restaurante para brindar la información correcta a los clientes.",
    "Mantener comunicación constante con el personal de cocina y supervisor (a) de restaurante para agilizar el flujo de atención.",
    "Controlar el uso adecuado del menaje, mobiliario, equipos y materiales del restaurante para preservar los recursos de la empresa.",
    "Escuchar y atender cada consulta o sugerencia de los clientes para darle una solución rápida y cuidar su experiencia en el Restaurante.",
    "Apoyo en el control de inventarios para evitar vencimientos, aglomeración de stock y desabastecimiento de los mismos.",
    "Preparar y abastecer la barra de todos los insumos que necesitara para cubrir la demanda del turno.",
    "Además, te encargarás de mantener el área limpia y ordenada, garantizando un ambiente seguro y agradable."
   ],
   "requisitos": [
    "Atención al Invitado: que brindes un servicio excepcional y personalizado",
    "Experiencia en puestos afines",
    "Orientación al Invitado: tu proactividad para anticipar y satisfacer las necesidades de nuestros invitados.",
    "Trabajo en Equipo: tu habilidad para colaborar y contribuir al éxito colectivo.",
    "Disponibilidad horario: De lunes a Sabado, 8 horas diarias (10 am - 6:45 pm)",
    "Disponiblidad de trabajar en Santiago de Surco, Casino Mambos. (referencia. Univ Ricardo Palma)"
   ],
   "beneficios": [
    "Salario S/1130",
    "Planes de crecimiento profesional y capacitación continua.",
    "Un ambiente de trabajo colaborativo donde tus ideas son valoradas.",
    "Un contrato en planilla desde el primer día con todos los beneficios de ley (CTS, gratificaciones, asignación familiar, Essalud, Vacaciones).",
    "Ser parte de nuestras olimpiadas, ferias de salud, fiestas de fin de año, entre otras actividades. Únete a nosotros y sé parte de una experiencia única en el mundo del entretenimiento.",
    "Si te interesa esta oportunidad, envía tu solicitud en esta publicación y nos comunicaremos contigo ¡No esperes más para unirte a nuestro equipo!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: restaurador"
   ],
   "url": "https://www.laborum.pe/job/gladcon/se-solicita-azafata-mozo-para-restaurante-en-casino/6ab47bfcba83cb6e704d2236"
  },
  {
   "id": 26,
   "puesto": "Supervisor",
   "empresa": "Servicios Postales del Perú S.A.",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Huancayo",
   "fuente": "Bumeran",
   "dias": 2,
   "vence": "2027-01-22",
   "restan": 118,
   "score": 86,
   "resumen": "SERPOST S.A es una empresa pública de derecho privado del sector transporte y comunicaciones y tiene por actividad principal la prestación de servicios postales con cobertura local, nacional e internacional.",
   "funciones": [
    "Recibir, verificar y clasificar los despachos postales asegurando que se encuentren en buen estado y correspondan a la zona y/o grupo sectorial designado, o de apoyo, para entrega a domicilio, cumpliendo con la distribución eficaz de los envíos al destinatario a través de la firma de cargos de entrega.",
    "Cumplir con el registro diario de envíos, indicando aquellos envíos no entregados y el motivo de la no entrega, así como realizar la devolución a su supervisor y/o a la persona designada, dando cuenta de las ocurrencias durante su jornada operativa.",
    "Verificar y controlar que los cargos recibidos cumplan con las condiciones de entrega solicitados por cada cliente y devolver a las oficinas de origen dentro del tiempo establecido.",
    "Prever la seguridad de la correspondencia, equipos, herramientas de trabajo y salvaguardar la buena imagen de la Empresa.",
    "Realizar la liquidación diaria de estampillas y control diario del estado de las máquinas franqueadoras y entregar al cajero el dinero recaudado de las ventas diarias.",
    "Recabar la firma de los cargos y realizar la descarga de la entrega de los envíos registrados o comunes en los sistemas SOP o CEDSUM, según corresponda.",
    "Recibir los despachos del Centro de Clasificación Postal de Lima, así como de las oficinas, representantes comerciales y de las empresas de transportes (aéreas y terrestres) verificando que se encuentren en buen estado e informando a su superior en caso contrario.",
    "Controlar mediante el cuaderno de tarja la salida, llegada, hora, peso y firma de los diferentes despachos en el área de su competencia; llevar un control estadístico del movimiento postal ejecutado en su área, asimismo llevar la estadística de envíos con portes insuficientes, exceso de portes y sin franquear.",
    "Apoyar por necesidad de la Gerencia de Administración de Canales, a las diferentes administraciones y/o departamentos.",
    "Cumplir de manera eficiente y eficaz con las funciones designadas a su cargo, así como efectuar las actividades relacionadas con el Sistema de Control Interno.",
    "Otras funciones que en materia de su competencia le sean asignadas, ejecutando las diferentes operaciones postales de acuerdo a las exigencias y necesidades de la Empresa."
   ],
   "requisitos": [
    "Educación Formal",
    "Secundaria completa (Indispensable).",
    "Estudio Técnico en Producción y Gestión Industrial relacionada al puesto o capacitación en almacén, producción u otras carreras afines a las funciones del puesto (Deseable).",
    "Cursos relacionados al puesto o al giro de la empresa (Deseable).",
    "No requiere experiencia previa.",
    "6 meses mínimo en puestos similares (Deseable).",
    "Función General",
    "Realizar las acciones relacionadas con los diversos procesos postales y/o aduaneros de atención, asignados a su cargo en las Administraciones Postales de acuerdo a las políticas de distribución en la Empresa."
   ],
   "beneficios": [
    "Planilla completa con TODOS los BENEFICIOS de LEY desde el 1er día (Gratificación, CTS, vacaciones, seguro vida ley, essalud, entre otros)",
    "Horario fijo (Mañana de L a S)",
    "Sueldo: S/1,130.00",
    "IMPORTANTE ADJUNTAR CV DOCUMENTADO"
   ],
   "url": "https://www.bumeran.com.pe/empleos/oficial-de-servicios-postales-administraciones-la-merced-servicios-postales-del-peru-s.a.-1118455954.html"
  },
  {
   "id": 27,
   "puesto": "Ayudante de Producción Turno Noche en Lurin",
   "empresa": "Inversiones San Gabriel S.A.",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 2,
   "vence": "2026-10-24",
   "restan": 28,
   "score": 85,
   "resumen": "Somos una empresa líder en el sector industrial, con más de 38 años fabricando envases y productos plásticos para alimentos.",
   "funciones": [
    "Apoyo en línea de producción.",
    "Abastecimiento de materiales.",
    "Empaque y acondicionado de productos.",
    "Mantener el orden y limpieza del área.",
    "Otras funciones asignadas por el supervisor."
   ],
   "requisitos": [
    "Ser mayor de 18 años.",
    "Contar con secundaria completa.",
    "Con o sin experiencia.",
    "Disponibilidad para trabajar en horario nocturno.",
    "Buena actitud y disposición para el trabajo en planta.",
    "Residir en zonas cercanas a Lurín.",
    "Horario fijo turno noche",
    "10:15 p.m. a 07:00 a.m.",
    "Turno noche ideal para quienes desean generar mayores ingresos realizando horas extras",
    "Rango de horario referencial: (07:00 p.m. a 07:00 a.m.)",
    "Las horas extras son opcionales."
   ],
   "beneficios": [
    "Sueldo mensual: S/ 1,130.",
    "Pago adicional por sobretasa nocturna.",
    "Pago de horas extras, según corresponda.",
    "Bono de alimentación de hasta S/ 226 mensuales mediante tarjeta Provis.",
    "Pagos semanales.",
    "Ingreso a planilla completa desde el primer día.",
    "Beneficios de ley: CTS, gratificaciones, vacaciones, asignación familiar, utilidades, entre otros.",
    "Lugar de trabajo",
    "Urb. Industrial Las Praderas de Lurín Lurín."
   ],
   "url": "https://www.laborum.pe/job/inversiones-san-gabriel-s-a/ayudante-de-produccion-turno-noche-en-lurin/6ab47926ba83cb6e704d1a00"
  },
  {
   "id": 28,
   "puesto": "Ejecutivo de Trade Marketing",
   "empresa": "Grupo Lucky",
   "cat": "Marketing",
   "min": 3200,
   "max": 3600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 3,
   "vence": "2026-12-22",
   "restan": 87,
   "score": 95,
   "resumen": "Mantener comunicación fluida y efectiva con el contacto definido por la cuenta para las coordinaciones de lineamiento, actualización de campañas, control y evaluación de indicadores de gestión.",
   "funciones": [
    "Mantener comunicación fluida y efectiva con el contacto definido por la cuenta para las coordinaciones de lineamiento, actualización de campañas, control y evaluación de indicadores de gestión.",
    "Proponer acciones que mejoren el trabajo de los equipos a cargo en este caso estructurales, eficiencias, productividad y efectividad al objetivo.",
    "Evaluación ejecutiva semanal del alcance del cumplimiento de objetivos y sobre ello, las mejoras respectivas para superarlas.",
    "Definición y evaluación de los indicadores críticos de cada acción, los cuales tengan implicancia directa sobre el resultado del servicio.",
    "Asegurar el cumplimiento al 100% de los lineamientos establecidos por el cliente en el PDV, predefinidos con el cliente.",
    "Evaluar el desempeño a través de los kpis del personal a su cargo (personal de campaña).",
    "Gestión administrativa y logística.",
    "Gestión de inventario del stock que se maneja para la cuenta a su cargo (kardex).",
    "Controlar y programar los despachos o atención de suministros cotizados al cliente.",
    "Cotizar a través del área de compras y su coordinación logística para su distribución.",
    "Evaluar y seleccionar al personal reclutado por R&S de cada equipo de trabajo de acuerdo con el perfil del cliente y asegurar que dicho equipo este entrenado (tercerización, producto, punto de venta y técnica de venta).",
    "Hacer seguimiento a los equipos para la entrega de los reportes de manera eficiente para la validación de gerencia."
   ],
   "requisitos": [
    "Bachiller o titulado(a) en Administración, Marketing, Ingeniería Industrial, Negocios o carreras afines.",
    "Experiencia mínima de 2 años en Trade Marketing, gestión comercial o gestión de cuentas del canal tradiconal, de preferencia en empresas de consumo masivo.",
    "Experiencia coordinando equipos de campo, como supervisores y/o mercaderistas.",
    "Manejo de indicadores de ejecución en punto de venta (SOS, OSA, Price Index, promociones y exhibiciones).",
    "Excel a nivel intermedio.",
    "Alta capacidad analítica, organización, orientación a resultados y seguimiento.",
    "Deseable experiencia participando en la implementación de proyectos o herramientas tecnológicas."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Oportunidades de aprendizaje y crecimiento.",
    "Un ambiente cercano, colaborativo y con buen humor.",
    "Reconocimientos y espacios para celebrar juntos nuestros logros."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-de-trade-marketing-temporal-grupo-lucky-1118454165.html"
  },
  {
   "id": 29,
   "puesto": "Chofer de Reparto",
   "empresa": "Grupo Marathon",
   "cat": "Logística",
   "min": 1785,
   "max": 1785,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 3,
   "vence": "2026-10-23",
   "restan": 27,
   "score": 94,
   "resumen": "RESUMEN: En Marathon, estamos en búsqueda del mejor talento para sumarse al equipo del Centro de Distribución en Huachipa.",
   "funciones": [
    "Verificar documentos (guía de remisión, facturación, boletas) y cargar la mercadería según la orden de pedido.",
    "Distribuir la mercadería entre tiendas y delivery según indicaciones del jefe, entregando al cliente y confirmando la conformidad.",
    "Mantenerse informado sobre el estado de las rutas, desvíos u otras eventualidades.",
    "Reportar condiciones inseguras y cumplir con el uso de EPPs y normas de seguridad y salud en el trabajo.",
    "Realizar tareas adicionales asignadas por el jefe inmediato."
   ],
   "requisitos": [
    "Secundaria completa mínimo.",
    "Licencia de conducir vigente AIIB.",
    "Record de conductor óptimo.",
    "Experiencia mínima de un año como chofer de reparto, de preferencia en entrega de delivery.",
    "Disponibilidad para trabajar en PLANTA HUACHIPA. Ref paradero Mapfre de la avenida Prialé.",
    "Turno Día: Lunes a sábado de 7 am - 4 pm."
   ],
   "beneficios": [
    "Salario Fijo S/. 1785.00 + Vale de Alimentos S/. 200 (en tarjeta de alimentos).",
    "Planilla COMPLETA, todos los beneficios al 100% (EsSalud, gratificaciones, CTS, asignación familiar, utilidades, vacaciones, etc.).",
    "Concursos y premiaciones internas.",
    "Descuentos exclusivos en nuestras tiendas a nivel nacional.",
    "Convenios, descuentos educativos y corporativos.",
    "Puntualidad en los pagos.",
    "Posibilidad de hacer línea de carrera.-Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: A partir de 18 añosConocimientos: Facturación, OrganizaciónLicencias de conducir: A-II-B Palabras clave: conductor, driver, choferes, chofer, delivery"
   ],
   "url": "https://www.laborum.pe/job/grupo-marathon/chofer-de-reparto-turno-dia-huachipa-s-1-785-s-200-tarjeta/6ab32b9dba83cb6e704baf19"
  },
  {
   "id": 30,
   "puesto": "Supervisor de producción en maquila",
   "empresa": "'VARTINI PACKING' S.A.C",
   "cat": "Ingeniería",
   "min": 2100,
   "max": 2100,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 3,
   "vence": "2026-10-23",
   "restan": 27,
   "score": 94,
   "resumen": "RESUMEN: ¡Únete a nuestro equipo! - Supervisor de Producción",
   "funciones": [
    "Supervisar y coordinar las líneas de producción operativas (procesos de maquila, empaquetado, acondicionado y procesos manuales).",
    "Controlar el avance de producción y verificar el cumplimiento de los programas diarios.",
    "Realizar estudios de tiempos y movimientos para la optimización de los procesos.",
    "Implementar estrategias de balanceo de línea para maximizar la eficiencia operativa.",
    "Monitorear, analizar y reportar los KPIs de producción para la toma de decisiones.",
    "Aplicar herramientas de Lean Manufacturing para la identificación y eliminación de desperdicios en planta.",
    "Asignar, liderar y motivar al personal operativo a su cargo, asegurando un clima de trabajo eficiente y seguro.",
    "Garantizar el cumplimiento de los estándares de calidad y seguridad en las operaciones."
   ],
   "requisitos": [
    "Educación: Profesional egresado o bachiller en Ingeniería Industrial, Administración Industrial, Ingeniería de Procesos o carreras afines.",
    "Experiencia: Mínimo 1 a 2 años de experiencia desempeñándote en planta, liderando operarios y líneas de producción en simultáneo (indispensable experiencia en procesos de maquila o manuales).",
    "Estudio de tiempos y movimientos.",
    "Balanceo de líneas de producción.",
    "Gestión y control mediante KPIs de producción.",
    "Metodología y herramientas de Lean Manufacturing.",
    "Habilidades: Liderazgo de equipos, comunicación asertiva, resolución de problemas y orientación a resultados.",
    "Disponibilidad: Disponibilidad para laborar de manera presencial en turnos rotativos."
   ],
   "beneficios": [
    "Remuneración: S/ 2,100.00 mensuales.",
    "Ingreso a planilla desde el primer día: Con todos los derechos y beneficios de ley al 100% (CTS, gratificaciones, 30 días de vacaciones pagadas y seguro de salud EsSalud).",
    "Beneficios adicionales: Asignación familiar adicional (S/ 113.00 en caso corresponda por ley), pago de horas extras conforme a ley y flexibilidad de pagos puntuales (quincenal y mensual).",
    "Desarrollo profesional: Oportunidades reales de crecimiento, capacitación constante e implementación de proyectos de mejora continua.",
    "Estabilidad y Bienestar: Pertenecer a una empresa sólida, segura, en constante crecimiento y con un excelente ambiente laboral.",
    "En Vartini Packing S.A.C. promovemos la inclusión y la diversidad. Evaluamos a todos los candidatos por sus competencias, garantizando un proceso de selección transparente y sin discriminación por motivos de género, raza, religión, edad o discapacidad."
   ],
   "url": "https://www.laborum.pe/job/vartini-packing-s-a-c/supervisor-de-produccion-en-maquila-santa-anita/6ab33cc4ba83cb6e704beeaf"
  },
  {
   "id": 31,
   "puesto": "Asesor Comercial",
   "empresa": "CTIYLAND TRAVEL",
   "cat": "Ventas",
   "min": 1130,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 3,
   "vence": "2026-11-22",
   "restan": 57,
   "score": 92,
   "resumen": "Gestionar y ejecutar el proceso de ventas, desde el contacto inicial hasta el cierre.",
   "funciones": [
    "Gestionar y ejecutar el proceso de ventas, desde el contacto inicial hasta el cierre.",
    "Brindar atención y soporte a clientes, resolviendo sus consultas y necesidades.",
    "Realizar seguimiento a prospectos y clientes para asegurar la satisfacción y fidelización.",
    "Trabajar activamente para alcanzar y superar los objetivos de venta establecidos.",
    "Mantener una comunicación fluida y efectiva a través de diversos canales.",
    "Demostrar proactividad, dinamismo y responsabilidad en las tareas asignadas.",
    "Utilizar herramientas digitales y tecnológicas para la gestión comercial."
   ],
   "requisitos": [
    "Experiencia comprobable en roles de ventas y atención al cliente.",
    "Habilidades sólidas de comunicación verbal y escrita.",
    "Capacidad demostrada para el seguimiento de ventas y el cierre de operaciones.",
    "Orientación clara hacia el cumplimiento de metas y objetivos comerciales.",
    "Actitud responsable, dinámica y proactiva.",
    "Manejo de herramientas informáticas y digitales básicas.",
    "Se valorará experiencia en el sector turístico, aunque no es excluyente."
   ],
   "beneficios": [
    "Un esquema de remuneración competitivo con base fija más comisiones.",
    "Oportunidades de desarrollo y crecimiento profesional dentro de la organización.",
    "Capacitación continua en productos y técnicas de venta."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-comercial-ctiyland-travel-1118452520.html"
  },
  {
   "id": 32,
   "puesto": "Ayudante de Reparto",
   "empresa": "Grupo Marathon",
   "cat": "Logística",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 3,
   "vence": "2026-10-23",
   "restan": 27,
   "score": 91,
   "resumen": "Estamos en búsqueda de un Ayudante de reparto, que complemente nuestro equipo de Transporte.",
   "funciones": [
    "Cargar mercadería a la unidad vehicular según la orden de pedido.",
    "Repartir la mercadería según las instrucciones del jefe inmediato (entre tiendas y delivery).",
    "Realizar llamadas a los clientes para informarles la entrega de sus pedidos",
    "Entregar el producto al cliente, corroborando la conformidad de la entrega.",
    "Brindar una atención cálida al momento de entrega del producto.",
    "Cumplir con el uso correcto de los EPPs y políticas de seguridad y salud en el trabajo.",
    "Otras actividades asignadas por el líder y jefe inmediato."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Conocimientos en boleta, Guías electrónicas, factura y notas de crédito.",
    "Experiencia mínima de 6 meses como ayudante de reparto, de preferencia en entrega de delivery.",
    "Facilidad para resolver consultas y dar seguimiento oportuno.",
    "Proactividad y capacidad para gestionar múltiples tareas.",
    "Disponibilidad para realizar trabajo en campo (70%) y en almacén (30%).",
    "Horario: lunes a sábado de 7am a 4pm",
    "Ubicación: CD Huachipa Marathon (Referencia: altura paradero Mapfre)."
   ],
   "beneficios": [
    "Salario Fijo S/. 1300.00 + Vale de Alimentos S/. 150.",
    "Planilla COMPLETA, todos los beneficios al 100% (EsSalud, gratificaciones, CTS, asignación familiar, utilidades, vacaciones, etc.).",
    "Concursos y premiaciones internas.",
    "Descuentos exclusivos en nuestras tiendas a nivel nacional."
   ],
   "url": "https://www.laborum.pe/job/grupo-marathon/ayudante-de-reparto-transporte-huachipa-s-1-300-s-150/6ab32815ba83cb6e704ba3f4"
  },
  {
   "id": 33,
   "puesto": "Operario de produccion",
   "empresa": "Centro Carnes S.R.L",
   "cat": "Ingeniería",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 3,
   "vence": "2026-10-23",
   "restan": 27,
   "score": 89,
   "resumen": "Ejecutar la operatividad de la producción.",
   "funciones": [
    "Ejecutar la operatividad de la producción.",
    "Recepción de materia prima, selección de materia prima, embolsado, etiquetado, sellado, paletizado, en algunos casos alimentar las máquinas de materia prima o insumos.",
    "Limpieza del área de trabajo.",
    "Picado de productos congelados",
    "Cumplir con los estándares de calidad (BPM, HACCP) asegurando la inocuidad de los productos.",
    "Cumplir con las políticas y lineamientos en materia de SST.",
    "Otras funciones que solicite el jefe inmediato."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Con o sin experiencia. en posiciones similares en plantas de alimentos y afines.",
    "No tener inconvenientes para trabajar en temperaturas frías a 10°C a 15°C.",
    "Contar con un alto grado de flexibilidad, para realizar diferentes tareas en diferentes áreas.",
    "Facilidad para la realización de trabajo manual",
    "Amplio manejo del criterio enfocado en los resultados.",
    "Un alto sentido de la puntualidad.",
    "Ser altamente responsable, capaz de cumplir plazos."
   ],
   "beneficios": [
    "Ingreso a Planilla con todos los beneficios de ley.",
    "Beneficios completos. (Afiliación a ESsalud, Asignación familiar, Seguro de vida ley)",
    "Pagos puntuales. (Pago quincenal y fin de mes).",
    "Salario de s/1,400",
    "Pertenecer a una empresa de REGIMEN GENERAL",
    "Aprendizaje constante.",
    "Horario: lunes a sábado de 7:00am a 04:00pm",
    "Dirección: AV. LOS ROSALES N° 141 URB. VALDIVIEZO ATE,",
    "Referencia: Altura del terminal de yerbateros.",
    "Vivir en zonas aledañas a ATE, San Luis, Agustino, Santa Anita entre otros.-Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 25 y 40 añosPalabras clave: operator, operario, operator, production, produce, fabricacion, manufactoring, elaboracion, making, manufactura, produccion"
   ],
   "url": "https://www.laborum.pe/job/centro-carnes-s-r-l/operario-de-produccion-sede-ate/6ab33a06ba83cb6e704be5a0"
  },
  {
   "id": 34,
   "puesto": "Jefe de Tienda y Operaciones",
   "empresa": "Carlysse EIRL",
   "cat": "Otros",
   "min": 2500,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Chiclayo",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-12-21",
   "restan": 86,
   "score": 100,
   "resumen": "Jefa de Tienda y Operaciones – Boutique de Novias",
   "funciones": [
    "Liderar y asegurar el correcto funcionamiento de la boutique, supervisando equipo, ventas, atención al cliente, tienda y taller , desde la venta del vestido hasta su confección, pruebas y entrega.",
    "Buscamos una persona con liderazgo, organización, autonomía, capacidad de seguimiento y orientación a resultados , capaz de tomar decisiones y mantener informada a la Dirección sobre la operación.",
    "Principales responsabilidades",
    "Liderazgo y gestión del equipo",
    "Liderar y supervisar al equipo de tienda y personal operativo.",
    "Organizar horarios, tareas, prioridades y responsabilidades.",
    "Supervisar desempeño, cumplimiento de funciones y estándares de atención.",
    "Resolver incidencias y situaciones del día a día.",
    "Participar en la selección, evaluación y contratación de nuevo personal .",
    "Promover un ambiente de trabajo organizado, colaborativo y orientado a resultados.",
    "Gestión comercial",
    "Supervisar el cumplimiento de metas de ventas."
   ],
   "requisitos": [
    "Mínimo 2 años de experiencia liderando equipos, tiendas u operaciones.",
    "Experiencia como jefa/encargada de tienda, supervisora de ventas, administradora, coordinadora de operaciones o posiciones similares.",
    "Experiencia gestionando metas e indicadores.",
    "Experiencia liderando y organizando equipos.",
    "Capacidad para resolver problemas y tomar decisiones.",
    "Alto nivel de organización y seguimiento.",
    "Manejo de Excel o Google Sheets.",
    "Excelente comunicación y orientación al cliente.",
    "Disponibilidad para trabajar presencialmente en Chiclayo.",
    "Será valorado",
    "Experiencia en retail, moda, boutiques, belleza, joyería o servicios.",
    "Experiencia coordinando talleres de confección."
   ],
   "beneficios": [
    "S/ 2, 5 0 0 de remuneración fija.",
    "Bono de hasta S/ 5 0 0 por cumplimiento de objetivos.",
    "Ingresos de hasta S/ 3, 0 0 0 mensuales según resultados.",
    "Planilla REMYPE, con los beneficios correspondientes.",
    "Posición de liderazgo y autonomía operativa.",
    "Capacitación y acompañamiento inicial.",
    "Oportunidad de crecimiento profesional.",
    "Si tienes experiencia liderando equipos y operaciones y estás preparada para asumir un rol de responsabilidad integral, queremos conocerte.",
    "Envía tu CV a",
    "Asunto: Jefa de Tienda y Operaciones – Chiclayo"
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-de-tienda-y-operaciones-carlysse-eirl-1118448422.html"
  },
  {
   "id": 35,
   "puesto": "Asistente de Finanzas",
   "empresa": "VIETTEL PERU SAC",
   "cat": "Contabilidad",
   "min": 2500,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-12-21",
   "restan": 86,
   "score": 98,
   "resumen": "En Bitel Digital Solutions nos encontramos en la búsqueda de un nuevo Analista de Finanzas, que tendrá las funciones de:",
   "funciones": [
    "Elaborar y presentar declaraciones mensuales y anuales (IGV, Renta, PDT 617, etc.).",
    "Elaborar y presentar los libros y registros electrónicos a SUNAT (PLE - SIRE)",
    "Analizar operaciones empresariales para determinar su impacto tributario.",
    "Revisar la correcta aplicación del crédito fiscal y la deducibilidad de gastos.",
    "Revisar contratos y operaciones desde el punto de vista fiscal.",
    "Monitorear cambios normativos y evaluar su impacto en la empresa."
   ],
   "requisitos": [
    "Bachiller o titulado de la carrera profesional de Contabilidad.",
    "Experiencia: Mínimo tres (3) años realizando actividades contables y tributarias",
    "Idioma: Inglés nivel avanzado . (se evaluará)",
    "Excel Nivel Intermedio.",
    "Conocimiento: Ley del Impuesto a la renta, reglamento de comprobantes de pago, Ley de Bancarización, NICs y NIIF's.",
    "Dominio de legislación tributaria peruana: IGV, Renta, Detracciones, Retenciones y Percepciones.",
    "Manejo del SIRE y elaboración de libros electrónicos."
   ],
   "beneficios": [
    "Ingreso a planilla completa de Bitel Digital Solutions",
    "Vacaciones, CTS, gratifiaciones",
    "Oportunidad de crecimiento y línea de carrera"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-finanzas-viettel-peru-sac-1118452405.html"
  },
  {
   "id": 36,
   "puesto": "Auxiliar de Calidad",
   "empresa": "Confidencial",
   "cat": "Ingeniería",
   "min": 1700,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-12-21",
   "restan": 86,
   "score": 98,
   "resumen": "Ejecutar inspecciones, pruebas y mediciones en línea de acuerdo con los instructivos,especificaciones técnicas, criterios de muestreo y QC Plan aplicables, validando la conformidad del producto fabricado.",
   "funciones": [
    "Ejecutar inspecciones, pruebas y mediciones en línea de acuerdo con los instructivos,especificaciones técnicas, criterios de muestreo y QC Plan aplicables, validando la conformidad del producto fabricado.",
    "Identificar desviaciones, defectos o productos sospechosos durante el proceso, etiquetarlos, segregarlos en la zona de cuarentena y registrarlos correctamente en el MES para asegurar su trazabilidad y definición posterior.",
    "Mantener actualizados los estados de calidad de los rollos y productos en el MES, asegurando información confiable para indicadores de scrap, trazabilidad, liberación, bloqueo o disposición del producto.",
    "Verificar el estado, disponibilidad, identificación y uso adecuado de los equipos e instrumentos de medición que inciden en la calidad del producto, reportando desviaciones que puedan comprometer la confiabilidad de los controles.",
    "Registrar paradas de línea, eventos relevantes del turno, causas aparentes y acciones tomadas,proporcionando información precisa para el análisis de desempeño, pérdidas, reprocesos y mejora continua.",
    "Realizar reportes de relevo y comunicación técnica de hallazgos en el MES y/o medios definidos, asegurando continuidad del control de calidad entre turnos y escalamientooportuno de condiciones críticas.",
    "Participar en auditorías de relevo, verificación de cumplimiento de estándares de calidad, orden, limpieza y disciplina operativa en la estación de control asignada.",
    "Cumplir los requisitos del Sistema Integrado de Gestión aplicables al puesto, incluyendo controles de calidad, seguridad, salud ocupacional, medio ambiente, registros, gestión de no conformidades y mejora continua.",
    "Apoyar en actividades técnicas inherentes al control de calidad solicitadas por su jefe inmediato,siempre que estén alineadas con su nivel de competencia, entrenamiento y autorización."
   ],
   "requisitos": [
    "Carrera Técnica completa en: Producción Industrial, Control de Calidad, Procesos Industriales, Mecánica de Producción, Química Industrial, Tecnología de Materiales, o carreras afines.",
    "1 año de experiencia en puestos de control de calidad, inspección de procesos productivos, laboratorio, producción industrial o posiciones técnicas afines en empresas manufactureras.",
    "Excel a nivel intermedio.",
    "Disponibilidad para laborar en turnos rotativos de 12 horas.",
    "Disponibilidad para laborar en nuestra planta ubicada en Lurín."
   ],
   "beneficios": [
    "Beneficios de ley (gratificación, CTS, asignación familiar, Vida ley, utilidades, etc.)",
    "Alimentación cubierta en planta.",
    "Tarjeta de alimentos mensual.",
    "Asignación de movilidad por día asistido.",
    "Movilidad de acercamiento.",
    "Bono de producción.",
    "Plan médico familiar EPS."
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-de-calidad-1118452434.html"
  },
  {
   "id": 37,
   "puesto": "Asistente de Certificaciones",
   "empresa": "Pomica Perú SAC",
   "cat": "Otros",
   "min": 1500,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Ica",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-12-21",
   "restan": 86,
   "score": 98,
   "resumen": "Ejecutar el muestreo de agua, fruta, materia orgánica, superficies y otros, de acuerdo con el programa del área y en coordinación con el laboratorio contratado.",
   "funciones": [
    "Ejecutar el muestreo de agua, fruta, materia orgánica, superficies y otros, de acuerdo con el programa del área y en coordinación con el laboratorio contratado.",
    "Ingresar información, realizar seguimiento e informar errores en la plataforma VUCE para la certificación de los lugares de producción y planta empacadora.",
    "Asegurar y monitorear la correcta instalación de los carteles.",
    "Coordinar y realizar inducciones y entrenamientos al personal sobre el cumplimiento de las normas.",
    "Consolidar el cumplimiento de las Buenas Prácticas Agrícolas, Buenas Prácticas de Manufactura, según corresponda, reportando las irregularidades a su jefe inmediato.",
    "Realizar el seguimiento in situ de las No Conformidades identificadas durante las auditorías.",
    "Coordinar con todas las áreas el cumplimiento de las diferentes normas y levantamiento de las observaciones dadas en las diferentes auditorias.",
    "Otras funciones que el empleador le solicite."
   ],
   "requisitos": [
    "Título universitario o grado de Bachiller en Ingeniería Agrónoma, Ingeniería Industrial, Ingeniería Química, Ingeniería de Alimentos o carreras afines.",
    "Experiencia mínima de 2 años en posiciones similares o en áreas relacionadas con calidad y auditoría.",
    "Formación o experiencia en toma de muestras, auditoría, Buenas Prácticas Agrícolas, Buenas Prácticas de Manufactura o control de calidad.",
    "Conocimiento en procesos de muestreo e inspecciones de auditorías.",
    "Se valorará conocimiento en estándares como",
    "(con adenda FSMA), GRASP, CLP, SMETA y Leaf Marque.",
    "Interés demostrado por el área de certificaciones.",
    "Capacidad para trabajar de forma independiente y como parte de un equipo.",
    "Habilidades de organización y atención al detalle."
   ],
   "beneficios": [
    "Habilidades de organización y atención al detalle.",
    "Ingreso a planilla desde el primer día.",
    "Línea de carrera.",
    "Pertenecer a una empresa líder en el sector agroindustrial en el sur del país.",
    "Capacitaciones constantes en las diferentes normas nacionales como internacionales que cuenta la compañía y las nuevas que se pretender implementar.",
    "Oportunidad de aprendizaje en un entorno profesional dinámico."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-certificaciones-pomica-peru-sac-1118448558.html"
  },
  {
   "id": 38,
   "puesto": "Cajera de Tienda",
   "empresa": "GRUPO KARODA SAC",
   "cat": "Ventas",
   "min": 1299,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 95,
   "resumen": "Realizar cobros en efectivo, tarjeta, Yape, transferencias y otros medios de pago.",
   "funciones": [
    "Realizar cobros en efectivo, tarjeta, Yape, transferencias y otros medios de pago.",
    "Cobrar los pedidos realizados por clientes comerciales ubicados en la zona de Caquetá, de acuerdo con el ticket o información entregada por el área de ventas.",
    "Emitir boletas, facturas y notas de crédito mediante el sistema de facturación.",
    "Verificar antes de cada emisión los datos del cliente, RUC, razón social, productos, cantidades, precios unitarios y totales.",
    "Confirmar que el total del comprobante coincida con el ticket y el monto pagado por el cliente.",
    "Mantener actualizado el registro de clientes frecuentes y sus datos de facturación, verificando en cada compra el RUC y la razón social indicados por el cliente.",
    "Realizar la apertura, cierre y cuadre diario de caja.",
    "Registrar correctamente todas las operaciones efectuadas durante el día.",
    "Recepcionar las facturas y demás comprobantes de compras de la empresa, verificar sus datos y archivarlos ordenadamente.",
    "Revisar los pedidos destinados a provincia y a la sucursal, verificando que la información y la mercadería coincidan con los documentos correspondientes.",
    "Emitir las guías de remisión para los pedidos enviados a provincia y para los traslados hacia la sucursal.",
    "Coordinar con el área de ventas y despacho la correcta facturación, preparación y entrega de los pedidos."
   ],
   "requisitos": [
    "Experiencia mínima de 1 año en puestos de caja. (deseable)",
    "Experiencia en emisión de boletas y facturas electrónicas. (deseable)",
    "Conocimiento en manejo de sistemas POS. (deseable)",
    "Familiaridad con diversos medios de pago.",
    "Habilidades de organización y atención al detalle.",
    "Capacidad para trabajar de manera eficiente y precisa.",
    "Disponibilidad para trabajar de manera presencial de lunes a viernes de 8: 3 0am a 6: 0 0pm y sábados de 8: 3 0am a 4: 0 0pm"
   ],
   "beneficios": [
    "Sueldo mensual de S/ 1, 3 0 0.",
    "Ingreso a planilla bajo el régimen laboral MYPE .",
    "Pagos puntuales.",
    "Capacitación inicial en el sistema de facturación y los procedimientos internos.",
    "Buen ambiente de trabajo.",
    "Estabilidad laboral de acuerdo con el desempeño.",
    "Incorporación inmediata."
   ],
   "url": "https://www.bumeran.com.pe/empleos/cajera-de-tienda-grupo-karoda-sac-1118447624.html"
  },
  {
   "id": 39,
   "puesto": "Asesor de Ventas",
   "empresa": "MI FONDO",
   "cat": "Ventas",
   "min": 1130,
   "max": 5000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 95,
   "resumen": "En MiFondo , empresa de Fondos Colectivos, nos encontramos en la búsqueda de Asesores/as de Ventas para Lima Norte (Los Olivos, San Martín de Porres, Independencia, Comas, Carabayllo y Puente Piedra).",
   "funciones": [
    "Prospectar y captar nuevos clientes a través de canales presenciales y digitales.",
    "Contactar y realizar seguimiento permanente a leads y potenciales clientes.",
    "Identificar las necesidades del cliente y asesorarlo sobre las alternativas de MiFondo.",
    "Presentar y explicar de manera clara el funcionamiento del sistema de fondos colectivos.",
    "Gestionar el proceso comercial desde la prospección hasta el cierre de la venta.",
    "Negociar y concretar operaciones, asegurando el cumplimiento de los procedimientos establecidos.",
    "Fidelizar, desarrollar y mantener la cartera de clientes.",
    "Registrar y mantener actualizada la información y seguimiento de clientes en los sistemas comerciales.",
    "Participar en activaciones, ferias, concesionarios y otras acciones comerciales.",
    "Cumplir con las metas e indicadores comerciales establecidos por la supervisión."
   ],
   "requisitos": [
    "Secundaria completa; deseable contar con estudios técnicos o universitarios en Administración, Marketing, Negocios o carreras afines.",
    "Experiencia mínima de 1 año en ventas , idealmente en fondos colectivos, productos financieros, automotrices o intangibles.",
    "Experiencia en fondos colectivos será considerada un plus .",
    "Manejo de herramientas digitales y canales de prospección comercial.",
    "Orientación a resultados, negociación y cierre, comunicación efectiva, autonomía y prospección, orientación al cliente, perseverancia y transparencia."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Excelente esquema remunerativo.",
    "Comisiones atractivas.",
    "Oportunidades de línea de carrera.",
    "Excelente clima laboral.",
    "¿Tienes experiencia comercial y te apasionan las ventas? ¡Postula y sé parte de nuestro equipo comercial en MiFondo!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-ventas-lima-norte-mi-fondo-1118448003.html"
  },
  {
   "id": 40,
   "puesto": "Jefe de Operaciones Call Center Ventas",
   "empresa": "GEATEL TELECOM",
   "cat": "Ventas",
   "min": 2500,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 95,
   "resumen": "Si tienes experiencia liderando campañas comerciales para España y buscas una oportunidad de crecimiento profesional con excelentes ingresos, esta es tu oportunidad.",
   "funciones": [
    "Liderar, supervisar y desarrollar equipos comerciales orientados al cumplimiento de metas.",
    "Diseñar e implementar estrategias para incrementar la productividad y rentabilidad de la plataforma.",
    "Realizar seguimiento diario a indicadores de gestión, ventas, productividad y calidad.",
    "Elaborar reportes de resultados y planes de acción para la mejora continua.",
    "Acompañar y capacitar a supervisores y asesores en el cumplimiento de objetivos.",
    "Garantizar el cumplimiento de procesos operativos y comerciales establecidos por la empresa.",
    "Gestionar el clima laboral y fortalecer el compromiso de los equipos.",
    "Identificar oportunidades de mejora que permitan maximizar los resultados de la campaña."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios concluidos, en curso o truncos.",
    "Experiencia mínima de 2 años liderando equipos comerciales en Call Center como Jefe de Plataforma, Supervisor Senior, Coordinador Comercial o puestos afines.",
    "Experiencia en campañas de telecomunicaciones para España.",
    "Deseable experiencia en campañas Vodafone España.",
    "Manejo de indicadores de gestión (KPIs comerciales y operativos).",
    "Manejo de Excel a nivel intermedio.",
    "Experiencia en formación, motivación y desarrollo de equipos comerciales.",
    "Disponibilidad para laborar de manera presencial en San Juan de Miraflores.",
    "Buscamos un líder con visión estratégica, capacidad de análisis y orientación a resultados, que pueda asumir el reto de dirigir una operación comercial y llevar a su equipo al cumplimiento de objetivos cada vez más ambiciosos."
   ],
   "beneficios": [
    "Ingreso a planilla.",
    "Sueldo fijo competitivo acorde a experiencia.",
    "Comisiones por cumplimiento de objetivos.",
    "Ingresos mensuales que pueden superar los S/ 6, 0 0 0 entre sueldo y comisiones.",
    "Pagos quincenales y puntuales.",
    "Excelente clima laboral.",
    "Oportunidad de crecimiento profesional.",
    "Estabilidad laboral en una empresa con más de 1 3 años de trayectoria en el sector.",
    "Centro de trabajo: San Juan de Miraflores.",
    "Si tienes experiencia liderando equipos de ventas para el mercado español y buscas asumir un nuevo reto profesional, esta es tu oportunidad. ¡Postula y únete a GEATEL TELECOM!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-de-operaciones-call-center-ventas-telefonia-espana-sjm-geatel-telecom-1118448196.html"
  },
  {
   "id": 41,
   "puesto": "Operador de Maquinaria Pesada",
   "empresa": "CONSORCIO INGENIERIA ANCAHUASI",
   "cat": "Construcción",
   "min": 2000,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Cuzco",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-12-21",
   "restan": 86,
   "score": 95,
   "resumen": "Buscamos un Operador de Maquinaria Pesada con experiencia para unirse a nuestro equipo. El candidato ideal será responsable de operar y mantener diversos tipos de maquinaria pesada en un entorno de obra.",
   "funciones": [
    "Operar maquinaria pesada de forma segura y eficiente.",
    "Realizar inspecciones diarias de la maquinaria para asegurar su correcto funcionamiento.",
    "Mantener la maquinaria limpia y en buen estado.",
    "Seguir las instrucciones del supervisor de obra.",
    "Colaborar con el equipo para cumplir los objetivos del proyecto."
   ],
   "requisitos": [
    "Experiencia comprobada en la operación de maquinaria pesada.",
    "Licencia de operador vigente (si aplica).",
    "Conocimiento de las normas de seguridad en obra.",
    "Capacidad para trabajar en equipo y seguir instrucciones.",
    "Disponibilidad para trabajar a tiempo completo.",
    "Se valorará la experiencia en obras de infraestructura."
   ],
   "beneficios": [
    "Contrato de duración indeterminada.",
    "Alimentación cubierta.",
    "Mayores informes administrador de obra: 9- 0- 0- 7- 6- 1- 2- 0- 2 Denilson Abado"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operador-de-maquinaria-pesada-consorcio-ingenieria-ancahuasi-1118448488.html"
  },
  {
   "id": 42,
   "puesto": "Asesor Comercial",
   "empresa": "EMPORA STORE",
   "cat": "Ventas",
   "min": 1130,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 95,
   "resumen": "Brindar asesoramiento y soporte a los clientes, resolviendo sus consultas y necesidades de manera efectiva.",
   "funciones": [
    "Brindar asesoramiento y soporte a los clientes, resolviendo sus consultas y necesidades de manera efectiva.",
    "Gestionar y procesar operaciones comerciales, asegurando la máxima precisión y eficiencia en cada transacción.",
    "Colaborar activamente con el equipo para alcanzar y superar los objetivos comerciales y de servicio establecidos.",
    "Mantener una actitud comercial proactiva para identificar oportunidades de mejora y potenciales ventas.",
    "Contribuir a un ambiente de trabajo positivo, colaborativo y enfocado en el éxito mutuo.",
    "Ejecutar otras funciones relacionadas al puesto y asignadas por su superior inmediato para asegurar el logro de los objetivos generales."
   ],
   "requisitos": [
    "Formación secundaria completa.",
    "Experiencia mínima de 6 meses en roles de ventas y/o atención al cliente, preferentemente en el sector retail.",
    "Disponibilidad para trabajar a tiempo completo.",
    "Excelentes habilidades de comunicación verbal y escrita.",
    "Capacidad para trabajar de forma autónoma y como parte integral de un equipo.",
    "Orientación a resultados y al logro de objetivos."
   ],
   "beneficios": [
    "Ingreso a planilla.",
    "Descuentos en nuestros productos de tienda.",
    "Grato ambiente laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-comercial-atencion-al-cliente-empora-store-1118448519.html"
  },
  {
   "id": 43,
   "puesto": "Call Center",
   "empresa": "GEATEL TELECOM",
   "cat": "Ventas",
   "min": 1600,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 95,
   "resumen": "Somos GEATEL TELECOM , empresa con más de 1 3 años de experiencia en el mercado, brindando servicios de Ventas Call Center de telefonía y energía a importantes empresas a nivel mundial.",
   "funciones": [
    "Diseñar, organizar e impartir capacitaciones cuando sean requeridas.",
    "Elaborar y actualizar materiales de capacitación (presentaciones, hojas de cálculo, evaluaciones, entre otros).",
    "Fortalecer las habilidades comerciales del equipo mediante técnicas de venta y acompañamiento.",
    "Elaborar y presentar reportes de capacitación (asistencia, ausencias, motivos de inasistencia, porcentaje de deserción, entre otros).",
    "Analizar los resultados obtenidos para evaluar la efectividad de las capacitaciones e implementar planes de mejora continua.",
    "Contribuir al cumplimiento de los objetivos de crecimiento y productividad establecidos por la empresa."
   ],
   "requisitos": [
    "Disponibilidad para laborar de lunes a viernes de 6: 0 0",
    "Experiencia mínima de 1 año como Formador o Capacitador (indispensable).",
    "Experiencia comprobada en el mercado de telefonía español (indispensable).",
    "Dominio de técnicas de ventas y habilidades comerciales.",
    "Experiencia en el diseño y elaboración de materiales de capacitación.",
    "Conocimiento de metodologías y herramientas modernas de formación.",
    "Manejo de Microsoft Office a nivel usuario (PowerPoint, Excel y Word).",
    "Excelente comunicación, facilidad para hablar en público y buena redacción.",
    "Disponibilidad para trabajar de manera presencial en San Juan de Lurigancho .",
    "Vocación por la enseñanza y el desarrollo de personas.",
    "Comunicación efectiva a todo nivel.",
    "Proactividad e iniciativa."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer mes completo.",
    "Sueldo mensual de S/ 2, 0 0 0 (pagos quincenales).",
    "Comisiones alcanzables.",
    "Horario fijo de lunes a viernes.",
    "La empresa asume el costo de desplazamiento entre sedes cuando sea requerido.",
    "Centro de trabajo en San Juan de Lurigancho .",
    "Excelente clima laboral.",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "¡Si disfrutas enseñar, desarrollar el talento de las personas y buscas seguir creciendo profesionalmente, esta es tu oportunidad de formar parte de GEATEL TELECOM!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/formador-de-ventas-call-center-telefonia-publico-espanol-sjl-geatel-telecom-1118448228.html"
  },
  {
   "id": 44,
   "puesto": "Operario de fundición",
   "empresa": "Grupo Tawa",
   "cat": "Ingeniería",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-12-21",
   "restan": 86,
   "score": 94,
   "resumen": "Importante empresa industrial del rubro metalúrgico, Transformadora de Metales Lima S.A.C. (TM LIMA) , dedicada a la producción de alambrón de cobre, se encuentra en búsqueda de Operarios de Colada para su planta en Ate.",
   "funciones": [
    "Apoyar la operación del área de colada según indicaciones del Supervisor.",
    "Abastecer y trasladar materiales (grafito, alambrón e insumos).",
    "Apoyar en carga y descarga de materiales.",
    "Realizar inspección del área y reportar anomalías.",
    "Ejecutar tareas de orden y limpieza (5S).",
    "Registrar información del turno (checklist, incidencias y producción).",
    "Otras funciones asignadas por jefatura."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 1 año y medio a 3 años en producción industrial.",
    "Experiencia en fundición, metalmecánica, plásticos industriales, tuberías o procesos de alta temperatura.",
    "Deseable experiencia en máquinas automatizadas.",
    "Disponibilidad para laborar en horarios rotativos.",
    "Disponibilidad para trabajar presencial en Ate.",
    "No presentar vértigo (trabajo en altura y plataformas).",
    "Disponibilidad inmediata."
   ],
   "beneficios": [
    "Turnos rotativos Lunes a Domingo: 6:30 a.m. – 3:15 p.m. / 3:15 p.m. – 10:00 p.m. /9:55 p.m. – 6:40 a.m.",
    "Posibilidad de turnos de 12 horas según producción.",
    "Cambio de turno semanal o quincenal.",
    "Trabajo con exposición al calor y esfuerzo físico.",
    "Ingreso a planilla desde el primer día (Régimen General).",
    "Sueldo: S/ 1,600 mensuales (pagos mensuales – BCP).",
    "Bono trimestral por desempeño (hasta S/ 375).",
    "Tarjeta de alimentos (S/ 10 por día laborado – primeras 2 semanas del mes).",
    "EPS cubierta al 100% a partir del segundo mes.",
    "Seguro SCTR.",
    "Pago de horas extras.",
    "Comedor y concesionaria disponible en planta."
   ],
   "url": "https://www.bumeran.com.pe/empleos/urgente-operario-de-fundicion-ate-sueldo-s-1600-grupo-tawa-1118452386.html"
  },
  {
   "id": 45,
   "puesto": "Asesor de Ventas en Campo",
   "empresa": "ALABAMA INVERSIONES SALES SAC",
   "cat": "Ventas",
   "min": 1130,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Ayacucho",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-12-21",
   "restan": 86,
   "score": 94,
   "resumen": "Identificar y contactar clientes potenciales en el territorio asignado.",
   "funciones": [
    "Identificar y contactar clientes potenciales en el territorio asignado.",
    "Presentar y demostrar los productos o servicios de manera efectiva.",
    "Negociar y cerrar acuerdos de venta.",
    "Mantener relaciones sólidas con los clientes existentes.",
    "Cumplir con los objetivos de ventas establecidos.",
    "Elaborar reportes de actividad y resultados."
   ],
   "requisitos": [
    "Experiencia previa en ventas, preferentemente en terreno.",
    "Habilidades de comunicación, persuasión y negociación.",
    "Capacidad para trabajar de forma autónoma y gestionar el tiempo eficientemente.",
    "Orientación a la consecución de objetivos.",
    "Disponibilidad para desplazarse en la zona de trabajo.",
    "Se valorará experiencia en el sector."
   ],
   "beneficios": [
    "Sueldo fijo de S/ 1, 1 3 0.",
    "Planilla desde el primer día.",
    "Comisiones ilimitadas.",
    "Bonos semanales.",
    "Bono de productividad de S/ 3 7 0.",
    "Capacitación constante.",
    "Acompañamiento y soporte en campo.",
    "Línea de carrera y oportunidades reales de crecimiento.",
    "Pagos puntuales."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-ventas-en-campo-ayacucho-ingreso-directo-s-.3000-alabama-inversiones-sales-sac-1118447597.html"
  },
  {
   "id": 46,
   "puesto": "Mecanico de Motos",
   "empresa": "OTSUKI",
   "cat": "Ingeniería",
   "min": 1300,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 94,
   "resumen": "Somos una PEQUEÑA EMPRESA dedicada a la venta de repuestos marca Honda y a brindar servicios de mantenimiento y reparación de motocicletas Honda y Motokars.",
   "funciones": [
    "Mantenimiento preventivo y servicios básicos",
    "Mantenimiento correctivo / Reparación de fallas.",
    "Sistema eléctrico, inyección /carburadas.",
    "Diagnóstico, reparación",
    "Ejecuta otras actividades propias del cargo y de la Unidad en que se desempeña."
   ],
   "requisitos": [
    "Experiencia mínima de 1 año",
    "Conocimiento en mecánica Básica",
    "Manejo de Office nivel básico.",
    "Vivir en zonas aledañas (COMAS) frente a la Municipalidad de COMAS (Av. 2 2 de agosto)"
   ],
   "beneficios": [
    "Régimen Mype",
    "Ingreso a Planilla desde el 1er día.",
    "Remuneración Bruta S/ 1, 3 0 0. 0 0 a S/ 1, 8 0 0. 0 0, sujeto a evaluación",
    "Vacaciones al año de labores, pago de gratificaciones Y Cts (Pequeña empresa",
    "Bono de asistencia de S/ 1 5 0. 0 0 posterior cumplido los 0 3 meses de prueba, adicional cumpliendo los requisitos.",
    "Seguro vida ley",
    "Horario de lunes a sábado de 0 8: 3 0"
   ],
   "url": "https://www.bumeran.com.pe/empleos/mecanico-de-motos-otsuki-1118447603.html"
  },
  {
   "id": 47,
   "puesto": "Asistente Administrativo C",
   "empresa": "OTSUKI",
   "cat": "Administración",
   "min": 1200,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 94,
   "resumen": "Somos una PEQUEÑA EMPRESA dedicada a la venta de repuestos marca Honda y a brindar servicios de mantenimiento y reparación de motocicletas Honda y Motokars.",
   "funciones": [
    "Facturación sede Ate y Comas",
    "Validación de Comprobante de pago por SUNAT",
    "Registro de reporte de ventas Diario",
    "Cuadres de Caja y Egreso",
    "Atención al cliente",
    "Cierres de venta mensual",
    "Depositar el efectivo que ingresa",
    "Control de Personal",
    "Reporte de Asistencia",
    "Orden y limpieza",
    "Apoyar en la gestión documentaria de la tienda",
    "Apoyo directo al jefe tienda en: Licencias, certificados municipales y/o servicios generales de la tienda"
   ],
   "requisitos": [
    "Estudios técnicos y/o universitarios en administración, industrial, Contabilidad, logística y/o afines.",
    "Experiencia mínima de 1 año.",
    "Manejo de Office nivel básico.",
    "SAP a nivel Básico",
    "Vivir en zonas aledañas (ATE) frente al Mall Plaza de Puruchuco"
   ],
   "beneficios": [
    "Régimen Pequeña Empresa",
    "Ingreso a Planilla desde el 1er día.",
    "Remuneración Bruta S/ 1, 2 0 0. 0 0 a S/ 1, 5 0 0. 0 0, sujeto a evaluación",
    "Vacaciones al año de labores, pago de gratificaciones Y Cts (Pequeña empresa",
    "Bono de asistencia de S/ 1 5 0. 0 0 posterior cumplido los 0 3 meses de prueba, adicional cumpliendo los requisitos.",
    "Seguro vida ley",
    "Horario de lunes a sábado de 0 9: 0 0"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-administrativo-c-experiencia-en-facturacion-otsuki-1118447620.html"
  },
  {
   "id": 48,
   "puesto": "Asesor de Ventas Call Center",
   "empresa": "Amy Vargas",
   "cat": "Ventas",
   "min": 1130,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 94,
   "resumen": "Realizar llamadas para ofrecer productos y servicios.",
   "funciones": [
    "Realizar llamadas para ofrecer productos y servicios.",
    "Gestionar consultas y objeciones de los clientes.",
    "Cumplir con los objetivos de ventas establecidos.",
    "Registrar la información de las interacciones con los clientes.",
    "Participar en capacitaciones para mejorar habilidades de venta."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia previa en ventas o atención al cliente (no excluyente).",
    "Habilidades de comunicación verbal y persuasión.",
    "Orientación a resultados y cumplimiento de metas.",
    "Disponibilidad para trabajar en horarios establecidos."
   ],
   "beneficios": [
    "Capacitación remunerada.",
    "Remuneración base competitiva.",
    "Comisiones sin límite.",
    "Bonos por asistencia y referidos.",
    "Ingreso a planilla con beneficios de ley.",
    "Oportunidades de desarrollo profesional y línea de carrera."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-ventas-call-center-amy-vargas-1118447844.html"
  },
  {
   "id": 49,
   "puesto": "Auxiliar de Almacén, Despacho y Producción",
   "empresa": "PERAGRO SEEDS",
   "cat": "Logística",
   "min": 1200,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 94,
   "resumen": "Recepción y Control: Recepcionar y verificar la mercadería e insumos (bolsajes de semillas, sobres, tintas, etiquetas) cotejando cantidad, lote y estado del empaque.",
   "funciones": [
    "Recepción y Control: Recepcionar y verificar la mercadería e insumos (bolsajes de semillas, sobres, tintas, etiquetas) cotejando cantidad, lote y estado del empaque.",
    "Apoyo a Producción: Abastecer manualmente las tolvas de la máquina de pintado mediante el levantamiento de sacos ( 1 0 kg a más), apoyar en el conteo y embolsado de sachets, y trasladar el producto terminado conforme al almacén.",
    "Control de Stock y Lotes: Identificar, ubicar y registrar productos por código interno (SKU) y número de lote para asegurar la trazabilidad de las semillas.",
    "Preparación y Despacho: Acondicionar, embalar y etiquetar pedidos según órdenes de despacho, asegurando la carga hacia vehículos de transporte o agencias.",
    "Merma y Limpieza: Reportar sobrantes o mermas del proceso de envasado/pintado y mantener el orden y limpieza en las zonas de almacenamiento y planta."
   ],
   "requisitos": [
    "Educación secundaria completa.",
    "Experiencia mínima de 6 meses a 1 año en almacenes, despachos o plantas de envasado/producción.",
    "Capacidad comprobada para levantamiento manual y continuo de sacos de 1 0 kg a más para alimentar la maquinaria y realizar estiba.",
    "Disponibilidad para laborar de forma presencial en Jicamarca.",
    "Deseable licencia de conducir (Auto o Moto).",
    "Orientación al detalle y precisión en el conteo.",
    "Puntualidad, proactividad y trabajo en equipo.",
    "Capacidad de respuesta y agilidad ante picos de demanda.",
    "Compromiso con las normas de seguridad e higiene industrial."
   ],
   "beneficios": [
    "Incorporación a una empresa sólida, formal y en constante crecimiento dentro del sector agroindustrial.",
    "Ingreso a planilla con todos los beneficios sociales de ley según el régimen laboral general.",
    "Almuerzo provisto al 1 0 0% por la empresa durante la jornada laboral.",
    "Seguro Vida Ley desde el primer día de trabajo.",
    "Pagos puntuales de manera mensual.",
    "Ubicación de la posición: Jicamarca, Lurigancho",
    "Recepción de CVs hasta el miércoles 2 3 de septiembre del 2 0 2 6"
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-de-almacen-despacho-y-produccion-peragro-seeds-1118447953.html"
  },
  {
   "id": 50,
   "puesto": "Operario de Limpieza",
   "empresa": "EFCO",
   "cat": "Otros",
   "min": 1130,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 94,
   "resumen": "¡Únete a nuestro equipo de limpieza! Buscamos personal responsable y comprometido para cubrir posiciones en supermercados de Lima.",
   "funciones": [
    "Realizar tareas de limpieza y desinfección en diversas áreas.",
    "Mantener la pulcritud de los espacios asignados.",
    "Gestionar y desechar residuos de manera adecuada.",
    "Colaborar con el equipo para asegurar la continuidad de las operaciones de limpieza.",
    "Reportar cualquier incidencia o necesidad de mantenimiento relacionada con la limpieza."
   ],
   "requisitos": [
    "Persona responsable y proactiva.",
    "Disponibilidad inmediata.",
    "Con o sin experiencia.",
    "Disposición para aprender."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día.",
    "Seguro Vida Ley + beneficios de ley.",
    "Pago puntual quincena y fin de mes.",
    "Estabilidad laboral.",
    "Sueldo básico + asignación familiar.",
    "Diversas sedes en diferentes distritos de Lima Sur"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-limpieza-efco-1118448201.html"
  },
  {
   "id": 51,
   "puesto": "Jefe",
   "empresa": "PLANET GAME S.A.C.",
   "cat": "Otros",
   "min": 1650,
   "max": 1650,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 4,
   "vence": "2026-10-22",
   "restan": 26,
   "score": 94,
   "resumen": "JEFE(A) DE SALA DE TRAGAMONEDAS - CONO NORTE",
   "funciones": [
    "Supervisar la operación diaria y el correcto funcionamiento de la sala.",
    "Liderar, organizar y motivar al equipo de trabajo.",
    "Controlar la asistencia, puntualidad, presentación y cumplimiento de funciones del personal.",
    "Garantizar el cumplimiento de las políticas y procedimientos internos.",
    "Supervisar la calidad del servicio y la atención al cliente.",
    "Atender y solucionar reclamos, requerimientos e incidencias durante el turno.",
    "Supervisar arqueos, movimientos de caja, cierres y reportes operativos.",
    "Controlar los inventarios de premios, materiales, suministros e insumos.",
    "Verificar el cumplimiento de sorteos, promociones y actividades comerciales.",
    "Supervisar el orden, la limpieza y la adecuada presentación de la sala.",
    "Reportar fallas de máquinas, equipos, infraestructura y necesidades de mantenimiento.",
    "Monitorear los indicadores de gestión y el cumplimiento de objetivos."
   ],
   "requisitos": [
    "Experiencia mínima de 2 años como jefe(a), supervisor(a) o encargado(a) de casinos o salas de tragamonedas.",
    "Experiencia comprobada en liderazgo y manejo de personal.",
    "Conocimientos de caja, inventarios, elaboración de reportes y atención al cliente.",
    "Manejo de Microsoft Office a nivel usuario.",
    "Disponibilidad para trabajar en horarios rotativos y realizar horas extras cuando sean requeridas.",
    "Disponibilidad para viajar a nuestras sedes de Huaral y Sullana.",
    "Disponibilidad inmediata.",
    "Liderazgo y manejo de equipos.",
    "Comunicación efectiva.",
    "Orientación al cliente y a resultados.",
    "Organización y planificación.",
    "Capacidad de análisis y toma de decisiones."
   ],
   "beneficios": [
    "Remuneración mensual de S/1,650 + bono de S/250.",
    "Ingreso a planilla desde el primer día, con todos los beneficios correspondientes.",
    "Pagos puntuales.",
    "Capacitación permanente.",
    "Buen ambiente laboral.",
    "Oportunidades de crecimiento y desarrollo profesional.",
    "Si cuentas con experiencia en el rubro de tragamonedas y tienes capacidad para liderar equipos y gestionar eficientemente una sala, ¡esta oportunidad es para ti",
    "1650 - 1900. -Requerimientos- Educación mínima: Técnico2 años de experienciaEdad: entre 24 y 43 añosPalabras clave: lider, jefe, gerente, manager, director, chief, lead, jefatura, regente"
   ],
   "url": "https://www.laborum.pe/job/planet-game-s-a-c/jefes-de-salas-tragamonedas-cono-norte-jefe-de-sala/6ab1e0f5ba83cb6e704a6853"
  },
  {
   "id": 52,
   "puesto": "Tecnico de Telecomunicaciones",
   "empresa": "Movilmax",
   "cat": "Ingeniería",
   "min": 1600,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 92,
   "resumen": "🔧 TÉCNICO INSTALADOR DE FIBRA ÓPTICA FTTH 📍 Magdalena del Mar – Lima 👷 Con o sin experiencia",
   "funciones": [
    "🔹 Instalación y tendido de redes de fibra óptica aérea.",
    "🔹 Despliegue y mantenimiento de redes FTTH.",
    "🔹 Apoyo en trabajos de fusión y medición de fibra óptica.",
    "🔹 Manejo de OTDR y equipos de medición, previa capacitación.",
    "🔹 Cumplimiento de procedimientos de seguridad.",
    "🔹 Trabajo coordinado con la cuadrilla técnica."
   ],
   "requisitos": [
    "✅ Experiencia en fibra óptica o telecomunicaciones.",
    "✅ Experiencia en trabajos de campo o planta externa.",
    "✅ Experiencia en trabajos en altura.",
    "✅ Ganas de aprender y desarrollarse profesionalmente.",
    "✅ Responsabilidad, compromiso y trabajo en equipo.",
    "✅ Licencia de conducir (deseable) .",
    "✅ No registrar antecedentes policiales, penales ni judiciales."
   ],
   "beneficios": [
    "📌 Ingreso a planilla.",
    "📌 Capacitación constante.",
    "📌 Oportunidad de desarrollo en el sector de telecomunicaciones.",
    "📲 ¡POSTULA AHORA! Envía tu CV al WhatsApp: 9 5 9 7 0 7 0 9 3",
    "👉 ¡Únete a nuestro equipo y crece con nosotros! 🚀"
   ],
   "url": "https://www.bumeran.com.pe/empleos/tecnico-de-telecomunicaciones-movilmax-1118447549.html"
  },
  {
   "id": 53,
   "puesto": "Agente de Seguridad",
   "empresa": "J &amp; O SECURITY SAC",
   "cat": "Otros",
   "min": 1200,
   "max": 1700,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 92,
   "resumen": "Buscamos un Agente de Seguridad proactivo y responsable para unirse a nuestro equipo. El candidato ideal poseerá experiencia previa en el área y demostrará un fuerte compromiso con la seguridad y el servicio.",
   "funciones": [
    "Vigilar y patrullar las instalaciones asignadas para prevenir y detectar actividades sospechosas.",
    "Garantizar la seguridad de las personas y los bienes.",
    "Responder a emergencias y activar los protocolos de seguridad correspondientes.",
    "Realizar rondas de inspección y reportar cualquier anomalía o incidente.",
    "Mantener un registro de las actividades diarias y los eventos ocurridos."
   ],
   "requisitos": [
    "Experiencia mínima de 1 año como Agente de Seguridad.",
    "Disponibilidad para laborar en turnos rotativos de 0 8 o 1 2 horas.",
    "Habilidades de observación y atención al detalle.",
    "Capacidad para mantener la calma y actuar con eficacia en situaciones de presión.",
    "Responsabilidad, puntualidad y compromiso con el servicio."
   ],
   "beneficios": [
    "Incorporación a planilla desde el primer día.",
    "Entrega de uniforme para el desempeño de las labores.",
    "Todos los beneficios de ley - MYPE.",
    "Pagos puntuales.",
    "Buen ambiente laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/agente-de-seguridad-j-amp-o-security-sac-1118447836.html"
  },
  {
   "id": 54,
   "puesto": "Asistente Administrativa",
   "empresa": "CARGO CARRASOS S.R.L.",
   "cat": "Administración",
   "min": 1699,
   "max": 1700,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 92,
   "resumen": "Empresa del Sector TRANSPORTE TERRESTRE DE CARGA ubicada en LA VICTORIA-LIMA , se encuentra en busca de una ASISTENTE ADMINISTRATIVA que cumpla con los siguientes requisitos:",
   "funciones": [
    "Atención a clientes a través de diferentes canales: Llamadas, WhatsApps y correos electrónicos: consultas, cotizaciones, solicitudes, incidencias y reclamos.",
    "Coordinación con los trabajadores para el control y actualización de los documentos de transporte (permisos, seguros, documentos de unidades, documentos de conductores, etc.)",
    "Realizar liquidaciones de unidades, emitir facturas electrónicas desde SUNAT y realizar el pago de las detracciones.",
    "Realizar trámites con proveedores y entidades como MTC, SUNAT, Sunarp.",
    "Recibir, verificar y registrar los documentos, solicitudes y llamadas que lleguen a la oficina para canalizarlos o gestionarlos según corresponda.",
    "Brindar asistencia a gerencia en la preparación de oficios, informes u otros documentos.",
    "Otras funciones inherentes al puesto y según lo designe su jefe inmediato."
   ],
   "requisitos": [
    "Que tenga disponibilidad INMEDIATA para trabajar a tiempo completo y de forma presencial de Lunes a Sábado de 9 am a 6 pm (INDISPENSABLE). De no cumplir con este requisito, por favor abstenerse de postular.",
    "Estudios demostrables de administración, contabilidad, negocios o afines.",
    "Experiencia en empresas de servicios, logística, distribución o similares.",
    "Que viva en zonas aledañas al distrito de LA VICTORIA o que no tenga inconvenientes para transportarse por esa zona.",
    "Que busque un trabajo permanente y a largo plazo, se firma como mínimo por 1 año de trabajo renovable.",
    "Conocimiento de las herramientas de MS Office a nivel intermedio (Word y Excel).",
    "Buen manejo de las tecnologías de información y comunicación.",
    "No tener antecedentes policiales, penales y judiciales.",
    "Que sea puntual y responsable.",
    "Que tenga buen trato y facilidad para coordinar con clientes y personal de todo rango."
   ],
   "beneficios": [
    "Incorporación a planilla MYPE con todos los beneficios de Ley",
    "Oportunidades de crecimiento dentro de la empresa",
    "Buen ambiente de trabajo",
    "Remuneración: S/. 1 7 0 0"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-administrativa-cargo-carrasos-s.r.l.-1118448254.html"
  },
  {
   "id": 55,
   "puesto": "Promotor Financiero para trabajar en el Banco Scotiabank",
   "empresa": "SEF PERU HOLDING",
   "cat": "Ventas",
   "min": 1000,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 92,
   "resumen": "¡¡Oportunidad Laboral – Rubro Financiero!!",
   "funciones": [
    "Ofrecer, vender y comercializar los diversos productos de las entidades financieras (préstamos personales y ventas de tarjetas) a los diversos clientes aprobados por el cliente.",
    "Cumplir los horarios de establecidos de trabajo, horario flexible.",
    "Fomentar la comunicación telefónica con las diversas bases de datos asignadas por el cliente.",
    "Abordar al cliente, empezando en lugares aledaños a su distrito."
   ],
   "requisitos": [
    "Personas proactivas y emprendedoras con el potencial de laborar dentro del rubro financiero",
    "Con experiencia en venta de productos intangibles (préstamos personales, banca empresa, banca negocio, tarjetas de crédito, vehiculares).",
    "Orientados a metas y resultados",
    "Responsables y comprometidos, con ganas de crecer a corto y mediano plazo",
    "Lunes a Viernes de 9: 0 0 am – 0 6: 0 0 pm y Sábados de 9: 0 0 am – 0 1: 0 0 pm",
    "Horario full time- Horario Flexible"
   ],
   "beneficios": [
    "Trabajar cerca a tu domicilio para tu comodidad y seguridad.",
    "Capacitación constante.",
    "Buen clima laboral",
    "Remuneraciones atractivas: Comisiones + Bonos + Campañas (DESDE TU PRIMERA VENTA)",
    "Manejo de cartera de clientes aprobados por la Financiera.",
    "Oportunidad de realizar línea de carrera con los diversos ascensos, promociones y más.",
    "¡¡No te pierdas la oportunidad de tener un gran cambio, VEN Y PARTICIPA DE NUESTRA CONVOCATORIA!!",
    "¡¡Te esperamos!!!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/promotor-financiero-para-trabajar-en-el-banco-scotiabank-convenios-todo-lima-y-callao-sef-peru-holding-1118448637.html"
  },
  {
   "id": 56,
   "puesto": "Asistente administrativo",
   "empresa": "Escuela de Conductores José Gálvez",
   "cat": "Administración",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 4,
   "vence": "2026-10-22",
   "restan": 26,
   "score": 92,
   "resumen": "RESUMEN: Grupo José Gálvez, escuela de manejo líder en el mercado con más de 50 años de trayectoria, se encuentra en la búsqueda de un Asistente Administrativo con experiencia en pagos y control administrativo.",
   "funciones": [
    "Pagos de servicios, proveedores, alquileres y haberes.",
    "Control de ingresos y egresos de cuentas bancarias, con sustento para reporte al área de Contabilidad.",
    "Apoyo en el cronograma mensual de pagos.",
    "Envío de constancias de pago de haberes y beneficios al área de RRHH.",
    "Gestión de útiles de oficina, limpieza y requerimientos administrativos.",
    "Trámites en municipalidades y entidades públicas.",
    "Revisión de recibos por honorarios, boletas de pago y facturas.-Requerimientos- Educación mínima: Técnico1 año de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, administrative, contabilidad, treasury, accounting, tesoreria, generalista, generalist, general"
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en Administración, Contabilidad o carreras afines.",
    "Experiencia en tesorería y funciones administrativas.",
    "Manejo de plataformas bancarias y Excel a nivel intermedio.",
    "Disponibilidad para laborar de manera presencial en Miraflores y en el siguiente horario",
    "L, M y V de 11:00 a.m. a 9:00 p.m. | M y J de 12:00 p.m. a 9:00 p.m. | S de 2:00 p.m. a 7:00 p.m."
   ],
   "beneficios": [
    "Sueldo fijo S/ 1,500.",
    "Beneficios de ley bajo régimen GENERAL (CTS, gratificaciones, vacaciones, EsSalud y Seguro Vida Ley).",
    "Estabilidad laboral en una empresa sólida y reconocida.",
    "Buen clima laboral y aprendizaje continuo."
   ],
   "url": "https://www.laborum.pe/job/escuela-de-conductores-jose-galvez/asistente-administrativo-experiencia-en-tesoreria-planilla/6ab21c4aba83cb6e704af00e"
  },
  {
   "id": 57,
   "puesto": "Analista de Marketing",
   "empresa": "ManpowerGroup Perú",
   "cat": "Marketing",
   "min": 3000,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-12-21",
   "restan": 86,
   "score": 92,
   "resumen": "Modalidad presencial en Santiago de Surco.",
   "funciones": [
    "Ejecutar y realizar seguimiento a campañas de marketing digital y acciones de captación.",
    "Gestionar y actualizar bases de datos, asegurando la calidad y consistencia de la información.",
    "Administrar y dar seguimiento a leads y oportunidades a través del CRM.",
    "Monitorear indicadores de desempeño comercial y marketing.",
    "Elaborar reportes y dashboards para el seguimiento de resultados.",
    "Coordinar la ejecución de campañas con agencias y proveedores.",
    "Apoyar en iniciativas de automatización y mejora de procesos.",
    "Dar seguimiento al proceso de captación y conversión de prospectos.",
    "Brindar soporte al equipo Comercial y Marketing en el análisis de información."
   ],
   "requisitos": [
    "Bachiller o titulado(a) en Marketing, Administración, Ingeniería Industrial, Comunicaciones o carreras afines.",
    "Experiencia en marketing, marketing digital, CRM, análisis de datos o posiciones similares.",
    "Manejo de Excel a nivel intermedio o avanzado.",
    "Deseable experiencia en CRM (Salesforce, HubSpot u otros).",
    "Deseable conocimiento de Google Analytics, Google Ads, Meta Ads, Looker Studio o Power BI.",
    "Capacidad analítica, orientación a resultados y organización."
   ],
   "beneficios": [
    "Contrato temporal por 4 meses.",
    "Remuneración: S/ 3,000 brutos mensuales.",
    "EPS cubierta al 85%.",
    "Oportunidad de adquirir experiencia en una reconocida institución educativa.",
    "En Manpower Perú promovemos la inclusión e inserción laboral sin distinción por razón de discapacidad, género, orientación, identidad y expresión de género, lugar de origen o generación. Por ello, todos los talentos son bienvenidos a participar en nuestros procesos de selección."
   ],
   "url": "https://www.bumeran.com.pe/empleos/analista-de-marketing-temporal-4-meses-santiago-de-surco-manpowergroup-peru-1118441994.html"
  },
  {
   "id": 58,
   "puesto": "Asesor de ventas call center",
   "empresa": "Pacific Trade",
   "cat": "Ventas",
   "min": 1300,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 89,
   "resumen": "¡GANA SIN LÍMITES COMO ASESOR DE VENTAS CALL CENTER!",
   "funciones": [
    "Responder mensaje a los clientes.",
    "Realizar llamadas a la base de datos.",
    "Inicio y cierre de ventas de los productos.",
    "Realizar seguimiento a cartera y cerrar ventas.",
    "Ubicación: Miraflores (a 2 cuadras de la estación Ricardo Palma del Metropolitano)."
   ],
   "requisitos": [
    "Experiencia en ventas (deseable en call center)",
    "Actitud ganadora y muchas ganas de crecer",
    "Orientación a metas (ventas)",
    "Atención al cliente",
    "Disponibilidad inmediata",
    "De lunes a viernes de 9: 0 0 am a 6: 0 0 pm",
    "Sábados de 9: 0 0 am a 0 1: 0 0pm"
   ],
   "beneficios": [
    "Sueldo fijo S/ 1 3 0 0 + bono por metas + comisiones ilimitadas",
    "Ingreso a planilla MYPE desde el primer día",
    "Horario fijo",
    "Línea de carrera",
    "Excelente ambiente laboral"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-ventas-call-center-pacific-trade-1118447880.html"
  },
  {
   "id": 59,
   "puesto": "Recepcionista Frontdesk para Hostel en Miraflores",
   "empresa": "Casaroma Hostels",
   "cat": "Administración",
   "min": 1130,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 89,
   "resumen": "Buscamos un/a Recepcionista con vocación de servicio para integrarse a nuestro equipo. Si disfrutas del trato con personas de diversas culturas y te apasiona brindar una experiencia excepcional a los visitantes, esta oportunidad es para ti.",
   "funciones": [
    "Gestionar el proceso de registro y salida de visitantes.",
    "Administrar reservas y la disponibilidad de espacios.",
    "Proporcionar información y orientación relevante.",
    "Coordinar con otras áreas para asegurar la operatividad.",
    "Procesar pagos y emitir la documentación correspondiente."
   ],
   "requisitos": [
    "Actitud proactiva y excelente disposición para la atención al cliente.",
    "Habilidades de comunicación y relaciones interpersonales sólidas.",
    "Manejo de Excel, Word.",
    "Demostrar responsabilidad, puntualidad y capacidad de colaboración.",
    "Flexibilidad para adaptarse a diferentes esquemas de trabajo."
   ],
   "beneficios": [
    "Un entorno laboral positivo y dinámico.",
    "La oportunidad de interactuar con personas de distintos orígenes.",
    "Seguro de trabajo"
   ],
   "url": "https://www.bumeran.com.pe/empleos/recepcionista-frontdesk-para-hostel-en-miraflores-casaroma-hostels-1118448404.html"
  },
  {
   "id": 60,
   "puesto": "Administrador Restaurante",
   "empresa": "RESTOINVESTMENTS S.A.C.",
   "cat": "Recursos Humanos",
   "min": 2500,
   "max": 2800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 87,
   "resumen": "Reconocido restaurante 4 tenedores se encuentra en la búsqueda del mejor talento para incorporarse como ADMINISTRADOR y ser parte de nuestro equipo.",
   "funciones": [
    "Supervisar operaciones diarias del restaurante (salón, cocina, caja y delivery).",
    "Cumplir y hacer cumplir los estándares de higiene y servicio.",
    "Supervisión, organización y planificación del equipo.",
    "Fomentar el buen clima laboral y el trabajo en equipo.",
    "Atención al Cliente y Solución de Problemas."
   ],
   "requisitos": [
    "Residir en zonas aledañas. (No indispensable)",
    "Estudios técnicos o universitarios en Administración, Hotelería, Gastronomía o afines.",
    "Experiencia mínima de 1 año como administrador en restaurantes o afines.",
    "Trabajo en equipo.",
    "Toma de decisiones y proactividad."
   ],
   "beneficios": [
    "Ingreso a planilla MYPE y todos los beneficios de ley.",
    "Oportunidad de crecimiento.",
    "Entrenamiento constante vinculado a tu puesto de trabajo."
   ],
   "url": "https://www.bumeran.com.pe/empleos/administrador-restaurante-restoinvestments-s.a.c.-1118447700.html"
  },
  {
   "id": 61,
   "puesto": "Conductor de Reparto",
   "empresa": "C Y R INTEGRAL SERVICE SOCIEDAD ANONIMA CERRADA",
   "cat": "Logística",
   "min": 1500,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 87,
   "resumen": ", empresa especializada en soluciones de telecomunicaciones, buscamos un Conductor de Despacho o Reparto para brindar apoyo logístico en el traslado de materiales, equipos y documentos dentro de Lima Metropolitana.",
   "funciones": [
    "Traslado y entrega de materiales, equipos y documentos.",
    "Apoyo en compras, recojos y gestiones administrativas.",
    "Realizar trámites asignados en campo.",
    "Revisión básica y cuidado del vehículo asignado."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Licencia de conducir A 1 (Deseable contar con A 2B ).",
    "Experiencia mínima de 1 año en reparto, trámites y traslado de equipos o documentos.",
    "Conocimiento de rutas de Lima Metropolitana.",
    "Puntualidad, orden y responsabilidad."
   ],
   "beneficios": [
    "Pagos puntuales",
    "Estabilidad laboral",
    "Buen ambiente de trabajo, equipo colaborativo y amigable.",
    "Oportunidades de crecimiento",
    "Capacitaciones internas, mejora tus habilidades constantemente."
   ],
   "url": "https://www.bumeran.com.pe/empleos/conductor-de-reparto--lima-metropolitana-c-y-r-integral-service-sociedad-anonima-cerrada-1118448146.html"
  },
  {
   "id": 62,
   "puesto": "Practicante Técnico Industrial",
   "empresa": "Rcj Inkpack",
   "cat": "Prácticas",
   "min": 1300,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 86,
   "resumen": "Si tienes buena base técnica, ganas de aprender y buscas desarrollarte profesionalmente, esta oportunidad puede ser para ti.",
   "funciones": [
    "Soporte técnico a clientes.",
    "Diagnóstico de fallas.",
    "Mantenimiento preventivo y correctivo.",
    "Instalación y puesta en marcha de equipos.",
    "Pruebas técnicas en taller.",
    "Visitas técnicas y soporte presencial/remoto."
   ],
   "requisitos": [
    "Estudiantes de últimos ciclos o egresados.",
    "Preferentemente estudiantes en turno noche.",
    "Conocimientos básicos de electrónica, electricidad, mecánica o mantenimiento.",
    "Responsabilidad, capacidad de análisis y disposición para aprender.",
    "Disponibilidad para trabajar presencialmente en San Luis – Lima.",
    "Lunes a viernes: 8: 0 0",
    "Sábados: 9: 0 0"
   ],
   "beneficios": [
    "Capacitación técnica permanente.",
    "Formación en equipos y maquinaria industrial.",
    "Experiencia con clientes reales.",
    "Línea de crecimiento dentro del área técnica."
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-tecnico-industrial--soporte-y-mantenimiento-rcj-inkpack-1118448063.html"
  },
  {
   "id": 63,
   "puesto": "Asesor Comercial Remoto",
   "empresa": "DataThesis",
   "cat": "Ventas",
   "min": 1300,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 86,
   "resumen": "Captar clientes potenciales interesados en servicios de tesis",
   "funciones": [
    "Captar clientes potenciales interesados en servicios de tesis",
    "Contactar y brindar información sobre nuestros servicios",
    "Realizar seguimiento a los potenciales clientes",
    "Gestionar y cerrar ventas",
    "Registrar y reportar clientes y ventas generadas"
   ],
   "requisitos": [
    "Personas con facilidad para comunicarse",
    "Estudiantes, egresados o profesionales de cualquier carrera",
    "Deseable formación en Administración, Contabilidad, Economía, Marketing o carreras afines",
    "Contactos, grupos o redes donde encontrar clientes potenciales",
    "Con o sin experiencia previa en ventas",
    "Orientación a resultados"
   ],
   "beneficios": [
    "Altas comisiones",
    "1 0 0 % remoto",
    "Horario flexible",
    "Pagos quincenales"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-comercial-remoto-datathesis-1118448434.html"
  },
  {
   "id": 64,
   "puesto": "Se busca Teleoperador de Cobranza",
   "empresa": "Clasa Net",
   "cat": "Contabilidad",
   "min": 1130,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 86,
   "resumen": "CLASA NET – Consultores Legales Asociados, empresa especializada en recuperación de cartera, se encuentra en búsqueda de Asesores de Cobranzas para integrarse a su equipo de trabajo.",
   "funciones": [
    "Gestionar cobranzas de cartera asignada",
    "Registrar información en el sistema",
    "Cumplir objetivos e indicadores de gestión",
    "Interesados(as), postular por este medio"
   ],
   "requisitos": [
    "Mayor de 2 0 años",
    "Experiencia deseable en cobranzas, ventas o atención al cliente para el Banco.",
    "Comunicación efectiva y orientación a resultados",
    "PC o laptop Core i 3 o superior",
    "Internet mínimo de 1 5 MB",
    "Headset o audífonos"
   ],
   "beneficios": [
    "Ingreso a planilla",
    "Comisiones variables",
    "Bono de movilidad",
    "Bono por referidos",
    "Capacitación remunerada",
    "Pagos puntuales",
    "Oportunidad de crecimiento profesional",
    "Lunes a viernes de 0 8: 0 0 am a 0 6: 3 6 pm"
   ],
   "url": "https://www.bumeran.com.pe/empleos/se-busca-teleoperador-de-cobranza-gestiona-2-dias-presencial-y-3-remoto-clasa-net-1118448285.html"
  },
  {
   "id": 65,
   "puesto": "Asesor de Soporte",
   "empresa": "LEGEND PLAY S.A.C.",
   "cat": "Tecnología",
   "min": 1300,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 4,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 84,
   "resumen": "Atender y brindar soporte a las consultas de los clientes.",
   "funciones": [
    "Atender y brindar soporte a las consultas de los clientes.",
    "Administrar el software y las herramientas técnicas asignadas.",
    "Realizar el diagnóstico y solución de incidencias o problemas técnicos."
   ],
   "requisitos": [
    "Experiencia en el uso de Microsoft Office y navegadores de internet.",
    "Trabajo presencial en el distrito de San Borja",
    "Disponibilidad laborar en Horarios Rotativos",
    "Comunicación efectiva.",
    "Capacidad de resolución de problemas.",
    "Trabajo en equipo."
   ],
   "beneficios": [
    "Planilla desde el 1er día con beneficios de ley",
    "Sueldo acorde del mercado"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-soporte--legend-play-s.a.c.-1118447571.html"
  },
  {
   "id": 66,
   "puesto": "Ejecutivo",
   "empresa": "GRAFTON PERU S.A.C.",
   "cat": "Logística",
   "min": 5000,
   "max": 5000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 97,
   "resumen": "RESUMEN: Por encargo de nuestro cliente, importante operador logístico con amplia presencia en el mercado peruano, nos encontramos en búsqueda de un(a)",
   "funciones": [
    "Supervisar el correcto desarrollo de los procesos de recepción, almacenamiento, picking, abastecimiento, inventarios y despacho.",
    "Coordinar y organizar los recursos necesarios para asegurar la continuidad y eficiencia de la operación.",
    "Liderar y realizar seguimiento al personal administrativo, supervisores y personal operativo asignado.",
    "Monitorear el cumplimiento de procedimientos, instructivos de trabajo y estándares establecidos para la operación.",
    "Gestionar inventarios rotativos y generales, asegurando la correcta correspondencia entre el stock físico y lo registrado en sistema.",
    "Realizar seguimiento a los indicadores operativos relacionados con ingresos, salidas, pedidos, inventarios, capacidad, productividad y nivel de servicio.",
    "Gestionar incidencias operativas y definir acciones correctivas que permitan mantener la continuidad del flujo.",
    "Coordinar horarios, horas extras, vacaciones y necesidades operativas del personal a cargo.",
    "Realizar seguimiento a pedidos y despachos, verificando el cumplimiento de fechas, cantidades y condiciones acordadas.",
    "Mantener comunicación permanente con el cliente, atendiendo requerimientos y asegurando el cumplimiento de los niveles de servicio establecidos.",
    "Participar en auditorías, inventarios y actividades de mejora continua de la operación.",
    "Asegurar el correcto manejo documental y la trazabilidad de los movimientos realizados dentro del almacén."
   ],
   "requisitos": [
    "Profesional universitario de las carreras de Ingeniería Industrial, Administración, Logística o carreras afines.",
    "Experiencia previa liderando operaciones logísticas y/o de almacén.",
    "Experiencia indispensable en operaciones vinculadas al sector cosmético.",
    "Experiencia gestionando procesos de recepción, almacenamiento, inventarios, picking y despacho.",
    "Experiencia liderando equipos operativos y/o administrativos.",
    "Conocimiento de indicadores de gestión logística y control de inventarios.",
    "Manejo de SAP.",
    "Manejo de WMS (Warehouse Management System).",
    "Disponibilidad para trabajar de manera presencial en Lurín.",
    "Deseable experiencia participando en implementaciones, aperturas o arranque de nuevas operaciones logísticas.",
    "Deseable experiencia en operadores logísticos o centros de distribución."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Remuneración fija de S/5,000.",
    "Alimentación: S/60.50.",
    "Asignación de movilidad: S/105.50.",
    "Bono adicional de S/100 para colaboradores que residan fuera de Lurín.",
    "Bus de acercamiento desde el Km 40 hacia la operación."
   ],
   "url": "https://www.laborum.pe/job/grafton-peru-s-a-c/ejecutivo-coordinador-de-operaciones-logisticas-lurin-turno/6ab0b3b11aa686d09065242c"
  },
  {
   "id": 67,
   "puesto": "Técnica",
   "empresa": "Overall Strategy",
   "cat": "Ventas",
   "min": 2400,
   "max": 2400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 94,
   "resumen": "Visitarás panaderías, negocios y empresas del rubro para conocer sus procesos de elaboración, identificar oportunidades de mejora y realizar demostraciones técnicas de nuestros productos, principalmente levaduras secas y frescas.",
   "funciones": [
    "Visitar clientes y conocer sus procesos de panificación.",
    "Realizar pruebas y demostraciones técnicas de productos.",
    "Elaborar diferentes tipos de productos de panificación para demostrar el rendimiento de los insumos.",
    "Presentar los beneficios técnicos y económicos de nuestros productos frente a la competencia.",
    "Brindar asesoría y soluciones técnicas a los clientes.",
    "Preparar fichas técnicas y materiales de capacitación.",
    "Identificar nuevas oportunidades comerciales y desarrollar cartera de clientes.",
    "Realizar pruebas con productos de panificación artesanal, industrial, congelada, entre otros.",
    "Apoyar en la evaluación y comparación de productos de la competencia."
   ],
   "requisitos": [
    "Formación técnica o universitaria en Panadería, Pastelería, Gastronomía, Industria Alimentaria o carreras afines.",
    "Especialización o experiencia sólida en panificación.",
    "Mínimo 2 años de experiencia en posiciones relacionadas a panificación y/o asesoría técnica.",
    "Experiencia trabajando con levadura seca y/o levadura fresca.",
    "Experiencia en atención, negociación y/o ventas con clientes.",
    "Conocimiento de costeo de recetas y manejo de formatos.",
    "Capacidad para realizar demostraciones prácticas de productos de panificación.",
    "Buen nivel de comunicación, orientación al cliente y capacidad de persuasión.",
    "Disponibilidad para realizar visitas a clientes en turno noche, 10 pm a 6 am de L - S.",
    "Sepa de panificación y pueda demostrarlo en la práctica.",
    "Disfrute del contacto con clientes.",
    "Tenga facilidad para explicar, persuadir y negociar."
   ],
   "beneficios": [
    "Planilla Completa",
    "Modalidad: Presencial + visitas a clientes",
    "Salario: S/ 2,400 + comisiones (ingreso aproximado de hasta S/ 4,000)",
    "Bono de alimentación de 240 soles",
    "Bono de movilidad de 400 soles",
    "Si eres especialista en panificación y quieres llevar tu experiencia técnica al mundo comercial, ¡queremos conocerte!. -Requerimientos- Educación mínima: Técnico2 años de experienciaPalabras clave: baker, panadero, cambaceo, sales, piso, vivienda, inmobiliario, inmueble, condominio, edificio, planta, floor"
   ],
   "url": "https://www.laborum.pe/job/overall-strategy/panadero-con-experiencia-en-ventas-comerciales-gana-hasta/6ab0e7961aa686d0906551c8"
  },
  {
   "id": 68,
   "puesto": "Consultora",
   "empresa": "EsTalent",
   "cat": "Atención al Cliente",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 94,
   "resumen": "RESUMEN: En , consultora especializada en atracción de talento, conectamos a profesionales destacados con oportunidades de crecimiento en empresas que confían en nosotros para incorporar a los mejores talentos.",
   "funciones": [
    "Ejecutar y facilitar procesos de capacitación dirigidos al personal del Centro de Atención al Cliente, asegurando el correcto aprendizaje de los procesos operativos, comerciales y protocolos de atención, así como realizar el seguimiento al desempeño y aprendizaje de los colaboradores durante su proceso de formación.",
    "Ejecutar capacitaciones presenciales y remotas dirigidas al personal del Centro de Atención al Cliente.",
    "Brindar inducción, acompañamiento y refuerzo continuo a los colaboradores sobre procesos operativos y comerciales.",
    "Capacitar al personal en atención al cliente, cumplimiento de protocolos y procedimientos establecidos por la operación.",
    "Realizar seguimiento al desempeño y aprendizaje de los colaboradores durante su proceso de formación.",
    "Elaborar reportes básicos, controles de asistencia y seguimiento mediante herramientas Microsoft Office.",
    "Garantizar el cumplimiento de los lineamientos y estándares de calidad durante las capacitaciones.",
    "Facilitar dinámicas y actividades de aprendizaje que promuevan la participación e integración del personal."
   ],
   "requisitos": [
    "Experiencia en capacitación, formación o manejo de grupos.",
    "Experiencia realizando capacitaciones presenciales y/o remotas.",
    "Manejo básico de Microsoft Office.",
    "Buen nivel de comunicación.",
    "Capacidad de liderazgo y manejo de grupos.",
    "Orientación al servicio.",
    "Lunes a sábado de 9:00 a.m. a 6:00 p.m."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,400.",
    "Bono de movilidad: S/ 100.",
    "Variable de S/ 300, sujeta al cumplimiento de indicadores de capacitación, asistencia, desempeño y productividad del personal formado.",
    "Línea de carrera y desarrollo profesional.",
    "Excelente clima laboral.",
    "Modalidad y lugar de trabajo",
    "Presencial Surquillo.",
    "Si cuentas con experiencia en formación y capacitación de equipos y buscas una oportunidad para seguir desarrollándote profesionalmente, ¡esta oportunidad es para ti!"
   ],
   "url": "https://www.laborum.pe/job/estalent/formador-a-de-centro-de-atencion-al-cliente-descanso-los/6ab0bc531aa686d090653514"
  },
  {
   "id": 69,
   "puesto": "Vendedor de Campo",
   "empresa": "SSP ventas del Peru",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 94,
   "resumen": "RESUMEN: Promotor de ventas | Lima Norte",
   "funciones": [
    "Visitar los puntos de venta asignados de acuerdo con la ruta diaria.",
    "Impulsar las ventas y asegurar el cumplimiento de los objetivos comerciales.",
    "Verificar la correcta exhibición y disponibilidad de los productos.",
    "Reportar resultados e incidencias al supervisor.",
    "Identificar oportunidades para incrementar las ventas en el punto de venta."
   ],
   "requisitos": [
    "Experiencia mínima de 6 meses en ventas de campo o gestoría comercial en consumo masivo.",
    "Conocimiento en ventas y manejo de rutas comerciales.",
    "Disponibilidad para realizar trabajo de campo en Lima Norte.",
    "Persona proactiva, con habilidades de comunicación, negociación y orientación a resultados.",
    "Lunes a sábado",
    "8:00 a. m. a 3:00 p. m.",
    "Sabados: 8:00 am a 12:00pm",
    "Descanso fijo: Domingo."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,130",
    "Movilidad: S/ 200",
    "Planilla completa desde el primer día.",
    "Beneficios de ley (CTS, gratificaciones, vacaciones, EsSalud y asignación familiar de corresponder).",
    "Capacitación constante.",
    "Oportunidad de crecimiento y desarrollo profesional.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 50 añosPalabras clave: salesman, saleswoman, seller, vendedor, comercial, salesperson, terreno, territorio, permanent, fijo"
   ],
   "url": "https://www.laborum.pe/job/ssp-ventas-del-peru/vendedor-de-campo-consumo-masivo-con-experiencia-en-venta-de/6ab0e94a1aa686d0906557bc"
  },
  {
   "id": 70,
   "puesto": "Operador",
   "empresa": "YANA CONSULTING",
   "cat": "Logística",
   "min": 2300,
   "max": 2300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 94,
   "resumen": "RESUMEN: ¡Estamos buscando un Conductor de Cargador Frontal // Montacargas!",
   "funciones": [
    "Operar cargador frontal y montacarga para la alimentación de tolvas y el traslado de materiales.",
    "Apoyar en los despachos de productos terminados.",
    "Realizar la inspección diaria del equipo antes de iniciar labores.",
    "Reportar cualquier anomalía o condición insegura.",
    "Mantener el orden y limpieza del área de trabajo.",
    "Cumplir con los procedimientos de seguridad, calidad y medio ambiente."
   ],
   "requisitos": [
    "Experiencia mínima de 2 años operando cargador frontal.",
    "Licencia de conducir A1 o superior",
    "Certificado de operador de cargador frontal o montacargas. (opcional)",
    "Deseable experiencia en plantas industriales."
   ],
   "beneficios": [
    "Sueldo: S/ 2,300.",
    "Movilidad de acercamiento",
    "Ingreso a planilla con todos los beneficios de ley desde el primer día.",
    "Estabilidad laboral y oportunidades de crecimiento profesional.",
    "Capacitación constante en seguridad y operación de equipos.",
    "Se parte una empresa industrial global, con destacada linea de carrera y estabilidad laboral.",
    "HOLCIM PERÚ. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 24 añosPalabras clave: operator, operario, operator, driver, choferes, chofer, machinery, maquinaria, toro, carretilla, torero, carretillero, forklift, clarkista, montacarguista, montacarga"
   ],
   "url": "https://www.laborum.pe/job/yana-consulting/operador-conductor-de-maquinaria-cargador-frontal-montacarga/6ab0eb191aa686d090655c06"
  },
  {
   "id": 71,
   "puesto": "Promotor de Ventas para Tottus Chorrillos, Plaza Vea, Ripley, Ingreso directo",
   "empresa": "Salesland Pacífico",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 94,
   "resumen": "RESUMEN: ¿Buscas una oportunidad para crecer, ganar comisiones ilimitadas y tener un turno fijo?",
   "funciones": [
    "Impulsar y promocionar los productos de Movistar en el módulo de ventas.",
    "Ofrecer equipos, portabilidad, migraciones, líneas fijas, chips y otros productos.",
    "Brindar una atención cercana y de calidad a los clientes.",
    "Concretar ventas y alcanzar los objetivos establecidos.",
    "¡ELIGE TU TURNO FIJO!",
    "Tú eliges el turno que mejor se adapte a tu disponibilidad",
    "Turno Apertura: 9:00 a. m. a 6:00 p. m.",
    "Turno Cierre: 2:00 p. m. a 10:00 p. m.",
    "Jornada Full Time 48 horas semanales."
   ],
   "requisitos": [
    "Personas con o sin experiencia en ventas.",
    "Secundaria completa o estudios técnicos/universitarios truncos o completos.",
    "Personas dinámicas, empáticas, comunicativas y con actitud positiva.",
    "Disponibilidad inmediata.",
    "Disponibilidad para realizar 3 días de capacitación virtual."
   ],
   "beneficios": [
    "Sueldo fijo S/ 1,130 + Bono S/ 200",
    "COMISIONES ILIMITADAS desde S/ 1,200 a más ¡SIN TOPE!",
    "Ingreso a planilla completa desde el primer día, con todos los beneficios de ley.",
    "Descuentos corporativos en educación, salud, entretenimiento, y más.",
    "Incentivos, premios y regalos de marca.",
    "Capacitación constante.",
    "Oportunidad de crecimiento laboral en corto tiempo.",
    "Acceso a eventos exclusivos y formación continua.· Si quieres trabajar en ventas, generar ingresos adicionales mediante comisiones y contar con un horario fijo, ¡postula ahora!",
    "¡Sé parte de Salesland Pacífico y comienza a crecer con nosotros!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 50 añosPalabras clave: promoter, promotor, cambaceo, sales, permanent, fijo"
   ],
   "url": "https://www.laborum.pe/job/salesland-pacifico/buscamos-promotor-de-ventas-para-tottus-chorrillos-plaza-vea/6ab07d731aa686d09064f4e4"
  },
  {
   "id": 72,
   "puesto": "Analista de datos junior",
   "empresa": "PUM",
   "cat": "Tecnología",
   "min": 1300,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 92,
   "resumen": "RESUMEN: Somos una empresa peruana dedicada al mercaderismo outsourcing, brindando servicios a marcas líderes del sector retail y consumo masivo en esquemas exclusivos y multimarca.",
   "funciones": [
    "Validar y analizar los reportes enviados por mercaderistas desde punto de venta (PDV), en esquemas exclusivos y multimarca.",
    "Verificar y clasificar la evidencia fotográfica como sustento visual de la ejecución en PDV.",
    "Automatizar dashboards y reportes de desempeño para el área comercial y clientes.",
    "Controlar indicadores por producto y cliente: cobertura, quiebres de stock, facing, entre otros.",
    "Detectar alertas operativas como falta de cobertura, baja productividad o incumplimiento de rutas."
   ],
   "requisitos": [
    "Egresada universitaria de las carreras de Ingeniería Industrial, Estadística, Economía, Marketing, Administración o afines.",
    "Dominio intermedio de Excel avanzado.",
    "Deseable experiencia o conocimientos en Office, OneDrive, SQL y Power BI.",
    "Interés por el análisis comercial y la mejora de procesos.",
    "Experiencia en la elaboración de reportes de marketing o servicios de marketing.",
    "Capacidad para trabajar en equipo, con organización, atención al detalle, proactividad y orientación al cliente."
   ],
   "beneficios": [
    "Remuneración mensual: S/ 1300 a S/1500.",
    "Ingreso a media planilla desde el primer día de trabajo.",
    "Agradable ambiente de trabajo.",
    "Modalidad: 100% presencial.",
    "Lugar de trabajo: La Perla, Callao (Altura cruce de Av. Insurgentes con Av. La Marina).",
    "Horario: Lunes a Viernes de 9:00 a.m. a 6:00 p.m· Postula ahora y forma parte de un equipo comprometido con la excelencia y la mejora continua.-Requerimientos- Educación mínima: Universitario años de experienciaConocimientos: Microsoft excelPalabras clave: analyst, data, jr, junior"
   ],
   "url": "https://www.laborum.pe/job/pum/analista-de-datos-junior-la-perla-callao/6ab0ee881aa686d090656a88"
  },
  {
   "id": 73,
   "puesto": "Fuerza de ventas",
   "empresa": "Arch Latam",
   "cat": "Recursos Humanos",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 92,
   "resumen": "RESUMEN: ¡MERCADERISTA DE CAMPO CANAL TRADICIONAL | VISTONY!",
   "funciones": [
    "Visitar aproximadamente 15 puntos de venta diarios, según la ruta asignada.",
    "Realizar labores de mercaderismo y negociación con clientes.",
    "Realizar seguimiento y negociación con los clientes.",
    "Gestionar indicadores de cobertura y promotoría.",
    "Realizar el levantamiento de información mediante aplicativo.",
    "Recoger semanalmente el material POP.",
    "Participar en reuniones semanales con el cliente y la fuerza de ventas.",
    "Reportar avances e incidencias de la ruta."
   ],
   "requisitos": [
    "Experiencia en mercaderismo y canal tradicional.",
    "Experiencia en negociación y trato con clientes.",
    "Deseable conocimiento de lubricantes automotrices o productos relacionados al sector automotor.",
    "Disponibilidad para visitar aproximadamente 15 puntos de venta diarios.",
    "Disponibilidad para realizar trabajo 100% en campo.",
    "Manejo de aplicativos para levantamiento de información.",
    "Disponibilidad para trabajar de lunes a sábado de 8:00 a.m. a 6:00 p.m."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,200",
    "Movilidad: S/ 300",
    "Bono: S/ 200",
    "Ingreso total: hasta S/ 1,700",
    "Equipo telefónico asignado para realizar tus funciones.",
    "Trabajo 100% en campo.",
    "HORARIO: Lunes a sábado de 8:00 a.m. a 6:00 p.m.",
    "¡Si tienes experiencia en canal tradicional, negociación y trabajo de campo, postula y sé parte del equipo!. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 20 y 45 añosPalabras clave: comerciante, merchandiser, merchant"
   ],
   "url": "https://www.laborum.pe/job/arch-latam/100-mercaderista-de-ruta-canal-tradicional-vistony-planilla/6ab0ec651aa686d0906561ea"
  },
  {
   "id": 74,
   "puesto": "Operario de Producción",
   "empresa": "Manpower",
   "cat": "Ingeniería",
   "min": 1350,
   "max": 1350,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 91,
   "resumen": "RESUMEN: En Manpower Perú estamos buscando al mejor talento para asumir el reto como Operario de Producción (Molinero), en el rubro industrial.",
   "funciones": [
    "Recoger mermas de las máquinas de inyección",
    "Raspado de plásticos que contengan impresión",
    "Alimentará molino con mermas de plástico",
    "Registrará en formato la merma trabajada",
    "Traslado de bigbag de zona de salida de molino hacia zona de producto terminado",
    "Limpieza de área"
   ],
   "requisitos": [
    "Disponibilidad para laborar en horario rotativo de lunes a sábado según los siguientes turnos 7am a 3pm - 3pm a 11pm - 11pm a 7am / Domingo descanso",
    "Se requiere 6 meses de experiencia mínimo en el puesto o afines",
    "Secundaria completa"
   ],
   "beneficios": [
    "Sueldo fijo S/.1350",
    "Bono de movilidad S/.221",
    "Unirte a una empresa líder a nivel global.",
    "Cuenta con todos los beneficios de estar en planilla Manpower",
    "Disfruta de beneficios corporativos como descuentos especiales, acceso a actividades, capacitaciones entre otros.",
    "Seguro vida ley desde el primer día de trabajo",
    "Pagos quincenales"
   ],
   "url": "https://www.laborum.pe/job/manpower/operario-de-produccion-almacen-sector-industrial-callao-con/6ab0eb731aa686d090655d6f"
  },
  {
   "id": 75,
   "puesto": "Auxiliar",
   "empresa": "YANA CONSULTING",
   "cat": "Logística",
   "min": 1254,
   "max": 1254,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 91,
   "resumen": "RESUMEN: Nos encontramos en la búsqueda de un Operario de Almacén para desempeñar funciones de recepción, almacenamiento, despacho y orden de mercadería dentro del almacén.",
   "funciones": [
    "Recepción y despacho de mercadería.",
    "Carga y descarga de productos.",
    "Apoyo en el almacenamiento y organización del almacén.",
    "Preparación de pedidos.",
    "Mantener el orden y limpieza del área de trabajo.",
    "Apoyo en inventarios cuando se requiera.",
    "Cumplir con los procedimientos de seguridad establecidos."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia en almacén, estiba, construcción o trabajos de esfuerzo físico.",
    "Capacidad para cargar entre 25 - 35 kg.",
    "Deseable experiencia manejando apiladores. (no es indispensable)",
    "Disponibilidad para trabajar en Oquendo - Ventanilla.",
    "Lunes a viernes: 9:00 a.m. a 7:00 p.m.",
    "Sábados: 9:00 a.m. a 12:30 p.m."
   ],
   "beneficios": [
    "Suedo: S/1254",
    "Movilidad: S/ 50 mensuales.",
    "Alimentación cubierta al 100% (sujeta a puntualidad y asistencia).",
    "Bono mensual: S/ 100.",
    "Utilidades entre 1.5 y 2 sueldos.",
    "Línea de carrera.",
    "Ingreso a planilla con todos los beneficios de ley.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 20 y 37 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, operator, operario, operator, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/yana-consulting/auxiliar-operario-de-almacen-ventanilla-y-callao/6ab0ebd41aa686d090655f41"
  },
  {
   "id": 76,
   "puesto": "Operador De Centro De Control– Callao",
   "empresa": "SSP ventas del Peru",
   "cat": "Tecnología",
   "min": 1800,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 89,
   "resumen": "RESUMEN: OPERADOR DE CENTRO DE CONTROL CALLAO / CON EXPERIENCIA · Importante empresa del rubro de seguridad y operaciones, ubicada en Callao, se encuentra en búsqueda de un(a) Operador(a) de Centro de Control para incorporarse a su equipo.",
   "funciones": [
    "Monitorear permanentemente los sistemas de CCTV, GPS, alarmas, radios y plataformas de comunicación.",
    "Realizar el seguimiento de unidades, rutas, puestos y servicios de seguridad.",
    "Mantener comunicación constante con agentes de seguridad, custodios, conductores y supervisores.",
    "Detectar y reportar incidencias, accidentes, emergencias, desviaciones de ruta, exceso de velocidad, pérdida de comunicación y detenciones no programadas.",
    "Activar los protocolos establecidos ante emergencias e incidentes, según el nivel de autoridad correspondiente.",
    "Escalar oportunamente las incidencias al supervisor o área responsable.",
    "Registrar cronológicamente las incidencias, alertas, comunicaciones y acciones realizadas.",
    "Elaborar reportes de novedades, ocurrencias e incidencias.",
    "Verificar el correcto funcionamiento de los sistemas y equipos de monitoreo y reportar cualquier falla.",
    "Mantener actualizada la información de los servicios y garantizar la confidencialidad de las imágenes, comunicaciones, ubicaciones y demás información operativa.",
    "Cumplir los procedimientos, protocolos y políticas internas de seguridad."
   ],
   "requisitos": [
    "Formación técnica o universitaria en Seguridad Patrimonial, Administración, Informática/Computación, Comunicaciones, Electrónica o carreras afines.",
    "Experiencia mínima de 6 meses en centros de control, monitoreo CCTV, seguridad electrónica, seguimiento GPS, operaciones de seguridad o funciones similares.",
    "Experiencia y/o conocimientos en CCTV y videovigilancia.",
    "Manejo de GPS y plataformas de monitoreo.",
    "Manejo de radio y sistemas de comunicación.",
    "Conocimiento de protocolos de atención de emergencias e incidencias.",
    "Manejo de Microsoft Word y Excel.",
    "Facilidad para trabajar con plataformas y herramientas informáticas.",
    "Buena comunicación y capacidad de reacción ante situaciones críticas.",
    "Atención y concentración.",
    "Capacidad de reacción ante incidencias.",
    "Comunicación efectiva."
   ],
   "beneficios": [
    "Modalidad: Presencial",
    "Sueldo: S/ 1,800",
    "Ingreso a planilla",
    "Horario: Turnos rotativos de lunes a domingo, con 1 día de descanso a la semana",
    "Turno mañana: 06:00 a. m. 02:00 p. m.",
    "Turno tarde: 02:00 p. m. 10:00 p. m.",
    "Turno noche: 10:00 p. m. 06:00 a. m.",
    "Importante: Disponibilidad para trabajar en cualquiera de los tres turnos y realizar rotación de acuerdo con la programación operativa.-Requerimientos- Educación mínima: Técnico1 año de experienciaEdad: A partir de 25 añosPalabras clave: operator, operario, operator"
   ],
   "url": "https://www.laborum.pe/job/ssp-ventas-del-peru/operador-de-centro-de-control-callao-con-experiencia/6ab0e76f1aa686d090655126"
  },
  {
   "id": 77,
   "puesto": "Administrador@ de recursos humanos",
   "empresa": "PUM",
   "cat": "Recursos Humanos",
   "min": 2500,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 89,
   "resumen": "RESUMEN: ADMINISTRADOR@ DE RECURSOS HUMANOS",
   "funciones": [
    "Realizar el registro y actualización de información en de todo el personal.",
    "Brindar soporte en procesos administrativos de personal, como contratos, vacaciones, permisos, beneficios y documentación.",
    "Atender y orientar a los colaboradores frente a consultas relacionadas con planilla y Administración de Personal.",
    "Realizar seguimiento y control de la documentación e información laboral.",
    "Manejo de T- Registro/PLAME",
    "Coordinar con las áreas internas para asegurar la correcta gestión de los procesos de personal.",
    "Apoyar en otras actividades propias del área."
   ],
   "requisitos": [
    "Universitario(a) completo(a) en Recursos Humanos, Administración, Psicología Organizacional o carreras afines.",
    "Mínimo 2 años de experiencia en Administración de Personal, incluyendo prácticas profesionales relacionadas al puesto.",
    "Experiencia en planillas, control y manejo de información confidencial.",
    "Excel a nivel intermedio, con certificación. deseable"
   ],
   "beneficios": [
    "Sueldo: S/2500",
    "Ingreso a Planilla Completa",
    "Si cumples con el perfil, te invitamos a postular y formar parte de nuestro equipo.-Requerimientos- Educación mínima: Universitario2 años de experienciaEdad: A partir de 26 añosConocimientos: Psicología, Recursos humanosPalabras clave: administrator, resources, human"
   ],
   "url": "https://www.laborum.pe/job/pum/administrador-de-recursos-humanos/6ab0ec231aa686d0906560c7"
  },
  {
   "id": 78,
   "puesto": "Trabaja en Golds Gym! Anfitrión de estacionamiento",
   "empresa": "APPARKA",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 89,
   "resumen": "RESUMEN: En apparka nos encontramos en búsqueda del mejor talento para el puesto de Anfitrión de Estacionamientos.",
   "funciones": [
    "Orientar y guiar al cliente para su ubicación y zona de acceso.",
    "Plaquear los vehículos que ingresan a nuestro estacionamiento.",
    "Realizar el cobro en caja, en caso corresponda.",
    "Informar y reportar las observaciones al supervisor.",
    "Realizar constantes rondas de seguridad dentro de la playa de estacionamiento."
   ],
   "requisitos": [
    "Personas con buena actitud, orientadas al servicio y dispuestas a aprender.",
    "Experiencia previa en atención al cliente (no es indispensable, ¡nosotros te capacitamos!).",
    "Disponibilidad para laborar en el turno tarde en horario fijo de 1:15 pm a 10:00 pm.",
    "Compromiso con el trabajo en equipo y la puntualidad."
   ],
   "beneficios": [
    "Sueldo: S/. 1130 mensual",
    "Planilla completa con todos los beneficios desde el primer día (CTS, Gratificación, Asignación Familiar)",
    "Seguro de salud",
    "¡Oportunidad de línea de carrera a corto plazo!"
   ],
   "url": "https://www.laborum.pe/job/apparka/trabaja-en-golds-gym-anfitrion-de-estacionamiento-san-borja/6ab0ba4a1aa686d0906530f6"
  },
  {
   "id": 79,
   "puesto": "Atención al cliente",
   "empresa": "Manpower",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 89,
   "resumen": "RESUMEN: En Manpower nos encontramos en busqueda de Promotores de Venta para el Tottus de Canta Callao para importante marca del rubro de Tecnologia y Telecomunicaciones",
   "funciones": [
    "Generar el posicionamiento de la marca en el punto de venta.",
    "Mantener su área de trabajo ordenada.",
    "Abordaje a los clientes en el punto de venta presentando los beneficios de los productos."
   ],
   "requisitos": [
    "Experiencia en ventas o atención al cliente de cualquier tipo.",
    "Estar disponible para trabajar de lunes a domingo con un día de descanso, en horario rotativos de 8 horas diarias (aperturas y cierres).",
    "Disponibilidad para poder trabajar en el Tottus Canta Callao",
    "Contar con Carnet de sanidad de Lima / Callao vigente (o disponibilidad de tramitarlo)"
   ],
   "beneficios": [
    "Salario base de S/.1130 + Comisiones ilimitadas (segun desempeño en venta)",
    "Ingresar a nuestra planilla con todos tus beneficios de ley desde el primer día de trabajo.",
    "Línea de carrera en corto y largo plazo.",
    "Pertenecer a sólida organización con convenios corporativos y educativos.",
    "Capacitaciones gratuitas y constantes.",
    "Manpower es un empleador inclusivo y está feliz de promover activamente la inserción laboral de personas con habilidades diferentes. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: cambaceo, sales"
   ],
   "url": "https://www.laborum.pe/job/manpower/promotor-a-de-ventas-full-time-tottus-canta-callao/6ab0ef0b1aa686d090656cb1"
  },
  {
   "id": 80,
   "puesto": "Supervisor",
   "empresa": "Overall Strategy",
   "cat": "Ventas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 88,
   "resumen": "RESUMEN: Buscamos Promotores de Ventas MICAS de HIDROGEL para CLARO. Únete a una empresa líder en telecomunicaciones y marca la diferencia desde el primer día",
   "funciones": [
    "Gestión de ventas de MICAS y accesorios",
    "Abordar de manera constante a las personas que ingresen al CAC para vender y colocar micas",
    "Informar los beneficios del producto y captar clientes",
    "Usar la herramienta expendedora de micas",
    "Reportar avances al supervisor y cumplir con la asistencia asignada",
    "Horario y sede",
    "Lunes a Domingo con 1 día de descanso a la semana",
    "11:00 am a 8:00 pm",
    "CAC Minka (Tienda de Claro)"
   ],
   "requisitos": [
    "3 meses en ventas o atención al cliente (Deseable)",
    "Secundaria completa o estudios técnicos",
    "Actitud proactiva y disponibilidad inmediata",
    "Poder laborar en la zona de: Callao"
   ],
   "beneficios": [
    "Sueldo fijo S/.1200",
    "Comisiones S/.300",
    "Contrato inmediato",
    "Ingreso a planilla con beneficios de ley",
    "Capacitaciones constantes",
    "Línea de carrera real",
    "Acceso a programa de salud para ti y tu familia",
    "Programa de beneficios: descuentos educativos, tiendas y más",
    "Reconocimientos por desempeño y excelente clima laboral",
    "¡Postula hoy y comienza tu crecimiento profesional con nosotros!"
   ],
   "url": "https://www.laborum.pe/job/overall-strategy/minka-promotores-para-claro-c-s-experiencia-ingreso/6ab0e6501aa686d090654c9b"
  },
  {
   "id": 81,
   "puesto": "Trabajo inmediato Barranco! Anfitrión(a) de estacionamiento en Plaza Vea El Cortijo",
   "empresa": "APPARKA",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 86,
   "resumen": "RESUMEN: En apparka nos encontramos en búsqueda del mejor talento para el puesto de Anfitrión de Estacionamientos.",
   "funciones": [
    "Orientar y guiar al cliente para su ubicación y zona de acceso.",
    "Plaquear los vehículos que ingresan a nuestro estacionamiento.",
    "Realizar el cobro en caja, en caso corresponda.",
    "Informar y reportar las observaciones al supervisor.",
    "Realizar constantes rondas de seguridad dentro de la playa de estacionamiento.",
    "Apoyo con la limpieza de la playa"
   ],
   "requisitos": [
    "Personas con buena actitud, orientadas al servicio y dispuestas a aprender.",
    "Experiencia previa en atención al cliente (no es indispensable, ¡nosotros te capacitamos!).",
    "Compromiso con el trabajo en equipo y la puntualidad.",
    "Disponibilidad para laborar 8 horas de 1:30 pm a 10:30 pm de LUNES A DOMINGO CON UN DÍA DE DESACANSO"
   ],
   "beneficios": [
    "Sueldo: S/. 1130 mensual (NETO, SIN DESCUENTOS)",
    "Seguro privado MAFRE",
    "¡Oportunidad de línea de carrera a corto plazo!"
   ],
   "url": "https://www.laborum.pe/job/apparka/trabajo-inmediato-barranco-anfitrion-a-de-estacionamiento-en/6ab0ba9e1aa686d0906531e4"
  },
  {
   "id": 82,
   "puesto": "Atención al cliente por canal escrito",
   "empresa": "Apuesta Total",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 84,
   "resumen": "Si buscas una oportunidad para crecer en una empresa líder y formar parte de un equipo campeón, ¡postula y juega para ganar con nosotros!",
   "funciones": [
    "Atender a clientes vía WhatsApp y llamadas.",
    "Registrar jugadas solicitadas.",
    "Ofrecer y promocionar productos y servicios."
   ],
   "requisitos": [
    "Mayor de 18 años (indispensable).",
    "Experiencia en atención al cliente (deseable).",
    "Disponibilidad para turnos rotativos.",
    "Disponibilidad para trabajar en nuestra sede de Ventanilla (ex zona comercial).",
    "Disponibilidad de Lunes a Domingo con 1 día de descanso."
   ],
   "beneficios": [
    "Planilla desde el 1er día con beneficios de ley.",
    "Sueldo: S/ 1,130 + S/ 100 movilidad + S/ 100 riesgo de caja + tarjeta sodexo",
    "Pagos quincenales y fin de mes.",
    "Buen clima laboral.· ¡Forma parte de esta gran familia!",
    "En Apuesta Total, creemos firmemente en la igualdad de oportunidades para todas las personas, sin importar su género, edad, origen étnico, nacionalidad o condición. Promovemos activamente la diversidad y valoramos la inclusión de todas las personas, tal como lo establece la Ley N° 29973 modificada por la Ley 31707.",
    "Al postular, declaro haber leído y aceptado los términos establecidos en la Política de Privacidad para Postulantes, disponible en la página web de Apuesta Total.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/apuesta-total/atencion-al-cliente-por-canal-escrito-trabaja-en-ventanilla/6ab0e58c1aa686d09065498e"
  },
  {
   "id": 83,
   "puesto": "Anfitrión de Estacionamientos",
   "empresa": "APPARKA",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 5,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 83,
   "resumen": "RESUMEN: En apparka nos encontramos en búsqueda del mejor talento para el puesto de Anfitrión de Estacionamientos.",
   "funciones": [
    "Orientar y guiar al cliente para su ubicación y zona de acceso.",
    "Plaquear los vehículos que ingresan a nuestro estacionamiento.",
    "Realizar el cobro en caja, en caso corresponda.",
    "Informar y reportar las observaciones al supervisor.",
    "Realizar constantes rondas de seguridad dentro de la playa de estacionamiento.",
    "Apoyo con la limpieza de la playa"
   ],
   "requisitos": [
    "Personas con buena actitud, orientadas al servicio y dispuestas a aprender.",
    "Experiencia previa en atención al cliente (no es indispensable, ¡nosotros te capacitamos!).",
    "Disponibilidad para laborar en el turno tarde en horario fijo de 2:15pm a 11 pm",
    "Compromiso con el trabajo en equipo y la puntualidad."
   ],
   "beneficios": [
    "Sueldo: S/. 1130 mensual",
    "Planilla completa con todos los beneficios de ley",
    "¡Oportunidad de línea de carrera a corto plazo!"
   ],
   "url": "https://www.laborum.pe/job/apparka/con-o-sin-experiencia-trabaja-por-el-parque-kennedy/6ab080871aa686d09064fa97"
  },
  {
   "id": 84,
   "puesto": "Operador de Puente Grúa para Chilca",
   "empresa": "Eurofirms Perú",
   "cat": "Ingeniería",
   "min": 2200,
   "max": 2600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 6,
   "vence": "2026-10-20",
   "restan": 24,
   "score": 100,
   "resumen": "RESUMEN: En Eurofirms Group, creemos en el liderazgo por valores y el crecimiento continuo, poniendo a las personas y su desarrollo profesional primero, potenciando un ambiente de trabajo positivo y colaborativo.",
   "funciones": [
    "Operar el puente grúa de nave para trasladar bobinas de acero, paquetes de tubos, insumos y productos terminados.",
    "Verificar que los materiales recibidos coincidan con los formatos de recepción.",
    "Coordinar las maniobras de izaje y asegurar el uso correcto de las normas de seguridad.",
    "Horarios Disponibles",
    "Disponibilidad para trabajar 12 hrs: 7:30am a 7:30pm / 7:30pm a 7:30am."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 01 año como Operador de puente Grúa de 5 toneladas a más, con manejo de materia prima como bobinas y flejes..",
    "Contar con Curso vigente de Operador de Grúas (Certificación).",
    "Educación mínima: Educación Secundaria",
    "3 años de experiencia. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: operator, operario, operator, crane, grua, metalurgia, metalurgico, metallurgical, metallurgy"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día con todos los beneficios de ley.",
    "Salario básico de s/.2200- 2600 a más + Horas extras.",
    "Alimentación cubierta al 100% (almuerzo - cena).",
    "Movilidad de acercamiento (Ida y vuelta)",
    "Pagos quincenales.",
    "Grato ambiente laboral",
    "Línea de carrera."
   ],
   "url": "https://www.laborum.pe/job/eurofirms-peru/operador-de-puente-grua-para-chilca-gana-mas-2500-soles/6aaf44881aa686d09064799e"
  },
  {
   "id": 85,
   "puesto": "Supervisor(a) de Call Center y Ventas",
   "empresa": "EsTalent",
   "cat": "Ventas",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 6,
   "vence": "2026-10-20",
   "restan": 24,
   "score": 97,
   "resumen": "RESUMEN: En , consultora especializada en atracción de talento, conectamos a profesionales destacados con oportunidades de crecimiento en empresas que confían en nosotros para incorporar a los mejores talentos.",
   "funciones": [
    "Liderar, supervisar y acompañar al equipo de supervisores y asesores de la campaña de Portabilidad.",
    "Garantizar el cumplimiento de los objetivos comerciales y de productividad de la operación.",
    "Analizar indicadores de gestión y desempeño para identificar oportunidades de mejora.",
    "Diseñar e implementar planes de acción orientados al cumplimiento de metas.",
    "Realizar seguimiento diario a los indicadores operativos y comerciales.",
    "Acompañar al equipo en la gestión y cumplimiento de los objetivos establecidos.",
    "Cumplir con las políticas organizacionales y lineamientos establecidos por la compañía."
   ],
   "requisitos": [
    "Experiencia previa en supervisión y liderazgo de equipos comerciales.",
    "Experiencia en operaciones de Call Center y/o ventas.",
    "Deseable experiencia en campañas de Portabilidad.",
    "Capacidad para analizar indicadores de gestión y desempeño.",
    "Habilidades de liderazgo, comunicación y orientación a resultados.",
    "Capacidad para diseñar e implementar planes de acción.",
    "Disponibilidad para laborar de manera presencial.",
    "Disponibilidad para trabajar de lunes a sábado."
   ],
   "beneficios": [
    "Planilla MYPE durante los 3 primeros meses.",
    "Planilla completa desde el 4.º mes.",
    "Salario básico: S/ 1,400.",
    "Bono variable de ventas de acuerdo con el cumplimiento de indicadores de gestión.",
    "Bono de movilidad: S/ 200, sujeto a asistencia perfecta, sin tardanzas ni faltas injustificadas.",
    "Los bonos se pagan con un mes de desfase.",
    "Línea de carrera.",
    "Agradable clima laboral.",
    "Modalidad presencial en Surquillo.",
    "Horario de trabajo",
    "Lunes a sábado",
    "9:00 a. m. a 6:00 p. m."
   ],
   "url": "https://www.laborum.pe/job/estalent/supervisor-a-de-call-center-y-ventas-portabilidad-surquillo/6aaf671d1aa686d090649bde"
  },
  {
   "id": 86,
   "puesto": "Supervisor(a) de Canales Digitales",
   "empresa": "EsTalent",
   "cat": "Atención al Cliente",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 6,
   "vence": "2026-10-20",
   "restan": 24,
   "score": 97,
   "resumen": "RESUMEN: En , consultora especializada en atracción de talento, conectamos a profesionales destacados con oportunidades de crecimiento en empresas que confían en nosotros para incorporar a los mejores talentos.",
   "funciones": [
    "Proporcionar entrenamiento y asistencia continua a los asesores a su cargo.",
    "Asegurar que los asesores sigan las mejores prácticas de la compañía para la administración y operación del servicio.",
    "Realizar monitoreos diarios y seguimiento al desempeño de los asesores.",
    "Brindar feedback y acompañamiento para fortalecer las habilidades del equipo.",
    "Comunicar y reforzar los objetivos y lineamientos establecidos para la campaña.",
    "Identificar oportunidades de mejora en la gestión de los asesores y proponer acciones correctivas."
   ],
   "requisitos": [
    "Contar con mínimo 1 año de experiencia como Supervisor(a).",
    "Experiencia en Call Center, ventas o atención al cliente.",
    "Experiencia liderando equipos.",
    "Habilidades de liderazgo, comunicación y orientación a resultados.",
    "Capacidad para brindar feedback y acompañamiento al equipo.",
    "Disponibilidad para laborar de manera presencial en Surquillo.",
    "Disponibilidad para trabajar de lunes a domingo."
   ],
   "beneficios": [
    "Planilla MYPE durante los 3 primeros meses.",
    "Sueldo fijo: S/ 1,400.",
    "Bono de movilidad: S/ 200.",
    "Bono variable ilimitado.",
    "Modalidad presencial en Surquillo.",
    "Horario de lunes a domingo, de 11:00 a. m. a 8:00 p. m.",
    "Descansos rotativos.",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Si cuentas con experiencia como supervisor(a), tienes habilidades de liderazgo y buscas seguir creciendo dentro de una operación comercial, ¡queremos conocerte!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: callcenter, teleoperador, telefonista, telephonist, telefonico"
   ],
   "url": "https://www.laborum.pe/job/estalent/supervisor-a-de-canales-digitales-turno-tarde-call-center/6aaf679b1aa686d090649d73"
  },
  {
   "id": 87,
   "puesto": "Coordinador de servicio tecnico",
   "empresa": "HD Head Hunters",
   "cat": "Salud",
   "min": 5000,
   "max": 5500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 6,
   "vence": "2026-10-20",
   "restan": 24,
   "score": 95,
   "resumen": "La posición tendrá como principal desafío liderar la reestructuración y optimización estratégica y operativa del área, asegurando la continuidad y calidad del servicio técnico y fortaleciendo la gestión del equipo.",
   "funciones": [
    "Diagnosticar y optimizar los procesos, herramientas e inventario de repuestos del área.",
    "Diseñar e implementar procedimientos operativos (SOP) e indicadores de gestión (KPIs).",
    "Planificar y supervisar mantenimientos preventivos, correctivos y calibraciones de equipos médicos.",
    "Coordinar instalaciones, puestas en marcha, capacitaciones y validaciones técnicas en clientes.",
    "Supervisar el cumplimiento de tiempos de atención y niveles de servicio (SLA).",
    "Liderar, capacitar y evaluar al equipo técnico.",
    "Gestionar repuestos, garantías y coordinación técnica con fabricantes internacionales.",
    "Atender incidencias y escalamientos de clientes.",
    "Coordinar con el área comercial servicios de mantenimiento, contratos y repuestos."
   ],
   "requisitos": [
    "Bachiller o egresado de Ingeniería Biomédica, Ingeniería Electrónica, Ingeniería Industrial o carreras afines.",
    "3 a 5 años de experiencia en posiciones similares de coordinación o jefatura.",
    "Deseable experiencia en empresas de equipamiento médico, hospitales, clínicas o áreas de servicio técnico.",
    "Experiencia liderando equipos técnicos.",
    "Capacidad de análisis, planificación, resolución de problemas y orientación al cliente."
   ],
   "beneficios": [
    "Remuneración: S/ 5,000 a S/ 5,500.",
    "Planilla General, con todos los beneficios de ley.",
    "Horario: lunes a viernes de 8:30 a.m. a 5:30 p.m.",
    "Modalidad: presencial.",
    "Lugar: Santiago de Surco."
   ],
   "url": "https://www.laborum.pe/job/hd-head-hunters/coordinador-de-servicio-tecnico-equipos-medicos/6aaf3c3a1aa686d090645fe5"
  },
  {
   "id": 88,
   "puesto": "Asistente administrativo área comercial",
   "empresa": "PLANET GAME S.A.C.",
   "cat": "Administración",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 6,
   "vence": "2026-10-20",
   "restan": 24,
   "score": 95,
   "resumen": "Somos una empresa peruana con más de 15 años de experiencia en el rubro de entretenimiento y recreación, y nos encontramos en búsqueda de un(a):",
   "funciones": [
    "Gestionar y responder correos electrónicos con diferentes áreas, clientes, proveedores y Gerencia.",
    "Coordinar llamadas, reuniones y comunicaciones internas.",
    "Coordinar las visitas y desplazamientos de trabajadores hacia los diferentes lugares de instalación.",
    "Realizar seguimiento y coordinación de actividades administrativas.",
    "Gestionar y hacer seguimiento a renovaciones de contratos.",
    "Coordinar la documentación y renovación de contratos relacionados con los establecimientos donde la empresa desarrolla sus operaciones.",
    "Mantener comunicación y coordinación con diferentes áreas de la empresa."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en Administración o carreras afines.",
    "Experiencia mínima de 6 meses en funciones administrativas.",
    "Manejo de herramientas de Office.",
    "Buena comunicación, organización y capacidad de seguimiento.",
    "Disponibilidad para trabajar de manera presencial en Chorrillos Cedros."
   ],
   "beneficios": [
    "Sueldo S/ 1,500 + S/ 150 bono de asistencia perfecta.",
    "Planilla completa desde el primer día.",
    "Pagos mensuales.",
    "Horario fijo de lunes a viernes.",
    "Descanso sábados, domingos y feriados.",
    "Grato ambiente laboral.",
    "Oportunidad de crecimiento dentro de la empresa.",
    "¡Postula y forma parte de nuestro equipo!",
    "Planet Game S.A.C. | Área de Reclutamiento. -Requerimientos- Educación mínima: Técnico años de experienciaEdad: entre 23 y 34 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, administrative, zona, area, salesman, saleswoman, seller, vendedor, comercial, salesperson"
   ],
   "url": "https://www.laborum.pe/job/planet-game-s-a-c/asistente-administrativo-area-comercial/6aaf35931aa686d0906454d2"
  },
  {
   "id": 89,
   "puesto": "Atención al cliente",
   "empresa": "PASTIPAN",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 6,
   "vence": "2026-12-19",
   "restan": 84,
   "score": 91,
   "resumen": "Buscamos colaboradores multifuncionales (atención al cliente y manipulación de alimentos) para nuestras tiendas ubicadas en Gonzales Prada, Ejército y San Miguel.",
   "funciones": [
    "Manejo de caja, atención al cliente y ventas.",
    "Manipulación de alimentos (porcionado de tortas y pasteles)",
    "Recepción, reposición e inventario de productos de tienda.",
    "Limpieza y orden de toda la tienda.",
    "Recepción, reposición e inventario de productos de tienda",
    "Horneado de pan, preparación de sandwiches, jugos y ensaladas"
   ],
   "requisitos": [
    "Deseable experiencia en atención al cliente, juguerias, restaurantes, cafeterías o tiendas por conveniencia",
    "Actitud de servicio",
    "Disponibilidad para laborar en horario rotativo en turno de apertura (6 a.m. a 2 p.m) y cierre (1;30 p.m. a 9:30 p.m.)"
   ],
   "beneficios": [
    "Sueldo: S/ 1,130 + Bono de Movilidad S/ 135",
    "Ingreso a planilla desde el primer día",
    "Todos los beneficios de ley",
    "Pagos quincenales",
    "Capacitaciones constantes"
   ],
   "url": "https://www.bumeran.com.pe/empleos/colaborador-multifuncional-miraflores-san-miguel-magdalena-pastipan-1118448851.html"
  },
  {
   "id": 90,
   "puesto": "Almacenero de Repuestos",
   "empresa": "M&M Repuestos y Servicios S.A.",
   "cat": "Logística",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 6,
   "vence": "2026-10-20",
   "restan": 24,
   "score": 90,
   "resumen": "Selección y preparación de pedidos.",
   "funciones": [
    "Selección y preparación de pedidos.",
    "Mantenimiento de un ambiente de trabajo organizado y eficiente.",
    "Deberás trabajar en estrecha colaboración con el equipo de logística para asegurar que los pedidos se entreguen a tiempo y con precisión.",
    "Se requiere experiencia previa en roles similares, así como habilidades de comunicación efectiva y capacidad para trabajar bajo presión."
   ],
   "requisitos": [
    "Con estudios secundarios o técnicos.",
    "Experiencia mínima de 6 meses de haber laborado en almacenes.",
    "Manejo de excel a nivel básico."
   ],
   "beneficios": [
    "Remuneración acorde al mercado más horas extras",
    "Ingreso a planilla desde el primer día con todos los beneficios de ley.",
    "Buen clima laboral.",
    "Sueldo s/1,600.00 de básico + s/175.00 de movilidad",
    "HORARIO DE TRABAJO",
    "Lunes a Viernes de 08:30 AM A 18:00 PM. Y sábados: 8.30 AM A 14:00 PM más horas extras.",
    "Lugar de trabajo: ATE- AV. TAPICEROS N° 280, altura de la Av. Separadora Industrial con frutales, a espaldas de la clínica Montefiori de Ate",
    "En M&M Repuestos y Servicios S.A., valoramos a nuestros empleados y buscamos personas comprometidas con su trabajo y con el éxito de la empresa.",
    "Si crees que tienes lo que se necesita, ¡únete a nuestro equipo y contribuye al éxito de M&M Repuestos y Servicios S.A.! Ofrecemos un ambiente de trabajo positivo y oportunidades de crecimiento profesional.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 años"
   ],
   "url": "https://www.laborum.pe/job/m-m-repuestos-y-servicios-s-a/almacenero-de-repuestos/6aaf38b11aa686d090645655"
  },
  {
   "id": 91,
   "puesto": "Atención al cliente",
   "empresa": "TECHNETSOL SOCIEDAD ANONIMA CERRADA",
   "cat": "Ventas",
   "min": 4800,
   "max": 4800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 6,
   "vence": "2026-12-18",
   "restan": 83,
   "score": 89,
   "resumen": "¿Buscas generar altos ingresos según tu desempeño? ¡Únete a nuestro equipo!",
   "funciones": [
    "Promocionar y colocar créditos empresariales (Banca Pyme).",
    "Gestionar y desarrollar una cartera de clientes (incluimos base de datos).",
    "La mayor parte del trabajo es ventas en campo (visitas a negocios).",
    "Seguimiento de clientes potenciales y cierre de ventas.",
    "Trabajo independiente, sin horario fijo ni relación de dependencia (horarios flexiboles según tu disponibilidad)."
   ],
   "requisitos": [
    "Experiencia mínima deseable de 6 meses en ventas o atención al cliente (finanzas, seguros, telecomunicaciones o intangibles).",
    "Habilidades de comunicación, negociación y cierre de ventas.",
    "Capacidad de organización.",
    "Experiencia en prospección y manejo de cartera de clientes."
   ],
   "beneficios": [
    "Ingresos 100% por comisiones, sin límite de ventas.",
    "Ganancias de hasta S/ 4,800 por operación.",
    "Bonos por desempeño.",
    "Capacitación constante.",
    "Soporte y acompañamiento personalizado por nuestros supervisores.",
    "Flexibilidad total para organizar tu trabajo.",
    "¡Postula y empieza a generar ingresos sin límites!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedores-para-banca-pyme-scotiabank-lima-sur-fuerza-de-ventas-externa-technetsol-sociedad-anonima-cerrada-1118448843.html"
  },
  {
   "id": 92,
   "puesto": "Técnico Electricista S",
   "empresa": "Confidencial",
   "cat": "Ingeniería",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 6,
   "vence": "2026-11-19",
   "restan": 54,
   "score": 87,
   "resumen": "Nos encontramos en la Búsqueda de 02 Técnicos de Electricistas:",
   "funciones": [
    "Ejecutar mantenimiento predictivo, preventivo y correctivo de los equipos, maquinarias e infraestructura.",
    "Ejecutar y desarrollar las implementaciones en plantas y proyectos de acuerdo a las especificaciones del Jefe de Mantenimiento.",
    "Garantizar la oportuna atención de requerimientos de la planta.",
    "Solicitar al Jefe de Mantenimiento los suministros requeridos para el trabajo.",
    "Otras responsabilidades indicadas en la documentación del Sistema de Gestión implementado."
   ],
   "requisitos": [
    "Técnico en Mantenimiento, electricidad y/o Mecánica",
    "6 meses de experiencia desempeñando funciones de mantenimiento",
    "Conocimientos sobre soldadura, electricidad, mantenimiento mecánico y maquinaria industrial.",
    "Conocimiento de BPM y HACCP"
   ],
   "beneficios": [
    "Sueldo Básico: S/. 1600.00 + 400 + Horas extras + Bono nocturno + Movilidad y Alimentación",
    "Planilla Completa",
    "Horarios Rotativos por semana",
    "Horario Mañana: 7:00 am - 4:00 pm Horario Noche: 11:00 pm - 7:00 am"
   ],
   "url": "https://www.bumeran.com.pe/empleos/tecnico-electricista-s-.-2800-horarios-rotativos-1118448718.html"
  },
  {
   "id": 93,
   "puesto": "Atención al cliente",
   "empresa": "Ablimatex Export SAC",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 6,
   "vence": "2026-10-20",
   "restan": 24,
   "score": 84,
   "resumen": "RESUMEN: En Tienda Hormiguita, nos encontramos en la búsqueda de vendedoras para nuestra tienda de ropa para bebés y niños, ubicada en el Centro Comercial Plaza Norte.",
   "funciones": [
    "Brindar información y asesoría adecuada sobre los productos a los clientes.",
    "Realizar el cobro de ventas en el punto de venta (manejo de POS).",
    "Mantener el orden y limpieza de la tienda y almacén.",
    "Recepcionar mercadería y realizar la reposición de productos en tienda."
   ],
   "requisitos": [
    "Secundaria completa",
    "Experiencia en ventas o atención al cliente (deseable)",
    "Disponibilidad para trabajar en horarios rotativos",
    "Proactividad y buen trato al cliente",
    "Full Time (rotativo)",
    "10:00 am a 7:00 pm",
    "2:00 pm a 10:00 pm",
    "Part Time (rotativo)",
    "10:00 am a 2:00 pm",
    "6:00 pm a 10:00 pm"
   ],
   "beneficios": [
    "Sueldo S/1130 (full time)",
    "Incentivos por ventas",
    "Pagos quincenales",
    "SCTR + Seguro Vida Ley",
    "Buen ambiente laboral. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 20 añosPalabras clave: boutique, tienda, store, establecimiento, bazar, bazzar"
   ],
   "url": "https://www.laborum.pe/job/ablimatex-export-sac/vendedora-de-tienda-full-time-part-time-plaza-norte/6aaf41f51aa686d09064707e"
  },
  {
   "id": 94,
   "puesto": "Consultor Comercial de Innovación y Transformación Digital PYMES",
   "empresa": "Capitale Consultores",
   "cat": "Ventas",
   "min": 2500,
   "max": 3500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 7,
   "vence": "2026-12-18",
   "restan": 83,
   "score": 100,
   "resumen": "¿Tienes facilidad para relacionarte con empresarios, identificar oportunidades y cerrar negocios?",
   "funciones": [
    "Participar activamente en reuniones, encuentros individuales y eventos de networking empresarial, representando a la empresa y fortaleciendo su red de contactos.",
    "Contactar y desarrollar relaciones con empresarios, prospectos y referidos pertenecientes a una amplia red de potenciales clientes.",
    "Identificar las necesidades y oportunidades de cada empresa para ofrecerle la solución más adecuada.",
    "Realizar reuniones de diagnóstico, presentar propuestas comerciales y conducir las oportunidades hasta el cierre.",
    "Gestionar disciplinadamente el pipeline comercial, actualizar el CRM y efectuar el seguimiento necesario para cumplir las metas de venta."
   ],
   "requisitos": [
    "Profesional universitario o egresado de Administración, Marketing, Negocios, Ingeniería Industrial, Comunicaciones o carreras afines.",
    "Experiencia mínima de dos años en ventas B2B, desarrollo de negocios o atención comercial a empresas.",
    "Conocimiento general del funcionamiento de las empresas y capacidad para comprender necesidades de distintas áreas del negocio.",
    "Experiencia o conocimientos en prospección, diagnóstico comercial, presentación de propuestas, seguimiento, negociación y cierre.",
    "Manejo de CRM y herramientas de Office o Google Workspace.",
    "Disponibilidad para participar en reuniones virtuales y presenciales, encuentros de networking y eventos empresariales.",
    "Experiencia en innovación, fondos concursables o ProInnóvate (No indispensable)."
   ],
   "beneficios": [
    "Remuneración fija más ingreso variable por resultados.",
    "Contratación en planilla con los beneficios de ley correspondientes al régimen laboral REMYPE .",
    "Modalidad mixta: trabajo virtual y asistencia presencial a reuniones comerciales, actividades de networking y eventos.",
    "Horario: lunes a jueves de 8:00 a. m. a 6:00 p. m. y viernes de 8:00 a. m. a 5:00 p. m.",
    "Lugar de trabajo: Lima.",
    "Acceso a una red de networking activa y a una amplia cartera de potenciales clientes y referidos.",
    "Bono trimestral por cumplimiento de metas comerciales, adicional a su esquema de comisiones.",
    "Bono de permanencia.",
    "Un plan de carrera, con proyección de crecimiento dentro del área comercial conforme la empresa siga escalando."
   ],
   "url": "https://www.bumeran.com.pe/empleos/consultor-comercial-de-innovacion-y-transformacion-digital-pymes-capitale-consultores-1118448455.html"
  },
  {
   "id": 95,
   "puesto": "Ejecutivo(a) Comercial",
   "empresa": "Capitale Consultores",
   "cat": "Ventas",
   "min": 2500,
   "max": 3500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 7,
   "vence": "2026-12-18",
   "restan": 83,
   "score": 100,
   "resumen": "Gestionar el proceso comercial de la línea de dispositivos y mobiliario médico, incluyendo equipamiento de cadena de frío, refrigeradoras para vacunas, congeladores y otros equipos especializados, orientados a clínicas, hospitales, laboratorios y farmacias.",
   "funciones": [
    "Gestionar el proceso comercial de la línea de dispositivos y mobiliario médico, incluyendo equipamiento de cadena de frío, refrigeradoras para vacunas, congeladores y otros equipos especializados, orientados a clínicas, hospitales, laboratorios y farmacias.",
    "Prospectar y desarrollar nuevas oportunidades de negocio en empresas y entidades del sector salud.",
    "Cumplir con los objetivos y metas de ventas asignados.",
    "Gestionar y fortalecer la cartera de clientes.",
    "Elaborar y dar seguimiento a cotizaciones hasta el cierre de ventas.",
    "Realizar visitas y reuniones comerciales en empresas de Lima Metropolitana.",
    "Mantener actualizada la información en CRM y reportes comerciales."
   ],
   "requisitos": [
    "Técnico o universitario (egresado o estudiante de últimos ciclos) en Administración, Marketing, Gestión Comercial, Negocios Internacionales, Ingeniería Industrial, Tecnología Médica, Enfermería o carreras afines.",
    "Mínimo un año de experiencia en ventas, prospección comercial o atención al cliente en empresas comerciales orientadas al sector salud.",
    "Deseable experiencia en ventas B2B.",
    "Manejo de Excel a nivel intermedio.",
    "Deseable experiencia con CRM."
   ],
   "beneficios": [
    "Modalidad de trabajo híbrida: visitas a clientes. Reuniones semanales en oficina ubicada en Lince.",
    "Sueldo básico + comisiones + bonos por cumplimiento de metas",
    "Ingreso a planilla MYPE - Pequeña Empresa (https://capitale.pe/webcapitale/2026/09/01/regimen-general-mype-y-remype-que-beneficios-laborales-te-corresponden/)",
    "Capacitación continua.",
    "Oportunidades de desarrollo y línea de carrera.",
    "Excelente ambiente de trabajo.",
    "Sistema de incentivos y reconocimientos por resultados."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivoa-comercial--clinicas-farmacias-y-laboratorios-capitale-consultores-1118448456.html"
  },
  {
   "id": 96,
   "puesto": "Auxiliar contable",
   "empresa": "ARCLAD DEL PERU S.A.C.",
   "cat": "Contabilidad",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 7,
   "vence": "2026-10-19",
   "restan": 23,
   "score": 97,
   "resumen": "Horario: Lunes a viernes de 8:00 a.m. a 5:30 p.m. y sábados de 9:00 a.m. a 1:00 p.m.",
   "funciones": [
    "Brindar soporte en los procesos administrativos y operativos del área de Administración.",
    "Realizar trámites y gestiones ante entidades públicas y privadas: municipalidades, SUNARP, SAT, Defensa Civil, servicios públicos, permisos y licencias, entre otros.",
    "Dar soporte en la gestión y seguimiento de mantenimientos generales de la empresa (fumigación, aire acondicionado, extintores, pozo a tierra, etc.).",
    "Gestionar compras locales menores y requerimientos administrativos.",
    "Revisar y registrar documentos de gastos operativos en el sistema Oracle NetSuite.",
    "Realizar provisiones y registros contables de gastos fijos y servicios.",
    "Registrar asientos contables y provisiones asignadas, así como sus respectivos extornos.",
    "Revisar, validar y gestionar el reembolso de viáticos nacionales y extranjeros.",
    "Apoyar en los procesos de cierre contable y registros administrativos.",
    "Registrar facturas de Leasing Financiero y seguros en el sistema.",
    "Revisar y controlar los gastos realizados mediante tarjetas de crédito empresariales.",
    "Brindar apoyo en operaciones de Tesorería y Cobranzas durante períodos de suplencia."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en curso de Contabilidad, Administración o carreras afines.",
    "Experiencia mínima de 1 año realizando funciones administrativas, contables u operativas similares.",
    "Manejo de Microsoft Office a nivel intermedio o avanzado.",
    "Manejo de algún ERP; deseable experiencia con Oracle NetSuite.",
    "Persona organizada, responsable, proactiva y con capacidad de seguimiento.",
    "Disponibilidad para trabajar en Ate."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Sueldo fijo: S/ 1,500.",
    "Movilidad: S/ 200.",
    "Capacitaciones constantes.-Requerimientos- Educación mínima: Técnico2 años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, contador, pagador, contralor, accountant, facturista, tesorero, contable"
   ],
   "url": "https://www.laborum.pe/job/arclad-del-peru-s-a-c/auxiliar-contable-ate/6aade1a7be522280f1ebd5fe"
  },
  {
   "id": 97,
   "puesto": "Asesor(a) Comercial Educativo",
   "empresa": "IBC INSTITUTE",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 7,
   "vence": "2026-10-19",
   "restan": 23,
   "score": 94,
   "resumen": "RESUMEN: ASESOR(A) COMERCIAL EDUCATIVO LINCE",
   "funciones": [
    "Participar en conferencias con clientes interesados en nuestros programas.",
    "Brindar información y asesoría sobre nuestros programas de capacitación.",
    "Resolver dudas y manejar objeciones.",
    "Realizar seguimiento a clientes interesados.",
    "Concretar matrículas y cumplir objetivos comerciales.",
    "No realizarás ventas en frío: trabajarás con personas que previamente han mostrado interés en nuestros programas."
   ],
   "requisitos": [
    "Experiencia en ventas, atención al cliente o call center comercial.",
    "Facilidad de comunicación y capacidad de persuasión.",
    "Orientación a resultados y cumplimiento de metas.",
    "Proactividad, compromiso y actitud comercial.",
    "Disponibilidad para trabajar de manera presencial en Lince."
   ],
   "beneficios": [
    "S/ 1,300 de sueldo fijo.",
    "Múltiples bonos por cumplimiento de objetivos.",
    "Posibilidad de superar los S/ 3,000 mensuales según resultados.",
    "Planilla completa desde el primer día.",
    "Capacitación comercial constante.",
    "Oportunidad de crecimiento y línea de carrera.",
    "Lunes a viernes: 8:00 a. m. 6:00 p. m.",
    "Sábados: 9:00 a. m. 12:00 p. m.",
    "Descanso: domingos y feriados.",
    "Lugar de trabajo: Lince"
   ],
   "url": "https://www.laborum.pe/job/ibc-institute/asesor-a-comercial-educativo-lince/6aadee7abe522280f1ec0b1c"
  },
  {
   "id": 98,
   "puesto": "Operario de Planta e Industria",
   "empresa": "Concyssa Industrial S.A.C",
   "cat": "Ingeniería",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 7,
   "vence": "2026-10-19",
   "restan": 23,
   "score": 92,
   "resumen": "RESUMEN: ¡En CONCYSSA INDUSTRIAL te estamos buscando!",
   "funciones": [
    "Cargar materiales a las máquinas y brindar apoyo directo al maquinista en la línea de producción.",
    "Ensamble, empaque, etiquetado y embolsado de productos plásticos.",
    "Registro de partes diarios de producción.",
    "Mantener el orden, limpieza y seguridad en el área de trabajo."
   ],
   "requisitos": [
    "Experiencia: como operario en plantas industriales, fábricas, producción, manufactura o almacén pesado.",
    "Educación: Secundaria completa (indispensable).",
    "Disponibilidad: Para trabajar en turnos rotativos de 12 horas (Día y Noche).",
    "Residencia: De preferencia vivir en San Luis o distritos aledaños (La Victoria, Ate, El Agustino, Santa Anita, etc.)."
   ],
   "beneficios": [
    "Ingreso a Planilla Completa (Régimen General): CTS, Gratificaciones, Utilidades, Vacaciones pagadas. ¡Todo bajo ley desde tu primer día!",
    "Remuneración: S/ 1,400 aprox. al mes + PAGO DE HORAS EXTRAS AL 100%.",
    "Pagos puntuales: Esquema quincenal y fin de mes.",
    "Seguridad garantizada: Seguro de EsSalud, Vida Ley y SCTR desde el primer día.",
    "Beneficios Corporativos: Giftcard de cumpleaños y descuentos especiales.",
    "Desarrollo profesional: Capacitaciones constantes, programa de formación interna y línea de carrera.",
    "Programa de reconocimiento: Premiamos tu desempeño, puntualidad y compromiso.",
    "JORNADA LABORAL",
    "Horario 4x3: Trabajas 4 días a la semana (jornadas de 12 horas) y gozas de 3 días libres seguidos.",
    "Turnos: Rotativos de 12 horas (Día y Noche).",
    "Lugar de trabajo: Planta San Luis (Jr. Felipe Santiago Salaverry 148, Urb. El Pino A 1 cdra. del Arco de San Luis)."
   ],
   "url": "https://www.laborum.pe/job/concyssa-industrial-s-a-c/operario-de-planta-e-industria-trabaja-4-dias-y-descansa-3/6aae133bbe522280f1ec893d"
  },
  {
   "id": 99,
   "puesto": "Anfitriona , Impulsadora de ventas",
   "empresa": "HVA INVERSIONES",
   "cat": "Ventas",
   "min": 950,
   "max": 950,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 7,
   "vence": "2026-10-19",
   "restan": 23,
   "score": 91,
   "resumen": "Dar la bienvenida a los clientes y dirigirlos a sus respectivas mesas.",
   "funciones": [
    "Dar la bienvenida a los clientes y dirigirlos a sus respectivas mesas.",
    "Impulsar la venta mediante repartición de volantes, o fomentar el ingreso de comensales.",
    "Despedir a los clientes.",
    "Coordinar las reservas y coordinar la disponibilidad de mesas con los mozos.",
    "Aprenderse la carta de platos.",
    "Otros asignados por el Administrador.",
    "Lunes a Domingo (un día de descanso)",
    "HORARIO PART TIME: 11 pm a 6 pm S/1000.00",
    "HORARIO PART TIME NOCHE: 5 PM A 11 PM S/950.00",
    "HORARIO FULL: 12 pm a 10 pm S/1500.00. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 19 y 35 añosPalabras clave: cambaceo, sales"
   ],
   "requisitos": [
    "Facilidad de palabra.",
    "Actitud positiva",
    "Capacidad de tolerancia.",
    "Adjuntar foto reciente (indispensable)",
    "Buena presencia -Impulsadora de Venta.",
    "Vivir cerca al distrito de Cercado de Lima."
   ],
   "beneficios": [
    "Sueldo desde S/. 950.00 según evaluación y experiencia",
    "Ingreso a planilla y asignación familiar.",
    "Pagos quincenales.",
    "Alimentación cubierta.",
    "Pago doble por feriados laborados.",
    "Comisiones individuales y semanales.",
    "30% de descuento en consumos (en nuestros locales).",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Bonos por metas mensuales.",
    "Un día de descanso a la semana."
   ],
   "url": "https://www.laborum.pe/job/hva-inversiones/anfitriona-impulsadora-de-ventas-cerca-a-la-plaza-de-armas/6aae04febe522280f1ec5d1b"
  },
  {
   "id": 100,
   "puesto": "Asesor(a) Comercial",
   "empresa": "IBC INSTITUTE",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 7,
   "vence": "2026-10-19",
   "restan": 23,
   "score": 91,
   "resumen": "RESUMEN: ¿TE GUSTAN LAS VENTAS Y QUIERES GANAR MÁS SEGÚN TUS RESULTADOS?",
   "funciones": [
    "Tu principal función será convertir el interés de nuestros clientes en matrículas.",
    "Participar en conferencias dirigidas a personas interesadas en nuestros programas.",
    "Brindar información y asesoría sobre nuestra oferta educativa.",
    "Resolver dudas y manejar objeciones.",
    "Realizar seguimiento a clientes interesados.",
    "Aplicar técnicas de venta y cierre.",
    "Cumplir objetivos comerciales.",
    "No realizarás ventas en frío. Trabajarás con clientes que previamente han mostrado interés en nuestros programas."
   ],
   "requisitos": [
    "Experiencia en ventas, atención al cliente o call center comercial.",
    "Buena comunicación y capacidad de persuasión.",
    "Orientación a resultados.",
    "Habilidad para negociar y manejar objeciones.",
    "Actitud comercial y compromiso.",
    "Disponibilidad para trabajar presencialmente en Lince.",
    "Lunes a viernes: 8:00 a. m. a 6:00 p. m.",
    "Sábados: 9:00 a. m. a 12:00 p. m.",
    "Descanso: domingos y feriados.",
    "Lugar de trabajo: Lince",
    "Si tienes experiencia en ventas y buscas una oportunidad con planilla desde el primer día, sueldo fijo y bonos por resultados, postula ahora.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: salesman, saleswoman, seller, vendedor, comercial, salesperson"
   ],
   "beneficios": [
    "S/ 1,300 de sueldo fijo.",
    "Planilla completa desde el primer día.",
    "Múltiples bonos por cumplimiento de objetivos.",
    "Posibilidad de generar ingresos superiores a S/ 3,000 mensuales según resultados.",
    "Capacitación comercial constante.",
    "Desarrollo y línea de carrera."
   ],
   "url": "https://www.laborum.pe/job/ibc-institute/asesor-a-comercial-gana-3000-planilla-completa/6aaded1dbe522280f1ec0505"
  },
  {
   "id": 101,
   "puesto": "Asesor de Negocios",
   "empresa": "TECHNETSOL SOCIEDAD ANONIMA CERRADA",
   "cat": "Ventas",
   "min": 4800,
   "max": 4800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 7,
   "vence": "2026-12-17",
   "restan": 82,
   "score": 89,
   "resumen": "¿Buscas generar altos ingresos según tu desempeño? ¡Únete a nuestro equipo!",
   "funciones": [
    "Promocionar y colocar créditos empresariales (Banca Pyme).",
    "Gestionar y desarrollar una cartera de clientes (incluimos base de datos).",
    "La mayor parte del trabajo es ventas en campo (visitas a negocios).",
    "Seguimiento de clientes potenciales y cierre de ventas.",
    "Trabajo independiente, sin horario fijo ni relación de dependencia (horarios flexiboles según tu disponibilidad)."
   ],
   "requisitos": [
    "Experiencia mínima deseable de 6 meses en ventas o atención al cliente (finanzas, seguros, telecomunicaciones o intangibles).",
    "Habilidades de comunicación, negociación y cierre de ventas.",
    "Capacidad de organización.",
    "Experiencia en prospección y manejo de cartera de clientes."
   ],
   "beneficios": [
    "Ingresos 100% por comisiones, sin límite de ventas.",
    "Ganancias de hasta S/ 4,800 por operación.",
    "Bonos por desempeño.",
    "Capacitación constante.",
    "Soporte y acompañamiento personalizado por nuestros supervisores.",
    "Flexibilidad total para organizar tu trabajo.",
    "¡Postula y empieza a generar ingresos sin límites!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-negocios-banca-pyme-scotiabank-agencia-arriola-fuerza-de-ventas-externa-technetsol-sociedad-anonima-cerrada-1118448639.html"
  },
  {
   "id": 102,
   "puesto": "Asesor de Negocios",
   "empresa": "TECHNETSOL SOCIEDAD ANONIMA CERRADA",
   "cat": "Ventas",
   "min": 4800,
   "max": 4800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Bumeran",
   "dias": 7,
   "vence": "2027-01-17",
   "restan": 113,
   "score": 89,
   "resumen": "¿Buscas generar altos ingresos según tu desempeño? ¡Únete a nuestro equipo!",
   "funciones": [
    "Promocionar y colocar créditos empresariales (Banca Pyme).",
    "Gestionar y desarrollar una cartera de clientes (incluye base de datos).",
    "La mayor parte del trabajo es ventas en campo (visitas a negocios).",
    "Seguimiento de clientes potenciales y cierre de ventas.",
    "Trabajo independiente, sin horario fijo ni relación de dependencia (horarios flexiboles según tu disponibilidad)."
   ],
   "requisitos": [
    "Experiencia mínima de 6 meses en ventas o atención al cliente (finanzas, seguros, telecomunicaciones o intangibles).",
    "Habilidades de comunicación, negociación y cierre de ventas.",
    "Capacidad de organización.",
    "Experiencia en prospección y manejo de cartera de clientes."
   ],
   "beneficios": [
    "Ingresos 100% por comisiones, sin límite de ventas.",
    "Ganancias de hasta S/ 4,800 por operación.",
    "Bonos por desempeño.",
    "Capacitación constante.",
    "Soporte y acompañamiento personalizado por nuestros supervisores.",
    "Flexibilidad total para organizar tu trabajo.",
    "¡Postula y empieza a generar ingresos sin límites!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-negocios-banca-pyme-scotiabank-agencia-faucett-fuerza-de-ventas-externa-technetsol-sociedad-anonima-cerrada-1118448642.html"
  },
  {
   "id": 103,
   "puesto": "Cocinero",
   "empresa": "USMP",
   "cat": "Gastronomía",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 7,
   "vence": "2026-10-19",
   "restan": 23,
   "score": 89,
   "resumen": "RESUMEN: Lugar de trabajo: Surquillo, Lima",
   "funciones": [
    "Preparar el menú diario para un promedio de 50 personas (personal administrativo de la empresa).",
    "Proponer menús variados, balanceados y atractivos semanalmente.",
    "Elaborar coffee breaks según los requerimientos de la empresa.",
    "Mantener la limpieza, orden y salubridad de la cocina cumpliendo los estándares establecidos.",
    "Supervisar el adecuado almacenamiento y conservación de los insumos.",
    "Coordinar con el área administrativa los requerimientos de insumos y presupuesto."
   ],
   "requisitos": [
    "Egresado tecnico 3 años S/ 1,500",
    "Técnico titulado 3 años en Gastronomía o Cocina S/ 1,800",
    "Bachiller S/ 2,000",
    "Titulado S/ 2,800",
    "Maestría S/ 3,000",
    "(Todos los grados deben estar registrados en SUNEDU).",
    "Experiencia mínima: 2 años en cocina criolla y preferiblemente en concesionarios de alimentos.",
    "Conocimientos en buenas prácticas de manipulación de alimentos.",
    "Capacidad para proponer menús creativos y balanceados.",
    "Debe adjuntar su certificado en el CV"
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Excelente ambiente laboral.",
    "Horario fijo (no rotativo).-Requerimientos- Educación mínima: Universitario2 años de experienciaEdad: entre 25 y 50 añosConocimientos: Gestión de equipos, OrganizaciónPalabras clave: chef, cocinero, university, college, facultad, campus, universidad"
   ],
   "url": "https://www.laborum.pe/job/usmp/cocinero-universidad/6aade2dcbe522280f1ebdc22"
  },
  {
   "id": 104,
   "puesto": "Asistente de ventas",
   "empresa": "Industrias Nacionales de Metales SAC",
   "cat": "Ventas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 7,
   "vence": "2026-10-19",
   "restan": 23,
   "score": 89,
   "resumen": "RESUMEN: Empresa líder en el sector Comercial ubicada en Villa el Salvador requiere cubrir el puesto de Asistente de Ventas, de preferencia que resida en el Villa el Salvador o cerca de la zona Sur de Lima.",
   "funciones": [
    "Atención a clientes vía Telefónica",
    "Contactar cartera asignada y ofrecer productos de la empresa",
    "Enviar publicidad, catálogos, información por e-mail a potenciales clientes",
    "Cotizar a clientes y lograr cierre de venta",
    "Otras funciones asignadas por la jefatura"
   ],
   "requisitos": [
    "Experiencia mínima de 1 año ocupando el cargo o similares.",
    "Manejo de Office a nivel usuario, Internet y correo electrónico",
    "Organizada, con deseos de superación y buen trato a clientes"
   ],
   "beneficios": [
    "Remuneración fija S/1200+ Bonos por Metas",
    "Planilla desde el primer día, Gratificaciones, CTS, vacaciones y utilidades.",
    "Regimen General",
    "Horario de Trabajo de lunes a viernes 8am a 5:45pm. -Requerimientos- Educación mínima: Técnico1 año de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, cambaceo, sales"
   ],
   "url": "https://www.laborum.pe/job/industrias-nacionales-de-metales-sac/asistente-de-ventas/6aae082bbe522280f1ec6772"
  },
  {
   "id": 105,
   "puesto": "Auxiliar de RRHH",
   "empresa": "Confidencial",
   "cat": "Recursos Humanos",
   "min": 800,
   "max": 800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 7,
   "vence": "2026-11-18",
   "restan": 53,
   "score": 85,
   "resumen": "Colaborar en la gestión de publicidad",
   "funciones": [
    "Colaborar en la gestión de publicidad",
    "Apoyar en el manejo y organización de documentación relevante del área",
    "Atender y gestionar la recepción de comunicaciones y consultas",
    "Asistir en diversas tareas administrativas propias del departamento de Recursos Humanos"
   ],
   "requisitos": [
    "Interés y disposición para aprender sobre gestión de Recursos Humanos.",
    "Habilidades de organización y atención al detalle.",
    "Capacidad para comunicarse de manera efectiva.",
    "Proactividad y actitud positiva.",
    "Se valorará, pero no es excluyente, experiencia previa en roles administrativos o de apoyo."
   ],
   "beneficios": [
    "Horarios flexibles adaptados a tus necesidades.",
    "Medio tiempo",
    "Oportunidades de aprendizaje y desarrollo profesional.",
    "Pagos semanales.",
    "Sueldo desde S/. 800.00, hasta S/ 1,200.00 (4horas).",
    "Bonos de productividad"
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-de-rrhh-1118448478.html"
  },
  {
   "id": 106,
   "puesto": "Operario (a) de Limpieza",
   "empresa": "Multimoldes S.A.C.",
   "cat": "Ingeniería",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 8,
   "vence": "2026-10-18",
   "restan": 22,
   "score": 97,
   "resumen": "RESUMEN: MULTIMOLDES S.A.C., empresa industrial dedicada a la fabricación de envases y productos de papel, se encuentra en búsqueda de:",
   "funciones": [
    "Realizar la limpieza y desinfección de las diferentes áreas de la empresa.",
    "Mantener limpios y ordenados los servicios higiénicos, oficinas y áreas comunes.",
    "Realizar la correcta disposición y retiro de residuos.",
    "Mantener en buen estado los materiales y equipos asignados para la limpieza.",
    "Cumplir con las normas de seguridad, higiene y procedimientos establecidos por la empresa.",
    "Otras funciones relacionadas con el puesto."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia en labores de limpieza, de preferencia en plantas industriales, almacenes u oficinas.",
    "Disponibilidad para trabajar de lunes a sábado.",
    "Disponibilidad para realizar horas extras cuando se requiera.",
    "Persona responsable, puntual, ordenada y comprometida.",
    "Disponibilidad para ingreso inmediato.",
    "De preferencia, residir en San Juan de Miraflores o distritos cercanos."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,300",
    "Bono de asistencia: S/ 100",
    "Bono de productividad: S/ 200",
    "Pago de horas extras según corresponda",
    "Ingreso a planilla con todos los beneficios de ley.",
    "Pagos puntuales.",
    "Buen ambiente de trabajo.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 25 y 35 añosPalabras clave: operator, operario, operator, aseo, higiene, cleaning, limpieza"
   ],
   "url": "https://www.laborum.pe/job/multimoldes-s-a-c/operario-a-de-limpieza-exp-en-plantas-industriales/6aacc87ed32c1dca67158d0f"
  },
  {
   "id": 107,
   "puesto": "Conductor",
   "empresa": "APPARKA",
   "cat": "Logística",
   "min": 565,
   "max": 565,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 8,
   "vence": "2026-10-18",
   "restan": 22,
   "score": 95,
   "resumen": "RESUMEN: Nos encontramos en búsqueda de un Conductor A1 para la posición de Valet Parking PartTime, para nuestra sede ubicada en Edificio Anglo Cavenecia, San Isidro.",
   "funciones": [
    "Recepcionar, trasladar y estacionar los vehículos de manera segura.",
    "Verificar y registrar las condiciones en las que ingresan los vehículos.",
    "Entregar los vehículos a los clientes en las mismas condiciones en las que fueron recibidos.",
    "Registrar y resguardar los objetos de valor dejados en los vehículos, de acuerdo con los procedimientos establecidos.",
    "Brindar una atención cordial y una adecuada bienvenida a los clientes.",
    "Agradecer a los clientes por su visita.",
    "Informar al supervisor sobre cualquier incidencia o anomalía identificada durante el servicio.",
    "Cobro en caja."
   ],
   "requisitos": [
    "Licencia de conducir A1 vigente.",
    "Disponibilidad para trabajar a tiempo completo.",
    "Experiencia en conducción de vehículos mecánicos y automáticos.",
    "Experiencia en atención al cliente, de preferencia.",
    "Experiencia en manejo de caja, de preferencia.",
    "Cumplir con los requisitos establecidos para la posición."
   ],
   "beneficios": [
    "Sueldo mensual de S/ 565 + propinas.",
    "Ingreso a planilla desde el primer día de labores.",
    "Todos los beneficios de ley: gratificaciones, vacaciones, asignación familiar, seguro, CTS, liquidación y utilidades.",
    "Uniforme de trabajo proporcionado por la empresa."
   ],
   "url": "https://www.laborum.pe/job/apparka/buscamos-chofer-a1-parttime-san-isidro-3pm-a-7pm-ingreso/6aacd705d32c1dca6715bc05"
  },
  {
   "id": 108,
   "puesto": "Vendedora San Miguel",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ventas",
   "min": 1250,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 8,
   "vence": "2026-12-17",
   "restan": 82,
   "score": 92,
   "resumen": "Asesoramiento a clientes en búsqueda de los diversos productos.",
   "funciones": [
    "Asesoramiento a clientes en búsqueda de los diversos productos.",
    "Alcanzar los objetivos de ventas establecidos por la empresa.",
    "Estar actualizado en los nuevos ingresos de productos",
    "Apoyar en el orden de los productos y visual de la tienda."
   ],
   "requisitos": [
    "Mujeres de nacionalidad peruana entre 20 y 38 años.",
    "Tener experiencia mínima de 6 meses como vendedor en tiendas retail de centros comerciales o boutiques puerta a calle.",
    "Conocimiento del idioma inglés a nivel intermedio al hablado para atender a los clientes.",
    "Excelente trato y orientación al cliente.",
    "Disponibilidad para laborar 48 horas semanales, 6 días a la semana por 8 horas diarias."
   ],
   "beneficios": [
    "Grato ambiente laboral.",
    "Beneficios laborales completos desde el 1er día de trabajo (2 gratificaciones completas al año, 2 CTS al año, 30 días de vacaciones al año, Seguro de Essalud, Asiganción Familiar).",
    "Pago quincena y fin de mes.",
    "Sueldo básico (S/.1250) + comisiones (en promedio desde S/.700 a más).",
    "Descuentos en productos de la marca.",
    "Reparto de utilidades.",
    "EPS a precio corporativo.",
    "Obsequio por navidad.",
    "Provis a fin de año."
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedora-san-miguel-adn-retail-s.a.c.-1118448268.html"
  },
  {
   "id": 109,
   "puesto": "Vendedor para tienda de natura",
   "empresa": "ADECCO PERU S.A.",
   "cat": "Ventas",
   "min": 572,
   "max": 572,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Arequipa",
   "fuente": "Bumeran",
   "dias": 8,
   "vence": "2027-01-16",
   "restan": 112,
   "score": 92,
   "resumen": "Por encargo de nuestro importante cliente en rubro de belleza, estamos en la búsqueda de buscando ASESOR DE VENTAS PART TIME para la tienda Natura",
   "funciones": [
    "Asesorar a los potenciales compradores sobre nuestros productos",
    "Conectar al cliente con la empresa para una experiencia satisfactoria",
    "Fidelizar a nuestros clientes mediante un servicio excepcional",
    "Captar nuevos compradores y expandir nuestra base de clientes"
   ],
   "requisitos": [
    "Experiencia previa en ventas retail o como promotor o anfitrión",
    "Mucha disposición y desenvolvimiento en funciones múltiples en tienda, trabajo en equipo, escucha activa, comunicación adecuada y capacidad resolutiva.",
    "Disponibilidad para trabajar 6 días a la semana, 4 horas diarias, con turnos rotativos retail, 1 día de descanso preferentemente entre semana (lunes a viernes)."
   ],
   "beneficios": [
    "Sueldo contrato Part time: s/572 + comisiones + s/ 115 en tarjeta de alimentación",
    "Rango Horario: 10 am a 10 pm (4 horas, apertura o cierre)",
    "Descuentos en Productos.",
    "Seguro EPS: Cobertura de salud y seguro oncológico.",
    "¡Promovemos el empleo formal, brindando trabajo a más personas!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedor-para-tienda-de-natura-part-time-arequipa-adecco-peru-s.a.-1118448299.html"
  },
  {
   "id": 110,
   "puesto": "Jefe Zonal de Market",
   "empresa": "GRUPO COPETROL",
   "cat": "Ventas",
   "min": 2500,
   "max": 3500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Trujillo",
   "fuente": "Bumeran",
   "dias": 8,
   "vence": "2027-01-16",
   "restan": 112,
   "score": 92,
   "resumen": "¡Únete a nuestro equipo como JEFE DE MARKET NORTE!",
   "funciones": [
    "Gestión Operativa y de Ventas: Realizar viajes constantes a las provincias asignadas para inspeccionar las tiendas y auditar el trabajo del personal a cargo.",
    "Control de Inventarios: Monitorear el análisis de rotación de productos, controlar las mermas y evitar productos vencidos.",
    "Capacitación y Liderazgo: Capacitar continuamente al personal en atención al cliente, protocolos de servicio, procesos operativos y elaboración de reportes. Realizar las mallas horarias según las necesidades del negocio.",
    "Gestión Comercial y Proveedores: Negociar con proveedores (precios, fechas de entrega, nuevos productos) y gestionar promociones o campañas según el calendario comercial.",
    "Análisis de Mercado: Elaborar reportes semanales de ventas, analizar las tendencias de compra y realizar estudios de la competencia."
   ],
   "requisitos": [
    "Experiencia previa en puestos de jefatura, supervisión o gestión de tiendas retail / minimarkets / estaciones de servicio.",
    "Disponibilidad para viajar entre Trujillo y Chimbote .",
    "Manejo de sistemas de gestión (deseable experiencia en Opensoft ) y herramientas ofimáticas.",
    "Competencias requeridas: Alto nivel de proactividad, capacidad de innovación, adaptabilidad al cambio y excelente actitud/disposición orientada al servicio y resultados."
   ],
   "beneficios": [
    "Sueldo Básico: S/ 2,500 a S/ 3,500 (según evaluación) + Bonos por cumplimiento de metas .",
    "Herramientas de trabajo brindadas por la empresa (Laptop y Celular).",
    "Oportunidad de desarrollo y crecimiento profesional en una empresa en constante expansión.",
    "Grato ambiente laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-zonal-de-market-grupo-copetrol-1118448325.html"
  },
  {
   "id": 111,
   "puesto": "Supervisor",
   "empresa": "Financiera QAPAQ",
   "cat": "Prácticas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 8,
   "vence": "2026-10-18",
   "restan": 22,
   "score": 92,
   "resumen": "Promocionar los créditos microempresa según la zona asignada.",
   "funciones": [
    "Promocionar los créditos microempresa según la zona asignada.",
    "Captar clientes mediante el trabajo de campo y evaluarlos con la ayuda del supervisor.",
    "Brindar una atención personalizada a los clientes de la entidad.",
    "Estudiar y aprobar los cursos del proceso de formación."
   ],
   "requisitos": [
    "EGRESADOS o BACHILLER (NO TITULADOS) de las carreras técnicas o universitarias de administración, contabilidad, economía, ingeniería industrial, o afines.",
    "Experiencia en ventas. (no indispensable).",
    "Experiencia y/o disponibilidad para realizar trabajo de campo ( 80% CAMPO Y 20% oficina)."
   ],
   "beneficios": [
    "Capacitación especializada en microfinanzas.",
    "Convalidación de prácticas profesionales.",
    "Subvención económica mensual: S/.1130.",
    "Horarios: Lunes - Viernes 8:45 am - 6:45 pm / Sábados 1:00 pm",
    "Duración del proceso de formación: 4 meses.",
    "Luego del proceso de formación ingresas a planilla.",
    "Gran línea de carrera con oportunidades de ascenso 3 veces al año.",
    "Grato ambiente de trabajo.",
    "Seguro FOLA. -Requerimientos- Educación mínima: Técnico años de experienciaConocimientos: Finanzas, VentasPalabras clave: advisor, asesor, training, capacitacion, capacitador"
   ],
   "url": "https://www.laborum.pe/job/financiera-qapaq/practicas-profesionales-asesor-de-negocios-en-formacion-san/6aaca7d9d32c1dca67155c38"
  },
  {
   "id": 112,
   "puesto": "Practicante de psicología",
   "empresa": "ASCENSORES TECNICOS ASOCIADOS SAC",
   "cat": "Prácticas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 8,
   "vence": "2026-10-18",
   "restan": 22,
   "score": 90,
   "resumen": "Soporte en la publicación de vacantes en portales de empleo, filtro curricular y citación para entrevistas (perfiles operativos y técnicos como cadistas, instaladores, etc.).",
   "funciones": [
    "Soporte en la publicación de vacantes en portales de empleo, filtro curricular y citación para entrevistas (perfiles operativos y técnicos como cadistas, instaladores, etc.).",
    "Apoyo en la organización de actividades de integración, campañas de salud y seguimiento de descansos médicos.",
    "Soporte en el control de asistencia, verificación de reportes de tareo/marcaciones (plataforma BUK) y armado de legajos digitales.",
    "Apoyo en la redacción y difusión de comunicados internos y políticas del área."
   ],
   "requisitos": [
    "Estudiantes de los últimos ciclos (universitarios o técnicos) de las carreras de Psicología (organizacional/clínica), Trabajo Social / Bienestar Social o Ingeniería Industrial / Administración, con conocimientos en MS Office a nivel intermedio (Excel indispensable).",
    "Competencias: Excelente nivel de comunicación, empatía, organización, orientación al servicio y confidencialidad.",
    "Disponibilidad: Inmediata para laborar de manera presencial."
   ],
   "beneficios": [
    "Convenio de Prácticas Preprofesionales firmado con tu casa de estudios.",
    "Subvención económica mensual: S/ 1,130.00 (Equivalente al 100% de la Remuneración Mínima Vital).",
    "Seguro Médico FOLA cubierto al 100% por la empresa.",
    "Media subvención económica cada 6 meses de prácticas continuas.",
    "Excelente clima laboral y oportunidad de aprendizaje integral.",
    "Modalidad y Horario de Prácticas",
    "Modalidad: 100% Presencial."
   ],
   "url": "https://www.laborum.pe/job/ascensores-tecnicos-asociados-sac/practicante-de-psicologia-bienestar-social-seleccion/6aac897bd32c1dca67150166"
  },
  {
   "id": 113,
   "puesto": "Asesora de Ventas por Call Center y Canales Digitales!",
   "empresa": "ALILOGISTIC",
   "cat": "Ventas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 8,
   "vence": "2026-12-17",
   "restan": 82,
   "score": 89,
   "resumen": "¡Únete a nuestro equipo como Asesora de Ventas por Call Center y Canales Digitales!",
   "funciones": [
    "Contactar y atender a clientes mediante llamadas y WhatsApp.",
    "Brindar información sobre productos, precios y promociones.",
    "Realizar seguimiento, manejar objeciones y concretar ventas.",
    "Realizar ventas en vivo por TikTok, presentando y promocionando los productos.",
    "Interactuar con los usuarios y responder consultas durante las transmisiones.",
    "Confirmar los datos y pedidos de los clientes.",
    "Coordinar y dar seguimiento a los despachos.",
    "Registrar las ventas y cumplir las metas comerciales."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en curso, truncos o concluidos.",
    "Experiencia mínima de seis meses en call center, telemarketing, ventas o atención al cliente.",
    "Experiencia o disposición para realizar ventas en vivo por TikTok.",
    "Facilidad de palabra, actitud comercial y orientación a resultados.",
    "Buen desenvolvimiento y seguridad frente a la cámara.",
    "Manejo básico de WhatsApp, TikTok y herramientas informáticas.",
    "Disponibilidad para trabajar presencialmente en Pueblo Libre."
   ],
   "beneficios": [
    "Sueldo mensual de S/ 1,500.",
    "Bonos de S/ 300 a S/ 500, según resultados.",
    "Ingreso a planilla MYPE.",
    "Horario de lunes a sábado, de 9:00 a. m. a 6:00 p. m.",
    "Oportunidad de aprendizaje y crecimiento.",
    "Si tienes talento para comunicar, vender y conectar con los clientes tanto por teléfono como frente a la cámara, ¡queremos conocerte! Postula y forma parte de nuestro equipo."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesora-de-ventas-por-call-center-y-canales-digitales-alilogistic-1118448283.html"
  },
  {
   "id": 114,
   "puesto": "Operario Estibador",
   "empresa": "GELAFRUT S.A.C.",
   "cat": "Logística",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 8,
   "vence": "2026-10-18",
   "restan": 22,
   "score": 88,
   "resumen": "Ejecutar las tareas solicitadas por su jefe inmediato en el área asignada.",
   "funciones": [
    "Ejecutar las tareas solicitadas por su jefe inmediato en el área asignada.",
    "Carga y descarga materias primas, insumos o productos terminados de camiones.",
    "Despacho de Alimentos a diversos restaurantes.",
    "Apoyo en la organización del trabajo diario.",
    "Pesaje y verificación de los productos en buen estado evitando las mermas y asegurando la frescura de los productos.",
    "Reemplazar bolsas dañadas o faltantes en pallets ya armados para completar el stock solicitado.",
    "Trasladar bolsas de manera manual desde la faja o zona de acopio hacia el el área de producción."
   ],
   "requisitos": [
    "Secundaria Completa.",
    "Mínimo 1 año de experiencia desempeñando funciones de estiba, despacho o carga pesada (no indispensable), con o sin experiencia.",
    "Gran capacidad para el levantamiento de carga constante y resistencia para trabajar de pie.",
    "Conocimiento básico en técnicas de apilamiento seguro y manipulación manual de cargas."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer con todos los beneficios de ley.",
    "Sueldo básico soles + Horas Extras S/.1,600.00",
    "Horario: Jornada de 8 horas diarias + 45 minutos de refrigerio (07:00 a.m. a 07:00 p.m.) + horas extras.",
    "Día de descanso rotativo a la semana",
    "Pagos mensuales puntuales",
    "Alimentación cubierta Almuerzo y Desayuno",
    "Todos los beneficios de Ley Régimen General al 100% (Gratificaciones, CTS, Vacaciones+ Asignación Familiar)",
    "Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: operator, operario, operator"
   ],
   "url": "https://www.laborum.pe/job/gelafrut-s-a-c/operario-estibador-con-o-sin-exp-en-carga-y-descarga-de/6aaca29dd32c1dca67154d04"
  },
  {
   "id": 115,
   "puesto": "Auxiliar de empaque para el área de Producción",
   "empresa": "ARCLAD DEL PERU S.A.C.",
   "cat": "Ingeniería",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 8,
   "vence": "2026-10-18",
   "restan": 22,
   "score": 88,
   "resumen": "RESUMEN: Auxiliar de Empaque Arclad del Perú (Ate)",
   "funciones": [
    "Encajado y empaque de producto terminado.",
    "Rotulado y verificación de productos antes del despacho.",
    "Paletizado y preparación de mercadería para su almacenamiento o envío.",
    "Apoyo en el conteo y control de inventario.",
    "Coordinación con el encargado de producción para la gestión de materiales.",
    "Orden y limpieza del área de trabajo.",
    "Cumplimiento de las normas de seguridad y calidad establecidas por la empresa."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 1 año en puestos similares (empaque, almacén, producción o estiba).",
    "Disponibilidad para realizar trabajo físico y para laborar en Ate.",
    "Persona puntual, responsable, honesta y orientada al trabajo en equipo."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley (CTS, gratificaciones, vacaciones, etc.).",
    "Capacitación y acompañamiento constante.",
    "Estabilidad laboral en una empresa sólida y en crecimiento.",
    "Buen ambiente de trabajo y posibilidad de desarrollo interno.",
    "Salario base s/ 1400.00",
    "Movilidad s/200.00",
    "Bono de productividad s/200 (Según cumplimiento).-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, zona, area, production, produce, fabricacion, manufactoring, elaboracion, making, manufactura, produccion"
   ],
   "url": "https://www.laborum.pe/job/arclad-del-peru-s-a-c/auxiliar-de-empaque-para-el-area-de-produccion-ate/6aacb0e2d32c1dca671571ff"
  },
  {
   "id": 116,
   "puesto": "Asesor comercial",
   "empresa": "Solventa",
   "cat": "Ventas",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 8,
   "vence": "2026-10-18",
   "restan": 22,
   "score": 86,
   "resumen": "Nos encontramos en la búsqueda de Asesores de Crédito para la aprobación y validación de préstamos virtuales, bajo modalidad 100% presencial en Lima, Perú.",
   "funciones": [
    "Atender eficazmente a los clientes que han solicitado un crédito a través de nuestra plataforma digital.",
    "Analizar perfiles crediticios para su validación y aprobación.",
    "Cerrar las operaciones de préstamos, asegurando el seguimiento y cumplimiento de pagos oportunos."
   ],
   "requisitos": [
    "Experiencia mínima de 1 año como asesor financiero, en venta de seguros, retención o cargos similares.",
    "Secundaria completa.",
    "Disponibilidad para poder laborar feriados.",
    "Buen nivel de comunicación oral y escrita.",
    "Capacidad de análisis, negociación y persuasión.",
    "Manejo básico de herramientas de Office.",
    "Alto nivel de compromiso, puntualidad y responsabilidad."
   ],
   "beneficios": [
    "Contrato régimen MYPE desde el primer día, incluyendo el pago por capacitación",
    "Sueldo: S/1,600 + S/300 bono de asistencia perfecta + COMISIONES ILIMITADAS",
    "EPS cubierta al 100% pasado el periodo de prueba de 3 meses.",
    "Horario laboral: lunes a sábados de 10:00 a.m. a 7:00 p.m.",
    "Modalidad de trabajo: 100% presencial en nuestra sede de Surco.-Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 18 y 38 añosPalabras clave: advisor, asesor, salesman, saleswoman, seller, vendedor, comercial, salesperson, analyst, financial, cambaceo, sales"
   ],
   "url": "https://www.laborum.pe/job/solventa/asesor-comercial-analista-financiero-ventas-s-1600-bono/6aac8eced32c1dca67150f5d"
  },
  {
   "id": 117,
   "puesto": "Asistente Administrativo",
   "empresa": "Afe Service Sac",
   "cat": "Administración",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 9,
   "vence": "2026-10-17",
   "restan": 21,
   "score": 94,
   "resumen": "Actualmente nos encontramos en la búsqueda del mejor talento para ocupar el puesto de Asistente Administrativo (Contar con título universitario o técnico)",
   "funciones": [
    "Agrupar y organizar documentación física y electrónica.",
    "Recabar, clasificar y entregar documentos, correspondencia o sobres al mensajero asignado.",
    "Generar Órdenes de Servicio.",
    "Apoyar en actividades administrativas relacionadas al área.",
    "Otras funciones inherentes al puesto."
   ],
   "requisitos": [
    "Bachiller en Administración de Empresas, Contabilidad, Ingeniería Industrial, Economía o carreras técnicas en administración, titulado en computación e informática o Secretariado.",
    "Obligatorio contar con título universitario o técnico vigente (INDISPENSABLE)",
    "Experiencia mínima de 6 meses como Asistente Administrativo o Puestos Similares.",
    "Manejo de Office a nivel intermedio (deseable).",
    "Cubrir descanso médico y vacaciones en la sede del Ministerio Público del Callao, Centro de Lima y Los Olivos",
    "Disponibilidad inmediata."
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1300",
    "Planilla completa desde el primer día",
    "2 gratificaciones al año",
    "30 días de vacaciones",
    "Pagos puntuales",
    "Excelente clima laboral",
    "Oportunidad de línea de carrera",
    "HORARIO DE TRABAJO",
    "Lunes a viernes: 8:00 am 6:00 pm",
    "Sábados: 9:00 am 2:00 pm o 11:00 am 4:00 pm (según necesidad de la operación)",
    "LUGAR DE TRABAJO: Tomas Marzano (5 minutos del Open Plaza de Angamos)"
   ],
   "url": "https://www.laborum.pe/job/afe-service-sac/asistente-administrativo-urgente-surquillo-disponibilidad/6aab772fd32c1dca6714388a"
  },
  {
   "id": 118,
   "puesto": "Barista",
   "empresa": "La Mora",
   "cat": "Gastronomía",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 9,
   "vence": "2026-12-16",
   "restan": 81,
   "score": 92,
   "resumen": "Nos encontramos en búsqueda de un(a) Barista para nuestra tienda ubicada en Surco, cerca al Puente Primavera .",
   "funciones": [
    "Preparación y elaboración de bebidas calientes y frías a base de café.",
    "Manejo de máquina de espresso y equipos de cafetería.",
    "Preparación de jugos, infusiones y otras bebidas de la carta.",
    "Mantener limpia y ordenada el área de trabajo y equipos.",
    "Reposición y correcta conservación de insumos.",
    "Cumplir con los procedimientos de higiene y manipulación de alimentos.",
    "Brindar una atención cordial y rápida a los clientes."
   ],
   "requisitos": [
    "Experiencia mínima de 6 meses como Barista o en posiciones similares.",
    "Manejo de máquina de espresso.",
    "Orientación al servicio y atención al cliente.",
    "Responsabilidad, rapidez, orden y capacidad para trabajar en equipo.",
    "Disponibilidad para trabajar en horarios rotativos."
   ],
   "beneficios": [
    "Sueldo: S/ 1,400",
    "S/ 75 de alimentación",
    "Ingreso a planilla desde el primer día.",
    "Pagos quincenales y fin de mes.",
    "Línea de carrera.",
    "HORARIOS ROTATIVOS – INDISPENSABLE",
    "Apertura: 6:30 a. m. a 3:30 p. m.",
    "Cierre: 2:30 p. m. a 11:00 p. m.",
    "Lunes a domingo, con 1 día de descanso rotativo .",
    "LUGAR DE TRABAJO Surco, cerca al Puente Primavera."
   ],
   "url": "https://www.bumeran.com.pe/empleos/barista-surco-la-mora-1118446906.html"
  },
  {
   "id": 119,
   "puesto": "Practicante Profesional de Asesoría",
   "empresa": "Engineering Services & Consulting S.A.C",
   "cat": "Prácticas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 9,
   "vence": "2026-12-16",
   "restan": 81,
   "score": 92,
   "resumen": "Apoyar en la coordinación y seguimiento de proyectos del área.",
   "funciones": [
    "Apoyar en la coordinación y seguimiento de proyectos del área.",
    "Participar en la elaboración de informes, presentaciones y entregables técnicos .",
    "Brindar soporte en el uso de softwares de modelamiento y herramientas tecnológicas .",
    "Recopilar y analizar información técnica y datos de los proyectos .",
    "Apoyar en las reuniones y coordinaciones con el equipo y clientes .",
    "Utilizar herramientas de Microsoft Office e inteligencia artificial para optimizar las actividades del área."
   ],
   "requisitos": [
    "Egresado(a) o Bachiller de Arquitectura, Ingeniería Industrial, Ingeniería Mecánica, Ingeniería Eléctrica o carreras afines.",
    "Experiencia o conocimientos previos en seguridad, coordinación y seguimiento de proyectos y/o softwares de modelamiento .",
    "Manejo de Excel a nivel intermedio o avanzado .",
    "Manejo de herramientas de Microsoft Office a nivel intermedio .",
    "Conocimiento y manejo del idioma inglés .",
    "Manejo de herramientas de inteligencia artificial aplicadas a la búsqueda y análisis de información, elaboración de documentos, presentaciones y optimización de actividades.",
    "Facilidad e interés por aprender y utilizar nuevas herramientas tecnológicas y softwares especializados .",
    "Proactividad e iniciativa para proponer soluciones y asumir nuevos retos.",
    "Comunicación efectiva y dominio de escena , con capacidad para interactuar con equipos de trabajo y diferentes stakeholders.",
    "Organización y orientación al cumplimiento de objetivos .",
    "Capacidad analítica y atención al detalle .",
    "Adaptabilidad y aprendizaje continuo , especialmente frente a nuevas tecnologías y herramientas digitales."
   ],
   "beneficios": [
    "Asesoría: Lunes a viernes de 8:30 a.m. a 5:30 p.m.",
    "Ubicación: Surquillo, Lima",
    "Subvención mensual: S/ 1,500",
    "Vacantes: 2 posiciones",
    "Acceso a capacitaciones constantes y oportunidades de aprendizaje continuo",
    "Participación en proyectos de alto impacto con clientes líderes",
    "Grato ambiente laboral y actividades de integración",
    "Pertenecer a una empresa líder y en constante crecimiento",
    "¡Sé parte del equipo SINCRO y lleva tu talento al siguiente nivel!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-profesional-de-asesoria-|-sincro-engineering-services-consulting-s.a.c-1118447054.html"
  },
  {
   "id": 120,
   "puesto": "Asesor(a) de Ventas Volante Lima Norte Comisiones",
   "empresa": "MONTALVO INSTITUTE CORP SAC",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 9,
   "vence": "2026-10-17",
   "restan": 21,
   "score": 91,
   "resumen": "Asesorar a clientes sobre cursos educativos que ofrece el Instituto.",
   "funciones": [
    "Asesorar a clientes sobre cursos educativos que ofrece el Instituto.",
    "Gestionar matrículas y cobranzas.",
    "Cumplir metas de ventas mensuales.",
    "(Rotativos con 1 día de descanso)",
    "Turno mañana: 8:00 a. m. a 5:00 p. m.",
    "Turno tarde: 12:30 p. m. a 9:30 p. m.",
    "Jornada de lunes a domingo con 1 día de descanso semanal."
   ],
   "requisitos": [
    "Experiencia mínima de 6 meses en ventas, presenciales o telefónicas.",
    "Experiencia trabajando con metas comerciales.",
    "Disponibilidad para trasladarse entre nuestras sedes de Carabayllo y Los Olivos.",
    "Disponibilidad para laborar en horarios rotativos."
   ],
   "beneficios": [
    "Sueldo fijo de S/1,300 + S/100 de bono de movilidad.",
    "Comisiones y bonos por cumplimiento.",
    "Desde el 4.º mes: S/1,400 + bono de movilidad + comisiones.",
    "Ingreso a planilla completa desde el primer día (Asignación Familiar, CTS, Gratificación, Vacaciones, Vida Ley, etc.).",
    "Oportunidad de línea de carrera.",
    "Si cuentas con experiencia en ventas y disponibilidad para trasladarte entre sedes, te invitamos a postular y formar parte de una institución líder en el sector belleza.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: cambaceo, sales"
   ],
   "url": "https://www.laborum.pe/job/montalvo-institute-corp-sac/asesor-a-de-ventas-volante-lima-norte-comisiones-bonos/6aab4c16d32c1dca6713c7d9"
  },
  {
   "id": 121,
   "puesto": "Vendedor Piura",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ventas",
   "min": 1250,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Piura",
   "fuente": "Bumeran",
   "dias": 9,
   "vence": "2026-12-16",
   "restan": 81,
   "score": 89,
   "resumen": "Asesoramiento a clientes en búsqueda de los diversos productos.",
   "funciones": [
    "Asesoramiento a clientes en búsqueda de los diversos productos.",
    "Alcanzar los objetivos de ventas establecidos por la empresa.",
    "Estar actualizado en los nuevos ingresos de productos",
    "Apoyar en el orden de los productos y visual de la tienda."
   ],
   "requisitos": [
    "Hombres de nacionalidad peruana entre 20 y 38 años.",
    "Tener experiencia mínima de 6 meses como vendedor en tiendas retail de centros comerciales o boutiques puerta a calle.",
    "Excelente trato y orientación al cliente.",
    "Disponibilidad para laborar 48 horas semanales, 6 días a la semana por 8 horas diarias."
   ],
   "beneficios": [
    "Grato ambiente laboral.",
    "Beneficios laborales completos desde el 1er día de trabajo (2 gratificaciones completas al año, 2 CTS al año, 30 días de vacaciones al año, Seguro de Essalud, Asiganción Familiar).",
    "Pago quincena y fin de mes.",
    "Sueldo básico (S/.1250) + comisiones (en promedio desde S/.600 a más).",
    "Descuentos en productos de la marca.",
    "Reparto de utilidades.",
    "EPS a precio corporativo.",
    "Obsequio por navidad.",
    "Provis a fin de año."
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedor-rambla-san-borja-adn-retail-s.a.c.-1118446954.html"
  },
  {
   "id": 122,
   "puesto": "Community Manager",
   "empresa": "ACUARIUS OPTICAL SAC",
   "cat": "Marketing",
   "min": 1800,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 9,
   "vence": "2026-12-16",
   "restan": 81,
   "score": 89,
   "resumen": "Proyectar la mejor imagen de empresa y marcas ante las redes sociales y mundo digital.",
   "funciones": [
    "Proyectar la mejor imagen de empresa y marcas ante las redes sociales y mundo digital.",
    "Desarrollar y ejecutar estrategias efectivas de MKT digital y gestión de RRSS para aumentar la presencia de la marca y el compromiso de la comunidad.",
    "Creación de contenido atractivo de calidad y valor para el público.",
    "Planificación y producción de contenido multimedia incluyendo videos promocionales, diseños gráficos y otros.",
    "Capacidad para interactuar de manera amigable y persuasiva con clientes potenciales.",
    "Uso de herramientas de análisis y medición de publicaciones.",
    "Realización de informes periódicos sobre el rendimiento y las tendencias.",
    "Cumplimiento del plazo para respuestas y seguimientos de leads.",
    "Otras actividades encomendadas por el jefe de área."
   ],
   "requisitos": [
    "Edad: 25 a 35 años.",
    "Sexo: Mujer y Hombre.",
    "Proactivo / empático / trabajo en equipo.",
    "Formación académica: Estudios Técnicos o Universitarios .",
    "Conocimiento: Adobe Ilustrator, Adobe Photoshop, Adobe After Effects, Adobe Premier.",
    "Experiencia mínima de 2 años en Manejo de redes sociales.",
    "Trabajos realizados en redes sociales."
   ],
   "beneficios": [
    "Sueldo Bruto S/. 1,800.00",
    "Horario : L A V de 8:30 a 18:00",
    "Contrato de trabajo.",
    "Capacitación permanente.",
    "Excelente ambiente de trabajo.",
    "Oportunidad de crecimiento."
   ],
   "url": "https://www.bumeran.com.pe/empleos/community-manager-acuarius-optical-sac-1118446972.html"
  },
  {
   "id": 123,
   "puesto": "Asistente de Marketing y Comunicaciones",
   "empresa": "ESCUELA NACIONAL DE ALTOS ESTUDIOS",
   "cat": "Marketing",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 9,
   "vence": "2026-10-17",
   "restan": 21,
   "score": 89,
   "resumen": "RESUMEN: En ERCA Educación Continua nos encontramos en búsqueda de una Asistente de Marketing y Comunicaciones para integrarse a nuestro equipo.",
   "funciones": [
    "Crear, grabar y editar videos publicitarios para redes sociales.",
    "Participar en la dirección y producción de videos comerciales para cursos, programas académicos, campañas y promociones.",
    "Proponer ideas para reels, videos comerciales y contenido educativo.",
    "Elaborar y organizar la grilla de contenido semanal o mensual.",
    "Coordinar grabaciones con docentes, estudiantes, equipo comercial y personal.",
    "Desarrollar piezas audiovisuales para promocionar cursos, certificaciones, beneficios y campañas educativas.",
    "Apoyar en campañas de admisión, promociones y lanzamientos.",
    "Proponer mejoras orientadas a incrementar el alcance, interacción y posicionamiento de la marca en canales digitales."
   ],
   "requisitos": [
    "Estudiante de últimos ciclos o egresado(a) de Marketing, Comunicaciones, Publicidad, Producción Audiovisual, Diseño, Multimedia o carreras afines.",
    "Conocimientos de grabación y edición de videos para redes sociales.",
    "Manejo intermedio de CapCut, Canva, Adobe Premiere, Filmora o herramientas similares.",
    "Conocimiento de formatos para TikTok, Instagram Reels y Facebook.",
    "Creatividad y criterio visual para desarrollar nuevas propuestas.",
    "Interés en marketing digital, producción audiovisual y redes sociales.",
    "Buena comunicación, organización, responsabilidad e iniciativa.",
    "Capacidad para trabajar en equipo y adaptarse a diferentes campañas."
   ],
   "beneficios": [
    "Remuneración mensual: S/ 1,400.",
    "Horario de lunes a viernes, de 9:00 a. m. a 6:00 p. m.",
    "Modalidad 100 % presencial.",
    "Lugar de trabajo: Breña, Lima.",
    "Bonos por productividad.",
    "Capacitación constante.",
    "Becas de estudio en nuestros programas.",
    "Oportunidad de línea de carrera.",
    "Aprendizaje práctico en marketing educativo, producción audiovisual, redes sociales y campañas digitales.",
    "Si te gusta crear, grabar, editar y convertir ideas en contenido que conecte con las personas, queremos conocerte.",
    "¡Postula y forma parte de ERCA Educación Continua!. -Requerimientos- Educación mínima: Universitario años de experienciaEdad: entre 20 y 30 añosConocimientos: CreatividadPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, mercadotecnia, marketing, mercadeo"
   ],
   "url": "https://www.laborum.pe/job/escuela-nacional-de-altos-estudios/asistente-de-marketing-y-comunicaciones-redes-sociales-y/6aab3140d32c1dca67137503"
  },
  {
   "id": 124,
   "puesto": "Supervisor",
   "empresa": "CALIMOD",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 9,
   "vence": "2026-10-17",
   "restan": 21,
   "score": 87,
   "resumen": "RESUMEN: En GRUPO CALIMOD ConfiamosEnTuTalento",
   "funciones": [
    "Apoyo en tareas de producción: sellado, armado, corte, habilitado y manejo de máquinas.",
    "Mantener el orden y limpieza del área de trabajo.",
    "Otras funciones asignadas por el supervisor."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 6 meses en el área de producción.",
    "Disponibilidad para laborar de lunes a sábado (turno día)."
   ],
   "beneficios": [
    "Sueldo S/.1130 + pago por horas extras.",
    "Turno diurno.",
    "Pagos semanales.",
    "Ingreso a planilla desde el primer día.",
    "Alimentación cubierta al 100%."
   ],
   "url": "https://www.laborum.pe/job/calimod/urgente-20-operarios-de-produccion-ate/6aab7ae4d32c1dca67144023"
  },
  {
   "id": 125,
   "puesto": "Operario(a) de Limpieza",
   "empresa": "MONTALVO INSTITUTE CORP SAC",
   "cat": "Recursos Humanos",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 9,
   "vence": "2026-10-17",
   "restan": 21,
   "score": 86,
   "resumen": "RESUMEN: Montalvo Instituto, institución líder en educación en cosmetología y belleza a nivel nacional, se encuentra en la búsqueda de OPERARIO(A) DE LIMPEIZA para nuestra sede ATE.",
   "funciones": [
    "Limpieza diaria de aulas, oficinas, servicios higiénicos y áreas comunes.",
    "Limpieza minuciosa de mobiliario, equipos y utensilios de la sede.",
    "Vaciado, limpieza y correcta gestión de contenedores de desechos.",
    "Control e inventario de los productos y herramientas de limpieza.",
    "Reportes incidencias o desperfectos directamente al jefe inmediato."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Mínimo 8 meses de experiencia en el puesto.",
    "Experiencia en ventas telefónicas o presenciales, atención al cliente.",
    "Disponibilidad para laborar de manera presencial en la sede de Huancayo.",
    "Buena comunicación y orientación a resultados.",
    "Turno (1 día de descanso rotativo: sábado o domingo)",
    "Lunes a viernes: 12:30p.m. 9:30 p.m.",
    "Sábado: 8:00 a.m. 5:00 p.m.",
    "Domingo: 8:00 a.m. 3:30 p.m.",
    "Sueldo fijo S/ 1,200 (incremento según permanencia) + bono de movilidad dominical",
    "Planilla completa (regimen general) desde el primer día (Asignación Familiar, CTS, Gratificación, Vacaciones, Vida Ley, etc.)"
   ],
   "beneficios": [
    "Línea de Carrera",
    "Capacitaciones constantes",
    "Si buscas estabilidad laboral y desarrollo en el área comercial, ¡esta es tu oportunidad!. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: A partir de 21 añosPalabras clave: aseo, higiene, cleaning, limpieza"
   ],
   "url": "https://www.laborum.pe/job/montalvo-institute-corp-sac/operario-a-de-limpieza-sede-ate-planilla-completa/6aab6bcdd32c1dca67141f65"
  },
  {
   "id": 126,
   "puesto": "Asistente Contable",
   "empresa": "BELTRAN",
   "cat": "Contabilidad",
   "min": 2200,
   "max": 2200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 9,
   "vence": "2026-10-17",
   "restan": 21,
   "score": 83,
   "resumen": "RESUMEN: Buscamos un Asistente Contable proactivo, para integrarse a nuestro equipo de Contabilidad. La persona seleccionada tendrá a su cargo las siguientes funciones:",
   "funciones": [
    "Declarar IGV Renta Mensual",
    "Declarar Retención del IGV",
    "Declarar Renta Anual 3ª Categoría",
    "Declarar ITAN",
    "Declarar INEI",
    "Declarar SENATI"
   ],
   "requisitos": [
    "Título profesional en Contabilidad.",
    "Deseable: Colegiatura vigente.",
    "Conocimiento sólido de normativa contable y tributaria peruana.",
    "Manejo de sistemas contables y herramientas de oficina.",
    "Actitud proactiva, organización, responsabilidad y capacidad de trabajo en equipo."
   ],
   "beneficios": [
    "Remuneración mensual de S/ 2,200.00.",
    "Contrato en planilla con todos los beneficios de ley."
   ],
   "url": "https://www.laborum.pe/job/beltran/asistente-contable-proactivo/6aab45bdd32c1dca6713b6f4"
  },
  {
   "id": 127,
   "puesto": "Practicante profesional de Administración y Gestión de Servicios",
   "empresa": "ESCUELA IBEROAMERICA DE NEGOCIOS",
   "cat": "Prácticas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 10,
   "vence": "2026-10-16",
   "restan": 20,
   "score": 95,
   "resumen": "RESUMEN: En EIN, nos encontramos en búsqueda del mejor talento para ocupar la posición de PRACTICANTE PROFESIONAL de Administración y Gestión de Servicios, para nuestra sede Prialé - Lurigancho",
   "funciones": [
    "Brindar atención y orientación a los clientes, gestionando consultas, solicitudes, quejas y requerimientos de manera presencial, telefónica y virtual.",
    "Realizar el seguimiento de los servicios de capacitación antes, durante y después de su ejecución, asegurando una adecuada atención y continuidad del servicio.",
    "Apoyar en la coordinación de instructores, traslados, viáticos, hospedaje, alimentación y materiales necesarios para el desarrollo de las capacitaciones.",
    "Coordinar la logística de las capacitaciones, verificando la disponibilidad y operatividad de salas, equipos, materiales, insumos e infraestructura.",
    "Gestionar y mantener actualizada la documentación de los participantes, incluyendo listas de asistencia, matrices de notas, certificados, informes y bases de datos.",
    "Elaborar reportes sobre el desarrollo y estado de los servicios, atendiendo los requerimientos de la jefatura y Gerencia General.",
    "Brindar soporte en la gestión administrativa de los servicios, incluyendo coordinación de facturación, rendición de gastos y recepción de comprobantes de instructores, psicólogas y proveedores.",
    "Apoyar en mantener el orden, limpieza y condiciones adecuadas de los ambientes de capacitación, cumpliendo las disposiciones de seguridad correspondientes."
   ],
   "requisitos": [
    "Egresado o Bachiller de Administración o Ingeniería Industrial.",
    "Microsoft Office nivel intermedio (indispensable).",
    "6 meses de prácticas o experiencia en funciones similares.",
    "Cursos o capacitación en atención y manejo de clientes (Deseable).",
    "Disponibilidad para laborar en Ramiro Prialé - Lurigancho de Lunes a Sábado de manera presencial"
   ],
   "beneficios": [
    "Modalidad: Presencial.",
    "Lugar de trabajo: Huachipa (Ramiro Prialé) - Lurigancho",
    "Horario: Lunes a miércoles de 7:30 a. m. a 5:30 p. m.; jueves y viernes de 8:00 a. m. a 5:30 p. m.; sábados de 8:00 a. m. a 12:00 p. m.",
    "Remuneración: S/ 1,130 mensuales.",
    "Modalidad: Convenio de prácticas profesionales.",
    "Seguro FOLA. -Requerimientos- Educación mínima: Universitario años de experienciaConocimientos: Microsoft excelPalabras clave: intern, practicante, becario, pasante, experto, profesional, professional, expert, intendencia, administration, quartermaster, administracion"
   ],
   "url": "https://www.laborum.pe/job/escuela-iberoamerica-de-negocios/practicante-profesional-de-administracion-y-gestion-de/6aa9e159d32c1dca6711edda"
  },
  {
   "id": 128,
   "puesto": "Auxiliar Producción en Cercado de Lima",
   "empresa": "Wellco Peruana SA",
   "cat": "Ingeniería",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 10,
   "vence": "2026-10-16",
   "restan": 20,
   "score": 92,
   "resumen": "Imprimir planos de producción.",
   "funciones": [
    "Imprimir planos de producción.",
    "Gestionar la firma de la Tarjeta de Producción para el inicio de producción.",
    "Registrar los pedidos en el sistema.",
    "Realizar actividades administrativas, digitación y procesamiento de datos.",
    "Cumplir con los procedimientos y documentos del Sistema de Gestión de Calidad.",
    "Informar incidentes, accidentes o condiciones de riesgo.",
    "Cumplir las normas y reglamentos internos de la empresa.",
    "Realizar otras funciones asignadas por el jefe inmediato."
   ],
   "requisitos": [
    "Estudiante de últimos ciclos o egresado técnico de Administración Industrial, Administración, Ing Industrial o carreras afines.",
    "Experiencia mínima de 6 meses en labores administrativas.",
    "Manejo de Microsoft Office y Excel a nivel intermedio.",
    "Deseable manejo de otros sistemas ERP.",
    "Organización, responsabilidad y atención al detalle."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día.",
    "SCTR y Vida Ley.",
    "Oportunidad de desarrollo profesional.",
    "Salario : S/1600",
    "De contar con el perfil y encontrarte interesado, no dudes en postular!. -Requerimientos- Educación mínima: Técnico años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, production, produce, fabricacion, manufactoring, elaboracion, making, manufactura, produccion"
   ],
   "url": "https://www.laborum.pe/job/wellco-peruana-sa/auxiliar-produccion-en-cercado-de-lima/6aa9f21fd32c1dca67122a85"
  },
  {
   "id": 129,
   "puesto": "Analista de Control y Automatizaciones para Rubro Entretenimiento Casinos",
   "empresa": "GLADCON",
   "cat": "Otros",
   "min": 3000,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 10,
   "vence": "2026-10-16",
   "restan": 20,
   "score": 92,
   "resumen": "RESUMEN: En Gladcon, somos un grupo corporativo peruano líder en el sector de juego y entretenimiento, con más de 30 años de experiencia y más de 23 salas tragamonedas y casinos distribuidos en más de 7 provincias del país.",
   "funciones": [
    "Realizar el cuadre de los progresivos en las Unidades de Negocio, detectando e informando oportunamente los pagos sin sustento con el soporte de los Sistemas y herramientas establecidas por la empresa, a fin de garantizar una respuesta rápida en caso de robo u error de procedimiento.",
    "Aprobar las solicitudes de reposición de llaves de las Unidades de Negocio, verificando su consistencia con el control establecido, a fin de asegurar la trazabilidad, el control y adecuado abastecimiento.",
    "Desarrollar e implementar automatizaciones orientadas a optimizar los procesos del área de Control Interno y Mejora Continua, reduciendo tareas manuales y mejorando la eficiencia operativa en la detección de robos y fraudes.",
    "Analizar información operativa y generar reportes que contribuyan a la toma de decisiones y a la identificación de oportunidades de mejora.",
    "Brindar soporte en proyectos de mejora continua, desarrollando soluciones prácticas mediante herramientas tecnológicas, enfocadas en promover la eficiencia y productividad en los procesos."
   ],
   "requisitos": [
    "Experiencia en manejo de SQL Nivel Intermedio.",
    "Experiencia en manejo de Entorno Google Workspace Nivel Avanzado.",
    "Experiencia en manejo de Visual Basic Query",
    "Experiencia Un (01) año realizando funciones similares de análisis de datos y automatización de procesos.",
    "Pensamiento analítico y resolución de problemas: Identificar problemas, analizar datos y proponer soluciones efectivas.",
    "Liderazgo y trabajo en equipo: liderar proyectos y colaborar con equipos de trabajo. Comunicación: Comunicar ideas y hallazgos de manera clara y convincente (escrita / oral)",
    "Orientación a resultados y eficiencia: Foco en lograr objetivos.",
    "Innovación y proactividad: Proponer nuevas ideas, pensar fuera de lo convencional y buscar activamente oportunidades de mejora.",
    "Gestión del cambio: Guiar a los equipos y promover una cultura de mejora continua.-Requerimientos- Educación mínima: Universitario2 años de experienciaPalabras clave: analyst"
   ],
   "beneficios": [
    "Remuneración s/3000",
    "Ingreso en planilla desde el primer día con todos los beneficios de ley CTS, gratificaciones, EsSalud, vacaciones, seguro vida ley, asignación familiar, entro otros).",
    "Planes de crecimiento profesional y capacitación continua.",
    "Un ambiente de trabajo colaborativo donde tus ideas son valoradas",
    "Ser parte de nuestras olimpiadas, ferias de salud, fiestas de fin de año entre otros.·"
   ],
   "url": "https://www.laborum.pe/job/gladcon/se-solicita-analista-de-control-y-automatizaciones-para/6aaa3a3dd32c1dca6712f2b9"
  },
  {
   "id": 130,
   "puesto": "Asistente Administrativo",
   "empresa": "INDELFI SAC",
   "cat": "Administración",
   "min": 1500,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 10,
   "vence": "2026-10-16",
   "restan": 20,
   "score": 92,
   "resumen": "Realizar el seguimiento de las ventas de las promociones.",
   "funciones": [
    "Realizar el seguimiento de las ventas de las promociones.",
    "Realizar el recorrido a en la estación para identificar alguna incidencia con los compresores de los diferentes productos de venta en playa.",
    "Verificar el cumplimiento del ciclo de servicio por parte de todo el personal a su cargo.",
    "Realizar el chek list diario de la imagen física de los diversos ambientes de la EESS.",
    "Realizar el check list diario de la imagen del personal: limpieza, trato al cliente, actitud, orden, uso correcto de uniforme, entre otros.",
    "Gestionar el abastecimiento de los productos de la playa y del Market.",
    "Asegurar la atención de la EESS de acuerdo a la programación horaria del personal."
   ],
   "requisitos": [
    "Experiencia de 2 años en gestiones administrativas y comerciales de restaurantes, EESS, tiendas por conveniencia, afines.",
    "Formación académica: Técnicos o universitarios",
    "Experiencia en liquidaciones de venta, caja e inventarios"
   ],
   "beneficios": [
    "Sueldo: Entre S/ 1500 a S/ 1800",
    "Planilla Régimen General",
    "Lugar de trabajo: Villa María del Triunfo",
    "Horario: Lunes a Viernes de 8:00 a.m. a 5:30 p.m. y Sábados de 8:00 a.m. a 1:30 p.m. -Requerimientos- Educación mínima: Técnico años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide"
   ],
   "url": "https://www.laborum.pe/job/indelfi-sac/asistente-administrativo-a-sede-villa-maria-del-triunfo/6aaa0115d32c1dca67126368"
  },
  {
   "id": 131,
   "puesto": "Practicante Pre Profesional de Desarrollo de Sistemas",
   "empresa": "Unibanca",
   "cat": "Prácticas",
   "min": 1150,
   "max": 1150,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 10,
   "vence": "2026-12-14",
   "restan": 79,
   "score": 89,
   "resumen": "Realizar el análisis y desarrollos de los requerimientos del área.",
   "funciones": [
    "Realizar el análisis y desarrollos de los requerimientos del área.",
    "Ejecutar las pruebas de los sistemas desarrollados.",
    "Documentar las especificaciones técnicas de los requerimientos.",
    "Nota: Todas las funciones son enunciativas mas no limitativas"
   ],
   "requisitos": [
    "Estudiante universitario de 7mo ciclo en adelante de Ingeniería de Sistemas o carreras afines.",
    "No se requiere experiencia.",
    "Conocimiento de Microsoft Office, Visio.",
    "Conocimientos de programación y desarrollo de programas (Java, C).",
    "Deseable conocimientos de UNIX.",
    "Facilidad de aprendizaje / interés en aprender.",
    "Buena capacidad de análisis.",
    "Dinámico y proactivo.",
    "Facilidad de comunicación.",
    "Capacidad de interrelacionarse a todo nivel, buen trato personal."
   ],
   "beneficios": [
    "Horario de lunes a viernes de 9am a 4pm.",
    "Subvención de S/ 1150.",
    "Media subvención cada 6 meses.",
    "Seguro médico privado.",
    "Grato ambiente de trabajo.",
    "Buen ambiente de trabajo.",
    "Día libre por cumpleaños.",
    "Línea de carrera."
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-pre-profesional-de-desarrollo-de-sistemas-unibanca-1118445473.html"
  },
  {
   "id": 132,
   "puesto": "Vigilante Sede Independencia",
   "empresa": "ACADEMIA VONEX",
   "cat": "Recursos Humanos",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 10,
   "vence": "2026-10-16",
   "restan": 20,
   "score": 89,
   "resumen": "RESUMEN: En Academia Vonex nos encontramos en la búsqueda de un Vigilante / Portero, responsable y comprometido para garantizar la seguridad y el orden en nuestra sede.",
   "funciones": [
    "Supervisar el ingreso y salida de los colaboradores",
    "Brindar atención y orientación al cliente interno y externo.",
    "Controlar y verificar el ingreso y salida de equipos, materiales y bienes, asegurando la correcta documentación.",
    "Mantenerse en estado de alerta para prevenir incidentes o actos delictivos.",
    "Supervisar la recepción de materiales en la sede.",
    "Cumplir con otras funciones asignadas por la jefatura inmediata."
   ],
   "requisitos": [
    "Experiencia mínima de 1 año como vigilante, agente de seguridad o portero.",
    "Experiencia en instituciones educativas (deseable).",
    "Residir en Independencia, Comas, Puente Piedra o zonas aledañas.",
    "Actitud responsable, proactiva y con orientación al servicio."
   ],
   "beneficios": [
    "Ingreso a planilla completa (régimen general) desde el primer día.",
    "Remuneración de S/ 1,400 con todos los beneficios de ley.",
    "Capacitaciones internas constantes.",
    "Horario de trabajo",
    "Lunes a viernes de 2:10 pm a 10:15 pm.",
    "Dos sábados al mes de 7:00 am a 4:35 pm.",
    "Si cuentas con experiencia en seguridad y deseas formar parte de una institución sólida, esta es tu oportunidad.",
    "Por ello, queremos que sepas que esta convocatoria también está dirigida para personas con discapacidad.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: escolta, guardia, vigilante, guarda, guard, vigilancia"
   ],
   "url": "https://www.laborum.pe/job/academia-vonex/vigilante-sede-independencia-turno-tarde-planilla-completa/6aa9f89dd32c1dca671243d5"
  },
  {
   "id": 133,
   "puesto": "Auxiliar Administrativo",
   "empresa": "Awara Human Solutions",
   "cat": "Administración",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 10,
   "vence": "2026-12-15",
   "restan": 80,
   "score": 87,
   "resumen": "En Awara Human Solutions nos encontramos en búsqueda de un (a) Auxiliar Administrativo para una importante empresa del sector maquinarias para minería.",
   "funciones": [
    "Asistir a la persona de ingresos y bancos",
    "Asistir a la persona con registro de compras, ventas y documentacion.",
    "Dar soporte a la coordinadora administrativa en la documentación que se requiera en el momento."
   ],
   "requisitos": [
    "Carrera completa o trunca técnica o universitaria en administración o similares",
    "01 año de experiencia en la posición o realizando las funciones",
    "Dominio de office a nivel básico intermedio",
    "Vivir en distritos aledaños a Surco",
    "Contar con disponibilidad inmediata"
   ],
   "beneficios": [
    "Lugar de trabajo: Santiago de surco (Av. Benavides c/ Av. Caminos del inca)",
    "Modalidad: 100% presencial",
    "Horario: lunes a viernes de 08:00am - 06:00pm y sábados de 09:00am - 12:00pm (intercalados)",
    "Ingreso a planilla de pequeña empresa (50% de beneficios)"
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-administrativo-surco-awara-human-solutions-1118445409.html"
  },
  {
   "id": 134,
   "puesto": "Recepcionista Chorrillos",
   "empresa": "Construredes S.A.C.",
   "cat": "Administración",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 10,
   "vence": "2026-10-16",
   "restan": 20,
   "score": 87,
   "resumen": "Recepcionar y/o derivar llamadas, a través del manejo de la central telefónica.",
   "funciones": [
    "Recepcionar y/o derivar llamadas, a través del manejo de la central telefónica.",
    "Recepcionar facturas de proveedores observando el cumplimiento de políticas, procedimientos, normativas y dispositivos legales establecidos.",
    "Brindar apoyo administrativo en el desarrollo de todas las actividades de RRHH."
   ],
   "requisitos": [
    "Profesional Técnica en Administración, Secretariado, Recursos Humanos o afines,",
    "Con conocimiento en Ofimática a nivel intermedio.",
    "Experiencia previa ocupando posiciones similares y atendiendo a clientes.",
    "De preferencia manejo de central telefónica.",
    "Comunicación Efectiva.",
    "Manejo de Recursos.",
    "Resolución de Problemas",
    "Organización y Planificación.",
    "Manejo de Presión.",
    "Trabajo en equipo."
   ],
   "beneficios": [
    "Ingreso a Planilla con todos los beneficios de ley.",
    "Sueldo S/. 1,130 Bruto.",
    "Horario de Lunes a Viernes. -Requerimientos- Educación mínima: Técnico años de experienciaPalabras clave: recepcion, receptionist, recepcionista"
   ],
   "url": "https://www.laborum.pe/job/construredes-s-a-c/recepcionista-chorrillos/6aa9e240d32c1dca6711f150"
  },
  {
   "id": 135,
   "puesto": "Técnico",
   "empresa": "HVA INVERSIONES",
   "cat": "Administración",
   "min": 1500,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 11,
   "vence": "2026-10-15",
   "restan": 19,
   "score": 97,
   "resumen": "Realizar revisión de áreas del restaurante (al inicio y al final)",
   "funciones": [
    "Realizar revisión de áreas del restaurante (al inicio y al final)",
    "Controlar el inventario de cocina, salón y bar.",
    "Realizar el correcto pedido a los proveedores. (manteniendo un stock de seguridad)",
    "Gestionar y supervisar al personal de cocina, salón, bar y caja.",
    "Impulsar las ventas y asegurar una excelente experiencia para el cliente.",
    "Elaborar reportes administrativos.",
    "Controlar la calidad del servicio, limpieza y cumplimiento de normas sanitarias.",
    "Resolver incidencias operativas durante la jornada.",
    "Optimizar costos, controlar mermas y mejorar la eficiencia del negocio.",
    "1 : 8AM A 6PM",
    "2: 5PM A 12PM",
    "Cerca a Plaza de Armas, Centro de Lima.-Requerimientos- Educación mínima: Universitario2 años de experienciaEdad: entre 25 y 45 añosConocimientos: AdministraciónPalabras clave: restaurador"
   ],
   "requisitos": [
    "Estudio universitario o técnico en Administración de Empresas o carreras afines.",
    "Experiencia mínima de 2 años como administrador(a) en restaurantes.",
    "Manejo administrativo y control de dinero.",
    "Conocimiento de temas sanitarios aplicados al rubro gastronómico.",
    "Conocimiento de Word y Excel (nivel intermedio o más)",
    "Actitud proactiva y responsabilidad",
    "Ganas de aprender y crecer con la empresa",
    "Vivir cerca al Centro de Lima (deseable)",
    "Experiencia con sistema Restaurant.pe (deseable, no excluyente)"
   ],
   "beneficios": [
    "Sueldo: Entre S/. 1500 A S/. 1600.00",
    "Ingreso a planilla y asignación familiar.",
    "Pagos quincenales.",
    "Alimentación cubierta.",
    "Pago doble por feriados laborados.",
    "Comisiones individuales y semanales.",
    "30% de descuento en consumos (en nuestros locales).",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Bonos por metas mensuales.",
    "Un día de descanso a la semana."
   ],
   "url": "https://www.laborum.pe/job/hva-inversiones/asistenta-administrativa-para-restaurante-cerca-a-la-plaza/6aa8a753d32c1dca6710b422"
  },
  {
   "id": 136,
   "puesto": "Asistente de selección de personal",
   "empresa": "Confidencial",
   "cat": "Recursos Humanos",
   "min": 1700,
   "max": 1700,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Arequipa",
   "fuente": "Bumeran",
   "dias": 11,
   "vence": "2027-01-13",
   "restan": 109,
   "score": 92,
   "resumen": "Nos encontramos en la búsqueda del siguiente puesto :",
   "funciones": [
    "Ejecutar los procesos de reclutamiento y selección de puesto operativos y administrativos a la brevedad.",
    "Realizar convocatorias en las diferentes bolsas laborales (computrabajo, Indeed y/o bolsas gratuitas).",
    "Realizar filtros telefónicos, citar candidatos.",
    "Evaluación psicológica a candidatos (psicometricos y proyectivos).",
    "Programaciones de exámenes médicos pre ocupacionales.",
    "Gestión de documentos con candidatos para firma de contrato.",
    "Elaboración de contrato inicial.",
    "Altas en el T registro (nuevos ingresos).",
    "Brindar inducción de RRHH.",
    "Solicitud de EPPS al area de SST para los nuevos ingresos.",
    "Armado de file de personal (físico como digital: SmartBoleta).",
    "Subir file digital a la plataforma Smartboleta."
   ],
   "requisitos": [
    "Egresado de la carrera de Administración, RRHH, Psicología (Indispensable).",
    "Experiencia mínima de 1 año realizando procesos de selección de personal (procesos individuales / masivos).",
    "Disponibilidad para trabajar de lunes a viernes de 8am a 6pm y sábados de 8am a 1pm.",
    "Experiencia en reclutamiento en campo.",
    "Disponibilidad para laborar en sede Arequipa (Parque industrial) Ref terminal Flores."
   ],
   "beneficios": [
    "Sueldo básico de 1700.00 soles",
    "Almuerzo cubierto al 100%",
    "Seguro de Salud",
    "Asignación Familiar",
    "Seguro vida ley.",
    "Seguro SCTR.",
    "Grato ambiente laboral.",
    "Por favor considerar el lugar de trabajo (Arequipa: Parque Industrial), tener acceso al whasapp ya que se realizaran las programación por ese medio.",
    "Adjuntar CV actualizado"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-seleccion-de-personal-sede-arequipa-1118442159.html"
  },
  {
   "id": 137,
   "puesto": "Auxiliar contable",
   "empresa": "Consultora Human Solutions S.A.C",
   "cat": "Contabilidad",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 11,
   "vence": "2026-12-14",
   "restan": 79,
   "score": 91,
   "resumen": "Por encargo de nuestro cliente, importante empresa del sector industrial, nos encontramos en búsqueda del mejor talento para el perfil de: Auxiliar de Contabilidad.",
   "funciones": [
    "Registro de compras de mercadería en el sistema",
    "Registro de cajas chicas",
    "Registro y conciliación de pagos bancarios",
    "Comparación de compras SIRE vs SISCONT",
    "Seguimiento de comprobantes electrónicos para su registro",
    "Apoyo en emisión de cheques",
    "Registro y conciliación de detracciones"
   ],
   "requisitos": [
    "Técnico en Contabilidad o egresado universitario en Contabilidad",
    "Experiencia mínima de 1 año como Auxiliar Contable",
    "Disponibilidad para laborar en Surco",
    "Disponibilidad para trabajo presencial",
    "Conocimiento de Excel intermedio (Indispensable).",
    "Conocimiento de SISCONT(Indispensable).",
    "Conocimiento de plataforma SUNAT (deseable)"
   ],
   "beneficios": [
    "Ingreso a planilla – Régimen General",
    "Sueldo S/. 1,400",
    "Pago de utilidades",
    "Capacitaciones inherentes al cargo",
    "Oportunidad de desarrollo profesional",
    "Grato clima laboral.",
    "Pertenecer a una empresa sólida en el sector",
    "Horario: De Lunes a viernes de 8:45 a.m. a 6:15 p.m.",
    "“En Human Solutions promovemos la inserción e inclusión laboral de personas con discapacidad”"
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-contable-surco-consultora-human-solutions-s.a.c-1118443776.html"
  },
  {
   "id": 138,
   "puesto": "Auxiliar Operativo",
   "empresa": "CICSA PERU S.A.C.",
   "cat": "Administración",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 11,
   "vence": "2026-10-15",
   "restan": 19,
   "score": 90,
   "resumen": "RESUMEN: Se parte de uno de los conglomerados más grandes e importantes de Latinoamérica.",
   "funciones": [
    "Gestión documental: Organizar órdenes de trabajo, reportes de mantenimiento HFC y archivos técnicos.",
    "Soporte operativo: Actualizar bases de datos, generar informes de avance y coordinar materiales con proveedores.",
    "Apoyo en campo: Realizar visitas ocasionales para verificar inventarios o registrar incidencias (20% del tiempo).",
    "Otras funciones designadas por su superior"
   ],
   "requisitos": [
    "Experiencia mínima de 1 año en roles administrativos o de soporte (preferible en áreas técnicas/operativas).",
    "Experiencia en gestión de inventarios o coordinación con equipos de mantenimiento.",
    "Ofimática avanzada: Excel (tablas dinámicas, fórmulas, etc), Word y Herramientas digitales (sheets, IA).",
    "Conceptos técnicos: Nociones básicas de redes HFC/FTTH y terminología de telecomunicaciones. (deseable)"
   ],
   "beneficios": [
    "Sueldo 1500 Soles",
    "Essalud cubierto al 100%",
    "Esquema remunerativo acorde al mercado",
    "Planilla completa. -Requerimientos- Educación mínima: Técnico1 año de experienciaEdad: entre 21 y 35 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, operative, operativo, capturista, digitador"
   ],
   "url": "https://www.laborum.pe/job/cicsa-peru-s-a-c/auxiliar-operativo-digitador-san-isidro/6aa89731d32c1dca67108520"
  },
  {
   "id": 139,
   "puesto": "Atención al cliente",
   "empresa": "ALILOGISTIC",
   "cat": "Ventas",
   "min": 500,
   "max": 500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 11,
   "vence": "2026-12-14",
   "restan": 79,
   "score": 89,
   "resumen": "¿Tienes experiencia en ventas, atención al cliente y manejo de canales digitales? ¡Esta oportunidad puede ser para ti!",
   "funciones": [
    "Atender y asesorar a los clientes por canales digitales.",
    "Confirmar los datos y pedidos realizados.",
    "Realizar seguimiento comercial y concretar ventas.",
    "Coordinar el despacho de pedidos por agencia.",
    "Participar en transmisiones de ventas por TikTok."
   ],
   "requisitos": [
    "Experiencia en ventas y atención al cliente.",
    "Experiencia en ventas en vivo mediante TikTok.",
    "Facilidad de palabra y buen desenvolvimiento frente a la cámara.",
    "Manejo de WhatsApp, TikTok y otras redes sociales."
   ],
   "beneficios": [
    "Sueldo mensual: S/ 1,500.",
    "Bono por ventas: Entre S/ 300 y S/ 500, según resultados.",
    "Ingreso a planilla MYPE.",
    "Horario: Lunes a sábado, de 9:00 a. m. a 6:00 p. m.",
    "Trabajo presencial en Pueblo Libre.",
    "Si te apasionan las ventas, disfrutas interactuar con clientes y tienes facilidad para comunicarte, ¡postula y forma parte de nuestro equipo!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesora-ventas-digitales-alilogistic-1118443775.html"
  },
  {
   "id": 140,
   "puesto": "Chofer de Reparto y Almacén con Licencia AIIIb en Lurín",
   "empresa": "Wellco Peruana SA",
   "cat": "Logística",
   "min": 2200,
   "max": 2400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 12,
   "vence": "2026-10-14",
   "restan": 18,
   "score": 100,
   "resumen": "RESUMEN: ¡Estamos buscando CHOFER ALMACENISTA!",
   "funciones": [
    "Revisar y contar los artículos destinados al despacho y los productos que ingresan al almacén.",
    "Preparar y apoyar en el embalaje de mercadería para los despachos.",
    "Controlar físicamente el ingreso y salida de los productos transportados en la unidad asignada.",
    "Gestionar y verificar las guías de remisión correspondientes a los despachos.",
    "Estibar y asegurar correctamente la mercadería en la unidad de transporte y en las zonas de almacenamiento.",
    "Apoyar en la ejecución de inventarios asignados por la jefatura.",
    "Realizar el traslado y entrega de mercadería, cumpliendo las rutas y procedimientos establecidos."
   ],
   "requisitos": [
    "Licencia de conducir AIIIB indispensable que este vigente (no negociable)",
    "Experiencia y conocimientos prácticos en almacén y despacho.",
    "Experiencia en manejo y traslado de mercadería.",
    "Disponibilidad para trabajar en Lurín.",
    "De preferencia, residir en Lurín o zonas aledañas."
   ],
   "beneficios": [
    "Sueldo: S/ 2,200 - 2,400",
    "Ingreso a planilla desde el primer día.",
    "Todos los beneficios de ley.",
    "Alimentación cubierta al 100%.",
    "Movilidad de acercamiento (recojo en Km40)",
    "LUGAR DE TRABAJO LURÍN",
    "De cumplir con el perfil, no dudes en postular!. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaPalabras clave: conductor, driver, choferes, chofer, delivery, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/wellco-peruana-sa/chofer-de-reparto-y-almacen-con-licencia-aiiib-en-lurin/6aa73e1c0b351bbb217deb98"
  },
  {
   "id": 141,
   "puesto": "Asesor(a) de Gerencia General",
   "empresa": "Bigmond Group",
   "cat": "Tecnología",
   "min": 500,
   "max": 500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 12,
   "vence": "2026-12-13",
   "restan": 78,
   "score": 97,
   "resumen": "En Mill Hire The Future, firma perteneciente a Bigmond Group, por encargo de nuestro cliente, importante empresa del sector Banca y Finanzas, nos encontramos en la búsqueda de un(a): Asesor de Gerencia Genera, con el siguiente perfil:",
   "funciones": [
    "Asesorar a la Gerencia General en el análisis de asuntos estratégicos, técnicos y administrativos que contribuyan a la toma de decisiones institucionales. Con especial énfasis en la gobernanza institucional.",
    "Coordinar con las gerencias y demás unidades orgánicas el levantamiento, consolidación y validación de información requerida por la Gerencia General para el cumplimiento de sus funciones.",
    "Brindar soporte a la Gerencia General en la preparación y organización de reuniones de trabajo, sesiones con autoridades, organismos públicos, entidades financieras y otros grupos de interés.",
    "Proponer mejoras en los procesos, procedimientos e instrumentos de gestión vinculados al ámbito de competencia de la Gerencia General, promoviendo la eficiencia institucional.",
    "Realizar seguimiento, analizar y evaluar la información reportada por las Gerencias respecto a la implementación de medidas correctivas y recomendaciones formuladas por los reguladores y supervisores, alertando de posibles incumplimientos.",
    "Proponer a la Gerencia General las medidas que aseguren una adecuada estructura de control interno para la buena marcha de la Corporación.",
    "Ejecutar las demás funciones afines al puesto que le sean asignadas por el Gerente General, en el ámbito de su competencia."
   ],
   "requisitos": [
    "Bachiller indispensable en Administración de Empresas, Ingeniería Industrial, Economía, Contabilidad, Derecho u otras carreras afines. Título deseable.",
    "Colegiatura y habilitación vigente en el caso de profesionales de la carrea de Derecho. (indispensable)",
    "Indispensable Grado de Maestro en MBA, Maestría en Administración, Dirección de Empresas, Maestría en Derecho o afines al puesto y/o sector; o alternativamente",
    "Título Profesional y constancia de egresado de Maestría.",
    "Título Profesional y más 03 años adicionales al requisito de experiencia en cargos gerenciales, jefaturales o equivalentes, en este caso, se requerirá la especialización como indispensable.",
    "Conocimientos en planeamiento operativo, gestión de gobernanza, gestión pública y gestión de proyectos o afines. (deseable)",
    "Especialización en planeamiento estratégico, gobierno corporativo, normas públicas y/o afines. (deseable)",
    "Diez (10) años mínimo de experiencia laboral en general desde egresado. (indispensable).",
    "Dos (2) años mínimo laborando en empresas públicas y/o privadas y/o entidades del gobierno nacional a fines al sector o sectores de la corporación."
   ],
   "beneficios": [
    "EPS Mapfre, para ti y tu familia, cubierto hasta el 90% por parte de la empresa.",
    "Seguro contra accidentes personales cubierto al 100% por parte de la empresa.",
    "Seguro de Vida Ley desde el 1er. día de trabajo.",
    "Seguro Oncológico, para ti y tu familia, cubierto al 100% por parte de la empresa.",
    "Seguro de salud internacional para titular y dependientes directos, cubierto al 100% por parte de la empresa.",
    "Seguro de vida internacional para titular, cubierto al 100% por parte de la empresa",
    "Póliza de Responsabilidad Civil.",
    "Vale de alimentos por un importe mensual de S/ 500.00, de acuerdo con lo establecido en el lineamiento interno de la empresa.",
    "Bono por cumplimiento de objetivos al año de hasta un tope de dos (02) sueldos.",
    "Estacionamiento en las instalaciones de la empresa.",
    "Asignación de equipos electrónicos: Celular y Laptop",
    "Las personas interesadas que cumplan con el perfil, es indispensable que remitan por este medio su currículo documentado sustentando los requisitos detallados"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-gerencia-general-bigmond-group-1118442045.html"
  },
  {
   "id": 142,
   "puesto": "Asistente de Negocios Escuela Mibanco para Agencia Sihuas",
   "empresa": "MANPOWERGROUP",
   "cat": "Educación",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Áncash",
   "fuente": "Bumeran",
   "dias": 12,
   "vence": "2027-01-11",
   "restan": 107,
   "score": 97,
   "resumen": "Cumplir con todas las tareas y actividades que conforman el Programa Formativo- Escuela.",
   "funciones": [
    "Cumplir con todas las tareas y actividades que conforman el Programa Formativo- Escuela.",
    "Realizar prospecciones de clientes con el apoyo del Asesor Tutor.",
    "Llevar a cabo acompañamientos a los asesores en sus visitas a clientes.",
    "Promoción y captación de los productos del banco.",
    "Apoyar actividades del proceso crediticio en agencia.",
    "Elaborar expedientes de crédito.",
    "Generar reportes de mora y cosechas en agencia como parte del entrenamiento"
   ],
   "requisitos": [
    "Universitarios: Titulado/Egresado/Estudiantes (IX ciclo en adelante) de las carreras de Administración, Economía, Contabilidad, Ingenierías o afines.",
    "Técnicos: Titulado/Egresado/Estudiantes (VI ciclo en adelante) de las carreras de Contabilidad, Administración o afines.",
    "Experiencia en atención al cliente, ventas de intangibles o tangibles (Deseable).",
    "Buena disposición para el trabajo de campo.",
    "Trabajo de Campo (80%) y oficina (20%).",
    "Disponibilidad de Lunes a Viernes de 8:45am a 6:30pm y Sábados de 8:45am a 1:00pm",
    "Residir en o tener disponibilidad de trabajar en Sihuas (indispensable)."
   ],
   "beneficios": [
    "Planilla Completa desde el 1er día laborado.",
    "Salario: Sueldo básico: S/1130 + Comisiones",
    "Bono de Movilidad: S/200",
    "EPS cubierta al 70%.",
    "Formar parte de uno de los grupos económicos más importantes del país.",
    "Oportunidad de línea de carrera.",
    "Excelente clima laboral.",
    "Descuentos en entidades educativas, de salud y establecimientos comerciales.",
    "Programas de Capacitación.",
    "Durante el proceso formativo tendrás el acompañamiento de un tutor.",
    "Certificación como Asesor de Negocios al finalizar exitosamente el Programa de Escuela.",
    "En Manpower Perú, promovemos la inclusión e inserción laboral sin distinción por razón de discapacidad, género, orientación, identidad y expresión de género, lugar de origen o generación. Por ello ¡Todos los talentos son bienvenidos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-negocios-escuela-mibanco-para-agencia-sihuas-ancash-manpowergroup-1118442059.html"
  },
  {
   "id": 143,
   "puesto": "Operario de Limpieza",
   "empresa": "Lavoro Perú",
   "cat": "Ingeniería",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 12,
   "vence": "2026-10-14",
   "restan": 18,
   "score": 97,
   "resumen": "Nuestro cliente, importante empresa localizada en el mercado de CHACLACAYO, destacada en el rubro de INDUSTRIAL, se encuentra en la búsqueda del/ los mejor(es) talento(s) para ocupar el puesto Operario de Limpieza y Mantenimiento de Planta.",
   "funciones": [
    "Realizar la limpieza y lavado de áreas operativas de la planta relacionadas con el proceso de germinación de cebada.",
    "Utilizar hidrolavadora y herramientas de limpieza para remover residuos y suciedad de las áreas de trabajo.",
    "Trabajar en un ambiente industrial donde pueden presentarse olores propios del proceso de germinación y de la limpieza.",
    "Cargar y mover materiales o residuos húmedos de hasta 20 kg.",
    "Retirar y recoger residuos generados durante las actividades de limpieza.",
    "HORARIO DE TRABAJO",
    "Horarios Rotativos semanales: MAÑANA 7am a 3pm/ TARDE 3pm a 11pm/ NOCHE 11pm a 7am.",
    "Modalidad (6x1) unes a domingo con un día de descanso rotativo programados con operaciones."
   ],
   "requisitos": [
    "Secundaria completa",
    "Experiencia mínima de 1 año en limpieza pesada, lavado industrial, hidrolavado, mantenimiento, manejo de residuos, plantas de abonos/ fertilizantes, piscinas, aguas residuales, desagües, plantas o trabajos similares.",
    "Experiencia o familiaridad en el uso de hidrolavadora.",
    "Disponibilidad para realizar labores de esfuerzo físico y permanecer de pie durante la jornada.",
    "Disponibilidad para trabajar en ambientes industriales, propios del proceso de germinación de cebada.",
    "Compromiso con el cumplimiento de normas y protocolos de seguridad."
   ],
   "beneficios": [
    "Agradable ambiente laboral.",
    "Ingreso directo a Planilla desde el primer día de labores con todos los Beneficios de Ley.",
    "Remuneración: S/. 1200",
    "Bono Bienvenida S/.100",
    "Bono de Permanencia S/.150",
    "Bono asistencia perfecta S/. 93",
    "Bono alimentación S/. 100",
    "Seguro SCTR (en caso corresponda) y Vida Ley",
    "Pertenecer a una empresa líder en el mercado",
    "Somos una empresa que promueve la igualdad de oportunidades y no discriminamos por motivo de origen, edad, sexo, discapacidad u otra condición, asi como la inclusión laboral de personas con discapacidad."
   ],
   "url": "https://www.laborum.pe/job/lavoro-peru/personal-para-mantenimiento-limpieza-de-piscinas-de/6aa749790b351bbb217e03ab"
  },
  {
   "id": 144,
   "puesto": "Supervisor",
   "empresa": "Salesland Pacífico",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 12,
   "vence": "2026-10-14",
   "restan": 18,
   "score": 94,
   "resumen": "RESUMEN: Somos SALESLAND, una empresa con 18 años de experiencia en el mercado, especialistas en gestión de equipos de ventas, con presencia en España, Portugal, Guatemala, Colombia, Chile, México y Perú, entre otros países.",
   "funciones": [
    "Abordar activamente a los clientes en los puntos de venta asignados.",
    "Promocionar y vender productos escolares de las marcas ARTESCO.",
    "Reponer productos en góndolas, anaqueles o vitrinas",
    "Reportar resultados diarios al supervisor/a.",
    "Si el arte, el dibujo y el diseño son tu lenguaje, ¡estás a un paso de convertir tu pasión en una carrera profesional! Únete a un entorno donde la imaginación, la técnica y la innovación van de la mano.",
    "¡Ven y únete a nuestro equipo!. -Requerimientos- Educación mínima: Técnico años de experienciaEdad: entre 18 y 30 añosConocimientos: Creatividad, Diseño, Microsoft excel, Ventas"
   ],
   "requisitos": [
    "Capacidad de aprendizaje y adaptación.",
    "Con o sin experiencia en Mercaderista Impulsador, ventas o atención al cliente(SE LES CAPACITARA).",
    "Habilidades de comunicación, asesoría y negociación.",
    "Alta proactividad y orientación a resultados.",
    "Interés o conocimiento en productos de utiles escolares, diseño y dibujo."
   ],
   "beneficios": [
    "Ingreso a planilla completa con todos los beneficios de ley desde el primer día.",
    "Sueldo base S/ 1130 + Bono fijo de S/ 260",
    "Capacitaciones constantes y acompañamiento continuo.",
    "Oportunidades de línea de carrera en una empresa en crecimiento.",
    "Excelente clima laboral y beneficios corporativos.",
    "Horario rotativo con disponibilidad",
    "De 10:00 a.m. a 7:00 p.m. (8 HORAS DE TRABAJO + 1 REFRIGERIO)",
    "DESCANSOS LOS MARTES"
   ],
   "url": "https://www.laborum.pe/job/salesland-pacifico/ingreso-inmediato-artesco-full-time-horario-fijo-san-isidro/6aa76a010b351bbb217e16fd"
  },
  {
   "id": 145,
   "puesto": "Ayudante de Cocina",
   "empresa": "GRAFTON LATAM",
   "cat": "Gastronomía",
   "min": 1150,
   "max": 1150,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 12,
   "vence": "2026-12-12",
   "restan": 77,
   "score": 92,
   "resumen": "Apoyar en la preparación y producción de alimentos según los procedimientos establecidos.",
   "funciones": [
    "Apoyar en la preparación y producción de alimentos según los procedimientos establecidos.",
    "Realizar el lavado, corte y acondicionamiento de verduras e insumos.",
    "Preparar ensaladas, salsas, tequeños, chicha y comida para el personal.",
    "Apoyar en las áreas de frituras, parrilla y horno, según la necesidad de la operación.",
    "Realizar el lavado y limpieza de vajilla, utensilios y equipos de cocina.",
    "Mantener el área de trabajo limpia, ordenada y abastecida.",
    "Verificar la calidad y correcta conservación de los insumos y productos.",
    "Apoyar en la recepción, almacenamiento y abastecimiento de productos.",
    "Cumplir con los procedimientos de higiene, seguridad y manipulación de alimentos.",
    "Brindar soporte en las diferentes actividades del área de cocina cuando sea necesario.",
    "Horario FULL time ROTATIVO",
    "LUNES A DOMINGO: 08:00am a 4:00pm | 4:00pm a 12:00pm"
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 3 meses en puestos de Ayudante de Cocina, Steward/Vajillero o afines al área de cocina y producción.",
    "Disponibilidad para laborar en horarios rotativos.",
    "Sedes disponibles: San Borja, Surco y La Molina.",
    "Persona responsable, ordenada y comprometida."
   ],
   "beneficios": [
    "Ingreso a Planilla Completa desde el primer día, conforme a ley. (GRATI, CTS, ESSALUD , VACACIONES , ASIGNACION FAMILIAR)",
    "Alimentación cubierta al 100%.",
    "Pago quincenal.",
    "Estabilidad laboral según desempeño.",
    "Oportunidad de crecimiento interno.",
    "Sueldo base: S/. 1150 + Propinas fijas: S/. 300"
   ],
   "url": "https://www.bumeran.com.pe/empleos/ayudante-de-cocina-full-time-san-borja-la-molina-surco-restaurante-grafton-latam-1118442007.html"
  },
  {
   "id": 146,
   "puesto": "Practicante de Negocios Internacionales",
   "empresa": "BOLD ASCENT",
   "cat": "Prácticas",
   "min": 700,
   "max": 700,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 12,
   "vence": "2026-12-13",
   "restan": 78,
   "score": 92,
   "resumen": "Programa de Desarrollo de Talento – Bold Ascent Ubicación: Santiago de Surco (frente al Jockey Plaza / Universidad de Lima)",
   "funciones": [
    "Brindar soporte en procesos operativos y administrativos diarios.",
    "Participar activamente en iniciativas de automatización, optimización y mejora continua.",
    "Organizar, registrar y realizar control de calidad a bases de datos y documentación.",
    "Apoyar en investigación de mercado, nuevas oportunidades y prospección comercial.",
    "Colaborar con distintas áreas estratégicas y, según tus intereses, aportar en proyectos de contenido y comunicación digital.",
    "Horarios Disponibles (Compatibles con tus estudios)",
    "Turno Tarde 12:30 p. m. – 4:30 p. m.",
    "Modalidad: Lunes a viernes presencial en Surco | Sábados rotativos (presencial o remoto según programación)."
   ],
   "requisitos": [
    "Estudiante activo de instituto o universidad de Negocios Internacionales o afines.",
    "Marcado interés por la tecnología, la Inteligencia Artificial y la automatización aplicadas a la productividad.",
    "Altas ganas de aprender, desarrollarse y crecer profesionalmente (¡No exigimos experiencia previa, priorizamos tu potencial y actitud!).",
    "Proactividad, responsabilidad, curiosidad y gran atención al detalle.",
    "Trabajo en equipo, comunicación y compromiso.",
    "Manejo básico de Microsoft Excel / Office.",
    "Deseable (No excluyente): Inglés básico/intermedio y desenvolvimiento para apoyar eventualmente en creación de contenido o videos para redes sociales."
   ],
   "beneficios": [
    "Subvención económica: S/ 700 mensuales (Part-Time).",
    "Excelente clima laboral , dinámico, colaborativo y enfocado en la innovación.",
    "Participación en proyectos reales de la empresa y con impacto directo en el negocio.",
    "Desarrollo de habilidades profesionales en operaciones, administración, comercial, e-commerce y mejora de procesos.",
    "Capacitación y mentoría desde el primer día con líderes de área.",
    "Oportunidad de crecimiento: Posibilidad de asumir mayores retos y proyectos estratégicos según tu desempeño."
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-de-negocios-internacionales-part-time-bold-ascent-1118442116.html"
  },
  {
   "id": 147,
   "puesto": "Turno mañana! Anfitrión (a) de Estacionamiento en Wong Chacarilla",
   "empresa": "APPARKA",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 12,
   "vence": "2026-10-14",
   "restan": 18,
   "score": 91,
   "resumen": "RESUMEN: En Apparka nos encontramos en búsqueda del mejor talento para el puesto de Anfitrión de Estacionamientos.",
   "funciones": [
    "Brindar una excelente atención a nuestros clientes, asegurando una experiencia agradable desde su llegada.",
    "Facilitar la entrada y salida de los vehículos en nuestras instalaciones.",
    "Colaborar con un equipo comprometido en garantizar la seguridad y el buen funcionamiento de cada estacionamiento.",
    "Utilizar nuestras herramientas tecnológicas para hacer el trabajo más eficiente.",
    "Disponibilidad para laborar en el turno de: 7:00 am - 3:45 pm con Descanso un día a la semana."
   ],
   "requisitos": [
    "Personas con buena actitud, orientadas al servicio y dispuestas a aprender.",
    "Experiencia previa en atención al cliente (no es indispensable, ¡nosotros te capacitamos!).",
    "Disponibilidad para trabajar en turnos rotativos.",
    "Compromiso con el trabajo en equipo y la puntualidad."
   ],
   "beneficios": [
    "Sueldo: S/. 1130 mensual + PLANILLA COMPLETA",
    "Ingreso a planilla completa desde el primer día.",
    "¡Oportunidad de línea de carrera a corto plazo!",
    "Capacitaciones constantes."
   ],
   "url": "https://www.laborum.pe/job/apparka/turno-manana-anfitrion-a-de-estacionamiento-en-wong/6aa73a020b351bbb217de079"
  },
  {
   "id": 148,
   "puesto": "Ingeniero Comercial Jr",
   "empresa": "Talento Humano Perú",
   "cat": "Ventas",
   "min": 4000,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Híbrido",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 12,
   "vence": "2026-12-13",
   "restan": 78,
   "score": 89,
   "resumen": "Previamente la persona que asuma el puesto será capacitada y formada en relación a los servicios que ofrece la empresa.",
   "funciones": [
    "Previamente la persona que asuma el puesto será capacitada y formada en relación a los servicios que ofrece la empresa.",
    "Elaborar y estructurar propuestas técnicas y económicas para empresas del sector Energía.",
    "Gestionar contratos de proyectos eléctricos y electromecánicos.",
    "Coordinar actividades técnicas con clientes, proveedores y equipos internos.",
    "Realizar seguimiento al cumplimiento de cronogramas, costos y alcance.",
    "Participar en estudios eléctricos (protecciones, sistemas de potencia, etc.).",
    "Apoyar procesos comerciales y de negociación con clientes."
   ],
   "requisitos": [
    "Egresado de carrera profesional de Ingeniería Eléctrica, Ingeniería Industrial, Ingeniería Mecánica, Ingeniería Electrónica o carreras afines.",
    "Experiencia general de dos (02) años en el sector energía",
    "Inglés, nivel avanzado.",
    "Comunicación asertiva, orientación al logro de resultados, planificación y organización, análisis y solución de problemas."
   ],
   "beneficios": [
    "Ingreso a planilla general, con todos los beneficios de acuerdo a ley.",
    "Remuneración: S/. 4,000.00 soles + Comisión Asegurable por 3 meses S/ 1,000.00 soles",
    "A partir del 4to. mes: Comisión 1%",
    "LUGAR Y HORARIO DE TRABAJO",
    "Lunes a viernes de 8:00 a.m. a 6:00 p.m.",
    "Oficina: San Isidro",
    "Trabajo Híbrido (Desde Casa) 70% - Visita a Clientes 30%"
   ],
   "url": "https://www.bumeran.com.pe/empleos/ingeniero-comercial-jr.--sector-energia-hibrido-talento-humano-peru-1118437746.html"
  },
  {
   "id": 149,
   "puesto": "Auxiliar de Maquila",
   "empresa": "Lavoro Perú",
   "cat": "Ingeniería",
   "min": 1230,
   "max": 1230,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 12,
   "vence": "2026-10-14",
   "restan": 18,
   "score": 88,
   "resumen": "RESUMEN: Nuestro cliente, importante empresa localizada en el mercado de Huachipa, destacada en el rubro de embotelladora de bebidas se encuentra en la búsqueda del/ los mejor(es) talento(s) para ocupar el puesto de AUXILIAR DE MAQUILA.",
   "funciones": [
    "Picking de productos",
    "Sellado de productos",
    "Uso sctoka manual",
    "Orden/limpieza en almacén",
    "Ubicación de productos de descarga",
    "Acomodo correcto de productos para colocarlos en ubicaciones correspondientes",
    "Etiquetado y embalaje de productos",
    "Conteo de productos"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Experiencia de 3 meses en cargos similares o trabajando en planta",
    "Disponibilidad inmediata",
    "Con Domicilio en zonas cercanas a Huachipa, Ate, San Juan de Lurigancho, Santa Anita, entre otro.",
    "HORARIO DE TRABAJO",
    "Turno día: 7:00 am a 4:00 pm (con disponibilidad de hacer horas extras)"
   ],
   "beneficios": [
    "Agradable ambiente laboral.",
    "Ingreso directo a Planilla desde el primer día de labores con todos los Beneficios de Ley.",
    "Remuneración: S/. 1230",
    "Bono de movilidad S/. 100",
    "Asignación Familiar S/ 113",
    "Horas Extras pagadas conforme a ley.",
    "Pagos puntuales en quincena y fin de mes",
    "Somos una empresa que promueve la igualdad de oportunidades y no discriminamos por motivo de origen, edad, sexo, discapacidad u otra condición , así como la inclusión laboral de personas con discapacidad.",
    "La información personal será tratada de forma confidencial y utilizada solo para el presente proceso de reclutamiento y selección.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, permanent, fijo"
   ],
   "url": "https://www.laborum.pe/job/lavoro-peru/auxiliar-de-maquila-turno-dia-fijo-ate-huachipa-huachipa/6aa753fd0b351bbb217e08c9"
  },
  {
   "id": 150,
   "puesto": "Auxiliar de almacén",
   "empresa": "Lavoro Perú",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 12,
   "vence": "2026-10-14",
   "restan": 18,
   "score": 88,
   "resumen": "RESUMEN: Nuestro cliente, importante empresa localizada en el mercado de Huachipa, destacada en el rubro de embotelladora de bebidas se encuentra en la búsqueda del/ los mejor(es) talento(s) para ocupar el puesto de AUXILIAR DE ALMACEN.",
   "funciones": [
    "Picking de productos",
    "Orden/limpieza en almacén",
    "Ubicación de productos de descarga",
    "Acomodo correcto de productos para colocarlos en ubicaciones correspondientes",
    "Etiquetado y embalaje de productos",
    "Conteo de productos"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Experiencia de 3 meses en cargos similares o trabajando en planta",
    "Disponibilidad inmediata.",
    "HORARIO DE TRABAJO",
    "Turno Tarde: 3pm - 11:00pm"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día de labores.",
    "Todos los beneficios de ley.",
    "Remuneración básica: S/ 1,130.",
    "Bono de movilidad: S/ 100.",
    "Asignación familiar: S/ 113 (en caso de tener hijos menores).",
    "Pago de horas extras conforme a ley.",
    "Pagos puntuales (quincena y fin de mes).",
    "Agradable ambiente laboral.",
    "Somos una empresa que promueve la igualdad de oportunidades y no discriminamos por motivo de origen, edad, sexo, discapacidad u otra condición , asi como la inclusión laboral de personas con discapacidad."
   ],
   "url": "https://www.laborum.pe/job/lavoro-peru/auxiliar-de-almacen-turno-tarde-fijo-villa-el-salvador-villa/6aa7545d0b351bbb217e0948"
  },
  {
   "id": 151,
   "puesto": "Anfitriona Full Time",
   "empresa": "REC01 S.A.C.",
   "cat": "Tecnología",
   "min": 1200,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 13,
   "vence": "2026-10-13",
   "restan": 17,
   "score": 91,
   "resumen": "RESUMEN: ¿Te gustaría desarrollar tu carrera en un entorno innovador?",
   "funciones": [
    "Conducir la transmisión (gestión de datos para plataformas internacionales) siguiendo los protocolos y estándares establecidos.",
    "Mantener una postura, imagen y presencia profesional durante las transmisiones.",
    "Seguir las instrucciones operativas brindadas por el supervisor a cargo, como la presentación frente a cámara y el correcto uso de los materiales operativos.",
    "Modalidad y horario",
    "Modalidad presencial en San Isidro.",
    "Horarios fijos: 6:50am a 3:35pm | 1:20pm a 10:05pm | 9:50pm a 7:20am.",
    "Un día de descanso a la semana."
   ],
   "requisitos": [
    "Carisma y seguridad al interactuar.",
    "Excelente comunicación, actitud positiva y proactividad.",
    "Compromiso, responsabilidad y disposición para aprender."
   ],
   "beneficios": [
    "Sueldo base de S/ 1,200 a S/1,400 + S/ 200 por concepto de movilidad.",
    "Sobre tasa (35%) trabajo nocturno.",
    "Ingreso a planilla desde el primer día de capacitación.",
    "Beneficios de ley: CTS, gratificaciones, vacaciones y EsSalud.",
    "Capacitación integral y oportunidades de crecimiento profesional.",
    "Estabilidad laboral en una empresa con proyección internacional.",
    "¡Postula ahora y forma parte de una empresa donde la tecnología, la innovación y el desarrollo profesional impulsan cada oportunidad!"
   ],
   "url": "https://www.laborum.pe/job/rec01-s-a-c/anfitriona-full-time-sin-experiencia/6aa5e9780b351bbb217d4902"
  },
  {
   "id": 152,
   "puesto": "Operador de Puente Grúa",
   "empresa": "Eurofirms Group",
   "cat": "Ingeniería",
   "min": 2200,
   "max": 2600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 14,
   "vence": "2026-12-11",
   "restan": 76,
   "score": 97,
   "resumen": "En Eurofirms Group, creemos en el liderazgo por valores y el crecimiento continuo, poniendo a las personas y su desarrollo profesional primero, potenciando un ambiente de trabajo positivo y colaborativo.",
   "funciones": [
    "Operar el puente grúa de nave para trasladar bobinas de acero, paquetes de tubos, insumos y productos terminados.",
    "Verificar que los materiales recibidos coincidan con los formatos de recepción.",
    "Coordinar las maniobras de izaje y asegurar el uso correcto de las normas de seguridad.",
    "Horarios Disponibles",
    "Disponibilidad para trabajar 12 hrs: 7:30am a 7:30pm / 7:30pm a 7:30am."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 01 año como Operador de puente Grúa de 5 toneladas a más, con manejo de materia prima como bobinas y flejes..",
    "Contar con Curso vigente de Operador de Grúas (Certificación).",
    "Educación mínima: Educación Secundaria",
    "3 años de experiencia",
    "Si cumples con los requisitos, estás interesado y crees que eres el match perfecto, te invitamos a postular",
    "¡Te esperamos!"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día con todos los beneficios de ley.",
    "Salario básico de s/.2200- 2600 a más + Horas extras.",
    "Alimentación cubierta al 100% (almuerzo - cena).",
    "Movilidad de acercamiento (Ida y vuelta)",
    "Pagos quincenales.",
    "Grato ambiente laboral",
    "Línea de carrera."
   ],
   "url": "https://www.bumeran.com.pe/empleos/operador-de-puente-grua-gana-mas-de-2500-solesmovilidadalimentacion-cubiertos-lima-y-chilca-eurofirms-group-1118439999.html"
  },
  {
   "id": 153,
   "puesto": "Cocinero",
   "empresa": "HVA INVERSIONES",
   "cat": "Gastronomía",
   "min": 1800,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 14,
   "vence": "2026-10-12",
   "restan": 16,
   "score": 97,
   "resumen": "Preparar mise en place para platos de la carta.",
   "funciones": [
    "Preparar mise en place para platos de la carta.",
    "Elaborar platos calientes y fríos: ceviches, tiraditos, arroces, sopas, entradas, jaleas, chicharrones y más.",
    "Garantizar la producción en tiempo y forma durante el servicio.",
    "Mantener seguridad e higiene en manipulación, almacenamiento y emplatado de alimentos.",
    "Presentar los platos según los estándares del restaurante.",
    "Mantener orden y limpieza del área de trabajo.",
    "Lugar de trabajo: CERCADO DE LIMA - PLAZA DE ARMAS",
    "Modalidad: PRESENCIAL. -Requerimientos- Educación mínima: Técnico2 años de experienciaEdad: entre 20 y 45 añosPalabras clave: chef, cocinero"
   ],
   "requisitos": [
    "Experiencia mínima de 2 años en restaurantes.",
    "Experiencia en cocina caliente y fría (ceviches, tiraditos, sopas, arroces, entradas marinas y frituras).",
    "Dominio en preparación de chicharrones y frituras (para chicharronero).",
    "Estudios técnicos truncos o completos en gastronomía o afines (deseable).",
    "Disponibilidad para laborar en Cercado de Lima (Plaza de Armas).",
    "Disponibilidad para laborar tiempo completo.",
    "Actitud proactiva, responsabilidad y trabajo en equipo."
   ],
   "beneficios": [
    "Sueldo: S/. 1800 según evaluación y experiencia.",
    "Ingreso a planilla y asignación familiar.",
    "Pagos quincenales.",
    "Alimentación cubierta.",
    "Pago doble por feriados laborados.",
    "Comisiones individuales y semanales.",
    "30% de descuento en consumos (en nuestros locales).",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Bonos por metas mensuales.",
    "Un día de descanso a la semana."
   ],
   "url": "https://www.laborum.pe/job/hva-inversiones/cocinero-chicharronero-pescados-y-mariscos-cerca-a-la-plaza/6aa4ab5a0b351bbb217c34bc"
  },
  {
   "id": 154,
   "puesto": "Vendedora Part time",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ventas",
   "min": 700,
   "max": 700,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 14,
   "vence": "2026-12-11",
   "restan": 76,
   "score": 94,
   "resumen": "Responsable de brindar una adecuada atención al cliente de acuerdo a los estándares de la compañía.",
   "funciones": [
    "Responsable de brindar una adecuada atención al cliente de acuerdo a los estándares de la compañía.",
    "Alcanzar los objetivos de ventas establecidos por la empresa.",
    "Apoyar con los stocks y hacer inventarios.",
    "Encargada de la exhibición y orden de productos en el lugar de trabajo.",
    "Apoyo en preparación de pedidos web."
   ],
   "requisitos": [
    "Solo mujeres entre 20 y 35 años de cualquier nacionalidad.",
    "Contar con una experiencia mínima de 1 año en venta retail de importantes marcas ubicadas en Centros Comerciales o boutique puerta a calle.",
    "Disponibilidad para trabajar 28 horas semanales. Se pueden trabajar 6 dias y descansar 1 o trabajar 5 días y descansar 2 dependiendo de como se arme el horario. El horario de la boutique es de lunes a sabádos de 10 am a 8 pm y los domingos de 10 am a 5 pm. Dentro de ese horario se organiza el trabajo semanal.",
    "Excelente trato, servicio al cliente y muy buena actitud."
   ],
   "beneficios": [
    "Grato ambiente laboral y posibilidad de hacer línea de carrera.",
    "Los primeros 3 meses será por recibo de honorarios y luego el pago mediante planilla Remype (15 vacaciones al año + Essalud + Asignación Familiar).",
    "Sueldo básico (S/.700) + comisiones desde la 1ra venta (en promedio son S/.400 a más) + bono de inventario mensual (hasta S/.100).",
    "Entrega de uniforme.",
    "EPS a precio corporativo en caso deseen tomarlo.",
    "Descuentos en productos de la marca."
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedora-part-time-adn-retail-s.a.c.-1118439858.html"
  },
  {
   "id": 155,
   "puesto": "Auxiliar de archivo",
   "empresa": "GRUCOIN SAC",
   "cat": "Contabilidad",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 14,
   "vence": "2026-10-12",
   "restan": 16,
   "score": 94,
   "resumen": "RESUMEN: ¡En GCI estamos buscando un Auxiliar de Archivo!",
   "funciones": [
    "Recibir, revisar y clasificar la documentación remitida al archivo del área de Contabilidad y otros que se le asignen.",
    "Organizar y archivar física y virtualmente la documentación utilizando criterios definidos por tipo documental, periodo, empresa, proyecto, proveedor u otro criterio establecido por Contabilidad.",
    "Mantener actualizado el inventario y ubicación de los archivos bajo su responsabilidad.",
    "Realizar la custodia, trazabilidad y control de la documentación contable y otras que se le asignen, entregada al archivo.",
    "Mantener los documentos en condiciones adecuadas de conservación, orden y seguridad.",
    "Atender las solicitudes de documentación efectuadas por Contabilidad, Auditoría, Finanzas, Administración u otras áreas autorizadas.",
    "Realizar seguimiento a la devolución de los documentos retirados temporalmente del archivo.",
    "Digitalizar documentación cuando sea requerido y almacenarla siguiendo la estructura de carpetas definida por la organización.",
    "Identificar documentación deteriorada, incompleta, duplicada o que presente alguna incidencia y comunicarla al responsable de Contabilidad o el área dueña de los archivos custodiados.",
    "Realizar el ordenamiento periódico del archivo y controlar la capacidad de almacenamiento disponible.",
    "Preparar documentación para transferencias, almacenamiento externo o eliminación documental, previa autorización de los responsables correspondientes.",
    "Mantener estricta confidencialidad respecto de la información a la que tenga acceso."
   ],
   "requisitos": [
    "Egresado técnico o universitario en Contabilidad, Administración, Archivística, Gestión Documental o carreras afines.",
    "Manejo de Excel y Word a nivel intermedio",
    "Experiencia mínima de 1 año en el puesto o similares.",
    "Disponibilidad de trabajo en San Juan de Miraflores."
   ],
   "beneficios": [
    "Ingreso a planilla de Régimen General con todos los beneficios de ley.",
    "Salario: S/.1300 + bono de alimentación S/.88 + movilidad S/.100.",
    "Subvención del 50% de alimentos.-Requerimientos- Educación mínima: Técnico años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, file, archivo"
   ],
   "url": "https://www.laborum.pe/job/grucoin-sac/auxiliar-de-archivo/6aa4c2af0b351bbb217c7808"
  },
  {
   "id": 156,
   "puesto": "Cajero",
   "empresa": "Eurofirms Group",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 14,
   "vence": "2026-12-11",
   "restan": 76,
   "score": 92,
   "resumen": "En Eurofirms Group , creemos en el liderazgo por valores y el crecimiento continuo, poniendo a las personas y su desarrollo profesional primero, potenciando un ambiente de trabajo positivo y colaborativo.",
   "funciones": [
    "Atención al cliente y promoción de ventas, brindando un servicio ágil y de calidad.",
    "Manejo de caja, recepción y reposición de mercadería en tienda y almacén.",
    "Manipulación de alimentos, limpieza integral del local y apoyo en inventarios."
   ],
   "requisitos": [
    "Deseable contar con 3 meses de experiencia en atención al cliente.",
    "Contar con Carnet de Sanidad con Manipulación de Alimentos . (Indispensable)",
    "Contar con disponibilidad para desempeñar funciones operativas propias de tienda (trabajo de pie, carga ligera, desplazamiento constante, entre otros).",
    "Experiencia en cargos similares: atención al Cliente; cajero, reponedor y promoción de productos, y conocimientos básicos de ventas.",
    "Turno rotativo de 07am - 03pm / 03pm - 11pm / 11pm - 07am"
   ],
   "beneficios": [
    "Sueldo básico 1,130 soles.",
    "Bono objetivos de hasta 120 soles.",
    "Bonos por no contar con faltante de caja de 50 soles.",
    "Bono por Movilidad de S/ 150.",
    "Bono nocturno.",
    "Ingreso a planilla con todos los beneficios de ley desde el 1er día.",
    "Línea de Carrera.",
    "Si cumples con los requisitos, estás interesado y crees que eres el match perfecto, te invitamos a postular ¡Te esperamos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/cajero-a-multifuncional-planilla-completabono-responsabilidadbono-movilidadbono-por-ventas-eurofirms-group-1118439998.html"
  },
  {
   "id": 157,
   "puesto": "Atención al Cliente",
   "empresa": "PIEERS",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 14,
   "vence": "2026-10-12",
   "restan": 16,
   "score": 92,
   "resumen": "RESUMEN: ATENCIÓN AL CLIENTE - CERCADO DE LIMA",
   "funciones": [
    "Brindar cordial atención a los clientes, exponiendo las características y ventajas de nuestras marcas.",
    "Promover y realizar la venta de prendas en tienda, siguiendo los procedimientos establecidos.",
    "Cumplir con los objetivos de ventas.",
    "Las demás funciones inherentes al cargo que le sean asignadas por su jefe inmediato."
   ],
   "requisitos": [
    "Mínimo 6 meses de experiencia laboral en ventas, impulsando productos o atención al cliente.",
    "Pasión y Gusto por la Moda.",
    "Disponibilidad inmediata y a tiempo completo.",
    "Disponibilidad de laborar de manera presencial."
   ],
   "beneficios": [
    "Sueldo básico: 1130 soles + bono + horas extras = 1400 a 1500 soles",
    "Buen clima laboral.",
    "Planilla con todos los beneficios de ley.",
    "Capacitaciones constantes.",
    "Crecimiento laboral.",
    "¡SÉ PARTE DE NUESTRA FAMILIA Y CRECE CON NOSOTROS!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 19 y 35 añosPalabras clave: demostrador, feriante, viajante, agent, representative, agente, representante"
   ],
   "url": "https://www.laborum.pe/job/pieers/agente-de-prevencion-cercado-de-lima-frente-del-congreso/6aa4b61f0b351bbb217c5402"
  },
  {
   "id": 158,
   "puesto": "Asistente de Almacén Full Time Topitop Gamarra",
   "empresa": "Topitop",
   "cat": "Logística",
   "min": 1180,
   "max": 1180,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 14,
   "vence": "2026-10-12",
   "restan": 16,
   "score": 92,
   "resumen": "RESUMEN: Somos una de las principales cadenas de Retail textil peruano, con presencia regional y con más de 70 tiendas a nivel nacional.",
   "funciones": [
    "Recepción y despacho de mercadería además de Gestionar el control de la recepción de las bolsas.",
    "Ingresar oportunamente las guías de recepción y/o despacho de mercadería.",
    "Reportar las discrepancias encontradas durante la recepción de mercadería dentro de las primeras 48 horas de detectado el hecho.",
    "Seguimiento y cumplir con los balanceos y/o transferencias entre tiendas."
   ],
   "requisitos": [
    "Experiencia minima de 6 meses en almacén(deseable).",
    "Secundaria completa.",
    "Contar con total disponibilidad para trabajar de lunes a domingo (con un día de descanso semanal)."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día de labor.",
    "Sueldo fijo S/. 1180 + bono por cumplimiento de metas de tienda.",
    "20% de descuento en compras dentro de nuestras tiendas.",
    "Línea de crédito sin interés para compras en nuestras tiendas a partir del 3er mes.",
    "Oportunidad de crecimiento y desarrollo.",
    "Reconocimiento por ser elegido el colaborador del mes· Interesados presentarse en nuestra tiendas Topitop ubicadas en",
    "TOPITOP- Agustín Gamarra 691, La Victoria",
    "HORARIO: LUNES a VIERNES de 10:00 a.m. a 12:00 p.m. o de 4:00 p.m. a 6:00 p.m.· No olvides llevar los siguientes documentos: CV, copia de DNI (titular e hijos), certijoven o certiadulto, recibo agua o luz· Preguntar por el jefe de tienda.",
    "¡No dejes pasar la oportunidad de pertenecer a la FAMILIA TOPITOP!",
    "¡Te Esperamos!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/topitop/asistente-de-almacen-full-time-topitop-gamarra/6aa4f6250b351bbb217ce4f1"
  },
  {
   "id": 159,
   "puesto": "Ayudante de almacén",
   "empresa": "Reyemsa",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 14,
   "vence": "2026-10-12",
   "restan": 16,
   "score": 89,
   "resumen": "Ejecutar las labores de recepción, almacenamiento, despacho de productos y el cumplimiento de los estándares de calidad y seguridad establecidos por la empresa.",
   "funciones": [
    "Ejecutar las labores de recepción, almacenamiento, despacho de productos y el cumplimiento de los estándares de calidad y seguridad establecidos por la empresa.",
    "Clasificar y ubicar los productos en las zonas designadas del almacén.",
    "Preparar pedidos (picking) de acuerdo con las órdenes de venta o requerimientos de tienda.",
    "Embalar productos (packing) asegurando su correcta protección para el despacho.",
    "Participar en inventarios físicos y conteos cíclicos, reportando diferencias.",
    "Otras funciones que su jefe inmediato le asigne, según la necesidad del área de Almacén."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 6 meses realizando funciones similares en plantas industriales, almacenes o áreas operativas (deseable).",
    "Conocimiento de Técnicas de estiba y desestiba (deseable).",
    "Disponibilidad para trabajar de lunes a viernes de 7:30am a 5:50pm, y hacer horas extras los sábado de 7:30am a 9:45am, en Cercado de Lima. Realizar horas extras cuando sea requerido por el jefe inmediato."
   ],
   "beneficios": [
    "Ingreso a Planilla con todos los beneficios de ley.",
    "Salario mensual S/ 1,130 (monto bruto) + Tarjeta de Alimentos.",
    "Comedor subvencionado.",
    "Oportunidad de crecimiento profesional y línea de carrera.",
    "Formarás parte de una empresa líder en el rubro industrial."
   ],
   "url": "https://www.laborum.pe/job/reyemsa/ayudante-de-almacen-planta-industrial/6aa4f6ec0b351bbb217ce7fe"
  },
  {
   "id": 160,
   "puesto": "Operario de Producción",
   "empresa": "INDUSTRIA QUIMICA MENDOZA E HIJOS S.A.C.",
   "cat": "Ingeniería",
   "min": 1250,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 14,
   "vence": "2026-10-12",
   "restan": 16,
   "score": 85,
   "resumen": "RESUMEN: Empresa industrial IQMEH con más de 20 años de experiencia en mercado internacional y nacional. Actualmente contamos con presencia en Perú, México, Ecuador, Bolivia y Paraguay.",
   "funciones": [
    "Apoyo en la producción diaria",
    "Colocar los productos en cajas",
    "Embalaje de cajas",
    "Limpieza de máquinas y área de trabajo",
    "Lunes a sábado 7am a 7pm (1 semana de día y noche)"
   ],
   "requisitos": [
    "Secundaria completa",
    "Disponibilidad de horarios rotativos (De lunes a sábado)",
    "Vivir cerca a San Juan de Lurigancho - Alt. Paradero 2 Cto. Grande",
    "Experiencia en producción"
   ],
   "beneficios": [
    "Remuneración : S/ 1,250",
    "Ingreso a planilla, con todos los beneficios de ley (Gratificación, CTS, bonificación nocturna, asignación familiar, Bono por asistencia, seguro de salud y vacaciones).",
    "Pago de horas extras y Bono Nocturno.",
    "Bono de S/ 50.00 mensual sujeto a asistencia.",
    "Pertenecer a una empresa en crecimiento.",
    "Descuentos corporativos. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: operator, operario, operator, production, produce, fabricacion, manufactoring, elaboracion, making, manufactura, produccion"
   ],
   "url": "https://www.laborum.pe/job/industria-quimica-mendoza-e-hijos-s-a-c/operario-de-produccion-turnos-rotativos-12-horas-rotativo/6aa4a9bd0b351bbb217c2fd6"
  },
  {
   "id": 161,
   "puesto": "Asesor Postpago Claro",
   "empresa": "Consultora CyL",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 14,
   "vence": "2026-11-11",
   "restan": 46,
   "score": 81,
   "resumen": "NO POSTULES POR AQUÍ , ESCRIBENOS AL 907082961 ENVIANOS TU CV + CUL",
   "funciones": [
    "🛠️ Recepción de llamadas con el objetivo de resolver consultas",
    "🛠️ Solicitudes y reclamos del servicio móvil",
    "🛠️ Realización de ventas de portabilidad",
    "🛠️ Líneas nuevas y migraciones cuidando la satisfacción del cliente"
   ],
   "requisitos": [
    "📝 Secundaria completa",
    "📝 Disponibilidad para turnos rotativos (lunes a sábado)",
    "📝 DNI o CE vigente"
   ],
   "beneficios": [
    "✅ Planilla desde el primer día",
    "✅ Sueldo S/.1300",
    "✅ Capacitaciones pagadas",
    "✅ Comisiones ilimitadas"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-postpago-claro-full-time-o-part-time-consultora-cyl-1118439822.html"
  },
  {
   "id": 162,
   "puesto": "Chofer de Reparto y Almacén con Licencia AIIIb",
   "empresa": "Wellco Peruana SA",
   "cat": "Logística",
   "min": 2000,
   "max": 2400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 15,
   "vence": "2026-10-11",
   "restan": 15,
   "score": 100,
   "resumen": "RESUMEN: ¡Estamos buscando CHOFER ALMACENISTA!",
   "funciones": [
    "Revisar y contar los artículos destinados al despacho y los productos que ingresan al almacén.",
    "Preparar y apoyar en el embalaje de mercadería para los despachos.",
    "Controlar físicamente el ingreso y salida de los productos transportados en la unidad asignada.",
    "Gestionar y verificar las guías de remisión correspondientes a los despachos.",
    "Estibar y asegurar correctamente la mercadería en la unidad de transporte y en las zonas de almacenamiento.",
    "Apoyar en la ejecución de inventarios asignados por la jefatura.",
    "Realizar el traslado y entrega de mercadería, cumpliendo las rutas y procedimientos establecidos."
   ],
   "requisitos": [
    "Licencia de conducir AIIIB indispensable que este vigente (no negociable)",
    "Experiencia y conocimientos prácticos en almacén y despacho.",
    "Experiencia en manejo y traslado de mercadería.",
    "Disponibilidad para trabajar en Lurín.",
    "De preferencia, residir en Lurín o zonas aledañas."
   ],
   "beneficios": [
    "Sueldo: S/ 2,000 - 2,400",
    "Ingreso a planilla desde el primer día.",
    "Todos los beneficios de ley.",
    "Alimentación cubierta al 100%.",
    "Movilidad de acercamiento (recojo en Km40)",
    "LUGAR DE TRABAJO LURÍN",
    "De cumplir con el perfil, no dudes en postular!. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaPalabras clave: conductor, driver, choferes, chofer, delivery, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/wellco-peruana-sa/chofer-de-reparto-y-almacen-con-licencia-aiiib/6aa3797e0b0fa0541eb52f21"
  },
  {
   "id": 163,
   "puesto": "Técnico de servicios generales en gasfitería",
   "empresa": "ACADEMIA VONEX",
   "cat": "Recursos Humanos",
   "min": 2090,
   "max": 2090,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 15,
   "vence": "2026-10-11",
   "restan": 15,
   "score": 97,
   "resumen": "¿Tienes experiencia en gasfitería, eléctrico y buscas estabilidad laboral? Buscamos un Técnico en Gasfitería para garantizar la operatividad y seguridad de las instalaciones eléctricas en nuestras sedes en Lima.",
   "funciones": [
    "Realizar instalaciones y reparaciones de tuberías, conexiones, válvulas, griferías, sanitarios y otros componentes de los sistemas de agua y desagüe.",
    "Detectar y solucionar fugas de agua, filtraciones, obstrucciones, atoros y problemas de presión en las instalaciones sanitarias.",
    "Ejecutar trabajos de mantenimiento preventivo y correctivo de redes de agua, desagüe y sistemas sanitarios de las instalaciones.",
    "Realizar trabajos de instalación, reparación y mantenimiento de inodoros, lavatorios, urinarios, duchas, grifos, lavaderos y otros aparatos sanitarios.",
    "Revisar periódicamente las instalaciones sanitarias, identificando deterioros, fugas, desperfectos o condiciones que puedan generar averías.",
    "Ejecutar cambios o reparaciones de tuberías y accesorios, considerando el tipo de material y las características de la instalación.",
    "Identificar necesidades de materiales, herramientas y repuestos para la ejecución de los trabajos asignados y comunicar oportunamente los requerimientos.",
    "Mantener limpia y ordenada el área de trabajo después de culminar las actividades de reparación o instalación."
   ],
   "requisitos": [
    "Secundaria completa y/o formación técnica en Gasfitería, Instalaciones Sanitarias",
    "Conocimientos en reparación de griferías, sanitarios, tuberías, válvulas y conexiones.",
    "Experiencia en detección y reparación de fugas, atoros y filtraciones.",
    "Manejo de herramientas propias del oficio.",
    "Disponibilidad para trabajar en turnos rotativos (mañana, tarde y noche)",
    "Disponibilidad para movilizarse entre sedes asignadas (se brindan pasajes según cobertura)"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día, régimen general al 100%",
    "Asignación familiar (S/ 113)",
    "Seguro de vida desde el primer día de trabajo",
    "Capacitaciones internas continuas",
    "Sueldo: S/ 2090.00 en planilla completa. -Requerimientos- Educación mínima: Técnico años de experienciaEdad: entre 25 y 50 añosPalabras clave: tecnologo, tecnico, tech, technician, technology, tecnologia, technologist"
   ],
   "url": "https://www.laborum.pe/job/academia-vonex/tecnico-de-servicios-generales-en-gasfiteria-planilla/6aa35c190b0fa0541eb4d2e7"
  },
  {
   "id": 164,
   "puesto": "Analista 1",
   "empresa": "Talento Humano Perú",
   "cat": "Ingeniería",
   "min": 3300,
   "max": 3450,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Cusco",
   "fuente": "Bumeran",
   "dias": 15,
   "vence": "2027-01-09",
   "restan": 105,
   "score": 95,
   "resumen": "Nuestro cliente, Empresa del Sector Eléctrico, nos ha encargado la búsqueda de un (1) Analista 1 – Activo Fijo No Eléctrico.",
   "funciones": [
    "Gestionar, supervisar, coordinar y controlar la administración de los activos fijos No eléctricos, así como velar por su seguridad y vigilancia.",
    "Realizar el registro y control de los activos fijos no Eléctricos a nivel empresarial, terrenos, edificaciones, automóviles, muebles y enseres y equipos diversos.",
    "Realizar el registro y control de las fichas de responsabilidad de los bienes asignados a los trabajadores.",
    "Controlar la hoja de costos de las obras en curso y en ejecución en coordinación con el área de proyectos.",
    "Supervisar y llevar a cabo los inventarios de activos fijos no eléctricos (terrenos, edificaciones, vehículos, muebles y enseres y equipos diversos) a nivel empresarial.",
    "Apoyar en gestiones de saneamiento de las propiedades de la empresa.",
    "Elaborar la documentación de baja de los bienes asignados para la venta y tramitar su subasta de acuerdo al cronograma.",
    "Coordinar con el área de logística la administración del módulo de existencias y su reporte de los Kardex valorizados de los suministros y unidades de reemplazo.",
    "Elaborar los libros contables del control de Kardex valorado de la empresa."
   ],
   "requisitos": [
    "Título Profesional en Administración y/o Contabilidad y/o Economía y/o Ingeniería Industrial.",
    "Experiencia general: Tres (03) años o más en funciones relacionadas con la carrera profesional, computables a partir de la condición de egresado.",
    "Conocimientos en gestión de inventarios físicos y codificación de activos.",
    "Manejo de GIS (Geographic Information System). SQL, herramientas de visualización (Power BI, Tableau).",
    "Manejo de ERP SAP módulo MM o AM (Materials Management/ Asset Accounting).",
    "Ms. Excel, nivel avanzado."
   ],
   "beneficios": [
    "Remuneración entre S/. 3,300.00 a S/. 3,450.00 soles. (De acuerdo a evaluación)",
    "Ingreso a planilla DL 728, con todos los beneficios de acuerdo a ley.",
    "Plazo Determinado, 03 meses periodo de prueba.",
    "LUGAR DE TRABAJO DE LA EMPRESA",
    "Dpto. de Cusco.",
    "Horario: Lunes a Viernes 7:30 a.m. y 1:00 p.m. y 1:45 p.m. a 5:00 p.m.",
    "Trabajo Presencial.",
    "FECHA LÍMITE DE RECEPCIÓN DE CURRICULUM VITAE: SÁBADO 19 DE SETIEMBRE DEL 2026"
   ],
   "url": "https://www.bumeran.com.pe/empleos/analista-1-activo-fijo-no-electrico-|-cusco-talento-humano-peru-1118439538.html"
  },
  {
   "id": 165,
   "puesto": "Anfitriona Automotriz A1",
   "empresa": "A&RC Consultores",
   "cat": "Ventas",
   "min": 2700,
   "max": 2700,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 15,
   "vence": "2026-11-10",
   "restan": 45,
   "score": 92,
   "resumen": "Dar la bienvenida a los clientes y/o Ejecutivos de la compañía.",
   "funciones": [
    "Dar la bienvenida a los clientes y/o Ejecutivos de la compañía.",
    "Atención a clientes de nuestros productos y/o a ejecutivos de la compañía en red de concesionarias y/o Malls, dar bienvenida, información, dirigirlos a sus lugares en eventos corporativos.",
    "Salir en fotografías de catálogo, y/o eventos promocionales, participar en desfiles temáticos, etc.",
    "Dar cotizaciones, presupuestos, etc. (se les dará capacitación orientada, pagada desde el primer día)",
    "Opcional proceso de venta de vehículo con comisiones adicionales al sueldo ofrecido, por auto colocado.",
    "Participar en desfiles en eventos promocionales, de realizarse"
   ],
   "requisitos": [
    "Edad: Hasta 35 años",
    "Buena Presencia y Figura (NO Excluyente)",
    "Orientadas a la excelencia del servicio",
    "Proactiva y buen trato al cliente.",
    "Deseos de superación y trabajo en equipo.",
    "Personalidad extrovertida, conversadora, carismática y muy sociable.",
    "Disponibilidad de traslado a cualquier punto de lima. (facilidad de elegir cercanía a domicilio y/o centro de estudios)",
    "Ser atenta, servicial, amable y siempre disponible en cualquier actividad que se le encomiende.",
    "Rapidez e iniciativa.",
    "No requiere experiencia (capacitaciones constantes).",
    "Disponibilidad inmediata.",
    "Las Srtas. Interesadas, Postular enviando su CV con FOTOGRAFIA ACTUALIZADA, para arc.seleccion en gmail. com y asunto ANFIAUTO-2026/II. y por favor confirmar postulación por whatsapp con asistente de selección de personal, Srta. Andrea Garcés al Whatsapp +51 933 089 161 ."
   ],
   "beneficios": [
    "Ingreso a planillas con todos sus beneficios de ley desde el primer día de Capacitación.",
    "Oportunidad de desarrollar una Línea de Carrera",
    "Asignación familiar de tener hijos menores de edad",
    "Salario: 2500 (fijo) + S/.200 (viáticos semanales) = S/. 2700 / mes",
    "Se dará Facilidades con sus horarios a las chicas que estudien.",
    "Horario y Contacto",
    "Mier, Jue, Vie, Sab de 9:30am a 5:30pm (Incluye refrigerio) y dos domingos al mes en el mismo horario."
   ],
   "url": "https://www.bumeran.com.pe/empleos/anfitriona-automotriz-a1-2026-marcas-de-autos-arc-consultores-1118439682.html"
  },
  {
   "id": 166,
   "puesto": "Analista de Mejora Continua",
   "empresa": "Grupo CONSERH",
   "cat": "Logística",
   "min": 2300,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 15,
   "vence": "2026-11-10",
   "restan": 45,
   "score": 90,
   "resumen": "Realizar mediciones en planta y elaborar procedimientos de trabajo y diagramas de flujo",
   "funciones": [
    "Realizar mediciones en planta y elaborar procedimientos de trabajo y diagramas de flujo",
    "Ejecutar cálculos estadísticos y proponer soluciones de mejora bajo metodología LEAN",
    "Capacitar y transmitir información a los equipos del CD"
   ],
   "requisitos": [
    "Formación técnica o universitaria en Gestión, Ingeniería o Logística",
    "2 años de experiencia en roles de calidad/proyectos aplicando LEAN en almacenes o CD",
    "Manejo de Excel, Visio y algún paquete estadístico"
   ],
   "beneficios": [
    "Banda salarial S/ 2,300 – S/ 2,500 (Previa evaluación)",
    "Bono de movilidad S/ 150",
    "Alimentación cubierta al 100%",
    "Horario: lunes a viernes, 8:00 a.m. a 6:00 p.m.",
    "Modalidad presencial – Planta en Villa El Salvador",
    "Si cumples con el perfil, postúlate ahora enviando tu CV al 907 383 003"
   ],
   "url": "https://www.bumeran.com.pe/empleos/analista-de-mejora-continua-cd-logistico-villa-el-salvador-grupo-conserh-1118438074.html"
  },
  {
   "id": 167,
   "puesto": "Auxiliar de reclutamiento y Selección en campo",
   "empresa": "GRUCOIN SAC",
   "cat": "Recursos Humanos",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 15,
   "vence": "2026-10-11",
   "restan": 15,
   "score": 89,
   "resumen": "RESUMEN: ¿Te apasiona la búsqueda de talento y el contacto directo con las personas?",
   "funciones": [
    "Realizar reclutamiento en campo para puestos operativos.",
    "Difundir vacantes en diferentes puntos estratégicos.",
    "Realizar filtro curricular y entrevistas iniciales.",
    "Cumplir con los objetivos de cobertura en los tiempos establecidos."
   ],
   "requisitos": [
    "Egresado de Psicología, Administración, Recursos Humanos o carreras afines.",
    "Experiencia en reclutamiento masivo de personal operativo o trabajo de campo (deseable).",
    "Disponibilidad para realizar captación de candidatos en diferentes zonas.",
    "Habilidades de comunicación, organización y orientación a resultados."
   ],
   "beneficios": [
    "Sueldo S/1,300.00 Bono de alimentación (S/.88 )y movilidad (S/.100) .",
    "Pago completo de los gastos de movilización a los distintos puntos.",
    "Ingreso a planilla Régimen General.",
    "Subvención del 50% de alimentación",
    "Oportunidad de aprendizaje y crecimiento profesional.",
    "Excelente clima laboral.-Requerimientos- Educación mínima: Universitario años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, reclutador, recruiter, headhunter, scout, reclutamiento, terreno, territorio"
   ],
   "url": "https://www.laborum.pe/job/grucoin-sac/auxiliar-de-reclutamiento-y-seleccion-en-campo-temporal/6aa389440b0fa0541eb559b4"
  },
  {
   "id": 168,
   "puesto": "Auxiliar de Almacén en Cañete",
   "empresa": "Multiservicios e Inversiones Inmobitec SAC",
   "cat": "Logística",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 15,
   "vence": "2026-10-11",
   "restan": 15,
   "score": 89,
   "resumen": "Apoyar en la recepción, revisión y registro de materiales, herramientas y equipos.",
   "funciones": [
    "Apoyar en la recepción, revisión y registro de materiales, herramientas y equipos.",
    "Verificar cantidades y condiciones de los materiales recibidos según la documentación correspondiente.",
    "Registrar y actualizar entradas y salidas de materiales en los formatos o sistemas establecidos.",
    "Apoyar en el control de stock e informar diferencias o faltantes.",
    "Organizar y mantener el almacén ordenado, asegurando la correcta ubicación de los materiales.",
    "Preparar y entregar materiales al personal autorizado según los requerimientos aprobados.",
    "Apoyar en la elaboración y control de guías, vales de salida y demás documentos del almacén.",
    "Participar en inventarios físicos periódicos.",
    "Mantener actualizado el archivo documentario del almacén.",
    "Informar incidencias relacionadas con materiales deteriorados, faltantes o sobrantes.",
    "Mantener el orden, limpieza y cumplimiento de las normas de seguridad y uso de EPP.",
    "Realizar otras funciones afines asignadas por el responsable del almacén."
   ],
   "requisitos": [
    "Disponibilidad para laborar en Cañete (obra) bajo régimen 5x2.",
    "Disponibilidad para permanecer en campamento durante los días de trabajo.",
    "Deseable experiencia en almacenes de construcción, ferretería, logística o posiciones similares.",
    "Responsabilidad, orden y compromiso con el cumplimiento de procedimientos."
   ],
   "beneficios": [
    "Sueldo: S/1,500 mensuales.",
    "Régimen de trabajo 5x2.",
    "100% de alimentación cubierta.",
    "100% de alojamiento cubierto en campamento.",
    "Trabajo de lunes a viernes.",
    "Excelente oportunidad para desarrollarte en el sector construcción."
   ],
   "url": "https://www.laborum.pe/job/multiservicios-e-inversiones-inmobitec-sac/auxiliar-de-almacen-en-canete-obra/6aa34bcd0b0fa0541eb49ad9"
  },
  {
   "id": 169,
   "puesto": "Ayudante de producción",
   "empresa": "Inversiones San Gabriel S.A.",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 15,
   "vence": "2026-10-11",
   "restan": 15,
   "score": 88,
   "resumen": "¡ÚNETE A NUESTRO EQUIPO INDUSTRIAL EN LURÍN!",
   "funciones": [
    "Realizar la producción asignada según los estándares de la planta.",
    "Participar en la mejora continua de los procesos productivos.",
    "Reportar incidentes o irregularidades al supervisor inmediato.",
    "Mantener el orden, limpieza y seguridad en su puesto de trabajo.",
    "Cumplir con las normas de seguridad y salud ocupacional.",
    "Otras funciones que le asigne su jefe inmediato."
   ],
   "requisitos": [
    "Ser mayor de 18 años.",
    "Varones y mujeres",
    "Secundaria completa.",
    "Con o sin experiencia previa.",
    "Habilidades manuales y actitud proactiva hacia el trabajo.",
    "Capacidad para trabajar en equipo y seguir instrucciones.",
    "Residir en zonas cercanas a Lurín."
   ],
   "beneficios": [
    "Sueldo mensual: S/ 1,130 + bono PROVIS de S/ 226(alimentación).",
    "Horario fijo: 2:15 pm a 11:00 pm de lunes a sábados",
    "Pago semanal.",
    "Movilidad de acercamiento a la salida (KM.40)",
    "Ingreso a planilla completa desde el primer día, con todos los beneficios de ley",
    "CTS, gratificaciones, vacaciones, asignación familiar, utilidades, entre otros.",
    "Estabilidad laboral y buen ambiente de trabajo."
   ],
   "url": "https://www.laborum.pe/job/inversiones-san-gabriel-s-a/ayudante-de-produccion-turno-tarde-fijo-8h-lurin-planilla/6aa382ec0b0fa0541eb5494d"
  },
  {
   "id": 170,
   "puesto": "Vendedor(a) en Saga Mall del Sur",
   "empresa": "Confidencial",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 15,
   "vence": "2026-11-10",
   "restan": 45,
   "score": 79,
   "resumen": "Únete a nuestro equipo y representa una marca innovadora en el cuidado de zapatillas.",
   "funciones": [
    "Abordar y asesorar a clientes en tienda",
    "Promover productos de la marca",
    "Impulsar ventas y cumplimiento de metas",
    "Mantener orden y exhibición de productos"
   ],
   "requisitos": [
    "Personas dinámicas, extrovertidas y con iniciativa",
    "Facilidad para abordar clientes y generar ventas",
    "Actitud proactiva y orientación a resultados",
    "Deseable experiencia en ventas o atención al cliente",
    "¡Postúlate y únete a nosotros!",
    "INTERESADOS ENVIAR CV AL 976 024 120"
   ],
   "beneficios": [
    "Horario: Martes a domingo de 1:00 p.m. a 9:00 p.m. (descanso lunes)",
    "Sueldo: S/ 1,130 en Planilla completa desde el primer día (Gratificación, CTS, vacaciones, seguro, asignación familiar, etc.)",
    "Comisiones hasta S/100 Soles",
    "Movilidad: S/ 90 mensual",
    "Disponibilidad para tramitar Carnet de Sanidad"
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedora-en-saga-mall-del-sur-1118438073.html"
  },
  {
   "id": 171,
   "puesto": "Operario de Limpieza",
   "empresa": "CORPORACION ARZOBISPO LOAYZA SOCIEDAD ANONIMA CERRADA",
   "cat": "Recursos Humanos",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 15,
   "vence": "2026-10-11",
   "restan": 15,
   "score": 75,
   "resumen": "RESUMEN: ¡Te invitamos a formar parte de nuestra Familia! Somos la CORPORACIÓN ARZOBISPO LOAYZA, lideres en la educación técnica de carreras de salud.",
   "funciones": [
    "Limpiar los espacios de la Institución.",
    "Apoyar la movilización de bienes muebles.",
    "Lunes a Viernes 2:00 PM A 10:00 PM",
    "Sábados de 1:00 PM A 7:00 PM"
   ],
   "requisitos": [
    "Mínimo secundaria completa",
    "Honesto, Responsable, Puntual",
    "Con o sin experiencia en limpieza de edificios o espacios públicos.",
    "Residir en zonas aledañas a SAN JUAN DE MIRAFLORES, VILLA EL SALVADOR, VILLA MARIA DEL TRIUNFO, CERCADO DE LIMA"
   ],
   "beneficios": [
    "S/ 1500.00 en Planilla completa bajo régimen general",
    "Pagos puntuales y mensuales.",
    "Envíanos tu CV al 922 946 424 Indicando sobre la vacante de MANTENIMIENTO Y EL DISTRITO. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 18 y 45 añosPalabras clave: operator, operario, operator, aseo, higiene, cleaning, limpieza"
   ],
   "url": "https://www.laborum.pe/job/corporacion-arzobispo-loayza-sociedad-anonima-cerrada/operario-de-limpieza-planilla-completa-presencial/6aa367650b0fa0541eb4f685"
  },
  {
   "id": 172,
   "puesto": "Asistente de Negocios Mibanco",
   "empresa": "Manpower",
   "cat": "Educación",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 97,
   "resumen": "Cumplir con todas las tareas y actividades que conforman el Programa Formativo- Escuela.",
   "funciones": [
    "Cumplir con todas las tareas y actividades que conforman el Programa Formativo- Escuela.",
    "Realizar prospecciones de clientes con el apoyo del Asesor Tutor.",
    "Llevar a cabo acompañamientos a los asesores en sus visitas a clientes.",
    "Promoción y captación de los productos del banco.",
    "Apoyar actividades del proceso crediticio en agencia.",
    "Elaborar expedientes de crédito.",
    "Generar reportes de mora y cosechas en agencia como parte del entrenamiento."
   ],
   "requisitos": [
    "Universitarios: Titulado/Egresado/Estudiantes (IX ciclo en adelante) de las carreras de Administración, Economía, Contabilidad, Ingenierías o afines.",
    "Técnicos: Titulado/Egresado/Estudiantes (VI ciclo en adelante) de las carreras de Contabilidad, Administración o afines.",
    "Experiencia en atención al cliente, ventas de intangibles o tangibles (Deseable).",
    "Buena disposición para el trabajo de campo.",
    "Trabajo de Campo (90%) y oficina (10%).",
    "Disponibilidad de Lunes a Viernes de 8:45am a 6:30pm y Sábados de 8:45am a 1:00pm",
    "Residir en o tener disponibilidad de trabajar en Villa Maria del Triunfo (indispensable)."
   ],
   "beneficios": [
    "Planilla Completa desde el 1er día laborado.",
    "Salario: Sueldo básico: S/1130 + Comisiones",
    "Bono de Movilidad: S/200",
    "EPS cubierta al 70%.",
    "Formar parte de uno de los grupos económicos más importantes del país.",
    "Oportunidad de línea de carrera.",
    "Excelente clima laboral.",
    "Descuentos en entidades educativas, de salud y establecimientos comerciales.",
    "Programas de Capacitación.",
    "Durante el proceso formativo tendrás el acompañamiento de un tutor.",
    "Certificación como Asesor de Negocios al finalizar exitosamente el Programa de Escuela."
   ],
   "url": "https://www.laborum.pe/job/manpower/asistente-de-negocios-mibanco-escuela-agencia-jose-galvez-ma/6aa22fdc0b0fa0541eb3ba76"
  },
  {
   "id": 173,
   "puesto": "Asistente de Admisión IRIs",
   "empresa": "ManpowerGroup Perú",
   "cat": "Contabilidad",
   "min": 5000,
   "max": 5000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 16,
   "vence": "2026-12-09",
   "restan": 74,
   "score": 94,
   "resumen": "En Manpower Perú buscamos al mejor talento para asumir el reto como Asistente de Admisión IRIs para la ciudad de Lima ¡La oportunidad de tener un futuro mejor, está aquí!",
   "funciones": [
    "Participar en la elaboración de las fichas de clasificación de intermediarios financieros.",
    "Participar en el seguimiento de los intermediarios financieros con la finalidad de retroalimentar la admisión de riesgos con la evolución de las operaciones con riesgo vigente.",
    "Apoyar en el diseño y la actualización de la metodología de admisión del riesgo de crédito de los intermediarios financieros.",
    "Elaborar informes de situación financiera y elegibilidad de intermediarios requeridos por terceros.",
    "Apoyar en la elaboración de la opinión sobre las propuestas de inversión que le presente la Gerencia de Finanzas e Inversiones, dentro del alcance de su evaluación según las políticas establecidas.",
    "Mantener actualizada la base de datos para la evaluación de riesgo de crédito de intermediarios financieros."
   ],
   "requisitos": [
    "Bachiller en Administración, Economía, Ingeniería Económica, Finanzas, Contabilidad o afines.",
    "Con Cursos en temas relacionados a riesgos (Deseable).",
    "Conocimientos en Financiamiento corporativo, gestión de proyectos, banca y finanzas, riesgos o negocios.",
    "Experiencia 2 años mínimo de experiencia laboral general, contada desde egresado.",
    "Experiencia 1 año en puesto de asistente, trainee, auxiliar o en puestos de similares características y responsabilidades.",
    "Office a nivel intermedio.",
    "Inglés a nivel intermedio."
   ],
   "beneficios": [
    "Ingreso a la planilla directa del cliente.",
    "EPS Mapfre, para ti y tu familia, cubierto hasta el 90% por parte de la empresa.",
    "Seguro contra accidentes personales cubierto al 100% por parte de la empresa.",
    "Seguro de Vida Ley desde el 1er. día de trabajo.",
    "Seguro Oncológico, para ti y tu familia, cubierto al 100% por parte de la empresa.",
    "Asignación por Uniforme, el cual se otorga dos (02) veces al año, siendo un importe anual aproximado de S/. 5,000, supeditado a la política interna y lineamientos corporativos presupuestales.",
    "Otorgamiento del vale por alimentos por el importe de S/. 500.00 soles mensuales.",
    "Bono por cumplimiento de objetivos al año de hasta un tope de dos (02) sueldos, ello supeditado a la política interna y lineamientos de FONAFE.",
    "Horario de Trabajo de lunes a viernes de 8:45 a.m. a 5:30 p.m (PRESENCIAL)",
    "En Manpower Perú, promovemos la inclusión e inserción laboral sin distinción por razón de discapacidad, genero, orientación, identidad y expresión de género, lugar de origen o generación. Por ello ¡Todos los talentos son bienvenidos!",
    "(ÚNICO MEDIO DE POSTULACIÓN). Ingresando al siguiente link: https://empleo.manpower.pe/TrabajosManpower/JobDetail/ASISTENTE-ADMINISTRATIVO/109413",
    "Colocando en palabras clave el siguiente número de Ref. 109413"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-admision-iris--sector-financiero-san-isidro-ref.-109413-manpowergroup-peru-1118438038.html"
  },
  {
   "id": 174,
   "puesto": "Despachador de combustible",
   "empresa": "COESTI S.A.",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 94,
   "resumen": "Despachador(a) de Combustible / Turno tarde",
   "funciones": [
    "Atención al cliente y despacho de combustible.",
    "Manejo de vouchers, vales y efectivo.",
    "Realizar cuadre de caja y reporte de ventas al finalizar el turno.",
    "Mantener el orden y limpieza de la estación.",
    "Cumplir con las normas de seguridad y salud en el trabajo."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia en atención al cliente (deseable).",
    "Manejo de caja (deseable).",
    "Disponibilidad para laborar en turno tarde (de 2pm a 11pm)",
    "Disponibilidad para trabajar en Primax"
   ],
   "beneficios": [
    "Sueldo de S/1130",
    "Movilidad de S/150",
    "35% adicional por horario nocturno",
    "Ingreso a planilla desde el primer día.",
    "Asignación familiar.",
    "Bonos por ventas.",
    "¡Únete a nuestro gran equipo!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: controlador, despachador, dispatcher, nominas, sueldo, paga, retribucion, salary, payslip, payroll, wage, pay, payment, nominista"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/primax-chilca-despachador-de-combustible-sueldo-bono-150/6aa206540b0fa0541eb33d18"
  },
  {
   "id": 175,
   "puesto": "Representante de Servicios",
   "empresa": "CAJA CUSCO",
   "cat": "Ingeniería",
   "min": 1380,
   "max": 1380,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Cusco",
   "fuente": "Bumeran",
   "dias": 16,
   "vence": "2027-01-08",
   "restan": 104,
   "score": 94,
   "resumen": "Caja Cusco, entidad financiera líder en la región sur del Perú, especializada en microfinanzas, ahorros y servicios financieros. Con más de 4,500 colaboradores comprometidos que lo hacen posible.",
   "funciones": [
    "Atender las operaciones de clientes y realizar la venta de servicios brindando un servicio de calidad, transparencia de información y guardando la confidencialidad de las operaciones e información de clientes garantizando la confidencialidad de información de los usuarios.",
    "Orientar a los clientes con información transparente sobre los productos y servicios que se ofrecen, brindando soluciones para asegurar una atención de calidad, cortesía a clientes y preservar una buena imagen institucional.",
    "Realizar la venta de servicios según las metas asignadas para el cumplimiento de objetivos de las Agencia y de la empresa.",
    "Realizar el cuadre de efectivo de forma diaria e informar en caso de identificar sobrantes o faltantes para cumplir con los procedimientos de control interno.",
    "Realizar actividades de control interno que aseguren que las operaciones se realicen cumpliendo los procedimientos internos y normas de entes supervisores, a fin de minimizar riesgos de sanciones."
   ],
   "requisitos": [
    "Experiencia al Cliente",
    "Espíritu Colaborativo",
    "Mejora Continua",
    "Profesional Técnico, Egresado, Bachiller o Estudios truncos de cualquier especialidad de Institutos o Universidades.",
    "Representante de servicios con categoría superior: Seis (6) meses en el mismo cargo o un (1) año como Auxiliar de Atención al Usuario y Calidad o Puestos equivalentes en empresas del Sistema Financiero.",
    "Representante de servicios junior: sin experiencia"
   ],
   "beneficios": [
    "Remuneración fija de S/. 1380 + Bono variable por cumplimiento de metas.",
    "Tarjeta de alimentación trimestral.",
    "Oportunidad de Línea de carrera.",
    "Ingreso a planilla con todos los beneficios de ley.",
    "Opcional: Acceso a seguro privado de salud.",
    "Fecha límite de postulación: LUNES , 14 DE SETIEMBRE del 2026."
   ],
   "url": "https://www.bumeran.com.pe/empleos/representante-de-servicios-san-martin-de-ica-ref-1569-2026-caja-cusco-1118437905.html"
  },
  {
   "id": 176,
   "puesto": "Practicante Profesional de Almacén",
   "empresa": "GRUPO WITHMORY",
   "cat": "Prácticas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 16,
   "vence": "2026-12-09",
   "restan": 74,
   "score": 92,
   "resumen": "Grupo Withmory, empresa peruana líder en el rubro automotriz con más de 30 años de trayectoria, se encuentra en la búsqueda del mejor talento para cubrir la posición de: Practicante Profesional de Almacén.",
   "funciones": [
    "Brindar soporte en la gestión del almacén, asegurando el adecuado control, registro y conservación de herramientas, equipos y materiales, contribuyendo al orden, disponibilidad y operatividad de los recursos.",
    "Apoyar en la gestión y control del inventario de repuestos y lubricantes.",
    "Realizar la recepción y verificación de repuestos provenientes de importación.",
    "Identificar y reportar oportunamente faltantes, sobrantes o diferencias detectadas durante la recepción.Identificar y reportar repuestos dañados, deteriorados o con observaciones.",
    "Recepcionar las compras realizadas a proveedores locales, verificando cantidades y condiciones de los productos.",
    "Realizar la ubicación y almacenamiento adecuado de los repuestos y materiales recibidos.",
    "Apoyar en el orden, clasificación y control físico del almacén.",
    "Brindar soporte en inventarios físicos y conciliaciones de stock.",
    "Mantener actualizada la documentación correspondiente a los ingresos y movimientos de almacén.",
    "Realizar otras funciones asignadas por su jefe inmediato."
   ],
   "requisitos": [
    "Recién egresado(a) de la carrera de Administración, Ing. Industrial, logística o carreras afines.",
    "Experiencia mínima de 03 meses en el área de almacén (deseable).",
    "Excel Nivel Intermedio (Indispensable).",
    "Disponibilidad para laborar en Nicolás Ayllón - Ate de Lunes a Viernes de 8:00 am a 6:30 pm"
   ],
   "beneficios": [
    "Subvención económica: S/ 1,130 mensuales.",
    "Convenio de Prácticas Profesionales",
    "Oportunidad de Desarrollo y línea de Carrera.",
    "Grato ambiente Laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-profesional-de-almacen-grupo-withmory-1118437941.html"
  },
  {
   "id": 177,
   "puesto": "Vendedor para tiendas Bitel en villa el salvador",
   "empresa": "Bitel",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 92,
   "resumen": "RESUMEN: En Alina corp sac nos encontramos en la búsqueda de 15 Vendedores para las tiendas de VILLA EL SALVADOR y VILLA MARIA DEL TRIUNFO",
   "funciones": [
    "Atender y asesorar a los clientes en tienda.",
    "Ofrecer y vender líneas móviles, equipos, portabilidad y servicios Bitel.",
    "Cumplir con las metas de ventas establecidas.",
    "Mantener el orden y correcta exhibición de los productos.",
    "Realizar activaciones y procesos operativos en sistema.",
    "Brindar una atención cordial y orientada a la satisfacción del cliente."
   ],
   "requisitos": [
    "No se requiere experiencia previa.",
    "Disponibilidad para trabajar en Villa El Salvador o Villa María del Triunfo.",
    "Disponibilidad para laborar 48 horas semanales.",
    "Buena actitud, facilidad de comunicación y orientación al cumplimiento de objetivos.",
    "Compromiso, responsabilidad y disposición para aprender."
   ],
   "beneficios": [
    "Ingreso a planilla bajo régimen MYPE desde el primer día.",
    "Sueldo fijo de S/1,130 + comisiones de hasta S/2,000.",
    "Oportunidad de línea de carrera.· Interesados enviar su cv al wsp 910 004 856. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 40 añosPalabras clave: salesman, saleswoman, seller, vendedor, comercial, salesperson"
   ],
   "url": "https://www.laborum.pe/job/bitel/vendedor-para-tiendas-bitel-en-villa-el-salvador/6aa2147f0b0fa0541eb367c2"
  },
  {
   "id": 178,
   "puesto": "Asesor de retenciones oncosalud trabajo de",
   "empresa": "Atento Perú",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 91,
   "resumen": "Sede: Av. La Molina 200, a 2 cuadras del Óvalo Santa Anita.",
   "funciones": [
    "Gestionar la retención y fidelización de clientes de Oncosalud.",
    "Brindar soluciones y alternativas para evitar la baja del servicio.",
    "Cumplir con los protocolos y procedimientos establecidos por la empresa.",
    "Mantener una comunicación efectiva y orientación al cliente.",
    "Horario de Trabajo",
    "Lunes a viernes de 8:00 a.m. a 6:30 p.m.",
    "Descanso fijo sábados y domingos.",
    "Jornada laboral de 48 horas semanales."
   ],
   "requisitos": [
    "Personas con actitud comercial y orientación al cliente.",
    "Disponibilidad para trabajar de manera presencial en La Molina.",
    "Excelente comunicación y disposición para aprender.",
    "Experiencia en ventas, retenciones o atención al cliente (deseable).-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 19 añosConocimientos: Negociación, Resolución de problemas"
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,130",
    "Comisiones ilimitadas (promedio entre S/ 500 y S/ 2,500)",
    "Bono de excelencia de hasta S/ 300",
    "Bono por efectividad desde S/ 200 hasta S/ 2,000 (según cumplimiento de objetivos)",
    "Planilla completa desde la firma de contrato",
    "Inicio: 11/09/2026",
    "Horario: 9:00 a.m. a 3:00 p.m. (Lunes a viernes)",
    "Duración: 12 días",
    "Capacitación remunerada con S/ 40 por día (hasta S/ 480 por todo el proceso).",
    "El pago se realiza después de la firma del contrato.",
    "Fecha referencial de firma: 29/09/2026",
    "Contrato inicial de 6 meses renovable."
   ],
   "url": "https://www.laborum.pe/job/atento-peru/ingreso-inmediato-asesor-de-retenciones-oncosalud-trabajo-de/6aa1fff30b0fa0541eb32a1d"
  },
  {
   "id": 179,
   "puesto": "Promotor de Ventas Sony",
   "empresa": "Touch Lima S.A.C.",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 91,
   "resumen": "Somos una compañía transnacional de soluciones de Sales & Marketing que provee, gestiona y potencia equipos de personas a través de distintas tecnologías.",
   "funciones": [
    "Concentrar la venta de los productos en el punto de venta.",
    "Realizar las visitas y seguimiento de los puntos asignados.",
    "Revisar de forma constante la correcta colocación de precios y exhibición productos comercializados.",
    "Evitar la reducción de espacios y verificar el arreglo y orden de los productos en los espacios respectivos.",
    "Realizar un reporte con el consolidado de las actividades de la competencia.",
    "Realizar el levantamiento de precios de productos y envío de informes y reportes."
   ],
   "requisitos": [
    "Contar con experiencia en ventas en tiendas Retail, de preferencia en parlantes, equipos de sonido, televisores, electrodomésticos y/o productos de tecnología.",
    "Contar con Carnet de Sanidad Vigente (sin manipulación).",
    "Tener disponibilidad para laborar en el punto de venta requerido: SAGA JOCKEY PLAZA.",
    "Ser una persona apasionada por las ventas.· Si estas interesado ¿Qué esperas? Postula y sé parte de esta GRAN FAMILIA!!! Estamos convencidos que tener un equipo de trabajo diverso y una cultura inclusiva nos ayuda a influir de manera positiva en nuestro entorno."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley desde el primero día. Régimen General.",
    "Salario: Sueldo Fijo S/1,130 + Movilidad S/150 + Comisiones de hasta S/775 (por cumplimiento de KPIs).",
    "Horario Laboral: Lunes a Viernes de 2PM - 9 PM, Sábado a Domingo de 11 AM - 9 PM. 1 día de descanso rotativo.",
    "Un trabajo dinámico en el que pondrás a prueba tu talento y oportunidad de línea de carrera.",
    "Oportunidad de ser categorizado y ganar mucho más (línea de carrera).",
    "Pagos puntuales a toda costa."
   ],
   "url": "https://www.laborum.pe/job/touch-lima-s-a-c/promotor-de-ventas-sony-saga-jockey-plaza/6aa22dbc0b0fa0541eb3b420"
  },
  {
   "id": 180,
   "puesto": "Operario de Producción Lurin S",
   "empresa": "Adecco Perú S.A.",
   "cat": "Ingeniería",
   "min": 1500,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 91,
   "resumen": "RESUMEN: Importante empresa del sector industrial se encuentra en la búsqueda de Operarios de Producción para su planta ubicada en Lurín.",
   "funciones": [
    "Apoyar en línea de producción.",
    "Encajado, embolsado, envasado y paletizado de productos.",
    "Alimentación de máquinas.",
    "Acomodar y verificacar productos.",
    "Control de calidad.",
    "Orden y limpieza del área de trabajo."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 3 meses en plantas industriales, producción, almacén u operaciones (esto incluye agro, construcción, pesca, distribución, etc).",
    "Disponibilidad para trabajar en turnos rotativos de día y noche.",
    "Disponibilidad para realizar horas extras.",
    "Disponibilidad para laborar en Lurín.",
    "Turnos rotativos de día y noche, se rota cada 15 días.",
    "Trabajo de lunes a sabado"
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,500 - 1800",
    "Bono nocturno del 35%.",
    "Planilla completa desde el primer día.",
    "Alimentación cubierta.",
    "Movilidad de acercamiento (CHILCA, LURIN, JAVIER PRADO, VMT, VES, SJL, CALLAO, VENTANILLA, CHORRILLOS, MANCHAY).",
    "Pago quincenal (15 y 25 de cada mes).",
    "Utilidades y todos los beneficios de ley al 100%.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 38 añosPalabras clave: operator, operario, operator, production, produce, fabricacion, manufactoring, elaboracion, making, manufactura, produccion"
   ],
   "url": "https://www.laborum.pe/job/adecco-peru-s-a/operario-de-produccion-lurin-s-1800-alimentacion-y-movilidad/6aa24f7e0b0fa0541eb400b7"
  },
  {
   "id": 181,
   "puesto": "Operario(a) de Producción",
   "empresa": "CYL",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 16,
   "vence": "2026-11-08",
   "restan": 43,
   "score": 88,
   "resumen": "Buscamos personas responsables, comprometidas y con disposición para aprender y desarrollarse dentro de una empresa estable del sector tecnológico",
   "funciones": [
    "Realizar la limpieza y acondicionamiento de equipos electrónicos y de telecomunicaciones",
    "Apoyar en el armado y desarmado de equipos",
    "Realizar lijado, pulido, etiquetado e impresión",
    "Realizar el empaque y acondicionamiento final de los equipos",
    "Mantener el orden y limpieza del área de trabajo",
    "Cumplir con los procedimientos de calidad y seguridad"
   ],
   "requisitos": [
    "Disponibilidad para realizar horas extras",
    "Disponibilidad para trabajar de lunes a sábado",
    "Responsabilidad, compromiso y disposición para aprender",
    "No es indispensable contar con experiencia previa, se brindará capacitación",
    "Residir en Santa Anita, Ate o zonas aledañas (no indispensable)",
    "Lunes a sábado",
    "Lunes a jueves: horario regular + horas extras hasta las 7:45 p. m.",
    "Viernes: hasta las 5:45 p. m.",
    "Sábados: turno por coordinar en entrevista",
    "LUGAR DE TRABAJO",
    "ATE – SANTA ANITA",
    "Referencia: Óvalo Santa Anita"
   ],
   "beneficios": [
    "Ingreso a planilla completa desde el primer día",
    "Sueldo base de S/ 1,130",
    "Pago de horas extras",
    "Bono de productividad de S/ 100 a S/ 200 según desempeño",
    "Pagos quincenales y fin de mes",
    "Todos los beneficios de ley: CTS, gratificaciones, Essalud, vacaciones y asignación familiar",
    "Telemedicina 24/7",
    "Oportunidad de desarrollo y línea de carrera",
    "Posibilidad de crecimiento dentro del área",
    "Estabilidad laboral y buen ambiente de trabajo"
   ],
   "url": "https://www.bumeran.com.pe/empleos/trabaja-en-ate-operarioa-de-produccion-|-bonos-cyl-1118436386.html"
  },
  {
   "id": 182,
   "puesto": "Asesor",
   "empresa": "Distribuidora de Relojes y Accesorios S.A.C.",
   "cat": "Ventas",
   "min": 1025,
   "max": 1025,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 87,
   "resumen": "Asesor a los clientes sobre los productos",
   "funciones": [
    "Asesor a los clientes sobre los productos",
    "Generar reporte de ventas",
    "Mantener el orden y limpieza de los productos",
    "¡No dejes pasar esta oportunidad, postula ya!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: cambaceo, sales"
   ],
   "requisitos": [
    "Experiencia de 1 año en ventas (INDISPENSABLE)",
    "Mayor de edad y documentos en regla",
    "Disponibilidad para trabajar en horarios rotativos",
    "Disponibilidad para laborar en el Real Plaza Primavera"
   ],
   "beneficios": [
    "Sueldo fijo s/1025 + Comisiones ilimitadas",
    "Planilla completa con todos los beneficios de ley"
   ],
   "url": "https://www.laborum.pe/job/distribuidora-de-relojes-y-accesorios-s-a-c/asesora-de-ventas-real-plaza-primavera-tiempo-completo/6aa216360b0fa0541eb36d46"
  },
  {
   "id": 183,
   "puesto": "Reponedor puesto fijo Crepier San Borja",
   "empresa": "LULLI & ASOCIADOS S.A.C. - ENLACE Headhunting",
   "cat": "Recursos Humanos",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 16,
   "vence": "2026-12-09",
   "restan": 74,
   "score": 86,
   "resumen": "Marca peruana líder en moda y accesorios de viaje, con más de 40 años de estilo e innovación.",
   "funciones": [
    "Marca peruana líder en moda y accesorios de viaje, con más de 40 años de estilo e innovación.",
    "Por encargo de nuestro cliente CREPIER , estamos en la búsqueda de personas apasionadas y talentosas para sumar a su equipo como",
    "REPONEDOR - TIENDA SAN BORJA",
    "Verificar el stock de la mercadería.",
    "Reposición de la mercadería.",
    "Exhibir los productos según lineamientos de la marca.",
    "Otras funciones que le asigne su jefe inmediato."
   ],
   "requisitos": [
    "Contar con estudios de Secundaria completa o técnicas.",
    "Disponibilidad para trabajar por campaña día de la madre.",
    "Tener experiencia mínima de 6 meses a más como Reponedor (a).",
    "INDISPENSABLE: Contar con disponibilidad inmediata para trabajar."
   ],
   "beneficios": [
    "Horarios rotativos Lunes a Domingo (1 día de descanso dentro de semana).",
    "10:00 am a 7:00 pm",
    "11:00am a 8:00pm",
    "12:00m a 9:00pm",
    "Salario S/1200 en planilla con todos los beneficios de ley.",
    "Tipo de puesto: Tiempo completo",
    "Lugar de trabajo: Empleo presencial EN TIENDA"
   ],
   "url": "https://www.bumeran.com.pe/empleos/reponedor-puesto-fijo-crepier-san-borja-lulli-asociados-s.a.c.-enlace-talento-trabajo-1118437845.html"
  },
  {
   "id": 184,
   "puesto": "Cajero",
   "empresa": "Fibramas",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 86,
   "resumen": "RESUMEN: En Fibramas, empresa del rubro de telecomunicaciones, buscamos un(a) cajero(a) responsable, organizado(a) y con iniciativa comercial para atender a nuestros clientes, registrar pagos y gestionar el cierre diario de caja.",
   "funciones": [
    "Realizar el cuadre y cierre diario de caja.",
    "Atender consultas sobre pagos, servicios y reclamos.",
    "Contactar a clientes con pagos pendientes para gestionar la cobranza.",
    "Orientar a clientes con el servicio suspendido para lograr su reactivación.",
    "Registrar las atenciones y reportar cualquier incidencia."
   ],
   "requisitos": [
    "Experiencia mínima de 6 meses en manejo de caja y POS.",
    "Conocimiento de Excel a nivel básico.",
    "Disponibilidad para laborar de lunes a sábado.",
    "Horario de trabajo",
    "Lunes a viernes: de 8:30 a. m. a 6:00 p. m.",
    "Sábados: de 8:30 a. m. a 2:00 p. m."
   ],
   "beneficios": [
    "Sueldo básico mensual de S/ 1,130.",
    "Ingreso a planilla completa desde el primer día.",
    "Comisiones variables por cumplimiento de metas de cobranza, recuperación de clientes y ventas.-Requerimientos- Educación mínima: Educación Secundaria años de experiencia"
   ],
   "url": "https://www.laborum.pe/job/fibramas/cajero-a-planilla-completa-vmt/6aa222170b0fa0541eb38fb1"
  },
  {
   "id": 185,
   "puesto": "Operario de almacén",
   "empresa": "INSIDE EMPRESARIAL",
   "cat": "Logística",
   "min": 1275,
   "max": 1275,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 16,
   "vence": "2026-10-10",
   "restan": 14,
   "score": 86,
   "resumen": "RESUMEN: ¡BUSCAMOS OPERARIOS DE ALMACÉN!",
   "funciones": [
    "Verificar que los productos se encuentren en buen estado",
    "Preparar y empaquetar los productos",
    "Acomodar los productos según las indicaciones de la ubicación del destino.",
    "Ubicar y realizar la carga de los productos"
   ],
   "requisitos": [
    "Secundaria completa.",
    "Mínimo 06 meses de experiencia.",
    "Experiencia realizando selección y empaquetado de productos.",
    "De preferencia vivir en Lurín o zonas aledañas."
   ],
   "beneficios": [
    "Salario de S/1275",
    "Vale de S/150",
    "Planilla completa desde el primer día de labores (Todos los beneficios de ley: CTS, gratificación)",
    "EPS cubierto al 100% para el empleado(a) sus hijos y conyugue (Seguro privado de salud: Clínicas)",
    "Movilidad de acercamiento (Ingreso y salida)",
    "Almuerzo cubierto",
    "Si cumples con los requisitos, envíanos tu CV actualizado, ¡postula ahora y crece con nosotros. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: operator, operario, operator, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/inside-empresarial/operario-de-almacen-turno-dia-movilidad-de-acercamiento/6aa1f39a0b0fa0541eb303a6"
  },
  {
   "id": 186,
   "puesto": "Auxiliar de Producción",
   "empresa": "CYL",
   "cat": "Ingeniería",
   "min": 1150,
   "max": 1150,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 17,
   "vence": "2026-11-08",
   "restan": 43,
   "score": 91,
   "resumen": "¡Únete a una empresa estable del sector automotriz y desarrolla tu experiencia en el área de despacho!",
   "funciones": [
    "Recibir y revisar la mercadería que llega del área de armado.",
    "Revisar que las cantidades, códigos, etiquetas, sellado y peso de los productos sean correctos.",
    "Preparar la mercadería colocando los productos sobre pallets, asegurándolos con zuncho y film stretch.",
    "Preparar y asegurar la carga para que quede lista para su despacho.",
    "Cumplir con las normas de seguridad, mantener el orden y seguir los procedimientos de calidad."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia en almacén, despacho o actividades similares (deseable) .",
    "Disponibilidad para trabajar en turnos rotativos.",
    "Disponibilidad para realizar horas extras.",
    "Responsabilidad, compromiso y disposición para trabajar en equipo.",
    "Lunes a viernes: 7:00 a. m. a 4:15 p. m.",
    "Sábados: 7:00 a. m. a 12:30 p. m.",
    "Lunes a viernes: 9:00 a. m. a 7:15 p. m.",
    "Los turnos rotan mensualmente.",
    "Domingo: día libre.",
    "Disponibilidad para ambos turnos.",
    "ATE – SANTA ANITA Referencia: Óvalo Santa Anita"
   ],
   "beneficios": [
    "Ingreso directo a planilla desde el primer día.",
    "Sueldo: S/ 1,150 .",
    "Incremento salarial al cumplir un año.",
    "Bono de transporte.",
    "Bono de asistencia.",
    "Alimentación con 70% de cobertura.",
    "Todos los beneficios de ley.",
    "Essalud, asignación familiar y utilidades.",
    "Estabilidad laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/oportunidad-en-ate-auxiliar-de-produccion-|-sin-experiencia-cyl-1118436385.html"
  },
  {
   "id": 187,
   "puesto": "Asesor",
   "empresa": "BUMERAN SELECTA",
   "cat": "Ventas",
   "min": 4000,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Piura",
   "fuente": "Bumeran",
   "dias": 18,
   "vence": "2027-01-06",
   "restan": 102,
   "score": 97,
   "resumen": "Gestionar y desarrollar comercialmente la cartera de clientes industriales asignada.",
   "funciones": [
    "Gestionar y desarrollar comercialmente la cartera de clientes industriales asignada.",
    "Realizar visitas técnicas y comerciales en campo, identificando necesidades y oportunidades de venta.",
    "Prospectar y captar nuevos clientes dentro de la Región Norte.",
    "Brindar soporte y asesoría técnica en soluciones de lubricación, equipos y sistemas de lubricación .",
    "Analizar los procesos y equipos de los clientes para identificar oportunidades de aplicación de las distintas soluciones de la compañía.",
    "Presentar el portafolio de productos, resaltando su valor agregado, rendimiento y potencial de ahorro de costos.",
    "Elaborar, presentar y realizar seguimiento a propuestas técnico-comerciales.",
    "Participar activamente en la negociación y cierre de ventas .",
    "Realizar seguimiento comercial y postventa a la cartera existente, fortaleciendo relaciones de largo plazo.",
    "Dictar capacitaciones, presentaciones y charlas técnicas a clientes.",
    "Identificar oportunidades de venta cruzada entre las distintas líneas de productos de la empresa.",
    "Comercializar soluciones vinculadas a lubricación, transmisión mecánica, sistemas de aire comprimido y otras soluciones industriales."
   ],
   "requisitos": [
    "Egresado/a de Ingeniería Mecánica, Ingeniería Mecatrónica, Ingeniería Química, Ingeniería Industrial o carreras afines.",
    "Experiencia mínima de 2 años en ventas B2B a clientes del sector industrial .",
    "Experiencia en posiciones comerciales con una base técnica sólida , que permita comprender procesos, maquinaria y necesidades del cliente.",
    "Conocimiento y/o experiencia en lubricación industrial, equipos y sistemas de lubricación .",
    "Experiencia en negociación, seguimiento y cierre de ventas.",
    "Capacidad para realizar presentaciones y capacitaciones técnicas a clientes.",
    "Inglés nivel intermedio.",
    "Licencia de conducir vigente.",
    "Disponibilidad para realizar trabajo principalmente en campo y viajar dentro de la Región Norte .",
    "De preferencia, residencia en Piura , considerando cobertura comercial hacia Lambayeque, Talara, Tumbes y otras zonas del norte.",
    "Orientación comercial, proactividad, comunicación efectiva, capacidad de negociación y orientación a resultados."
   ],
   "beneficios": [
    "Sueldo básico bruto: S/4,000 mensuales.",
    "Comisiones variables sobre las ventas facturadas.",
    "Al inicio, el cliente estima un ingreso aproximado de S/2,500 a S/3,000 mensuales en comisiones , sujeto al nivel de ventas y cumplimiento comercial.",
    "Posibilidad de incrementar progresivamente las comisiones conforme se desarrolle una mayor cartera y volumen de ventas.",
    "La posición inicia con una base y cartera de clientes existente , por lo que no se parte desde cero.",
    "Movilidad cubierta por la empresa para los desplazamientos comerciales correspondientes.",
    "Viáticos cubiertos para viajes a otras provincias o departamentos , incluyendo traslado, alojamiento y alimentación, sujetos a rendición semanal.",
    "Capacitación y acompañamiento comercial para el conocimiento de las distintas líneas y soluciones de la empresa.",
    "Oportunidad de desarrollar una cartera propia y generar mayores ingresos en función de resultados."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-a-tecnico-comercial--region-norte-copia-bumeran-selecta-1118434406.html"
  },
  {
   "id": 188,
   "puesto": "Promotor Senior de Celulares",
   "empresa": "Arch Latam",
   "cat": "Ventas",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 97,
   "resumen": "RESUMEN: ARCH LATAM se encuentra en búsqueda de PROMOTORES(AS) SENIOR DE CELULARES para representar una importante marca de tecnología en el canal RETAIL, con vacantes disponibles en diferentes zonas de Lima Metropolitana.",
   "funciones": [
    "Asesorar a los clientes en la elección de celulares y equipos móviles.",
    "Realizar ventas y cierre comercial.",
    "Explicar características, beneficios y funcionalidades de los equipos.",
    "Identificar las necesidades del cliente y recomendar el producto adecuado.",
    "Cumplir y superar los objetivos de venta establecidos.",
    "Mantener una adecuada exhibición y presentación de los productos.",
    "Realizar seguimiento de indicadores y resultados comerciales."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia comprobada en venta de celulares, smartphones y/o telefonía.",
    "Experiencia en retail o canal moderno.",
    "Experiencia trabajando con metas comerciales y comisiones por ventas.",
    "Conocimiento de características, funcionalidades y principales marcas de celulares.",
    "Habilidad para asesorar clientes, manejar objeciones y concretar ventas.",
    "Disponibilidad para trabajar en diferentes zonas de Lima Metropolitana. (Se le asignara un atienda fija)",
    "Carné de Sanidad vigente de Lima Metropolitana o disponibilidad para tramitarlo.",
    "Disponibilidad inmediata."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,400",
    "Comisiones de hasta S/ 1,000",
    "Bono de movilidad: S/ 100",
    "Planilla desde el primer día.",
    "Todos los beneficios de ley.",
    "Capacitación constante.",
    "Oportunidad de crecimiento.",
    "Lunes a viernes: 2:00 p. m. a 9:00 p. m.",
    "Sábados y domingos: 11:00 a. m. a 9:00 p. m.",
    "1 día de descanso semanal, según programación.",
    "PUNTO DE VENTA",
    "Cada promotor será asignado a una tienda específica según disponibilidad y zona de residencia."
   ],
   "url": "https://www.laborum.pe/job/arch-latam/promotor-senior-de-celulares-retail-exp-gana-hasta-s-2-500/6a9fa8ad442c35a1bc076529"
  },
  {
   "id": 189,
   "puesto": "Atencion al Cliente Grifo Primax Av. Aviacion con Villaran",
   "empresa": "COESTI S.A.",
   "cat": "Atención al Cliente",
   "min": 1230,
   "max": 1230,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 97,
   "resumen": "Si quieres pertenecer al Grupo Primax y formar parte de un equipo lleno de energía y pasión te invitamos a postular a la posición de DESPACHO DE COMBUSTIBLE FULL TIME para que cumplas las siguientes",
   "funciones": [
    "Atención al clientes de manera oportuna en despachando combustible",
    "Controlar los Boucher, vales y efectivo de las ventas de los dispensadores que le son asignados",
    "Realizar el reporte de ventas al término del turno - cuadre de caja",
    "Cumplir con las normas de seguridad y salud en el trabajo",
    "Mantener la EESS limpia y ordenada"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Deseable experiencia en atención al cliente",
    "Deseable manejo de caja",
    "Disponibilidad para laborar en el turno tarde de 3 pm a 11pm",
    "Disponibilidad para laborar en Aviacion con Villaran"
   ],
   "beneficios": [
    "Ingreso a planilla",
    "Sueldo aproximado de S/1230 para full time",
    "Contrato indeterminado",
    "Canasta navidiña + Vale de Pavo en navidad",
    "Asignación familiar",
    "Bonos por ventas",
    "Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/atencion-al-cliente-grifo-primax-av-aviacion-con-villaran/6a9f5109442c35a1bc0689f2"
  },
  {
   "id": 190,
   "puesto": "Analista Senior Contable",
   "empresa": "BUMERAN SELECTA",
   "cat": "Contabilidad",
   "min": 4000,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 18,
   "vence": "2026-12-07",
   "restan": 72,
   "score": 95,
   "resumen": "Participar en el cierre contable mensual de la empresa.",
   "funciones": [
    "Participar en el cierre contable mensual de la empresa.",
    "Realizar el análisis y revisión de cuentas contables .",
    "Apoyar en la preparación del Balance General, Estado de Resultados y Flujo de Efectivo .",
    "Revisar y controlar retenciones tributarias y otros conceptos fiscales del día a día.",
    "Registrar y analizar facturas y operaciones contables vinculadas a impuestos.",
    "Analizar los costos de importación y apoyar en el cálculo del costo de ventas.",
    "Verificar el cumplimiento de las normas contables y fiscales aplicables.",
    "Elaborar reportes contables y financieros para apoyar la toma de decisiones de la gerencia.",
    "Preparar información y documentación de soporte para auditorías internas y externas .",
    "Coordinar con la contadora externa y la gerencia respecto al avance y revisión de la información contable.",
    "Mantener la documentación contable ordenada, actualizada y debidamente sustentada.",
    "Cumplir con los cronogramas y fechas de cierre establecidos por la empresa."
   ],
   "requisitos": [
    "Profesional titulado/a en Contabilidad .",
    "Colegiatura vigente como requisito.",
    "Experiencia mínima de 3 años como Analista Contable o en posiciones de similar responsabilidad .",
    "Experiencia participando en cierres contables mensuales y análisis de cuentas.",
    "Experiencia en costos de importación y cálculo de costo de ventas .",
    "Conocimiento de registro de facturas, impuestos, retenciones, detracciones y percepciones .",
    "Experiencia en preparación y análisis de estados financieros .",
    "Conocimiento de normativa contable y tributaria vigente.",
    "Experiencia preparando información para auditorías internas y/o externas.",
    "Alto nivel de responsabilidad, organización, análisis, atención al detalle y ética profesional.",
    "Disponibilidad para trabajar de manera presencial en Los Olivos ."
   ],
   "beneficios": [
    "Sueldo bruto mensual: S/4,000.",
    "Régimen laboral general.",
    "Contrato firmado antes del inicio de labores.",
    "Posibilidad de afiliación a EPS , sujeto a las condiciones y porcentaje de cobertura de la empresa.",
    "Proyección de crecimiento dentro del área contable, con posibilidad de asumir a futuro la posición de contador/a interno/a .",
    "Posible bono asociado a desempeño/compromiso, sujeto a evaluación y confirmación de condiciones.",
    "Horario de trabajo",
    "Lunes a viernes de 8:30 a. m. a 5:00 p. m.",
    "Dos sábados al mes de 8:00 a. m. a 12:00 p. m.",
    "Lugar de trabajo: Los Olivos, Lima ."
   ],
   "url": "https://www.bumeran.com.pe/empleos/analista-senior-contable-los-olivos-bumeran-selecta-1118434409.html"
  },
  {
   "id": 191,
   "puesto": "Especialista en Unidad de Cuidados Intensivos",
   "empresa": "GRUPO PROGESTION",
   "cat": "Ingeniería",
   "min": 4000,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 95,
   "resumen": "Ejecutar capacitaciones especializadas sobre el uso y manejo de ventiladores mecánicos.",
   "funciones": [
    "Ejecutar capacitaciones especializadas sobre el uso y manejo de ventiladores mecánicos.",
    "Brindar acompañamiento clínico directo durante las instalaciones y puestas en marcha de los equipos.",
    "Realizar el seguimiento técnico-asistencial para asegurar el correcto uso de los dispositivos.",
    "Brindar soporte técnico al equipo comercial durante presentaciones y demostraciones.",
    "Elaborar informes detallados sobre las actividades de soporte y capacitación realizadas."
   ],
   "requisitos": [
    "Licenciatura en Enfermería o Tecnología Médica.",
    "Experiencia mínima de 2 años en Unidades de Cuidados Intensivos (UCI), Emergencias o en el área de Soporte Clínico de equipos médicos.",
    "Conocimientos sólidos y comprobados en Ventilación Mecánica y Cuidados Críticos.",
    "Habilidades de comunicación verbal, empatía y trabajo en equipo.",
    "Disponibilidad para laborar de forma presencial en Miraflores.",
    "Disponibilidad para realizar viajes y atender casos de emergencia excepcionales en fines de semana."
   ],
   "beneficios": [
    "Rango salarial atractivo: S/4000 mensuales.",
    "Ingreso a planilla con todos los beneficios de ley bajo contrato a plazo fijo.",
    "EPS para el colaborador cubierta al 100%.",
    "Disponibilidad para trabajar de lunes a viernes de 9:00 AM a 6:00 PM.",
    "Oportunidad de pertenecer a una empresa líder en el sector de equipamiento médico.-Requerimientos- Educación mínima: Técnico2 años de experienciaEdad: entre 29 y 50 añosPalabras clave: specialist, especialista"
   ],
   "url": "https://www.laborum.pe/job/grupo-progestion/especialista-en-unidad-de-cuidados-intensivos-disponibilidad/6a9f5d3a442c35a1bc06dfa6"
  },
  {
   "id": 192,
   "puesto": "Cajera(s) Multifuncional",
   "empresa": "MONTALVO",
   "cat": "Ventas",
   "min": 1400,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 95,
   "resumen": "RESUMEN: ¡ÚNETE A LA FAMILIA MONTALVO Y VIVE LA EXPERIENCIA MONTALVO!",
   "funciones": [
    "Realizar ventas y promover los servicios y productos de la marca.",
    "Brindar una atención amable, personalizada y de calidad a los clientes.",
    "Organizar y gestionar turnos y citas de los clientes.",
    "Promocionar paquetes, convenios, canjes y precios vigentes según las necesidades de cada cliente.",
    "Actualizar la base de datos de clientes de acuerdo con los lineamientos del área de Marketing.",
    "Realizar seguimiento a sugerencias, quejas y reclamos, informando oportunamente a la Administradora para su atención y solución.",
    "Ejecutar la apertura y/o cierre de caja, de acuerdo con los procedimientos establecidos por el área Financiera.",
    "Apoyar en las diferentes actividades operativas de la sede."
   ],
   "requisitos": [
    "Estudios técnicos en Cajero Comercial o carreras afines (deseable, no indispensable).",
    "Experiencia mínima de 1 año en caja, atención al cliente o posiciones similares.",
    "Manejo de Excel a nivel intermedio (indispensable).",
    "Orientación al cliente y habilidades para las ventas.",
    "Buena comunicación, organización y responsabilidad.",
    "Disponibilidad para trabajar de manera presencial en nuestras sedes de Lima.",
    "Disponibilidad para laborar de lunes a domingo, con 1 día de descanso (lunes).",
    "Disponibilidad para cumplir jornadas según el horario: De 12.pm a 9.00 de lunes a viernes sábados de 9 a 9 domingos de 9 a 7.00pm"
   ],
   "beneficios": [
    "Sueldo entre S/ 1,400 y S/ 1,500.",
    "Descuentos corporativos en nuestros servicios.",
    "Oportunidades de desarrollo profesional.",
    "Línea de carrera.",
    "Formar parte de una empresa sólida y reconocida en el sector de belleza y bienestar.",
    "¡Si eres una persona orientada al servicio, te apasionan las ventas y buscas crecer profesionalmente, esta oportunidad es para ti!",
    "¡Postula y sé parte de la familia Montalvo!. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 22 y 40 años"
   ],
   "url": "https://www.laborum.pe/job/montalvo/cajera-s-multifuncional-es-bellavista-miraflores-part-time/6a9f5103442c35a1bc0689ac"
  },
  {
   "id": 193,
   "puesto": "Ejecutivo de Gobierno Corporativo",
   "empresa": "ManpowerGroup Perú",
   "cat": "Recursos Humanos",
   "min": 5000,
   "max": 5000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 18,
   "vence": "2026-12-07",
   "restan": 72,
   "score": 94,
   "resumen": "En Manpower Perú buscamos al mejor talento para asumir el reto como Ejecutivo(a) de Gobierno Corporativo para la ciudad de Lima ¡La oportunidad de tener un futuro mejor, está aquí!",
   "funciones": [
    "Asegurar y gestionar la implementación de la política y lineamientos de gobernanza corporativa en COFIDE y asesorar y absolver consultas en temas relacionado a gobierno corporativo,",
    "Planificar y gestionar la elaboración, actualización y difusión de normas, metodologías y procedimientos de gobernanza, incluyendo gestión de directores y continuidad operativa.",
    "Garantizar y gestionar la elaboración de los reportes de gobernanza, informando sobre el nivel de madurez y cumplimiento de los procesos institucionales. Incluyendo el aseguramiento de la gestión de Buen Gobierno Corporativo aplicable a COFIDE.",
    "Gestionar la capacitación a trabajadores y directores en mejores prácticas de gobernanza y tendencias internacionales.",
    "Supervisar la gestión documental y normativa del Directorio, incluyendo revisión de contenidos y emisión de visados.",
    "Asegurar el seguimiento de acuerdos y actividades del Directorio y Comités, reportando desviaciones o incumplimientos.",
    "Gestionar eficientemente las comunicaciones internas y externas del Directorio, garantizando oportunidad y trazabilidad.",
    "Representar al área en comités y grupos de trabajo relacionados con gobernanza.",
    "Brindar soporte al Gerente de la Gerencia de Asesoría Jurídica en las labores de secretaría del Directorio y del Comité de Gerencia.",
    "Brindar apoyo en la revisión y/o elaboración de actas, transcripciones, mociones, entre otros documentos de índole societario, así como coadyuvar en la atención a accionistas, directores y demás Stakeholders de COFIDE.",
    "Cumplir con otras funciones asignadas por la gerencia"
   ],
   "requisitos": [
    "Derecho, Economía, Administración, Contabilidad, Ingeniería Industrial o carreras afines.",
    "Experiencia laboral general de 5 años desde egresado.",
    "Experiencia específica: 3 años en puestos de ejecutivo, jefe, supervisor, analista o en puestos de similares características y responsabilidades.",
    "Conocimientos técnicos principales requeridos para el puesto (No requieren documentación de sustento): Principios de gobierno corporativo, gestión de directorios y comités, normativa societaria y de cumplimiento, coordinación institucional, otros afines.",
    "Curso o diplomado o especialización en Gobierno Corporativo. (Indispensable)",
    "Curso: Dirección de Empresas o Gestión Pública, Compliance y Gestión de Riesgos. (Deseable)",
    "Office - Intermedio (Deseable)",
    "Inglés - Intermedio (Deseable)"
   ],
   "beneficios": [
    "Ingreso a la planilla directa del cliente.",
    "EPS Mapfre, para ti y tu familia, cubierto hasta el 90% por parte de la empresa.",
    "Seguro contra accidentes personales cubierto al 100% por parte de la empresa.",
    "Seguro de Vida Ley desde el 1er. día de trabajo.",
    "Seguro Oncológico, para ti y tu familia, cubierto al 100% por parte de la empresa.",
    "Asignación por Uniforme, el cual se otorga dos (02) veces al año, siendo un importe anual aproximado de S/. 5,000, supeditado a la política interna y lineamientos corporativos presupuestales.",
    "Otorgamiento del vale por alimentos por el importe de S/. 500.00 soles mensuales.",
    "Bono por cumplimiento de objetivos al año de hasta un tope de dos (02) sueldos, ello supeditado a la política interna y lineamientos de FONAFE.",
    "Horario de Trabajo de lunes a viernes de 8:45 a.m. a 5:30 p.m (PRESENCIAL)",
    "(ÚNICO MEDIO DE POSTULACIÓN)",
    "Las postulaciones serán hasta el 15 de Setiembre (Hasta las 23:59 horas del último día de recepción).",
    "En Manpower Perú, promovemos la inclusión e inserción laboral sin distinción por razón de discapacidad, género, orientación, identidad y expresión de género, lugar de origen o generación. Por ello ¡Todos los talentos son bienvenidos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-de-gobierno-corporativo-sector-financiero-san-isidro-ref109412-manpowergroup-peru-1118434386.html"
  },
  {
   "id": 194,
   "puesto": "Auxiliar de almacen Aduanero",
   "empresa": "Eurofirms Perú",
   "cat": "Logística",
   "min": 1800,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 94,
   "resumen": "Revisar y gestionar la documentación aduanera para los procesos de importación y exportación, coordinando aforos, inspecciones y reconocimientos físicos ante Aduanas, SENASA y otras entidades.",
   "funciones": [
    "Revisar y gestionar la documentación aduanera para los procesos de importación y exportación, coordinando aforos, inspecciones y reconocimientos físicos ante Aduanas, SENASA y otras entidades.",
    "Supervisar el retiro, traslado, carga y descarga de mercancías, verificando el cumplimiento de los procedimientos operativos y reportando incidencias durante el proceso.",
    "Coordinar con clientes, proveedores de transporte, depósitos temporales y autoridades competentes para asegurar la correcta ejecución de las operaciones.",
    "Registrar y actualizar el estado de las operaciones en el sistema, garantizando el seguimiento oportuno de la carga y la trazabilidad de los despachos.",
    "Ejecutar inspecciones de contenedores, consolidación de carga de exportación, pesajes y demás actividades relacionadas con el despacho aduanero."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Contar con Curso de Auxiliar de Despacho Aduanero acreditado por IAT (indispensable).",
    "Experiencia mínima de 1 año desempeñándose como Auxiliar de Despacho Aduanero.",
    "Disponibilidad para laborar en Puerto.",
    "Disponibilidad para realizar trámites y supervisiones operativas en almacenes, terminales portuarios, depósitos temporales y entidades aduaneras.",
    "Contar con competencias: Organización y planificación, Comunicación efectiva, Proactividad y capacidad resolutiva."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley desde el primer día.",
    "Remuneración mensual S/ 1,800 (negociable)",
    "Horario de trabajo presencial",
    "Lunes a viernes de 8:00 a.m. a 6:00 p.m",
    "Sábados de 8:00 a.m. a 11:00 a.m.",
    "Cobertura de EPS al 25%.",
    "Contrato a plazo fijo renovable cada 6 meses, con proyección de permanencia y desarrollo dentro de la organización.",
    "Medio día libre por cumpleaños.",
    "14 horas libres al año para atenciones médicas u otros trámites personales."
   ],
   "url": "https://www.laborum.pe/job/eurofirms-peru/auxiliar-de-almacen-aduanero-despacho-todos-los-beneficios/6a9fe202442c35a1bc08862a"
  },
  {
   "id": 195,
   "puesto": "Reponedor",
   "empresa": "Overall Strategy",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 94,
   "resumen": "RESUMEN: Nos encontramos en la búsqueda de Mercaderistas para laborar en las principales cadenas de supermercados del Callao.",
   "funciones": [
    "Reposición y ordenar los productos de la marca.",
    "Implementación de exhibiciones y cabeceras.",
    "Cambio de precios y material POP publicitario.",
    "Levantamiento de quiebres de stock.",
    "Negociación de espacios adicionales.",
    "Reporte de incidencias y competencia."
   ],
   "requisitos": [
    "Experiencia mínima de 3 meses a más como Mercaderista, Reponedor(a), Auxiliar de Reposición o posiciones similares en supermercados, autoservicios o tiendas de conveniencia (Mass, Oxxo, Listo, Tambo, entre otros). Consideramos también canal tradicional, mercados, bodegas, etc.",
    "Experiencia realizando reposición, rotación de productos y control de fechas de vencimiento.",
    "Disponibilidad para laborar en horario Full Time.",
    "Disponibilidad para trasladarse dentro de la zona asignada.",
    "Contar con carnet de sanidad del vigente o disponibilidad para tramitarlo.",
    "Buena actitud, proactividad y orientación al cumplimiento de objetivos."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,130.",
    "Bono de movilidad: S/ 200.",
    "Comisiones: S/ 200.",
    "Ingreso a planilla completa desde el primer día de trabajo.",
    "Todos los beneficios de ley (CTS, gratificaciones, vacaciones, asignación familiar, ESSALUD, entre otros).",
    "Descuentos corporativos.",
    "Oportunidad de crecimiento y línea de carrera.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 19 añosPalabras clave: comerciante, merchandiser, merchant, aseo, higiene, cleaning, limpieza"
   ],
   "url": "https://www.laborum.pe/job/overall-strategy/mercaderista-metro-bellavista-tottus-san-miguel-gana-s-1530/6a9fd513442c35a1bc083e76"
  },
  {
   "id": 196,
   "puesto": "Atención al cliente",
   "empresa": "COESTI S.A.",
   "cat": "Recursos Humanos",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 94,
   "resumen": "RESUMEN: ¡Trabaja en Primax La Perla! Dirección: Av. La Paz 2370, La Perla",
   "funciones": [
    "Atención al cliente y despacho de combustible.",
    "Manejo de vouchers, vales y efectivo.",
    "Realizar cuadre de caja y reporte de ventas al finalizar el turno.",
    "Mantener el orden y limpieza de la estación.",
    "Cumplir con las normas de seguridad y salud en el trabajo."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia en atención al cliente (deseable).",
    "Manejo de caja (deseable).",
    "Disponibilidad para laborar en turno tarde de 3:00 p.m. a 11:00 p.m.",
    "Disponibilidad para trabajar en Primax La Perla"
   ],
   "beneficios": [
    "Sueldo aproximado de S/1200.",
    "Ingreso a planilla desde el primer día.",
    "Asignación familiar.",
    "Bonos por ventas.",
    "¡Únete a nuestro gran equipo!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: nominas, sueldo, paga, retribucion, salary, payslip, payroll, wage, pay, payment, nominista"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/trabaja-en-primax-la-perla-primax-atc-turno-tarde-sueldo/6a9fc7d5442c35a1bc07fa6e"
  },
  {
   "id": 197,
   "puesto": "Cajera",
   "empresa": "HVA INVERSIONES",
   "cat": "Administración",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 94,
   "resumen": "Manejo de caja (efectivo, POS y otros medios de pago)",
   "funciones": [
    "Manejo de caja (efectivo, POS y otros medios de pago)",
    "Control y administración de caja chica",
    "Cuadre diario de caja y verificación de ventas",
    "Control de bebidas, comisiones y propinas",
    "Pago y control de proveedores",
    "Depósitos bancarios de ventas",
    "Verificación de billetes y prevención de errores",
    "Mantener el área de trabajo ordenada y profesional",
    "Lunes a Domingo: de 8am a 6 pm S/1600.00. -Requerimientos- Educación mínima: Técnico2 años de experienciaEdad: entre 25 y 35 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, restaurador"
   ],
   "requisitos": [
    "Experiencia mínima de 2 años como cajera en restaurantes",
    "Manejo administrativo y control de dinero",
    "Conocimiento de Word y Excel (nivel intermedio o más)",
    "Actitud proactiva y responsabilidad",
    "Ganas de aprender y crecer con la empresa",
    "Vivir cerca al Centro de Lima (deseable)",
    "Experiencia con sistema Restaurant.pe (deseable, no excluyente)"
   ],
   "beneficios": [
    "Sueldo S/. 1600.00",
    "Ingreso a planilla y asignación familiar.",
    "Pagos quincenales.",
    "Alimentación cubierta.",
    "Pago doble por feriados laborados.",
    "Comisiones individuales y semanales.",
    "30% de descuento en consumos (en nuestros locales).",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Bonos por metas mensuales.",
    "Un día de descanso a la semana."
   ],
   "url": "https://www.laborum.pe/job/hva-inversiones/cajera-asistente-administrativa-para-restaurante-ceviche/6a9f547f442c35a1bc06a905"
  },
  {
   "id": 198,
   "puesto": "Cajera(o) Multifuncional",
   "empresa": "Aromas Cafe Delicatessen S.A.C",
   "cat": "Ingeniería",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 94,
   "resumen": "RESUMEN: Cajera(o) Multifuncional Atención al Cliente | Clínica Tezza",
   "funciones": [
    "Realizar cobros en efectivo, POS, transferencias y otros medios de pago.",
    "Emitir boletas y/o facturas.",
    "Realizar apertura, arqueo y cierre de caja.",
    "Registrar y verificar correctamente las ventas.",
    "Elaborar reportes diarios de caja y ventas.",
    "Atender y orientar cordialmente a los clientes.",
    "Resolver consultas sobre productos, promociones y medios de pago.",
    "Promover productos y apoyar en el incremento de ventas.",
    "Tomar pedidos de manera rápida y precisa.",
    "Servir alimentos y bebidas con buena presentación.",
    "Mantener limpia y ordenada el área de trabajo.",
    "Realizar lavado de menaje y apoyar en las actividades operativas."
   ],
   "requisitos": [
    "Secundaria completa, indispensable.",
    "Estudios técnicos concluidos o en curso en Administración, Contabilidad, Hotelería, Gastronomía o carreras afines.",
    "Mínimo 1 año de experiencia como cajera(o), atención al cliente o puestos similares, de preferencia en restaurantes, cafeterías o negocios gastronómicos.",
    "Experiencia en apertura, arqueo y cierre de caja.",
    "Manejo de POS y diferentes medios de pago.",
    "Conocimiento en emisión de comprobantes de pago.",
    "Deseable conocimiento en Buenas Prácticas de Manipulación de Alimentos (BPM).",
    "Responsabilidad, honestidad, excelente comunicación, proactividad y capacidad para trabajar en equipo."
   ],
   "beneficios": [
    "Sueldo: S/ 1,400 + S/ 200 de movilidad.",
    "Ingreso a planilla MYPE con beneficios sociales de ley.",
    "Alimentación cubierta al 100%.",
    "Horario: lunes a viernes de 1:30 p. m. a 9:30 p. m. y sábados de 9:00 a. m. a 6:00 p. m.",
    "Lugar de trabajo: Clínica Tezza Surco.",
    "Disponibilidad inmediata.",
    "¡Postula y forma parte de nuestro equipo!. -Requerimientos- Educación mínima: Técnico1 año de experienciaPalabras clave: polivalente, multiusos, versatil, multifuncion, plurifuncional, multifuntion, polifacetico"
   ],
   "url": "https://www.laborum.pe/job/aromas-cafe-delicatessen-s-a-c/cajera-o-multifuncional-atc-surco/6a9f5452442c35a1bc06a7ba"
  },
  {
   "id": 199,
   "puesto": "Ejecutivo Comercial B2B",
   "empresa": "Smart Sales Partners",
   "cat": "Ventas",
   "min": 1700,
   "max": 1700,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 94,
   "resumen": "RESUMEN: Somos el dealer N.º 1 de Culqi (Credicorp) y Movistar en el Perú.",
   "funciones": [
    "Prospección y cierre de ventas de equipos POS.",
    "Gestión de clientes con transacciones +S/50,000 mensuales.",
    "Seguimiento postventa durante 3 meses.",
    "Planificar y organizar Videoconferencias con clientes potenciales para negociar oportunidades de venta.",
    "Reporte de gestión CRM en Excel (funnel, prospección, contactabilidad)."
   ],
   "requisitos": [
    "1 año de experiencia en ventas corporativas (B2B).",
    "Deseable experiencia en POS, medios de pago, telecomunicaciones, banca, seguros o servicios empresariales.",
    "Habilidades de comunicación, negociación y enfoque en resultados."
   ],
   "beneficios": [
    "Modalidad presencial: de Lunes a viernes de 08:30am a 6:15pm, trabaja solo 2 sábados al mes de 09:00am a 1pm.",
    "Básico entre S/1,700(1400+300de movilidad fija el primer mes )",
    "Ingreso a planilla con todos los beneficios, desde el primer día.",
    "Comisiones sin tope (promedio equipo alto desempeño > S/3,000 mensuales).",
    "Capacitaciones pagadas, convenios corporativos, Incentivos adicionales y bonos comerciales.",
    "Línea de carrera real (Supervisor / Jefatura).",
    "Te brindamos herramientas de trabajo (Computadora, equipo telefónico)."
   ],
   "url": "https://www.laborum.pe/job/smart-sales-partners/ejecutivo-comercial-b2b-medios-de-pago-culqi-bcp-perfil/6a9f55b1442c35a1bc06b2cf"
  },
  {
   "id": 200,
   "puesto": "UPN San Juan de Lurigancho: Asesor de ventas Educativas Pregrado",
   "empresa": "Laureate Perú",
   "cat": "Ventas",
   "min": 1350,
   "max": 1350,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 18,
   "vence": "2026-12-07",
   "restan": 72,
   "score": 92,
   "resumen": "En UPN contribuimos a la transformación del Perú a través de una educación inclusiva, accesible y de calidad que transforma vidas .",
   "funciones": [
    "Gestionar el proceso de venta según el producto asignado, cumpliendo la meta mensual y los indicadores diarios de gestión.",
    "Registrar correctamente cada interacción con el prospecto y/o alumno en nuestros sistemas (CRM, Sócrates, Banner, entre otros).",
    "Aplicar el guión de ventas (argumentario, manejo de objeciones, beneficios) siguiendo los lineamientos de capacitación y calidad.",
    "Brindar una atención de calidad durante todo el proceso de venta, cumpliendo el reglamento de admisión y el manual de venta ética."
   ],
   "requisitos": [
    "Estudiante universitario a partir de 7mo ciclo, o con estudios culminados, en Administración, Ingeniería o carreras afines.",
    "Mínimo 1 año de experiencia en puestos de ventas (indispensable).",
    "Deseable: experiencia en Call Center y/o atención al público, y en ventas del sector educativo.",
    "Manejo de Office nivel intermedio (Excel, Word, PowerPoint, correo, Internet).",
    "Buena redacción, ortografía y técnicas de atención al cliente.",
    "Como institución, en UPN valoramos la diversidad y promovemos entornos inclusivos donde todas las personas tengan las mismas oportunidades de participación y desarrollo, sin distinción de origen, género, edad, discapacidad u otras condiciones.",
    "#LoQueNosUneNosHaceUno #OportunidadesLaborales"
   ],
   "beneficios": [
    "Lugar: UPN SJL (San Juan de Lurigancho)",
    "Día de trabajo: Lunes a sábado",
    "Rango horario: 9am a 9pm (Turno 1: 9:00 am a 3:00pm | Turno 2: 3:00 pm a 9:00pm )",
    "Día de descanso: Domingo",
    "Sueldo: S/ 1,350 + Comisiones.",
    "Ingreso a planilla completa desde el primer día.",
    "Seguro Vida Ley desde el primer día de labores.",
    "EPS cubierta al 50% y Seguro Oncológico al 100%.",
    "Beneficios educativos en las instituciones de la Red Laureate (UPN, UPC y Cibertec).",
    "Línea de carrera.",
    "Excelente clima laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/upn-san-juan-de-lurigancho-asesor-de-ventas-educativas-pregrado-call-center-s-1350-comisiones-laureate-peru-1118434411.html"
  },
  {
   "id": 201,
   "puesto": "Asesor de Renovaciones",
   "empresa": "NetCall",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "La Libertad",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 91,
   "resumen": "Nuestra empresa líder en la prestación de servicios con más de 500 colaboradores se encuentra en búsqueda de nuevos talentos VÁLIDO PARA TODO EL PERÚ con disponibilidad para trabajar como:",
   "funciones": [
    "Realizar llamadas a clientes tanto interesados como no interesados para ofrecerles renovar su equipo móvil.",
    "Brindar asesoría acerca de la información y procedimientos del servicio.",
    "Brindar soporte a las consultas o dudas vía telefónica a los clientes.",
    "Otorgar seguimiento de entrega del producto vendido al usuario.",
    "HORARIO DE TRABAJO",
    "Lunes a sábado de 9 a. m. a 6 p. m.",
    "Descansos fijos los DOMINGOS Y FERIADOS."
   ],
   "requisitos": [
    "Tener laptop o PC con internet de modem o router.",
    "Disponibilidad para trabajar desde casa.",
    "Educación mínima: Educación Secundaria",
    "Mayo de edad 18 +",
    "Contar con equipo para gestión remota. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 añosPalabras clave: advisor, asesor, casa, remoto, remote, teletrabajo, home"
   ],
   "beneficios": [
    "Recibe sueldo fijo de S/ 1,130 (Planilla Completa) + Comisiones Ilimitadas + Bonos por desempeño y Productividad",
    "Capacitaciones cortas, virtuales totalmente pagadas.",
    "Oportunidad de crecimiento.",
    "Convenios corporativos y educativos.",
    "Excelente ambiente laboral.",
    "Aplicación de adelanto de sueldo."
   ],
   "url": "https://www.laborum.pe/job/netcall/asesor-de-renovaciones-planilla-completa-trabajo-remoto/6aa03c4f442c35a1bc0a2cb9"
  },
  {
   "id": 202,
   "puesto": "Gestor de PDV",
   "empresa": "Tándem Eje de soluciones Integradas",
   "cat": "Salud",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 91,
   "resumen": "Panorama se encuentra en la búsqueda de un Gestor de PDV para laborar en diversos distritos de Lima.",
   "funciones": [
    "Ejecutar exhibición y reposición de productos OTC en farmacias.",
    "Cumplir ruta asignada en Lima Metropolitana.",
    "Verificar stock, precios y material POP en PDV.",
    "Reportar quiebres, incidencias y avances de ruta.",
    "Monitorear competencia y rotación de productos.",
    "Mantener relación con personal de farmacia."
   ],
   "requisitos": [
    "Estudios técnicos y/o universitarios (administración, Mktg, Farmacia,o carreras afines).",
    "Experiencia en promotoría y mercaderismo (Ideal en mercado farma OTC canal farmacias).",
    "Experiencia en ruta a nivel de Arequipa"
   ],
   "beneficios": [
    "Sueldo: S/ 1,400 + S/ 300 de movilidad.",
    "Comisiones: S/. 600 Por cumplimiento de objetivos.",
    "Horario: De Lunes a Viernes de 9:00 am a 6:00 pm y sábados de 09:00 am a 01:00 pm.",
    "Ingreso a planilla desde el primer día con todos los beneficios de Ley (ESSALUD, CTS, Gratificaciones, Vacaciones, etc.).",
    "Asignación familiar: S/ 113 (de corresponder).",
    "Tipo de puesto: Tiempo completo. -Requerimientos- Educación mínima: Técnico años de experienciaEdad: entre 25 y 45 añosPalabras clave: residente, encargado, supervisor, subgerente, responsable, coordinador, gestor, capitan, farmacologo, farmaceutico, apothecary, pharmacist, boticario"
   ],
   "url": "https://www.laborum.pe/job/tandem-eje-de-soluciones-integradas/gestor-de-pdv-canal-farmaceutico-comisiones-arequipa/6aa021d6442c35a1bc09b562"
  },
  {
   "id": 203,
   "puesto": "Promotora Comercial",
   "empresa": "Grones Sac",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 89,
   "resumen": "RESUMEN: ¡BUSCAMOS PROMOTORA COMERCIAL RECLUTAMIENTO DE CONDUCTORES!",
   "funciones": [
    "Captar activamente conductores de carga pesada mediante llamadas, redes sociales y contacto presencial.",
    "Realizar difusión de ofertas laborales en puertos, almacenes, grifos y zonas estratégicas.",
    "Contactar y realizar seguimiento constante a los candidatos hasta concretar su incorporación.",
    "Convencer y motivar a los postulantes, explicando los beneficios y condiciones de la empresa.",
    "Manejar y publicar convocatorias en bolsas de trabajo y redes sociales.",
    "Realizar llamadas, coordinar entrevistas y hacer seguimiento a los postulantes.",
    "Mantener actualizada la base de datos de candidatos.",
    "Apoyar en el proceso de inducción del personal nuevo.",
    "Apoyar en labores administrativas de oficina"
   ],
   "requisitos": [
    "Tenga poder de convencimiento y persuasión.",
    "Sea proactiva, dinámica y orientada a resultados.",
    "Tenga facilidad para comunicarse y relacionarse con diferentes personas.",
    "Sea perseverante y tenga capacidad para insistir y hacer seguimiento hasta lograr el objetivo.",
    "No tenga temor a realizar llamadas ni contacto presencial.",
    "Tenga experiencia en ventas, call center, promoción, reclutamiento o atención al cliente.",
    "Tenga capacidad para trabajar bajo presión y cumplir metas.",
    "Sepa utilizar bolsas de trabajo y redes sociales para captar candidatos.",
    "Maneje Excel· REQUISITOS",
    "Secundaria completa o estudios técnicos/universitarios.",
    "Experiencia mínima de 6 meses en ventas, call center, promoción, reclutamiento o puestos similares.",
    "Experiencia o facilidad para ofrecer y explicar oportunidades laborales mediante llamadas y presencialmente."
   ],
   "beneficios": [
    "Sueldo: S/ 1,130",
    "+ BONO POR CADA CONTRATACIÓN DE CONDUCTOR",
    "Mientras más conductores logres captar y contratar, mayor será tu ingreso.",
    "Buscamos una persona que quiera ganar más por sus resultados, que tenga actitud comercial y que disfrute de los retos.",
    "Si tienes facilidad para convencer, captar y cerrar incorporaciones, esta oportunidad es para ti.",
    "¡POSTULA Y DEMUESTRA TU TALENTO COMERCIAL!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: salesman, saleswoman, seller, vendedor, comercial, salesperson, seleccion, reclutador, recruiter, headhunter, scout, reclutamiento"
   ],
   "url": "https://www.laborum.pe/job/grones-sac/promotora-comercial-reclutamiento-de-conductores/6a9fd696442c35a1bc084722"
  },
  {
   "id": 204,
   "puesto": "Atención al cliente",
   "empresa": "Grupo Expro",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 89,
   "resumen": "RESUMEN: ¿Te apasiona el mundo aeroportuario y la atención al cliente? ¡Esta es tu oportunidad de formar parte de una empresa líder en servicios aeroportuarios!",
   "funciones": [
    "Recibir, organizar, trasladar, descargar, paletizar y despaletizar la carga que ingresa y sale de los almacenes.",
    "Identificar cargas en mal estado (roturas, violaciones, derrames)",
    "Registro de información en el sistema para ingresar y verificar datos relativos a la carga."
   ],
   "requisitos": [
    "Solo caballeros",
    "Secundaria completa",
    "Con o sin experiencia",
    "Disponibilidad para turnos rotativos: Turno mañana, tarde y noche",
    "Residencia en zonas cercanas al aeropuerto del Callao"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día",
    "Sueldo S/1,130 + Bono nocturno (S/325)",
    "Tarjeta de alimentos S/60",
    "Bono trimestral del 25% (S/282.50)",
    "Cobertura del 85% en alimentación",
    "Línea de carrera y desarrollo profesional",
    "Beneficios de ley: CTS, gratificaciones, vacaciones, asignación familiar, utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 45 añosPalabras clave: airport"
   ],
   "url": "https://www.laborum.pe/job/grupo-expro/trabaja-en-el-aeropuerto-nuevo-del-callao-almaceneros-sin/6a9fd1ac442c35a1bc082c84"
  },
  {
   "id": 205,
   "puesto": "Asistente de",
   "empresa": "INVERSOL RECARGA SELVA EIRL",
   "cat": "Prácticas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 89,
   "resumen": "RESUMEN: ¡OPORTUNIDAD LABORAL ASISTENTE DE CONTABILIDAD!",
   "funciones": [
    "Asistir en la preparación de información para libros electrónicos.",
    "Realizar el registro y provisión de gastos mensuales.",
    "Brindar apoyo en la ejecución de análisis de cuentas.",
    "Recolectar, organizar y mantener actualizada la documentación requerida para auditorías.",
    "Apoyar en las actividades propias del área contable."
   ],
   "requisitos": [
    "Egresado(a) 2026 de la carrera de Contabilidad.",
    "Manejo de Excel a nivel intermedio.",
    "Interés por desarrollarse en el área contable.",
    "Disponibilidad para realizar labores de manera presencial en el distrito de la Victoria."
   ],
   "beneficios": [
    "Ingreso bajo modalidad de Recibo por Honorarios desde el primer día.(Ingreso a planilla Mype de forma posterior según desempeño",
    "Sueldo: S/ 1,130.00",
    "Oportunidad de adquirir experiencia y fortalecer conocimientos en el área de Contabilidad.",
    "Ingreso bajo modalidad de Recibo por Honorarios desde el primer día.",
    "Lunes a viernes: 09:00 a.m. 06:00 p.m.",
    "Sábados: 09:00 a.m. 01:00 p.m.",
    "Modalidad: Presencial Edificio Tanguis -La Victoria. -Requerimientos- Educación mínima: Técnico años de experienciaEdad: entre 20 y 30 añosPalabras clave: intern, practicante, becario, pasante"
   ],
   "url": "https://www.laborum.pe/job/inversol-recarga-selva-eirl/grupoinversol-lider-distribuidor-claro-busca-practicante-de/6a9f558f442c35a1bc06b1b5"
  },
  {
   "id": 206,
   "puesto": "Asesor de Cobranzas Temprana",
   "empresa": "Phoeninca",
   "cat": "Contabilidad",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 89,
   "resumen": "RESUMEN: PHOENINCA | Conectamos talento, clientes y resultados",
   "funciones": [
    "Gestionar la cartera asignada realizando cobranzas de manera efectiva y profesional.",
    "Realizar seguimiento y negociación con clientes para lograr la regularización de sus obligaciones.",
    "Maximizar la recuperación de deuda mediante estrategias de negociación.",
    "Cumplir con los objetivos y metas de recuperación establecidos.",
    "Brindar una atención clara, cordial y orientada a resultados."
   ],
   "requisitos": [
    "Experiencia mínima de 6 meses en Cobranzas Temprana.",
    "Experiencia en gestión de productos financieros. (Indispensable)",
    "Orientación al cumplimiento de metas y resultados.",
    "Comunicación clara, persuasiva y capacidad de negociación.",
    "Actitud proactiva y capacidad para resolver situaciones de manera efectiva."
   ],
   "beneficios": [
    "Sueldo: S/ 1,300",
    "Planilla completa, con todos los beneficios de ley.",
    "Ingreso directo.",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Integrarte a una empresa en crecimiento.",
    "Lugar de trabajo: San Isidro, Lima.",
    "Horario Full Time",
    "Lunes a viernes: 8:00 a. m. a 6:00 p. m.",
    "Sábados: 9:00 a. m. a 12:00 p. m.",
    "¡Únete a PHOENINCA!",
    "Sé parte de un equipo que apuesta por el talento, el crecimiento profesional y la generación de resultados.",
    "¡Tu experiencia puede ser el siguiente paso hacia nuevas oportunidades!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 20 y 50 añosPalabras clave: advisor, asesor"
   ],
   "url": "https://www.laborum.pe/job/phoeninca/ingreso-inmediato-asesor-de-cobranzas-temprana-planilla/6a9f52be442c35a1bc069ada"
  },
  {
   "id": 207,
   "puesto": "Reponedor@",
   "empresa": "Grupo Solimano",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 89,
   "resumen": "RESUMEN: Por encargo de nuestro cliente, una empresa líder en consumo masivo, estamos en la búsqueda de TALENTOS COMO TÚ para ocupar la vacante de Mercaderista de ruta en supermercados.",
   "funciones": [
    "Exposición y exhibición en punto de venta.",
    "Gestión de espacios adicionales.",
    "Colocado de material POP.",
    "Relevo de información en APP."
   ],
   "requisitos": [
    "Contar con 03 meses minimo de experiencia como mercaderista o reponedor@.",
    "Deseable experiencia haciendo ruta.",
    "Ser pro activo, dinámico y contar con la orientación al cliente.",
    "Experiencia en atención a clientes en supermercados.",
    "Carnet de Sanidad vigente"
   ],
   "beneficios": [
    "Remuneración: Básico S/.1130 + movilidad S/. 300 + variable s/. 100 + datos moviles S/.35.90",
    "Planilla completa con todos los beneficios de acuerdo a ley desde el primer día de trabajo.",
    "Capacitaciones constantes.",
    "Horario: Lunes a Domingo: 7AM A 4PM (descanso fijo martes)",
    "ZONA LIMA SUR: San Juan de Miraflores, Villa Maria del Triubnfo, Chorrillos, Villa El Salvador, Surco y San Borja (Plaza Vea, Makro, Metro o Tottus)",
    "3 puntos de venta por día (Tiendas cercanas entre si, acomodamos las tiendas para que la ruta sea sencilla)",
    "¡No pierdas la oportunidad!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: comerciante, merchandiser, merchant, area"
   ],
   "url": "https://www.laborum.pe/job/grupo-solimano/mercaderista-supermercados-zona-lima-sur-gana-hasta-s-1565/6a9f62c7442c35a1bc06f704"
  },
  {
   "id": 208,
   "puesto": "Operador",
   "empresa": "Salesland Pacífico",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 89,
   "resumen": "Contactar clientes del mismo operador para ofrecer un cambio de plan",
   "funciones": [
    "Contactar clientes del mismo operador para ofrecer un cambio de plan",
    "Brindar información clara sobre planes, promociones y beneficios",
    "Registrar correctamente los datos en el sistema",
    "Cumplir objetivos de ventas y calidad"
   ],
   "requisitos": [
    "Con o sin experiencia en Call Center/presencial",
    "(ventas, retenciones o portabilidad)",
    "Buena comunicación y actitud comercial",
    "Compromiso y orientación a resultados",
    "Disponibilidad inmediata"
   ],
   "beneficios": [
    "Sueldo fijo de S/ 1,130",
    "Bono Bienvenida S/100",
    "Bono por Asistencia S/200",
    "Comisiones ILIMITADAS + atractivos incentivos",
    "Bono de productividad de S/100, S/150 - S/200",
    "Capacitación corta y pagada (3 días)",
    "Pagos quincenales",
    "Línea de carrera y oportunidades de crecimiento",
    "Lunes a sábado",
    "Turnos disponibles",
    "9:00 a.m. a 4:30 p.m.",
    "10:00 a.m. a 5:30 p.m."
   ],
   "url": "https://www.laborum.pe/job/salesland-pacifico/urgente-con-o-sin-experiencia-semi-full-elige-tu-turno/6a9f5595442c35a1bc06b236"
  },
  {
   "id": 209,
   "puesto": "Call center",
   "empresa": "Grupo Expro",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 89,
   "resumen": "RESUMEN: ¡Únete al equipo! Nos encontramos en la búsqueda de talentos dinámicos y comerciales para asumir la posición de:",
   "funciones": [
    "Realizar llamadas a clientes potenciales para la comercialización de productos y servicios de telecomunicaciones.",
    "Identificar necesidades comerciales y concretar cierres de venta efectivos.",
    "Brindar asesoría clara, precisa y atractiva sobre las promociones y ofertas vigentes.",
    "Alcanzar los objetivos diarios de ventas y métricas de productividad establecidas."
   ],
   "requisitos": [
    "Secundaria completa certificada (indispensable contar con documento probatorio: certificado de estudios o CVI).",
    "Experiencia mínima de 3 meses en ventas o cross-selling por call center.",
    "Contar con equipo propio para el periodo formativo (capacitación virtual).",
    "Conexión a internet fija en casa igual o mayor a 50 Mbps.",
    "Residir actualmente en Lima o Callao (excluyente).",
    "Horario de trabajo",
    "Jornada completa (48 h semanales): Lunes a sábado de 11:00 a. m. a 8:00 p. m.",
    "Descanso fijo: Domingos."
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,130.00 en planilla completa desde el primer día con todos los beneficios de ley (CTS, gratificaciones, vacaciones, asignación familiar, EsSalud, utilidades).",
    "Bono de bienvenida: S/ 150.00 adicionales durante los dos primeros meses.",
    "Comisiones y bonificaciones por cumplimiento de metas.",
    "Equipamiento brindado por la empresa para tu gestión laboral (computadora/periféricos para el trabajo regular).",
    "Oportunidades reales de línea de carrera y crecimiento interno.",
    "Hoy te elegimos a través de nuestro proceso de selección; nuestra meta es que al final del día seas tú quien decida quedarse y crecer con nosotros.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: callcenter, teleoperador, telefonista, telephonist, telefonico, permanent, fijo"
   ],
   "url": "https://www.laborum.pe/job/grupo-expro/te-brindamos-laptop-o-pc-teleoperador-call-horario-fijo/6a9f948e442c35a1bc073f42"
  },
  {
   "id": 210,
   "puesto": "Operario de Limpieza en Altura",
   "empresa": "EULEN del Perú",
   "cat": "Otros",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 87,
   "resumen": "RESUMEN: OPERARIO DE LIMPIEZA EN ALTURA UNIVERSIDAD CERCA A SAN MIGUEL",
   "funciones": [
    "Realizar labores de limpieza y mantenimiento en altura dentro de la institución.",
    "Cumplir con los protocolos de higiene y seguridad establecidos.",
    "Mantener las áreas asignadas en óptimas condiciones.",
    "Reportar cualquier incidente o novedad al encargado de la instalación."
   ],
   "requisitos": [
    "Experiencia realizando labores de limpieza en altura.",
    "Disponibilidad para trabajar de manera fija.",
    "Disponibilidad para laborar de lunes a sábado.",
    "Compromiso y responsabilidad en el trabajo."
   ],
   "beneficios": [
    "Horario: Lunes a sábado de 7:00 a.m. a 4:00 p.m.",
    "Descanso: Domingo fijo.",
    "Remuneración: S/ 1,600 mensuales.",
    "Ingreso a planilla y beneficios de ley."
   ],
   "url": "https://www.laborum.pe/job/eulen-del-peru/operario-de-limpieza-en-altura-importante-universidad-en-san/6a9fbc5c442c35a1bc07c098"
  },
  {
   "id": 211,
   "puesto": "Atención al cliente",
   "empresa": "XINTECH",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 87,
   "resumen": "RESUMEN: ¡Estamos buscando Asesores de Ventas con actitud comercial y muchas ganas de generar ingresos!",
   "funciones": [
    "Realizar llamadas a clientes del público argentino.",
    "Ofrecer y promocionar tarjetas de crédito.",
    "Brindar información clara sobre los beneficios del producto.",
    "Cumplir con los objetivos de ventas establecidos."
   ],
   "requisitos": [
    "Experiencia en ventas, call center o atención al cliente.",
    "Deseable experiencia en ventas de productos intangibles.",
    "Facilidad de palabra y buena comunicación.",
    "¡Muchas ganas de aprender y generar excelentes comisiones!",
    "Turno mañana",
    "Lunes a viernes de 7:00 a. m. a 4:00 p. m.",
    "Lunes a viernes de 10:00 a. m. a 7:00 p. m.",
    "De 8:00 a. m. a 2:00 p. m."
   ],
   "beneficios": [
    "Sueldo fijo de S/ 1,300.",
    "Bono de S/ 50.",
    "Comisiones ilimitadas.",
    "Capacitaciones pagadas.",
    "Excelente ambiente laboral."
   ],
   "url": "https://www.laborum.pe/job/xintech/asesores-de-ventas-call-center-s-1350/6a9f623c442c35a1bc06f5c1"
  },
  {
   "id": 212,
   "puesto": "Auxiliar de Despacho",
   "empresa": "Lavoro Perú",
   "cat": "Logística",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Callao",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 85,
   "resumen": "RESUMEN: Nuestro cliente, importante empresa localizada en Oquendo, destacada en el rubro de embotelladora de bebidas se encuentra en la búsqueda del/ los mejor(es) talento(s) para ocupar el puesto de Auxiliar de Despacho.",
   "funciones": [
    "Picking de productos",
    "Orden/limpieza en almacén",
    "Ubicación de productos de descarga",
    "Acomodo correcto de productos para colocarlos en ubicaciones correspondientes",
    "Etiquetado y embalaje de productos",
    "Conteo de productos"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Experiencia de 3 meses en cargos similares o trabajando en planta",
    "Disponibilidad inmediata",
    "Con Domicilio en zonas cercanas Callao, entre otro.",
    "HORARIO DE TRABAJO: Turno TARDE",
    "Lunes a Sábado",
    "Turno 1: 3pm a 11pm"
   ],
   "beneficios": [
    "Agradable ambiente laboral.",
    "Ingreso directo a Planilla desde el primer día de labores con todos los Beneficios de Ley.",
    "Remuneración: S/. 1200",
    "Bono de movilidad S/. 100",
    "Bono de Asistencia S/. 100",
    "Asignación Familiar S/ 113",
    "Horas Extras pagadas conforme a ley.",
    "Pagos puntuales en quincena y fin de mes",
    "Convenios institucionales",
    "Seguro SCTR (en caso corresponda) y Vida Ley",
    "Pertenecer a una empresa líder en el mercado",
    "Somos una empresa que promueve la igualdad de oportunidades y no discriminamos por motivo de origen, edad, sexo, discapacidad u otra condición , asi como la inclusión laboral de personas con discapacidad."
   ],
   "url": "https://www.laborum.pe/job/lavoro-peru/auxiliar-de-despacho-callao-tarde-urgente-callao/6a9fca4f442c35a1bc0806fa"
  },
  {
   "id": 213,
   "puesto": "Cajero Almacenero",
   "empresa": "Salesland Pacífico",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 84,
   "resumen": "En Salesland Pacífico estamos en búsqueda de personas responsables y ordenadas para sumarse a nuestro equipo operativo.",
   "funciones": [
    "Manejo de caja y cobros.",
    "Emisión de comprobantes y cuadre diario.",
    "Recepción, control y orden de mercadería.",
    "Apoyo en reposición y despacho en tienda."
   ],
   "requisitos": [
    "Experiencia como cajero/a o almacenero/a.",
    "Orden, responsabilidad y trabajo en equipo.",
    "Disponibilidad para laborar: 8:30 a 6, Domingo descanso."
   ],
   "beneficios": [
    "Planilla completa desde el primer día.",
    "Sueldo básico S/.1130.",
    "Capacitación constante.",
    "Buen clima laboral.",
    "¡Postula ahora y forma parte de Movistar Minka!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 añosPalabras clave: cashier, cajero"
   ],
   "url": "https://www.laborum.pe/job/salesland-pacifico/trabaja-en-canepa-cajero-almacenero-a-en-movistar-tiendas/6a9f5ea8442c35a1bc06e97e"
  },
  {
   "id": 214,
   "puesto": "Supervisor",
   "empresa": "Manpower",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 83,
   "resumen": "RESUMEN: ¡Únete a nuestro equipo de ventas!",
   "funciones": [
    "Prospección y captación de clientes en calle, hogares y negocios.",
    "Ventas en puerta fría.",
    "Negociación y cierre de ventas.",
    "Trabajo 100% de campo."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Con o sin experiencia en ventas.",
    "Habilidades de comunicación y negociación.",
    "Disponibilidad para realizar trabajo de campo.",
    "Rango de horario: Lunes a Sábado: 48 horas semanales y Descanso FIJO todos los domingos",
    "Horarios referenciales según visitas agendadas y coordinación con el supervisor."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,130.",
    "Tarjeta de alimentos: S/ 100.",
    "Comisiones ilimitadas desde la primera venta (Promedio de comisiones: S/. 3.000 - S/. 5,000)",
    "EPS al 80% desde el tercer mes.",
    "Seguro oncológico al 100% para el titular.",
    "Línea de carrera.",
    "Posibilidad de acceder a vehículo con combustible según cumplimiento de objetivos.",
    "Aguinaldo por Navidad y descuentos en productos de la empresa.",
    "Smartphone, uniforme y herramientas comerciales.",
    "¡Postula ahora y empieza a generar comisiones desde tu primera venta!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 19 y 43 añosPalabras clave: terreno, territorio, permanent, fijo, escolta, guardia, vigilante, guarda, guard, vigilancia"
   ],
   "url": "https://www.laborum.pe/job/manpower/sin-experiencia-vendedores-de-campo-planilla-completa-fijo/6a9f62b5442c35a1bc06f6d5"
  },
  {
   "id": 215,
   "puesto": "Vendedor",
   "empresa": "Atento Perú",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 18,
   "vence": "2026-10-08",
   "restan": 12,
   "score": 80,
   "resumen": "Sede: Av. La Molina 200, Ate (a 2 cuadras del Óvalo Santa Anita)",
   "funciones": [
    "Contactar clientes Movistar Prepago para ofrecer migración a Plan Postpago.",
    "Brindar promociones y descuentos exclusivos.",
    "Gestionar ventas mediante llamadas de salida."
   ],
   "requisitos": [
    "Personas con ganas de aprender y desarrollarse en ventas.",
    "Buen nivel de comunicación.",
    "Con o sin experiencia.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 19 añosConocimientos: Negociación, Resolución de problemasPalabras clave: salesman, saleswoman, seller, vendedor, comercial, salesperson"
   ],
   "beneficios": [
    "Sueldo fijo de S/ 1,130",
    "Comisiones ilimitadas",
    "Promedio de comisiones: S/ 300",
    "Top de comisiones: S/ 700",
    "Planilla completa desde el primer día.",
    "Vale de alimentos para comisiones desde S/ 226.",
    "Lunes a Sábado de 9:00 a.m. a 6:00 p.m.",
    "Descanso fijo: Domingo",
    "48 horas semanales",
    "Duración: 7 días",
    "Horario: 8:00 a.m. a 2:00 p.m.",
    "Pago de capacitación: S/ 20 por día asistido (hasta S/ 140)"
   ],
   "url": "https://www.laborum.pe/job/atento-peru/asesor-a-comercial-movistar-presencial-gana-mas-de-s-1-800/6a9f5c43442c35a1bc06da17"
  },
  {
   "id": 216,
   "puesto": "Operario de Producción",
   "empresa": "Eurofirms Group",
   "cat": "Ingeniería",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 19,
   "vence": "2026-12-06",
   "restan": 71,
   "score": 97,
   "resumen": "En Eurofirms Group, creemos en el liderazgo por valores y el crecimiento continuo, poniendo a las personas y su desarrollo profesional primero, potenciando un ambiente de trabajo positivo y colaborativo.",
   "funciones": [
    "Ejecuta procesos y/u operaciones relacionadas a la producción de mezclas líquidas y de polvo, haciendo buen uso de insumos, equipos e instalaciones, acorde a las Instrucciones de Producción y a las normas de seguridad y salud en el trabajo de la Planta.",
    "Colocarse los equipos de protección personal (EPPs) antes de iniciar las labores de producción según el tipo de proceso y protección indicada antes de iniciar la producción.",
    "Verificar que las máquinas y equipos de fabricación, se encuentren debidamente limpios.",
    "Verificar que la materia prima dispensada corresponda a lo indicado en la Instrucción de manufactura.",
    "Iniciar el proceso de producción según la instrucción de manufactura.",
    "Extraer una muestra del lote fabricado para realizar el control de calidad respectivo.",
    "Envasar el producto según lo indicado en la Instrucción de manufactura.",
    "Verificar que los Insumos Químicos y Bienes Fiscalizados (IQBF) dispensados para el proceso productivo cumplan con lo señalado en la Instrucción de Manufactura.",
    "Utilizar responsablemente los Insumos Químicos y Bienes Fiscalizados (IQBF) para el proceso productivo.",
    "Informar al Supervisor de producción, cualquier merma, perdida, derrame o incidencia que ocurra respecto a los Insumos Químicos y Bienes Fiscalizados (IQBF).",
    "Cumplir con realizar correctamente la segregación de los residuos sólidos que se generan producto de la actividad, utilizando los tachos diferenciados de colores.",
    "Llenar correctamente los registros de la Instrucción de Manufactura."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia previa en producción, en industrias de químicos, construcción, entre otros.",
    "Disponibilidad para laborar en Lurín.",
    "Experiencia en manejo de Montacargas (certificado)",
    "Disponibilidad para trabajar en horarios rotativos de manera semanal: Lunes a viernes de 7am a 5pm / Lunes a Jueves de 7pm a 7am."
   ],
   "beneficios": [
    "Sueldo base: S/ 1,600",
    "Servicio de movilidad",
    "Bono de Alimentos S/ 13.80 por día efectivo laborado",
    "Ingreso a planilla con todos los beneficios de ley (CTS, Gratificación, Utilidades, Seguro Es salud, Asignación familiar)",
    "Si cumples con los requisitos, estás interesado y crees que eres el match perfecto, te invitamos a postular ¡Te esperamos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-produccion-movilidad-ida-y-vuelta-alimentacion-eurofirms-group-1118432554.html"
  },
  {
   "id": 217,
   "puesto": "Cajera",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Otros",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 19,
   "vence": "2026-12-06",
   "restan": 71,
   "score": 94,
   "resumen": "Realizar el manejo, control y cuadre de caja durante el turno.",
   "funciones": [
    "Realizar el manejo, control y cuadre de caja durante el turno.",
    "Recibir y procesar pagos mediante efectivo, tarjetas, transferencias y otros medios autorizados.",
    "Emitir boletas, facturas y demás comprobantes de pago según corresponda.",
    "Vender y recomendar suplementos deportivos, productos funcionales, snacks y demás productos del Market Saludable.",
    "Brindar información clara y responsable sobre las características y beneficios de los productos, de acuerdo con la capacitación y lineamientos de la empresa.",
    "Promover y vender los servicios de asesoría nutricional personalizada.",
    "Captar potenciales clientes interesados en asesoría nutricional y gestionar el agendamiento de sus citas.",
    "Realizar seguimiento a potenciales clientes y oportunidades comerciales mediante los canales autorizados por la empresa.",
    "Promover productos y servicios complementarios de acuerdo con las necesidades del cliente.",
    "Cumplir con metas individuales y objetivos comerciales establecidos por la empresa.",
    "Mantenerse actualizado sobre los productos, suplementos, servicios y promociones ofrecidos por Buenas Fibras.",
    "Apoyar en la correcta exhibición y presentación de los productos del Market Saludable."
   ],
   "requisitos": [
    "Solo mujeres de cualquier nacionalidad entre 20 y 35 años.",
    "Tener experiencia mínima de 1 año de experiencia en ventas, atención al cliente, caja o puestos comerciales similares.",
    "Disponibilidad para trabajar 48 horas semanales, 6 días a la semana con 1 dia de descanso rotativo. Los horarios pueden ser: de 7:45 am a 4:30 pm o de 1:15 pm a 10 pm."
   ],
   "beneficios": [
    "Grato ambiente laboral y posibilidad de hacer línea de carrera.",
    "Pago quincena y fin de mes.",
    "Beneficios laborales de planilla remype desde el 1er día de trabajo, tienen 15 días de vacaciones al año y Essalud.",
    "Sueldo básico (S/.1500) + comisiones + alimentación."
   ],
   "url": "https://www.bumeran.com.pe/empleos/cajera-adn-retail-s.a.c.-1118432596.html"
  },
  {
   "id": 218,
   "puesto": "Jefe",
   "empresa": "Caja de Pensiones Militar Policial",
   "cat": "Ingeniería",
   "min": 10500,
   "max": 10500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 19,
   "vence": "2026-12-06",
   "restan": 71,
   "score": 92,
   "resumen": "Revisar las propuestas de elegibilidad de instrumentos u operaciones de inversión, emisores, intermediarios de negociación, contrapartes, entidades de custodia, mecanismos electrónicos de negociación, indicadores de referencia, entre otros, de renta variable y alternativos.",
   "funciones": [
    "Revisar las propuestas de elegibilidad de instrumentos u operaciones de inversión, emisores, intermediarios de negociación, contrapartes, entidades de custodia, mecanismos electrónicos de negociación, indicadores de referencia, entre otros, de renta variable y alternativos.",
    "Revisar y custodiar los documentos que forman parte del sustento del análisis de la decisión de inversión, el mismo que integra el Archivo de Expediente de Inversión de los instrumentos u operaciones de inversión de renta variable y alternativos.",
    "Seleccionar y remitir propuestas de inversión y/u operación de clases de activos, emisores, intermediarios de negociación, contrapartes, entidades de custodia, mecanismos electrónicos de negociación, entre otros, en relación a las inversiones en renta variable y alternativos.",
    "Revisar y supervisar la elaboración de la propuesta de ejecución de instrumentos de inversión de renta variable y alternativos en el sistema integrado de inversiones.",
    "Gestionar de manera integral la atención y registro de llamados de capital, distribuciones, estados de cuenta y otros aspectos administrativos y operativos relacionados con los fondos de inversión, asegurando una comunicación fluida y precisa con las partes involucradas.",
    "Atender el requerimiento regulatorio y control de límites de inversión.",
    "Representar a los regímenes administrados en los comités que se le encarguen en relación a las inversiones en renta variable y alternativos.",
    "Proponer lineamientos de política de inversiones y estrategias para optimizar la administración del portafolio de renta variable y alternativos.",
    "Formular las propuestas de inversión en renta variable y alternativos, y aquellas que permitan dar atención a los requerimientos realizados por el Comité de Inversiones, según el ámbito de su competencia.",
    "Ejecutar las operaciones de inversión en instrumentos financieros de renta variable y alternativos.",
    "Presentar información y reportes sobre el mercado financiero, seguimiento de indicadores para la toma de decisiones, según se requiera.",
    "Presentar el informe mensual de la situación de las inversiones en renta variable y alternativos y su desempeño."
   ],
   "requisitos": [
    "Título profesional o bachiller en contabilidad, economía, administración, ingeniería industrial o afines.",
    "De preferencia, postgrado en inversiones o afines.",
    "Certificación internacionalmente reconocida en inversiones – Nivel I.",
    "De preferencia, capacitación en interpretación de la Norma internacional ISO 37001:2016 Antisoborno.",
    "Experiencia general de seis (6) años y experiencia especifica de cuatro (4) años acumulable como subgerente, jefe o puesto similar.",
    "FUNCIÓN BÁSICA"
   ],
   "beneficios": [
    "Remuneración: S/. 10,500.00",
    "Horario de Trabajo regular: 9:00 a.m. a 5:15 p.m. de lunes a viernes (presencial).",
    "Modalidad de contratación: Plazo indeterminado (Planilla, beneficios según ley).",
    "Capacitación acorde a la necesidad del puesto.",
    "Los datos personales que el postulante facilite a través de su hoja de vida, podrán ser recopilados, tratados, e incorporados a un Banco de Datos Personales de la CPMP, solo para dichos fines específicos, y serán conservados por un periodo máximo de dos años.",
    "La Caja de Pensiones Militar Policial - CPMP declara que respeta los principios de legalidad, consentimiento, finalidad, proporcionalidad, calidad, disposición de recurso, nivel de protección adecuado, conforme a las disposiciones de la Ley de Protección de Datos Personales vigente en Perú\"",
    "\" En la Caja de Pensiones Militar Policial creemos firmemente en la igualdad de oportunidades por lo cual promovemos la diversidad de género, origen étnico, nacionalidad, la inclusión de personas con discapacidad y/o habilidades diferentes (de acuerdo con lo establecido en la Ley N° 29973).\""
   ],
   "url": "https://www.bumeran.com.pe/empleos/subgerente-de-renta-variable-y-alternativos-caja-de-pensiones-militar-policial-1118432592.html"
  },
  {
   "id": 219,
   "puesto": "Representante de Servicios Part Time",
   "empresa": "CAJA CUSCO",
   "cat": "Ingeniería",
   "min": 690,
   "max": 690,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Cusco",
   "fuente": "Bumeran",
   "dias": 19,
   "vence": "2027-01-05",
   "restan": 101,
   "score": 92,
   "resumen": "Actualmente estamos en búsqueda del mejor talento para el puesto de: REPRESENTANTE DE SERVICIOS PART TIME - JULIACA",
   "funciones": [
    "Atender las operaciones de clientes y realizar la venta de servicios brindando un servicio de calidad, transparencia de información y guardando la confidencialidad de las operaciones e información de clientes garantizando la confidencialidad de información de los usuarios.",
    "Orientar a los clientes con información transparente sobre los productos y servicios que se ofrecen, brindando soluciones para asegurar una atención de calidad, cortesía a clientes y preservar una buena imagen institucional.",
    "Realizar la venta de servicios según las metas asignadas para el cumplimiento de objetivos de las Agencia y de la empresa.",
    "Realizar el cuadre de efectivo de forma diaria e informar en caso de identificar sobrantes o faltantes para cumplir con los procedimientos de control interno.",
    "Realizar actividades de control interno que aseguren que las operaciones se realicen cumpliendo los procedimientos internos y normas de entes supervisores, a fin de minimizar riesgos de sanciones."
   ],
   "requisitos": [
    "Experiencia al Cliente",
    "Espíritu Colaborativo",
    "Mejora Continua",
    "Profesional Técnico, Egresado, Bachiller o Estudios truncos de cualquier especialidad de Institutos o Universidades.",
    "Representante de servicios con experiencia: Seis (6) meses en el mismo cargo o un (1) año como Auxiliar de Atención al Usuario y Calidad o Puestos equivalentes en empresas del Sistema Financiero.",
    "Representante de servicios junior: sin experiencia"
   ],
   "beneficios": [
    "Remuneración fija de S/. 690",
    "Oportunidad de Línea de carrera.",
    "Ingreso a planilla con todos los beneficios de ley.",
    "FECHA LIMITE DE POSTULACION: DOMINGO 13 DE SEPTIEMBRE DEL 2026"
   ],
   "url": "https://www.bumeran.com.pe/empleos/representante-de-servicios-part-time-juliaca-ref-1553-2026-caja-cusco-1118432537.html"
  },
  {
   "id": 220,
   "puesto": "Atención al cliente para minimarket! Trabaja en Miraflores",
   "empresa": "Eurofirms Group",
   "cat": "Atención al Cliente",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 19,
   "vence": "2026-12-06",
   "restan": 71,
   "score": 89,
   "resumen": "En Eurofirms Group , creemos en el liderazgo por valores y el crecimiento continuo, poniendo a las personas y su desarrollo profesional primero, potenciando un ambiente de trabajo positivo y colaborativo.",
   "funciones": [
    "Atención al cliente y promoción de ventas, brindando un servicio ágil y de calidad.",
    "Manejo de caja, recepción y reposición de mercadería en tienda y almacén.",
    "Manipulación de alimentos, limpieza integral del local y apoyo en inventarios."
   ],
   "requisitos": [
    "Deseable contar con 3 meses de experiencia en atención al cliente.",
    "Contar con Carnet de Sanidad con Manipulación de Alimentos . (Indispensable)",
    "Contar con disponibilidad para desempeñar funciones operativas propias de tienda (trabajo de pie, carga ligera, desplazamiento constante, entre otros).",
    "Experiencia en cargos similares: atención al Cliente; cajero, reponedor y promoción de productos, y conocimientos básicos de ventas.",
    "(Turno Rotativo)",
    "Lunes a Domingo con 1 día de descanso a la semana."
   ],
   "beneficios": [
    "Sueldo básico 1,130 soles.",
    "Bono objetivos de hasta 120 soles.",
    "Bonos por no contar con faltante de caja de 50 soles.",
    "Bono por Movilidad de S/ 150.",
    "Ingreso a planilla con todos los beneficios de ley desde el 1er día.",
    "Línea de Carrera.",
    "Si cumples con los requisitos, estás interesado y crees que eres el match perfecto, te invitamos a postular ¡Te esperamos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/cajero-a-multifuncional-para-importante-cadena-de-tienda-miraflores-surco-o-san-borja-eurofirms-group-1118432573.html"
  },
  {
   "id": 221,
   "puesto": "Ayudante de panadería",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ingeniería",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 21,
   "vence": "2026-12-03",
   "restan": 68,
   "score": 89,
   "resumen": "Preparar y pesar ingredientes según las recetas.",
   "funciones": [
    "Preparar y pesar ingredientes según las recetas.",
    "Apoyar en la elaboración de panes, focaccias, pizzas, pastas y demás productos.",
    "Apoyar en horneado, porcionado, empaquetado y etiquetado.",
    "Preparar y realizar el despacho de pedidos.",
    "Mantener el orden, limpieza y desinfección del área de producción.",
    "Cumplir con las buenas prácticas de manipulación de alimentos.",
    "Apoyar en el control y almacenamiento de insumos y productos.",
    "Apoyar con el control de calidad de los productos finalizados."
   ],
   "requisitos": [
    "Hombres o mujeres de cualquier nacionalidad entre 22 y 38 años.",
    "Tener experiencia mínima de 1 año en puestos similares o en atención al cliente en restaurantes, cafeterías o establecimientos gastronómicos o en cocina, panadería o poducción de alimentos.",
    "Disponibilidad para trabajar de lunes a viernes de 8 am a 5 pm y los sábados 8 am a 12 pm.",
    "Responsable, puntual, ordenado/a y con atención al detalle // Ágil y con disposición para aprender // Capacidad para seguir recetas y procesos con precisión // Buena disposición para el trabajo en equipo //"
   ],
   "beneficios": [
    "Grato ambiente laboral y posibilidad de hacer línea de carrera.",
    "Pago fin de mes.",
    "Beneficios laborales de planilla remype desde el 1er día de trabajo, tienen 15 días de vacaciones al año y Essalud.",
    "Bonificación de medio sueldo en julio y diciembre dependiendo de la productividad.",
    "Sueldo básico (S/.1500)."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ayudante-de-panaderia-adn-retail-s.a.c.-1118430619.html"
  },
  {
   "id": 222,
   "puesto": "Ejecutivo Comercial Kam Jr. Servicios Logísticos",
   "empresa": "GRAFTON LATAM",
   "cat": "Ventas",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 22,
   "vence": "2026-12-03",
   "restan": 68,
   "score": 97,
   "resumen": "Por encargo de nuestro cliente, importante operador logístico con amplia presencia en el mercado peruano, nos encontramos en búsqueda de un(a)",
   "funciones": [
    "Gestionar y desarrollar la cartera de clientes asignada, principalmente de los sectores cosmético y farmacéutico .",
    "Mantener una relación cercana y de largo plazo con los clientes, atendiendo oportunamente sus requerimientos y necesidades.",
    "Realizar seguimiento al cumplimiento de los servicios y niveles de atención acordados.",
    "Identificar oportunidades de crecimiento, nuevos servicios y desarrollo de las cuentas asignadas.",
    "Elaborar y realizar seguimiento de cotizaciones y propuestas comerciales.",
    "Dar seguimiento a reclamos e incidencias, coordinando con las áreas internas hasta su resolución.",
    "Realizar seguimiento a indicadores de servicio, satisfacción del cliente y cumplimiento de compromisos.",
    "Asegurar que los servicios brindados se encuentren correctamente formalizados y facturados.",
    "Realizar seguimiento a contratos, alcances de servicio y condiciones comerciales.",
    "Participar en reuniones periódicas con clientes y presentar resultados, hallazgos y oportunidades de mejora.",
    "Coordinar permanentemente con las áreas operativas para asegurar una adecuada experiencia del cliente.",
    "Contribuir al cumplimiento de las metas comerciales grupales ."
   ],
   "requisitos": [
    "Profesional universitario de Administración, Ingeniería Industrial o carreras afines .",
    "Experiencia previa como KAM, Ejecutivo de Cuenta, Ejecutivo Comercial, Ejecutivo de Servicios o posiciones similares .",
    "Experiencia gestionando y desarrollando carteras de clientes B2B .",
    "Experiencia ideal en empresas del sector logístico, cosmético y/o farmacéutico .",
    "Conocimiento del mercado logístico y gestión de servicios.",
    "Manejo de Excel a nivel avanzado .",
    "Deseable conocimiento de Power BI .",
    "Inglés intermedio , necesario para reuniones con clientes.",
    "Habilidades de negociación, comunicación, orientación al cliente y desarrollo comercial.",
    "Disponibilidad para trabajar presencialmente de lunes a viernes.",
    "Disponibilidad para realizar desplazamientos ocasionales entre las diferentes sedes de la compañía.",
    "Lugar de trabajo principal: zona Los Frutales - Ate, Lima, con visitas ocasionales a la operación de Lurín."
   ],
   "beneficios": [
    "Compensación referencial: hasta S/6,000 , compuesta inicialmente por S/4,000 básico + S/2,000 variable",
    "Ingreso a planilla bajo régimen general desde el primer día .",
    "Bono de movilidad: S/105.60 .",
    "Bono de alimentación: S/60.50 .",
    "EPS cubierta al 30% .",
    "Traslados entre sedes mediante taxi corporativo o reconocimiento de combustible , según corresponda.",
    "Gratificación complementaria equivalente a un sueldo adicional , aplicable desde el segundo año completo de labores y abonada en marzo.",
    "Oportunidad de gestionar importantes cuentas de los sectores belleza, cosmética y farmacéutico."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-comercial-kam-jr.-servicios-logisticos-grafton-latam-1118430406.html"
  },
  {
   "id": 223,
   "puesto": "Operario de almacén",
   "empresa": "GRAFTON PERU S.A.C.",
   "cat": "Logística",
   "min": 1180,
   "max": 1180,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 97,
   "resumen": "Recibir la mercadería de importación, producción local, premios y catálogos, material de empaque.",
   "funciones": [
    "Recibir la mercadería de importación, producción local, premios y catálogos, material de empaque.",
    "Almacenar la mercadería procedente de la recepción y artículos excedentes de conferencia cumpliendo la política de inventarios.",
    "Surtir las líneas de picking cumpliendo los plazos establecidos de acuerdo a marca y campaña.",
    "Picar los productos utilizando el equipo de RF, cuidando el orden, presentación y estética del pedido en su armado.",
    "Otras funciones asignadas por el jefe inmediato."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 3 meses en almacén o logística.",
    "Disponibilidad para laborar en Villa el salvador.",
    "Responsabilidad, compromiso y trabajo en equipo.",
    "Disponibilidad de trabajar en horario fijo (mañana) o rotativo (mañana y noche)."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Sueldo básico: S/ 1,180.",
    "Movilidad: S/ 217.",
    "Tarjeta de alimentos: S/ 110. (Asistencia perfecta)",
    "EPS cubierta al 100%.",
    "Descanso en feriados.",
    "Capacitación constante.",
    "Excelente clima laboral.",
    "Información Adicional",
    "Modalidad: Presencial.",
    "Horario a elegir según disponibilidad",
    "Horario fijo: Lunes a viernes 07:00 am - 05:00 pm Sábado 07:00 am - 10:00 am"
   ],
   "url": "https://www.laborum.pe/job/grafton-peru-s-a-c/operario-de-almacen-villa-el-salvador-planilla-completa/6a98ce932020b210c843f62b"
  },
  {
   "id": 224,
   "puesto": "Supervisor de Limpieza",
   "empresa": "Tándem Eje de soluciones Integradas",
   "cat": "Recursos Humanos",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 97,
   "resumen": "RESUMEN: En Panorama creemos en el talento, la responsabilidad y el compromiso. Si buscas estabilidad laboral, un excelente ambiente de trabajo y la oportunidad de desarrollarte en una empresa líder, ¡esta es tu oportunidad!",
   "funciones": [
    "Supervisar y controlar la asistencia, puntualidad y desempeño del personal operativo.",
    "Verificar el correcto uso y estado de los EPP, insumos, accesorios y equipos de limpieza.",
    "Supervisar el manejo adecuado de maquinaria como aspiradoras y fregadoras de piso.",
    "Controlar el orden y limpieza de las áreas destinadas al almacenamiento de insumos y equipos.",
    "Registrar y reportar incidencias, ocurrencias, accidentes y condiciones inseguras.",
    "Elaborar reportes básicos de asistencia e insumos.",
    "Velar por la seguridad de las instalaciones, equipos y bienestar del personal.",
    "Coordinar y realizar visitas a clientes, según necesidad del servicio.",
    "Promover el cumplimiento de las normas de Seguridad y Salud en el Trabajo (SST)."
   ],
   "requisitos": [
    "Mínimo 1 año de experiencia como Supervisor(a) de Limpieza.",
    "Experiencia comprobable en servicios de limpieza y manejo de personal.",
    "Perfil con liderazgo, compromiso y capacidad para resolver incidencias.",
    "Conocimiento en manejo de maquinaria de limpieza.",
    "Disponibilidad para movilizarse a diferentes clientes cuando sea requerido."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,500.",
    "Bono de movilidad: S/ 100.",
    "Movilidad adicional por visitas a clientes.",
    "Planilla completa Régimen General.",
    "Pagos quincenales.",
    "Horario: Lunes a sábado de 7:00 a.m. a 4:00 p.m.",
    "Descanso fijo los domingos.",
    "Lugar de trabajo: Av. Los Faisanes 420, Chorrillos.",
    "¡ÚNETE A NUESTRO EQUIPO!"
   ],
   "url": "https://www.laborum.pe/job/tandem-eje-de-soluciones-integradas/supervisor-de-limpieza-sede-chorrillos-chorrillos/6a9a060a2020b210c8452dde"
  },
  {
   "id": 225,
   "puesto": "Soldador de Maquinaria Pesada",
   "empresa": "Grupo Tawa",
   "cat": "Otros",
   "min": 3800,
   "max": 4400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Puno",
   "fuente": "Bumeran",
   "dias": 23,
   "vence": "2027-01-01",
   "restan": 97,
   "score": 95,
   "resumen": "Importante empresa del sector se encuentra en la búsqueda de Soldadores de Maquinaria Pesada para integrarse a un proyecto de Conservación Vial .",
   "funciones": [
    "Realizar trabajos de soldadura en equipos de maquinaria pesada de línea amarilla y línea blanca .",
    "Ejecutar trabajos de reparación y reforzamiento de componentes estructurales de maquinaria pesada.",
    "Realizar trabajos de soldadura en volquetes, excavadoras, tractores y otros equipos .",
    "Realizar la preparación de los componentes y superficies antes de ejecutar los trabajos de soldadura.",
    "Ejecutar trabajos de soldadura de acuerdo con los procedimientos establecidos.",
    "Realizar reparaciones y refuerzos en componentes metálicos de los equipos.",
    "Cumplir con los procedimientos de seguridad establecidos para los trabajos de soldadura."
   ],
   "requisitos": [
    "Estudios técnicos en Soldadura Universal o carreras técnicas afines.",
    "Experiencia mínima de 2 años en el cargo.",
    "Experiencia realizando trabajos de soldadura en maquinaria pesada.",
    "Experiencia en reparación y reforzamiento de componentes estructurales de equipos de línea amarilla y línea blanca.",
    "Experiencia con volquetes, excavadoras, tractores u otros equipos de maquinaria pesada.",
    "Contar con homologación vigente 3G .",
    "Deseable experiencia en proyectos mineros.",
    "Disponibilidad para trabajar bajo régimen 28x7 .",
    "Contar con CV documentado.",
    "Contar con Certiadulto.",
    "Disponibilidad para validación de antecedentes."
   ],
   "beneficios": [
    "Planilla con beneficios de ley.",
    "Hospedaje durante la permanencia en el proyecto.",
    "Sueldo: S/ 3,800 a S/ 4,400 + destaque.",
    "Contrato inicial de 3 meses , renovable por el mismo periodo de acuerdo con las necesidades de la obra.",
    "Traslado desde el lugar de origen hacia el proyecto.",
    "Para personal de Lima , el traslado hacia Juliaca será vía aérea.",
    "Para personal de Puno y Cusco , el traslado podrá realizarse vía terrestre.",
    "Para otras localidades, el traslado será vía aérea o terrestre, dependiendo de la cercanía.",
    "Horario de trabajo",
    "Régimen: 28 días de trabajo x 7 días de descanso .",
    "Los 7 días de descanso no son remunerados .",
    "Lugar de trabajo: Proyecto de Conservación Vial, ubicado aproximadamente a 5 horas de Juliaca."
   ],
   "url": "https://www.bumeran.com.pe/empleos/soldador-de-maquinaria-pesada--regimen-28x7-juliaca-grupo-tawa-1118428777.html"
  },
  {
   "id": 226,
   "puesto": "Trabaja en Atención al cliente para Natura Argentina Callcenter",
   "empresa": "Atento Perú",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 92,
   "resumen": "RESUMEN: Puesto: Asesor Voz Inbound - Natura Chile (Atención al cliente)",
   "funciones": [
    "Atender consultas y/o dudas de los consultores de Natura a nivel general, como también atender reclamos, cobranzas, reprogramaciones y/o cambios o devoluciones de producto.",
    "Lunes a Viernes",
    "Sábado: 7am a 12 pm",
    "Descanso FIJO DOMINGO"
   ],
   "requisitos": [
    "Experiencia 3 meses en Call center o Atención al cliente (O puestos similares)",
    "Secundaria completa",
    "Disponibilidad para trabajar en Av. La molina 190 (A dos cuadras del óvalo de Santa Anita)",
    "Edad: >20· INICIO DE CAPACITACIÓN: 3 DE SEPTIEMBRE",
    "¡Postula hoy!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 añosConocimientos: Servicio al cliente, Atención al clientePalabras clave: care, costumer, cliente, call, callcenter, teleoperador, telefonista, telephonist, telefonico"
   ],
   "beneficios": [
    "Sueldo base: S/1130.00",
    "Maqueta hasta 20%",
    "Vale de Alimentos",
    "Línea de carrera",
    "Planilla desde el primer día (Con todos los beneficios de Ley: CTS, gratificaciones, utilidades, seguro, vacaciones Línea de carrera Pagos puntuales Asignación familiar Descuentos en marcas e instituciones educativas)",
    "Contamos con Sala de descanso, comedores, lockers, Sala de estudios, Cancha de fútbol, etc.",
    "CAPACITACIÓN: (Pagada)",
    "20 días de capacitación de Lunes a Sábado, horario (8:00 a.m. a 2:00 p.m.)",
    "10 días OJT bajo contrato"
   ],
   "url": "https://www.laborum.pe/job/atento-peru/trabaja-en-atencion-al-cliente-para-natura-argentina/6a98d3e62020b210c84405bc"
  },
  {
   "id": 227,
   "puesto": "Promotor Venta Activa Real Plaza San Miguel",
   "empresa": "CORPORACION AMGRA E.I.R.L.",
   "cat": "Ventas",
   "min": 2300,
   "max": 2300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 92,
   "resumen": "Abordamiento y prospección permanente de clientes potenciales.",
   "funciones": [
    "Abordamiento y prospección permanente de clientes potenciales.",
    "Realizar demostración de los productos y tratamientos de la marca.",
    "Cumplimiento de los objetivos de ventas trazados por sus lideres.",
    "Colaboración con todos los procesos operativos de la tienda."
   ],
   "requisitos": [
    "Tener un alto grado de resiliencia, entusiasmo y enfoque personal aspiracional.",
    "Ser seguro de sí mismo y contar con un alto nivel de autoestima.",
    "Experiencia en ventas y/o atención al cliente (no indispensable).",
    "Disponibilidad para trabajar 6 días a la semana en turnos de 8 horas operado ( Full Time) y 4 horas operando ( Part Time)",
    "La atención de tienda es de lunes a domingo y el día de descanso es entre lunes a viernes."
   ],
   "beneficios": [
    "Sueldo básico mensual seguro.",
    "Bonos y comisiones competitivas que te dan la oportunidad de duplicar (S/2,300.00) o triplicar (S/3,500.00) el sueldo básico.",
    "Excelente clima laboral y posibilidad de Crecimiento Profesional.",
    "Capacitaciones virtuales y físicas de forma permanente.",
    "Bono anual al treceavo mes de trabajo.",
    "Vacaciones después de cumplir el año laborando (15 días remunerados).-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 35 añosPalabras clave: promoter, promotor"
   ],
   "url": "https://www.laborum.pe/job/corporacion-amgra-e-i-r-l/promotor-venta-activa-real-plaza-san-miguel/6a9919db2020b210c844be1e"
  },
  {
   "id": 228,
   "puesto": "Call Center",
   "empresa": "Salesland Pacífico",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 92,
   "resumen": "RESUMEN: Asesor(a) de Call Center PORTABILIDAD MOVISTAR",
   "funciones": [
    "Realizar llamadas a clientes para ofrecer portabilidad de líneas móviles Movistar, brindando información y concretando ventas.",
    "¡POSTULA DIRECTAMENTE CONMIGO!",
    "Si estás interesado(a), postula a esta oferta y me pondré en contacto contigo para brindarte toda la información del proceso.",
    "¡Únete a Salesland Pacífico y trabaja desde la comodidad de tu hogar!· Carlos Andres Pacherres Bajonero",
    "Analista de Gestión y Atracción del Talento Humano"
   ],
   "requisitos": [
    "Secundaria completa",
    "Con experiencia en ventas o Call Center",
    "DNI o CE vigente",
    "Contar con PC o laptop y conexión estable a Internet",
    "Disponibilidad para trabajar de manera remota"
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,130",
    "Comisiones ilimitadas",
    "Bono de productividad de hasta S/ 200",
    "Capacitación pagada: S/ 120",
    "Pagos quincenales",
    "Línea de carrera y oportunidad de crecimiento",
    "Trabajo 100% remoto",
    "Lunes a sábado 7 horas diarias",
    "Descanso: domingos y feriados"
   ],
   "url": "https://www.laborum.pe/job/salesland-pacifico/trabaja-desde-casa-con-salesland-portabilidad-exp-minima/6a99020b2020b210c8448cf9"
  },
  {
   "id": 229,
   "puesto": "Asesor de Fidelización",
   "empresa": "Grupo Expro",
   "cat": "Recursos Humanos",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 91,
   "resumen": "RESUMEN: ¡Únete a nuestro gran equipo comercial!",
   "funciones": [
    "Atender llamadas de usuarios que evalúan la continuidad de sus servicios.",
    "Identificar motivos y necesidades para brindar una atención empática y consultiva.",
    "Presentar ofertas exclusivas, alternativas y promociones vigentes de Claro Chile.",
    "Negociar y aplicar soluciones comerciales para asegurar la permanencia del cliente.",
    "Registrar de forma precisa cada gestión en las plataformas correspondientes."
   ],
   "requisitos": [
    "Secundaria completa culminada (indispensable contar con certificado/documento de sustento).",
    "Experiencia mínima de 3 meses en fidelización, retenciones o cross-selling en Call Center.",
    "Orientación a resultados, capacidad de negociación y excelente comunicación.",
    "Disponibilidad para laborar en nuestra sede de Real Plaza Centro Cívico (Centro de Lima).",
    "¡Compromiso, actitud positiva y ganas de estabilidad laboral!",
    "JORNADA LABORAL",
    "48 horas semanales",
    "Franja horaria: 7:00 a.m. a 6:00 p.m.",
    "¡2 días de descanso rotativo a la semana!"
   ],
   "beneficios": [
    "Planilla completa desde el primer día con todos los beneficios de ley (Gratificación, CTS, Utilidades, Vacaciones pagadas, Asignación Familiar, EsSalud, etc.).",
    "Sueldo básico: S/ 1,200 fijo.",
    "Variable por indicadores: hasta S/ 250 adicionales.",
    "Bono de contratación: S/ 150 (50% en el 1.º mes y 50% en el 2.º mes).",
    "Oportunidades reales de línea de carrera y desarrollo interno.",
    "Excelente clima laboral e instalaciones céntricas y accesibles.",
    "Hoy te elegimos a través de un proceso de selección; nuestra apuesta es que al final del día seas tú quien nos elija para crecer juntos.",
    "¡Postula ahora y asegura tu ingreso!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: advisor, asesor, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/grupo-expro/asesor-de-fidelizacion-cliente-claro-2-dias-de-descanso/6a991e2b2020b210c844cb8d"
  },
  {
   "id": 230,
   "puesto": "Turno mañana Anfitrión de Estacionamiento en Wong Chacarilla",
   "empresa": "APPARKA",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 91,
   "resumen": "RESUMEN: En Apparka nos encontramos en búsqueda del mejor talento para el puesto de Anfitrión de Estacionamientos.",
   "funciones": [
    "Brindar una excelente atención a nuestros clientes, asegurando una experiencia agradable desde su llegada.",
    "Facilitar la entrada y salida de los vehículos en nuestras instalaciones.",
    "Colaborar con un equipo comprometido en garantizar la seguridad y el buen funcionamiento de cada estacionamiento.",
    "Utilizar nuestras herramientas tecnológicas para hacer el trabajo más eficiente.",
    "Disponibilidad para laborar en el turno de: 7:00 am - 3:45 pm con Descanso un día a la semana."
   ],
   "requisitos": [
    "Personas con buena actitud, orientadas al servicio y dispuestas a aprender.",
    "Experiencia previa en atención al cliente (no es indispensable, ¡nosotros te capacitamos!).",
    "Disponibilidad para trabajar en turnos rotativos.",
    "Compromiso con el trabajo en equipo y la puntualidad."
   ],
   "beneficios": [
    "Sueldo: S/. 1130 mensual + PLANILLA COMPLETA",
    "Ingreso a planilla completa desde el primer día.",
    "¡Oportunidad de línea de carrera a corto plazo!",
    "Capacitaciones constantes."
   ],
   "url": "https://www.laborum.pe/job/apparka/turno-manana-pagos-quincenales-anfitrion-de-estacionamiento/6a991e172020b210c844cb4a"
  },
  {
   "id": 231,
   "puesto": "Auxiliar de limpieza",
   "empresa": "RETAIL SERVICES SAC",
   "cat": "Otros",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 91,
   "resumen": "En Retail Services buscamos a una persona para el puesto Auxiliar de Limpieza. Te invitamos a postular y ser parte de nuestro equipo en el restaurante KION en la Mar Miraflores.",
   "funciones": [
    "Limpiar y desinfectar las áreas asignadas.",
    "Organizar el área de lavado, ordenando la batería del bar.",
    "Apoyar al personal de cocina a mantener sus áreas de trabajo siempre ordenadas.",
    "Apoyar en la limpieza del local en general.",
    "HORARIOS ROTATIVOS (8 horas) 1 día de descanso a la semana.",
    "Sueldo básico: S/1130.00 + Recargo de consumo: S/300 + Propinas mensuales"
   ],
   "requisitos": [
    "Experiencia en la posición.",
    "Disponibilidad para trabajar en turnos rotativos.",
    "Disponibilidad para trabajar en Miraflores."
   ],
   "beneficios": [
    "Descuentos corporativos",
    "Alimentación cubierta",
    "Planilla General desde el primer día de trabajo con todo los beneficios de ley al 100%. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 22 y 36 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide"
   ],
   "url": "https://www.laborum.pe/job/retail-services-sac/auxiliar-de-limpieza-vajillero-kion-peruvian-chinese/6a9929022020b210c844e568"
  },
  {
   "id": 232,
   "puesto": "Primax despachador de combustible",
   "empresa": "COESTI S.A.",
   "cat": "Recursos Humanos",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 91,
   "resumen": "RESUMEN: ¡Trabaja en Primax La Encantada de Chorrillos!",
   "funciones": [
    "Atención al cliente y despacho de combustible.",
    "Manejo de vouchers, vales y efectivo.",
    "Realizar cuadre de caja y reporte de ventas al finalizar el turno.",
    "Mantener el orden y limpieza de la estación.",
    "Cumplir con las normas de seguridad y salud en el trabajo."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia en atención al cliente (deseable).",
    "Manejo de caja (deseable).",
    "Disponibilidad para laborar en turnos de 2 pm a 10 pm"
   ],
   "beneficios": [
    "Sueldo aproximado de S/1200.",
    "Ingreso a planilla desde el primer día.",
    "Asignación familiar.",
    "Bonos por ventas.",
    "¡Únete a nuestro gran equipo!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: controlador, despachador, dispatcher, nominas, sueldo, paga, retribucion, salary, payslip, payroll, wage, pay, payment, nominista"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/trabaja-cerca-a-los-cedros-de-villa-chorrillos-primax/6a991b592020b210c844c293"
  },
  {
   "id": 233,
   "puesto": "Asistente de Tienda Junior",
   "empresa": "NEW ATHLETIC",
   "cat": "Ventas",
   "min": 1180,
   "max": 1180,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 90,
   "resumen": "Buscamos una persona dinámica, responsable y con interés en desarrollarse en el área comercial y de gestión de tiendas.",
   "funciones": [
    "Brindar apoyo en la gestión y operación diaria de la tienda.",
    "Apoyar en la supervisión y orientación del equipo para el cumplimiento de los objetivos de venta.",
    "Brindar atención y asesoría a los clientes, promoviendo la venta adicional y venta cruzada.",
    "Supervisar el orden, exhibición, reposición y correcta presentación de los productos."
   ],
   "requisitos": [
    "Experiencia mínima de 6 meses en ventas, atención al cliente o retail.",
    "Deseable experiencia previa como encargado(a), líder o apoyo en la supervisión de tienda.",
    "Conocimientos básicos de ventas, atención al cliente y manejo de equipos.",
    "Disponibilidad para laborar de lunes a domingo en horarios rotativos, con 1 día de descanso."
   ],
   "beneficios": [
    "Sueldo: S/ 1,180 + comisiones por cumplimiento de metas.",
    "Planilla completa desde el primer día.",
    "Todos los beneficios de ley.",
    "Descuentos corporativos de 30% hasta 70% en productos New Athletic.",
    "Oportunidad de crecimiento y línea de carrera.",
    "¡Esta puede ser tu oportunidad para crecer y desarrollarte junto a New Athletic!"
   ],
   "url": "https://www.laborum.pe/job/new-athletic/asistente-de-tienda-junior-megaplaza/6a98c0662020b210c843c98a"
  },
  {
   "id": 234,
   "puesto": "Promotor de Ruta Sodimac y Promart",
   "empresa": "Salesland Pacífico",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 90,
   "resumen": "Dirigirse a la tienda asignada (previamente coordinado con supervisor)",
   "funciones": [
    "Dirigirse a la tienda asignada (previamente coordinado con supervisor)",
    "Asesoramiento de los productos de la marca a promotores y clientes.",
    "Realizar la capacitación a vendedores, clientes finales y público en general, brindando asesoramiento técnico (muestras).",
    "Incentivar la venta de pegamentos, aditivos y drywalls."
   ],
   "requisitos": [
    "Contar con experiencia en ventas retail y/o del rubro ferretero.",
    "Disponibilidad para MOVILIZARSE ENTRE LAS SUCURSALES DE Sodimac y Promart San Miguel, Promart Salaverry, Promart Brasil y Sodimac Mega Plaza.",
    "Disponibilidad para trabajar en el horario fijo de LUNES A SÁBADOS de 8AM A 5PM Ó 9AM - 6PM (descanso fijo domingos)"
   ],
   "beneficios": [
    "S/1130 fijo + S/250 movilidad fija + S/30 de línea telefónica + variable de hasta S/200",
    "PLANILLA COMPLETA desde el 1er día de gestión con todos los beneficios de ley (Seguro, 14 sueldos al año, asignación familiar, CTS, utilidades, gratificaciones, vacaciones pagadas, etc.)",
    "Línea de carrera y excelente clima laboral",
    "Contratación INMEDIATA. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: promoter, promotor"
   ],
   "url": "https://www.laborum.pe/job/salesland-pacifico/promotor-de-ruta-sodimac-y-promart-norte-y-centro-rubro/6a98ded02020b210c8442539"
  },
  {
   "id": 235,
   "puesto": "Zona Este! Coordinador de Trade Marketing",
   "empresa": "Overall Strategy",
   "cat": "Marketing",
   "min": 2750,
   "max": 2750,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 89,
   "resumen": "RESUMEN: ¡Estamos en búsqueda de un Coordinador(a) de Trade Ejecución!",
   "funciones": [
    "Supervisar la correcta ejecución de la marca en los puntos de venta.",
    "Dar seguimiento a los indicadores (KPIs) y proponer mejoras.",
    "Implementar y monitorear los planes de trabajo del canal asignado.",
    "Capacitar y acompañar a mercaderistas y vendedores.",
    "Supervisar el uso de herramientas digitales de gestión.",
    "Controlar y distribuir el material POP en los puntos de venta.",
    "Elaborar reportes e indicadores de gestión."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios (egresado o bachiller) en Administración, Marketing, Ingeniería Industrial, Negocios Internacionales o carreras afines.",
    "Experiencia mínima de 1 año como Supervisor(a) de Ventas, Trade Marketing o puestos similares.",
    "Microsoft Excel y Office a nivel intermedio.",
    "Disponibilidad para realizar 70% trabajo de campo y 30% administrativo.",
    "Liderazgo, comunicación efectiva y orientación a resultados."
   ],
   "beneficios": [
    "Ingreso total de hasta S/ 5,500",
    "Sueldo básico: S/ 2,750",
    "Remuneración variable: Hasta S/ 2,750",
    "Planilla completa con todos los beneficios de ley.",
    "Lugar de trabajo: Lima ESTE (Santa Clara y San Juan de Lurigancho)· Horario",
    "Lunes a sábado de 7:00 a.m. a 5:30 p.m. (1 hora de refrigerio).-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 19 y 50 añosPalabras clave: area, residente, encargado, supervisor, subgerente, responsable, coordinador, gestor, capitan, comercio, mercadotecnia, marketing, mercadeo"
   ],
   "url": "https://www.laborum.pe/job/overall-strategy/zona-este-coordinador-de-trade-marketing-canal-tradicional/6a9922832020b210c844d832"
  },
  {
   "id": 236,
   "puesto": "Supervisor Jr de Planta",
   "empresa": "Salesland Pacífico",
   "cat": "Ingeniería",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 89,
   "resumen": "RESUMEN: SUPERVISOR(A) JR DE PLANTA PRODUCCIÓN· En Salesland encontramos en la búsqueda de un(a) Supervisor(a) Jr de Planta Producción",
   "funciones": [
    "Supervisar y verificar el trabajo de los capataces y operarios, asegurando el correcto desarrollo de las actividades en planta.",
    "Garantizar el cumplimiento del programa de producción, realizando seguimiento a las metas establecidas por turno.",
    "Velar por el cumplimiento de las normas de seguridad, calidad y reglamento interno, promoviendo la prevención de accidentes y la disciplina operativa.",
    "Controlar y dar seguimiento a las órdenes de mantenimiento de máquinas y herramental, contribuyendo a la disponibilidad y continuidad operativa de los equipos.",
    "Reportar los avances, resultados y cuadres de las órdenes de producción, brindando información oportuna para la toma de decisiones.",
    "Coordinar los traslados, servicios externos y demás requerimientos logísticos, asegurando la continuidad de las operaciones.",
    "Participar en planes de mejora continua y en la implementación de mejoras en los procesos operativos.",
    "Supervisar los turnos asignados y brindar soporte en la difusión y aplicación del Sistema de Gestión Integrado (SGI).",
    "Participar activamente en comités, actividades administrativas y otras tareas relacionadas con el puesto, contribuyendo al cumplimiento de los objetivos organizacionales."
   ],
   "requisitos": [
    "Técnico o universitario egresado de Ingeniería Industrial, Ingeniería Mecánica, Producción o carreras afines.",
    "Mínimo 1 año de experiencia en entornos industriales o puestos de supervisión básica.",
    "Conocimiento de procesos productivos.",
    "Experiencia en manejo de personal.",
    "Conocimiento de normas de seguridad.",
    "Control y seguimiento de órdenes de producción (OP).",
    "Registro y manejo de documentación técnica.",
    "Conocimiento básico en mantenimiento preventivo y correctivo.",
    "Familiaridad con Sistemas de Gestión Integrados (SGI).",
    "Manejo de Microsoft Office: Excel, Word y PowerPoint."
   ],
   "beneficios": [
    "Sueldo básico: S/ 2,000 mensuales.",
    "Ingreso a planilla con todos los beneficios de ley.",
    "Contrato por obra determinada o servicio específico.",
    "Oportunidad de desarrollo profesional.",
    "Primer turno",
    "Lunes a viernes de 7:00 a. m. a 4:15 p. m.",
    "Sábados de 7:00 a. m. a 12:30 p. m.",
    "Se requiere disponibilidad para realizar turno noche, de acuerdo con las necesidades de la operación.",
    "LUGAR DE TRABAJO",
    "El puesto contempla la rotación entre las siguientes sedes ubicadas en Ate",
    "René Descartes 146, Ate."
   ],
   "url": "https://www.laborum.pe/job/salesland-pacifico/supervisor-jr-de-planta-produccion/6a98f0242020b210c8445a5f"
  },
  {
   "id": 237,
   "puesto": "Cajero(a) de Estacionamiento",
   "empresa": "NEW ATHLETIC",
   "cat": "Contabilidad",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 89,
   "resumen": "RESUMEN: NEW CAR, empresa perteneciente al grupo NEW ATHLETIC, se encuentra en búsqueda de CAJERO(A) DE ESTACIONAMIENTO para nuestra sede ubicada en Jr. Unión 1080 Cercado de Lima.",
   "funciones": [
    "Realizar el cobro y registro de los servicios del estacionamiento.",
    "Controlar el ingreso y salida de vehículos.",
    "Realizar el cuadre y control de caja.",
    "Brindar atención y orientación a los clientes."
   ],
   "requisitos": [
    "Mínimo 6 meses de experiencia en caja,cobranza o atención al cliente.",
    "Experiencia en manejo de dinero y medios de pago.",
    "Disponibilidad completa.",
    "Disponibilidad para laborar en Jr. Unión 1080 Cercado de Lima."
   ],
   "beneficios": [
    "S/ 1,130 + pago de horas extras.",
    "Planilla completa desde el primer día.",
    "Todos los beneficios de ley.",
    "Descuentos corporativos en productos New Athletic.",
    "Oportunidad de crecimiento y capacitación.-Requerimientos- Educación mínima: Educación Secundaria años de experiencia"
   ],
   "url": "https://www.laborum.pe/job/new-athletic/cajero-a-de-estacionamiento-cercado-de-lima/6a9917802020b210c844b678"
  },
  {
   "id": 238,
   "puesto": "Ejecutivo(a) de Atención al Cliente",
   "empresa": "Entel Connect Center",
   "cat": "Atención al Cliente",
   "min": 1350,
   "max": 1350,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 89,
   "resumen": "RESUMEN: Nos encontramos en búsqueda de personas comprometidas, responsables y con actitud de servicio para el puesto de Ejecutivos(as) de Atención al Cliente para formar parte de SKY.",
   "funciones": [
    "Atención solamente por chat.",
    "Atender consultas, solicitudes y requerimientos de los clientes.",
    "Brindar una atención cordial, eficiente y de calidad.",
    "Orientar a los clientes de acuerdo con sus necesidades.",
    "Cumplir con los protocolos y procedimientos establecidos para la atención."
   ],
   "requisitos": [
    "Experiencia en atención al cliente, call center o puestos similares.",
    "Orientación al cliente y vocación de servicio.",
    "Disponibilidad para trabajar de manera presencial en Cercado de Lima (Real Plaza del Centro Cívico).",
    "Disponibilidad para laborar 6 días a la semana.",
    "¡POSTULA Y FORMA PARTE DE NUESTRO EQUIPO!. -Requerimientos- Educación mínima: Técnico años de experienciaEdad: A partir de 18 añosPalabras clave: care, costumer, cliente"
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,350",
    "Bono de movilidad: S/ 150",
    "Todos los beneficios de acuerdo a ley.",
    "Trabajo presencial.",
    "Jornada laboral de 48 horas semanales + refrigerio diario.",
    "6 días de trabajo a la semana y 1 día de descanso rotativo.",
    "Rango horario de la jornada 12PM. - 10PM. (8h + refrigerio)"
   ],
   "url": "https://www.laborum.pe/job/entel-connect-center/consigue-s-1350-bono-ejecutivo-a-de-atencion-al-cliente/6a9a03ef2020b210c84523be"
  },
  {
   "id": 239,
   "puesto": "Asesores Ventas Call Center",
   "empresa": "Nexxo Servicios Comerciales",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 87,
   "resumen": "RESUMEN: Grupo Empresarial líder en el Sector de las Telecomunicaciones, se encuentra en la búsqueda de ASESORES DE VENTAS, para nuestro CALL CENTER MOVISTAR - LOS OLIVOS.",
   "funciones": [
    "Realizar llamadas para la campaña de Portabilidad - Movistar.",
    "Participar activamente en las Campañas y Promociones de los productos del negocio, realizando el seguimiento de la gestión comercial para el cierre de venta del cliente.",
    "Entre otras funciones delegadas por el jefe inmediato."
   ],
   "requisitos": [
    "Estudiantes en curso y/o Truncos de las carreras de Administración, Ciencias de la Comunicación, Marketing, Psicología y otras afines.",
    "Experiencia mínima de 6 meses en VENTAS POR CALL CENTER (Telecomunicaciones - Deseable).",
    "Conocimiento de Office (Nivel básico).",
    "Proactivo, empático, carismático.",
    "Secundaria completa.",
    "Deseable experiencia en venta (Nosotros te capacitamos).",
    "Disponibilidad inmediata para laborar en CC Plaza Norte 3er piso Oficina Movistar."
   ],
   "beneficios": [
    "Trabaja sólo 43 horas semanales, de lunes a viernes de 8:30 a.m. a 6:00 p.m.",
    "Sueldo S/ 1130 - Planilla completa.",
    "Comisiones ilimitadas.",
    "Comisión de bienvenida de S/300, por los primeros 3 meses.",
    "Capacitaciones pagadas."
   ],
   "url": "https://www.laborum.pe/job/nexxo-servicios-comerciales/asesores-ventas-call-center-planilla-completa-lunes-a/6a9925442020b210c844de7c"
  },
  {
   "id": 240,
   "puesto": "Atención al Cliente",
   "empresa": "Nexxo Servicios Comerciales",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 83,
   "resumen": "RESUMEN: Grupo Empresarial líder en el Sector de las Telecomunicaciones, se encuentra en la búsqueda de Ejecutivo/a de Atención al Cliente y Ventas, para laborar en nuestra tienda de Movistar del CC RAMBLA BREÑA.",
   "funciones": [
    "Atención y venta directa a clientes.",
    "Atender los reclamos de los clientes.",
    "Entre otras funciones delegadas por el jefe inmediato."
   ],
   "requisitos": [
    "Estudiantes en curso y/o Truncos de las carreras de Administración, Ciencias de la Comunicación, Marketing, Psicología y otras afines.",
    "Experiencia mínima de 6 meses en Atención al cliente o ventas presenciales o Call center.",
    "Conocimiento de Office (Nivel básico).",
    "Proactivo, empático, carismático, capacidad de liderazgo y negociación."
   ],
   "beneficios": [
    "Sueldo básico: S/. 1130 + comisiones ilimitadas + Incentivos de Marcas + Bono de Asistencia 300 soles",
    "Planilla desde primer día de trabajo",
    "Capacitaciones pagadas.",
    "Desarrollo Profesional y Línea de carrera.",
    "Incorporarse a Institución Líder en el Sector de las Telecomunicaciones."
   ],
   "url": "https://www.laborum.pe/job/nexxo-servicios-comerciales/rambla-brena-atencion-al-cliente-y-ventas-s-1130-mas/6a98efb22020b210c84458b6"
  },
  {
   "id": 241,
   "puesto": "Asesor Comercial Corporativo",
   "empresa": "Automatizate Negocios",
   "cat": "Ventas",
   "min": 10000,
   "max": 10000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 23,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 83,
   "resumen": "En Automatízate Negocios impulsamos el crecimiento de negocios a través de soluciones en medios de pago (POS) y telecomunicaciones, con presencia nacional y una sólida estructura de desarrollo comercial.",
   "funciones": [
    "Prospección y cierre de ventas de equipos POS.",
    "Gestión de clientes con transacciones +S/10,000 mensuales.",
    "Seguimiento postventa durante 3 meses."
   ],
   "requisitos": [
    "Experiencia en ventas corporativas (B2B).",
    "Deseable experiencia en POS, medios de pago, telecomunicaciones, banca, seguros o servicios empresariales.",
    "Habilidades de comunicación, negociación y enfoque en resultados.",
    "Contar con celular y laptop o tablet. (deseable)"
   ],
   "beneficios": [
    "Sueldo fijo (planilla desde el 1er día).",
    "Movilidad hasta S/500 (según indicadores).",
    "Comisiones altas (hasta S/10,000) + incentivos semestrales.",
    "Capacitaciones pagadas, convenios corporativos y línea de carrera.",
    "Lunes a viernes | 8:00 a.m. a 7:00 p.m.",
    "Promovemos la inclusión y la equidad de género, ofreciendo las mismas oportunidades para hombres y mujeres. Además, brindamos empleo a nivel nacional, apoyando el desarrollo de talento en todo el país.",
    "¡Si compartes nuestra visión, postúlate ahora y únete a nuestro equipo!. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 18 y 65 añosPalabras clave: advisor, asesor, salesman, saleswoman, seller, vendedor, comercial, salesperson, corporate, empresarial, corporativo"
   ],
   "url": "https://www.laborum.pe/job/automatizate-negocios/asesor-comercial-corporativo-culqi-bcp-crecimiento-ingresos/6a98b8f12020b210c843ab1c"
  },
  {
   "id": 242,
   "puesto": "Atencion al Cliente en Grifo Primax de Paseo de la Republica",
   "empresa": "COESTI S.A.",
   "cat": "Atención al Cliente",
   "min": 1230,
   "max": 1230,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 97,
   "resumen": "Si quieres pertenecer al Grupo Primax y formar parte de un equipo lleno de energía y pasión te invitamos a postular a la posición de DESPACHO DE COMBUSTIBLE FULL TIME para que cumplas las siguientes funciones:",
   "funciones": [
    "Atención al clientes de manera oportuna en despachando combustible",
    "Controlar los Boucher, vales y efectivo de las ventas de los dispensadores que le son asignados",
    "Realizar el reporte de ventas al término del turno",
    "Cumplir con las normas de seguridad y salud en el trabajo",
    "Mantener la EESS limpia y ordenada"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Deseable experiencia en atención al cliente",
    "Deseable manejo de caja",
    "Disponibilidad para trabajar en el mañana de 2 pm a 10 pm",
    "Disponibilidad para laborar en de San Isidro, al costado de la torre de RPP, frente a la Estacion Aramburu del Metropolitano"
   ],
   "beneficios": [
    "Ingreso a planilla",
    "Contrato indeterminado",
    "Sueldo aproximado de S/1230",
    "Asignación familiar",
    "Bonos por ventas",
    "Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/atencion-al-cliente-en-grifo-primax-de-paseo-de-la-republica/6a979de42020b210c84285d4"
  },
  {
   "id": 243,
   "puesto": "Atencion al Cliente Grifo Primax cerca de la Municipalidad de San",
   "empresa": "COESTI S.A.",
   "cat": "Atención al Cliente",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 97,
   "resumen": "Si quieres pertenecer al Grupo Primax y formar parte de un equipo lleno de energía y pasión te invitamos a postular a la posición de DESPACHO DE COMBUSTIBLE FULL TIME para que cumplas las siguientes",
   "funciones": [
    "Atención al clientes de manera oportuna en despachando combustible",
    "Controlar los Boucher, vales y efectivo de las ventas de los dispensadores que le son asignados",
    "Realizar el reporte de ventas al término del turno - Cuadre de caja",
    "Cumplir con las normas de seguridad y salud en el trabajo",
    "Mantener la EESS limpia y ordenada"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Experiencia en atención al cliente, deseable",
    "Manejo de caja (deseable)",
    "Disponibilidad para trabajar en el turno tarde de 2 pm a 10 pm",
    "Disponibilidad para laborar en San Miguel"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer dia",
    "Contrato indeterminado",
    "Sueldo aproximado de S/1130",
    "Asignación familiar",
    "Bonos por ventas",
    "Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/atencion-al-cliente-grifo-primax-cerca-de-la-municipalidad/6a98b45a2020b210c843955c"
  },
  {
   "id": 244,
   "puesto": "Chofer A1",
   "empresa": "APPARKA",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 97,
   "resumen": "RESUMEN: Nos encontramos en búsqueda de un Conductor A1 para la posición de Valet Parking, para nuestra sede ubicada en Juan de Arona II San Isidro.",
   "funciones": [
    "Recepcionar, trasladar y estacionar los vehículos de manera segura.",
    "Verificar y registrar las condiciones en las que ingresan los vehículos.",
    "Entregar los vehículos a los clientes en las mismas condiciones en las que fueron recibidos.",
    "Registrar y resguardar los objetos de valor dejados en los vehículos, siguiendo los procedimientos establecidos.",
    "Brindar una atención cordial y una adecuada bienvenida a los clientes.",
    "Agradecer a los clientes por su visita.",
    "Informar y reportar al supervisor cualquier incidencia o anomalía identificada durante el servicio."
   ],
   "requisitos": [
    "Licencia de conducir A1 vigente.",
    "Disponibilidad para trabajar a tiempo completo.",
    "Disponibilidad para realizar horas extras.",
    "Experiencia en conducción de vehículos mecánicos y automáticos.",
    "Experiencia en atención al cliente, de preferencia.",
    "Experiencia en manejo de caja, de preferencia.",
    "Tener 23 años a más."
   ],
   "beneficios": [
    "Sueldo mensual de S/ 1,130 + bono de S/ 200 + propinas.",
    "Ingreso a planilla desde el primer día de labores.",
    "Todos los beneficios de ley: gratificaciones, vacaciones, asignación familiar, seguro, CTS, liquidación y utilidades.",
    "Uniforme de trabajo proporcionado por la empresa.",
    "Recuerda que, si cuentas con alguna discapacidad, ¡no te límites y anímate a postular! ¡Garantizamos un proceso justo y transparente, velando por la igualdad de tus derechos! (Ley General de la Persona con Discapacidad - Ley N° 29973, reglamento N°107-2015-MIMP y normas complementarias).",
    "¡Únete a nuestro equipo y haz la diferencia desde donde estés!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: conductor, driver, choferes, chofer"
   ],
   "url": "https://www.laborum.pe/job/apparka/chofer-a1-valet-parking-juan-de-arona-san-isidro-8am-a-4/6a977e812020b210c84212ab"
  },
  {
   "id": 245,
   "puesto": "Atención al cliente",
   "empresa": "COESTI S.A.",
   "cat": "Atención al Cliente",
   "min": 1230,
   "max": 1230,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 97,
   "resumen": "Si quieres pertenecer al Grupo Primax y formar parte de un equipo lleno de energía y pasión te invitamos a postular a la posición de DESPACHO DE COMBUSTIBLE para que cumplas las siguientes",
   "funciones": [
    "Atención al clientes de manera oportuna en despachando combustible",
    "Controlar los Boucher, vales y efectivo de las ventas de los dispensadores que le son asignados",
    "Realizar el reporte de ventas al término del turno - Cuadre de caja",
    "Cumplir con las normas de seguridad y salud en el trabajo",
    "Mantener la EESS limpia y ordenada"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Deseable experiencia en atención al cliente",
    "Manejo de caja, deseable",
    "Disponibilidad para laborar turno tarde de 2 pm a 10 pm",
    "Disponibilidad para laborar en Miraflores, en Ricardo Palma"
   ],
   "beneficios": [
    "Ingreso a planilla con contrato indeterminado",
    "Sueldo aproximado de S/1230 en Full Time",
    "Asignación familiar",
    "Bonos por ventas",
    "Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/atencion-al-cliente-grifo-primax-en-miraflores-bono-de-s-100/6a98b5312020b210c843993c"
  },
  {
   "id": 246,
   "puesto": "Supervisor del área",
   "empresa": "Gestión del talento Humano",
   "cat": "Logística",
   "min": 1205,
   "max": 1350,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 24,
   "vence": "2026-11-01",
   "restan": 36,
   "score": 95,
   "resumen": "Importante empresa del rubro de alimentos y productos cárnicos se encuentra en búsqueda de personal para cubrir diferentes posiciones en Callao:",
   "funciones": [
    "Entregar productos cárnicos a tiendas, restaurantes y otros clientes.",
    "Cargar y trasladar productos en jabas de hasta 6 kg.",
    "Apoyar en la devolución y cambio de productos cuando corresponda.",
    "Mantener comunicación con el supervisor del área.",
    "Realizar actividades relacionadas con la distribución y despacho.",
    "Horario referencial: 5:00 a. m. a 1:00 p. m., con disponibilidad para realizar horas extras según ruta.",
    "OPERARIO DE CÁMARA DE FRÍO",
    "Descargar productos del almacén.",
    "Almacenar y acomodar productos dentro de la cámara.",
    "Realizar picking de productos.",
    "Preparar la mercadería para despacho y distribución.",
    "Cargar productos en los vehículos asignados."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Disponibilidad para trabajar en Callao.",
    "Disponibilidad para trabajar de lunes a domingo con un día de descanso rotativo, según la posición.",
    "Disponibilidad para realizar horas extras.",
    "Disponibilidad para trabajar en horarios establecidos por la operación.",
    "Experiencia mínima de 1 a 3 meses, según la posición.",
    "Para cámara de frío: experiencia previa en ambientes de frío, almacén, picking, pesquera, puerto u operaciones similares.",
    "Para reparto: experiencia en distribución, logística, reparto o almacén.",
    "Para estiba: experiencia en labores operativas que impliquen esfuerzo físico.",
    "LUGAR DE TRABAJO",
    "Callao – Bellavista / Av. Argentina, según la posición.",
    "Si cumples con los requisitos y estás interesado en formar parte de una empresa líder del rubro de alimentos, postula indicando la posición de tu interés."
   ],
   "beneficios": [
    "Ingreso directo a planilla desde el primer día.",
    "Todos los beneficios de ley.",
    "Sueldo desde S/ 1,205 hasta S/ 1,350, según la posición y antigüedad.",
    "Incrementos salariales de acuerdo con la posición.",
    "Horas extras remuneradas.",
    "EPP completo proporcionado por la empresa.",
    "Alimentación cubierta por la empresa, según la posición.",
    "Línea de carrera.",
    "Pagos quincenales y a fin de mes, según la posición."
   ],
   "url": "https://www.bumeran.com.pe/empleos/operarios-reparto-camara-de-frio-gestion-del-talento-humano-1118427069.html"
  },
  {
   "id": 247,
   "puesto": "Promotor de ventas",
   "empresa": "Grupo Tawa",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 95,
   "resumen": "RESUMEN: Si te apasionan los retos y estar en la cancha ¡Este trabajo es para ti!",
   "funciones": [
    "Alcanzar los objetivos establecidas por tienda.",
    "Promover activamente los productos orientando en base a las necesidades del cliente final.",
    "Demostrar el uso adecuado de los productos y responder a las preguntas de los clientes.",
    "Asesorar a los clientes sobre las características, beneficios y especificaciones técnicas de los electrodomésticos.",
    "Lunes a jueves de 2:00 pm a 9:00 pm y viernes a domingo de 11:00 am a 9:00 pm",
    "Descanso un día a la semana."
   ],
   "requisitos": [
    "Secundaria completa",
    "Tengo 3 meses de experiencia en ventas, impulso, promotoría o mercaderismo.",
    "Excelente habilidad de comunicación y capacidad para establecer relaciones con los clientes.",
    "Disponibilidad de laborar en las tiendas mencionadas.",
    "¡Te esperamos!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 20 y 65 añosPalabras clave: promoter, promotor, cambaceo, sales"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día de trabajo, con todos los beneficios de ley.",
    "Combo de fidelización valorizado en S/600 2 veces al año.",
    "Remuneración Fija S/ 1130",
    "Bono de Movilidad S/ 100",
    "Comisiones hasta los S/500"
   ],
   "url": "https://www.laborum.pe/job/grupo-tawa/promotor-de-ventas-con-o-sin-exp-tottus-begonias-con-tottus/6a9790cb2020b210c842565f"
  },
  {
   "id": 248,
   "puesto": "Vendedora de Tienda",
   "empresa": "Koketa",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 94,
   "resumen": "RESUMEN: Importante empresa del sector textil con mas de 36 años de experiencia en el rubro, se encuentra en búsqueda del mejor talento para ocupar el puesto de:",
   "funciones": [
    "Ofrecer los productos de la marca y orientar al cliente en su compra.",
    "Promover la venta diaria de nuestros productos.",
    "Ordenar la mercadería en el Punto de Venta.",
    "Hacer sugeridos de unidades para las reposiciones.",
    "Recibir y revisar la mercadería que ingresa.",
    "Manejo de caja y sistema de facturación.",
    "Llevar un control de ventas y hacer cierre de caja."
   ],
   "requisitos": [
    "Disponibilidad inmediata",
    "Secundaria completa",
    "Experiencia en ventas mínimo 6 meses en centros comerciales (Indispensable)",
    "Manejo de caja y POS (Deseable)",
    "Manejo de Office nivel básico",
    "Disponibilidad para trabajar a tiempo completo."
   ],
   "beneficios": [
    "Sueldo fijo S/1130 + Comisiones",
    "Planilla al 100% desde el primer día.",
    "Descuentos en la marca",
    "Capacitaciones constantes.",
    "Horario de trabajo",
    "Lunes a Domingos: 10: 00 am - 07:00 pm / 02:00 pm - 09:00 pm (horario rotativo)",
    "1 día de descanso.-Requerimientos- Educación mínima: Educación Secundaria años de experiencia"
   ],
   "url": "https://www.laborum.pe/job/koketa/vendedora-de-tienda-lima-norte/6a977a182020b210c842017d"
  },
  {
   "id": 249,
   "puesto": "Operario Metalúrgico",
   "empresa": "Grupo Tawa",
   "cat": "Ingeniería",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 94,
   "resumen": "RESUMEN: Importante empresa industrial del rubro metalúrgico, Transformadora de Metales Lima S.A.C. (TM LIMA), dedicada a la producción de alambrón de cobre, se encuentra en búsqueda de Operarios de Colada para su planta en Ate.",
   "funciones": [
    "Apoyar la operación del área de colada según indicaciones del Supervisor.",
    "Abastecer y trasladar materiales (grafito, alambrón e insumos).",
    "Apoyar en carga y descarga de materiales.",
    "Realizar inspección del área y reportar anomalías.",
    "Ejecutar tareas de orden y limpieza (5S).",
    "Registrar información del turno (checklist, incidencias y producción).",
    "Otras funciones asignadas por jefatura."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 1 año y medio a 3 años en producción industrial.",
    "Experiencia en fundición, metalmecánica, plásticos industriales, tuberías o procesos de alta temperatura.",
    "Deseable experiencia en máquinas automatizadas.",
    "Disponibilidad para laborar en horarios rotativos.",
    "Disponibilidad para trabajar presencial en Ate.",
    "No presentar vértigo (trabajo en altura y plataformas).",
    "Disponibilidad inmediata."
   ],
   "beneficios": [
    "Turnos rotativos Lunes a Domingo: 6:30 a.m. 3:15 p.m. / 3:15 p.m. 10:00 p.m. /9:55 p.m. 6:40 a.m.",
    "Posibilidad de turnos de 12 horas según producción.",
    "Cambio de turno semanal o quincenal.",
    "Trabajo con exposición al calor y esfuerzo físico.",
    "Ingreso a planilla desde el primer día (Régimen General).",
    "Sueldo: S/ 1,600 mensuales (pagos mensuales BCP).",
    "Bono trimestral por desempeño (hasta S/ 375).",
    "Tarjeta de alimentos (S/ 10 por día laborado primeras 2 semanas del mes).",
    "EPS cubierta al 100% a partir del segundo mes.",
    "Seguro SCTR.",
    "Pago de horas extras.",
    "Comedor y concesionaria disponible en planta."
   ],
   "url": "https://www.laborum.pe/job/grupo-tawa/operario-metalurgico-ate-ate/6a97a88b2020b210c842acf4"
  },
  {
   "id": 250,
   "puesto": "Atencion al cliente",
   "empresa": "IBR Perú S.A.",
   "cat": "Recursos Humanos",
   "min": 725,
   "max": 725,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 94,
   "resumen": "de Ejecutivo(a) de atencion al cliente NUAM CHILE · ¿A QUIÉN BUSCAMOS?",
   "funciones": [
    ".Atender llamadas y correos de los inversionistas y accionistas de la Bolsa de valores de chile · * *Modalidad: * presencial por los primeros 10 dias en SANTA ANITA luego pasaras a modalidad remota",
    "Nosotros te brindamos PC + Headphone Indispensable contar con Internet cableado > a 50 mb· Horario de trabajo",
    "Horario : PART TIME",
    "LUNES A MIERCOLES 9:00AM 2:00 PM",
    "JUEVES Y VIERNES 9:00 AM A 1:25PM",
    "Descanso fijo Sabados y domingos"
   ],
   "requisitos": [
    "Estudiantes universitarios de ultimos ciclos de carrera universitaria o tecnicos finalizados",
    "1año de experiencia en atencion al cliente call center",
    "Deseable experiencia en back office",
    "Excel a nivel intermedio"
   ],
   "beneficios": [
    "Sueldo * S/725 +variable S/200+ Planilla completa+",
    "Capacitación remunerada (*3 dias *) !!*· * CAPACITACION",
    "3 días aula + 2 dias de OJT",
    "Capa de 8:00 am a 5:00 pm",
    "Pago: 20 soles por día asistido a capacitacion",
    "Planilla completa"
   ],
   "url": "https://www.laborum.pe/job/ibr-peru-s-a/home-office-asesor-atencion-al-cliente-nuam-chile-part-time/6a9794fd2020b210c8426558"
  },
  {
   "id": 251,
   "puesto": "Asistente de Cuentas por Pagar",
   "empresa": "GM BUSINESS SOLUTIONS",
   "cat": "Contabilidad",
   "min": 1800,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 92,
   "resumen": "RESUMEN: Somos un grupo de empresas dedicado al sector automotriz, así como a la comercialización de vehículos. Estamos en la búsqueda de un profesional para el puesto de Asistente de Cuentas por Pagar, que cuente con el siguiente perfil:",
   "funciones": [
    "Gestionar las cobranzas de clientes en general a través de llamadas telefónicas, envío de estados de cuenta (email o físicas).",
    "Analizar y reportar a los responsables Comerciales el estado actual de la deuda corriente de cada empresa asignada, a fin de tomar acción para evitar morosidad de clientes.",
    "Preparar la documentación de los casos de clientes morosos, para iniciar reporte en las centrales de riesgo y/o envió de la documentación al área Legal.",
    "Conciliar saldos con clientes y contabilidad diariamente, validando la información con nuestro ERP.",
    "Supervisar y validar la facturación diaria, revisando que todo pedido se encuentre debidamente facturado, y reportar al área comercial los pedidos no facturados de manera periódica.",
    "Revisar la BD de clientes y que esta cuente con toda la información requerida correos, teléfonos, vendedor responsable, canal etc.",
    "Ordenar, clasificar y archivar documentos relacionados con facturación, pagos y cobranzas, asegurando un adecuado control documentario."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en Administración, Contabilidad o afines.",
    "Excel intermedio/avanzado (tablas dinámicas, conciliaciones, indicadores).",
    "Sistemas contables y ERP (Odoo, Starsoft u otros).",
    "Normativa tributaria básica (detracciones, retenciones, igv, renta).",
    "Manejo de centrales de riesgo.",
    "Análisis de cuentas y conciliaciones.",
    "Experiencia mínima de 1 año en áreas de facturación, créditos y cobranzas."
   ],
   "beneficios": [
    "Lugar de Trabajo: Cercado de Lima (Av. Argentina)",
    "Horario de trabajo: Lunes a Viernes de 8 am a 5:30 pm y sabados de 8 am a 1 pm",
    "Salario: s/1800 a s/2000 (según evaluación)",
    "Planilla mype desde el 1er día de trabajo",
    "Oportunidad de crecimiento laboral. -Requerimientos- Educación mínima: Técnico1 año de experienciaEdad: entre 18 y 45 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, account, cuentas"
   ],
   "url": "https://www.laborum.pe/job/gm-business-solutions/asistente-de-cuentas-por-pagar-cercado-de-lima/6a9771832020b210c841decf"
  },
  {
   "id": 252,
   "puesto": "Teleoperador",
   "empresa": "Grupo Tawa",
   "cat": "Atención al Cliente",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 92,
   "resumen": "RESUMEN: Importante empresa del sector se encuentra en la búsqueda de Teleoperador para integrarse al una empresa líder en servicios empresariales en el distrito de Jesús María.",
   "funciones": [
    "Realizar llamadas telefónicas a empresas de una base de datos previamente trabajada.",
    "Contactar y validar información de empresas medianas y pequeñas.",
    "Brindar información sobre el servicio y sus beneficios.",
    "Gestionar la afiliación de nuevas empresas al sistema o servicio asignado.",
    "Realizar seguimiento a clientes interesados hasta concretar la afiliación.",
    "Registrar correctamente la información y avance de cada contacto en el sistema.",
    "Cumplir con los objetivos diarios y mensuales de afiliación.",
    "Mantener una comunicación persuasiva, clara y orientada a resultados."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 1 años en venta telefónica, call center.",
    "Experiencia en ventas telefónicas o afiliación de clientes (deseable).",
    "Deseable experiencia en seguros, AFP, banca o medios de comunicación.",
    "Manejo de Office a nivel intermedio y herramientas digitales.",
    "Perfil orientado a resultados, negociación y prospección comercial."
   ],
   "beneficios": [
    "Planilla con beneficios de ley.",
    "Sueldo básico: S/ 1200 + comisiones.",
    "Horario de trabajo",
    "Horario: Lunes a viernes de 8:30 am a 5:30 pm",
    "Lugar de trabajo: Jesús María",
    "Modalidad: Presencial. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 18 y 40 añosPalabras clave: callcenter, teleoperador, telefonista, telephonist, telefonico"
   ],
   "url": "https://www.laborum.pe/job/grupo-tawa/teleoperadores-call-center-jesus-maria/6a97abfe2020b210c842ba0c"
  },
  {
   "id": 253,
   "puesto": "Chili's Auxiliar de Mantenimiento Chorrillos",
   "empresa": "Adecco Perú S.A.",
   "cat": "Ingeniería",
   "min": 1739,
   "max": 1739,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 92,
   "resumen": "RESUMEN: ¿Tienes experiencia en mantenimiento general o servicios generales? ¡Esta es tu oportunidad de formar parte de una de las cadenas gastronómicas más reconocidas del país!",
   "funciones": [
    "Armado, desarmado, limpieza y mantenimiento de máquinas de fritura, equipos de frío y dispensadores.",
    "Desmontaje y limpieza profunda de equipos.",
    "Desinfección especializada de maquinaria.",
    "Reporte de faltantes de productos de limpieza.",
    "Elaboración de reportes y check list diarios.",
    "Garantizar el correcto funcionamiento de baños e instalaciones.",
    "Reportar incidencias en luminarias, equipos y maquinarias.",
    "Otras funciones inherentes al puesto."
   ],
   "requisitos": [
    "Deseable estudiante o egresado técnico de Servicios Generales o carreras afines.",
    "Experiencia mínima de 3 meses en posiciones de mantenimiento general o servicios generales.",
    "Disponibilidad para trabajar en turno nocturno",
    "11:00 p.m. a 7:00 a.m.",
    "12:00 a.m. a 8:00 a.m."
   ],
   "beneficios": [
    "Ingreso a planilla completa directa con Chilis.",
    "Salario de S/ 1,739 + pago de horas nocturnas (aprox. S/ 200 a S/ 250 adicionales).",
    "Alimentación cubierta.",
    "Todos los beneficios de ley.",
    "Pagos puntuales.",
    "Sedes disponibles",
    "Plaza Lima Sur. -Requerimientos- Educación mínima: Técnico años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, maintenance, conservacion, conservation, mantenimiento"
   ],
   "url": "https://www.laborum.pe/job/adecco-peru-s-a/chili-s-auxiliar-de-mantenimiento-chorrillos-planilla/6a9779a02020b210c841ff95"
  },
  {
   "id": 254,
   "puesto": "Personal de limpieza",
   "empresa": "Tentaciones Gourmet",
   "cat": "Recursos Humanos",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 92,
   "resumen": "RESUMEN: Empresa del rubro gastronómico, se encuentra en búsqueda de Personal de Limpieza para formar parte de nuestro equipo.",
   "funciones": [
    "Realizar la limpieza y desinfección de las diferentes áreas del restaurante.",
    "Mantener limpios y ordenados los ambientes asignados.",
    "Realizar la limpieza de pisos, baños, áreas comunes y zonas de atención.",
    "Utilizar correctamente los productos e implementos de limpieza.",
    "Cumplir con los protocolos de higiene y seguridad establecidos.",
    "Brindar apoyo en las labores de limpieza requeridas según las necesidades del restaurante."
   ],
   "requisitos": [
    "Experiencia en limpieza, preferentemente en restaurantes, cafeterías, hoteles o establecimientos similares.",
    "Conocimiento en el uso adecuado de productos e implementos de limpieza.",
    "Disponibilidad para trabajar en el horario de mañana o tarde.",
    "Responsabilidad, puntualidad y compromiso.",
    "Capacidad para trabajar en equipo y mantener un buen ritmo de trabajo."
   ],
   "beneficios": [
    "Sueldo: S/ 1,500 mensuales.",
    "Ingreso a planilla desde el primer día (Planilla Mype)",
    "Todos los beneficios de ley.",
    "Oportunidad de crecimiento y desarrollo dentro de la empresa.",
    "Formar parte de una empresa del rubro gastronómico.-Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 25 y 45 añosPalabras clave: staff, empleado, trabajador, worker, employee, personal, aseo, higiene, cleaning, limpieza"
   ],
   "url": "https://www.laborum.pe/job/tentaciones-gourmet/se-precisa-urgente-personal-de-limpieza-s-1-600-mensual/6a9788f12020b210c8423a71"
  },
  {
   "id": 255,
   "puesto": "Atención al cliente",
   "empresa": "REC01 STUDIOS",
   "cat": "Atención al Cliente",
   "min": 1200,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 91,
   "resumen": "RESUMEN: ¿Te gustaría desarrollar tu carrera en un entorno innovador?",
   "funciones": [
    "Conducir la transmisión (Dar el procedimiento de los juegos) siguiendo los protocolos y estándares establecidos.",
    "Mantener una postura, imagen y presencia profesional durante las transmisiones.",
    "Seguir las instrucciones operativas brindadas por el supervisor a cargo como presentación frente a cámara, procedimiento correcto de uso de cartas.",
    "Modalidad y horario",
    "Modalidad presencial en San Isidro.",
    "Horarios comprendidos: 6:50am a 3:35pm | 1:20pm a 10:05pm | 9:50pm a 7:20am.",
    "Un día de descanso a la semana."
   ],
   "requisitos": [
    "Carisma y seguridad al interactuar.",
    "Excelente comunicación, actitud positiva y proactividad.",
    "Compromiso, responsabilidad y disposición para aprender."
   ],
   "beneficios": [
    "Sueldo base de S/ 1,200 a S/1,400 + S/ 200 por concepto de movilidad.",
    "Sobre tasa (35%) trabajo nocturno.",
    "Ingreso a planilla desde el primer día de capacitación.",
    "Beneficios de ley: CTS, gratificaciones, vacaciones y EsSalud.",
    "Capacitación integral y oportunidades de crecimiento profesional.",
    "Estabilidad laboral en una empresa con proyección internacional.",
    "¡Postula ahora y forma parte de una empresa donde la tecnología, la innovación y el desarrollo profesional impulsan cada oportunidad!"
   ],
   "url": "https://www.laborum.pe/job/rec01-studios/atencion-al-cliente-sin-experiencia/6a979c662020b210c842801c"
  },
  {
   "id": 256,
   "puesto": "Analista Sr. de Planificación y Control Temporal por Proyecto para San",
   "empresa": "Overall Strategy",
   "cat": "Ingeniería",
   "min": 5200,
   "max": 5200,
   "moneda": "PEN",
   "modalidad": "Híbrido",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 90,
   "resumen": "RESUMEN: ¡En Overall creemos que tu talento merece la mejor oportunidad!",
   "funciones": [
    "Participar en los procesos generales de presupuestación, planificación, provisión e informes sobre control de OPEX / CAPEX.",
    "Realizar análisis y monitoreo de costos de operación y mantenimiento, asegurando su alineamiento con los presupuestos y proyecciones económicas y financieras.",
    "Implementar metodologías de control de costos que permitan evaluar y optimizar los procesos.",
    "Elaborar reportes de seguimiento."
   ],
   "requisitos": [
    "Técnico o Graduado Universitario en Ingeniería de Sistemas, Industrial, Informática o carreras afines.",
    "Experiencia mínima de 4 años en el puesto o 2 años en posiciones de menor rango.",
    "Experiencia en presupuestación, planificación, provisión y control de costos (OPEX/CAPEX).",
    "Manejo de MS Office (Excel a nivel Avanzado) e Inglés a nivel Intermedio.",
    "Conocimientos en Power BI, SAP Business One, SAP RP3 o SAP HANA.",
    "Disponibilidad para trabajar en San Isidro bajo modalidad híbrida."
   ],
   "beneficios": [
    "Horario y Modalidad: Híbrido (3 días presencial / 2 días remoto). Lunes a Jueves de 8:00 a.m. a 6:45 p.m. y Viernes de 8:00 a.m. a 1:00 p.m.",
    "Sueldo: S/ 5,200 brutos.",
    "Contrato: Temporal (3 a 4 meses).",
    "Ingreso a planilla desde el primer día con todos los beneficios de ley (CTS, gratificación, vacaciones, ESSALUD).",
    "Oportunidad de trabajar con una empresa líder del sector energía (PLUZ ENERGIA PERU S.A.A.).",
    "En el Corporativo Overall creemos que la diversidad nos fortalece y la inclusión nos une. Únete a un equipo donde cada talento es valorado y respetado.-Requerimientos- Educación mínima: Universitario2 años de experienciaPalabras clave: analyst, senior, sr, planning, planeacion, project"
   ],
   "url": "https://www.laborum.pe/job/overall-strategy/analista-sr-de-planificacion-y-control-temporal-por-proyecto/6a978e3f2020b210c8424cb9"
  },
  {
   "id": 257,
   "puesto": "Call Center",
   "empresa": "TCONTAKTO",
   "cat": "Ventas",
   "min": 6000,
   "max": 6000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 89,
   "resumen": "RESUMEN: Ejecutivo(a) de Ventas Call Center Scotiabank",
   "funciones": [
    "Contactar clientes de una base de datos preaprobada.",
    "Asesorar sobre los beneficios de los productos financieros Scotiabank.",
    "Gestionar la negociación y el cierre de ventas, cumpliendo los objetivos comerciales."
   ],
   "requisitos": [
    "Mínimo 4 meses de experiencia en ventas por Call Center de tarjetas de crédito, préstamos, portabilidad con público extranjero o productos financieros.",
    "Disponibilidad para laborar de forma presencial en Cercado de Lima.",
    "Horario: lunes a viernes de 9:00 a. m. a 6:00 p. m. y sábados de 9:00 a. m. a 3:00 p. m."
   ],
   "beneficios": [
    "Sueldo fijo + capacitación pagada.",
    "Comisiones ilimitadas (promedio de S/ 4,500 a S/ 6,000).",
    "Bono de permanencia de S/ 300.",
    "Planilla completa desde el primer día.",
    "Pagos puntuales.",
    "Oportunidad de crecimiento y línea de carrera.",
    "Convenios educativos y descuentos corporativos."
   ],
   "url": "https://www.laborum.pe/job/tcontakto/call-ventas-scotiabank-s-3500-a-mas-exp-en-portabilidad/6a97a2322020b210c84295f6"
  },
  {
   "id": 258,
   "puesto": "Analista de Reclutamiento y Selección",
   "empresa": "SSP ventas del Peru",
   "cat": "Recursos Humanos",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 89,
   "resumen": "Descripción del puesto: Serás responsable de liderar los procesos de reclutamiento y selección de perfiles masivos y clave para el rubro de abarrotes/consumo masivo, garantizando la cobertura oportuna de las vacantes.",
   "funciones": [
    "Publicación de ofertas laborales en diversas plataformas y redes profesionales.",
    "Filtro curricular, entrevistas por competencias y evaluaciones psicotécnicas.",
    "Manejo de procesos de selección masivos e individuales para el sector abarrotes.",
    "Verificación de antecedentes, referencias laborales y armado de expedientes.",
    "Seguimiento de métricas de rotación y tiempos de cobertura."
   ],
   "requisitos": [
    "Experiencia previa comprobable en reclutamiento para el sector consumo masivo / abarrotes (indispensable).",
    "Profesional de Psicología.",
    "Manejo de fuentes de reclutamiento masivo y entrevistas por competencias.",
    "Alta capacidad de organización, trabajo por objetivos y autonomía.",
    "Laptop/PC y conexión a internet estable para trabajo desde casa."
   ],
   "beneficios": [
    "Modalidad: 100% Home Office.",
    "Horario: Lunes a viernes de 8:00 a.m. a 5:00 p.m.",
    "Sueldo: S/ 1,200 básicos + comisiones por cobertura de vacantes.",
    "Excelente clima laboral y estabilidad.-Requerimientos- Educación mínima: Técnico2 años de experienciaPalabras clave: analyst, reclutador, recruiter, headhunter, scout, reclutamiento, casa, remoto, remote, teletrabajo, home, oficina"
   ],
   "url": "https://www.laborum.pe/job/ssp-ventas-del-peru/analista-de-reclutamiento-y-seleccion-consumo-masivo/6a9788132020b210c84236ab"
  },
  {
   "id": 259,
   "puesto": "Conductor A3B o A3C para Camiones grandes C",
   "empresa": "Ransa Comercial S.A.",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 86,
   "resumen": "RESUMEN: Somos SLA, el centro logístico automotriz más grande del Perú, perteneciente al grupo Ransa. Actualmente nos encontramos en la búsqueda de Conductores con Licencia A3B/A3C para Unidades Cigüeña.",
   "funciones": [
    "Manejar las unidades cigüeña para trasladar los autos de puertos al lugar asignado y verificar con check-list la operatividad de la unidad",
    "Realizar la carga, descarga y asegurar los autos a la unidad cigüeña",
    "Verificar la documentación (guía remisión, acta de entrega, guía remisión transportista) del servicio"
   ],
   "requisitos": [
    "Experiencia de mínimo 1 año manejando camiones grandes",
    "Contar con brevete A3B o A3C vigente",
    "Contar con secundaria completa",
    "Disponibilidad para llegar a planta ubicado en Lurín"
   ],
   "beneficios": [
    "Salario base: S/1130",
    "Variable mensual varía según la demanda de producción (desde S/3000 hasta S/6000).",
    "Bono de ingreso (S/1800) en el primer mes de ingreso cada uno.",
    "Viáticos diarios local: S/15 a S/40 (Si cumples el horario de trabajo completo, puedes llegar a obtener hasta S/900 en viáticos en local).",
    "Viáticos diarios provincia: S/50",
    "Asignación familiar: S/113",
    "Planilla desde el primer mes de trabajo con todos los beneficios de ley: CTS, Asignación Familiar, Gratificación, Vacaciones, Utilidades, Seguro EsSalud y opción de afiliarse al seguro privado Pacífico (EPS) con coberturaal 75%",
    "Canasta y vale de pavo (En diciembre)",
    "Asignación por fallecimiento (Al cumplir 1 año de labores)",
    "Día libre por cumpleaños",
    "Programa de Ransa Contigo (Para usted y familiares)",
    "Entrega de uniforme"
   ],
   "url": "https://www.laborum.pe/job/ransa-comercial-s-a/conductor-a3b-o-a3c-para-camiones-grandes-c-s-en-ciguenas/6a9781022020b210c8421c7b"
  },
  {
   "id": 260,
   "puesto": "Asesor de Ventas",
   "empresa": "Aadaru",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 24,
   "vence": "2026-10-02",
   "restan": 6,
   "score": 83,
   "resumen": "RESUMEN: ¿Te apasiona el servicio al cliente y las ventas? ¡Esta es tu oportunidad de crecer profesionalmente con Movistar!",
   "funciones": [
    "Atender y asesorar a los clientes, resolviendo consultas y gestionando reclamos de manera efectiva.",
    "Ofrecer productos y servicios de Movistar, destacando promociones, planes y dispositivos para maximizar las ventas.",
    "Garantizar un servicio ágil y eficiente, cumpliendo con los más altos estándares de atención.",
    "Lo que ofrecemos",
    "Sueldo básico de S/1,130 +",
    "Comisiones ilimitadas",
    "Incentivos por desempeño."
   ],
   "requisitos": [
    "Disponibilidad para horarios rotativos en modalidad Retail de lunes a domingo.",
    "Pasión por la atención al cliente y habilidad para superar expectativas.",
    "Promovemos un ambiente de trabajo inclusivo, donde todas las personas, sin importar edad, género, cultura, orientación sexual o discapacidad, tienen las mismas oportunidades.",
    "¡Postula ahora y forma parte de un equipo que transforma experiencias en soluciones!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: advisor, asesor, cambaceo, sales"
   ],
   "beneficios": [
    "Oportunidades reales de desarrollo profesional desde el tercer mes.",
    "Capacitación continua para potenciar tus habilidades en ventas y atención al cliente."
   ],
   "url": "https://www.laborum.pe/job/aadaru/asesor-de-ventas-puruchuco-altas-comisiones-movistar/6a97dfd72020b210c8434475"
  },
  {
   "id": 261,
   "puesto": "Técnico",
   "empresa": "Desysweb S.A.C.",
   "cat": "Ingeniería",
   "min": 1400,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 25,
   "vence": "2026-10-01",
   "restan": 5,
   "score": 95,
   "resumen": "Inspeccionar y probar equipos, realizando el mantenimiento preventivo (limpieza, verificación de potencia)",
   "funciones": [
    "Inspeccionar y probar equipos, realizando el mantenimiento preventivo (limpieza, verificación de potencia)",
    "Probar rendimiento y calidad de señal ( Validando servicio con cliente y cnoc)",
    "Realizar el retiro de la configuiracion antes y despues del MP para backup",
    "Reportar Trabajo y entrega de Documencion en linea (FOTOGRAFIAS E INFORME TECNICO)",
    "Reporte diario de avances mediante fotografías y evidencias de campo.",
    "Cumplimiento de los procedimientos de seguridad y uso correcto de EPP.",
    "Apoyo en actividades operativas asignadas por la supervisión."
   ],
   "requisitos": [
    "Técnico egresado o estudios truncos en Telecomunicaciones, Electrónica, Computación o afines.",
    "Con Experiencia o Sin experiencia.",
    "Conocimiento básico en instalaciones, configuraciones de router CISCO, Fibra o Microondas.",
    "Disponibilidad completa 24*7 según demanda del cliente y rotativos",
    "Licencia A-1 (no indispensable)",
    "Ganas de querer aprender y compromiso"
   ],
   "beneficios": [
    "Capacitación constante.",
    "Pagos puntuales y mensuales.",
    "Salario de: s/1,400.00 A s/1,500.00 previa entrevista",
    "Oportunidad de crecimiento profesional.",
    "Excelente clima laboral.· Postula y forma parte del equipo!. -Requerimientos- Educación mínima: Técnico años de experienciaLicencias de conducir: A-I Palabras clave: tecnologo, tecnico, tech, technician, technology, tecnologia, technologist, maintenance, conservacion, conservation, mantenimiento"
   ],
   "url": "https://www.laborum.pe/job/desysweb-s-a-c/tecnico-con-o-sin-exp-telecomunicaciones-mantenimiento/6a961f7e2020b210c8409b35"
  },
  {
   "id": 262,
   "puesto": "Atención al cliente",
   "empresa": "Total Talent Human Resources",
   "cat": "Ventas",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 25,
   "vence": "2026-10-01",
   "restan": 5,
   "score": 94,
   "resumen": "RESUMEN: En Total Talent, creemos firmemente en proporcionar oportunidades laborales sin COSTOS o PAGOS asociados para los postulantes.",
   "funciones": [
    "Brindar atención al cliente y resolver consultas, solicitudes e incidencias.",
    "Comunicar las promociones vigentes de acuerdo con cada campaña.",
    "Captar y registrar pedidos de los clientes.",
    "Realizar seguimiento post venta y mantener una atención oportuna.",
    "Analizar el segmento asignado e identificar oportunidades de mejora en la atención."
   ],
   "requisitos": [
    "Secundaria completa (mínimo requerido).",
    "Experiencia mínima de 6 meses en ventas, atención al cliente o asesoría comercial.",
    "Manejo básico de herramientas de Office (Excel, Word, PowerPoint).",
    "Horario: lunes a viernes de 8:00 a.m. a 6:00 p.m.",
    "Lugar de trabajo: Huachipa"
   ],
   "beneficios": [
    "Remuneración: S/ 1,400",
    "Capacitación: 1 a 2 días previos al inicio de labores.",
    "Estabilidad laboral",
    "Planilla directa",
    "Capacitaciones constantes",
    "Trabajo de lunes a viernes"
   ],
   "url": "https://www.laborum.pe/job/total-talent-human-resources/asesora-de-ventas-por-catalogo-call-center-leonisa-1400-ate/6a96201f2020b210c8409ca6"
  },
  {
   "id": 263,
   "puesto": "Asesor comercial Culqi",
   "empresa": "Smart Sales Partners",
   "cat": "Ventas",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 25,
   "vence": "2026-10-01",
   "restan": 5,
   "score": 94,
   "resumen": "RESUMEN: Somos el dealer N.º 1 de Culqi (Credicorp) y Movistar en el Perú.",
   "funciones": [
    "Prospección y cierre de ventas de equipos POS.",
    "Gestión de clientes con transacciones +S/50,000 mensuales.",
    "Seguimiento postventa durante 3 meses.",
    "Planificar y organizar Videoconferencias con clientes potenciales para negociar oportunidades de venta.",
    "Reporte de gestión CRM en Excel (funnel, prospección, contactabilidad)."
   ],
   "requisitos": [
    "1 año de experiencia en ventas corporativas (B2B).",
    "Deseable experiencia en POS, medios de pago, telecomunicaciones, banca, seguros o servicios empresariales.",
    "Habilidades de comunicación, negociación y enfoque en resultados.",
    "Disponibilidad para laborar en San isidro - Cerca a estación Canaval y Moreyra."
   ],
   "beneficios": [
    "Modalidad presencial: de Lunes a viernes de 08:30am a 6:15pm, trabaja solo 2 sábados al mes de 09:00am a 1pm.",
    "Ingreso fijo S/1,400 + movilidad + comisiones ilimitadas.",
    "Ingreso a planilla con todos los beneficios, desde el primer día.",
    "Comisiones altas (hasta + de S/10,000) + incentivos(viajes, descuentos corporativos, vales, entre otros).",
    "Capacitaciones pagadas, convenios corporativos y línea de carrera."
   ],
   "url": "https://www.laborum.pe/job/smart-sales-partners/asesor-comercial-culqi-bcp-sueldo-comisiones-incentivos-san/6a9618052020b210c840813a"
  },
  {
   "id": 264,
   "puesto": "Asesor (a) de Ventas",
   "empresa": "Total Talent Human Resources",
   "cat": "Ventas",
   "min": 1330,
   "max": 1330,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 25,
   "vence": "2026-10-01",
   "restan": 5,
   "score": 92,
   "resumen": "RESUMEN: Importante empresa del sector retail, se encuentra en busqueda de:",
   "funciones": [
    "Brindar atención personalizada al cliente siguiendo el speech comercial de la marca.",
    "Generar una experiencia de compra enfocada en el valor del producto.",
    "Cumplir con los objetivos de ventas individuales y de tienda.",
    "Promocionar productos y campañas comerciales vigentes.",
    "Mantener una adecuada presentación personal alineada a la imagen de la marca.",
    "Apoyar en el orden, reposición y exhibición de productos en tienda.",
    "Realizar seguimiento a clientes potenciales cuando corresponda.",
    "Participar activamente en campañas internas para el cumplimiento de metas."
   ],
   "requisitos": [
    "Experiencia mínima de 6 meses en ventas o atención al cliente.",
    "Alta orientación al cliente y cumplimiento de metas de ventas.",
    "Disponibilidad de laborar en Jockey Plaza",
    "Disponibilidad para trabajar en turnos rotativos de lunes a domingo (con un día de descanso).",
    "Horarios rotativos",
    "Apertura: 9:30 a.m. 6:30 p.m.",
    "Intermedio: 12:00 p.m. 9:00 p.m.",
    "Cierre: 2:00 p.m. 10:00 p.m."
   ],
   "beneficios": [
    "Sueldo base: S/ 1,330",
    "Comisiones: S/ 800 a S/ 1,500+",
    "Bonos por campañas comerciales: S/ 50 a S/ 100.",
    "Ingreso a planilla general desde el primer día.·",
    "En Total Talent, creemos firmemente en proporcionar oportunidades laborales sin COSTOS o PAGOS asociados para los postulantes."
   ],
   "url": "https://www.laborum.pe/job/total-talent-human-resources/asesor-a-de-ventas-sector-retail-jockey-plaza-planilla/6a963b8c2020b210c840e3c5"
  },
  {
   "id": 265,
   "puesto": "Gestor",
   "empresa": "Confidencial",
   "cat": "Contabilidad",
   "min": 2500,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Híbrido",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 25,
   "vence": "2026-11-30",
   "restan": 65,
   "score": 92,
   "resumen": "Somos Zeruk Corredores de Seguros, un broker de seguros digital en Perú, enfocado en transformar la manera en que las empresas gestionan y administran sus seguros a través de tecnología.",
   "funciones": [
    "Gestionar diariamente la cartera de cuentas por cobrar.",
    "Realizar seguimiento a clientes por teléfono, WhatsApp y correo.",
    "Gestionar y hacer seguimiento a compromisos de pago.",
    "Identificar y priorizar cartera vencida.",
    "Mantener actualizada la información y los indicadores de cobranza.",
    "Coordinar con las áreas Comercial, Operaciones y Administración. Escalar oportunamente casos críticos o compromisos incumplidos."
   ],
   "requisitos": [
    "Experiencia mínima de 2 año en cobranzas, idealmente B2B.",
    "Excelente comunicación y negociación.",
    "Sobre todo, buscamos una persona persistente, organizada, proactiva, con sentido de urgencia y orientada a resultados, que quiera desarrollarse dentro de una compañía que está llevando tecnología y automatización a la industria de seguros."
   ],
   "beneficios": [
    "Sueldo Bruto: S/ 2,500",
    "Régimen PYME",
    "EPS PYME a partir de 3er mes, bajo evalución de resultados.",
    "Bonos por cumplimiento de indicadores de cobranza.",
    "Horario Híbrido: De lunes a jueves de 9am a 6pm en oficina (Surquillo) y viernes home office.",
    "Estabilidad laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/gestor-a-de-cobranzas-zeruk-brokers-1118425262.html"
  },
  {
   "id": 266,
   "puesto": "Supervisor",
   "empresa": "CEELIMP GROUP S.A.C",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 25,
   "vence": "2026-10-01",
   "restan": 5,
   "score": 91,
   "resumen": "Nos encontramos en la búsqueda de Operarios(as) Limpieza con o sin experiencia para laborar en Surco",
   "funciones": [
    "Limpieza de las áreas de trabajo (SS.HH, Pasadizos, Salas de Espera, Escaleras, Andenes, Cocinas, Escritorios, etc.).",
    "Manejo de materiales de limpieza y uso adecuado de los insumos.",
    "Mantener las áreas de servicio acuerdo según el Plan de Trabajo.",
    "Cumplir con el cronograma tareas diarias al 100 %.",
    "Velar por el área asignada por parte del Supervisor.",
    "Reportar las incidencias de la operativa diaria.",
    "Velar por el correcto uso de los uniformes, EPPS, materiales e insumos asignados."
   ],
   "requisitos": [
    "Con/sin experiencia.",
    "Disponibilidad para laborar de Lunes a Domingo (descanso entre semana).",
    "Vacunas Covid-19",
    "Actitud de servicio.",
    "Comunicación cordial con los clientes.",
    "Puntualidad y responsabilidad.",
    "Compromiso y lealtad."
   ],
   "beneficios": [
    "Ingreso a una empresa formal y sostenible en el rubro.",
    "Ingreso a planilla desde el primer día con todos los beneficios (CTS, gratificaciones, Essalud, Vacaciones).",
    "Asignación familiar S/113",
    "Línea de carrera y capacitación constante.",
    "Unidad de trabajo y horarios fijo.",
    "Disponible: T. Mañana 6:00am a 3:00pm / T. Tarde 12:00pm a 9:00pm",
    "Pagos puntuales: S/1130 + Bono Movilidad S/70 y/o S/150",
    "Pagos mensuales (puntuales).",
    "Presentes por Días Festivos.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 19 y 55 añosConocimientos: MantenimientoPalabras clave: aseo, higiene, cleaning, limpieza, permanent, fijo"
   ],
   "url": "https://www.laborum.pe/job/ceelimp-group-s-a-c/operarios-as-de-limpieza-clinica-surco-alt-puente/6a96397c2020b210c840def2"
  },
  {
   "id": 267,
   "puesto": "Asesor(a) de Ventas Call Center",
   "empresa": "MF Asesoria y Consultoria SAC",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 25,
   "vence": "2026-10-01",
   "restan": 5,
   "score": 89,
   "resumen": "RESUMEN: ¿Buscas tu primer empleo en el sector financiero o quieres potenciar tus ingresos al máximo?",
   "funciones": [
    "Contactar telefónicamente a la cartera asignada con productos preaprobados.",
    "Asesorar al cliente y concretar el cierre de la venta de forma ágil.",
    "Cumplir con las metas establecidas para maximizar tus comisiones."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Excelente fluidez verbal, buena actitud y orientación a resultados.",
    "Disponibilidad para trabajar de manera 100% presencial en La Victoria."
   ],
   "beneficios": [
    "Sueldo fijo (S/ 1,130) + Comisiones ilimitadas (sin techo de ventas).",
    "Ingreso a Planilla MYPE desde el primer día con los beneficios de ley.",
    "Capacitaciones pagadas.",
    "Pagos puntuales, ambiente dinámico y oportunidades de línea de carrera.",
    "Horario de trabajo (Descanso fijo los domingos)",
    "Lunes a Viernes: 9:00 a.m. a 6:00 p.m."
   ],
   "url": "https://www.laborum.pe/job/mf-asesoria-y-consultoria-sac/asesor-a-de-ventas-call-center-banco-santander-sin/6a9629342020b210c840b57b"
  },
  {
   "id": 268,
   "puesto": "Asistente de Procesos y Mejora Continua",
   "empresa": "DICOMSA S.A.",
   "cat": "Recursos Humanos",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 25,
   "vence": "2026-10-01",
   "restan": 5,
   "score": 89,
   "resumen": "RESUMEN: Asistente de Procesos y Mejora Continua | Santa Anita",
   "funciones": [
    "Seguimiento de reclamos, incidencias y productos observados.",
    "Coordinación con Comercial, Producción, Planeamiento, Almacén, Logística, Facturación y Calidad.",
    "Control de notas de crédito, descuentos, pérdidas y devoluciones.",
    "Elaboración y actualización de bases de datos, indicadores y reportes.",
    "Análisis de información para identificar desviaciones y oportunidades de mejora.",
    "Seguimiento de acciones correctivas hasta su cierre."
   ],
   "requisitos": [
    "Egresado(a) o bachiller de Ingeniería Industrial, Administración, Gestión de Procesos o afines.",
    "Experiencia mínima de 1 año en procesos, operaciones, planeamiento, calidad o funciones relacionadas.",
    "Experiencia deseable en empresas industriales o manufactureras.",
    "Excel intermedio como mínimo: tablas dinámicas, fórmulas, cruces y análisis de información.",
    "Capacidad para coordinar con diferentes niveles y áreas.",
    "Organización, iniciativa, comunicación y atención al detalle.",
    "Capacidad para trabajar bajo presión y con múltiples pendientes."
   ],
   "beneficios": [
    "Remuneración S/.1300",
    "Planilla completa + todos los beneficios de ley",
    "Pagos puntuales: 15 y fin de mes",
    "Línea de carrera",
    "Aprendizaje y desarrollo profesional",
    "Desarrollo dentro de una empresa industrial",
    "Lunes a viernes: 8:00 a. m. a 6:35 p. m.",
    "Trabajo presencial Santa Anita - Los Ficus (al lado de la empresa Eurocamiones en Evitamiento). -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 22 y 45 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, bachelor, degree, licenciatura"
   ],
   "url": "https://www.laborum.pe/job/dicomsa-s-a/asistente-de-procesos-y-mejora-continua-santa-anita-s-1300/6a9631a12020b210c840cba7"
  },
  {
   "id": 269,
   "puesto": "Call Center",
   "empresa": "Atento Perú",
   "cat": "Atención al Cliente",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 25,
   "vence": "2026-10-01",
   "restan": 5,
   "score": 81,
   "resumen": "RESUMEN: ¡TRABAJA DESDE CASA! | ASESOR(A) DE VENTAS CALL CENTER PORTABILIDAD MOVISTAR",
   "funciones": [
    "En Atento estamos buscando Asesores(as) de Ventas para Movistar Perú con el objetivo de generar cierre de ventas ofreciendo una atención de calidad.",
    "Lunes a Sábado: 9:00 a.m. a 6:00 p.m.",
    "Descanso fijo: domingos y feriados"
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima 3 meses Call Center o ventas presenciales.",
    "Contar con PC o laptop propia."
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,130",
    "Planilla completa desde el primer día de contrato: CTS, gratificaciones, asignación familiar, EsSalud, vacaciones y utilidades.",
    "Comisiones ilimitadas (S./ 226 soles en tarjeta de alimento + lo restante en efectivo)",
    "CAPACITACIÓN (remunerada)",
    "Duración: 7 días SIN OJT",
    "Horario: lunes a sábado de 9:00 a.m. a 4:00 p.m.",
    "Pago: S/ 30 por día",
    "Modalidad: Remoto (Lima y provincias)."
   ],
   "url": "https://www.laborum.pe/job/atento-peru/trabaja-desde-casa-ingreso-directo-con-o-sin-experiencia/6a962c2f2020b210c840bd4f"
  },
  {
   "id": 270,
   "puesto": "Asesor(a) de Ventas Presencial y Digital",
   "empresa": "Lift Ranger S.A.C.",
   "cat": "Ventas",
   "min": 1500,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 26,
   "vence": "2026-11-29",
   "restan": 64,
   "score": 95,
   "resumen": "Impulsa tu carrera comercial en Lift Ranger, empresa líder en importación, distribución y comercialización de productos para instalación y automatización de puertas de garaje, sistemas automáticos y soluciones tecnológicas.",
   "funciones": [
    "Atención presencial en tienda y asesoría comercial a clientes.",
    "Gestión de ventas digitales por WhatsApp, redes sociales y llamadas.",
    "Cotización y seguimiento.",
    "Venta B2C y B2B: clientes finales, técnicos y empresas contratistas.",
    "Manejo de objeciones y cierre de ventas de productos high ticket.",
    "Fidelización de clientes y construcción de relaciones comerciales."
   ],
   "requisitos": [
    "Edad referencial: 18 a 35 años.",
    "Excelente presencia, comunicación clara y trato cordial.",
    "Experiencia en ventas presenciales, atención al cliente y ventas por WhatsApp / redes sociales.",
    "Habilidad para cerrar ventas, persuadir y manejar objeciones.",
    "Rapidez para aprender productos técnicos (automatización, seguridad, accesos).",
    "Disponibilidad inmediata para trabajo presencial en Surquillo.",
    "Ideal si resides en Surquillo, Miraflores, San Borja, Barranco, Surco o zonas cercanas."
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,500 – S/ 1,800 (según evaluación).",
    "Comisiones variables: Hasta S/ 1,300 adicionales por cumplimiento de metas.",
    "Ingreso a planilla desde el primer día (Gratificaciones, CTS, Vacaciones, Essalud, Asignación Familiar).",
    "Capacitación técnica y comercial continua en productos de seguridad y automatización.",
    "Excelente clima laboral, estabilidad y oportunidades reales de crecimiento.",
    "Horario y ubicación",
    "Lunes a viernes: 8:30 am – 6:00 pm",
    "Sábados: 8:30 am – 2:00 pm",
    "Lugar: Tienda Lift Ranger – Av. Principal 345, Surquillo"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesora-de-ventas-presencial-y-digital-tienda-fisica-surquillo-lift-ranger-s.a.c.-1118421632.html"
  },
  {
   "id": 271,
   "puesto": "Atención al cliente",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Otros",
   "min": 1250,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 27,
   "vence": "2026-11-28",
   "restan": 63,
   "score": 94,
   "resumen": "Recibir y atender cordialmente a los clientes desde su llegada al establecimiento y tomar correctamente los pedidos y registrarlos en el sistema correspondiente.",
   "funciones": [
    "Recibir y atender cordialmente a los clientes desde su llegada al establecimiento y tomar correctamente los pedidos y registrarlos en el sistema correspondiente.",
    "Recomendar productos de acuerdo con las preferencias y necesidades del cliente, conociendo progresivamente la propuesta gastronómica y saludable de la marca.",
    "Servir alimentos y bebidas de manera correcta, ordenada y oportuna.",
    "Realizar el seguimiento de las mesas durante toda la experiencia del cliente.",
    "Verificar que los pedidos sean entregados correctamente y en los tiempos establecidos.",
    "Atender consultas, solicitudes y requerimientos de los clientes con amabilidad y disposición.",
    "Coordinar constantemente con cocina, barra, caja y las demás áreas del local.",
    "Mantener las mesas y el área de atención limpias, ordenadas y correctamente acondicionadas.",
    "Apoyar en el montaje y desmontaje del salón cuando sea necesario.",
    "Apoyar en las actividades necesarias para el correcto funcionamiento y productividad del turno.",
    "Participar en la limpieza y organización de las áreas de trabajo de acuerdo con las funciones asignadas."
   ],
   "requisitos": [
    "Solo mujeres de cualquier nacionalidad entre 20 y 30 años.",
    "Tener experiencia mínima de 1 año en puestos similares, atención al cliente, restaurantes, cafeterías o establecimientos gastronómicos.",
    "Disponibilidad para trabajar 48 horas semanales, 6 días a la semana con 1 dia de descanso rotativo. Los horarios pueden ser: de 7:45 am a 4:30 pm o de 1:15 pm a 10 pm."
   ],
   "beneficios": [
    "Grato ambiente laboral y posibilidad de hacer línea de carrera.",
    "Pago quincena y fin de mes.",
    "Beneficios laborales de planilla remype desde el 1er día de trabajo, tienen 15 días de vacaciones al año y Essalud.",
    "Sueldo básico (S/.1250) + propinas + alimentación."
   ],
   "url": "https://www.bumeran.com.pe/empleos/meseras-adn-retail-s.a.c.-1118421597.html"
  },
  {
   "id": 272,
   "puesto": "Ejecutivo de Ventas",
   "empresa": "ManpowerGroup Perú",
   "cat": "Ventas",
   "min": 1800,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 27,
   "vence": "2026-11-28",
   "restan": 63,
   "score": 89,
   "resumen": "En Manpower Perú estamos en la búsqueda del mejor talento para asumir la posición de Ejecutiv de Ventas para una importante empresa líder en su sector. ¡La oportunidad de tener un futuro mejor está aquí!",
   "funciones": [
    "Contactar y asesorar a los leads asignados sobre los proyectos inmobiliarios.",
    "Realizar seguimiento comercial hasta el cierre de la venta.",
    "Coordinar visitas a los terrenos y brindar información del proyecto.",
    "Elaborar cotizaciones y propuestas comerciales.",
    "Registrar y actualizar la información de clientes en el sistema.",
    "Cumplir con los objetivos de ventas establecidos."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 1 año en ventas, deseablemente en los rubros inmobiliario, seguros, productos financieros, automotriz o afines.",
    "Excel básico - intermedio",
    "Disponibilidad para realizar visitas en los terrenos (se brinda traslado de ida y retorno)."
   ],
   "beneficios": [
    "Ingreso a planilla régimen general desde el 1er día de trabajo.",
    "Sueldo base S/1800 + comisiones ilimitadas (promedio 5k).",
    "Horario de trabajo: lunes a domingo de 8:30 am a 6:00pm, con un día de descanso rotativo.",
    "Días de semana trabajo en oficina y fines de semana visita a terrenos.",
    "En Manpower Perú, promovemos la inclusión e inserción laboral sin distinción por razón de discapacidad, género, orientación, identidad y expresión de género, lugar de origen o generación. Por ello, ¡todos los talentos son bienvenidos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-de-ventas-rubro-inmobiliario-manpowergroup-peru-1118421571.html"
  },
  {
   "id": 273,
   "puesto": "Coordinador(a) de Operaciones Logísticas",
   "empresa": "GRAFTON LATAM",
   "cat": "Logística",
   "min": 5000,
   "max": 5000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 28,
   "vence": "2026-11-27",
   "restan": 62,
   "score": 97,
   "resumen": "Por encargo de nuestro cliente, importante operador logístico con amplia presencia en el mercado peruano, nos encontramos en búsqueda de un(a)",
   "funciones": [
    "Supervisar el correcto desarrollo de los procesos de recepción, almacenamiento, picking, abastecimiento, inventarios y despacho.",
    "Coordinar y organizar los recursos necesarios para asegurar la continuidad y eficiencia de la operación.",
    "Liderar y realizar seguimiento al personal administrativo, supervisores y personal operativo asignado.",
    "Monitorear el cumplimiento de procedimientos, instructivos de trabajo y estándares establecidos para la operación.",
    "Gestionar inventarios rotativos y generales, asegurando la correcta correspondencia entre el stock físico y lo registrado en sistema.",
    "Realizar seguimiento a los indicadores operativos relacionados con ingresos, salidas, pedidos, inventarios, capacidad, productividad y nivel de servicio.",
    "Gestionar incidencias operativas y definir acciones correctivas que permitan mantener la continuidad del flujo.",
    "Coordinar horarios, horas extras, vacaciones y necesidades operativas del personal a cargo.",
    "Realizar seguimiento a pedidos y despachos, verificando el cumplimiento de fechas, cantidades y condiciones acordadas.",
    "Mantener comunicación permanente con el cliente, atendiendo requerimientos y asegurando el cumplimiento de los niveles de servicio establecidos.",
    "Participar en auditorías, inventarios y actividades de mejora continua de la operación.",
    "Asegurar el correcto manejo documental y la trazabilidad de los movimientos realizados dentro del almacén."
   ],
   "requisitos": [
    "Profesional universitario de las carreras de Ingeniería Industrial, Administración, Logística o carreras afines.",
    "Experiencia previa liderando operaciones logísticas y/o de almacén.",
    "Experiencia indispensable en operaciones vinculadas al sector cosmético .",
    "Experiencia gestionando procesos de recepción, almacenamiento, inventarios, picking y despacho.",
    "Experiencia liderando equipos operativos y/o administrativos.",
    "Conocimiento de indicadores de gestión logística y control de inventarios.",
    "Manejo de SAP .",
    "Manejo de WMS (Warehouse Management System) .",
    "Disponibilidad para trabajar de manera presencial en Lurín .",
    "Deseable experiencia participando en implementaciones, aperturas o arranque de nuevas operaciones logísticas.",
    "Deseable experiencia en operadores logísticos o centros de distribución."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Remuneración fija de S/5,000 .",
    "Alimentación: S/60.50 .",
    "Asignación de movilidad: S/105.50 .",
    "Bono adicional de S/100 para colaboradores que residan fuera de Lurín.",
    "Bus de acercamiento desde el Km 40 hacia la operación .",
    "Oportunidad de incorporarse a una operación logística de gran escala para una reconocida compañía multinacional del sector belleza y cosmética."
   ],
   "url": "https://www.bumeran.com.pe/empleos/coordinadora-de-operaciones-logisticas-experiencia-rubro-cosmetica-y-o-belleza-grafton-latam-1118421381.html"
  },
  {
   "id": 274,
   "puesto": "Operador",
   "empresa": "GRAFTON LATAM",
   "cat": "Logística",
   "min": 5000,
   "max": 5000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 28,
   "vence": "2026-11-27",
   "restan": 62,
   "score": 97,
   "resumen": "Por encargo de nuestro cliente, importante operador logístico con amplia presencia en el mercado peruano, nos encontramos en búsqueda de un(a)",
   "funciones": [
    "Supervisar el correcto desarrollo de los procesos de recepción, almacenamiento, picking, abastecimiento, inventarios y despacho.",
    "Coordinar y organizar los recursos necesarios para asegurar la continuidad y eficiencia de la operación.",
    "Liderar y realizar seguimiento al personal administrativo, supervisores y personal operativo asignado.",
    "Monitorear el cumplimiento de procedimientos, instructivos de trabajo y estándares establecidos para la operación.",
    "Gestionar inventarios rotativos y generales, asegurando la correcta correspondencia entre el stock físico y lo registrado en sistema.",
    "Realizar seguimiento a los indicadores operativos relacionados con ingresos, salidas, pedidos, inventarios, capacidad, productividad y nivel de servicio.",
    "Gestionar incidencias operativas y definir acciones correctivas que permitan mantener la continuidad del flujo.",
    "Coordinar horarios, horas extras, vacaciones y necesidades operativas del personal a cargo.",
    "Realizar seguimiento a pedidos y despachos, verificando el cumplimiento de fechas, cantidades y condiciones acordadas.",
    "Mantener comunicación permanente con el cliente, atendiendo requerimientos y asegurando el cumplimiento de los niveles de servicio establecidos.",
    "Participar en auditorías, inventarios y actividades de mejora continua de la operación.",
    "Asegurar el correcto manejo documental y la trazabilidad de los movimientos realizados dentro del almacén."
   ],
   "requisitos": [
    "Profesional universitario de las carreras de Ingeniería Industrial, Administración, Logística o carreras afines.",
    "Experiencia previa liderando operaciones logísticas y/o de almacén.",
    "Experiencia indispensable en operaciones vinculadas al sector cosmético .",
    "Experiencia gestionando procesos de recepción, almacenamiento, inventarios, picking y despacho.",
    "Experiencia liderando equipos operativos y/o administrativos.",
    "Conocimiento de indicadores de gestión logística y control de inventarios.",
    "Manejo de SAP .",
    "Manejo de WMS (Warehouse Management System) .",
    "Disponibilidad para trabajar de manera presencial en Lurín .",
    "Deseable experiencia participando en implementaciones, aperturas o arranque de nuevas operaciones logísticas.",
    "Deseable experiencia en operadores logísticos o centros de distribución."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Remuneración fija de S/5,000 .",
    "Alimentación: S/60.50 .",
    "Asignación de movilidad: S/105.50 .",
    "Bono adicional de S/100 para colaboradores que residan fuera de Lurín.",
    "Bus de acercamiento desde el Km 40 hacia la operación .",
    "Oportunidad de incorporarse a una operación logística de gran escala para una reconocida compañía multinacional del sector belleza y cosmética."
   ],
   "url": "https://www.bumeran.com.pe/empleos/lider-de-operaciones-logisticas-experiencia-rubro-cosmetica-y-o-belleza-copia-grafton-latam-1118421384.html"
  },
  {
   "id": 275,
   "puesto": "Ejecutivo de Operaciones Logísticas",
   "empresa": "GRAFTON LATAM",
   "cat": "Logística",
   "min": 5000,
   "max": 5000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 28,
   "vence": "2026-11-27",
   "restan": 62,
   "score": 97,
   "resumen": "Por encargo de nuestro cliente, importante operador logístico con amplia presencia en el mercado peruano, nos encontramos en búsqueda de un(a)",
   "funciones": [
    "Supervisar el correcto desarrollo de los procesos de recepción, almacenamiento, picking, abastecimiento, inventarios y despacho.",
    "Coordinar y organizar los recursos necesarios para asegurar la continuidad y eficiencia de la operación.",
    "Liderar y realizar seguimiento al personal administrativo, supervisores y personal operativo asignado.",
    "Monitorear el cumplimiento de procedimientos, instructivos de trabajo y estándares establecidos para la operación.",
    "Gestionar inventarios rotativos y generales, asegurando la correcta correspondencia entre el stock físico y lo registrado en sistema.",
    "Realizar seguimiento a los indicadores operativos relacionados con ingresos, salidas, pedidos, inventarios, capacidad, productividad y nivel de servicio.",
    "Gestionar incidencias operativas y definir acciones correctivas que permitan mantener la continuidad del flujo.",
    "Coordinar horarios, horas extras, vacaciones y necesidades operativas del personal a cargo.",
    "Realizar seguimiento a pedidos y despachos, verificando el cumplimiento de fechas, cantidades y condiciones acordadas.",
    "Mantener comunicación permanente con el cliente, atendiendo requerimientos y asegurando el cumplimiento de los niveles de servicio establecidos.",
    "Participar en auditorías, inventarios y actividades de mejora continua de la operación.",
    "Asegurar el correcto manejo documental y la trazabilidad de los movimientos realizados dentro del almacén."
   ],
   "requisitos": [
    "Profesional universitario de las carreras de Ingeniería Industrial, Administración, Logística o carreras afines.",
    "Experiencia previa liderando operaciones logísticas y/o de almacén.",
    "Experiencia indispensable en operaciones vinculadas al sector cosmético .",
    "Experiencia gestionando procesos de recepción, almacenamiento, inventarios, picking y despacho.",
    "Experiencia liderando equipos operativos y/o administrativos.",
    "Conocimiento de indicadores de gestión logística y control de inventarios.",
    "Manejo de SAP .",
    "Manejo de WMS (Warehouse Management System) .",
    "Disponibilidad para trabajar de manera presencial en Lurín .",
    "Deseable experiencia participando en implementaciones, aperturas o arranque de nuevas operaciones logísticas.",
    "Deseable experiencia en operadores logísticos o centros de distribución."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Remuneración fija de S/5,000 .",
    "Alimentación: S/60.50 .",
    "Asignación de movilidad: S/105.50 .",
    "Bono adicional de S/100 para colaboradores que residan fuera de Lurín.",
    "Bus de acercamiento desde el Km 40 hacia la operación .",
    "Oportunidad de incorporarse a una operación logística de gran escala para una reconocida compañía multinacional del sector belleza y cosmética."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-de-operaciones-logisticas-experiencia-rubro-cosmetica-y-o-belleza-copia-grafton-latam-1118421465.html"
  },
  {
   "id": 276,
   "puesto": "Atencion al Cliente en Grifo Primax al Costado del Jockey Plaza",
   "empresa": "COESTI S.A.",
   "cat": "Atención al Cliente",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 97,
   "resumen": "Si quieres pertenecer al Grupo Primax y formar parte de un equipo lleno de energía y pasión te invitamos a postular a la posición de DESPACHO DE COMBUSTIBLE para que cumplas las siguientes funciones:",
   "funciones": [
    "Atención al clientes de manera oportuna en despachando combustible",
    "Controlar los Boucher, vales y efectivo de las ventas de los dispensadores que le son asignados",
    "Realizar el reporte de ventas al término del turno (Cuadre de caja)",
    "Cumplir con las normas de seguridad y salud en el trabajo",
    "Mantener la EESS limpia y ordenada"
   ],
   "requisitos": [
    "Secundaria Completa.",
    "Experiencia en atención al cliente (deseable)",
    "Manejo de caja (deseable)",
    "Disponibilidad para laborar en turno tarde de 3 pm a 11 pm o turno rotativo",
    "Disponibilidad para laborar al costado del Jockey Plaza"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer dia de trabajo",
    "Contrato indeterminado",
    "Sueldo aproximado de S/1300 (incluida la movilidad)",
    "Bono de movilidad de S/100",
    "Asignación familiar",
    "Bonos por ventas",
    "Pago de Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/atencion-al-cliente-en-grifo-primax-al-costado-del-jockey/6a924bf22020b210c83e9c04"
  },
  {
   "id": 277,
   "puesto": "Atención al Cliente",
   "empresa": "Franquicias Unidas del Perú SAC",
   "cat": "Atención al Cliente",
   "min": 565,
   "max": 565,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 97,
   "resumen": "Buscamos EMBAJADORES DE EXPERIENCIA MULTIFUNCIONALES PART TIME y FULL TIME para nuestras tiendas en PLAZA SAN MIGUEL",
   "funciones": [
    "Preparar productos con calidad: Helados gourmet en Gelarti y Juan Valdez Café",
    "Brindar un servicio memorable: Asesorar, atender y conectar con los clientes de manera auténtica y amable.",
    "Manejo de caja básica: Apoyar en el cobro y procesos simples de atención.",
    "Cuidar el ambiente de tienda: Mantener el espacio ordenado, limpio y siempre presentable.",
    "Apoyar al equipo: Colaborar en tareas operativas y mantener el ritmo de la tienda.",
    "Impulsar la marca: Promocionar productos y participar activamente en la experiencia del cliente."
   ],
   "requisitos": [
    "Con o sin experiencia (nosotros te capacitamos).",
    "Mucha disposición para aprender y buena actitud.",
    "Excelente trato al cliente, comunicación y energía positiva.",
    "Disponibilidad para trabajar en horarios rotativos (incluye fines de semana).",
    "Disponibilidad para tramitar el carnet de sanidad (no es necesario tenerlo para postular, solo la disposición para sacarlo si eres seleccionado/a)."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día con todos los beneficios de ley.",
    "Contrato directo con la compañía.",
    "Sueldo base para PART TIME : S/ 565.00 + S/ 25.00 (bono de movilidad)",
    "Sueldo base para FULL TIME : S/ 1,130.00 + S/ 50.00 (bono de movilidad)",
    "Descuentos exclusivos en productos en ambas marcas.",
    "Capacitación constante.",
    "Línea de carrera: aquí puedes crecer de verdad.",
    "Ambiente profesional y con buena vibra."
   ],
   "url": "https://www.laborum.pe/job/franquicias-unidas-del-peru-sac/multifuncionales-part-y-full-time-en-gelarti-y-juan-valdez/6a9267082020b210c83ece46"
  },
  {
   "id": 278,
   "puesto": "Operario de Producción",
   "empresa": "GRAFTON PERU S.A.C.",
   "cat": "Ingeniería",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 97,
   "resumen": "Abastecer máquinas y equipos con materia prima e insumos necesarios para la producción.",
   "funciones": [
    "Abastecer máquinas y equipos con materia prima e insumos necesarios para la producción.",
    "Realizar el cargado, descargado y traslado manual de piezas metálicas entre las diferentes áreas de producción.",
    "Verificar que todas las piezas conformes sean procesadas correctamente y separar las piezas no conformes según los procedimientos establecidos.",
    "Manipular y transportar materiales, productos terminados y residuos de producción dentro de la planta.",
    "Trasladar la chatarra generada durante el proceso productivo hacia las zonas de almacenamiento de merma.",
    "Realizar el lavado y limpieza de piezas metálicas para la eliminación de aceite, grasa y residuos industriales.",
    "Mantener el orden y la limpieza del área de trabajo, herramientas y equipos asignados."
   ],
   "requisitos": [
    "Experiencia mínima de 3 meses como operario (En almacén o producción).",
    "Se puede considerar con secundaria incompleta.",
    "Disponibilidad para trabajar en turnos rotativos.",
    "Disponibilidad para laborar en la planta ubicada en Ate.",
    "Responsabilidad, puntualidad, proactividad, trabajo en equipo, con ganas de aprender y crecer."
   ],
   "beneficios": [
    "Sueldo base: S/. 1,200 + Bono de movilidad S/. 104 (sujeto a asistencia) + Bono de alimentos S/. 50 (en tarjeta Provis)",
    "Bono por área: Prensas S/. 80.",
    "Bono por turno tarde: S/. 25 Este bono lo recibirá siempre cuando realice el horario de 3:15pm a 11pm (15 días cumplidos mes).",
    "Pagos quincenales.",
    "Planilla completa desde el primer día con todos los beneficios de ley.",
    "Utilidades un sueldo aproximadamente",
    "Gratificaciones de un sueldo aprox de acuerdo con su fecha de ingreso (julio y Diciembre)",
    "Seguro de Essalud",
    "Se les brinda presente por cumpleaños.",
    "Actividades de integración (día de la madre, día del padre, día del trabajo, fiestas patrias y navidad)",
    "Horas extras remuneradas.",
    "Información Adicional"
   ],
   "url": "https://www.laborum.pe/job/grafton-peru-s-a-c/operario-de-produccion-ate-s-1-459-pago-quincenal-cantol/6a9279e92020b210c83ee7bc"
  },
  {
   "id": 279,
   "puesto": "Asesor de Cobranzas c",
   "empresa": "Biznes Sac",
   "cat": "Contabilidad",
   "min": 1230,
   "max": 1230,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 94,
   "resumen": "Realizar llamadas de cobranza a clientes con deudas vencidas.",
   "funciones": [
    "Realizar llamadas de cobranza a clientes con deudas vencidas.",
    "Contactar a clientes para la verificación de pagos.",
    "Negociar alternativas de pago y acuerdos con clientes morosos.",
    "Brindar información clara y mantener la calidad en cada gestión realizada.",
    "Registrar las gestiones efectuadas y elaborar reportes de incidencias.",
    "Cumplir con los objetivos e indicadores establecidos por el área."
   ],
   "requisitos": [
    "Ser mayor de 18 años.",
    "Contar con secundaria completa o estudios superiores en curso o inconclusos.",
    "Experiencia en cobranzas del rubro financiero (tarjetas de crédito, préstamos, entre otros). No indispensable.",
    "Actitud positiva, compromiso, responsabilidad y orientación a resultados.",
    "Facilidad de comunicación y capacidad de negociación."
   ],
   "beneficios": [
    "Sueldo desde S/ 1,230.",
    "Incentivos y comisiones ilimitadas.",
    "Ingreso a planilla MYPE desde el primer día (vacaciones, gratificaciones, CTS, EsSalud, AFP y asignación familiar, según corresponda).",
    "Bono por puntualidad.",
    "Bono por referidos.",
    "Ingreso inmediato.",
    "Oportunidad de crecimiento y línea de carrera a corto plazo.",
    "Pagos puntuales.",
    "Excelente clima laboral.",
    "Descanso los domingos y feriados.",
    "Horario de trabajo",
    "Lunes a viernes: 8:00 a. m. a 6:00 p. m."
   ],
   "url": "https://www.laborum.pe/job/biznes-sac/asesor-de-cobranzas-c-s-experiencia-interbank-planilla/6a9254b82020b210c83eb355"
  },
  {
   "id": 280,
   "puesto": "Supervisor HSE",
   "empresa": "Overall Strategy",
   "cat": "Ingeniería",
   "min": 5500,
   "max": 5500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 92,
   "resumen": "RESUMEN: ¡En Overall creemos que tu talento merece la mejor oportunidad!",
   "funciones": [
    "Supervisar el cumplimiento de los estándares HSE durante la ejecución del proyecto.",
    "Verificar la implementación de controles de seguridad y medio ambiente en campo.",
    "Revisar y aprobar documentación HSE de contratistas (IPERC, ICAAS, procedimientos, instructivos, entre otros).",
    "Dictar capacitaciones e inducciones en Seguridad, Salud Ocupacional y Medio Ambiente.",
    "Participar en investigaciones de incidentes y seguimiento de acciones correctivas.",
    "Supervisar el cumplimiento de los compromisos ambientales establecidos en el proyecto.",
    "Elaborar y controlar el programa HSE del proyecto.",
    "Coordinar permanentemente con contratistas y la supervisión del proyecto."
   ],
   "requisitos": [
    "Ingeniero Ambiental, Industrial, Eléctrico, Mecánico Eléctrico, Higiene y Seguridad Industrial o carreras afines.",
    "Colegiado y habilitado (indispensable).",
    "Experiencia mínima de 5 años en posiciones similares.",
    "Haber participado en al menos 3 proyectos eléctricos de alta tensión.",
    "Conocimiento de Ley 29783, D.S. 005-2012-TR, ISO 45001, ISO 14001 y normativa del sector eléctrico.",
    "Experiencia en supervisión HSE de contratistas y gestión documentaria.",
    "Deseable manejo de Power BI."
   ],
   "beneficios": [
    "Remuneración: S/ 5,500.",
    "Régimen 35x7.",
    "Planilla con todos los beneficios de ley.",
    "Alimentación cubierta (S/ 60 diarios).",
    "Hospedaje cubierto (S/ 60 diarios).",
    "Traslado terrestre cubierto por la empresa.",
    "Excelente oportunidad de desarrollo en proyectos del sector energético.",
    "PROYECTO : CONTRATO 1 MES",
    "Lugar de trabajo: Proyecto Coya San Ramón, Oxapampa.-Requerimientos- Educación mínima: Universitario5 años de experienciaPalabras clave: residente, encargado, supervisor, subgerente, responsable, coordinador, gestor, capitan, project"
   ],
   "url": "https://www.laborum.pe/job/overall-strategy/supervisor-hse-lineas-de-transmision-de-alta-tension/6a9232c82020b210c83e5d20"
  },
  {
   "id": 281,
   "puesto": "Vendedor Salaverry",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ventas",
   "min": 1330,
   "max": 1330,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 28,
   "vence": "2026-11-27",
   "restan": 62,
   "score": 92,
   "resumen": "Asesoramiento a clientes en búsqueda de los diversos productos.",
   "funciones": [
    "Asesoramiento a clientes en búsqueda de los diversos productos.",
    "Alcanzar los objetivos de ventas establecidos por la empresa.",
    "Estar actualizado en los nuevos ingresos de productos",
    "Apoyar en el orden de los productos y visual de la tienda."
   ],
   "requisitos": [
    "Hombres o mujeres de nacionalidad peruana entre 22 y 36 años.",
    "Tener experiencia mínima de 1 año como vendedor en tiendas retail de centros comerciales o boutiques puerta a calle.",
    "Excelente trato y orientación al cliente.",
    "Inicio a la brevedad posible.",
    "Disponibilidad para trabajar 48 horas a la semana, 6 días x 8 horas diarias."
   ],
   "beneficios": [
    "Grato ambiente laboral.",
    "Beneficios laborales de planilla completa desde el 1er día de trabajo. 2 gratificaciones completas, 2 CTS completas, 30 días de vacaciones al año, seguro de vida, seguro de Essalud y asignación familiar.",
    "Sueldo básico (S/.1330) + comisiones (en promedio van desde los S/.800 a mas y sin tope).",
    "Descuentos en productos de la marca.",
    "Reparto de utilidades.",
    "Posibilidad de hacer línea de carrera."
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedor-salaverry-adn-retail-s.a.c.-1118421447.html"
  },
  {
   "id": 282,
   "puesto": "Coordinador",
   "empresa": "Lavoro Perú",
   "cat": "Ingeniería",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 91,
   "resumen": "Nuestro cliente, importante empresa localizada en el mercado de LIMA, destacada en el rubro de INDUSTRIAL, se encuentra en la búsqueda del/ los mejor(es) talento(s) para ocupar el puesto LIDER OPERATIVO - SEDE CHACLACAYO.",
   "funciones": [
    "Distribuir al personal.",
    "Elaborar y reportar órdenes de producción",
    "Desplegar charlas de 5 minutos a los trabajadores",
    "Asegurar los materiales y productos en la zona de trabajo.",
    "Validar la calidad del trabajo y cumplimiento de los procedimientos",
    "Elaborar reportes de la actividad ejecutada.",
    "Coordinar actividades operativas con el cliente",
    "Supervisar las actividades que realiza el personal en planta.",
    "Realizar otras funciones afines al puesto que le asigne su superior inmediato",
    "HORARIO DE TRABAJO",
    "Horarios: MAÑANA 7am 3pm / TARDE 3pm 11pm/ NOCHE 11pm - 7:00am",
    "Modalidad 6x1 Lunes a domingo con un día de descanso rotativo programados con operaciones."
   ],
   "requisitos": [
    "Experiencia mínima 1 año en rubro INDUSTRIAL como Coordinador o Líder",
    "Manejo office - Nivel Intermedio",
    "Vivir aledaños a la zona de ATE"
   ],
   "beneficios": [
    "Sueldo: S/. 1400",
    "Bono de asistencia Perfecta S/. 93",
    "Bono de alimentación S/. 100",
    "Ingreso directo a planilla desde el primer día",
    "Beneficios completos de ley (CTS, gratificaciones, vacaciones, seguro, etc.)",
    "¡Estabilidad laboral y posibilidad de crecimiento!",
    "Somos una empresa que promueve la igualdad de oportunidades y no discriminamos por motivo de origen, edad, sexo, discapacidad u otra condición, asi como la inclusión laboral de personas con discapacidad."
   ],
   "url": "https://www.laborum.pe/job/lavoro-peru/lider-de-operaciones-sede-chaclacayo-planta-cervecera/6a92730b2020b210c83ed880"
  },
  {
   "id": 283,
   "puesto": "Call center",
   "empresa": "Entel Connect Center",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 91,
   "resumen": "RESUMEN: ¡Forma parte del Entel Connect (EC)!",
   "funciones": [
    "Contactar telefónicamente a clientes, para vender multiproductos de Pacífico Seguros, cumpliendo con los objetivos comerciales de la campaña.· Modalidad de trabajo",
    "Presencial : Av. España 382 - Cercado de Lima",
    "Horario laboral",
    "Lunes a sábado (1 día de descanso fijo : Domingo y feriados)",
    "Franja horaria : Mini Full.",
    "Lunes a Sábados Horarios disponibles a elección",
    "9:00 a . m - 3:45 p .m",
    "10:00 a. m - 4:45 p.m",
    "11:00 a. m. 5:45 p. m.",
    "12:00 p. m. 6:45 p. m.",
    "(solo 6 horas de trabajo efectivo + 45 minutos de break diario)"
   ],
   "requisitos": [
    "Disponibilidad para trabajar presencialmente en Cercado de Lima.",
    "Experiencia mínima de 3 meses en ventas o call center.",
    "Fluidez verbal, buena comunicación y orientación a resultados."
   ],
   "beneficios": [
    "Sueldo : S/. 1,130 + Comisiones ilimitadas",
    "Ingreso a planilla desde el primer día con todos los beneficios de ley.",
    "Cts, Gratificación",
    "Convenios corporativos.",
    "Capacitaciones pagadas.",
    "Linea de carrera.",
    "En Entel, tu desarrollo es nuestra prioridad.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: insurance, aseguradora, seguros"
   ],
   "url": "https://www.laborum.pe/job/entel-connect-center/trabaja-solo-6-horas-solo-4-dias-de-capa-asesor-a-de-seguros/6a922d252020b210c83e4e96"
  },
  {
   "id": 284,
   "puesto": "Operador de Almacen",
   "empresa": "Adecco Perú S.A.",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 89,
   "resumen": "RESUMEN: ¡¡¡DESCANSA LOS DOMINGOS!!! · Adecco es la empresa de Recursos Humanos más grande del mundo. Somos líderes en gestión humana. Diariamente ayudamos a más de 700,000 personas a encontrar trabajo a través de nuestra amplia red.",
   "funciones": [
    "Armado de pedidos paletizando mercadería (cajas de productos para supermercados de 5kg a 20 kg)",
    "Manejo de radiofrecuencia (RF).",
    "Trasladar paletas con Estoca"
   ],
   "requisitos": [
    "Disponibilidad para laborar en Punta Negra, Altura del Peaje de Punta negra (contamos con buses de acercamiento gratuitos para todo Lima)",
    "Disponibilidad laborar en horarios rotativos quincenal/ mensual (turno día: 7:00am a 4:00pm // turno tarde: 1:00 pm a 10:00 pm// turno noche: 10:00pm a 7:00am)",
    "Mucha actitud y ganas de aprender.",
    "Trabajo físico",
    "Disponibilidad para Manipular y trasladar productos de 20kg."
   ],
   "beneficios": [
    "Remuneración: S/1130",
    "Bono de movilidad: s/150",
    "Bono de Alimentación: S/150",
    "Bono Asistencia: s/100",
    "Pago de asignación familiar s/113 + horas extras + bonificación de horario nocturno",
    "Buses de acercamiento gratis en todo Lima.",
    "Ingreso a planilla Adecco desde el primer día de trabajo con todos los Beneficios de ley (CTS, Gratificación, Vacaciones, Seguro Social, Seguro Vida Ley, Asignación Familiar)",
    "Beneficios corporativos.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: operator, operario, operator, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/adecco-peru-s-a/operador-de-almacen-movilidad-gratis-alimentacion-bonos/6a923a6b2020b210c83e7013"
  },
  {
   "id": 285,
   "puesto": "Cajero",
   "empresa": "Eurofirms Perú",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 89,
   "resumen": "RESUMEN: En Eurofirms Group, creemos en el liderazgo por valores y el crecimiento continuo, poniendo a las personas y su desarrollo profesional primero, potenciando un ambiente de trabajo positivo y colaborativo.",
   "funciones": [
    "Atención al cliente y promoción de ventas, brindando un servicio ágil y de calidad.",
    "Manejo de caja, recepción y reposición de mercadería en tienda y almacén.",
    "Manipulación de alimentos, limpieza integral del local y apoyo en inventarios."
   ],
   "requisitos": [
    "Deseable contar con 3 meses de experiencia en atención al cliente.",
    "Contar con Carnet de Sanidad con Manipulación de Alimentos. (Indispensable)",
    "Contar con disponibilidad para desempeñar funciones operativas propias de tienda (trabajo de pie, carga ligera, desplazamiento constante, entre otros).",
    "Experiencia en cargos similares: atención al Cliente; cajero, reponedor y promoción de productos, y conocimientos básicos de ventas.",
    "(Turno Rotativo) De 07am a 03pm / De 03pm a 11pm / De 11pm a 07am",
    "Lunes a Domingo con 1 día de descanso entre lunes a jueves."
   ],
   "beneficios": [
    "Sueldo básico 1,130 soles.",
    "Bono objetivos de hasta 120 soles.",
    "Bonos por no contar con faltante de caja de 50 soles.",
    "Bono por Movilidad de S/ 150.",
    "Ingreso a planilla con todos los beneficios de ley desde el 1er día.",
    "Línea de Carrera.",
    "Si cumples con los requisitos, estás interesado y crees que eres el match perfecto, te invitamos a postular ¡Te esperamos!"
   ],
   "url": "https://www.laborum.pe/job/eurofirms-peru/cajero-a-multifuncional-miraflores-surco-o-san-borja/6a92574f2020b210c83eb87d"
  },
  {
   "id": 286,
   "puesto": "Auxiliar de Tienda Multifuncional",
   "empresa": "Eurofirms Perú",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 89,
   "resumen": "RESUMEN: En Eurofirms Group, creemos en el liderazgo por valores y el crecimiento continuo, poniendo a las personas y su desarrollo profesional primero, potenciando un ambiente de trabajo positivo y colaborativo.",
   "funciones": [
    "Atención al cliente y promoción de ventas, brindando un servicio ágil y de calidad.",
    "Manejo de caja, recepción y reposición de mercadería en tienda y almacén.",
    "Manipulación de alimentos, limpieza integral del local y apoyo en inventarios."
   ],
   "requisitos": [
    "Deseable contar con 3 meses de experiencia en atención al cliente.",
    "Contar con Carnet de Sanidad con Manipulación de Alimentos. (Indispensable)",
    "Contar con disponibilidad para desempeñar funciones operativas propias de tienda (trabajo de pie, carga ligera, desplazamiento constante, entre otros).",
    "Experiencia en cargos similares: atención al Cliente; cajero, reponedor y promoción de productos, y conocimientos básicos de ventas.",
    "(Turno Rotativo) De 07am a 03pm / De 03pm a 11pm / De 11pm a 07am",
    "Lunes a Domingo con 1 día de descanso entre lunes a jueves."
   ],
   "beneficios": [
    "Sueldo básico 1,130 soles.",
    "Bono objetivos de hasta 120 soles.",
    "Bonos por no contar con faltante de caja de 50 soles.",
    "Bono por Movilidad de S/ 150.",
    "Ingreso a planilla con todos los beneficios de ley desde el 1er día.",
    "Línea de Carrera.",
    "Si cumples con los requisitos, estás interesado y crees que eres el match perfecto, te invitamos a postular ¡Te esperamos!"
   ],
   "url": "https://www.laborum.pe/job/eurofirms-peru/auxiliar-de-tienda-multifuncional-miraflores-surco-o-san/6a9259642020b210c83ebd60"
  },
  {
   "id": 287,
   "puesto": "Promotora",
   "empresa": "Total Talent Human Resources",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 89,
   "resumen": "RESUMEN: En Total Talent, creemos firmemente en proporcionar oportunidades laborales sin COSTOS o PAGOS asociados para los postulantes.",
   "funciones": [
    "Asesorar al cliente en la compra del producto.",
    "Gestionar el abastecimiento de los productos en tienda.",
    "Gestionar la exhibición y orden de productos dentro del punto de trabajo."
   ],
   "requisitos": [
    "Experiencia certificada mínima desde 6 meses en general, como promotora, impulsadora, asesora, vendedora.",
    "Experiencia general en el rubro Retail/ventas presenciales o afines.",
    "Conocimiento de Excel a nivel básico o usuario.",
    "Se requiere disponibilidad para laborar en Mall Plaza Angamos y Real Plaza Primavera.",
    "Contar con disponibilidad completa para laborar 8 horas diarias en turnos rotativos de apertura o cierre según requerimiento de tienda.",
    "Horarios rotativos de lunes a domingo con 01 día de descanso."
   ],
   "beneficios": [
    "Sueldo fijo S/.1300 + Comisiones acorde a la venta.",
    "Ingreso a PLANILLA GENERAL desde el primer día de firma de contrato.",
    "Contrato directo con Leonisa con todos los beneficios desde el primer mes de trabajo y al 100% de acuerdo a ley (CTS, asignación familiar, vacaciones pagadas, gratificaciones, etc).",
    "Seguro Médico Particular EPS cubierto al 90%.",
    "Pago de UTILIDADES más altas del rubro.",
    "Pagos quincenales y puntuales.",
    "25% Descuentos Corporativos en todos los productos de la marca.",
    "Formar parte de una empresa internacional, sólida y líder en el rubro."
   ],
   "url": "https://www.laborum.pe/job/total-talent-human-resources/asesora-vendedora-promotora-leonisa-comisiones-planilla/6a924ec92020b210c83ea352"
  },
  {
   "id": 288,
   "puesto": "Técnica",
   "empresa": "Mind selection",
   "cat": "Logística",
   "min": 1470,
   "max": 1470,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 28,
   "vence": "2026-11-27",
   "restan": 62,
   "score": 89,
   "resumen": "En Molinos del Triunfo nos encontramos en la búsqueda de un Montacarguista comprometido y responsable para formar parte de nuestro equipo.",
   "funciones": [
    "Recepcionar y trasladar los productos terminados desde las líneas de producción hacia el almacén, respetando la zonificación y las Buenas Prácticas de Almacenamiento (BPA).",
    "Almacenar correctamente los productos terminados verificando la cantidad, el correcto paletizado y su conservación.",
    "Realizar el picking de productos aplicando la metodología FIFO.",
    "Apoyar en el despacho de productos terminados.",
    "Inspeccionar el montacargas al inicio de cada turno, mantenerlo limpio y reportar cualquier desperfecto.",
    "Realizar el relevo de turno, entregando el equipo, documentos e información correspondiente.",
    "Participar en los inventarios del área."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Licencia de conducir A1 vigente.",
    "Certificación técnica como Montacarguista u Operador de Equipos Pesados.",
    "Experiencia mínima de 1 año certificada como montacarguista.",
    "Tipo de trabajo",
    "Jornada de lunes a domingo.",
    "3 turnos rotativos.",
    "Turnos de 8 horas de trabajo + 45 minutos de refrigerio.",
    "Descanso de acuerdo con la programación."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,470 .",
    "Provisión de alimentos: S/ 170 .",
    "Participación en utilidades.",
    "Ingreso a planilla con todos los beneficios de ley.",
    "Excelente ambiente laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/montacarguista-experiencia-certificada-molinos-del-triunfo-sede-callao-mind-selection-1118421259.html"
  },
  {
   "id": 289,
   "puesto": "Solo Atención al cliente para Banco Nacional",
   "empresa": "A365",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 88,
   "resumen": "RESUMEN: Somos la empresa ImpulsA365 y por encargo de nuestro importante cliente, nos encontramos en búsqueda de un Asesor de Atención en prevención de fraude.",
   "funciones": [
    "Realizar el monitoreo de las operaciones.",
    "Identificar posibles fraudes financieros.",
    "Levantar alertas sobre los posibles casos de fraude.",
    "Horario de capacitación",
    "Duración : 5 días (PAGADAS: 30 soles por cada día de capacitación / 1 mes de desfase )"
   ],
   "requisitos": [
    "No contar con deudas crediticias en estado de pérdida o posiblemente pérdida",
    "Capacidad de Análisis y Resolución de Problemas",
    "Excel nivel Básico / Intermedio",
    "Disponibilidad para trabajar en un rango de 12pm a 10pm (8hrs de gestión + 1hr de break)"
   ],
   "beneficios": [
    "Sueldo base: S/1130",
    "Bono de adherencia de S/120",
    "Bono de indicadores de hasta: S/300",
    "Planilla completa- régimen genera al 100% (Todos los beneficios de ley)",
    "Recibirá talleres gratuitos por parte del CEEC para potenciar su talento.",
    "Linea de carrera a partir de 4to mes: En base a indicadores (Formadores, Team Leaders, etc)",
    "Descuentos corporativos· Lugar de trabajo"
   ],
   "url": "https://www.laborum.pe/job/a365/solo-atencion-al-cliente-para-banco-nacional-turno-pm/6a9236ca2020b210c83e6549"
  },
  {
   "id": 290,
   "puesto": "Solo atención al cliente chat y voz",
   "empresa": "Atento Perú",
   "cat": "Atención al Cliente",
   "min": 540,
   "max": 540,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 87,
   "resumen": "RESUMEN: Atento Perú, líder en soluciones de experiencia del cliente y atención al cliente en América Latina, está buscando un Asesor(a) FULL Atención al cliente Falabella de voz y chat para unirse a nuestro equipo.",
   "funciones": [
    "Brindar soporte a los clientes (por llamada o chat) por pedidos que generaron en la página de Falabella",
    "Lunes a sábados en un rango de 9AM a 7PM ( 8 horas de gestión + 1 hora break)",
    "Descanso fijo domingo"
   ],
   "requisitos": [
    "Secundaria completa",
    "Experiencia 2 meses en atención al cliente presencial (cualquier rubro)",
    "Disponibilidad para trabajar de manera presencial (Referencia: 2 cuadras del Óvalo Santa Anita)"
   ],
   "beneficios": [
    "Sueldo base: S/1 130 + tarjeta de alimentos",
    "Planilla completa desde el primer día: CTS, gratificación, asignación familiar y utilidades.",
    "Capacitaciones totalmente pagadas S/540"
   ],
   "url": "https://www.laborum.pe/job/atento-peru/solo-atencion-al-cliente-chat-y-voz-descansa-fijo-el-domingo/6a924a312020b210c83e9701"
  },
  {
   "id": 291,
   "puesto": "Call Center",
   "empresa": "FAGUEL & JESS EIRL",
   "cat": "Ventas",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 87,
   "resumen": "RESUMEN: Asesor(a) Call Center Turno Mañana | Sin experiencia | Los Olivos",
   "funciones": [
    "Contactar clientes para ofrecer renovación de equipos de telefonía e internet",
    "Brindar información clara y adecuada sobre los servicios",
    "Gestionar consultas y seguimiento de clientes",
    "Verificar cobertura y actualizar datos en el sistema"
   ],
   "requisitos": [
    "Secundaria completa",
    "Disponibilidad inmediata",
    "No se requiere experiencia"
   ],
   "beneficios": [
    "Sueldo base: S/ 1400 FIJO",
    "Comisiones ilimitadas",
    "Incremento salarial a partir del 6.º mes",
    "Capacitación corta y pagada",
    "Buen clima laboral",
    "Oportunidades de crecimiento",
    "Modalidad de contratación: Planilla REMYPE o Recibos por Honorarios"
   ],
   "url": "https://www.laborum.pe/job/faguel-jess-eirl/sin-experiencia-asesor-comercial-capacitacion-de-3-dias/6a9277462020b210c83ee25f"
  },
  {
   "id": 292,
   "puesto": "Atención al Cliente",
   "empresa": "TALENTOS CQ E.I.R.L.",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 86,
   "resumen": "RESUMEN: Asesor(a) de Atención al Cliente LATAM",
   "funciones": [
    "Atender consultas y solicitudes de los clientes.",
    "Brindar información sobre vuelos, reservas y servicios.",
    "Gestionar consultas y reclamos de acuerdo con los procedimientos establecidos.",
    "Ofrecer una atención cordial y orientada al cliente."
   ],
   "requisitos": [
    "Con o sin experiencia.",
    "Secundaria completa.",
    "Disponibilidad para laborar de manera presencial.",
    "Buena comunicación y orientación al cliente."
   ],
   "beneficios": [
    "Sueldo fijo de S/ 1,130.",
    "Bono variable de hasta S/ 300 por cumplimiento de indicadores.",
    "Bono de bienvenida sujeto al cumplimiento de las condiciones establecidas por la empresa.",
    "Capacitación previa al inicio de funciones.",
    "Ingreso a planilla completa con todos los beneficios de ley.",
    "Contrato renovable.",
    "Modalidad: Presencial.",
    "Lugar de trabajo: Ex Hotel Crillón, Cercado de Lima.",
    "Firma de contrato al día 15 del proceso de formación.",
    "Si deseas desarrollarte en el área de atención al cliente, postula y forma parte de nuestro equipo.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 70 añosPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/talentos-cq-e-i-r-l/trabaja-ya-con-o-sin-experiencia-en-atencion-al-cliente-en/6a9274ff2020b210c83edd7d"
  },
  {
   "id": 293,
   "puesto": "Asesor de Repuestos",
   "empresa": "Grupo Empresarial Espinoza",
   "cat": "Ventas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 86,
   "resumen": "En la actualidad nos encontramos en búsqueda de un ASESOR DE REPUESTOS en nuestra concesionaria de Toyota.",
   "funciones": [
    "Gestionar y fidelizar la cartera de clientes asignada.",
    "Identificar nuevas oportunidades de negocio y captar clientes.",
    "Asesorar técnicamente sobre el stock de repuestos disponibles.",
    "Cumplir con las metas de venta mensuales estipuladas.",
    "Gestionar cobranzas de su cartera"
   ],
   "requisitos": [
    "Mínima de 1 a 2 años en ventas de repuestos o sector afín.",
    "Contar con brevete A1 (Deseable)",
    "Dominio de herramientas de Ofimática",
    "Manejo de programas EPC"
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley",
    "Excelente clima laboral.",
    "Atractivo esquema por cumplimiento de metas.",
    "Monto salarial: S/1500+COMISIONES",
    "Sede: San Luis. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaLicencias de conducir: A-I"
   ],
   "url": "https://www.laborum.pe/job/grupo-empresarial-espinoza/asesores-de-repuestos-toyota/6a936fe02020b210c83f5ae4"
  },
  {
   "id": 294,
   "puesto": "Sin experiencia! Anfitrion(a) de estacionamiento para Wong Benavides",
   "empresa": "APPARKA",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 28,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 86,
   "resumen": "RESUMEN: En apparka nos encontramos en búsqueda del mejor talento para el puesto de Anfitrión de Estacionamientos.",
   "funciones": [
    "Brindar una excelente atención a nuestros clientes, asegurando una experiencia agradable desde su llegada.",
    "Facilitar la entrada y salida de los vehículos en nuestras instalaciones.",
    "Colaborar con un equipo comprometido en garantizar la seguridad y el buen funcionamiento de cada estacionamiento.",
    "Utilizar nuestras herramientas tecnológicas para hacer el trabajo más eficiente.",
    "Disponibilidad para laborar en el turno tarde en horario fijo."
   ],
   "requisitos": [
    "Personas con buena actitud, orientadas al servicio y dispuestas a aprender.",
    "Experiencia previa en atención al cliente (no es indispensable, ¡nosotros te capacitamos!).",
    "Disponibilidad para trabajar en turnos rotativos.",
    "Compromiso con el trabajo en equipo y la puntualidad."
   ],
   "beneficios": [
    "Sueldo: S/. 1130.00 mensual",
    "Ingreso a planilla completa desde el primer día",
    "¡Oportunidad de línea de carrera a corto plazo!"
   ],
   "url": "https://www.laborum.pe/job/apparka/sin-experiencia-anfitrion-a-de-estacionamiento-para-wong/6a9224c82020b210c83e34b1"
  },
  {
   "id": 295,
   "puesto": "Promotor para marca de audifonos tiendas retail",
   "empresa": "Confidencial",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 28,
   "vence": "2026-11-27",
   "restan": 62,
   "score": 82,
   "resumen": "Importante empresa representante de la marca Sleve en Perú se encuentra en la búsqueda de promotores de ventas con experiencia en tiendas Retai l, con conocimiento en accesorios tecnológicos (audifonos)",
   "funciones": [
    "Impulsar la venta de los productos y accesorios de la marca.",
    "Fidelizar a la fuerza de ventas del punto asignado",
    "Enviar reportes fotográficos, stock y ventas semanales"
   ],
   "requisitos": [
    "Experiencia en Tecnología, audífonos, celulares, computo o similares",
    "Experiencia de 6 meses en puestos similares. En tiendas Retail o supermercados.",
    "Disponibilidad inmediata y a tiempo completo.",
    "HORARIOS DE LUNES A DOMINGO CON UN DÍA DE DESCANSO ROTATIVO : 48 horas semanales."
   ],
   "beneficios": [
    "Horario: Lunes a domingo, de 12:00 p. m. a 9:00 p. m.",
    "Descanso: Miércoles",
    "Remuneración: S/ 1,130 mensuales",
    "Movilidad: S/ 250 mensuales",
    "Bono por uso de celular: S/ 50",
    "Mensuales Ingreso mensual total: S/ 1,430"
   ],
   "url": "https://www.bumeran.com.pe/empleos/promotor-para-marca-de-audifonos-tiendas-retail-s-1430-mensual-1118421275.html"
  },
  {
   "id": 296,
   "puesto": "Operador de la",
   "empresa": "Talento Humano Perú",
   "cat": "Ingeniería",
   "min": 3000,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 29,
   "vence": "2026-11-26",
   "restan": 61,
   "score": 97,
   "resumen": "Nuestro cliente, servicios de Operación y Mantenimiento Industrial, nos ha encargado la búsqueda de dos (2) Técnicos Mantenedor de Operación Continua.",
   "funciones": [
    "Garantizar la eficiencia y disponibilidad del sistema de toma de muestras automatizado y subsistemas asociados. Asegurar la calidad y seguridad en el mantenimiento preventivo y correctivo del sistema de toma de muestras automatizado, conforme a procedimientos, normativas y plan de mantenimiento",
    "Inspeccionar el sistema automatizado para asegurar el cumplimiento del plan de mantenimiento y los estándares de calidad.",
    "Atender fallas e indicaciones de manera segura y eficiente, cumpliendo las normas de seguridad y salud ocupacional.",
    "Mantener informado al operador de la consola del sistema automatizado sobre el estado de la operación vista desde campo.",
    "Verificar la calidad de las muestras y prevenir la contaminación de estas, asegurando el cumplimiento de normativas ambientales y de calidad.",
    "Identificar, reportar y mitigar riesgos laborales dentro del área de operación y mantenimiento del sistema robotizado.",
    "Apoyar en la operación cuando se desempeñe en el horario diurno y realizar el mantenimiento en el horario nocturno.",
    "Mantener una cultura de aprendizaje continuo, promoviendo el autoaprendizaje, la actualización de conocimientos y su transferencia efectiva a nuevos colaboradores."
   ],
   "requisitos": [
    "Egresado de carrera Técnica en Mantenimiento Industrial, Electricidad, Electricidad-Mecánica o Instrumentación.",
    "Experiencia mínima de tres (03) años de experiencia en la operación y mantenimiento de sistemas o maquinas industriales o automatizadas.",
    "Conocimientos en mantenimiento de sistemas mecánicos, eléctricos y/o electrónicos.",
    "Conocimientos en seguridad, salud ocupacional y medio ambiente.",
    "Ms. Office, nivel básico. Elaboración de informes técnicos (excluyente)"
   ],
   "beneficios": [
    "Planilla general, con todos los beneficios de acuerdo a Ley.",
    "Remuneración entre S/. 3,000.00 soles",
    "Se brinda Desayuno, Almuerzo y Cena, de acuerdo al horario de trabajo.",
    "Servicio de Lavandería (EPPs-Uniforme)",
    "Se brinda movilidad de acercamiento y salida de la planta (Ruta: Puente Nuevo, Puente Caqueta, Av. Universitaria, Sede Callao)",
    "LUGAR DE TRABAJO Y HORARIO",
    "Horario Rotativo Semanal: Trabaja 4 días, Descansa 3 días.",
    "Horario Diurno: 6:00 a.m. – 7:00 p.m.",
    "Horario Nocturno: 7:00 p.m. – 6:00 a.m.",
    "Modalidad Presencial.",
    "Las personas interesadas que cumplan con el perfil solicitado, enviar su cv. actualizado a la página web: https://www.talentohumanoperu.pe/postula-con-nosotros o al WhatsApp mediante este link https://wa.link/1v0ioy o al 907994423, indícanos el puesto al que postula : TÉCNICO MANTENEDOR DE OPERACIÓN CONTINUA."
   ],
   "url": "https://www.bumeran.com.pe/empleos/tecnicos-mantenedor-de-operacion-continua-talento-humano-peru-1118421018.html"
  },
  {
   "id": 297,
   "puesto": "Imaco! Promotor de ventas",
   "empresa": "Grupo Tawa",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 95,
   "resumen": "RESUMEN: Si te apasionan los retos y estar en la cancha ¡Este trabajo es para ti!",
   "funciones": [
    "Alcanzar los objetivos establecidas por tienda.",
    "Promover activamente los productos orientando en base a las necesidades del cliente final.",
    "Demostrar el uso adecuado de los productos y responder a las preguntas de los clientes.",
    "Asesorar a los clientes sobre las características, beneficios y especificaciones técnicas de los electrodomésticos.",
    "Lunes a jueves de 2:00 pm a 9:00 pm y viernes a domingo de 11:00 am a 9:00 pm",
    "Descanso un día a la semana."
   ],
   "requisitos": [
    "Secundaria completa",
    "Tengo 3 meses de experiencia en ventas, impulso, promotoría o mercaderismo.",
    "Excelente habilidad de comunicación y capacidad para establecer relaciones con los clientes.",
    "Disponibilidad de laborar en las tiendas mencionadas.",
    "¡Te esperamos!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 20 y 65 añosPalabras clave: promoter, promotor, cambaceo, sales"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día de trabajo, con todos los beneficios de ley.",
    "Combo de fidelización valorizado en S/600 2 veces al año.",
    "Remuneración Fija S/ 1130",
    "Bono de Movilidad S/ 100",
    "Comisiones hasta los S/500"
   ],
   "url": "https://www.laborum.pe/job/grupo-tawa/imaco-promotor-de-ventas-tottus-begonias-y-tottus-huaylas/6a9126ae2020b210c83d8563"
  },
  {
   "id": 298,
   "puesto": "Asesor(a) de belleza",
   "empresa": "Adecco Perú S.A.",
   "cat": "Ventas",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 95,
   "resumen": "RESUMEN: ¡Sé parte de una importante empresa del rubro belleza!",
   "funciones": [
    "Asesorar a los clientes brindando una excelente experiencia de compra.",
    "Identificar y conocer los diferentes tipos de productos, marcas y categorías.",
    "Cumplir con el presupuesto de ventas, transacciones y ticket promedio.",
    "Mantener la tienda limpia, ordenada y preparada para vender.",
    "Cumplir con el protocolo de ventas y servicio."
   ],
   "requisitos": [
    "Experiencia en ventas y atención al cliente retail, comida rápida o similares.",
    "Disponibilidad para trabajar de L a D, 1 día de descanso. Turnos rotativos (9am - 6pm/1pm - 10pm)",
    "Descanso rotativo",
    "Deseable conocimiento en maquillaje, belleza o productos dermatológicos.",
    "Buena comunicación y orientación al cliente."
   ],
   "beneficios": [
    "Salario base de S/1300 + comisiones + incentivos por distintas temporadas.",
    "Planilla completa directa desde el primer día.",
    "Todos los beneficios de ley (CTS, gratificaciones, vacaciones, seguro y asignación familiar).",
    "Línea de carrera",
    "Horarios: L a D, de 9:00 am a 6:00 pm /1:00 pm a 10:00 pm",
    "UN DÍA DE DESCANSO",
    "Pagos mensuales. -Requerimientos- Educación mínima: Educación Secundaria años de experiencia"
   ],
   "url": "https://www.laborum.pe/job/adecco-peru-s-a/asesor-a-de-belleza-mega-plaza-con-experiencia-horarios/6a921c702020b210c83e1275"
  },
  {
   "id": 299,
   "puesto": "Sub jefe de tienda",
   "empresa": "MEYTEN",
   "cat": "Recursos Humanos",
   "min": 1600,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 95,
   "resumen": "RESUMEN: Actualmente nos encontramos en la búsqueda del mejor talento para ocupar la posición de:",
   "funciones": [
    "Realizar la Apertura y Cierre de la Tienda.",
    "Llevar el control de la existencia, mantención y adecuada funcionalidad de los activos de la tienda y solicitar el arreglo de los mismos en caso de desperfecto.",
    "Supervisar que la mercadería tenga los productos actualizados en relación a precios y seguridad.",
    "Supervisar que toda la mercadería esté representada en la sala de ventas, acorde al Merchandising y Visual; según los Instructivos.",
    "Supervisar el adecuado orden, limpieza de la tienda y de almacén.",
    "Brindar asesoramiento y atender las solicitudes de los clientes.",
    "Involucrarse con el personal a su cargo en tienda brindándole coaching y retroalimentación constante para el mejor desempeño de sus funciones.",
    "Cumplir con entregar a tiempo los informes que su Jefe de Tienda le solicite."
   ],
   "requisitos": [
    "Mayores de 24 años",
    "Experiencia mínima de 1 año como encargado o sub encargado de tienda del rubro retail. (Indispensable)",
    "Contar con experiencia teniendo personal a su cargo.",
    "Disponibilidad para trabajar en TURNO ROTATIVOS, (8 horas + 1 hora de refrigerio diarios) de lunes a domingo con 1 día de descanso.",
    "Tener experiencia con el manejo de POS."
   ],
   "beneficios": [
    "Sueldo S/1600 + Bono por indicadores.",
    "Ingreso a Planilla completa.",
    "Línea de Carrera.",
    "Pagos quincenales y mensuales.",
    "Buen ambiente laboral",
    "¡No te pierdas la oportunidad de ser parte de esta gran familia!. -Requerimientos- Educación mínima: Técnico1 año de experienciaEdad: entre 20 y 44 añosPalabras clave: lider, jefe, gerente, manager, director, chief, lead, jefatura, regente, boutique, tienda, store, establecimiento, bazar, bazzar"
   ],
   "url": "https://www.laborum.pe/job/meyten/sub-jefe-de-tienda-retail-jesus-maria/6a921ff02020b210c83e224d"
  },
  {
   "id": 300,
   "puesto": "Promotor de ventas consumo masivo",
   "empresa": "Soft Group",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 94,
   "resumen": "Casco urbano: San Miguel, Miraflores, San Isidro, Magdalena, Lince, Jesús María, Cercado de Lima, La Victoria, San Borja, Rímac, Breña, Surco",
   "funciones": [
    "Visitar los puntos de venta (contamos con cartera de clientes)",
    "Estimular el interés de compra",
    "Seguimiento de prospecto hasta el cierre de venta",
    "Reporte de venta",
    "Acompañamiento del distribuidor· En SOFT GROUP estamos convencidos que influir de manera positiva en nuestro entorno, es contar con un equipo diverso y fomentar una cultura inclusiva."
   ],
   "requisitos": [
    "Experiencia en promotor de campo, consumo masivo",
    "Valores: Responsabilidad, puntualidad, honestidad.",
    "Disponibilidad de hacer ruta en la zona asignada"
   ],
   "beneficios": [
    "Incorporación inmediata",
    "Sueldo fijo: S/1130.00",
    "Movilidad: S/500.00",
    "Bono de telefonía: S/30.00",
    "Variable: S/400",
    "Planilla Mype",
    "Trabajo de Lunes a sábado 7 am - 4 pm"
   ],
   "url": "https://www.laborum.pe/job/soft-group/promotor-de-ventas-consumo-masivo-tradicional-casco-urbano/6a90d6d72020b210c83cbc27"
  },
  {
   "id": 301,
   "puesto": "Asesor de Cobranzas Interbank",
   "empresa": "Biznes Sac",
   "cat": "Contabilidad",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 94,
   "resumen": "Realizar llamadas de cobranza a clientes con deudas vencidas.",
   "funciones": [
    "Realizar llamadas de cobranza a clientes con deudas vencidas.",
    "Contactar a clientes para la verificación de pagos.",
    "Negociar alternativas de pago y acuerdos con clientes morosos.",
    "Brindar información clara y mantener la calidad en cada gestión realizada.",
    "Registrar las gestiones efectuadas y elaborar reportes de incidencias.",
    "Cumplir con los objetivos e indicadores establecidos por el área."
   ],
   "requisitos": [
    "Ser mayor de 18 años.",
    "Contar con secundaria completa o estudios superiores en curso o inconclusos.",
    "Experiencia en cobranzas del rubro financiero (tarjetas de crédito, préstamos, entre otros). No indispensable.",
    "Actitud positiva, compromiso, responsabilidad y orientación a resultados.",
    "Facilidad de comunicación y capacidad de negociación."
   ],
   "beneficios": [
    "Sueldo desde S/ 1,200.",
    "Incentivos y comisiones ilimitadas.",
    "Ingreso a planilla MYPE desde el primer día (vacaciones, gratificaciones, CTS, EsSalud, AFP y asignación familiar, según corresponda).",
    "Bono por puntualidad.",
    "Bono por referidos.",
    "Ingreso inmediato.",
    "Oportunidad de crecimiento y línea de carrera a corto plazo.",
    "Pagos puntuales.",
    "Excelente clima laboral.",
    "Descanso los domingos y feriados.",
    "Horario de trabajo",
    "Lunes a viernes: 8:00 a. m. a 6:00 p. m."
   ],
   "url": "https://www.laborum.pe/job/biznes-sac/asesor-de-cobranzas-interbank-sueldo-comisiones-sin-tope/6a90d39b2020b210c83cb4c0"
  },
  {
   "id": 302,
   "puesto": "Promotor técnico agrónomo retail y campo",
   "empresa": "Salesland Pacífico",
   "cat": "Ventas",
   "min": 500,
   "max": 500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 92,
   "resumen": "Zonas a visitar: Lima, Callao, Áncash, Arequipa, Cajamarca, Cusco, Ica, Junín, La Libertad, Lambayeque, Piura, Puno, Tacna, Tumbes, San Martín, Ucayali, Loreto, Huánuco, Moquegua, Madre de Dios y Apurímac.",
   "funciones": [
    "Visitar distribuidores y puntos de venta de acuerdo con la ruta asignada.",
    "Impulsar la venta y rotación de los productos.",
    "Realizar demostraciones y presentaciones de los productos.",
    "Capacitar al personal de los puntos de venta sobre el uso y beneficios de los productos.",
    "Verificar precios, stock, exhibición y presencia de material POP.",
    "Identificar oportunidades comerciales y reportar acciones de la competencia.",
    "Apoyar activaciones y acciones comerciales.",
    "Elaborar y presentar reportes de visitas, incidencias y oportunidades detectadas."
   ],
   "requisitos": [
    "Experiencia mínima de 1 año en ventas de campo, promotoria, impulsación o gestión comercial.",
    "Deseable experiencia en ferreterías, distribuidores, herramientas, maquinaria, productos técnicos o categorías afines.",
    "Capacidad para explicar características, beneficios y aplicaciones de productos.",
    "Disponibilidad para realizar trabajo de campo y viajes, según la zona asignada.",
    "Manejo básico de Office y aplicativos para elaboración de reportes.",
    "Perfil proactivo, autónomo y orientado al cumplimiento de resultados."
   ],
   "beneficios": [
    "S/1800 fijo + hasta S/500 de variable + S/300 movilidad + S/30 de línea telefónica + VIATICOS (cuando se solicite viajar)",
    "PLANILLA COMPLETA desde el 1er día de gestión con todos los beneficios de ley (Seguro, 14 sueldos al año, asignación familiar, CTS, utilidades, gratificaciones, vacaciones pagadas, etc.)",
    "Línea de carrera y excelente clima laboral",
    "Contratación INMEDIATA. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 25 y 50 añosDisponibilidad de viajar: SiPalabras clave: promoter, promotor, tecnologo, tecnico, tech, technician, technology, tecnologia, technologist, agronomist, agronomo, terreno, territorio"
   ],
   "url": "https://www.laborum.pe/job/salesland-pacifico/promotor-tecnico-agronomo-retail-y-campo-husqvarna-lima/6a90d3952020b210c83cb4a0"
  },
  {
   "id": 303,
   "puesto": "Técnico de Contabilidad",
   "empresa": "Talento Humano Perú",
   "cat": "Contabilidad",
   "min": 2300,
   "max": 2300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 29,
   "vence": "2026-11-26",
   "restan": 61,
   "score": 92,
   "resumen": "Procesar y atender las solicitudes entrantes de equipos y repuestos.",
   "funciones": [
    "Procesar y atender las solicitudes entrantes de equipos y repuestos.",
    "Organizar base de datos de clientes y transportistas.",
    "Optimizar la cadena de distribución de despacho de materiales.",
    "Elaborar guías de remisión, boletas, facturas, etc.",
    "Optimizar tiempos de entrega de despacho de materiales.",
    "Gestionar la recepción y traslado de la carga."
   ],
   "requisitos": [
    "Carrera Técnica en Contabilidad.",
    "Experiencia mínima de un (01) año en cargos similares.",
    "Conocimiento de manejo de ERP o herramientas de gestión. (Deseable)",
    "Conocimiento básico de comprobantes contables (guías de remisión, boletas, facturas, etc)",
    "Licencia de Conducir",
    "Excel, intermedio."
   ],
   "beneficios": [
    "Remuneración entre: S/. 2,300.00 soles - S/. 2,500.00 soles. (De acuerdo a evaluación).",
    "Planilla PYME, con todos los beneficios de Ley.",
    "50% de beneficios (Gratificaciones y CTS.)",
    "LUGAR Y HORARIO DE TRABAJO",
    "Pueblo Libre.",
    "Lunes a Viernes de 8:00 a.m. a 6:00 p.m.",
    "Trabajo Presencial.",
    "Las personas interesadas que cumplan con el perfil solicitado, enviar su cv. actualizado a la página web: https://www.talentohumanoperu.pe/postula-con-nosotros o al WhatsApp mediante este link ️ https://wa.link/1v0ioy o al 907994423 indícanos el puesto al que postula.",
    "ASUNTO: TÉCNICO DE CONTABILIDAD – RESPONSABLE DE ALMACÉN"
   ],
   "url": "https://www.bumeran.com.pe/empleos/tecnico-de-contabilidad-responsable-de-almacen-talento-humano-peru-1118421011.html"
  },
  {
   "id": 304,
   "puesto": "Coordinador de Selección Senior",
   "empresa": "Consulting Advisers",
   "cat": "Recursos Humanos",
   "min": 4000,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 92,
   "resumen": "Liderar y supervisar los procesos de reclutamiento y selección de personal.",
   "funciones": [
    "Liderar y supervisar los procesos de reclutamiento y selección de personal.",
    "Gestionar procesos de reclutamiento masivo, principalmente para posiciones operativas.",
    "Liderar y desarrollar al equipo de selección, asegurando el cumplimiento de objetivos.",
    "Gestionar la atención y seguimiento de los requerimientos de los clientes.",
    "Analizar y realizar seguimiento de KPIs, métricas e indicadores de selección.",
    "Elaborar reportes de gestión y presentar resultados a la jefatura y/o gerencia.",
    "Proponer estrategias para optimizar los tiempos y resultados de cobertura.",
    "Asegurar el cumplimiento de los estándares y lineamientos establecidos para cada cliente."
   ],
   "requisitos": [
    "Profesional de las carreras de Psicología, Administración, Recursos Humanos o afines.",
    "Experiencia mínima de 8 años en posiciones similares.",
    "Experiencia en reclutamiento masivo y selección de perfiles operativos.",
    "Experiencia gestionando clientes del rubro minero.",
    "Manejo de KPIs, métricas e indicadores de selección.",
    "Experiencia liderando equipos de trabajo."
   ],
   "beneficios": [
    "Planilla completa con todos los beneficios de ley.",
    "Sueldo de S/.4000",
    "Oportunidad de crecimiento y desarrollo profesional.",
    "Participación en proyectos con importantes clientes.",
    "Formar parte de una empresa con años de experiencia y sólida trayectoria en el mercado.",
    "¡Sé parte de GCA y acompáñanos en esta etapa de crecimiento y expansión!. -Requerimientos- Educación mínima: Universitario3 años de experienciaPalabras clave: residente, encargado, supervisor, subgerente, responsable, coordinador, gestor, capitan, reclutador, recruiter, headhunter, scout, reclutamiento, senior, sr"
   ],
   "url": "https://www.laborum.pe/job/consulting-advisers/coordinador-de-seleccion-senior-rubro-consultoria-miraflores/6a90fbb92020b210c83d2ef2"
  },
  {
   "id": 305,
   "puesto": "Supervisor de Operaciones Logisticas",
   "empresa": "Shalom Empresarial S.A.C.",
   "cat": "Logística",
   "min": 2500,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 89,
   "resumen": "RESUMEN: En SHALOM, empresa líder en el sector de transporte, envíos y encomiendas a nivel nacional; se encuentra en la búsqueda de un talento para el puesto de Supervisor de Operaciones Logísticas.",
   "funciones": [
    "Dirigir y organizar al personal operativo en turno",
    "Asegurar disciplina, orden y cumplimiento de normas",
    "Corregir conductas inseguras o inadecuadas en tiempo real",
    "Supervisar carga, descarga y despacho en campo",
    "Controlar tiempos operativos y flujo de trabajo",
    "Verificar cumplimiento de SOP (Procedimientos Operativos Estándar)",
    "Evitar cuellos de botella en la operación (embarque, desembarque , y sorter)",
    "Asegurar el uso correcto de EPP (casco, guantes, uniforme, etc.)",
    "Realizar y liderar charlas de 5 minutos antes del inicio de operaciones",
    "Identificar riesgos operativos antes y durante la jornada",
    "Detener operaciones ante condiciones inseguras",
    "Supervisar manipulación adecuada de carga (ergonomía)"
   ],
   "requisitos": [
    "Estudios universitarios en Ingeniería Industrial, Administración o a fines (especialización en Logística).",
    "Excel intermedio.",
    "Conocimientos en estiba, indicadores operativos, SST y mejora continua (Lean, 5S).",
    "Experiencia mínima de 1 año en puestos similares (deseable)."
   ],
   "beneficios": [
    "Ingreso a Planilla con todos los beneficios de ley Regimen general",
    "Seguro Vida Ley, SCTR y EsSalud.",
    "Línea de carrera con nosotros.",
    "Pagos puntuales mensuales",
    "Sueldo: s/ 2.500",
    "Horario de trabajo rotativo",
    "Lugar de trabajo: Huachipa. -Requerimientos- Educación mínima: Universitario2 años de experienciaPalabras clave: residente, encargado, supervisor, subgerente, responsable, coordinador, gestor, capitan, operations, operaciones"
   ],
   "url": "https://www.laborum.pe/job/shalom-empresarial-s-a-c/supervisor-de-operaciones-logisticas-cobertura-de/6a90d2ad2020b210c83cb131"
  },
  {
   "id": 306,
   "puesto": "Asesor de Renovación Call Center Básico",
   "empresa": "Partner Service",
   "cat": "Atención al Cliente",
   "min": 800,
   "max": 800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 88,
   "resumen": "Modalidad: Presencial (Sede Santa Anita o Sede Céntrica El Agustino).",
   "funciones": [
    "Contactar a clientes vía telefónica para ofrecer la renovación de su equipo móvil con mejores tarifas y promociones.",
    "Realizar venta cruzada de líneas adicionales y seguros de protección móvil.",
    "Asegurar una experiencia de atención de calidad y el cumplimiento de las metas comerciales.",
    "HORARIOS DISPONIBLES",
    "FULL TIME: Lunes a Sábado de 9:00 a.m. a 6:00 p.m.",
    "SEMI FULL (Turnos a elegir)",
    "9:00 a.m. a 4:00 p.m.",
    "10:00 a.m. a 5:00 p.m.",
    "11:00 a.m. a 6:00 p.m."
   ],
   "requisitos": [
    "Secundaria completa (Certificado C4 / Constancia de estudios) o superior.",
    "Contar con DNI vigente (copia).",
    "Contar con RUC y Clave SOL activos en SUNAT (para emisión de Recibos por Honorarios durante periodo de prueba).",
    "Certijoven o Certiadulto (Antecedentes policiales y penales).",
    "Recibo de servicio (Luz o Agua) actualizado."
   ],
   "beneficios": [
    "Opción FULL TIME",
    "Básico mensual: S/. 1,130.00",
    "Bono de Permanencia: S/. 400.00 (S/. 100 mensuales durante los primeros 4 meses)",
    "Bono de Movilidad: S/. 100.00 (sujeto a asistencia)",
    "Comisiones Ilimitadas desde la primera venta.",
    "Opción SEMI FULL",
    "Básico mensual: S/. 800.00",
    "Comisiones Ilimitadas.",
    "Capacitación Pagada: S/. 180.00 soles por los 11 días de inducción y OJT (S/. 30.00 por día completado y aprobado).",
    "Pagos puntuales a fin de mes a través de BCP o Scotiabank.",
    "Excelente clima laboral y oportunidades de desarrollo interno."
   ],
   "url": "https://www.laborum.pe/job/partner-service/asesor-de-renovacion-call-center-basico-comisiones/6a90f84a2020b210c83d22f1"
  },
  {
   "id": 307,
   "puesto": "Operario de Almacen s",
   "empresa": "Adecco Perú S.A.",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 86,
   "resumen": "RESUMEN: Adecco es la empresa de Recursos Humanos más grande del mundo. Somos líderes en gestión humana. Diariamente ayudamos a más de 700,000 personas a encontrar trabajo a través de nuestra amplia red.",
   "funciones": [
    "Armado de pedidos paletizando mercadería (cajas de productos para supermercados de 5kg a 20 kg)",
    "Manejo de radiofrecuencia (RF).",
    "Trasladar paletas con Estoca"
   ],
   "requisitos": [
    "Disponibilidad para laborar en Punta Negra, Altura del Peaje de Punta negra (contamos con buses de acercamiento gratuitos para todo Lima)",
    "Disponibilidad laborar en horarios rotativos (turno día: 7:00am a 4:00pm // turno tarde: 1:00 pm a 10:00 pm// turno noche: 10:00pm a 7:00am)",
    "Mucha actitud y ganas de aprender."
   ],
   "beneficios": [
    "Remuneración: S/1130",
    "Bono de movilidad: s/150",
    "Bono de Alimentación: S/150",
    "Bono Asistencia: s/100",
    "Pago de asignación familiar s/113 + horas extras + bonificación de horario nocturno",
    "Buses de acercamiento gratis en todo Lima.",
    "Ingreso a planilla Adecco desde el primer día de trabajo con todos los Beneficios de ley (CTS, Gratificación, Vacaciones, Seguro Social, Seguro Vida Ley, Asignación Familiar)",
    "Beneficios corporativos.-Requerimientos- Educación mínima: Técnico años de experienciaPalabras clave: operator, operario, operator, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/adecco-peru-s-a/operario-de-almacen-s-1800-bus-y-alimentacion-gratuitas/6a912cd22020b210c83d9755"
  },
  {
   "id": 308,
   "puesto": "Ayudante de almacén para villa el salvador",
   "empresa": "REPRESENTACIONES MARTIN S.A.C",
   "cat": "Logística",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 85,
   "resumen": "Nos encontramos en la búsqueda del mejor talento para asumir el cargo de :",
   "funciones": [
    "Picking, packing.",
    "Ordenamiento de mercadería.",
    "Recepción y distribución de mercadería en el almacén.",
    "Verificar ingreso y salida de mercadería.",
    "Control de calidad de productos,",
    "Preparación de pedidos.",
    "Carga y descarga de mercadería.",
    "Dar soporte a distribución (reparto).",
    "Orden y limpieza del área asignada.",
    "Otras funciones indicadas por jefe inmediato."
   ],
   "requisitos": [
    "Contar con DNI físico.",
    "Experiencia en: Almacén, producción, estiba, despacho, construcción y/o trabajos informales.",
    "Disponibilidad para laborar: De lunes a sábado de 8am a 6:30pm.",
    "Lugar de trabajo: Villa el salvador (Av. Forestales)."
   ],
   "beneficios": [
    "Sueldo básico inicial de 1200.00 soles.",
    "Adicional de 270.00 soles.",
    "Asignación Familiar 113.00 soles.",
    "Almuerzo cubierto al 100%.",
    "Planilla completa régimen general con todos los beneficios de ley (Beneficios sociales completos al 100%).",
    "Oportunidad de crecimiento.",
    "Seguro de Salud."
   ],
   "url": "https://www.laborum.pe/job/representaciones-martin-s-a-c/ayudante-de-almacen-para-villa-el-salvador-av-forestales/6a9100732020b210c83d3c1a"
  },
  {
   "id": 309,
   "puesto": "Asesor(a) Comercial de Campo Movistar Postpago Presencial",
   "empresa": "Atento Perú",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 81,
   "resumen": "RESUMEN: Funciones: Realizar llamadas de salida a clientes Movistar.",
   "funciones": [
    "Visitar clientes para mejorar su plan postpago y ofrecer promociones exclusivas.",
    "Gestionar migraciones a planes postpago mediante descuentos y beneficios.",
    "Brindar una excelente atención y asesoría comercial."
   ],
   "requisitos": [
    "Experiencia en ventas, atención al cliente o call center (deseable).",
    "Facilidad de comunicación y negociación.",
    "Proactividad y orientación a resultados."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,130",
    "Comisiones ilimitadas",
    "Promedio de comisiones: S/ 300",
    "Potencial de ingresos adicionales hasta S/ 700",
    "Planilla completa desde el primer día de contrato",
    "Inicio: 28/08/2026",
    "Duración: 7 días",
    "Horario: 8:00 a.m. a 2:00 p.m.",
    "Pago por capacitación: S/ 20 por día asistido (S/ 140 al finalizar y aprobar la capacitación).",
    "Contrato inicial de 6 meses renovable según desempeño.",
    "OJT (acompañamiento evaluativo): 10 días.",
    "Primera remuneración: a finales de septiembre."
   ],
   "url": "https://www.laborum.pe/job/atento-peru/asesor-a-comercial-de-campo-movistar-postpago-presencial/6a90e5822020b210c83cea72"
  },
  {
   "id": 310,
   "puesto": "Operario de Lavandería",
   "empresa": "GRAFTON PERU S.A.C.",
   "cat": "Ingeniería",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 29,
   "vence": "2026-09-28",
   "restan": 2,
   "score": 80,
   "resumen": "Lavado y clasificación de prendas.",
   "funciones": [
    "Lavado y clasificación de prendas.",
    "Doblado, planchado y empaque.",
    "Control de calidad y orden del área."
   ],
   "requisitos": [
    "Con o sin experiencia (¡te capacitamos!).",
    "Residir en Chorrillos o zonas aledañas.",
    "Responsabilidad, compromiso y trabajo en equipo.",
    "Disponibilidad de trabajar en horario rotativo."
   ],
   "beneficios": [
    "Sueldo base: S/ 1200 + Bono de movilidad S/. 300",
    "Pago de horas extras.",
    "Ingreso a planilla mype desde el primer día.",
    "Capacitación constante.",
    "Oportunidad de crecimiento.",
    "Información Adicional",
    "Modalidad: Presencial",
    "Horario fijo",
    "Turno mañana: 5:30 am - 2:00 pm",
    "Turno tarde: 1pm a 10pm",
    "Ubicación: Chorrillos.",
    "En Grafton Perú promovemos una cultura basada en la diversidad, inclusión y desarrollo del talento. Creemos que las personas son el motor del éxito de las organizaciones y buscamos profesionales comprometidos, apasionados y orientados a generar impacto."
   ],
   "url": "https://www.laborum.pe/job/grafton-peru-s-a-c/operario-de-lavanderia-s-1-500-sin-experiencia-turno-fijo/6a910aae2020b210c83d5574"
  },
  {
   "id": 311,
   "puesto": "Jefe de Compensaciones y Desempeño",
   "empresa": "Well Consulting Services",
   "cat": "Otros",
   "min": 5000,
   "max": 6000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 30,
   "vence": "2026-11-25",
   "restan": 60,
   "score": 95,
   "resumen": "Liderar la gestión integral de los procesos de Compensaciones y Gestión del Desempeño .",
   "funciones": [
    "Liderar la gestión integral de los procesos de Compensaciones y Gestión del Desempeño .",
    "Diseñar, implementar y asegurar la correcta aplicación de estructuras salariales y políticas de compensación.",
    "Gestionar y controlar el presupuesto de personal.",
    "Analizar indicadores de compensaciones y desempeño para la toma de decisiones estratégicas.",
    "Liderar el ciclo de gestión del desempeño y las herramientas asociadas.",
    "Generar reportes y análisis de información para la gestión estratégica de personas.",
    "Participar en proyectos corporativos vinculados a compensaciones, desempeño y planeamiento de personas.",
    "Velar por la equidad interna y competitividad externa de las estructuras salariales."
   ],
   "requisitos": [
    "Bachiller o Titulado/a en Administración, Ingeniería Industrial, Economía, Finanzas, Recursos Humanos o carreras afines.",
    "Experiencia mínima de 4 años en compensaciones, gestión del desempeño, planeamiento de personas o funciones afines.",
    "Deseable experiencia de al menos 1 año liderando personas, proyectos o procesos de alcance corporativo .",
    "Inglés a nivel intermedio .",
    "Conocimientos en estructuras salariales, presupuestos de personal, indicadores y procesos de gestión del desempeño .",
    "Dominio de Excel y Power BI a nivel avanzado .",
    "Deseable manejo de SAP, BUK u otros sistemas de gestión de personas."
   ],
   "beneficios": [
    "Lugar de trabajo: San Isidro, Lima.",
    "Modalidad: Híbrida.",
    "Horario: Lunes a viernes de 9:00 a. m. a 7:00 p. m.",
    "Remuneración: S/ 5,000 a S/ 6,000, según evaluación y experiencia.",
    "Beneficios: Descuentos corporativos.",
    "Si cumples con los requisitos y estás interesado/a en asumir un rol estratégico dentro de la gestión de personas, te invitamos a postular."
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-de-compensaciones-y-desempeno-|-san-isidro-well-consulting-services-1118418037.html"
  },
  {
   "id": 312,
   "puesto": "Vendedor de Campo",
   "empresa": "Adecco Perú S.A.",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 30,
   "vence": "2026-09-27",
   "restan": 1,
   "score": 95,
   "resumen": "Realizar visitas diarias a bodegas, panaderías y minimarkets asignados en ruta.",
   "funciones": [
    "Realizar visitas diarias a bodegas, panaderías y minimarkets asignados en ruta.",
    "Gestionar la venta del portafolio de productos de consumo masivo.",
    "Registrar visitas, pedidos y resultados en aplicativo móvil.",
    "Asegurar la correcta exhibición de productos y material promocional en los puntos de venta."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 6 meses en ventas de campo, preventa o canal tradicional.",
    "Experiencia visitando bodegas, minimarkets, panaderías o negocios similares.",
    "Manejo básico de aplicaciones móviles y WhatsApp.",
    "Disponibilidad para trabajo 100% en campo.",
    "Disponibilidad para laborar de lunes a sábado."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley desde el primer día.",
    "Sueldo básico: S/ 1,130",
    "Movilidad: S/ 208 (pago quincenal)",
    "Comisiones mensuales desde S/ 785 o más según desempeño",
    "Bonos e incentivos por concursos comerciales.",
    "Capacitaciones constantes.",
    "Oportunidad de línea de carrera dentro del grupo empresarial.",
    "Zonas de trabajo",
    "Lima Metropolitana. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: salesman, saleswoman, seller, vendedor, comercial, salesperson, terreno, territorio"
   ],
   "url": "https://www.laborum.pe/job/adecco-peru-s-a/vendedor-de-campo-canal-tradicional-planilla-completa/6a8fdf682020b210c83c46a8"
  },
  {
   "id": 313,
   "puesto": "Supervisor",
   "empresa": "Quasar People",
   "cat": "Otros",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 30,
   "vence": "2026-09-27",
   "restan": 1,
   "score": 92,
   "resumen": "RESUMEN: Estamos en búsqueda de OPERADORES DE CAMPO para realizar labores de implementación y supervisión en tiendas.",
   "funciones": [
    "Instalación y desinstalación de elementos y materiales en tiendas.",
    "Implementación de materiales según los lineamientos establecidos.",
    "Registro de llegada y actividades realizadas en tienda.",
    "Uso adecuado de uniforme, fotocheck y herramientas de trabajo.",
    "Registro fotográfico de las implementaciones realizadas.",
    "Elaboración de inventario de elementos instalados y no instalados.",
    "Reporte de incidencias y campañas no instaladas al supervisor.",
    "Limpieza, mantenimiento y verificación de los elementos instalados.",
    "Coordinación de instalaciones y desinstalaciones dentro de las tiendas.",
    "Cumplir otras funciones asignadas por el jefe inmediato."
   ],
   "requisitos": [
    "Disponibilidad para realizar trabajo de campo.",
    "Disponibilidad para desplazarse por las tiendas asignadas.",
    "Responsabilidad, organización y compromiso.",
    "Experiencia en campo, implementación o mercaderismo será valorada."
   ],
   "beneficios": [
    "Sueldo: S/ 1,300",
    "Movilidad: S/ 200",
    "Ingreso a planilla MYPE",
    "Izaguirre, universitaria, los olivos, pro, comas, puente piedra, makro independencia, makro SMP y makro comas.-Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: entre 23 y 45 añosPalabras clave: terreno, territorio"
   ],
   "url": "https://www.laborum.pe/job/quasar-people/operadores-de-campo-lima-norte/6a8fc1672020b210c83c037a"
  },
  {
   "id": 314,
   "puesto": "Vendedora San Isidro",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ventas",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 30,
   "vence": "2026-11-25",
   "restan": 60,
   "score": 92,
   "resumen": "Asesoramiento a clientes en búsqueda de los diversos productos.",
   "funciones": [
    "Asesoramiento a clientes en búsqueda de los diversos productos.",
    "Alcanzar los objetivos de ventas establecidos por la empresa.",
    "Estar actualizado en los nuevos ingresos de productos",
    "Apoyar en el orden de los productos y visual de la tienda."
   ],
   "requisitos": [
    "Solo mujeres de nacionalidad peruana entre 23 y 40 años.",
    "Tener experiencia mínima de 1 año en ventas de marcas retail ubicadas en centros comerciales o boutiques puerta a calle o como promotoría en departamentales.",
    "Excelente trato y orientación al cliente.",
    "Con experiencia en manejo de caja.",
    "Disponibilidad para laborar 48 horas semanales solo de lunes a sábados de 10 am a 7 pm. No se trabajan domingos."
   ],
   "beneficios": [
    "Grato ambiente laboral y posibilidad de hacer línea de carrera.",
    "Los 3 primeros meses se pagará mediante recibos de honorarios por ser periodo de prueba y luego pasa a planilla pyme con contrato por tiempo indefinido.",
    "Beneficios laborales de planilla pyme (50% de gratificación en julio y diciembre + 50% de CTS anual en mayo y noviembre + 15 días de vacaciones al año + Essalud + asignación familiar).",
    "Sueldo básico (S/.1400) + comisiones desde la 1ra venta y sin tope (en promedio son de S/.300 a S/.400 a más)."
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedora-san-isidro-adn-retail-s.a.c.-1118419457.html"
  },
  {
   "id": 315,
   "puesto": "Vendedor(a) de Soluciones TI",
   "empresa": "MARISOL ZEVALLOS PSICOLOGIA Y MANAGEMENT E.I.R.L.",
   "cat": "Ventas",
   "min": 3200,
   "max": 3200,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 30,
   "vence": "2026-11-25",
   "restan": 60,
   "score": 92,
   "resumen": "Buscamos una persona con experiencia comercial en el sector tecnológico, iniciativa para generar nuevas oportunidades de negocio y capacidad para presentar soluciones de manera clara y convincente.",
   "funciones": [
    "Prospectar activamente y generar leads y oportunidades comerciales en el mercado B2B.",
    "Identificar y contactar potenciales clientes y tomadores de decisión.",
    "Realizar reuniones comerciales y presentaciones de las soluciones de Identia.",
    "Elaborar propuestas comerciales y cotizaciones.",
    "Registrar, actualizar y realizar seguimiento a las oportunidades mediante el CRM corporativo HubSpot.",
    "Realizar seguimiento a prospectos durante las diferentes etapas del proceso comercial.",
    "Identificar oportunidades de upselling y cross-selling con los clientes actuales.",
    "Apoyar en la gestión de contenidos comerciales y de marketing para los canales de venta.",
    "Participar en reuniones, webinars, ferias y otros eventos corporativos.",
    "Cumplir los objetivos, indicadores y cuotas comerciales establecidos."
   ],
   "requisitos": [
    "Experiencia mínima de dos años en venta de soluciones tecnológicas, SaaS, Cloud o servicios TI dirigidos a empresas.",
    "Experiencia realizando prospección activa y generando nuevas oportunidades comerciales.",
    "Experiencia trabajando con objetivos y cuotas de venta.",
    "Manejo de CRM, preferentemente HubSpot.",
    "Conocimiento de herramientas de prospección comercial como Apollo, LinkedIn Sales Navigator, Waalaxy o Lusha.",
    "Facilidad para realizar presentaciones comerciales y explicar soluciones tecnológicas.",
    "Comunicación efectiva, iniciativa, perseverancia y orientación a resultados.",
    "Disponibilidad para trabajar bajo modalidad híbrida en Barranco."
   ],
   "beneficios": [
    "Remuneración básica mensual de S/ 3,200 .",
    "Comisiones de venta del 2% , de acuerdo con las condiciones comerciales establecidas.",
    "Ingreso a planilla bajo el régimen laboral de la micro y pequeña empresa (MYPE) .",
    "Modalidad híbrida: 50% de trabajo remoto y 50% presencial en la oficina de Barranco.",
    "Laptop, teléfono y herramientas comerciales.",
    "Ambiente de trabajo cercano y colaborativo.",
    "Oportunidad de desarrollarse en una empresa especializada en soluciones tecnológicas innovadoras.",
    "Si cuentas con experiencia comercializando soluciones tecnológicas y te motiva desarrollar nuevas oportunidades de negocio, te invitamos a postular."
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedora-de-soluciones-ti-ejecutivoa-comercial-b2b-marisol-zevallos-psicologia-y-management-e.i.r.l.-1118403571.html"
  },
  {
   "id": 316,
   "puesto": "Trabaja en Jockey Plaza 2pm a 11pm! Anfitrión(a) de estacionamiento",
   "empresa": "APPARKA",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 30,
   "vence": "2026-09-27",
   "restan": 1,
   "score": 90,
   "resumen": "RESUMEN: En apparka estamos en búsqueda de personas con actitud de servicio y ganas de crecer para incorporarse a nuestro equipo como Anfitrión(a) de Estacionamiento en nuestra playa del CC Jockey Plaza en el Turno Tarde",
   "funciones": [
    "Brindar una atención cordial y de calidad a nuestros clientes.",
    "Facilitar el ingreso y salida de vehículos.",
    "Utilizar herramientas tecnológicas para realizar las operaciones de manera eficiente.",
    "Trabajar en equipo para brindar una excelente experiencia al cliente."
   ],
   "requisitos": [
    "Buena actitud y orientación al servicio.",
    "Ganas de aprender y desarrollarse.",
    "Compromiso, responsabilidad y puntualidad.",
    "Disponibilidad para laborar en el horario indicado.",
    "No necesitas experiencia previa."
   ],
   "beneficios": [
    "Sueldo: S/ 1,130 mensuales + Bono por asistencia (S/100) + Bono por Movilidad (S/50)",
    "Capacitaciones constantes.",
    "Oportunidad de línea de carrera a corto plazo.",
    "Ingreso a una empresa con oportunidades de crecimiento."
   ],
   "url": "https://www.laborum.pe/job/apparka/trabaja-en-jockey-plaza-2pm-a-11pm-anfitrion-a-de/6a8fa4e02020b210c83bbecb"
  },
  {
   "id": 317,
   "puesto": "Operario de limpieza para Villa el Salvador",
   "empresa": "Limtek Servicios Integrales",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 30,
   "vence": "2026-09-27",
   "restan": 1,
   "score": 89,
   "resumen": "RESUMEN: ¡ÚNETE A LIMTEK SERVICIOS INTEGRALES!",
   "funciones": [
    "Limpieza general de edificios y locales comerciales.",
    "Cumplimiento de estándares de higiene y seguridad.",
    "Atención al detalle y capacidad para trabajar en equipo."
   ],
   "requisitos": [
    "Somos LIMTEK Servicios Integrales, parte del grupo Tawa, y estamos buscando un Operario de Limpieza que pueda laborar en VILLA EL SALVADOR , con o sin experiencia para unirse a nuestro equipo. Si eres una persona responsable, detallista y buscas un ambiente de trabajo dinámico y colaborativo, ¡esta es tu oportunidad!",
    "Con experiencia certificada en limpieza (preferible, no excluyente).",
    "Contar con DNI o C.E. vigente (+18).",
    "Disponibilidad inmediata para iniciar y trasladarse por su zona.",
    "Disponibilidad para trabajar 8 horas diarias, seis días a la semana"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día.",
    "Salario: S/. 1130.00 + Asignación familiar (si tienes hijos menores de edad).",
    "14 sueldos anuales (12 meses + 2 gratificaciones en julio y diciembre).",
    "Beneficios: CTS, seguro de vida, SCTR, y seguro en ESSALUD para ti y tu familia directa.",
    "Vacaciones pagadas.",
    "Oportunidades de crecimiento profesional.",
    "¿Te interesa?",
    "Escribe por WhatsApp al +51 939 849 947 para más detalles.· Limtek Servicios Integrales es una empresa comprometida con la igualdad de oportunidades y valora la diversidad en su equipo. Si cuentas con la experiencia y el enfoque meticuloso que buscamos, ¡podrías ser el candidato ideal!",
    "¡Esperamos tu postulación para formar parte de nuestro equipo!. -Requerimientos- Educación mínima: Educación Primaria años de experienciaPalabras clave: operator, operario, operator, aseo, higiene, cleaning, limpieza, permanent, fijo"
   ],
   "url": "https://www.laborum.pe/job/limtek-servicios-integrales/operario-de-limpieza-para-villa-el-salvador-planilla/6a8fbac02020b210c83bf4f0"
  },
  {
   "id": 318,
   "puesto": "Asistente de planillas",
   "empresa": "PLANET GAME S.A.C.",
   "cat": "Recursos Humanos",
   "min": 1350,
   "max": 1350,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 30,
   "vence": "2026-09-27",
   "restan": 1,
   "score": 89,
   "resumen": "RESUMEN: ASISTENTE DE PLANILLAS CHORRILLOS",
   "funciones": [
    "Registrar y actualizar información del personal en el T-Registro y sistema interno.",
    "Apoyar en la elaboración y procesamiento de planillas.",
    "Controlar y verificar asistencias, tardanzas, faltas, permisos y horas extras.",
    "Gestionar la documentación relacionada con el personal.",
    "Apoyar en el cálculo y control de beneficios laborales.",
    "Revisar y organizar la documentación necesaria para el proceso de planillas.",
    "Brindar soporte en otras actividades administrativas del área de Recursos Humanos.",
    "HORARIO DE TRABAJO",
    "Lunes a viernes: 8:00 a. m. a 6:36 p. m.",
    "Refrigerio: 1 hora.",
    "Descanso: sábados, domingos y feriados."
   ],
   "requisitos": [
    "Vivir en Chorrillos o distritos aledaños.",
    "Egresado(a) técnico o universitario de Recursos Humanos, Psicología, Administración, Contabilidad o carreras afines.",
    "Experiencia mínima de 1 año en puestos similares, de preferencia en el área de planillas.",
    "Conocimientos en administración de personal y normativa laboral.",
    "Manejo de T-Registro y PLAME.",
    "Manejo de Microsoft Excel a nivel intermedio.",
    "Persona organizada, responsable, proactiva y con capacidad para trabajar en equipo."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día.",
    "Sueldo: S/ 1,350.00",
    "Oportunidad de pertenecer a una empresa en crecimiento.",
    "Si cumples con los requisitos y estás interesado(a) en formar parte de nuestro equipo, postula por Computrabajo y nos estaremos comunicando contigo.-Requerimientos- Educación mínima: Técnico años de experienciaEdad: entre 24 y 35 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide"
   ],
   "url": "https://www.laborum.pe/job/planet-game-s-a-c/asistente-de-planillas-asistente-de-planillas/6a8fb9232020b210c83bf015"
  },
  {
   "id": 319,
   "puesto": "Asesor Cobranzas Castigada",
   "empresa": "Phoeninca",
   "cat": "Contabilidad",
   "min": 1400,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 30,
   "vence": "2026-09-27",
   "restan": 1,
   "score": 81,
   "resumen": "Phoeninca se especializa en la externalización de procesos empresariales (BPO Services) y ofrece soluciones de cobranza, atención al cliente y ventas.",
   "funciones": [
    "Gestionar cobros de manera efectiva y profesional.",
    "Con Experiencia en Cobranzas Castigada de 6 meses en adelante",
    "Asegurar el cumplimiento de los plazos establecidos.",
    "Maximizar la recuperación de deuda mediante estrategias de negociación."
   ],
   "requisitos": [
    "Asesor(a) de Cobranza Castigada Maf Judicial ( Créditos vehiculares )",
    "Actitud proactiva y orientación a resultados.",
    "Comunicación clara, persuasiva y enfocada en el cliente.",
    "Capacidad de análisis y resolución de problemas."
   ],
   "beneficios": [
    "Remuneración por encima del mercado.",
    "Sueldo 1400 a 1500 soles ( Planilla Completa con todos los beneficio de acuerdo a ley )",
    "Comisiones Ilimitadas",
    "Modalidad: Presencial.",
    "Horario Full Time",
    "Lunes a viernes: 8:00 a.m. 6:00 p.m.",
    "Sábados: 9:00 a.m. 12:00 p.m.",
    "¡Sé parte de un equipo en crecimiento y construye tu futuro con nosotros!. -Requerimientos- Educación mínima: Educación Secundaria1 año de experienciaEdad: A partir de 18 añosPalabras clave: advisor, asesor"
   ],
   "url": "https://www.laborum.pe/job/phoeninca/asesor-cobranzas-castigada/6a8fcd592020b210c83c1921"
  },
  {
   "id": 320,
   "puesto": "Operario de Lavandería Industrial S",
   "empresa": "GRAFTON PERU S.A.C.",
   "cat": "Ingeniería",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 30,
   "vence": "2026-09-27",
   "restan": 1,
   "score": 80,
   "resumen": "Lavado y clasificación de prendas.",
   "funciones": [
    "Lavado y clasificación de prendas.",
    "Doblado, planchado y empaque.",
    "Control de calidad y orden del área."
   ],
   "requisitos": [
    "Con o sin experiencia (¡te capacitamos!).",
    "Residir en Chorrillos o zonas aledañas.",
    "Responsabilidad, compromiso y trabajo en equipo.",
    "Disponibilidad de trabajar en turno fijo (mañana o tarde)"
   ],
   "beneficios": [
    "Sueldo base: S/ 1200 + Bono de movilidad S/. 300",
    "Pago de horas extras.",
    "Pagos quincenales",
    "Ingreso a planilla mype desde el primer día.",
    "Capacitación constante.",
    "Oportunidad de crecimiento.",
    "Información Adicional",
    "Modalidad: Presencial",
    "Horario fijo según disponibilidad: Turno mañana (5:30 a.m. a 2:30 p.m.) y Turno tarde (1 p.m. a 10 p.m.)",
    "Ubicación: Chorrillos. (Referencia: Prolongación huaylas altura de Liderman / Galaxy Plaza)",
    "¡Postula con tu CV actualizado!"
   ],
   "url": "https://www.laborum.pe/job/grafton-peru-s-a-c/operario-de-lavanderia-industrial-s-1500-con-o-sin/6a8f7ee72020b210c83b5de2"
  },
  {
   "id": 321,
   "puesto": "Inspector Técnico JR",
   "empresa": "RH TALENT CONSULTORA E.I.R.L.",
   "cat": "Recursos Humanos",
   "min": 1600,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 98,
   "resumen": "RESUMEN: ESTAMOS CONTRATANDO UN INSPECTOR TÉCNICO JR.",
   "funciones": [
    "Realizar visitas técnicas a los clientes asignados.",
    "Inspeccionar e identificar los daños presentes en los inmuebles.",
    "Registrar evidencia fotográfica y completar las fichas técnicas correspondientes.",
    "Elaborar informes técnicos y presupuestos de acuerdo con los daños identificados.",
    "Sustentar técnicamente los daños ante los ajustadores.",
    "Realizar apelaciones y seguimiento de casos mediante llamadas, mensajes y otros canales de comunicación.",
    "Analizar las observaciones realizadas por los ajustadores y preparar los argumentos técnicos correspondientes.",
    "Realizar seguimiento a las cotizaciones y casos asignados.",
    "Participar en reuniones con Gerencia para revisar avances, resultados y oportunidades de mejora.· ¡POSTULA YAAA!. -Requerimientos- Educación mínima: Técnico1 año de experienciaEdad: entre 20 y 30 añosPalabras clave: tecnologo, tecnico, tech, technician, technology, tecnologia, technologist, junior"
   ],
   "requisitos": [
    "Formación técnica en Arquitectura o estudios de Ingeniería.",
    "Experiencia o conocimientos en elaboración de presupuestos.",
    "Experiencia realizando visitas e inspecciones técnicas.",
    "Conocimiento para identificar y evaluar daños ocasionados por lluvias, sismos u otros eventos.",
    "Capacidad de análisis y argumentación técnica para sustentar, refutar y apelar observaciones de los ajustadores, buscando que los daños correspondientes sean reconocidos por la compañía de seguros.",
    "Disponibilidad para realizar trabajo de campo en las zonas indicadas.",
    "Contar con movilidad propia (moto o auto).",
    "Contar con laptop propia."
   ],
   "beneficios": [
    "Modalidad: Presencial Campo.",
    "Rutas de trabajo: Carabayllo, Ate y Rímac.",
    "Horario: Lunes a viernes: 9:00 a.m. a 6:00 p.m. / Sábados: 8:00 a.m. a 1:00 p.m.",
    "Sueldo: S/ 1,600 a S/ 2,000, según evaluación.",
    "Planilla MYPE",
    "Bono de movilidad: Según las rutas asignadas.",
    "Celular: Se puede brindar equipo, de ser conveniente."
   ],
   "url": "https://www.laborum.pe/job/rh-talent-consultora-e-i-r-l/buscamos-inspector-tecnico-jr-rubro-de-indemnizaciones-por/6a8e32682020b210c83a03fb"
  },
  {
   "id": 322,
   "puesto": "Atencion al Cliente Grifo Primax cruce de la Av. San Luis",
   "empresa": "COESTI S.A.",
   "cat": "Atención al Cliente",
   "min": 1330,
   "max": 1330,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 97,
   "resumen": "Si quieres pertenecer al Grupo Primax y formar parte de un equipo lleno de energía y pasión te invitamos a postular a la posición de DESPACHO DE COMBUSTIBLE FULL TIME para que cumplas las siguientes funciones:",
   "funciones": [
    "Atención al clientes de manera oportuna en despachando combustible",
    "Controlar los Boucher, vales y efectivo de las ventas de los dispensadores que le son asignados",
    "Realizar el reporte de ventas al término del turno (Cuadre de caja)",
    "Cumplir con las normas de seguridad y salud en el trabajo",
    "Mantener la EESS limpia y ordenada"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Experiencia en atención al cliente - deseable",
    "Manejo de caja - deseable",
    "Disponibilidad para trabajar en turno tarde de 2 pm a 10 pm",
    "Disponibilidad para trabajar en San Luis con San Borja Sur"
   ],
   "beneficios": [
    "Ingreso a planilla",
    "Contrato indeterminado",
    "Sueldo aproximado de S/1330 en Full Time",
    "Asignación familiar",
    "Bonos por ventas",
    "Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/atencion-al-cliente-grifo-primax-cruce-de-la-av-san-luis-con/6a8e930b2020b210c83af28b"
  },
  {
   "id": 323,
   "puesto": "Atencion al Cliente el Grifo Primax a 3 Cuadras del Mercado",
   "empresa": "COESTI S.A.",
   "cat": "Atención al Cliente",
   "min": 1230,
   "max": 1230,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 97,
   "resumen": "Si quieres pertenecer al Grupo Primax y formar parte de un equipo lleno de energía y pasión te invitamos a postular a la posición de DESPACHO DE COMBUSTIBLE para que cumplas las siguientes",
   "funciones": [
    "Atención al clientes de manera oportuna en despachando combustible",
    "Controlar los Boucher, vales y efectivo de las ventas de los dispensadores que le son asignados",
    "Realizar el reporte de ventas al término del turno - Cuadre de caja",
    "Cumplir con las normas de seguridad y salud en el trabajo",
    "Mantener la EESS limpia y ordenada"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Experiencia en atención al cliente - deseable",
    "Manejo de caja - deseable",
    "Disponibilidad para laborar turno tarde de 2 pm a 10 pm",
    "Disponibilidad para trabajar a 3 cuadras del mercado de Magdalena"
   ],
   "beneficios": [
    "Ingreso a planilla",
    "Sueldo aproximo de S/1230",
    "Bono de movilidad de S/50",
    "Contrato indeterminado",
    "Asignación familiar",
    "Bonos por ventas",
    "Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente, marketplace"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/atencion-al-cliente-el-grifo-primax-a-3-cuadras-del-mercado/6a8e93182020b210c83af2ab"
  },
  {
   "id": 324,
   "puesto": "Atencion al Cliente Grifo en Primax de la Av El Ejercito",
   "empresa": "COESTI S.A.",
   "cat": "Atención al Cliente",
   "min": 1230,
   "max": 1230,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 97,
   "resumen": "Si quieres pertenecer al Grupo Primax y formar parte de un equipo lleno de energía y pasión te invitamos a postular a la posición de DESPACHO DE COMBUSTIBLE para que cumplas las siguientes",
   "funciones": [
    "Atención al clientes de manera oportuna en despachando combustible",
    "Controlar los Boucher, vales y efectivo de las ventas de los dispensadores que le son asignados",
    "Realizar el reporte de ventas al término del turno - Cuadre de caja",
    "Cumplir con las normas de seguridad y salud en el trabajo",
    "Mantener la EESS limpia y ordenada"
   ],
   "requisitos": [
    "Secundaria Completa",
    "Deseable experiencia en atención al cliente",
    "Manejo de caja, deseable",
    "Disponibilidad para laborar turno tarde de 2 pm a 10 pm o turno mañana de 6 am a 2 pm",
    "Disponibilidad para laborar terminando el Hospital Larco Herrera"
   ],
   "beneficios": [
    "Ingreso a planilla",
    "Contrato Indeterminado",
    "Sueldo aproximado de S/1230",
    "Asignacion de movilidad al terminar el turno",
    "Linea de Carrera",
    "Asignación familiar",
    "Bonos por ventas",
    "Utilidades. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: care, costumer, cliente"
   ],
   "url": "https://www.laborum.pe/job/coesti-s-a/atencion-al-cliente-grifo-en-primax-de-la-av-el-ejercito/6a8f76972020b210c83b4022"
  },
  {
   "id": 325,
   "puesto": "Operador de montacarga Ate",
   "empresa": "GRUPO PROGESTION",
   "cat": "Logística",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 95,
   "resumen": "Operar montacargas de 4 uñas de la marca LINDE para la manipulación y traslado de carga.",
   "funciones": [
    "Operar montacargas de 4 uñas de la marca LINDE para la manipulación y traslado de carga.",
    "Ejecutar procesos de carga y descarga de camiones siguiendo estrictos protocolos de seguridad.",
    "Realizar el acomodo y ordenamiento eficiente de mercadería dentro del almacén.",
    "Inspeccionar el estado del equipo antes y después de cada jornada mediante check-list.",
    "Garantizar la integridad total de los productos durante el proceso de estiba.",
    "Mantener el orden y limpieza en las zonas de tránsito y almacenamiento."
   ],
   "requisitos": [
    "Mínimo 3 años de experiencia comprobada y certificada como Operador de Montacargas.",
    "Experiencia previa en operaciones de carga y descarga en almacenes de alto volumen.",
    "Contar con Certificado vigente como Operador de Montacargas.",
    "Licencia de conducir (Brevete) categoría A1 vigente.",
    "Dominio técnico operativo de montacargas marca LINDE y manejo de 4 uñas.",
    "Disponibilidad para laborar en Ate en horarios rotativos."
   ],
   "beneficios": [
    "Sueldo de S/ 1500 en Planilla Completa con todos los beneficios de ley desde el primer día.",
    "Bono Fijos de S/.300.00 Soles, bono de movilidad de S/. 100 soles + Alimentación cubierta al 100%",
    "Horarios rotativos: Mañana (7:00 am-15:00 pm), Tarde (15:00pm-23:00pm) y Noche (23:00 pm-7:00 am).",
    "Interesados contactar con la Srta. Paola y mandar su cv : 933470002. -Requerimientos- Educación mínima: Técnico2 años de experienciaEdad: entre 20 y 55 añosPalabras clave: operator, operario, operator, toro, carretilla, torero, carretillero, forklift, clarkista, montacarguista, montacarga"
   ],
   "url": "https://www.laborum.pe/job/grupo-progestion/operador-de-montacarga-ate/6a8e51572020b210c83a6af0"
  },
  {
   "id": 326,
   "puesto": "Supervisor(a) Comercial",
   "empresa": "Manpower",
   "cat": "Ventas",
   "min": 500,
   "max": 500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 95,
   "resumen": "RESUMEN: En Manpower Perú buscamos al mejor talento para asumir el reto como Supervisor(a) Comercial Canal Tradicional para una importante empresa líder del sector Consumo Masivo.",
   "funciones": [
    "Supervisar y monitorear el desempeño del equipo en campo.",
    "Realizar seguimiento al cumplimiento de cobertura, exhibición y objetivos comerciales.",
    "Brindar acompañamiento y soporte al equipo, identificando oportunidades de mejora.",
    "Gestionar y fortalecer la relación con clientes mayoristas y minoristas.",
    "Monitorear asistencia, productividad e indicadores de gestión.",
    "Elaborar reportes y realizar seguimiento de indicadores mediante aplicativos y herramientas de gestión.",
    "Proponer y ejecutar estrategias para mejorar los resultados de la operación."
   ],
   "requisitos": [
    "Experiencia mínima de 1 año como Supervisor(a) de Ventas, Supervisor(a) Comercial o posiciones afines.",
    "Experiencia liderando y supervisando equipos en campo.",
    "Experiencia en Canal Tradicional, trabajando con mercados, bodegas y/o clientes mayoristas y minoristas.",
    "Experiencia en gestión comercial y cumplimiento de objetivos.",
    "Manejo de Excel a nivel intermedio.",
    "Disponibilidad para realizar trabajo de campo en Lima Metropolitana.",
    "Deseable formación técnica o universitaria en Administración, Marketing o carreras afines.",
    "Deseable experiencia en empresas de Consumo Masivo."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley desde el primer día.",
    "S/ 1,600 de sueldo fijo + hasta S/ 500 de variable + S/ 400 de movilidad.",
    "Variable sujeto al cumplimiento de objetivos.",
    "Horario: lunes a sábado de 8:00 a.m. a 5:00 p.m.",
    "Trabajo principalmente en campo.",
    "Disponibilidad de posiciones para Lima Centro, Norte y Sur."
   ],
   "url": "https://www.laborum.pe/job/manpower/supervisor-a-comercial-canal-tradicional-consumo-masivo/6a8e79992020b210c83abde3"
  },
  {
   "id": 327,
   "puesto": "Ayudante de cocina",
   "empresa": "RETAIL SERVICES SAC",
   "cat": "Gastronomía",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 94,
   "resumen": "RESUMEN: En Retail Services buscamos el mejor talento para la posición Ayudante de cocina (Trozador). Te invitamos a postular y ser parte de nuestro equipo en el restaurante KION Av. El Polo - Surco.",
   "funciones": [
    "Controlar la rotación de los productos según PEPS (primero entra primero sale).",
    "Realizar el check list de sabores y mise en place antes de cada servicio.",
    "Realizar los cortes de las carnes rojas y blancas.",
    "Empacar y almacenar correctamente los productos restantes del día. (mermas entre otros)",
    "Apoyo en la elaboración de platillos.",
    "Apoyo en la limpieza de las áreas de Cocina y zona de trabajo.",
    "Básico de S/1130.00 + Recargo de Consumo S/700.00 + Propinas mensuales"
   ],
   "requisitos": [
    "Formación técnico profesional y/o Universitario de la carrera de Gastronomía.",
    "Experiencia mínima de 6 meses como trozador (proteínas, verduras, etc).",
    "Disponibilidad para trabajar en horarios rotativos.",
    "Residir en zonas aledañas a la Av. El Polo - Surco."
   ],
   "beneficios": [
    "Alimentación cubierta",
    "Ingreso a planilla desde el primer día",
    "Descuentos corporativos. -Requerimientos- Educación mínima: Técnico1 año de experienciaEdad: entre 22 y 40 añosPalabras clave: asistente, auxiliar, aux, auxiliary, assistant, helper, aide, culinario, culinary, cook, cocina"
   ],
   "url": "https://www.laborum.pe/job/retail-services-sac/ayudante-de-cocina-trozador-kion-peruvian-chinese/6a8e4d712020b210c83a5d32"
  },
  {
   "id": 328,
   "puesto": "Back office de Seguros vehiculares Sede Lurín",
   "empresa": "TCONTAKTO",
   "cat": "Ventas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 94,
   "resumen": "Actualmente nos encontramos en la búsqueda de Back office para brindar EL SOPORTE ADMINISTRATIVO Y OPERATIVO A LOS IMPLANT Y CORREDORES DE SEGUROS",
   "funciones": [
    "Emision cotizacion de seguros vehiculares",
    "Aplicación de Pagos",
    "Impresión y envio de polizas, según requerimiento",
    "Absolver consultas respecto a temas de pago, deuda de clientes, envio de estado de cuenta etc.",
    "Genarar tickets para la atencion enc aso de fallas de sistema, seguimiento de los mismos",
    "Envio a las areas correspondientes, creacion de Modelos vehiculares que nos e ecuentren en los planes actuales.",
    "Solictar y llevar control de Merchandising, gestionar el envio del mismo",
    "Lunes a Viernes: 9:00 am a 6:00 pm y Sábados de 9:00 am a 2:00 pm (Sede Concesionaria VOLVO)"
   ],
   "requisitos": [
    "Experiencia en posiciones como back office",
    "Experiencia en el rubro banca, seguros, etc.",
    "Manejo de Office nivel básico Ser proactivo, comprometido y contar con habilidades de comunicación a todo nivel.",
    "Vivir en lurin o aledaños"
   ],
   "beneficios": [
    "Sueldo fijo S/1200",
    "Bonos movilidad S/150",
    "Planilla completa desde el primer día",
    "Línea de carrera y crecimiento profesional",
    "Pagos puntuales (quincena y fin de mes)",
    "Convenios corporativos y excelente clima laboral",
    "¡Atrévete y forma parte de la Familia TContakto. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 50 añosPalabras clave: oficina, insurance, aseguradora, seguros"
   ],
   "url": "https://www.laborum.pe/job/tcontakto/back-office-de-seguros-vehiculares-sede-lurin/6a8e31ab2020b210c83a0166"
  },
  {
   "id": 329,
   "puesto": "Asesor Comercial Entel",
   "empresa": "POWER TIMBER S.A.C.",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 94,
   "resumen": "RESUMEN: Power Timber Socio Comercial de Entel",
   "funciones": [
    "Promocionar y vender productos y servicios Entel en tienda asignada.",
    "Atender de manera personalizada a los clientes, resolviendo dudas y brindando una experiencia de calidad.",
    "Cumplir con los objetivos de ventas mensuales establecidos.",
    "Mantener el orden y adecuada presentación de tienda.",
    "Registrar operaciones en el sistema según los protocolos establecidos."
   ],
   "requisitos": [
    "Educación secundaria completa (valorable formación técnica o universitaria).",
    "Deseable experiencia en ventas presenciales o atención al cliente.",
    "Habilidades de comunicación, negociación y cierre de ventas.",
    "Buena presencia y actitud proactiva.",
    "Disponibilidad para laborar en tienda fija, según zona de residencia."
   ],
   "beneficios": [
    "Sueldo fijo: S/1130 + Comisiones ilimitadas.",
    "Ingreso a planilla desde el primer día con todos los beneficios de ley: CTS, gratificaciones, vacaciones, Essalud.",
    "Planilla MYPE.",
    "Capacitaciones constantes y acompañamiento.",
    "Excelente ambiente laboral y oportunidades de crecimiento real dentro de la empresa."
   ],
   "url": "https://www.laborum.pe/job/power-timber-s-a-c/asesor-comercial-entel-ingreso-inmediato-sueldo-fijo/6a8e69002020b210c83aac7f"
  },
  {
   "id": 330,
   "puesto": "Asesor de Repuestos camacho monitor",
   "empresa": "Alese SAC",
   "cat": "Otros",
   "min": 1900,
   "max": 1900,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 92,
   "resumen": "Actualmente nos encontramos en la búsqueda de un personal para el puesto ASESOR DE REPUESTOS.",
   "funciones": [
    "Gestionar la venta de repuestos automotrices, asegurando el cumplimiento de los objetivos de facturación, margen y satisfacción del cliente mediante una atención oportuna y una gestión comercial activa.",
    "Atender y cotizar requerimientos de clientes.",
    "Gestionar oportunidades de venta mediante plataformas de aseguradoras.",
    "Realizar seguimiento a cotizaciones hasta el cierre de la venta.",
    "Ejecutar visitas comerciales para desarrollar nuevos clientes y fortalecer la cartera existente.",
    "Cumplir las metas de facturación, margen y rentabilidad.",
    "Coordinar con almacén la disponibilidad y entrega de los pedidos."
   ],
   "requisitos": [
    "Experiencia mínima en el Cargo 1 Año",
    "Conocimiento Mecánica Básica",
    "Manejo Excel Office Nivel Básico",
    "Estudios técnicos o universitarios en Administración, Mecánica Automotriz o carreras afines.",
    "Mínimo 2 años de experiencia en venta de repuestos automotrices.",
    "Experiencia en atención a talleres, clientes corporativos y compañías de seguros.",
    "Manejo de plataformas de cotización de aseguradoras (e-Taller, Inpart u otras).",
    "Experiencia en visitas comerciales a talleres, aseguradoras y clientes para captar y fidelizar negocios.",
    "Conocimiento de catálogos electrónicos (VIN/EPC), ERP y Microsoft Excel.",
    "Orientación a resultados.",
    "Negociación y cierre de ventas.",
    "Comunicación efectiva."
   ],
   "beneficios": [
    "Pertenecer a un sólido grupo empresarial.",
    "Ingreso a planilla con todos los beneficios de ley.",
    "Asimismo, el sueldo básico aplica a S/.1900 + Comisiones por cumplimiento de Objetivo¡",
    "LUGAR DE TRABAJO: CAMACHO, MONITOR , INDEPENDENCIA",
    "Contratación inmediata¡. -Requerimientos- Educación mínima: Técnico2 años de experienciaEdad: entre 24 y 55 añosPalabras clave: advisor, asesor"
   ],
   "url": "https://www.laborum.pe/job/alese-sac/asesor-de-repuestos-camacho-monitor-camacho/6a8e38232020b210c83a17e3"
  },
  {
   "id": 331,
   "puesto": "Anfitrion de estacionamiento",
   "empresa": "URBAPARK S.A.C.",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 92,
   "resumen": "Orientar y guiar al cliente durante su estadía en el estacionamiento.",
   "funciones": [
    "Orientar y guiar al cliente durante su estadía en el estacionamiento.",
    "Realizar la cobranza de la estancia en el estacionamiento.",
    "Informar las observaciones e incidencias al supervisor.",
    "Cumplir con los protocolos del servicio."
   ],
   "requisitos": [
    "Mayores de 18 años.",
    "Contar con Secundaria completa.",
    "Experiencia mínima de 6 meses en rubro retail y servicios (deseable).",
    "Actitud positiva, predisposición y ganas de aprender (primer trabajo).",
    "Disponibilidad para trabajar de lunes a domingos (Turno de 8 hrs y 45 min de refrigerio en el rango de 06:30 a.m. - 11:00 p.m.) 48 horas semanales con descanso correspondiente a ley.",
    "Disponibilidad para laborar en Real Plaza Primavera."
   ],
   "beneficios": [
    "Sueldo: S/1,130 mensual + planilla completa (CTS, gratificación, asignación familiar + vacaciones + seguro)",
    "Pago de horas extras y feriados",
    "Bono por horario nocturno.",
    "Bonos por desempeño.",
    "Capacitación constante.",
    "Línea de carrera.",
    "Excelente clima laboral.",
    "¡Postula y sé nuestro próximo Talento Urbapark!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 58 añosPalabras clave: host, hostess"
   ],
   "url": "https://www.laborum.pe/job/urbapark-s-a-c/planilla-completa-desde-el-primer-dia-anfitrion-de/6a8e42512020b210c83a35c7"
  },
  {
   "id": 332,
   "puesto": "Asistente de Almacén Full Time Topitop Jockey Plaza",
   "empresa": "Topitop",
   "cat": "Logística",
   "min": 1180,
   "max": 1180,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 92,
   "resumen": "RESUMEN: Somos una de las principales cadenas de Retail textil peruano, con presencia regional y con más de 70 tiendas a nivel nacional.",
   "funciones": [
    "Recepción y despacho de mercadería además de Gestionar el control de la recepción de las bolsas.",
    "Ingresar oportunamente las guías de recepción y/o despacho de mercadería.",
    "Reportar las discrepancias encontradas durante la recepción de mercadería dentro de las primeras 48 horas de detectado el hecho.",
    "Seguimiento y cumplir con los balanceos y/o transferencias entre tiendas."
   ],
   "requisitos": [
    "Experiencia minima de 6 meses en almacén(deseable).",
    "Secundaria completa.",
    "Contar con total disponibilidad para trabajar de lunes a sábado"
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día de labor.",
    "Sueldo fijo S/. 1180 + bono por cumplimiento de metas de tienda.",
    "20% de descuento en compras dentro de nuestras tiendas.",
    "Línea de crédito sin interés para compras en nuestras tiendas a partir del 3er mes.",
    "Oportunidad de crecimiento y desarrollo.",
    "Reconocimiento por ser elegido el colaborador del mes",
    "Las personas interesadas y que cumplan con los requisitos mencionados anteriormente, acercarse a la tienda ubicada en CC JOCKEY PLAZA",
    "ENTREVISTAS DE LUNES A VIERNES DE 10:00AM A 12:00PM O DE 3:00PM A 5:00PM",
    "ACERCARSE LLEVANDO",
    "CV IMPRESO, CERTIFICADO UNICO LABORAL, COPIA DE DNI",
    "¡No dejes pasar la oportunidad de pertenecer a la FAMILIA TOPITOP!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 añosPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, bodega, warehouse, storehouse, almacen"
   ],
   "url": "https://www.laborum.pe/job/topitop/asistente-de-almacen-full-time-topitop-jockey-plaza/6a8e97b62020b210c83afd2f"
  },
  {
   "id": 333,
   "puesto": "Agente back office rubro salud",
   "empresa": "A365",
   "cat": "Atención al Cliente",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Híbrido",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 92,
   "resumen": "RESUMEN: Únete a nuestro equipo de Agentes de Call Center y contribuye a brindar servicios de salud excepcionales a nuestros asegurados!",
   "funciones": [
    "Programar y organizar citas médicas de todas las especialidades en el sistema.",
    "Registro diario de gestiones realizadas",
    "Utilizar diversas plataformas y aplicaciones para gestión de citas y exámenes de laboratorio.",
    "Coordinar y registrar teleconsultas médicas.",
    "Comunicación constante con el equipo de trabajo."
   ],
   "requisitos": [
    "Experiencia en back oficce y/o call center (mín. 3 meses)",
    "Deseable conocimiento en servicio de agendamiento de citas.",
    "Equipo PC/Laptop y conexión a Internet cableada para trabajar desde casa (Modalidad Semipresencial).",
    "Disponibilidad inmediata",
    "Residir en Lima, aledaño a San Isidro",
    "Excelente nivel de comunicación y compromiso.",
    "Compromiso y responsabilidad con la gestión.",
    "Otras funciones indicadas por el Supervisor Inmediato."
   ],
   "beneficios": [
    "48 horas semanales",
    "Rango horario de 8horas diarias de Lunes a Domingo: 7:00am a 11:00pm rotativo",
    "Descanso rotativo",
    "Salario: S/1130 + S/170 (bono variable) + S/35 (bono internet).",
    "Ubicación: San Isidro (cerca a estación Canaval y Moreyra)",
    "Oportunidad de línea de carrera.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 añosPalabras clave: demostrador, feriante, viajante, agent, representative, agente, representante, oficina, health, salut"
   ],
   "url": "https://www.laborum.pe/job/a365/agente-back-office-rubro-salud-semipresencial-san-isidro/6a8e5b672020b210c83a8b20"
  },
  {
   "id": 334,
   "puesto": "Jefe",
   "empresa": "Adecco Perú S.A.",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 92,
   "resumen": "Reponer la mercadería en sala de ventas según el tipo de reposición de la tienda y tomando en cuenta los bloques de reposición, asegurando la rotación de los productos. Manteniendo las góndolas ordenadas y limpias.",
   "funciones": [
    "Colocar las viñeras de precios y asegurar que las mismas coincidan con los productos exhibidos.",
    "Realizar otras funciones afines o complementarias asignadas por su Jefe inmediato con el fin de fomentar y garantizar el cumplimiento de los estándares de calidad y seguridad de los productos, colaboradores y clientes, así como el de los reglamentos de la empresa buscando capacitarse de manera proactiva.",
    "Realizar la recepción, almacenamiento y reposición de mercadería."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Disponibilidad para laborar en estos horarios",
    "Lunes a domingo de 7:00 am a 4:00 pm u 8:00 am a 5:00 pm / Descanso Lunes o Martes.",
    "03 meses como mínimo de experiencia como Mercaderista, almacenero, reponedor, distribuidor y/o similar.",
    "Disponibilidad para laborar en Plaza Vea San Borja y Vivanda Javier Prado."
   ],
   "beneficios": [
    "Sueldo s/.1,130 + Movilidad s/.250 + Alimentación s/.120 + Variable s/.100.",
    "Ingreso a planilla 100% de Adecco desde el primer día con todos los beneficios de ley (CTS, Gratificación, Vacaciones, Seguro Social, Seguro Vida Ley, Asignación Familiar, entre otros).",
    "Entrega de uniforme.",
    "Programa de capacitación a través de nuestra aula virtual.",
    "Hasta 75% de descuentos corporativos en los rubros de Educación, Salud, Belleza y Transporte.",
    "¡Promovemos el empleo formal, brindando trabajo a más personas!. -Requerimientos- Educación mínima: Educación Primaria años de experienciaPalabras clave: comerciante, merchandiser, merchant, nominas, sueldo, paga, retribucion, salary, payslip, payroll, wage, pay, payment, nominista"
   ],
   "url": "https://www.laborum.pe/job/adecco-peru-s-a/mercaderista-san-fernando-canal-moderno-s-1600-plaza-vea/6a8f7c582020b210c83b559c"
  },
  {
   "id": 335,
   "puesto": "Jefe",
   "empresa": "Overall Strategy",
   "cat": "Recursos Humanos",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 92,
   "resumen": "RESUMEN: ¡Trabaja con nosotros! | Vajillero Dish Washer Restaurante en Santiago de Surco",
   "funciones": [
    "Lavar y ordenar los cubiertos, cristalería y vajilla, para mantenerlos disponibles para la atención.",
    "Velar por el mantenimiento y buen estado las máquinas y equipos que opera, para identificar fallas o necesidades de cambio.",
    "Velar por la integridad de los materiales e insumos a su cargo, con la finalidad de mantener la operatividad del servicio.",
    "Otras funciones inherentes a la naturaleza del puesto y/o asignadas por el Jefe inmediato."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia de 6 meses en puestos similares en restaurantes, cafeterías, fast food o concesionarios.",
    "Conocimiento en limpieza e higiene de cocina, uso de limpiadores y desinfectantes.",
    "Disponibilidad para laborar Lunes a Sábado en turnos rotativos."
   ],
   "beneficios": [
    "Horario: Lunes a sábado 7am a 4pm",
    "Modalidad: Presencial Surco.",
    "Tipo de contrato: 3 meses renovable.",
    "Sueldo: S/ 1200 + Bono de movilidad S/ 100.",
    "Capacitación y línea de carrera.",
    "Beneficios: Planilla Overall desde el primer día y de acuerdo a ley.",
    "Alimentación cubierta al 100%.",
    "¡No pierdas la oportunidad de formar parte de esta nueva apertura y crecer junto a un gran equipo!. -Requerimientos- Educación mínima: Técnico1 año de experienciaEdad: entre 20 y 30 añosPalabras clave: nominas, sueldo, paga, retribucion, salary, payslip, payroll, wage, pay, payment, nominista, restaurador"
   ],
   "url": "https://www.laborum.pe/job/overall-strategy/vajillero-dish-washer-surco-sueldo-s-1-300-soles-planilla/6a8e894f2020b210c83ad50f"
  },
  {
   "id": 336,
   "puesto": "Promotor",
   "empresa": "Manpower",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 92,
   "resumen": "RESUMEN: ¡En Manpower Perú estamos en búsqueda de un/a mecaderista de Ruta que quiera brillar en grande!",
   "funciones": [
    "Asesorar, promover y orientar al cliente en la compra efectiva de los productos.",
    "Realizar ruta diaria en base a los pdv asignados por supervisor de la zona elegida",
    "Realizar visitas de 15 a 30 aprox PDV diarios.",
    "Soporte en la reposición de mercadería, rotular, gestionar los palets, asesorar en base a la asignación e implementación de material POP",
    "Levantar información en general (Precios, prospección, afiliados, quiebres de stock, vencimiento, etc)"
   ],
   "requisitos": [
    "Experiencia mínima como Mercaderista en canal tradicional o moderno, consumo masivo, reposición, multifunción, Promotor(a) y/o afines.",
    "Con conocimiento técnico para la venta de productos de cartera. (Deseable)",
    "Disponibilidad para laborar en ruta, sujeto a la necesidad de la operación. De Lunes a Sábado de 8am a 5pm con un día de descanso a la semana(domingos)(1h refrigerio)."
   ],
   "beneficios": [
    "Planilla desde el primer día.",
    "Sueldo superior al mercado: S/1,130 + variable S/300 + S/200 movilidad",
    "Línea de carrera y beneficios corporativos.",
    "Y lo mejor: un espacio donde todos los talentos son bienvenidos.",
    "Si buscas un futuro mejor aquí empieza tu historia.-Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: marketplace, comerciante, merchandiser, merchant"
   ],
   "url": "https://www.laborum.pe/job/manpower/mercado-productores-busca-mercaderista-canal-tradicional/6a8e8dda2020b210c83ae11f"
  },
  {
   "id": 337,
   "puesto": "Auxiliar de Reparto de Gaseosas C",
   "empresa": "Grupo Expro",
   "cat": "Recursos Humanos",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 91,
   "resumen": "RESUMEN: En Grupo Expro, consultora estratégica en búsqueda de talento para nuestro cliente, empresa líder en el sector logística:",
   "funciones": [
    "Repartir los productos que distribuye la empresa (gaseosas, energizantes, agua embotellada), ofreciendo un buen servicio al cliente y asegurando una distribución y atención oportuna.",
    "Verificar la carga del camión de reparto.",
    "Organizar los comprobantes de pago de los clientes de la ruta.",
    "Entregar el pedido de cada cliente con eficacia y responsabilidad.",
    "Recoger los cambios de producto programados, según procedimiento establecido.",
    "Entregar el cobro total del pedido entregado al encargado del Camión.",
    "Apoyo en el cuadre de envases y organización de envases.",
    "Entre otras funciones que designe el Supervisor.",
    "¡Postula ya! ¡El ingreso es inmediato!. -Requerimientos- Educación mínima: Educación Secundaria años de experienciaPalabras clave: ayudante, asistente, auxiliar, aux, auxiliary, assistant, helper, aide, delivery"
   ],
   "requisitos": [
    "Secundaria completa",
    "Con o sin experiencia en reparto",
    "Con mucha actitud positiva y ganas de aprender",
    "Disponibilidad para trabajar en la modalida de horario no fiscalizado de Lunes a Sábado de 6:30 am hasta FINALIZAR LA RUTA DE REPARTO DE PRODUCTOS (no hay horario fijo ni aproximado de salida)"
   ],
   "beneficios": [
    "Ingreso a planilla completa al 100% desde el 1er día (Gratificación en Julio y Diciembre, CTS, Vacaciones, Seguro EsSalud y Seguro Vida Ley cubiertos por la empresa)",
    "Remuneración: Sueldo fijo S/ 1,300.00 + Asig. Fam S/113.00",
    "Bonos de productividad: S/200.00",
    "Tarjeta de alimentos: S/91.00",
    "Entrega de Uniformes (verano e invierno)",
    "Alimentación cubierta: Desayuno power (contiene un desayuno + un plato de comida)",
    "Movilidad de acercamiento para Lima Este"
   ],
   "url": "https://www.laborum.pe/job/grupo-expro/auxiliar-de-reparto-de-gaseosas-c-sin-exp-huachipa-planilla/6a8e56662020b210c83a7b31"
  },
  {
   "id": 338,
   "puesto": "Operador de Cargador Frontal Huachipa",
   "empresa": "Petramás sac",
   "cat": "Ingeniería",
   "min": 3200,
   "max": 3200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 90,
   "resumen": "Nos encontramos en la búsqueda del mejor talento para la posición de un Operador de Cargador Frontal.",
   "funciones": [
    "Cumplir con las instrucciones del supervisor de plataforma .",
    "Revisar diariamente la maquinaria previamente al iniciar actividades.",
    "Reportar desperfecto de maquinaria al supervisor.",
    "Extraer el material de cobertura de las canteras o cerros aledaños a la plataforma."
   ],
   "requisitos": [
    "Secundaria completa (contar con copia de certificado de secundaria o C4 de Reniec).",
    "3 años de experiencia laboral operando cargador frontal (contar con certificado).",
    "Contar con certificación como operador de equipos y maquinaria pesada.",
    "Licencia AIIB."
   ],
   "beneficios": [
    "Horario: De lunes a sábado de 6:00 am a 2:00 pm o 2:00pm a 10:00pm (turnos rotativos) descanso día domingo.",
    "Lugar de trabajo: Planta Huaycoloro (Huachipa).",
    "Sueldo bruto: S/3200.00 soles brutos (Básico + Bonos)",
    "Ingreso a planilla con todos los beneficios de ley + Movilidad de acercamiento."
   ],
   "url": "https://www.laborum.pe/job/petramas-sac/operador-de-cargador-frontal-huachipa-huachipa/6a8e40f22020b210c83a3323"
  },
  {
   "id": 339,
   "puesto": "Analista de Procesamiento de Datos",
   "empresa": "Grupo Tawa",
   "cat": "Tecnología",
   "min": 3000,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 89,
   "resumen": "RESUMEN: Analista de Procesamiento de Datos Retail Analytics",
   "funciones": [
    "Analizar información comercial y generar reportes para la toma de decisiones del equipo de ventas.",
    "Realizar seguimiento de indicadores comerciales (KPIs) y cumplimiento de objetivos de la fuerza de ventas.",
    "Gestionar información relacionada al sell out, targets comerciales y resultados de ventas.",
    "Consolidar, procesar y validar bases de datos para asegurar la calidad de la información.",
    "Elaborar reportes utilizando herramientas de análisis de datos."
   ],
   "requisitos": [
    "Profesional egresado o titulado de carreras como",
    "Ingeniería de Sistemas.",
    "Ingeniería Industrial.",
    "Carreras afines.",
    "Experiencia mínima de 2 a 3 años en posiciones similares dentro del sector Retail.",
    "Experiencia en",
    "Análisis de datos comerciales.",
    "Seguimiento de ventas y KPIs.",
    "Elaboración de reportes comerciales.",
    "Gestión y procesamiento de información.",
    "Manejo avanzado de",
    "Tablas dinámicas."
   ],
   "beneficios": [
    "EPS cubierta al 70% por la empresa.",
    "Desarrollo profesional en una empresa líder del sector tecnológico.",
    "Participación en proyectos de análisis comercial y transformación de datos."
   ],
   "url": "https://www.laborum.pe/job/grupo-tawa/analista-de-procesamiento-de-datos-retail-analytics-sector/6a8f7c242020b210c83b54bb"
  },
  {
   "id": 340,
   "puesto": "Operario de Limpieza para Oficinas Corporativas",
   "empresa": "Grupo Tawa",
   "cat": "Ingeniería",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 87,
   "resumen": "¿Buscas una oportunidad laboral en una empresa líder del sector tecnológico?",
   "funciones": [
    "Realizar la limpieza y orden de las oficinas del área de Servicio Técnico.",
    "Recepcionar equipos entregados por los clientes, verificando la correcta atención y derivación correspondiente.",
    "Brindar soporte operativo para mantener una adecuada organización del área.",
    "Atender a los clientes con amabilidad y orientación durante la entrega de equipos."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de 3 a 6 meses en posiciones similares como",
    "Auxiliar de servicios generales, limpieza de oficina.",
    "Atención al cliente.",
    "Recepción de equipos.",
    "Apoyo operativo.",
    "Disponibilidad para laborar en Miraflores.",
    "Personas con actitud de servicio, responsabilidad y compromiso.",
    "Honestidad e integridad.",
    "Empatía y buen trato al cliente.",
    "Orden y compromiso."
   ],
   "beneficios": [
    "Ingreso a planilla con beneficios de ley.",
    "Seguro y cobertura correspondiente.",
    "Oportunidad de formar parte de una empresa reconocida del sector tecnológico."
   ],
   "url": "https://www.laborum.pe/job/grupo-tawa/operario-de-limpieza-para-oficinas-corporativas-miraflores/6a8f77e92020b210c83b45a2"
  },
  {
   "id": 341,
   "puesto": "Promotor de ventas WIN",
   "empresa": "Impulsate",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 86,
   "resumen": "RESUMEN: ¡GANA S/ 3,200+ AL MES CON WIN!",
   "funciones": [
    "Vender servicios de Internet Fibra Óptica y TV en casas",
    "Captar nuevos clientes en las zonas asignadas.",
    "Asesorar a los clientes y cerrar ventas.",
    "Cumplir objetivos y alcanzar tus metas comerciales."
   ],
   "requisitos": [
    "Con o sin experiencia en ventas.",
    "Buena comunicación y actitud comercial.",
    "Orientación a resultados.",
    "Disponibilidad para trabajar en campo.",
    "Ganas de aprender, crecer y generar ingresos altos.",
    "Lunes a sábado",
    "9:00 a.m. a 6:00 p.m.",
    "ZONA DE TRABAJO: LIMA ESTE - SAN JUAN DE LURIGANCHO, EL AGUSTINO, ATE Y ZONAS ALEDAÑAS",
    "¡VACANTES LIMITADAS!",
    "¿Quieres empezar a generar ingresos y crecer con nosotros? . -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: entre 18 y 70 añosPalabras clave: promoter, promotor, cambaceo, sales"
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,130",
    "Comisiones ilimitadas SIN TOPE",
    "Ingreso promedio: S/ 3,200+ al mes",
    "Bono de movilidad S/ 400",
    "Bono semanal de hasta S/ 100",
    "Planilla desde el primer día",
    "Capacitación constante",
    "Línea de carrera: Supervisor / Jefe de Ventas"
   ],
   "url": "https://www.laborum.pe/job/impulsate/promotor-de-ventas-win-sede-lima-este-400-de-movilidad/6a8e32c72020b210c83a0581"
  },
  {
   "id": 342,
   "puesto": "Trabaja Todo Desde Casa! Asesor Ventas",
   "empresa": "Atento Perú",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 83,
   "resumen": "RESUMEN: ¡TRABAJA DESDE CASA! | ASESOR(A) DE VENTAS CALL CENTER PORTABILIDAD MOVISTAR",
   "funciones": [
    "En Atento estamos buscando Asesores(as) de Ventas para Movistar Perú con el objetivo de generar cierre de ventas ofreciendo una atención de calidad.",
    "Escoge Tu Horario",
    "Lunes a sábado: 9:00 a.m. a 6:00 p.m.",
    "Lunes a sábado: 10:00 a.m. a 7:00 p.m.",
    "Descanso fijo: domingos y feriados"
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima 3 meses Call Center.",
    "Contar con PC o laptop propia."
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,130",
    "Planilla completa desde el primer día de contrato: CTS, gratificaciones, asignación familiar, EsSalud, vacaciones y utilidades.",
    "Comisiones ilimitadas (S./ 226 soles en tarjeta de alimento + lo restante en efectivo)",
    "CAPACITACIÓN (remunerada)",
    "Duración: 12 días (8 teórico y práctico + 4 pre conexión)",
    "Horario: lunes a sábado de 9:00 a.m. a 4:00 p.m.",
    "Pago: S/ 25 por día",
    "Modalidad: Remoto (Lima y provincias)."
   ],
   "url": "https://www.laborum.pe/job/atento-peru/trabaja-todo-desde-casa-asesor-ventas-lunes-a-sabado-con-o/6a8e8bb72020b210c83adacf"
  },
  {
   "id": 343,
   "puesto": "Asesor Comercial para Entel",
   "empresa": "Actitud Group SAC",
   "cat": "Ventas",
   "min": 1400,
   "max": 1400,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Laborum",
   "dias": 31,
   "vence": "2026-09-26",
   "restan": 0,
   "score": 81,
   "resumen": "RESUMEN: ¿Buscas un trabajo estable donde puedas incrementar tus ingresos y crecer profesionalmente?",
   "funciones": [
    "Contactar empresas para ofrecer los servicios de Entel Empresas.",
    "Brindar asesoría comercial y detectar oportunidades de negocio.",
    "Dar seguimiento a prospectos hasta el cierre de la venta.",
    "Cumplir con los indicadores comerciales establecidos."
   ],
   "requisitos": [
    "Sean dinámicas, proactivas y con facilidad de comunicación.",
    "Manejen herramientas informáticas a nivel básico.",
    "Tengan muchas ganas de aprender y generar altos ingresos.",
    "Modalidad: Presencial Miraflores (Francisco Bolognesi 125). -Requerimientos- Educación mínima: Educación Secundaria años de experienciaEdad: A partir de 18 añosPalabras clave: advisor, asesor, salesman, saleswoman, seller, vendedor, comercial, salesperson"
   ],
   "beneficios": [
    "Sueldo fijo de S/ 1,400",
    "Comisiones ilimitadas.",
    "Bonos de productividad",
    "Horario de lunes a viernes de 9:00 a. m. a 6:00 p. m.",
    "Descanso sábados, domingos y feriados.",
    "Capacitación a cargo de la empresa.",
    "Excelente clima laboral."
   ],
   "url": "https://www.laborum.pe/job/actitud-group-sac/asesor-comercial-para-entel/6a8e52282020b210c83a6e06"
  },
  {
   "id": 344,
   "puesto": "Practicante Profesional de Comercio Exterior e Inteligencia Comercial",
   "empresa": "Protiviti Perú",
   "cat": "Prácticas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 34,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 97,
   "resumen": "En Protiviti Perú nos encontramos en búsqueda de un(a) Practicante Profesional de Comercio Exterior y Análisis de Datos para uno de nuestros importantes clientes, Beta Complejo Industrial .",
   "funciones": [
    "Elaborar, actualizar y analizar reportes e indicadores comerciales .",
    "Diseñar y mejorar reportes utilizando Excel y Power BI .",
    "Analizar información y generar insights que contribuyan a la toma de decisiones.",
    "Dar soporte en actividades relacionadas con comercio internacional y gestión comercial .",
    "Apoyar en la elaboración y seguimiento de información para clientes y/o áreas internas.",
    "Participar en la mejora y automatización de reportes y herramientas de análisis."
   ],
   "requisitos": [
    "Estudiantes de últimos ciclos o recién egresados de Administración, Comercio Exterior, Ingeniería Industrial o carreras afines .",
    "Experiencia o conocimientos en análisis de datos y elaboración de reportes .",
    "Manejo de Excel a nivel intermedio o avanzado .",
    "Inglés avanzado , principalmente para comunicación y gestión relacionada con comercio internacional.",
    "Persona analítica, organizada, proactiva y con interés en desarrollarse en áreas de análisis y gestión comercial.",
    "Disponibilidad para trabajar de manera presencial al 100% en Lima .",
    "Disponibilidad para trabajar en jornada completa de 8:00 a.m. a 5:00 p.m."
   ],
   "beneficios": [
    "💰 Subvencion de S/ 1,200 mensuales.",
    "🏢 Modalidad 100% presencial .",
    "🕐 Jornada laboral de lunes a viernes de 8:00 a.m. a 5:00 p.m.",
    "🩺 Seguro FOLA .",
    "📄 Convenio de Practicas",
    "Oportunidad de desarrollar experiencia en comercio exterior, análisis de datos e inteligencia comercial .",
    "Si tienes ganas de aprender, te apasiona el análisis y quieres potenciar tu experiencia profesional en un entorno dinámico, postula y sé parte de nuestro equipo ."
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-profesional-de-comercio-exterior-e-inteligencia-comercial-protiviti-peru-1118412965.html"
  },
  {
   "id": 345,
   "puesto": "Auxiliar de Almacén",
   "empresa": "ManpowerGroup Perú",
   "cat": "Logística",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 34,
   "vence": "2026-11-21",
   "restan": 56,
   "score": 86,
   "resumen": "encargo de uno de nuestros clientes, nos encontramos en búsqueda de talentos para cubrir la posición de:",
   "funciones": [
    "Recepción de mercadería",
    "Almacenamiento y organización",
    "Despacho de productos",
    "Control de inventario",
    "Manejo de documentos y registros",
    "Apoyo logístico",
    "Cumplimiento de normas de seguridad"
   ],
   "requisitos": [
    "Secundaria completa",
    "Con o Sin Experiencia",
    "Disponibilidad completa e inmediata",
    "Disponibilidad para trabajar de manera PRESENCIAL en HUACHIPA (limite con Chosica/Ate)",
    "HORARIO ROTATIVO",
    "Turno mañana: 7:00 am – 4:00 pm (descanso domingos)",
    "Turno noche: 8:00 pm – 5:00 am (descanso sábados)",
    "Disponibilidad para realizar horas extras (PAGADOS)",
    "Disponibilidad para laborar feriados (PAGADOS)"
   ],
   "beneficios": [
    "S/. 1300 + S/150 tarjeta SODEXO + bono nocturno + Bono de horas extras + convenios corporativos + descuentos del 30% en MARATHON.",
    "Planilla completa.",
    "En Manpower promovemos la inserción e inclusión laboral de personas con discapacidad."
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-de-almacen-planilla-marathon-huachipa-con-o-sin-experiencia-manpowergroup-peru-1118413007.html"
  },
  {
   "id": 346,
   "puesto": "Ejecutivo de Admisión Emergencias",
   "empresa": "Clínica Providencia",
   "cat": "Recursos Humanos",
   "min": 1250,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 35,
   "vence": "2026-11-20",
   "restan": 55,
   "score": 94,
   "resumen": "Clínica Providencia se encuentra en la búsqueda de los mejores talentos para que forme parte de su equipo de Clínica Providencia se encuentra en búsqueda de los mejores talentos para la posición de Ejecutivo de Admisión Emergencias.",
   "funciones": [
    "Generar la atención y registros de pacientes (Identificando atención por SOAT, SCTR o convenios).",
    "Tramitar cartas de garantía.",
    "Explicar al paciente coberturas, presupuestos y documentos hospitalarios.- Revisión y organización de expedientes SOAT",
    "Registrar, cobrar y agendar citas requerido por el paciente y/o medico tratante.",
    "Ingreso y procedimientos al sistema Spring.",
    "Cuadre de caja.",
    "Otras funciones inherentes al puesto."
   ],
   "requisitos": [
    "Mínimo 6 meses de experiencia en posiciones similares.",
    "Técnico o Bachiller en administración, contabilidad o carreras a fines.",
    "Manejo de sistema SITEDS y cobertura de seguros.",
    "Experiencia en caja, equipo POS.",
    "Disponibilidad para cubrir turnos rotativos (2x2)",
    "Disponibilidad para trabajar en San Miguel."
   ],
   "beneficios": [
    "Se ofrece sueldo s/. 1250 + riesgo de caja s/. 100. + bono nocturno",
    "Capacitaciones pagadas.",
    "Ingreso a planilla completa desde el primer día de trabajo.",
    "Acceso a Descuentos corporativos.",
    "Tarifas especiales para colaboradores y familiares directos"
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-de-admision-emergencias-clinica-providencia-1118220420.html"
  },
  {
   "id": 347,
   "puesto": "Operador de Maquinaria",
   "empresa": "Confidencial",
   "cat": "Logística",
   "min": 2300,
   "max": 2300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 35,
   "vence": "2026-12-20",
   "restan": 85,
   "score": 92,
   "resumen": "Buscamos Operador de Maquinaria con experiencia en cargador frontal para importante empresa industrial cementera. Elije de acuerdo a tu ubicación una de nuestras sedes Callao o Villa el Salvador. Postula para más información.",
   "funciones": [
    "Operar cargador frontal y montacarga para la alimentación de tolvas y el traslado de materiales.",
    "Apoyar en los despachos de productos terminados..",
    "Realizar la inspección diaria del equipo antes de iniciar labores.",
    "Reportar cualquier anomalía o condición insegura.",
    "Mantener el orden y limpieza del área de trabajo.",
    "Cumplir con los procedimientos de seguridad, calidad y medio ambiente."
   ],
   "requisitos": [
    "Experiencia operando cargador frontal.",
    "Licencia de conducir A1 o superior",
    "Certificado de operador de cargador frontal vigente.",
    "Deseable experiencia en plantas industriales."
   ],
   "beneficios": [
    "Sueldo: S/ 2,300",
    "Almuerzo subvencionado a S/ 2.50",
    "Utilidades de 1 a 2 sueldos",
    "Planilla con todos los beneficios de ley",
    "Estabilidad laboral y oportunidades de crecimiento",
    "Capacitación constante en seguridad y operación de equipos."
   ],
   "url": "https://www.bumeran.com.pe/empleos/operador-de-maquinaria-montacargas-cargador-frontal-callao-villa-el-salvador-1118412778.html"
  },
  {
   "id": 348,
   "puesto": "Especialista en Gestión de Proyectos",
   "empresa": "Caja de Pensiones Militar Policial",
   "cat": "Ingeniería",
   "min": 5000,
   "max": 5000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 35,
   "vence": "2026-11-20",
   "restan": 55,
   "score": 89,
   "resumen": "Realizar las actividades de planificación, seguimiento, ejecución y cierre de los proyectos a cargo de la Gerencia de Informática bajo los lineamientos, procedimientos y metodologías establecidos.",
   "funciones": [
    "Realizar las actividades de planificación, seguimiento, ejecución y cierre de los proyectos a cargo de la Gerencia de Informática bajo los lineamientos, procedimientos y metodologías establecidos.",
    "Definir el alcance de los p royectos de informática, telecomunicaciones o innovación tecnológica, estableciendo el alcance del proyecto.",
    "Gestionar la atención de lo s requerimientos de desarrollo y mantenimiento de los sistemas de información de l a CPMP.",
    "Proponer la planificación y estimación de los recursos necesarios para la ejecución de los proyectos de informática, telecomunicaciones o innovación tecnológica incluyendo la gestión de presupuestos.",
    "Identificar y gestionar los recursos, tareas y riesgos del proyecto de informática, telecomunicaciones o innovación tecnológica, así como elaboración de acciones correctivas y preventivas.",
    "Brindar asistencia técnica en la formulación del plan operativo institucional y presupuesto anual.",
    "Realizar la gestión y seguimiento de los proyectos de informática, telecomunicaciones o innovación tecnológica a cargo de la Gerencia de informática.",
    "Realizar el seguimiento de los proyectos para el cierre de cada proyecto según el cronograma establecido.",
    "Mantener actualizados las partes esenciales del proceso de gestión de los proyectos durante todas las etapas del proyecto.",
    "Elaborar los informes y métricas de desempeño de los proyectos a cargo de la Gerencia de Informática, de acuerdo con la frecuencia establecida.",
    "Realizar los informes de seguimiento de los proyectos a cargo de la Gerencia de Informática.",
    "Formular propuestas de elaboración y modificación de lineamientos y procedimientos que ayuden a mejorar las directivas en la gestión de proyectos."
   ],
   "requisitos": [
    "Título profesional o bachiller en ingeniería de sistemas, ingeniería informática, ciencias de la computación o afines.",
    "Experiencia general de cuatro (4) años y experiencia especifica de tres (3) años acumulable como especialista, analista o puesto similar.",
    "De preferencia capacitación especializada en gestión de proyectos, metodologías ágiles o gestión pública, acreditada mediante cursos que totalicen una duración no menor a treinta horas (30) horas lectivas."
   ],
   "beneficios": [
    "Remuneración: S/. 5,000.00",
    "Horario de Trabajo regular: 9:00am a 5:15pm de lunes a viernes (presencial).",
    "Capacitación acorde a la necesidad del puesto.",
    "Los datos personales que el postulante facilite a través de su hoja de vida, podrán ser recopilados, tratados, e incorporados a un Banco de Datos Personales de la CPMP, solo para dichos fines específicos y serán conservados por un periodo máximo de dos años.",
    "La Caja de Pensiones Militar Policial - CPMP declara que respeta los principios de legalidad, consentimiento, finalidad, proporcionalidad, calidad, disposición de recurso, nivel de protección adecuado, conforme a las disposiciones de la Ley de Protección de Datos Personales vigente en Perú\"",
    "\" En la Caja de Pensiones Militar Policial creemos firmemente en la igualdad de oportunidades por lo cual promovemos la diversidad de género, origen étnico, nacionalidad, la inclusión de personas con discapacidad y/o habilidades diferentes (de acuerdo con lo establecido en la Ley N° 29973).\""
   ],
   "url": "https://www.bumeran.com.pe/empleos/especialista-en-gestion-de-proyectos-caja-de-pensiones-militar-policial-1118412764.html"
  },
  {
   "id": 349,
   "puesto": "Auxiliar de Almacén en el Nuevo Aeropuerto",
   "empresa": "Consultora CyL",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 35,
   "vence": "2026-10-21",
   "restan": 25,
   "score": 83,
   "resumen": "CONVOCATORIA: Auxiliar de Carga C/S experiencia ✈️",
   "funciones": [
    "🛠️ Realizar tareas de apoyo al líder de carga y descarga, en el manejo de bultos y equipaje.",
    "🛠️ Ejecutar procesos de recepción, almacenamiento, paletizado y despacho de carga.",
    "🛠️ Mantener el orden y la seguridad en las operaciones, cumpliendo los procedimientos y normas de la empresa."
   ],
   "requisitos": [
    "📝 Secundaria completa",
    "📝 Vivir en zonas aledañas al Callao",
    "📝 Bienestar físico",
    "📝 Disponibilidad para turnos rotativos"
   ],
   "beneficios": [
    "✅ Planilla desde el primer día + Beneficios de ley al 100%",
    "✅ Sueldo S/.1130 + horas extras + bonos",
    "✅ Alimentación cubierta al 80%",
    "✅ Capacitaciones y línea de carrera"
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-de-almacen-en-el-nuevo-aeropuerto-consultora-cyl-1118412593.html"
  },
  {
   "id": 350,
   "puesto": "Practicante Profesional",
   "empresa": "Caja de Pensiones Militar Policial",
   "cat": "Prácticas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 36,
   "vence": "2026-11-19",
   "restan": 54,
   "score": 94,
   "resumen": "Somos la empresa “Caja de Pensiones Militar Policial” ubicada en el distrito de San Isidro, y actualmente estamos en búsqueda de un nuevo talento para el puesto de Practicante Profesional para el Departamento de Contabilidad",
   "funciones": [
    "Apoyar en el registro de compras y ventas para la declaración de impuestos",
    "Colaborar en el análisis de cuentas por cobrar vinculadas a la actividad inmobiliaria",
    "Apoyar en la gestión y archivo de escritos presentados ante SUNAT",
    "Realizar consultas masivas de comprobantes de pago electrónicos (compras y ventas)",
    "Brindar apoyo en trámites documentarios y requerimientos internos del área",
    "Otras funciones afines que le asigne el jefe inmediato"
   ],
   "requisitos": [
    "Egresado/a universitario/a o Bachiller de la carrera de Contabilidad.",
    "De preferencia, conocimiento en Tributación: IGV, detracciones , retenciones y declaración de Libros Electrónicos",
    "FUNCIÓN BÁSICA",
    "Brindar soporte operativo, técnico y administrativo al Departamento de Contabilidad, contribuyendo al cumplimiento de los procesos contables, tributarios y documentarios de la institución."
   ],
   "beneficios": [
    "Subvención : S/. 1,130.00",
    "Horario: 9:00 am a 5:15 pm de lunes a viernes (presencial).",
    "Modalidad de contratacion: Convenio.",
    "Seguro FOLA.",
    "Capacitación acorde a la necesidad del puesto.",
    "Los datos personales que el postulante facilite a través de su hoja de vida, podrán ser recopilados, tratados, e incorporados a un Banco de Datos Personales de la CPMP, solo para dichos fines específicos, y serán conservados por un periodo máximo de dos años.",
    "La Caja de Pensiones Militar Policial - CPMP declara que respeta los principios de legalidad, consentimiento, finalidad, proporcionalidad, calidad, disposición de recurso, nivel de protección adecuado, conforme a las disposiciones de la Ley de Protección de Datos Personales vigente en Perú\"",
    "\" En la Caja de Pensiones Militar Policial creemos firmemente en la igualdad de oportunidades por lo cual promovemos la diversidad de género, origen étnico, nacionalidad, la inclusión de personas con discapacidad y/o habilidades diferentes (de acuerdo con lo establecido en la Ley N° 29973).\""
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-profesional-dpto.-de-contabilidad-caja-de-pensiones-militar-policial-1118412438.html"
  },
  {
   "id": 351,
   "puesto": "Abogado",
   "empresa": "Caja de Pensiones Militar Policial",
   "cat": "Legal",
   "min": 3700,
   "max": 3700,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 36,
   "vence": "2026-11-19",
   "restan": 54,
   "score": 89,
   "resumen": "Estamos creciendo contigo! Somos la empresa “Caja de Pensiones Militar Policial”, ubicada en el distrito de San Isidro y actualmente estamos en búsqueda de un nuevo talento para el puesto de ABOGADO - ANALISTA DE SERVICIOS INMOBILIARIOS.",
   "funciones": [
    "Revisar los contratos referentes a los servicios de seguridad, limpieza de la CPMP, y otros que le sean designados.",
    "Realizar la verificación del otorgamiento de garantías, así como monitorear la ejecución de las penalidades vinculadas al incumplimiento de cláusulas, términos de referencia y bases de licitación, de encontrar alguna observación deberá comunicar y elaborar el informe correspondiente.",
    "Revisar el expediente para otorgar la conformidad de los servicios y obras realizados por los proveedores de servicio de mantenimiento, seguridad, limpieza y otros que le designen.",
    "Brindar soporte legal en la elaboración de las especificaciones técnicas y/o términos de referencias de los bienes y servicios solicitados.",
    "Coordinar las acciones de saneamiento físico-legal de los inmuebles con el Departamento de Gestión Inmobiliaria cuando amerite.",
    "Monitorear el cumplimiento de la normativa de INDECI y proponer mejoras en cumplimiento de la normativa vigente.",
    "Cumplir con la normativa interna y externa aplicable a su ámbito de acción.",
    "Participar de manera activa y permanente en las acciones dispuestas en la gestión integral de riesgos, asociadas a las actividades que realiza.",
    "Participar en la implementación y actualización del sistema de control interno.",
    "Desempeñar las demás funciones inherentes a su cargo que le sean asignadas por el Jefe de Departamento de Servicios Inmobiliarios."
   ],
   "requisitos": [
    "Título profesional en Derecho, con colegiatura.",
    "Experiencia general de cuatro (4) años y experiencia específica de dos (2) años acumulable como abogado, analista legal o puesto similar.",
    "De preferencia, capacitación especializada en contratos de servicios inmobiliarios, derecho registral inmobiliario, derecho corporativo y contrataciones con el estado, la misma que es acreditada mediante cursos que totalicen una duración no menor a treinta (30) horas lectivas.",
    "FUNCIÓN BÁSICA"
   ],
   "beneficios": [
    "Remuneración: S/. 3,700.00",
    "Horario de Trabajo regular: 9:00 a.m. a 5:15 p.m. de lunes a viernes (presencial).",
    "Modalidad de contratación: Plazo fijo - D.L. 728 (Planilla, beneficios según ley).",
    "Capacitación acorde a la necesidad del puesto .",
    "Los datos personales que el postulante facilite a través de su hoja de vida, podrán ser recopilados, tratados, e incorporados a un Banco de Datos Personales de la CPMP, solo para dichos fines específicos, y serán conservados por un periodo máximo de dos años.",
    "La Caja de Pensiones Militar Policial - CPMP declara que respeta los principios de legalidad, consentimiento, finalidad, proporcionalidad, calidad, disposición de recurso, nivel de protección adecuado, conforme a las disposiciones de la Ley de Protección de Datos Personales vigente en Perú\".",
    "\" En la Caja de Pensiones Militar Policial creemos firmemente en la igualdad de oportunidades por lo cual promovemos la diversidad de género, origen étnico, nacionalidad, la inclusión de personas con discapacidad y/o habilidades diferentes (de acuerdo con lo establecido en la Ley N° 29973).\""
   ],
   "url": "https://www.bumeran.com.pe/empleos/abogado-analista-de-servicios-inmobiliarios-caja-de-pensiones-militar-policial-1118412513.html"
  },
  {
   "id": 352,
   "puesto": "Abogado Previsonal",
   "empresa": "Caja de Pensiones Militar Policial",
   "cat": "Legal",
   "min": 4300,
   "max": 4300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 38,
   "vence": "2026-11-17",
   "restan": 52,
   "score": 89,
   "resumen": "Analizar, evaluar y verificar la validez de la documentación de sustento para el otorgamiento de pensiones y compensaciones remitida por las FF.AA. y PNP, asegurando el cumplimiento de la normativa previsional vigente.",
   "funciones": [
    "Analizar, evaluar y verificar la validez de la documentación de sustento para el otorgamiento de pensiones y compensaciones remitida por las FF.AA. y PNP, asegurando el cumplimiento de la normativa previsional vigente.",
    "Calificar el derecho establecido en las resoluciones que otorgan pensiones de sobrevivientes, invalidez, incapacidad y por mandato judicial, previa revisión y verificación de la correcta aplicación de las normas legales vigentes.",
    "Elaborar los recursos de impugnación y observación de resoluciones que otorgan beneficios previsionales, para ser presentados en la vía administrativa, e informar sobre la situación del trámite de los mismos.",
    "Validar, en primera instancia, la calificación de procedencia y la forma de ejecución de los mandatos judiciales que gravan beneficios previsionales, así como preparar la información requerida por los juzgados.",
    "Visar los escritos con los que se envían los certificados de depósitos judiciales a los juzgados.",
    "Visar poderes para el retiro de cheques de pensiones, compensaciones y descuentos judiciales.",
    "Elaborar los decretos de las acciones a seguir referentes a los procesos judiciales comunicados por la Gerencia Legal.",
    "Elaborar informes técnico-legales relacionados al pago de beneficios previsionales.",
    "Asesorar al personal de la Gerencia de Pensiones, en aspectos legales de los regímenes previsionales administrados.",
    "Monitorear la atención de requerimientos legales en temas previsionales y tratamiento de impugnaciones administrativas.",
    "Emitir dictamen legal sobre los beneficios previsionales del Decreto Legislativo Nº 1133.",
    "Evaluar y emitir las resoluciones administrativas de otorgamiento de beneficios previsionales del Decreto Legislativo Nº 1133."
   ],
   "requisitos": [
    "Título profesional en derecho , con colegiatura.",
    "De preferencia, capacitación especializada en derecho administrativo, derecho civil, derecho procesal civil, derecho previsional o gestión pública, la misma que es acreditada mediante cursos que totalizan una duración no menor a treinta (30) horas lectivas.",
    "Experiencia general de cuatro (4) años, experiencia específica de dos (2) años como abogado, analista legal o puesto similar, especialista en procesos previsionales.",
    "FUNCIÓN BÁSICA",
    "Monitorear las actividades referentes a la calificación de beneficios previsionales y mandatos judiciales relacionados a los mismos, elaboración de recursos de impugnación, en el marco de la normativa vigente, con la finalidad de contribuir al logro de las metas planteadas para la entidad."
   ],
   "beneficios": [
    "Remuneración: S/. 4,300",
    "Horario de Trabajo regular: 9:00 a.m. a 5:15 p.m. de lunes a viernes (presencial).",
    "Capacitación acorde a la necesidad del puesto.",
    "Los datos personales que el postulante facilite a través de su hoja de vida, podrán ser recopilados, tratados, e incorporados a un Banco de Datos Personales de la CPMP, solo para dichos fines específicos, y serán conservados por un periodo máximo de dos años.",
    "La Caja de Pensiones Militar Policial - CPMP declara que respeta los principios de legalidad, consentimiento, finalidad, proporcionalidad, calidad, disposición de recurso, nivel de protección adecuado, conforme a las disposiciones de la Ley de Protección de Datos Personales vigente en Perú\"",
    "\" En la Caja de Pensiones Militar Policial creemos firmemente en la igualdad de oportunidades por lo cual promovemos la diversidad de género, origen étnico, nacionalidad, la inclusión de personas con discapacidad y/o habilidades diferentes (de acuerdo con lo establecido en la Ley N° 29973).\""
   ],
   "url": "https://www.bumeran.com.pe/empleos/abogado-previsonal-caja-de-pensiones-militar-policial-1118353438.html"
  },
  {
   "id": 353,
   "puesto": "Asistente de Vestuario Ripley Chimbote",
   "empresa": "GRUPO RIPLEY",
   "cat": "Ventas",
   "min": 1253,
   "max": 1253,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Chimbote",
   "fuente": "Bumeran",
   "dias": 38,
   "vence": "2026-12-17",
   "restan": 82,
   "score": 86,
   "resumen": "¿Te apasiona la moda, las ventas y el trato con las personas?",
   "funciones": [
    "Abordar proactivamente a clientes y asesorarlos durante su experiencia en tienda logrando las ventas de productos textiles (marcas de ropa propias o externas de Ripley)",
    "Recibir, asesorar y acompañar al cliente durante su proceso de compra.",
    "Identificar sus necesidades y ofrecer productos adecuados según estilo, tallas y preferencias.",
    "Promover y concretar ventas cumpliendo metas individuales y/o grupales.",
    "Velar por la exhibición atractiva."
   ],
   "requisitos": [
    "Experiencia mínima 4 meses en ventas presenciales, atención al cliente, retail de vestuarios o similares.",
    "Tener compromiso, dinamismo, agilidad, comunicación efectiva, persuasión y servicio al cliente.",
    "Habilidades: Orientación al cliente y al cumplimiento de metas.",
    "Disponibilidad para trabajar en horarios rotativos."
   ],
   "beneficios": [
    "Full Time: Horario: 46 horas semanales",
    "Disponibilidad completa para rotar entre 11 am a 10:00pm",
    "Días de trabajo rotativos: 5 veces por semana.",
    "Descansos rotativos entre lunes a domingo ( 2 días por semana)",
    "Salario Full Time: S/. 1253 soles.",
    "Comisiones hasta 1200 soles.",
    "Par Time: 18 horas semanales",
    "Tener disponibilidad de miércoles a domingo a partir de las 3pm",
    "Sueldo fijo 450 soles",
    "Comisión hasta 520 soles.",
    "Planilla directa de Ripley con todos los beneficios de ley.",
    "Beneficios corporativos en nuestras tiendas, instituciones educativas, etc."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-vestuario-ripley-chimbote-grupo-ripley-1118159442.html"
  },
  {
   "id": 354,
   "puesto": "Jefe de Proyectos Inmobiliarios",
   "empresa": "GRAFTON LATAM",
   "cat": "Ingeniería",
   "min": 6000,
   "max": 8000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 39,
   "vence": "2026-11-16",
   "restan": 51,
   "score": 98,
   "resumen": "Por encargo de nuestro cliente, empresa peruana especializada en el desarrollo y ejecución de proyectos inmobiliarios y edificaciones a gran escala , nos encontramos en búsqueda de un(a):",
   "funciones": [
    "Analizar la viabilidad de nuevos proyectos inmobiliarios , realizando análisis de terrenos, desarrollo de cabidas y planteamiento del producto.",
    "Realizar análisis de mercado inmobiliario como parte de la evaluación de nuevas oportunidades y proyectos.",
    "Gestionar y realizar seguimiento al ciclo de los proyectos inmobiliarios , desde las etapas iniciales de análisis y desarrollo hasta su ejecución, entrega y postventa.",
    "Desarrollar y realizar seguimiento a anteproyectos y expedientes técnicos , coordinando con los especialistas involucrados.",
    "Participar y realizar seguimiento a los procesos de licitación, construcción y cierre de los proyectos inmobiliarios.",
    "Liderar la negociación, coordinación y gestión de stakeholders internos y externos , incluyendo socios, arquitectos, especialistas, constructoras, proveedores y demás actores vinculados a los proyectos.",
    "Trabajar de manera coordinada con Gerencia y Finanzas , brindando información técnica y alternativas que contribuyan a la evaluación y toma de decisiones de los proyectos.",
    "Evaluar alternativas arquitectónicas considerando criterios de diseño, costos, presupuesto, producto, mercado y cliente objetivo .",
    "Liderar, gestionar, organizar y planificar las actividades del equipo de proyectos a cargo , contando con autonomía para la gestión de sus recursos y actividades.",
    "Coordinar las etapas de entrega, postventa, garantías, implementación de áreas comunes y cierre de los proyectos inmobiliarios.",
    "Desarrollar y liderar el próximo proyecto hotelero de cadena internacional en Cusco , coordinando con stakeholders nacionales e internacionales.",
    "Proponer soluciones, alternativas y recomendaciones de alto impacto para el negocio , aplicando pensamiento crítico, criterio técnico y capacidad de análisis."
   ],
   "requisitos": [
    "Profesional titulado(a) en Arquitectura .",
    "Experiencia de 3 a 5 años en proyectos inmobiliarios .",
    "Experiencia en análisis de terrenos y desarrollo de cabidas inmobiliarias .",
    "Dominio del Reglamento Nacional de Edificaciones (RNE) y lectura de Certificados de Parámetros .",
    "Experiencia participando y coordinando las distintas etapas del ciclo de un proyecto inmobiliario, desde su evaluación y desarrollo hasta la construcción, cierre y entrega.",
    "Experiencia en liderazgo y organización de equipos multidisciplinarios .",
    "Deseable contar con sensibilidad financiera y comercial aplicada a los planteamientos arquitectónicos.",
    "Manejo avanzado de AutoCAD y SketchUp .",
    "Manejo intermedio de Revit y conocimientos básicos de Excel .",
    "Disponibilidad para realizar viajes de trabajo a Cusco (aproximadamente 10 días al mes) y visitas de obra eventualmente los sábados."
   ],
   "beneficios": [
    "Banda salarial: S/ 6,000 a S/ 8,000 brutos mensuales.",
    "Ingreso a planilla bajo régimen general .",
    "EPS cubierta al 100% por la empresa para el colaborador y sus dependientes, luego del periodo de prueba.",
    "Trabajo presencial en San Isidro, Lima .",
    "Participación y liderazgo en proyectos inmobiliarios de alta relevancia, incluyendo el desarrollo de un hotel de cadena internacional en Cusco ."
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-de-proyectos-inmobiliarios-grafton-latam-1118407578.html"
  },
  {
   "id": 355,
   "puesto": "Supervisor HSEC",
   "empresa": "GRAFTON LATAM",
   "cat": "Ingeniería",
   "min": 7300,
   "max": 7300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 39,
   "vence": "2026-11-16",
   "restan": 51,
   "score": 97,
   "resumen": "Por encargo de nuestro cliente, importante operación minera ubicada en Áncash se encuentra en búsqueda de un(a) profesional para asumir la posición de:",
   "funciones": [
    "Liderar la gestión HSEC de las actividades de operación y mantenimiento de equipos móviles y maquinaria pesada dentro de la unidad minera.",
    "Supervisar permanentemente las condiciones de seguridad en campo, identificando peligros, evaluando riesgos y estableciendo medidas preventivas y correctivas.",
    "Gestionar, revisar y validar herramientas de gestión de seguridad como IPERC, ATS, PETS y permisos de trabajo.",
    "Ejecutar inspecciones y auditorías de seguridad, asegurando el levantamiento y seguimiento oportuno de los hallazgos identificados.",
    "Liderar la investigación de incidentes y accidentes, determinando causas y gestionando acciones correctivas y preventivas.",
    "Elaborar, analizar y realizar seguimiento a indicadores de desempeño HSEC.",
    "Asesorar y brindar soporte técnico en seguridad al equipo operativo y de mantenimiento.",
    "Liderar capacitaciones, charlas y acciones de sensibilización orientadas a fortalecer una cultura preventiva.",
    "Garantizar el cumplimiento de los procedimientos, estándares y lineamientos de seguridad establecidos por la operación minera.",
    "Mantener una comunicación efectiva con la gerencia y el cliente, reportando resultados, riesgos y oportunidades de mejora en materia HSEC."
   ],
   "requisitos": [
    "Profesional titulado y colegiado de Ingeniería de Seguridad, Ingeniería Industrial, Ingeniería Ambiental, Ingeniería de Minas o carreras afines.",
    "Experiencia mínima de 4 a 5 años desempeñándose como Supervisor HSEC, Supervisor SSOMA, Supervisor de Seguridad o posiciones similares en operaciones mineras.",
    "Experiencia previa acreditada en la unidad minera ANTAMINA.",
    "Experiencia supervisando actividades vinculadas con mantenimiento y/o operación de equipos móviles y maquinaria pesada .",
    "Sólido dominio y experiencia práctica en IPERC, ATS, PETS, permisos de trabajo, investigación de incidentes y auditorías de seguridad.",
    "Experiencia trabajando bajo estándares y procedimientos exigentes de seguridad minera.",
    "Capacidad de gestión, liderazgo, orientación al cliente y habilidad para brindar soporte a equipos multidisciplinarios.",
    "Disponibilidad para trabajar bajo régimen 10x10 en Áncash .",
    "Experiencia con equipos de izaje y grúas será valorada, pero no es excluyente.",
    "Licencia de conducir deseable."
   ],
   "beneficios": [
    "Remuneración aproximada de S/ 7,300 mensuales .",
    "Tarjeta Sodexo de S/ 600 mensuales .",
    "Ingreso a planilla completa desde el primer día , con todos los beneficios de ley.",
    "EPS Rímac – Plan Base cubierta al 100% por la empresa",
    "Revisión salarial semestral de acuerdo con la inflación.",
    "Incremento salarial anual sujeto a evaluación de desempeño y mérito.",
    "Régimen de trabajo 10x10 .",
    "Capacitación previa para la incorporación al proyecto.",
    "Participación en un proyecto minero de largo plazo ."
   ],
   "url": "https://www.bumeran.com.pe/empleos/supervisor-hsec-ssoma-proyecto-minero-en-ancash-grafton-latam-1118407582.html"
  },
  {
   "id": 356,
   "puesto": "Ejecutivo Senior de Ventas B2B",
   "empresa": "On Negocios",
   "cat": "Ventas",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 39,
   "vence": "2026-11-16",
   "restan": 51,
   "score": 91,
   "resumen": "Ejecutivo de Ventas Senior B2B – ON NEGOCIOS",
   "funciones": [
    "Prospectar y captar nuevos clientes del segmento microempresa.",
    "Gestionar y desarrollar cartera de clientes.",
    "Presentar reportes a su supervisor.",
    "Cumplir con los objetivos comerciales establecidos.",
    "Elaborar propuestas comerciales y realizar seguimiento de oportunidades.",
    "Disponibilidad para trabajar en San Isidro - Javier Prado"
   ],
   "requisitos": [
    "Experiencia en ventas B2B (indispensable).",
    "Experiencia mínima de 1 año certificada en ventas corporativas.",
    "Habilidades de negociación, comunicación efectiva y cierre de ventas.",
    "Orientación al cumplimiento de objetivos comerciales.",
    "Capacidad de organización, proactividad y enfoque en el cliente.",
    "Deseable experiencia en telecomunicaciones, banca, inmobiliarias o servicios."
   ],
   "beneficios": [
    "💰 Sueldo base: S/ 2,000",
    "🚗 Movilidad: S/ 500",
    "Horario Lunes a Viernes (9am - 6pm)",
    "Comisiones ilimitadas",
    "Ingreso a planilla completa desde el primer día",
    "EPS y seguro oncológico",
    "Descuentos corporativos",
    "Línea de carrera",
    "Buen clima laboral",
    "📩 Postula con nosotros enviando tu CV al número 936 982 424, indicando el puesto: EJECUTIVO DE VENTAS B2B."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-senior-de-ventas-b2b--on-negocios-on-negocios-1118407383.html"
  },
  {
   "id": 357,
   "puesto": "Jefe Zonal",
   "empresa": "Confidencial",
   "cat": "Otros",
   "min": 3500,
   "max": 3500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 42,
   "vence": "2026-11-13",
   "restan": 48,
   "score": 95,
   "resumen": "Importante empresa del rubro de bebidas y alimentos se encuentra en la búsqueda de un(a) Gerente Regional o Jefe Zonal para liderar la operación de un grupo de tiendas.",
   "funciones": [
    "Asegurar el correcto funcionamiento y desempeño de las tiendas asignadas.",
    "Realizar visitas periódicas a los puntos de venta, identificando oportunidades y dando seguimiento a planes de acción.",
    "Liderar, acompañar y desarrollar a los responsables de tienda y sus equipos.",
    "Monitorear indicadores comerciales y operativos: ventas, productividad, costos, calidad y experiencia del cliente.",
    "Asegurar el cumplimiento de procesos, estándares y protocolos establecidos.",
    "Analizar resultados y dar reporte a la Gerencia."
   ],
   "requisitos": [
    "Experiencia mínima de 3 años liderando equipos en tienda, como Administrador, Jefe o Gerente de Tienda en rubros afines.",
    "Experiencia en cafeterías, alimentos y bebidas, retail o negocios de atención al cliente.",
    "Se valorará experiencia en cadenas reconocidas como Mcdonalds, Cinnabon, Juan Valdez, Starbucks, Dunkin', liderando equipos de tienda.",
    "Disponibilidad para realizar trabajo de campo.",
    "Disponibilidad para laborar de manera presencial, 6 días a la semana."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Remuneración de S/3,500 + bono por cumplimiento de objetivos de hasta S/1,500",
    "Bono de Movilidad.",
    "Vales de consumo.",
    "Oportunidad de crecimiento."
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-zonal-gerente-regional-de-tiendas-1118403771.html"
  },
  {
   "id": 358,
   "puesto": "Ejecutivo de Admisión Ambulatoria",
   "empresa": "Clínica Providencia",
   "cat": "Recursos Humanos",
   "min": 1150,
   "max": 1150,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 42,
   "vence": "2026-11-13",
   "restan": 48,
   "score": 94,
   "resumen": "Clínica Providencia se encuentra en búsqueda de los mejores talentos para la posición de Ejecutivo de Admisión Ambulatoria - Full Time.",
   "funciones": [
    "Atender oportunamente en el servicio ambulatorio, brindar satisfacción en la información otorgada, acorde con los protocolos y procesos de atención definidos por la Clínica.",
    "Asegurar la atención del paciente de forma inmediata.",
    "Apertura de Historias clínicas.",
    "Desempeñar correctamente las funciones de caja desde la apertura, durante los registros y cuadre final.",
    "Realizar el cobro al paciente por la atención médica a recibir, de acuerdo a la cobertura de su póliza.",
    "Otras funciones inherentes al puesto."
   ],
   "requisitos": [
    "Mínimo 6 meses de experiencia en posiciones similares.",
    "Manejo de sistema SITEDS y cobertura de seguros (SCTR - SOATS, otros).",
    "Experiencia en caja-admisión.",
    "Egresado de la carrera técnica en administración, banca, secretariado o afines.(Deseable)"
   ],
   "beneficios": [
    "Se ofrece sueldo s/. 1150+ riesgo de caja s/. 100",
    "Capacitaciones pagadas",
    "Ingreso a planilla completa desde el primer día de trabajo.",
    "Acceso a Descuentos corporativos.",
    "Descuento en atenciones en clínica para ti y familiares directos."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-de-admision-ambulatoria-full-time-clinica-providencia-1118063299.html"
  },
  {
   "id": 359,
   "puesto": "Vendedora Miraflores",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 42,
   "vence": "2026-11-13",
   "restan": 48,
   "score": 92,
   "resumen": "Asesoramiento a clientes en búsqueda de los diversos productos.",
   "funciones": [
    "Asesoramiento a clientes en búsqueda de los diversos productos.",
    "Alcanzar los objetivos de ventas establecidos por la empresa.",
    "Estar actualizado en los nuevos ingresos de productos",
    "Apoyar en el orden de los productos y visual de la tienda."
   ],
   "requisitos": [
    "Solo mujeres de nacionalidad peruana entre 20 y 48 años.",
    "Tener experiencia mínima de 6 meses en ventas de marcas retail ubicadas en centros comerciales o boutiques puerta a calle o como promotoría en departamentales.",
    "Excelente trato y orientación al cliente.",
    "No hace falta que sepa hacer caja ya que se les capacitará.",
    "Disponibilidad para laborar 48 horas semanales, 6 días a la semana por 8 horas diarias (los turnos pueden ser de 9 am a 6 pm o de 11 am a 8 pm). La boutique opera de lunes a sábado de 9 am a 8 pm y domingos de 11 am a 6 pm."
   ],
   "beneficios": [
    "Grato ambiente laboral y posibilidad de hacer línea de carrera.",
    "Beneficios laborales de planilla pyme desde el 1er día de trabajo (50% de gratificación en Julio y Diciembre + 50% de CTS en Mayo y Noviembre + ESSALUD + 15 dias de vacaciones al año) . + asignación familiar.",
    "Sueldo básico (S/.1130) + comisiones desde la 1ra venta y sin tope.",
    "Bonos mensuales por sobrecumplimiento de meta entregados en tarjeta PROVIS.",
    "Descuentos en productos de la marca hasta un 30%."
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedora-miraflores-adn-retail-s.a.c.-1118403955.html"
  },
  {
   "id": 360,
   "puesto": "Analista de Desarrollo Organizacional",
   "empresa": "Well Consulting Services",
   "cat": "Recursos Humanos",
   "min": 4200,
   "max": 4200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 43,
   "vence": "2026-11-12",
   "restan": 47,
   "score": 92,
   "resumen": "Gestionar y ejecutar programas de capacitación y desarrollo.",
   "funciones": [
    "Gestionar y ejecutar programas de capacitación y desarrollo.",
    "Participar en la gestión de clima y cultura organizacional.",
    "Ejecutar acciones de comunicación interna.",
    "Elaborar y realizar seguimiento a indicadores de gestión relacionados con Desarrollo Organizacional.",
    "Generar reportes e información para el análisis y toma de decisiones.",
    "Brindar soporte en las diferentes iniciativas y proyectos del área.",
    "Realizar visitas de campo a los almacenes ubicados en Ancón, Pasamayo, Santa Rosa y Callao."
   ],
   "requisitos": [
    "Bachiller en Ingeniería Industrial, Recursos Humanos, Ingeniería de Sistemas, Administración o carreras afines.",
    "Experiencia mínima de 2 años en posiciones similares .",
    "Conocimientos en indicadores de clima, capacitación y/o comunicación interna.",
    "Dominio de Excel a nivel avanzado .",
    "Disponibilidad para realizar visitas de campo a los diferentes almacenes.",
    "Capacidad de análisis, organización, comunicación y orientación a resultados."
   ],
   "beneficios": [
    "Lugar de trabajo: San Isidro.",
    "Modalidad: Híbrida.",
    "Horario: Lunes a viernes de 9:00 a. m. a 7:00 p. m.",
    "Remuneración: Hasta S/ 4,200, de acuerdo con experiencia y perfil.",
    "Descuentos corporativos.",
    "Línea de carrera.",
    "Si cumples con el perfil y estás interesado(a) en asumir nuevos retos en el área de Desarrollo Organizacional, te invitamos a postular."
   ],
   "url": "https://www.bumeran.com.pe/empleos/analista-de-desarrollo-organizacional-san-isidro-well-consulting-services-1118403549.html"
  },
  {
   "id": 361,
   "puesto": "Analista de Selección y Desarrollo",
   "empresa": "Well Consulting Services",
   "cat": "Recursos Humanos",
   "min": 4200,
   "max": 4200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 43,
   "vence": "2026-11-12",
   "restan": 47,
   "score": 92,
   "resumen": "Gestionar integralmente los procesos de atracción y selección de talento.",
   "funciones": [
    "Gestionar integralmente los procesos de atracción y selección de talento.",
    "Levantar y analizar los requerimientos de personal junto con los clientes internos.",
    "Ejecutar las diferentes etapas del proceso de selección, desde el filtro curricular hasta la presentación y seguimiento de candidatos.",
    "Realizar seguimiento a los indicadores de selección y elaborar reportes de gestión.",
    "Asegurar el cumplimiento de los tiempos establecidos para la cobertura de las posiciones.",
    "Coordinar entrevistas, evaluaciones y las actividades correspondientes al proceso de incorporación.",
    "Brindar seguimiento a los candidatos durante las diferentes etapas del proceso, asegurando una adecuada experiencia.",
    "Realizar visitas de campo a los almacenes ubicados en Ancón, Pasamayo, Santa Rosa y Callao."
   ],
   "requisitos": [
    "Bachiller en Psicología, Administración, Recursos Humanos, Ingeniería Industrial o carreras afines.",
    "Experiencia mínima de 2 años en posiciones similares .",
    "Conocimiento en indicadores de selección .",
    "Dominio de Excel a nivel intermedio .",
    "Disponibilidad para realizar visitas de campo a los diferentes almacenes.",
    "Capacidad de organización, análisis, comunicación y orientación a resultados."
   ],
   "beneficios": [
    "Lugar de trabajo: San Isidro.",
    "Modalidad: Híbrida.",
    "Horario: Lunes a viernes de 9:00 a. m. a 7:00 p. m.",
    "Remuneración: Hasta S/ 4,200 , de acuerdo con experiencia y perfil.",
    "Descuentos corporativos.",
    "Línea de carrera.",
    "Si cumples con el perfil y estás interesado(a) en asumir nuevos retos en el área de Atracción y Desarrollo, te invitamos a postular."
   ],
   "url": "https://www.bumeran.com.pe/empleos/analista-de-seleccion-y-desarrollo-san-isidro-well-consulting-services-1118403556.html"
  },
  {
   "id": 362,
   "puesto": "Coordinador SIG",
   "empresa": "MOLINOS ASOCIADOS SAC",
   "cat": "Contabilidad",
   "min": 3000,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 43,
   "vence": "2026-11-12",
   "restan": 47,
   "score": 89,
   "resumen": "We are 100% Peruvian Brand specialized in Plant based ingredients from Tara Pod derivatives.",
   "funciones": [
    "Diseñar y estandarizar procesos : mapas de procesos, diagramas de flujo y matrices de control junto a los líderes de cada área.",
    "Liderar el programa de auditorías internas de todos los estándares del Sistema Integrado de Gestión.",
    "Gestionar el control documental : asegurar que todo el personal trabaje con procedimientos vigentes y aprobados.",
    "Impulsar la mejora continua : supervisar el cierre oportuno de no conformidades y acciones correctivas.",
    "Diseñar el plan anual de capacitación en normativas internacionales de seguridad, calidad e inocuidad alimentaria.",
    "Evaluar y homologar proveedores clave , cuidando la calidad de la materia prima (tara) desde el origen.",
    "Monitorear KPIs y OKRs del SIG, brindando información confiable para la toma de decisiones gerenciales.",
    "Supervisar inspecciones de seguridad e higiene en planta, garantizando ambientes de trabajo seguros.",
    "Vigilar el cumplimiento legal aplicable a la actividad industrial y de exportación.",
    "Atender los requisitos de FSSC 22000, BASC, INDECI y DIGESA , entre otros aplicables al puesto."
   ],
   "requisitos": [
    "Profesional en Ingeniería Industrial, Ingeniería en Industrias Alimentarias, Ingeniería Ambiental o carreras afines.",
    "Experiencia previa liderando o coordinando Sistemas Integrados de Gestión (Calidad, Seguridad, Inocuidad).",
    "Conocimiento sólido y comprobado en normas como BASC, FSSC 22000, ISO 9001, Halal y Kosher (excluyente).",
    "Experiencia en auditorías internas y externas, y en la gestión de certificaciones ante organismos acreditadores.",
    "Deseable experiencia en el sector agroindustrial o de exportación.",
    "Habilidades de liderazgo, comunicación efectiva y capacidad de trabajo transversal con distintas áreas.",
    "Orientación a resultados, mejora continua y alto sentido de responsabilidad."
   ],
   "beneficios": [
    "Formar parte de una empresa con proyección internacional y un producto 100% peruano que conquista mercados globales.",
    "Participar activamente en la certificación ISO 9001 y en el fortalecimiento de un sistema de gestión de talla mundial.",
    "Un rol estratégico con visibilidad directa ante la Gerencia .",
    "Ambiente de trabajo colaborativo, con retos constantes y oportunidades de crecimiento profesional.",
    "Sueldo: S/. 3,000.00",
    "Planilla completa y beneficios de acuerdo a ley."
   ],
   "url": "https://www.bumeran.com.pe/empleos/coordinador-sig-unete-a-la-empresa-que-lleva-la-tara-del-peru-al-mundo-molinos-asociados-sac-1118403478.html"
  },
  {
   "id": 363,
   "puesto": "Operario de Almacén",
   "empresa": "MOLINOS ASOCIADOS SAC",
   "cat": "Logística",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 43,
   "vence": "2026-11-12",
   "restan": 47,
   "score": 89,
   "resumen": "We are 100% Peruvian Brand specialized in Plant based ingredients from Tara Pod derivatives.",
   "funciones": [
    "Paletizar sacos de 25 kg de harina",
    "Enzunchar, poner film y etiquetar los pallets",
    "Apoyo en corte de cartones, limpieza y orden del almacén"
   ],
   "requisitos": [
    "Lo que buscamos en ti",
    "Experiencia previa en trabajo físico o estiba (en cualquier rubro)",
    "Puntualidad y compromiso",
    "No se exige secundaria completa ni estudios técnicos",
    "Disponibilidad full time, presencial en planta VES."
   ],
   "beneficios": [
    "Planilla desde el día 1, con todos los beneficios de ley",
    "Sueldo fijo de S/ 1,200 — sin letra chica, sin sorpresas",
    "Bono de puntualidad: S/. 100.00",
    "Bono de asistencia: S/. 100.00",
    "Comida: GRATIS",
    "Otros bonos a partir de los 6 meses.",
    "Aquí sí se crece: varios de nuestros líderes de almacén empezaron como operarios.",
    "Te capacitamos: no necesitas experiencia previa en almacén.",
    "Buen ambiente de trabajo y equipo que se apoya mutuamente.",
    "Estabilidad laboral"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-almacen-estiba-y-paletizado-unete-a-la-planta-que-lleva-la-tara-del-peru-al-mundo-molinos-asociados-sac-1118403539.html"
  },
  {
   "id": 364,
   "puesto": "Sip: Asesor de Ventas",
   "empresa": "Sip",
   "cat": "Ventas",
   "min": 1100,
   "max": 1100,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 44,
   "vence": "2026-11-11",
   "restan": 46,
   "score": 92,
   "resumen": "Perfilar al cliente, despertar interés, brindar asesoría financiera, ofrecer y vender la combinación de productos establecidos por Sip, según la necesidad del cliente.",
   "funciones": [
    "Perfilar al cliente, despertar interés, brindar asesoría financiera, ofrecer y vender la combinación de productos establecidos por Sip, según la necesidad del cliente.",
    "Promover y enseñar el uso del MAA para la autoatención de los clientes, así como el App y la Web.",
    "Informar de manera clara y transparente las características del producto o servicio que adquirió el Cliente",
    "Asistir a las reuniones de acuerdo a la metodología de gestión de ventas, para realizar el seguimiento a mis indicadores, así como informarme sobre nuevos procesos, políticas, campañas, concursos, promociones y descuentos ofrecidos en el Retail y CAS.",
    "Asistir a las capacitaciones programadas a fin de mantenerme actualizado con los productos y/o nuevos procedimientos",
    "Mantener la imagen corporativa personal, cuidando mi apariencia personal y usando el uniforme de acuerdo a las normas y criterios establecidos por la empresa para la atención al público.",
    "Realizar los procedimientos y controles que Sip haya definido.",
    "Realizar otras funciones afines o complementarias asignadas por mi jefe inmediato superior."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en curso en la especialidad de ventas, negociación, comercial o administración.",
    "Experiencia en ventas de productos intangibles (6 meses como mínimo).",
    "Experiencia en atención al cliente en el sector de Banca, Financiera y/o Seguros (6 meses como mínimo).",
    "Disponibilidad para laborar 8 horas diarias de lunes a domingo, con 01 día de descanso."
   ],
   "beneficios": [
    "Ingreso directo a planilla desde el primer día con todos los beneficios de ley.",
    "S/. 1100 + S/. 100 provis + Comisiones + bonos.",
    "Agradable clima laboral.",
    "Oportunidad de desarrollarse dentro de una empresa en expansión."
   ],
   "url": "https://www.bumeran.com.pe/empleos/sip-asesor-de-ventas-modulo-de-atencion-jesus-maria-salaverry-sip-1118402167.html"
  },
  {
   "id": 365,
   "puesto": "Asesor de Ventas",
   "empresa": "Sip",
   "cat": "Ventas",
   "min": 600,
   "max": 600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 44,
   "vence": "2026-11-11",
   "restan": 46,
   "score": 92,
   "resumen": "Perfilar al cliente, despertar interés, brindar asesoría financiera, ofrecer y vender la combinación de productos establecidos por Sip, según la necesidad del cliente.",
   "funciones": [
    "Perfilar al cliente, despertar interés, brindar asesoría financiera, ofrecer y vender la combinación de productos establecidos por Sip, según la necesidad del cliente.",
    "Promover y enseñar el uso del MAA para la autoatención de los clientes, así como el App y la Web.",
    "Informar de manera clara y transparente las características del producto o servicio que adquirió el Cliente",
    "Asistir a las reuniones de acuerdo a la metodología de gestión de ventas, para realizar el seguimiento a mis indicadores, así como informarme sobre nuevos procesos, políticas, campañas, concursos, promociones y descuentos ofrecidos en el Retail y CAS.",
    "Asistir a las capacitaciones programadas a fin de mantenerme actualizado con los productos y/o nuevos procedimientos",
    "Mantener la imagen corporativa personal, cuidando mi apariencia personal y usando el uniforme de acuerdo a las normas y criterios establecidos por la empresa para la atención al público.",
    "Realizar los procedimientos y controles que Sip haya definido.",
    "Realizar otras funciones afines o complementarias asignadas por mi jefe inmediato superior."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en curso en la especialidad de ventas, negociación, comercial o administración.",
    "Experiencia en ventas de productos intangibles (6 meses como mínimo).",
    "Experiencia en atención al cliente en el sector de Banca, Financiera y/o Seguros (6 meses como mínimo).",
    "Disponibilidad para laborar 4 horas diarias de lunes a domingo, con 01 día de descanso."
   ],
   "beneficios": [
    "Ingreso directo a planilla desde el primer día con todos los beneficios de ley.",
    "S/. 600 básico + Comisiones + bonos.",
    "Agradable clima laboral.",
    "Oportunidad de desarrollarse dentro de una empresa en expansión."
   ],
   "url": "https://www.bumeran.com.pe/empleos/sip-asesor-de-ventas-ventas-productos-financieros-plaza-vea-san-miguel-sip-1118402171.html"
  },
  {
   "id": 366,
   "puesto": "Asistente Administrativo(a) para grifo",
   "empresa": "GRUPO COPETROL",
   "cat": "Administración",
   "min": 1500,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 45,
   "vence": "2026-11-10",
   "restan": 45,
   "score": 92,
   "resumen": "Realizar el seguimiento de las ventas de las promociones.",
   "funciones": [
    "Realizar el seguimiento de las ventas de las promociones.",
    "Elaborar los reportes de ventas y liquidaciones de combustibles y Market.",
    "Analizar los reportes de ventas para identificar los desvíos sobre los objetivos a cumplir.",
    "Realizar el recorrido a en la estación para identificar alguna incidencia con los compresores de los diferentes productos de venta en playa.",
    "Verificar el cumplimiento del ciclo de servicio por parte de todo el personal a su cargo.",
    "Realizar el chek list diario de la imagen física de los diversos ambientes de la EESS.",
    "Realizar el check list diario de la imagen del personal: limpieza, trato al cliente, actitud, orden, uso correcto de uniforme, entre otros.",
    "Gestionar el abastecimiento de los productos de la playa y del Market.",
    "Asegurar la atención de la EESS de acuerdo a la programación horaria del personal.",
    "Permanecer en playa realizando la supervisión directa de atención al cliente por su personal a cargo."
   ],
   "requisitos": [
    "Experiencia de 2 años en gestiones administrativas y comerciales de restaurantes, EESS, tiendas por conveniencia, afines.",
    "Formación académica: Técnicos o universitarios",
    "Experiencia en liquidaciones de venta, caja e inventarios"
   ],
   "beneficios": [
    "Sueldo: Entre S/ 1500 a S/ 1800",
    "Planilla Régimen General",
    "Lugar de trabajo: Villa María del Triunfo",
    "Horario: Lunes a Viernes de 8:00 a.m. a 5:30 p.m. y Sábados de 8:00 a.m. a 1:30 p.m"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-administrativoa-para-grifo--villa-maria-del-triunfo-grupo-copetrol-1118391025.html"
  },
  {
   "id": 367,
   "puesto": "Vendedor de Campo Exp. Consumo Masivo",
   "empresa": "Confidencial",
   "cat": "Ventas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Cusco",
   "fuente": "Bumeran",
   "dias": 45,
   "vence": "2026-11-10",
   "restan": 45,
   "score": 90,
   "resumen": "Nos encontramos en la búsqueda de un Vendedor de Campo para Cusco:",
   "funciones": [
    "Realizar visitas a zonas industriales, obras, talleres y ferreterías para identificar y captar nuevos clientes potenciales.",
    "Presentar los productos de la empresa de manera efectiva, resaltando sus beneficios y características.",
    "Registrar las visitas y oportunidades comerciales, asegurando un seguimiento adecuado y detallado.",
    "Colaborar con el equipo comercial en la gestión y seguimiento de cotizaciones, asegurando atención oportuna a los prospectos.",
    "Realizar viajes a provincias cuando se presenten oportunidades comerciales que permitan la expansión de la cartera de clientes."
   ],
   "requisitos": [
    "Experiencia en ventas del rubro ferretero, iluminación o consumo masivo",
    "Residir en la zona de Cusco",
    "Alta responsabilidad, puntualidad y compromiso con las tareas asignadas.",
    "Disponibilidad para trabajo en campo al 100% (Cusco, Apurimac, Madre de Dios, Ayacucho)",
    "Manejo de Indicadores"
   ],
   "beneficios": [
    "Sueldo Básico: S/. 1500",
    "Bono de S/. 1000 (2 primeros meses)",
    "Viáticos cubiertos Comisiones variables según indicadores",
    "Planilla Completa Oportunidad de Crecimiento",
    "Horarios: Lunes a Viernes: 8:00 am - 6:00 pm Sábados: 8:00 am - 1:00 pm"
   ],
   "url": "https://www.bumeran.com.pe/empleos/vendedor-de-campo-exp.-consumo-masivo-1118400553.html"
  },
  {
   "id": 368,
   "puesto": "Operador de Máquina designado",
   "empresa": "ManpowerGroup Perú",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 45,
   "vence": "2026-11-10",
   "restan": 45,
   "score": 89,
   "resumen": "Por encargo de uno de nuestros clientes líderes en el mercado, nos encontramos en búsqueda del siguiente perfil:",
   "funciones": [
    ". Ofrecer soporte al Operador de Máquina designado en la preparación de las maquinarias, implementos y equipos",
    "Tomar conocimiento del plan de producción del día y la semana, así como de la distribución de persona por su supervisor.",
    "Ayudar en la preparación de las cargas de materia prima con anticipación"
   ],
   "requisitos": [
    "Experiencia como operario de producción",
    "Secundaria completa",
    "Disponibilidad para laborar en Huachipa",
    "Disponibilidad inmediata"
   ],
   "beneficios": [
    "Salario básico S/1130 + S/200 asignación de alimentos",
    "Planilla completa al 100%",
    "Bono adicional a partir del 5to mes.",
    "Bono de escolaridad",
    "Bono nocturno",
    "Movilidad de acercamiento ida y vuelta gratuito",
    "Pagos quince lares",
    "Trabajo de lunes a domingo de 6.30 am a 6.30 pm / 6.30 pm a 6.30 am Horario madrugada rotativo",
    "Régimen de 4x3 (4 días trabajados con 3 día de descanso)"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operarios-produccion-gloria-trupal-sede-agustino-y-huachipa-manpowergroup-peru-1118399052.html"
  },
  {
   "id": 369,
   "puesto": "Coordinador SAP Business One",
   "empresa": "BLUE PARTNER",
   "cat": "Tecnología",
   "min": 6000,
   "max": 8000,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 46,
   "vence": "2026-11-09",
   "restan": 44,
   "score": 100,
   "resumen": "Por encargo de nuestro cliente, importante empresa del sector de distribución de productos de salud, nos encontramos en búsqueda de:",
   "funciones": [
    "Llevar un control exhaustivo de los datos que se ingresan y registran en las diversas plataformas y sistemas de la empresa.",
    "Proporcionar asistencia técnica a los usuarios en la creación e ingreso de SKUs, precios, datos o cualquier otro dato que se necesite ingresar en los sistemas.",
    "Realizar el ingreso de los datos necesarios en los sistemas de la empresa.",
    "Desarrollar un programa de capacitación para las áreas usuarias en el manejo y creación de datos.",
    "Mantener la base de datos actualizada de acuerdo con las necesidades de las áreas usuarias, asegurándose de que los sistemas estén correctamente configurados para garantizar la operatividad y seguridad de los datos.",
    "Implementar medidas de seguridad para proteger los datos maestros en los diversos sistemas informáticos de la organización contra amenazas internas y externas.",
    "Ejecutar el plan de backups de la compañía, con la finalidad de proteger toda la información de los usuarios.",
    "Colaborar con otros departamentos para identificar las necesidades tecnológicas de la organización y proponer soluciones tecnológicas que mejoren la eficiencia operativa y la productividad.",
    "Realizar las diferentes actividades requeridas por la gerencia o responsable del área."
   ],
   "requisitos": [
    "Bachiller o Titulado en Ingeniería Industrial, Ingeniería de Sistemas, Administración o carreras afines.",
    "Experiencia mínima de 6 años desempeñando funciones similares.",
    "Manejo de SAP Business One. (Indispensable)",
    "Conocimiento de SQL. (Indispensable)",
    "Conocimiento de SQL Server y SAP HANA. (Indispensable)",
    "Manejo de Excel a nivel avanzado. (Indispensable)",
    "Experiencia en Visual K. (Deseable)",
    "Experiencia en Salesforce. (Deseable)",
    "Experiencia en sistemas CRM. (Deseable)",
    "Buena comunicación, orientación al detalle, organización, orientación al cliente y capacidad para la resolución de problemas.",
    "Residir en Lima para asistir a la sede ubicada en Huachipa bajo modalidad híbrida."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día con todos los beneficios de ley.",
    "Remuneración en el rango de: S/ 6,000 - S/ 8,000.",
    "Modalidad híbrida (Lunes y Viernes Presencial; Martes, Miércoles y Jueves REMOTO).",
    "Horario de Lunes a Viernes de 8:00 a. m. a 5:00 p. m.",
    "Disponibilidad para atender eventualidades los sábados, solo en caso de ser requerido.",
    "EPS cubierta al 80% para el titular.",
    "Bus de acercamiento (En ICPNA al frente de la U de Lima y en Puente Nuevo, tanto de ida como de vuelta).",
    "Día libre por cumpleaños.",
    "Descuentos corporativos en los productos de la empresa."
   ],
   "url": "https://www.bumeran.com.pe/empleos/coordinador-sap-business-one-datos-maestros-blue-partner-1118398871.html"
  },
  {
   "id": 370,
   "puesto": "Asistente de Almacén",
   "empresa": "ISTP ALTA COCINA D´GALLIA SAC",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 46,
   "vence": "2026-11-09",
   "restan": 44,
   "score": 97,
   "resumen": "Instituto Gastronómico peruano cuya misión es formar cocineros íntegros con visión empresarial, artistas comprometidos con el desarrollo del Perú y la difusión de su gastronomía dentro y fuera del país.",
   "funciones": [
    "Realizar el armado de las taras con los requerimientos solicitados para cada clase.",
    "Verificar que las taras contengan todos los insumos requeridos, tanto en calidad de producto como en cantidad.",
    "Realizar la preproducción de los insumos siguiendo las normas establecidas.",
    "Cumplir con el proceso de limpieza y desinfección del almacén y las buenas prácticas de manipulación de alimentos (BPM).",
    "Recepcionar y verificar la mercadería que ingresan los proveedores.",
    "Almacenar los insumos que ingresan al almacén teniendo en cuenta los estándares de calidad y almacenamiento.",
    "Realizar mensualmente el inventario de los productos que se encuentran en el almacén.",
    "Realizar el cruce de información en los cambios de turno (incidencias, pendientes).",
    "Realizar el recorrido de las cocinas y talleres para el recojo de insumos sobrantes.",
    "Cuidar con responsabilidad los insumos que se guardan en el almacén.",
    "Resguardar los insumos que se encuentran en el almacén.",
    "Verificar entradas y salidas de insumos."
   ],
   "requisitos": [
    "Educación: Nivel Superior – Estudios Técnicos. en carreras carreras afines a gastronomía.",
    "Conocimiento de BPM.",
    "Conocimientos de temperaturas de congelación, refrigeración y empaque al vacío de alimentos.",
    "Experiencia mínima de 06 meses en el área de almacén en alimentos.",
    "Experiencia mínima de 06 meses en el rubro gastronómico y /o en el sector de hotelería."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día (régimen general).",
    "Descuento en D'Gallia del 50% en carreras y cursos programas entre otros.",
    "Descuentos Corporativos con otras instituciones.",
    "Un ambiente dinámico y colaborativo que fomenta la creatividad y el éxito.",
    "Estabilidad laboral y oportunidad de crecer en una institución reconocida.",
    "S/. 1,130 básico + 270 movilidad",
    "Horas Extras reconocidas",
    "Horario Laboral",
    "Lunes a Viernes: 01:00 pm a 10:00 pm y sábados de: 07:00 am a 04:00 pm",
    "¡Forma parte de una comunidad que forma talento con pasión y sabor!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-almacen-rubro-alimentos-istp-alta-cocina-dgallia-sac-1118398850.html"
  },
  {
   "id": 371,
   "puesto": "Gestor de Producto",
   "empresa": "MONT GROUP SAC",
   "cat": "Ventas",
   "min": 3000,
   "max": 3500,
   "moneda": "PEN",
   "modalidad": "Híbrido",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 46,
   "vence": "2026-11-09",
   "restan": 44,
   "score": 92,
   "resumen": "El Gestor de Producto brindará soporte al Jefe de Producto en la gestión y desarrollo de las líneas a cargo. El rol combina funciones de coordinación operativa, análisis comercial y apoyo en marketing, con proyección a asumir mayor responsabilidad a mediano plazo.",
   "funciones": [
    "El Gestor de Producto brindará soporte al Jefe de Producto en la gestión y desarrollo de las líneas a cargo. El rol combina funciones de coordinación operativa, análisis comercial y apoyo en marketing, con proyección a asumir mayor responsabilidad a mediano plazo.",
    "Apoyar en la elaboración y seguimiento de planes de marketing para el lanzamiento de productos.",
    "Coordinar con el área de Marketing la producción de materiales promocionales y piezas de comunicación.",
    "Monitorear indicadores de ventas y preparar reportes periódicos de desempeño por línea de producto.",
    "Gestionar el seguimiento operativo de stock: niveles de inventario, alertas de reposición y coordinación de pedidos.",
    "Actualizar listas de precios y gestionar solicitudes de la fuerza de ventas.",
    "Coordinar con el equipo de Asesoría Técnica en consultas y seguimiento de casos de producto.",
    "Apoyar en el control de procesos de lanzamiento de nuevos productos y coordinación con proveedores.",
    "Participar en reuniones de seguimiento con equipos internos y externos.",
    "Apoyar en el análisis de información de mercado para la identificación de nuevas oportunidades."
   ],
   "requisitos": [
    "Egresado o titulado en Tecnología Médica, Ingeniería Biomédica, Biología, Química Farmacéutica, Administración de Empresas con orientación al sector salud, o carrera afín.",
    "Estudios complementarios en marketing, gestión comercial o gestión de productos son valorados.",
    "De 1 a 3 años de experiencia en empresas del sector salud, diagnóstico, farmacéutico o dispositivos médicos.",
    "Experiencia previa en áreas comerciales o de producto es un diferencial.",
    "Manejo de Microsoft Office a nivel intermedio (Excel requerido).",
    "Conocimiento de herramientas de visualización de datos como Power BI — deseable.",
    "Capacidad de redacción para elaborar informes, propuestas y presentaciones.",
    "Comprensión del ciclo de vida de productos en el sector salud.",
    "Orientación al trabajo en equipo y colaboración con equipos comerciales.",
    "Proactividad e iniciativa para proponer mejoras y soluciones.",
    "Organización, atención al detalle y capacidad de gestión de múltiples tareas.",
    "Comunicación efectiva, clara y directa."
   ],
   "beneficios": [
    "Ingreso a planilla (Régimen general) desde el primer día",
    "Beneficios sociales de acuerdo a ley (Gratificación, CTS, Utilidades, Vacaciones 30 días, Essalud, Póliza Vida Ley)",
    "Remuneración que oscila de S/. 3,000.00 a S/. 3500 (De acuerdo a la experiencia del postúlate)",
    "Oportunidad de formar parte de una división en crecimiento y expansión.",
    "Participación activa en el desarrollo de nuevas categorías y marcas.",
    "Ambiente dinámico y orientado a innovación.",
    "Línea de carrera y crecimiento profesional.",
    "HORARIO DE TRABAJO",
    "De lunes a viernes de 8:30 am a 5:30 pm y sábado de 8:30 am a 1:00 pm (Solo el último sábado de cada mes)",
    "Trabajo hibrido: Remoto (2 días a la semana) y Presencial (3 días a la semana)",
    "Si quieres formar parte de este gran equipo, ¡esperamos tu postulación! Envíanos tu CV indicando en el asunto “Postulación Gestor de Producto”."
   ],
   "url": "https://www.bumeran.com.pe/empleos/gestor-de-producto-mont-group-sac-1118398811.html"
  },
  {
   "id": 372,
   "puesto": "Operario de Producción",
   "empresa": "EUROFIRMS",
   "cat": "Ingeniería",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 46,
   "vence": "2026-11-09",
   "restan": 44,
   "score": 89,
   "resumen": "En Eurofirms Group, creemos en el liderazgo por valores y el crecimiento continuo, poniendo a las personas y su desarrollo profesional primero, potenciando un ambiente de trabajo positivo y colaborativo.",
   "funciones": [
    "Descarga, selección y lavado de materias primas.",
    "Limpieza de bolsas de productos terminados.",
    "Actividades en envasado, etiquetado y paletizado de productos terminados.",
    "Labores de acarreo y lavado de materiales y envases."
   ],
   "requisitos": [
    "Tener estudios de Secundaria Completa.",
    "Con o Sin Experiencia",
    "Disponibilidad para laborar en Lurín.",
    "Disponibilidad para trabajar en horarios rotativos de Lunes a Sábado (Mañana, tarde y noche):7AM-4PM/2PM-10:30PM/10PM-7AM"
   ],
   "beneficios": [
    "Planilla completa con todos los beneficios de ley.",
    "Salario básico de S/1,130.",
    "Bono por movilidad S/150",
    "Bono por movilidad S/100",
    "Bono de Producción S/300.",
    "Línea de carrera.",
    "Si cumples con los requisitos, estás interesado y crees que eres el match perfecto, te invitamos a postular ¡Te esperamos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-produccion-c-s-exp-lurin-alimentosmovilidad-gana-mas-de-s-1680-eurofirms-1118398791.html"
  },
  {
   "id": 373,
   "puesto": "Tecnico Operador Produccion",
   "empresa": "Confidencial",
   "cat": "Ingeniería",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 47,
   "vence": "2026-11-08",
   "restan": 43,
   "score": 95,
   "resumen": "Importante empresa del sector Industrial se encuentra en búsqueda de TECNICO OPERADOR MAQUINA INDUSTRIAL . Para la sede del Callo.",
   "funciones": [
    "Inspeccionar diariamente el estado de la maquina e.",
    "Ejecutar correctamente el funcionamiento de la Máquina.",
    "Asegurarse que los proveedores y personal propio en la zona utilicen los elementos de protección personal y chalecos.",
    "Coordinar y ejecutar la limpieza de la fosa de la faja mensualmente.",
    "Realizar actividades propias del mantenimiento menor (correctivo/preventivo) de los equipos destinados al área (Mecánicos Hidráulicos).",
    "Desempeñar otra actividad conexa y complementaria relacionada a las funciones descritas."
   ],
   "requisitos": [
    "Educación técnica de la carrera de Mecánica, Mecatrónica, otros.",
    "Experiencia de 02 años en empresas industriales o consumo masivo como operador de Maquina Industrial.",
    "Experiencia en elaboración de instructivos y procedimientos para máquinas industriales.",
    "Experiencia en mantenimientos preventivos y correctivos menores.",
    "Especializado en procesos de mantenimiento Mecánico, Eléctrico, Hidráulicos.",
    "Habilidad en uso de equipos, herramientas y maquinas industriales.",
    "Conocimiento de herramientas Office.",
    "Experiencia en el manejo de Máquinas industriales, montacargas y minicargadores frontales.",
    "Licencia de conducir A1 (Deseable)",
    "Vivir en Distritos Aledaños (Callao, Los Olivos, Ventanilla, Comas, San Martín de Porres, etc.)"
   ],
   "beneficios": [
    "Salario básico de S/. 2000 + 314 (variable) - Planilla completa desde el primer día con todos los beneficios de ley (gratificación, cts, vacaciones, etc)",
    "Movilidad de acercamiento",
    "Alimentación cubierta al 100%",
    "Aguinaldo en Julio y Diciembre.",
    "Bolsones con Productos de la empresa",
    "Otros beneficios corporativas",
    "¡TE ESPERAMOS!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/tecnico-operador-produccion-licencia-a1-callao-planilla-directa-1118395606.html"
  },
  {
   "id": 374,
   "puesto": "¡Urgente Asistente de Servicio al Cliente",
   "empresa": "Consulting & Management",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 49,
   "vence": "2026-10-07",
   "restan": 11,
   "score": 92,
   "resumen": "¡En búsqueda de un Asistente de servicio al cliente!",
   "funciones": [
    "Recepcionar y registrar los productos que ingresen de los productos que presenten garantía con previa cita.",
    "Atender a los clientes que dejan sus productos por garantía.",
    "Brindar información a los clientes del proceso de reparación de los equipos.",
    "Otras funciones que se le deleguen."
   ],
   "requisitos": [
    "Estudios de Carrera Técnica de Sistemas, Computación o Administración.",
    "Experiencia de trabajo en Servicio al Cliente mínima de un año.",
    "Experiencia en recepción y atención al cliente",
    "Comunicación asertiva",
    "Integridad, compromiso, iniciativa",
    "Residir cerca al distrito de la Victoria",
    "Disponibilidad para trabajar de lunes a viernes de 9:15 pm a 7:00 pm y los sábados de 9:15 pm a 1:30 pm."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día de trabajo.",
    "Beneficios de acuerdo a ley (CTS, vacaciones, beneficios sociales, etc.)",
    "Pertenecer a una de las empresas líderes en el mercado",
    "Una remuneración de S/.1130 + 400 (movilidad)",
    "¡Únete a nosotros, te estamos esperando! Déjanos tu CV al 908896045"
   ],
   "url": "https://www.bumeran.com.pe/empleos/urgente-asistente-de-servicio-al-cliente-planilla-consulting-management-1118395246.html"
  },
  {
   "id": 375,
   "puesto": "Ejecutivo de Ventas",
   "empresa": "Consulting & Management",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 49,
   "vence": "2026-10-07",
   "restan": 11,
   "score": 92,
   "resumen": "Captar y atención a clientes por redes sociales, correos o llamadas.",
   "funciones": [
    "Captar y atención a clientes por redes sociales, correos o llamadas.",
    "Asesorar al cliente brindando soluciones que permitan realizar cierres de venta.",
    "Informar a sus clientes acerca de las promociones y ofertas vigentes.",
    "Feed back sobre niveles de stock de productos para clientes.",
    "Generar órdenes de venta en el sistema.",
    "Seguimiento al cliente para garantizar su satisfacción."
   ],
   "requisitos": [
    "Nuestra empresa principal comercializador de hardware con operaciones a nivel nacional, busca al mejor talento para que se incorpore al puesto de Ejecutivo de Ventas",
    "Experiencia mínima de 6 meses realizando ventas de rubro TI.",
    "Experiencia como vendedor de campo o call.",
    "Experiencia en ventas call, e-commerce."
   ],
   "beneficios": [
    "Pertenecer a una empresa líder en su rubro.",
    "Ingreso a planilla desde el primer día de trabajo",
    "Beneficios de acuerdo a ley (EsSalud, gratificación, asignación familiar, vacaciones)",
    "Sueldo S/. 1130 (básico) + S/. 700 (comisiones).",
    "¡Únete a nuestro equipo! Déjanos tu CV al 908896045"
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-de-ventas-comisiones-planilla-completa-consulting-management-1118395252.html"
  },
  {
   "id": 376,
   "puesto": "Operario de Almacén",
   "empresa": "CYL",
   "cat": "Logística",
   "min": 1150,
   "max": 1150,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 49,
   "vence": "2026-10-07",
   "restan": 11,
   "score": 91,
   "resumen": "Ejecuta la recepción, almacenamiento, picking y despacho de mercancías vinculadas a procesos de importación y exportación",
   "funciones": [
    "Ejecuta la recepción, almacenamiento, picking y despacho de mercancías vinculadas a procesos de importación y exportación",
    "Verifica cantidades, documentación y estado físico de la mercancía crítica.",
    "Registra los movimientos de ingreso, salida y ubicación de mercancías en los sistemas y formatos establecidos.",
    "Identifica, separa y reporta productos no conformes (PNC).",
    "Controla y reporta los stocks de mercancía crítica.",
    "Cumplir con las normas de calidad y seguridad y reglamento interno de trabajo dentro de la planta."
   ],
   "requisitos": [
    "Secundaria completa",
    "Experiencia 1 mes como mínimo",
    "DISPOSICIÓN PARA APRENDER Y ADAPTARSE EN EL ÁREA.",
    "Disponibilidad de horas extra",
    "TURNO: Domingo día libre (ROTAN DE MANERA MENSUAL)",
    "Turno 1 Lunes a Viernes de 7 am a 4:15 pm y sábados de 7 am a 12:30 pm",
    "Turno 2 Lunes a Viernes de 9 am a 7:15 pm."
   ],
   "beneficios": [
    "Ingreso a planilla directo (todos los beneficios+SSALUD+ASIGNACION FAMILIAR+UTILIDADES)",
    "Bono de transporte+ bono por asistencia",
    "Estabilidad laboral",
    "Sueldo s/1150.00 aumenta al año",
    "Alimentación un 70% cubierto"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-almacen-|-bonos-beneficios-|-ate-cyl-1118395216.html"
  },
  {
   "id": 377,
   "puesto": "Operario de Limpieza de Piezas",
   "empresa": "CYL",
   "cat": "Ingeniería",
   "min": 1180,
   "max": 1180,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 49,
   "vence": "2026-10-07",
   "restan": 11,
   "score": 91,
   "resumen": "¡VACANTES DISPONIBLES YA! – INGRESO INMEDIATO",
   "funciones": [
    "Limpieza y conteo de piezas durante el proceso de producción",
    "Apoyo a operarios de máquina",
    "Mantener el orden y limpieza del área de trabajo",
    "Cumplir con normas de calidad y seguridad",
    "Trabajo dentro de planta"
   ],
   "requisitos": [
    "Secundaria completa",
    "Con o sin experiencia",
    "Disponibilidad para realizar horas extras",
    "Actitud para aprender y adaptarse al área de producción",
    "Disponibilidad para turno rotativos",
    "Turno 1 Lunes a Viernes de 7 am a 4:15 pm y sábados de 7 am a 12:30 pm",
    "Turno 2 Lunes a Viernes de 9 am a 7:15 pm."
   ],
   "beneficios": [
    "Ingreso a planilla directa con todos los beneficios de ley",
    "Bono de transporte + bono por asistencia",
    "Sueldo S/ 1150.00 con incremento a S/ 1180.00 al cumplir un año",
    "Alimentación cubierta al 70%",
    "Buen clima laboral y oportunidades de crecimiento"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-limpieza-de-piezas-produccion-santa-anita-cyl-1118395169.html"
  },
  {
   "id": 378,
   "puesto": "Diseñador(a) Gráfico",
   "empresa": "Confidencial",
   "cat": "Logística",
   "min": 1900,
   "max": 2100,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 49,
   "vence": "2026-11-06",
   "restan": 41,
   "score": 89,
   "resumen": "Realizar el diseño gráfico de las campañas de las estaciones de servicio y markets.",
   "funciones": [
    "Realizar el diseño gráfico de las campañas de las estaciones de servicio y markets.",
    "Supervisar la ejecución de las campañas de publicidad de inicio a fin",
    "Administrar y actualizar las promociones en las estaciones de servicio y markets.",
    "Proponer nuevos materiales POP que permitan mejorar la visibilidad de productos y promociones.",
    "Desarrollar y actualizar el cronograma de promociones para las estaciones de servicios y markets.",
    "Contactar a proveedores y al área de logística para la realización de solicitud de material POP o elementos de unidades del grupo.",
    "Apoyar en la realización preliminar de eventos corporativos y activaciones.",
    "Enviar a Logística el material POP para ser distribuido a las estaciones de servicio.",
    "Asistir en gestiones y reportes en general del área de marketing."
   ],
   "requisitos": [
    "Experiencia de 2 años en las funciones mencionadas en empresas del sector retail, tiendas por conveniencias, grifos, agencias de marketing, etc.",
    "Formación académica: Técnicos o universitarios",
    "Dominio de programas de diseño y edición de videos"
   ],
   "beneficios": [
    "Sueldo: Entre S/ 1900 a S/ 2100",
    "Planilla Régimen General",
    "Lugar de trabajo: Breña",
    "Horario: Horario: lunes a viernes de 8:45 a.m. a 6:15 p.m. y sábados de 9:15 a.m. a 1:30 p.m (Intercalados)"
   ],
   "url": "https://www.bumeran.com.pe/empleos/disenadora-grafico-1118395202.html"
  },
  {
   "id": 379,
   "puesto": "Auxiliar de Servicio al Pasajero en el nuevo aeropuerto",
   "empresa": "Consultora CyL",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 49,
   "vence": "2026-10-07",
   "restan": 11,
   "score": 84,
   "resumen": "CONVOCATORIA: Auxiliar de Servicio al pasajero ✈️",
   "funciones": [
    "🛠️ Asistir a pasajeros con discapacidad o movilidad reducida",
    "🛠️ Brindar atención amable, segura y oportuna durante el embarque y desembarque",
    "🛠️ Orientar y acompañar a los pasajeros dentro del aeropuerto",
    "🛠️ Apoyar en el flujo de pasajeros en zonas de alta demanda"
   ],
   "requisitos": [
    "📝 Secundaria completa",
    "📝 Vivir en zonas aledañas al Callao",
    "📝 Disponibilidad para turnos rotativos"
   ],
   "beneficios": [
    "✅ Sueldo S/.1130 + Planilla desde el primer día",
    "✅ Todos los beneficios de ley (Gratificaciones, CTS, vacaciones, seguro)",
    "✅ Capacitaciones virtuales + Línea de carrera"
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-de-servicio-al-pasajero-en-el-nuevo-aeropuerto-consultora-cyl-1118395207.html"
  },
  {
   "id": 380,
   "puesto": "Únete al Aeropuerto como Auxiliar de Acopio",
   "empresa": "Consultora CyL",
   "cat": "Recursos Humanos",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 49,
   "vence": "2026-10-07",
   "restan": 11,
   "score": 84,
   "resumen": "CONVOCATORIA: Auxiliar de Acopio Aeroportuario ✈️",
   "funciones": [
    "🛠️ Recolectar coches portaequipajes abandonados.",
    "🛠️ Recoge los coches portaequipajes que usan los pasajeros de todas las áreas.",
    "🛠️ Mantiene la cantidad de los coches portaequipajes en las áreas especificas.",
    "🛠️ Mantener en buen estado y asegurar que las áreas designadas tengan suficientes coches disponibles de manera eficiente y segura."
   ],
   "requisitos": [
    "📝 Secundaria completa",
    "📝 Vivir en zonas aledañas al Callao",
    "📝 Bienestar físico",
    "📝 Disponibilidad para turnos rotativos"
   ],
   "beneficios": [
    "✅ Sueldo S/.1130 + Planilla desde el primer día",
    "✅ Todos los beneficios de ley (Gratificaciones, CTS, vacaciones, seguro)",
    "✅ Capacitaciones virtuales + Línea de carrera"
   ],
   "url": "https://www.bumeran.com.pe/empleos/unete-al-aeropuerto-como-auxiliar-de-acopio-consultora-cyl-1118395208.html"
  },
  {
   "id": 381,
   "puesto": "Jefa de Tienda Miraflores",
   "empresa": "ADN RETAIL S.A.C.",
   "cat": "Ventas",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 51,
   "vence": "2026-11-04",
   "restan": 39,
   "score": 97,
   "resumen": "Responsable de monitorear el desempeño comercial de la marca.",
   "funciones": [
    "Responsable de monitorear el desempeño comercial de la marca.",
    "Responsable del control del stock físico y digital de la empresa.",
    "Responsable del control de ingresos, egresos y conciliaciones financieras.",
    "Responsable de mantener la información administrativa organizada y disponible.",
    "Responsable de asegurar que las operaciones diarias de la empresa funcionen correctamente."
   ],
   "requisitos": [
    "Solo mujeres de cualquier nacionalidadentre 28 y 45 años.",
    "Tener experiencia mínima de 1 año en posiciones similares en marcas retail ubicadas en centros comerciales o boutiques puerta a calle o showrooms.",
    "Excelente trato y orientación al cliente.",
    "Manejo de excel a nivel intermedio.",
    "Disponibilidad para laborar 5 días a la semana y se descansan 2 días (se descanda siempre el domingo y 1 dia más a la semana que puede ser entre lunes a jueves). El horario es de lunes a sábados de 9:30 am a 8 pm y los domingos de 9:30 am a 7 pm."
   ],
   "beneficios": [
    "Grato ambiente laboral y posibilidad de hacer línea de carrera.",
    "Se paga mediante recibos de honorarios (por ende no hay descuento de ley).",
    "Entrega de 50% de la gratificación en julio y diciembre y 15 días de vacaciones al año.",
    "Sueldo básico (S/.2000) + comisiones desde la 1ra venta de toda la tienda (en promedio van desde los S/.500 a más).",
    "Descuentos en productos de la marca",
    "La marca entrega un polo de la marca y se trabajan en jean y zapatillas."
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefa-de-tienda-miraflores-adn-retail-s.a.c.-1118392893.html"
  },
  {
   "id": 382,
   "puesto": "Asesor Comercial Cajamarca",
   "empresa": "TECSUP",
   "cat": "Ventas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Trujillo",
   "fuente": "Bumeran",
   "dias": 51,
   "vence": "2026-12-04",
   "restan": 69,
   "score": 95,
   "resumen": "Realizar prospección y visitas a colegios de Cajamarca para promover la oferta educativa de TECSUP Trujillo.",
   "funciones": [
    "Realizar prospección y visitas a colegios de Cajamarca para promover la oferta educativa de TECSUP Trujillo.",
    "Generar relacionamiento con directivos, docentes y estudiantes de 5.° de secundaria.",
    "Brindar información y asesoría sobre las carreras técnicas y beneficios de estudiar en TECSUP.",
    "Identificar oportunidades de captación y concretar inscripciones.",
    "Realizar seguimiento a potenciales estudiantes durante el proceso comercial.",
    "Cumplir los objetivos de captación e inscripción establecidos."
   ],
   "requisitos": [
    "Egresado(a) técnico o universitario de Administración, Ciencias de la Comunicación, Ingeniería Industrial o carreras afines.",
    "Experiencia mínima de 2 años en áreas comerciales, ventas consultivas o captación de clientes.",
    "Deseable experiencia en el sector educativo.",
    "Manejo de Microsoft Office a nivel intermedio.",
    "Disponibilidad para realizar trabajo de campo en Cajamarca."
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Sueldo fijo de S/ 1,500.",
    "Comisiones por cumplimiento de objetivos.",
    "Línea de carrera.",
    "Capacitaciones constantes.",
    "Oportunidad de formar parte de una institución líder en educación tecnológica.",
    "Información adicional",
    "Modalidad: Presencial.",
    "Horario: Lunes a viernes de 08:00 a.m. a 06:00 p.m.",
    "Lugar de trabajo: Cajamarca.",
    "En TECSUP promovemos la igualdad de oportunidades. Todas las personas serán consideradas en el proceso de selección sin distinción alguna."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-comercial-cajamarca-tecsup-1118393186.html"
  },
  {
   "id": 383,
   "puesto": "Ejecutivo de Ventas acabados premium",
   "empresa": "ABANZA CONSULTING E.I.R.L",
   "cat": "Ventas",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Arequipa",
   "fuente": "Bumeran",
   "dias": 51,
   "vence": "2026-12-04",
   "restan": 69,
   "score": 90,
   "resumen": "¿Tienes experiencia comercial en el sector de acabados de la construcción o venta de productos técnicos y te apasiona generar nuevos negocios en Arequipa?",
   "funciones": [
    "Generar nuevas oportunidades comerciales y posicionar los productos de la empresa en obras y proyectos de edificación, desarrollando relaciones de largo plazo con arquitectos, constructoras, inmobiliarias, diseñadores de interiores y distribuidores.",
    "Prospectar y desarrollar nuevos clientes dentro del sector construcción, identificando oportunidades comerciales en obras y proyectos desde sus etapas iniciales.",
    "Realizar visitas comerciales, brindar asesoría técnica y realizar presentaciones de producto. Registrar y dar seguimiento a las oportunidades comerciales en el CRM.",
    "Identificar nuevos segmentos y oportunidades para incrementar la participación de la empresa en el mercado."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en Arquitectura, Diseño de Interiores, Administración, Marketing, Ciencias de la Comunicación, Economía o carreras afines.",
    "Indispensable experiencia mínima de 3 años en ventas de materiales de construcción, acabados, ferretería o productos que requieran asesoría técnica al cliente .",
    "Se valorará experiencia en revestimientos cerámicos, porcelanatos, pisos, sanitarios, griferías y acabados de interiores, así como, conocimiento del mercado de la construcción en Arequipa.",
    "Manejo de Microsoft Office. Deseable experiencia utilizando CRM."
   ],
   "beneficios": [
    "Sueldo de S/ 2,000 fijo + comisiones sin tope",
    "Ingreso a planilla con todos los beneficios de ley (Régimen General).",
    "Pago de movilidad para la ruta asignada.",
    "Horario de lunes a viernes de 8:00 a.m. a 5:30 p.m. y sábados de 8:00 a.m. a 1:00 p.m.",
    "Trabajo 100% presencial en Arequipa ."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-de-ventas-acabados-premium-en-arequipa-abanza-consulting-e.i.r.l-1118393537.html"
  },
  {
   "id": 384,
   "puesto": "Ingeniero Residente",
   "empresa": "Confidencial",
   "cat": "Ingeniería",
   "min": 5200,
   "max": 5500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Ica",
   "fuente": "Bumeran",
   "dias": 51,
   "vence": "2026-12-04",
   "restan": 69,
   "score": 89,
   "resumen": "Empresa que brinda servicios de Ingeniería, Mantenimiento y Logística Interna a empresas mineras se encuentra en la búsqueda de un RESIDENTE para laborar en Marcona.",
   "funciones": [
    "Planificar, monitorear y controlar la ejecución del proyecto.",
    "Coordinar con ingenieros y supervisores las actividades diarias.",
    "Supervisar el avance, calidad y cumplimiento de los procedimientos, cronogramas, planes de calidad y SSOMA.",
    "Programar actividades y verificar disponibilidad de personal, equipos y materiales.",
    "Supervisar compras, almacén e inventario.",
    "Coordinar con el Ingeniero SSOMA las acciones relacionadas con seguridad."
   ],
   "requisitos": [
    "TÍTULO PROFESIONAL EN INGENIERÍA MECÁNICA O INGENIERÍA MECÁNICO-ELÉCTRICA , habilitado y con colegiatura vigente.",
    "Mínimo 3 años de experiencia en gestión de proyectos de ingeniería.",
    "Experiencia en elaboración de planes de trabajo, cronogramas, ingreso de personal y distribución de personal operativo.",
    "Experiencia en cálculos de materiales, gestión de presupuestos, valorizaciones, liquidaciones y control de proyectos hasta su entrega al cliente."
   ],
   "beneficios": [
    "Sueldo: S/ 5,200 – S/ 5,500.",
    "Horario: Lunes a sábado de 08:00 a.m. a 05:00 p.m.",
    "Alojamiento y alimentación.",
    "Importante: Los postulantes deben contar con su CERTIADULTO en el CV registrado en Búmeran."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ingeniero-residente-marcona--1118393243.html"
  },
  {
   "id": 385,
   "puesto": "Asistente Contable o Administrativo para Tienda",
   "empresa": "K-Globalis Consulting S.A.C.",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 51,
   "vence": "2026-10-05",
   "restan": 9,
   "score": 84,
   "resumen": "Atención, acompañamiento, gestión de ventas y promoción de productos a nuestros clientes en base a técnicas en asesoría de imagen.",
   "funciones": [
    "Atención, acompañamiento, gestión de ventas y promoción de productos a nuestros clientes en base a técnicas en asesoría de imagen.",
    "Reposición de mercadería, diseñar y proponer el visual de la tienda.",
    "Manejo de Caja y POS.",
    "Generación de Reportes.",
    "Cuidar de la imagen, orden y limpieza de la tienda a cargo.",
    "Capacitación del personal a cargo.",
    "Supervisión de terceros.",
    "Otras funciones administrativas que designe su jefe inmediato."
   ],
   "requisitos": [
    "Edad mínima 2 2 años.",
    "Vivir en zonas aledañas al Cercado de Lima.",
    "Tener experiencia como Administradora de tienda y gestión de ventas retail directo al cliente, con interés en aprender.",
    "Experiencia en control de inventarios, manejo de caja y POS.",
    "Manejo de Excel básico (reportes, inventarios, control).",
    "Manejo de información básica de redes sociales.",
    "Amabilidad al momento de atender al cliente.",
    "Comunicación asertiva y actitud positiva.",
    "Escucha activa y empatía.",
    "Responsable, puntual, comunicativa y presentable.",
    "Pasión y gusto por la moda.",
    "De 1 1: 0 0 am a 8: 0 0 pm de Lunes a Sábado."
   ],
   "beneficios": [
    "Ingreso a planilla.",
    "Sueldo mínimo + incentivos de ventas (S/ 1, 9 0 0 según desempeño).",
    "Buen ambiente laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-contable-o-administrativo-para-tienda--experiencia-en-venta-de-ropa-de-ninos-k-globalis-consulting-s.a.c.-1118390400.html"
  },
  {
   "id": 386,
   "puesto": "Administradora) de Tienda",
   "empresa": "K-Globalis Consulting S.A.C.",
   "cat": "Ventas",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 51,
   "vence": "2026-10-05",
   "restan": 9,
   "score": 84,
   "resumen": "Atención, acompañamiento, gestión de ventas y promoción de productos a nuestros clientes en base a técnicas en asesoría de imagen.",
   "funciones": [
    "Atención, acompañamiento, gestión de ventas y promoción de productos a nuestros clientes en base a técnicas en asesoría de imagen.",
    "Reposición de mercadería, diseñar y proponer el visual de la tienda.",
    "Manejo de Caja y POS.",
    "Generación de Reportes.",
    "Cuidar de la imagen, orden y limpieza de la tienda a cargo.",
    "Capacitación del personal a cargo.",
    "Supervisión de terceros.",
    "Otras funciones administrativas que designe su jefe inmediato."
   ],
   "requisitos": [
    "Edad mínima 2 2 años.",
    "Vivir en zonas aledañas al Cercado de Lima.",
    "Tener experiencia como Administradora de tienda y gestión de ventas retail directo al cliente, con interés en aprender.",
    "Experiencia en control de inventarios, manejo de caja y POS.",
    "Manejo de Excel básico (reportes, inventarios, control).",
    "Manejo de información básica de redes sociales.",
    "Amabilidad al momento de atender al cliente.",
    "Comunicación asertiva y actitud positiva.",
    "Escucha activa y empatía.",
    "Responsable, puntual, comunicativa y presentable.",
    "Pasión y gusto por la moda.",
    "De 1 1: 0 0 am a 8: 0 0 pm de Lunes a Sábado."
   ],
   "beneficios": [
    "Ingreso a planilla.",
    "Sueldo mínimo + incentivos de ventas (S/ 2, 0 0 0 según desempeño).",
    "Buen ambiente laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/administradora-de-tienda--experiencia-en-venta-de-ropa-de-ninos-k-globalis-consulting-s.a.c.-1118390406.html"
  },
  {
   "id": 387,
   "puesto": "Mecánico de Mantenimiento Industrial",
   "empresa": "Confidencial",
   "cat": "Ingeniería",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 52,
   "vence": "2026-11-03",
   "restan": 38,
   "score": 94,
   "resumen": "Importante empresa del sector Industrial se encuentra en búsqueda de TÉCNICO MECANICO DE MANTENIMIENTO (TEMPORAL) - INDUSTRIAL para la SEDE SANTA ANITA que cumpla con las siguientes tareas:",
   "funciones": [
    "Ejecutar las tareas de Mantenimiento Predictivo, Correctivo y Reactivo de la máquina de acuerdo a las indicaciones dadas por su Jefatura.",
    "Reportar en forma clara e inmediata todas las acciones ejecutadas en su turno de trabajo, así como las actividades pendientes o en curso de ejecución (registro de Actividades).",
    "Controlar la operatividad de los equipos y maquinarias de planta.",
    "Mantener limpio y ordenado su área de trabajo.",
    "Desempeñar cualquier otra actividad conexa y complementarias relacionadas al puesto."
   ],
   "requisitos": [
    "Técnico Egresado o titulado de la carrera de mecánica de mantenimiento o afines.",
    "Experiencia mínima de 02 años en empresas industriales.",
    "Conocimiento de mantenimiento preventivo, correctivo.",
    "Conocimiento sobre transmisión de faja y polea, montaje, desmontaje, bombas, soldadura, ASR, reducción de averías, etc.",
    "Conocimiento en TPM / 5s /SAP/ Office básico – intermedio (deseable).",
    "Disponibilidad para trabajar en turnos rotativos de 8h (mañana, tarde y madrugada), con un día de descanso a la semana."
   ],
   "beneficios": [
    "Remuneración fija de S/. 2000 + bono variable de S/200",
    "Planilla completa c/beneficios de Ley (EsSalud, CTS, Asignación familiar, Gratificación, entre otros)",
    "Contrato temporal - renovable de acuerdo a la evaluación.",
    "Movilidad de Acercamiento.",
    "Alimentación cubierta al 100 %.",
    "Bolsones con productos de la empresa",
    "Entre otros beneficios corporativos",
    "Si cumples con el perfil y deseas formar parte de nuestro equipo, postula ahora y únete a una empresa reconocida en el mercado."
   ],
   "url": "https://www.bumeran.com.pe/empleos/mecanico-de-mantenimiento-industrial-temporal-santa-anita-planilla-directa-1118392356.html"
  },
  {
   "id": 388,
   "puesto": "Asesor de Atención al Cliente",
   "empresa": "IBR PERU S.A.",
   "cat": "Atención al Cliente",
   "min": 1250,
   "max": 1250,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 52,
   "vence": "2026-11-03",
   "restan": 38,
   "score": 92,
   "resumen": "En búsqueda de un asesor para Atención al Cliente – presencial San Isidro",
   "funciones": [
    "Atender consultas y reclamos de usuarios finales mediante correo electrónico.",
    "Gestionar llamadas de clientes derivadas por ejecutivos para brindar soporte y solución a sus consultas.",
    "Registrar y actualizar las gestiones realizadas en los sistemas correspondientes.",
    "Brindar información al cliente sobre casos y avances de gestión de usuarios finales.",
    "Mantener actualizado el registro de clientes atendidos.",
    "Elaborar y enviar reportes de atención cuando sean solicitados por el cliente."
   ],
   "requisitos": [
    "Contar con experiencia mínima de 1 año en atención al cliente, reclamos, Back Office. (Banca o Vehicular).",
    "Conocimiento en Fondos Colectivos (deseable).",
    "Manejo de sistemas CRM y/o SAP (deseable).",
    "Disponibilidad para laborar de lunes a sábado, dentro del rango de 9:00 a. m. a 9:00 p. m., con horarios rotativos (48 horas semanales)."
   ],
   "beneficios": [
    "Sueldo: S/ 1,250 + Bono fijo de S/ 250",
    "Planilla completa desde el primer día de trabajo (todo los beneficios acorde a ley).",
    "Línea de carrera a partir del 6to mes."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-atencion-al-cliente-ibr-peru-s.a.-1118392029.html"
  },
  {
   "id": 389,
   "puesto": "Ejecutivo Comercial",
   "empresa": "Beliving Talents",
   "cat": "Ventas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 52,
   "vence": "2026-10-04",
   "restan": 8,
   "score": 92,
   "resumen": "Por encargo de nuestro cliente, importante empresa del sector de alimentos y bebidas, nos encontramos en búsqueda de 1 Ejecutivo(a) Comercial:",
   "funciones": [
    "Prospectar, captar, negociar y cerrar acuerdos comerciales con clientes B2B del canal HORECA.",
    "Realizar visitas comerciales, presentaciones de producto y degustaciones.",
    "Gestionar eventos corporativos, activaciones y formatos de venta móvil.",
    "Identificar oportunidades para nuevos puntos de venta.",
    "Desarrollar alianzas estratégicas y campañas de co-branding."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en Administración, Marketing, Negocios Internacionales, Comunicaciones, Turismo, Hotelería o carreras afines.",
    "Mínimo 1 año de experiencia en ventas B2B, desarrollo comercial o generación de nuevos negocios.",
    "Experiencia en CRM, trade marketing o activaciones comerciales dentro del canal HORECA.",
    "Manejo de Microsoft Office a nivel intermedio."
   ],
   "beneficios": [
    "Sueldo de S/ 1,500 + bonos + comisiones .",
    "Ingreso a planilla MYPE .",
    "Modalidad híbrida (2 días presenciales con visitas comerciales).",
    "Horario: lunes a viernes de 9:00 a.m. a 6:00 p.m. y sábados de 9:00 a.m. a 1:00 p.m.",
    "Lugar de trabajo: Benavides, Miraflores ."
   ],
   "url": "https://www.bumeran.com.pe/empleos/ejecutivo-comercial-beliving-talents-1118392161.html"
  },
  {
   "id": 390,
   "puesto": "Operario de Limpieza de Piezas-producción con",
   "empresa": "CYL",
   "cat": "Ingeniería",
   "min": 1180,
   "max": 1180,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 52,
   "vence": "2026-10-04",
   "restan": 8,
   "score": 91,
   "resumen": "¡TU OPORTUNIDAD EN PRODUCCIÓN ESTÁ AQUÍ!",
   "funciones": [
    "Limpieza y conteo de piezas durante el proceso de producción",
    "Apoyo a operarios de máquina",
    "Mantener el orden y limpieza del área de trabajo",
    "Cumplir con normas de calidad y seguridad",
    "Trabajo dentro de planta"
   ],
   "requisitos": [
    "Secundaria completa",
    "Con o sin experiencia",
    "Disponibilidad para realizar horas extras",
    "Actitud para aprender y adaptarse al área de producción",
    "Disponibilidad para turno rotativos",
    "Turno 1 Lunes a Viernes de 7 am a 4:15 pm y sábados de 7 am a 12:30 pm",
    "Turno 2 Lunes a Viernes de 9 am a 7:15 pm."
   ],
   "beneficios": [
    "Ingreso a planilla directa con todos los beneficios de ley",
    "Bono de transporte + bono por asistencia",
    "Sueldo S/ 1150.00 con incremento a S/ 1180.00 al cumplir un año",
    "Alimentación cubierta al 70%",
    "Buen clima laboral y oportunidades de crecimiento"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-limpieza-de-piezas-produccion-con-sin-experiencia-ate-cyl-1118391848.html"
  },
  {
   "id": 391,
   "puesto": "Asistente de Tesorería",
   "empresa": "INSALUD CORP",
   "cat": "Contabilidad",
   "min": 1800,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 52,
   "vence": "2026-11-03",
   "restan": 38,
   "score": 90,
   "resumen": "Somos un centro especializado en salud que brinda servicios de salud de calidad enfocados en la seguridad de nuestros pacientes.",
   "funciones": [
    "Elaborar y mantener el flujo de caja proyectado (semanal y mensual), consolidado y por empresa",
    "Programar y ejecutar pagos, gestionando el cronograma y priorización de proveedores, obligaciones tributarias y planilla",
    "Diseñar e implementar políticas de pago y documentar los procesos de tesorería",
    "Reportar posición de caja y ejecución vs. proyección al área de Finanzas"
   ],
   "requisitos": [
    "Bachiller o Titulado(a) en Contabilidad, Finanzas, Administración o afines",
    "+2 años en tesorería, idealmente en entornos multi-empresa / empresas relacionadas",
    "Sólido dominio de flujo de caja proyectado y presupuesto de caja",
    "Excel avanzado y manejo de telebanking multi-banco",
    "Deseable: conocimiento de detracciones y régimen tributario peruano, experiencia en sector servicios/salud"
   ],
   "beneficios": [
    "Modalidad presencial de Lunes a Sábado — sede Surco",
    "Planilla régimen Mype desde el primer día",
    "Remuneración: S/1,800.00"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-tesoreria-insalud-corp-1118391781.html"
  },
  {
   "id": 392,
   "puesto": "Analista de Planillas",
   "empresa": "Talento Humano Perú",
   "cat": "Recursos Humanos",
   "min": 3500,
   "max": 4000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 52,
   "vence": "2026-11-03",
   "restan": 38,
   "score": 89,
   "resumen": "Nuestro cliente, empresa que brinda servicios al sector minero, nos ha solicitado la búsqueda de un (1) Analista de Planillas.",
   "funciones": [
    "Gestionar integralmente las planillas, boletas, PLAME, AFP Net, contratos laborales y el control de asistencia.",
    "Asegurar la gestión de inducción, armado de legajos.",
    "Coordinar exámenes médicos ocupacionales, vacunas y pólizas de seguro necesarias para el personal y proyectos.",
    "Tramitar ante ESSALUD los subsidios por incapacidad, maternidad y el canje de certificados médicos.",
    "Mantener actualizados las estructuras organizacionales y cumplir los estándares de seguridad indicados por la empresa"
   ],
   "requisitos": [
    "Egresado de carrera profesional de Administración, Recursos Humanos o carreras afines al puesto.",
    "Experiencia mínima de 2 años realizando las funciones del puesto.",
    "Conocimientos en legislación laboral peruana y tributos laborales.",
    "Ms. Office, nivel intermedio."
   ],
   "beneficios": [
    "Ingreso a planilla general, con todos los beneficios de acuerdo a ley.",
    "Remuneración entre S/. 3,500.00 a S/. 4,000.00 soles (De acuerdo a evaluación).",
    "LUGAR Y HORARIO DE TRABAJO",
    "San Isidro - Lima.",
    "Lunes a viernes de 8:00 a.m. a 6:00 p.m.",
    "Trabajo Presencial."
   ],
   "url": "https://www.bumeran.com.pe/empleos/analista-de-planillas-|-san-isidro-talento-humano-peru-1118392367.html"
  },
  {
   "id": 393,
   "puesto": "Operario de Producción",
   "empresa": "MOLINOS ASOCIADOS SAC",
   "cat": "Ingeniería",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 52,
   "vence": "2026-11-03",
   "restan": 38,
   "score": 89,
   "resumen": "We are 100% Peruvian Brand specialized in Plant based ingredients from Tara Pod derivatives.",
   "funciones": [
    "Alimentar el molino con pala, echando la tara a la tolva",
    "Habilitar los sacos y llenarlos en la zona de llenado de harina",
    "Pesar, coser y paletizar los sacos",
    "Apoyo en limpieza, orden y otras tareas del área"
   ],
   "requisitos": [
    "Lo que buscamos en ti",
    "Experiencia previa en trabajo físico (en cualquier rubro)",
    "Disponibilidad para turno rotativo 4x3 (4 días de 12 horas, 3 de descanso), incluye turnos de día y de noche",
    "Puntualidad y compromiso",
    "No se exige secundaria completa ni estudios técnicos",
    "Full time, presencial en planta VES"
   ],
   "beneficios": [
    "Línea de carrera real: Puedes pasar a otras áreas después de 1 año, con mejores sueldos y beneficios.",
    "Bono nocturno: ingreso adicional en los turnos de noche (70-80 soles)",
    "Planilla desde el día 1, con todos los beneficios de ley al 100%",
    "Sueldo fijo de S/ 1,200",
    "Bono de puntualidad y asistencia: S/. 200.00",
    "Comida: GRATIS",
    "Otros bonos a partir de los 6 meses",
    "Te capacitamos, no necesitas experiencia previa en planta",
    "Estabilidad laboral"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-produccion--unete-a-la-planta-que-lleva-la-tara-del-peru-molinos-asociados-sac-1118392449.html"
  },
  {
   "id": 394,
   "puesto": "Operario de Almacén",
   "empresa": "Talento Humano Perú",
   "cat": "Logística",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 52,
   "vence": "2026-11-03",
   "restan": 38,
   "score": 83,
   "resumen": "Empresa de reconocido prestigio del rubro de Sistemas de Riego, nos ha encargado la búsqueda de un (1) Operario de Almacén.",
   "funciones": [
    "Carga y descarga de productos de aprox. 25k del almacén a los contenedores y viceversa.",
    "Acompañar a los despachadores en ruta de entrega de productos.",
    "Recepción y organización de mercancías.",
    "Preparación y despacho de mercadería.",
    "Colaborar en el inventario y control de existencias.",
    "Apoyo en las tareas indicadas en el área de Almacén."
   ],
   "requisitos": [
    "Secundaria completa.",
    "Experiencia mínima de tres (03) meses como operario de almacén, producción, despachador, estibador y/o afines al puesto.",
    "Habilidad para realizar tareas físicas.",
    "Trabajo en equipo.",
    "Seguir indicaciones."
   ],
   "beneficios": [
    "Ingreso a planilla general, con todos los beneficios de labor según Ley.",
    "Remuneración: S/ 1,500.00 soles.",
    "HORARIO Y LUGAR DE TRABAJO",
    "Distrito: San Borja.",
    "Lunes a Viernes de 8:30 a.m. a 6:00 p.m. y Sábado de 9:00 a.m. a 12:00 p.m.",
    "Trabajo Presencial."
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-almacen-|-san-borja-talento-humano-peru-1118392373.html"
  },
  {
   "id": 395,
   "puesto": "Asistente Comercial B2B",
   "empresa": "BUMERAN SELECTA",
   "cat": "Ventas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 53,
   "vence": "2026-11-02",
   "restan": 37,
   "score": 95,
   "resumen": "Importante empresa peruana dedicada a la importación y comercialización de herramientas para la construcción.",
   "funciones": [
    "Realizar el seguimiento de cotizaciones para asegurar una atención oportuna y efectiva.",
    "Atender consultas de clientes a través de llamadas telefónicas, correo electrónico y WhatsApp.",
    "Brindar atención presencial a clientes que visiten nuestras oficinas.",
    "Registrar y procesar pedidos en el sistema.",
    "Coordinar la entrega de pedidos con las áreas involucradas.",
    "Realizar seguimiento comercial a clientes actuales y potenciales.",
    "Apoyar en las actividades administrativas y comerciales del área."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios en curso, concluidos o egresados.",
    "Experiencia mínima de 6 meses en atención al cliente, ventas o soporte comercial.",
    "Manejo básico de Microsoft Office.",
    "Buena redacción y comunicación escrita.",
    "Deseable experiencia utilizando CRM o herramientas comerciales.",
    "Experiencia en empresas de construcción, ferretería, suministros industriales o rubros afines será valorada."
   ],
   "beneficios": [
    "Ingreso a planilla mype desde el primer día.",
    "Remuneración S/1500.00 + bono por cumplimiento de objetivos.",
    "Modalidad presencial.",
    "Lugar: Surco.",
    "Lunes a viernes de 8:00 a.m. a 5:30 p.m. y Sábados de 8:00 a.m. a 12:00 p.m."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-comercial-b2b--herramientas-para-la-construccion-bumeran-selecta-1118390206.html"
  },
  {
   "id": 396,
   "puesto": "Asesor de Cobranza",
   "empresa": "GRAFTON LATAM",
   "cat": "Contabilidad",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 53,
   "vence": "2026-11-02",
   "restan": 37,
   "score": 95,
   "resumen": "Realizar llamadas de cobranza a clientes con obligaciones pendientes.",
   "funciones": [
    "Realizar llamadas de cobranza a clientes con obligaciones pendientes.",
    "Negociar alternativas de pago de acuerdo con los lineamientos establecidos por la empresa.",
    "Realizar seguimiento a compromisos de pago.",
    "Registrar de manera correcta, completa y oportuna las gestiones realizadas."
   ],
   "requisitos": [
    "Formación académica: Secundaria completa.",
    "Contar con un mínimo de 02 años de experiencia certificada en el rubro de cobranzas, la cual será validada considerando la continuidad laboral dentro del sector.",
    "La experiencia puede corresponder a cualquier tramo de morosidad (temprana, intermedia o castigo).",
    "Disponibilidad para laborar de manera presencial en el distrito de Surquillo.",
    "Competencias: Trabajo en equipo, orientación a resultados, comunicación efectiva."
   ],
   "beneficios": [
    "Sueldo base: S/.1200",
    "Bono de asistencia: S/.200 (sujeto al cumplimiento de asistencia y puntualidad)",
    "Bono de Banco: S/.300",
    "Comisiones : S/.500 a S/.1000",
    "Ingreso a planilla MYPE desde el primer día, conforme a ley.",
    "Gratificación al 100%",
    "Pago de CTS al 50%",
    "Seguro de Essalud.",
    "Vacaciones de 15 días al año.",
    "Capacitación inicial.",
    "Línea de carrera y oportunidad de crecimiento."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesor-de-cobranza-1700comisiones-full-time-am-descanso-domingo-grafton-latam-1118390346.html"
  },
  {
   "id": 397,
   "puesto": "Asistente Jr de SSOMA",
   "empresa": "Well Consulting Services",
   "cat": "Ingeniería",
   "min": 1300,
   "max": 1300,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 53,
   "vence": "2026-11-02",
   "restan": 37,
   "score": 95,
   "resumen": "En Well Human Resources estamos en la búsqueda de Asistente SSOMA, para uno de nuestros principales clientes del rubro industrial, ubicado su planta en Chancay:",
   "funciones": [
    "Elaborar informes de investigación de accidentes e incidentes laborales.",
    "Realizar inspecciones de seguridad y salud en el trabajo, validando el cumplimiento de estándares.",
    "Impartir charlas de prevención en temas de seguridad, salud y medio ambiente.",
    "Gestionar el inventario e inspección de equipos de seguridad y emergencia.",
    "Asesorar a colaboradores, contratistas y terceros en la elaboración de análisis de trabajo seguro."
   ],
   "requisitos": [
    "Egresado, Bachiller en Ingeniería Industrial, Seguridad Industrial, o carreras afines a SST.",
    "Experiencia mínima de 6 meses a 1 año en el área de SSOMA (como practicante, auxiliar o asistente).",
    "Se valorará experiencia previa en plantas industriales.",
    "Sólidos conocimientos en normativas de seguridad y salud ocupacional y gestión ambiental.",
    "Capacidad para trabajar de forma independiente y como parte de un equipo.",
    "Excelentes habilidades de comunicación y redacción."
   ],
   "beneficios": [
    "Salario básico: S/. 1300",
    "Planilla completa con todos los beneficios de ley",
    "Pago de horas extras",
    "Bonos adicionales: aseo (S/. 52), asistencia (S/. 42) y movilidad (S/. 75) → Total: S/. 169",
    "Horario de trabajo: Lunes a sábado, turnos rotativos semanales"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-jr-de-ssoma-chancay-well-consulting-services-1118390471.html"
  },
  {
   "id": 398,
   "puesto": "Asistente de Control de Activos Fijos I",
   "empresa": "Caja de Pensiones Militar Policial",
   "cat": "Ingeniería",
   "min": 2000,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 53,
   "vence": "2026-11-02",
   "restan": 37,
   "score": 92,
   "resumen": "Somos la empresa “Caja de Pensiones Militar Policial\", ubicada en San Isidro y actualmente estamos en búsqueda de (01) nuevo talento para el puesto de ASISTENTE DE CONTROL DE ACTIVOS FIJOS I de la Subgerencia de Logística.",
   "funciones": [
    "Realizar el cruce de información mensual con el almacén sobre las adquisiciones de los bienes, asimismo, llevar el control oportuno del mismo.",
    "Atender las consultas de los entes reguladores en el marco de la Gestión Patrimonial.",
    "Reportar trimestralmente a la subgerencia mediante informes sobre los bienes muebles patrimoniales y los que se encuentren bajo su administración, a fin de lograr una eficiente gestión de los mismos.",
    "Realizar la ejecución del inventario físico anual de bienes muebles, equipos y maquinarias, así como proponer las bajas de bienes de activo fijo.",
    "Ejecutar la carga de información de los bienes patrimoniales de la CPMP en la plataforma del SINABIP.",
    "Reportar mediante informes a la Subgerencia de logística sobre bienes patrimoniales de la CPMP.",
    "Participar en la elaboración de instructivos, directivas, procesos y procedimientos referentes a la gestión patrimonial.",
    "Coordinar el traslado de los bienes patrimoniales en la CPMP, así como el retiro de los bienes en desuso.",
    "Reportar sobre las altas de sobrantes y bajas de bienes muebles de la CPMP, asimismo, elaborar el informe correspondiente, en caso lo designe el Subgerente.",
    "Solicitar mediante requerimiento la contratación de una empresa tercera para la tasación de activos fijos de la CPMP, a fin de actualizar los valores contables en el Sistema de Control Patrimonial.",
    "Cumplir la normativa interna y externa aplicable a su ámbito de acción.",
    "Participar de manera activa y permanente en las acciones dispuestas en la gestión integral de riesgos, asociadas a las actividades que realiza."
   ],
   "requisitos": [
    "Bachiller en administración , ingeniería industrial, economía , o afines. .",
    "De preferencia, capacitación especializada en gestión de servicios generales o control de activos , la misma que es acreditada mediante cursos que totalicen una duración no menor a veinticuatro (24) horas lectivas. .",
    "Experiencia general de dos (2) años y experiencia específica de un (1) año acumulable como asistente, auxiliar o puesto similar.",
    "FUNCIÓN BÁSICA",
    "Efectuar las actividades referidas al control patrimonial de los bienes de activo fijo, asignados al personal de la CPMP."
   ],
   "beneficios": [
    "Remuneración: S/. 2,000.00",
    "Horario de Trabajo regular: 9:00 a.m. a 5:15 p.m. de lunes a viernes (presencial).",
    "Modalidad de contratación: Plazo fijo - D.L. 728 (Planilla, beneficios según ley).",
    "Capacitación acorde a la necesidad del puesto.",
    "Los datos personales que el postulante facilite a través de su hoja de vida, podrán ser recopilados, tratados, e incorporados a un Banco de Datos Personales de la CPMP, solo para dichos fines específicos, y serán conservados por un periodo máximo de dos años.",
    "La Caja de Pensiones Militar Policial - CPMP declara que respeta los principios de legalidad, consentimiento, finalidad, proporcionalidad, calidad, disposición de recurso, nivel de protección adecuado, conforme a las disposiciones de la Ley de Protección de Datos Personales vigente en Perú\"",
    "\" En la Caja de Pensiones Militar Policial creemos firmemente en la igualdad de oportunidades por lo cual promovemos la diversidad de género, origen étnico, nacionalidad, la inclusión de personas con discapacidad y/o habilidades diferentes (de acuerdo con lo establecido en la Ley N° 29973).\""
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-control-de-activos-fijos-i-caja-de-pensiones-militar-policial-1117953060.html"
  },
  {
   "id": 399,
   "puesto": "Operaria de Limpieza de Planta",
   "empresa": "CYL",
   "cat": "Ingeniería",
   "min": 1150,
   "max": 1150,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 53,
   "vence": "2026-10-03",
   "restan": 7,
   "score": 91,
   "resumen": "Limpiar y contar las piezas durante el proceso de producción.",
   "funciones": [
    "Limpiar y contar las piezas durante el proceso de producción.",
    "Brindar apoyo a los operarios de máquina.",
    "Mantener el orden y la limpieza del área de trabajo.",
    "Cumplir con las normas de calidad y seguridad.",
    "Desarrollar sus funciones dentro de planta."
   ],
   "requisitos": [
    "Secundaria completa",
    "Con o sin experiencia.",
    "DISPOSICIÓN PARA APRENDER Y ADAPTARSE EN EL ÁREA.",
    "Disponibilidad para realizar horas extras.",
    "Disponibilidad inmediata para laborar dentro de planta.",
    "Domingo día libre (ROTAN DE MANERA MENSUAL)",
    "Turno 1 Lunes a Viernes de 7 am a 4:15 pm y sábados de 7 am a 12:30 pm",
    "Turno 2 Lunes a Viernes de 9 am a 7:15 pm."
   ],
   "beneficios": [
    "Ingreso a planilla directo (todos los beneficios+SSALUD+ASIGNACION FAMILIAR+UTILIDADES)",
    "Bono de transporte+ bono por asistencia",
    "Estabilidad laboral",
    "Sueldo s/1150.00 aumenta al año s/1180. 00",
    "Alimentación un 70% cubierto"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operaria-de-limpieza-de-planta-|-bonos-alimentacion-cyl-1118390245.html"
  },
  {
   "id": 400,
   "puesto": "Practicante de Proyectos",
   "empresa": "Well Consulting Services",
   "cat": "Prácticas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 53,
   "vence": "2026-11-02",
   "restan": 37,
   "score": 90,
   "resumen": "En Well Human Resources estamos en la búsqueda de Practicante Profesional de Proyectos para el área de Proyectos , para uno de nuestros principales clientes del rubro industrial, ubicado su planta en Chancay:",
   "funciones": [
    "Administración de proyectos y seguimiento continuo a proveedores.",
    "Apoyar en la coordinación y seguimiento operativo con proveedores, incluyendo documentación, respuestas y cumplimiento de plazos.",
    "Egistrar y actualizar información de los proyectos.",
    "Apoyar en otras actividades relacionadas con la gestión de proyectos, siempre bajo supervisión del responsable del área."
   ],
   "requisitos": [
    "Recien egresados universitarios en Adm. de empresas, Ing. Industrial, Ing. de Sistemas y afines.",
    "Conocimiento básico de metodologías de gestión de proyectos (deseable).",
    "Disponibilidad para realizar prácticas profesionales.",
    "Disponibilidad presencial en Planta Chancay."
   ],
   "beneficios": [
    "Salario básico: S/.1200",
    "Planilla practicante desde el primer día.",
    "Seguro FOLA.",
    "Oportunidad de crecimiento y línea de carrera."
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-de-proyectos-chancay-well-consulting-services-1118390427.html"
  },
  {
   "id": 401,
   "puesto": "Jefe Dpto. de Marketing y Filatelia",
   "empresa": "Servicios Postales del Perú S.A.",
   "cat": "Marketing",
   "min": 3655,
   "max": 3655,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 53,
   "vence": "2026-11-02",
   "restan": 37,
   "score": 89,
   "resumen": "SERPOST S.A es una empresa pública de derecho privado del sector transporte y comunicaciones y tiene por actividad principal la prestación de servicios postales con cobertura local, nacional e internacional.",
   "funciones": [
    "A) Diseñar e implementar el plan de marketing de la Empresa.",
    "B) Definir las estrategias de marketing para la oferta de los servicios.",
    "C) Disponer de información válida y confiable del entorno competitivo.",
    "D) Delimitar el público objetivo al que se le va a ofrecer un producto o servicio.",
    "E) Desarrollar e implementar estrategias y campañas de publicidad, a fin de impulsar el alcance de la marca.",
    "F) Identificar a la competencia y a los mercados potenciales.",
    "G) Proponer el calendario anual de emisiones postales en base a la demanda del mercado filatélico.",
    "H) Coordinar la elaboración de artes de los sellos postales, matasellos y todo producto filatélico.",
    "I) Gestionar las redes sociales, construir comunidades, crear conversaciones en torno a la marca en internet.",
    "J) Cumplir de manera eficiente y eficaz con las funciones designadas a su cargo, así como efectuar las actividades relacionadas con el Sistema de Control Interno.",
    "K) Participar en equipos de trabajo que le sean asignados, así como realizar otras funciones afines al cargo que le sean encargadas por el Subgerente."
   ],
   "requisitos": [
    "Educación Formal",
    "Bachiller en Marketing, Comunicaciones, Gestión Comercial, Ingeniería Industrial, Administración y Marketing u otras carreras afines a las funciones del puesto. (Indispensable)",
    "Maestría en Marketing, Gestión Comercial o relacionado al puesto. (Deseable)",
    "Título universitario, Especializaciones en Marketing Digital, Marketing Estratégico u otras especializaciones afines a las funciones del puesto. (Deseable)",
    "Cursos relacionados al puesto o al giro de la empresa. (Deseable)",
    "Los grados académicos serán validados en las plataformas digitales de la Superintendencia Nacional de Educación Superior Universitaria (SUNEDU) y el Ministerio de Educación (MINEDU), en cuanto corresponda.",
    "Se entiende por \"puestos equivalentes”, a aquellos puestos que tienen asignadas funciones iguales o similares y que para su desempeño se exijan requisitos de estudio, experiencia y competencias laborales iguales o similares, sin embargo; tienen nomenclaturas distintas.",
    "La experiencia indispensable señalada como requisito en los perfiles descritos deberá corresponder a funciones directamente asociadas al área funcional donde se ejerce el cargo.",
    "5 años mínimo de experiencia laboral en general, 3 años mínimo en puestos equivalentes (3) o 4 años como coordinador, administrador o supervisor en empresas públicas o privadas. (Indispensable)",
    "Función General",
    "Programar, proponer y ejecutar el plan de marketing de los servicios y productos que comercializa Serpost S.A."
   ],
   "beneficios": [
    "Planilla completa con TODOS los BENEFICIOS de LEY desde el 1er día (Gratificación, CTS, vacaciones, seguro vida ley, essalud, entre otros)",
    "Horario fijo (Mañana de L a V)",
    "Sueldo: S/3,655.00",
    "IMPORTANTE ADJUNTAR CV DOCUMENTADO"
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-dpto.-de-marketing-y-filatelia-servicios-postales-del-peru-s.a.-1118376071.html"
  },
  {
   "id": 402,
   "puesto": "Jefe de Departamento de Recaudacion y Recuperaciones",
   "empresa": "Caja de Pensiones Militar Policial",
   "cat": "Ingeniería",
   "min": 6000,
   "max": 6000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 53,
   "vence": "2026-11-02",
   "restan": 37,
   "score": 86,
   "resumen": "Conducir y controlar las actividades referentes a la recaudación de aportes y a la recuperación de cuentas por cobrar previsionales, en el marco de la normativa y procedimientos vigentes, con la finalidad de contribuir al logro de las metas planteadas para la entidad.",
   "funciones": [
    "Conducir y controlar las actividades referentes a la recaudación de aportes y a la recuperación de cuentas por cobrar previsionales, en el marco de la normativa y procedimientos vigentes, con la finalidad de contribuir al logro de las metas planteadas para la entidad.",
    "Supervisar y controlar la verificación y actualización de la información de los aportantes y aportes de cuota Estado y cuota miembro (estos últimos deben corresponder a lo establecido por Ley), así como informar al respecto.",
    "Controlar que los depósitos de los aportes se efectúen dentro del plazo establecido por ley e informar a la Gerencia de Pensiones al respecto.",
    "Conducir y controlar la conciliación entre las planillas de aportes y los depósitos respectivos.",
    "Proporcionar al Departamento de Tesorería la información de los depósitos de aportes identificados.",
    "Proporcionar al Departamento de Contabilidad la información de las operaciones de aportes.",
    "Monitorear la elaboración de la información requerida para la realización de estudios actuariales y remitirla a la Gerencia de Riesgos y Desarrollo.",
    "Supervisar la ejecución de los cálculos de las cuentas por cobrar previsionales.",
    "Supervisar que las negociaciones con los afiliados para el reconocimiento de las cuentas por cobrar, se realicen según los procedimientos establecidos.",
    "Controlar la atención de las cuentas por cobrar previsionales de acuerdo a su antigüedad, de manera que no prescriban.",
    "Supervisar la entrega de los expedientes de castigo de cuentas por cobrar a la Gerencia de Administración y Finanzas, según los procedimientos establecidos; así como efectuar el seguimiento respectivo.",
    "Supervisar la entrega de los expedientes de cobranza prejudicial de las cuentas por cobrar previsionales a la Gerencia Legal, según los procedimientos establecidos; así como efectuar el seguimiento respectivo."
   ],
   "requisitos": [
    "Título profesional en derecho, administración, economía, ingeniería industrial, ingeniería de sistemas, contabilidad o afines.",
    "Experiencia general de cinco (5) años, y experiencia específica de cuatro (4) años acumulable como jefe, supervisor o puesto similar.",
    "De preferencia, capacitación especializada en procesos de verificación, normativa de sistemas previsionales, gestión por procesos, gestión administrativa, gestión pública, la misma que es acreditada mediante cursos que totalicen una duración no menor a cuarenta (40) horas lectivas.",
    "De preferencia, capacitación en interpretación de la Norma internacional ISO 37001:2016 Antisoborno."
   ],
   "beneficios": [
    "Remuneración: S/. 6,000.00",
    "Horario de Trabajo regular: 9:00 a.m. a 5:15 p.m. de lunes a viernes (presencial).",
    "Modalidad de contratación: Plazo indeterminado - D.L. 728 (Planilla completa)",
    "Capacitación acorde a la necesidad del puesto."
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-de-departamento-de-recaudacion-y-recuperaciones-caja-de-pensiones-militar-policial-1118128328.html"
  },
  {
   "id": 403,
   "puesto": "Asesor(a) de Ventas",
   "empresa": "CoverPrime",
   "cat": "Ventas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 55,
   "vence": "2026-10-31",
   "restan": 35,
   "score": 92,
   "resumen": "Buscamos una persona proactivo(a) y organizado(a) para unirse a nuestro equipo. El candidato ideal tendrá experiencia en atención al cliente, gestión de ventas y logística de despacho.",
   "funciones": [
    "Gestionar y dar seguimiento via WhatsApp, llamadas y correo electrónico, para realizar cotizaciones, venta y cierre de negocios de los productos/servicios de la empresa.",
    "Realizar el seguimiento de propuestas y coordinar con las áreas de logística y técnica para asegurar una entrega impecable del servicio/producto.",
    "Coordinar con Facturación para validar los depósitos de los pedidos realizados",
    "Gestionar y ampliar la cartera de clientes actuales actualizando la base de datos",
    "Cumplir metas de ventas establecidas y reportar avances al área comercial.",
    "Capacitaciones permanentes en la línea de producto, feedback con el staff de ejecutivo",
    "Seguimiento de leads en el CRM, elaboración de contratos y gestión de instalaciones.",
    "Manejo de consultas y requerimientos post-venta para asegurar la satisfacción del cliente."
   ],
   "requisitos": [
    "Experiencia mínima de 1 a 2 años en el rubro industrial",
    "Técnico y/o Universitario completo o inconcluso",
    "Experiencia de 1 a 2 años en posiciones de ventas y atención al cliente",
    "Experiencia en venta de productos textiles industriales, estructuras metálicas o similares (deseable).",
    "Habilidades de comunicación, persuasión y negociación",
    "Manejo del Office (Word, Excel PPT, Outlook) a nivel Intermedio",
    "De preferencia vivir a zonas aledañas a la Victoria",
    "Horario de trabajo de 8 a 6pm de lunes a viernes",
    "Excelente capacidad de comunicación, persuasión y negociación.",
    "Actitud positiva, proactiva y con gran empatía.",
    "Pasión por las ventas y el logro de metas.",
    "Vocación de servicio y orientación al cliente."
   ],
   "beneficios": [
    "Sueldo base 1500 soles + Comisión escalonada según margen de venta",
    "Disponibilidad a gozar 2 medio días libres al año para lo que gustes (cumpleaños, trámite, etc)",
    "Disponibilidad de 1 día al año para que realices tu examen oncológico preventivo",
    "Bonos por proyectos institucionales o de alto volumen y/o cumplimiento de objetivos",
    "Línea de carrera en el área",
    "Respaldo técnico y operativo",
    "PLanilla todos los beneficios en regimen REMYPE",
    "En Cover prime S.A.C. promovemos una cultura inclusiva, basada en el respeto, la diversidad y la equidad de oportunidades para todos. Fomentamos la participación de personas con discapacidad, en cumplimiento de la Ley N.° 29973, y aseguramos igualdad de condiciones para todos."
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesora-de-ventas-b2b-sector-industrial-coverprime-1118387558.html"
  },
  {
   "id": 404,
   "puesto": "Arquitecto(a) Supervisor de Obra",
   "empresa": "CoverPrime",
   "cat": "Construcción",
   "min": 2500,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 55,
   "vence": "2026-10-31",
   "restan": 35,
   "score": 89,
   "resumen": "COVER PRIME SAC es una empresa peruana que brinda servicios de fabricación e instalación de todo tipo de coberturas para exteriores, con planes de crecimiento. Nos encontramos en busca del mejor talento para el puesto de SUPERVISOR DE OBRA",
   "funciones": [
    "Supervisar la ejecución de trabajos arquitectónicos y acabados, asegurando el cumplimiento de planos, especificaciones técnicas y estándares de calidad.",
    "Coordinar con la constructora, contratistas y especialidades para prevenir y resolver interferencias durante la obra.",
    "Coordinar y dirigir al personal interno en obra",
    "Verificar el avance de obra y el cumplimiento del cronograma, realizando seguimiento a la programación diaria.",
    "Gestionar la logística y los recursos necesarios para la obra.",
    "Controlar la calidad de materiales, acabados y procedimientos constructivos, gestionando oportunamente las observaciones.",
    "Administrar y mantener actualizados los planos, cambios, documentación técnica y reportes de obra."
   ],
   "requisitos": [
    "Observador y detallista",
    "Comunicación efectiva a todo nivel tanto con cliente externo como interno",
    "Proactividad y orientación a resultados.",
    "Capacidad para en equipo y bajo presión",
    "Capacidad para resolución de problemas."
   ],
   "beneficios": [
    "Ingreso a planilla",
    "Todos los beneficios de ley",
    "Capacitación constante",
    "Línea de carrera a futuro dentro de la empresa",
    "Agradable ambiente laboral",
    "Remuneración bruta de S/.2500",
    "¡te invitamos a postular y ser parte de COVERPRIME y crecer juntos!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/arquitectoa-supervisor-de-obra-coverprime-1118387541.html"
  },
  {
   "id": 405,
   "puesto": "Asistente de Tesoreria",
   "empresa": "CoverPrime",
   "cat": "Contabilidad",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 56,
   "vence": "2026-10-30",
   "restan": 34,
   "score": 95,
   "resumen": "Garantizar el cumplimiento oportuno de las obligaciones financieras de la empresa (pagos a proveedores, entidades bancarias, nómina, entre otros).",
   "funciones": [
    "Garantizar el cumplimiento oportuno de las obligaciones financieras de la empresa (pagos a proveedores, entidades bancarias, nómina, entre otros).",
    "Manejo de plataforma de bancos.",
    "Gestionar y actualizar el flujo de caja diario y semanal.",
    "Mantener ordenado actualizados los registros de tesorería y bancos.",
    "Conciliaciones bancarias por centro de costo y partidas de presupuesto.",
    "Apoyar en la coordinación con las áreas de Contabilidad y Operaciones Logísticas para validar facturas, impuestos, fechas de pago y vencimientos, asegurando el cumplimiento de los compromisos financieros.",
    "Elaborar proyecciones de liquidez y flujo de efectivo de manera quincenal",
    "Elaborar reportes financieros y de tesorería de manera mensual",
    "Proponer mejoras en los procesos y controles del área.",
    "Otras funciones asignadas por su jefe inmediato."
   ],
   "requisitos": [
    "Bachiller/Egresado de las carreras de Contabilidad, Economía, Administración, finanzas o carreras afines del área financiera. (INDISPENSABLE).",
    "Contar con 1 año de experiencia mínima en el área de contabilidad, finanzas o tesorería",
    "Manejo de Office a nivel avanzado.",
    "Manejo de plataformas bancarias",
    "Experiencia comprobable en manejo de caja chica y conciliaciones bancarias",
    "Disponibilidad inmediata",
    "Horario de lunes a viernes de 09:00 a.m. a 06:00 p.m. y sábados de 09:00 a.m. a 01:00 p.m en nuestra oficina ubicado en el distrito de la victoria",
    "Responsabilidad y confidencialidad en el manejo de información.",
    "Comunicación efectiva.",
    "Proactividad y orientación a resultados.",
    "Excelentes habilidades analíticas y de organización.",
    "Capacidad para trabajar de forma autónoma y colaborativa."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día",
    "Todos los beneficios de ley",
    "Posibilidad de capacitación constante",
    "Línea de carrera a futuro dentro de la empresa",
    "Agradable ambiente laboral",
    "Remuneración de S/.1500",
    "Si te apasiona el análisis financiero y deseas contribuir al crecimiento de una organización comprometida con aumentar la calidad de vida de las personas, ¡te invitamos a postular y ser parte de COVERPRIME!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-tesoreria-coverprime-1118387435.html"
  },
  {
   "id": 406,
   "puesto": "Asistente de Marketing y Comercial",
   "empresa": "CoverPrime",
   "cat": "Ventas",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Híbrido",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 56,
   "vence": "2026-10-30",
   "restan": 34,
   "score": 89,
   "resumen": "Crear y editar videos para las distintas redes sociales",
   "funciones": [
    "Crear y editar videos para las distintas redes sociales",
    "Crear artículos para linkedin",
    "Crear los copys y contenido cumpliendo con el cronograma de publicaciones en las distintas redes sociales (Facebook, LinkedIn, YouTube, Instagram, tiktok, pinterest) e historias",
    "Participar en videos promocionales según sea necesario",
    "Dar seguimiento operativo al CRM: asignación de leads, monitoreo del embudo comercial y control de calidad de la información registrada detectando tendencia de compra, realización de compra cruzada",
    "Monitorear los precios del mercado para proponer ajustes estratégicos que aumenten la competitividad.",
    "Creación de flyer o promociones para el área comercial",
    "Elaborar y mantener actualizado los catálogos de productos y servicios",
    "Elaborar reportes de ventas para medir el cumplimiento de las metas comerciales.",
    "Responder consultas en las diferentes redes sociales",
    "Prospección de nuevas oportunidades de mercado y tendencias.",
    "Apoyo en la creación de promociones y publicidad en general."
   ],
   "requisitos": [
    "Estudios técnicos o universitarios concluidos en las carreras de Ciencias de la Comunicación, Marketing, Administración o carreras afines",
    "Manejo de Ilustrator, Photoshop, Premiere Pro, Indesign a nivel intermedio, Capcut y herramientas de IA",
    "Manejo de Ms Office a nivel intermedio para reportes y de Google drive",
    "Disponibilidad para realizar toma de fotos en toda lima metropolitana o fuera de lima cuando se requiera (viáticos cubiertos)",
    "Mínimo 2 años de experiencia en la posición de Auxiliar o Asistente de Marketing o puestos afines, en el sector construcción, inmobiliaria o afines (deseable)",
    "Trabajo Hibrido: Disponibilidad para trabajar 2 o 3 días a la semana rotativos en oficina en La victoria de lunes a viernes de 09:00 a.m. a 06:00 p.m. y sábados de 09:00 a.m. a 01:00 p.m.",
    "Disponibilidad para trabajar desde 1 de setiembre",
    "Alto sentido de responsabilidad, orden y criterio lógico",
    "Capacidad de aprendizaje rápido",
    "Capacidad para trabajar de forma autónoma y en equipo.",
    "Capacidad de planificación y organización",
    "Comunicación a todo nivel"
   ],
   "beneficios": [
    "Línea de carrera a futuro dentro de la empresa",
    "Ambiente de trabajo dinámico y enfocado en la excelencia",
    "Remuneración 1500 soles",
    "Ingreso a planilla bajo régimen MYPE con todos los beneficios de ley",
    "Flexibilidad de horario"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asistente-de-marketing-y-comercial-coverprime-1118387417.html"
  },
  {
   "id": 407,
   "puesto": "Asesor de Ventas",
   "empresa": "CAMARA DE COMERCIO EXTERIOR S.A.C.",
   "cat": "Ventas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 56,
   "vence": "2026-10-30",
   "restan": 34,
   "score": 84,
   "resumen": "¿Buscas un lugar donde crecer y ganar bien? ¡Esta es tu oportunidad! En CE buscamos personas con energía, buena comunicación y ganas de vender, para sumarse a nuestro equipo comercial.",
   "funciones": [
    "Atender a nuestros clientes por redes sociales.",
    "Darles a nuestros clientes la información de los productos de su interés.",
    "Hacer el seguimiento al cliente de su curso de interés por nuestra redes sociales."
   ],
   "requisitos": [
    "Habilidades de comunicación",
    "Actitud positiva y orientación a resultados",
    "Ganas de aprender y crecer con nosotros",
    "Lunes a Sábado de 2:00 pm - 10:45 pm.",
    "Lugar: San Isidro | Presencial | Tiempo completo",
    "📌 Apto para personas con discapacidad",
    "¡Si te apasionan las ventas y quieres formar parte de un equipo ganador, postula ahora!"
   ],
   "beneficios": [
    "💰 Sueldo base S/1,200 + Comisiones atractivas todas las semanas",
    "🚌 Bono de movilidad de S/200",
    "🚇 Fácil acceso por el Corredor Rojo (Javier Prado) y cerca a la estación Canaval y Moreyra del Metropolitano",
    "🤝 Excelente ambiente laboral, con un equipo que te acompaña",
    "📚 Capacitaciones constantes para que sigas creciendo profesionalmente",
    "🧘 Pausas activas durante tu jornada",
    "☕ Café los miércoles y 🍿 canchita los viernes, ¡para hacer tu semana más amena!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/unete-a-nuestro-equipo-de-ventas-asesor-de-ventas--turno-tarde-camara-de-comercio-exterior-s.a.c.-1118387337.html"
  },
  {
   "id": 408,
   "puesto": "Coordinador de Mantenimiento",
   "empresa": "Pigmalion Consultores",
   "cat": "Ingeniería",
   "min": 2500,
   "max": 3000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 100,
   "resumen": "Si se encuentra interesado(a), debe enviar su CV al siguiente número: 933 429 319",
   "funciones": [
    "Planificar, coordinar y supervisar el mantenimiento preventivo y correctivo de la planta.",
    "Gestionar el correcto funcionamiento de equipos eléctricos y mecánicos.",
    "Elaborar y dar seguimiento a indicadores e informes de mantenimiento.",
    "Gestionar la compra y cotización de repuestos e insumos necesarios para el área.",
    "Coordinar con el equipo de producción la ejecución de los mantenimientos para asegurar la continuidad de la operación.",
    "Apoyar en la supervisión de personal técnico y promover el cumplimiento de los estándares de seguridad."
   ],
   "requisitos": [
    "Técnico o Egresado en Ingeniería Mecánica o similares.",
    "Entre 1 a 2 años de experiencia en mantenimiento industrial.",
    "Conocimiento en mantenimiento de máquinas envasadoras, hornos y equipos que tengan PLC.",
    "Conocimiento en compras de repuestos y gestión de cotizaciones.",
    "Experiencia en empresas del rubro de alimentos. (Deseable)",
    "Disponibilidad para laborar en San Luis, Lima Perú."
   ],
   "beneficios": [
    "Planilla desde primer día bajo Régimen MYPE",
    "Sueldo entre S/. 2500 – S/. 3000",
    "Asignación familiar según ley",
    "Capacidad de realizar horas extras.",
    "Bonos de Movilidad y Alimentación",
    "Ubicación: San Luis, frente al Mercado de fruta."
   ],
   "url": "https://www.bumeran.com.pe/empleos/coordinador-de-mantenimiento-wimpon-pigmalion-consultores-1118386015.html"
  },
  {
   "id": 409,
   "puesto": "Ingeniero de Proyectos Gestión SSOMA",
   "empresa": "Engineering Services & Consulting S.A.C",
   "cat": "Ingeniería",
   "min": 2500,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 97,
   "resumen": "Apoyar en la planificación, ejecución y seguimiento de los proyectos SSOMA.",
   "funciones": [
    "Apoyar en la planificación, ejecución y seguimiento de los proyectos SSOMA.",
    "Coordinar los recursos necesarios para la correcta ejecución de los servicios.",
    "Realizar el seguimiento al personal destacado en campo y al cumplimiento de los entregables.",
    "Elaborar informes, reportes y documentación técnica para clientes.",
    "Verificar el cumplimiento de la normativa vigente, los requisitos del cliente y los estándares de SINCRO.",
    "Participar en reuniones de coordinación con clientes, proveedores y equipos internos.",
    "Brindar soporte en la elaboración de propuestas técnicas, cotizaciones y procesos de licitación.",
    "Mantener actualizada la documentación técnica y administrativa de los proyectos."
   ],
   "requisitos": [
    "Bachiller de Ingeniería Industrial, Ingeniería de Seguridad, Ingeniería Ambiental, Ingeniería de Minas, Ingeniería Civil, Ingeniería Química o carreras afines.",
    "Deseable título profesional y colegiatura.",
    "Experiencia mínima de 2 años en SSOMA, SST, HSE/EHS o Sistemas Integrados de Gestión.",
    "Experiencia en gestión y seguimiento de proyectos o servicios SSOMA.",
    "Conocimiento de la Ley N.° 29783, IPERC, trabajos de alto riesgo y Sistemas de Gestión ISO 45001, ISO 14001 e ISO 9001.",
    "Manejo de Microsoft Office a nivel intermedio.",
    "Deseable experiencia en elaboración de propuestas técnicas, licitaciones y coordinación con clientes y proveedores.",
    "Deseable conocimiento de herramientas de inteligencia artificial aplicadas a la gestión documental y análisis de información.",
    "Disponibilidad para realizar visitas a campo según requerimiento."
   ],
   "beneficios": [
    "Ubicación: Surquillo, Lima",
    "Horario: Lunes a viernes de 8:30 a.m. a 5:30 p.m.",
    "Remuneración: S/ 2,500",
    "Ingreso a planilla desde el primer día con todos los beneficios de ley",
    "EPS cubierta al 50% y Seguro Oncológico al 100%",
    "Seguro Vida Ley desde el primer día de labores",
    "Participación en utilidades",
    "Acceso a capacitaciones constantes y oportunidades de aprendizaje continuo",
    "Participación en proyectos de alto impacto con clientes líderes",
    "Grato ambiente laboral y actividades de integración",
    "Pertenecer a una empresa líder y en constante crecimiento",
    "¡Sé parte del equipo SINCRO y lleva tu talento al siguiente nivel!"
   ],
   "url": "https://www.bumeran.com.pe/empleos/ingeniero-de-proyectos-gestion-ssoma-engineering-services-consulting-s.a.c-1118386028.html"
  },
  {
   "id": 410,
   "puesto": "Administrador de Operaciones",
   "empresa": "Beliving Talents",
   "cat": "Contabilidad",
   "min": 2000,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 95,
   "resumen": "ENVIA TU CV: 922 - 054 - 346 (UNICO MEDIO DE RECEPCION)",
   "funciones": [
    "Gestionar la emisión de comprobantes de pago mediante plataformas ERP y SUNAT.",
    "Realizar seguimiento y control de comprobantes emitidos.",
    "Revisar cuentas de clientes, identificando pagos pendientes, realizados y diferencias por regularizar.",
    "Gestionar la recepción y validación de documentos para la generación de cuentas por pagar.",
    "Realizar seguimiento de obligaciones pendientes y programación de pagos, asegurando el cumplimiento de plazos.",
    "Gestionar información bancaria de clientes (movimientos, estados de cuenta, carga de documentos e instrucciones operativas).",
    "Elaborar reportes administrativos y financieros sobre pagos, saldos pendientes y disponibilidad de fondos."
   ],
   "requisitos": [
    "Técnico, Bachiller o Licenciado en Administración, Ingeniería Industrial, Contabilidad o carreras afines.",
    "Experiencia mínima de 3 años en áreas administrativas, especialmente en gestión de tesorería y facturación.",
    "Experiencia en outsourcing.",
    "Manejo de Excel intermedio – avanzado.",
    "Deseable nivel de inglés intermedio – avanzado."
   ],
   "beneficios": [
    "Ubicación: Magdalena del Mar (Av. Del Ejército).",
    "Horario: Lunes a viernes de 9:00 a.m. a 6:00 p.m.",
    "Régimen laboral: Planilla completa con todos los beneficios de ley.",
    "Remuneración: S/2000 a S/2500."
   ],
   "url": "https://www.bumeran.com.pe/empleos/administrador-de-operaciones-outsourcing-beliving-talents-1118386627.html"
  },
  {
   "id": 411,
   "puesto": "Practicante Profesional de Contabilidad",
   "empresa": "CENTAURO EDITORES SAC",
   "cat": "Prácticas",
   "min": 1300,
   "max": 1600,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 95,
   "resumen": "Aprenderás a llevar la Contabilidad de la mano de una Contadora experta.",
   "funciones": [
    "Aprenderás a llevar la Contabilidad de la mano de una Contadora experta.",
    "Aprenderás a manejar Flujo de Caja de la mano de una Contadora experta",
    "Realizar cruce y validación de documentos: facturas, guías de remisión y pagos",
    "Detectar y reportar inconsistencias o diferencias",
    "Mantener orden y control documentario"
   ],
   "requisitos": [
    "Egresado(a) reciente de Contabilidad",
    "Alto nivel de orden y organización personal",
    "Responsabilidad en el cumplimiento de tareas y fechas",
    "Pensamiento lógico y estructurado Requisitos técnicos",
    "Manejo de Office a nivel intermedio, mínimo.",
    "Deseable conocimiento básico de documentos contables.",
    "Disponibilidad inmediata"
   ],
   "beneficios": [
    "Aprendizaje directo en gestión real de flujo de caja empresarial",
    "Formación práctica en control financiero",
    "Acompañamiento cercano en tu desarrollo",
    "Oportunidad de crecimiento según desempeño",
    "Grato ambiente de trabajo.",
    "La Empresa invita los almuerzos del día viernes",
    "Acuerdos de practicas con todo lo que la ley establece.",
    "Modalidad: Presencial (Lima)",
    "Subvención: S/ 1,300-1600 Postulación"
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-profesional-de-contabilidad-centauro-editores-sac-1118386941.html"
  },
  {
   "id": 412,
   "puesto": "Auxiliar Operativo",
   "empresa": "Servicios Postales del Perú S.A.",
   "cat": "Logística",
   "min": 1130,
   "max": 1130,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 95,
   "resumen": "Carga y descarga de paquetes, encomiendas, sacas y documentación.",
   "funciones": [
    "Carga y descarga de paquetes, encomiendas, sacas y documentación.",
    "Traslado, ordenamiento y acondicionamiento de envíos dentro del área operativa.",
    "Recepción, clasificación y verificación de documentación y cargos.",
    "Apertura y despacho de cargos y documentación.",
    "Digitalización y registro de documentos en el sistema.",
    "Apoyo en actividades operativas propias de la distribución y procesamiento de envíos."
   ],
   "requisitos": [
    "Secundaria completa. (Indispensable)",
    "Manejo de base de datos: Excel nivel intermedio (deseable)",
    "Manejo de digitalización de documentos",
    "Experiencia mínima de 6 meses en actividades similares y afines a las descritas",
    "Experiencia en clasificación documentaria manual y electrónica",
    "Experiencia en atención al cliente (deseable)",
    "Disponibilidad para laborar en el distrito de Los Olivos"
   ],
   "beneficios": [
    "Ingreso a planilla con todos los beneficios de ley.",
    "Remuneración mensual de S/1130 soles.",
    "Horario de trabajo: Lunes a sabados (48 horas semanales)",
    "Alimentación cubierta al 50%",
    "Aquellos que cumplan con el perfil deberán ENVIAR su cv al correo de seleccion(arroba)serpost.com.pe"
   ],
   "url": "https://www.bumeran.com.pe/empleos/auxiliar-operativo-almacen-los-olivos-servicios-postales-del-peru-s.a.-1118387001.html"
  },
  {
   "id": 413,
   "puesto": "Operario de montaje",
   "empresa": "Estanterías Metálicas JRM S.A.C",
   "cat": "Logística",
   "min": 1800,
   "max": 2000,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 93,
   "resumen": "La empresa Estanterías Metálicas JRM👨🏻‍🏭🔩🙌🏻, - Somos una empresa especializada en el diseño y construcción de centros de distribución y operaciones logísticas",
   "funciones": [
    "Solicitar y disponer de las herramientas requeridas para la ejecución del montaje según procedimiento.",
    "Recepcionar y apoyar con la manipulación de los materiales y accesorios entregados a obra.",
    "Realizar el inventario de saldos de obra al finalizar la instalación.",
    "Conocer y aplicar los planes y respuestas a las emergencias establecidas en la organización."
   ],
   "requisitos": [
    "Experiencia mínima d e 6 meses a 1 año en trabajos de altura, estanterías y/o racks. (Comprobada)",
    "Disponibilidad para realizar trabajo físico.",
    "Personas proactivas y responsables con el trabajo.",
    "Disponibilidad para laborar en diferentes obras."
   ],
   "beneficios": [
    "Pago quincenal y mensual",
    "Sueldo superior al mercado (S/1800 - S/2000 segùn evaluaciòn del àrea)",
    "Pago de movilidad y almuerzo (S/340 mensuales)",
    "Adicional de condición de trabajo (S/200)",
    "Línea de carrera. * Convenios corporativos",
    "Ingreso a planilla desde el primer día con todo los beneficios de Ley al 100%.",
    "Trabajo de Lunes a Viernes (8-5:30) Sábado (8-1pm) - Posibilidad de hacer horas extras",
    "Puedes mandar tu CV al 989 304 973 RUTH INCA - O postular directamente por acá",
    "Puedes acercarte directamente de Lunes a viernes de 10am a 12:30 a Av las Américas 275, La Victoria"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operario-de-montaje-urgente-estanterias-metalicas-jrm-s.a.c-1118386775.html"
  },
  {
   "id": 414,
   "puesto": "Asesor(a) de ventas San Isidro",
   "empresa": "ADECCO",
   "cat": "Ventas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 92,
   "resumen": "En Adecco, por encargo de nuestro cliente Cámara de Comercio Exterior , nos encontramos en la búsqueda de Asesor(a) de Ventas Digitales - San Isidro con experiencia en ventas y enfoque a resultados.",
   "funciones": [
    "Asesorar y vender cursos digitales a clientes potenciales.",
    "Gestionar leads provenientes de diferentes canales digitales.",
    "Atender consultas a través de chat y otros medios de contacto.",
    "Realizar seguimiento comercial hasta el cierre de la venta.",
    "Brindar soporte al cliente respecto a accesos e inicio de cursos."
   ],
   "requisitos": [
    "Secundaria completa, estudios técnicos o universitarios en curso.",
    "Experiencia mínima de 6 meses en Ventas digitales, Call Center, Ventas directas o Atención comercial.",
    "Manejo de herramientas digitales y seguimiento comercial.",
    "Disponibilidad para trabajar en San Isidro.",
    "Excelente comunicación, proactividad y orientación a resultados."
   ],
   "beneficios": [
    "Sueldo fijo: S/ 1,200.",
    "Atractivas comisiones semanales desde la primera semana.",
    "Ingresos variables aproximados entre S/ 1,300 y S/ 2,000 adicionales según desempeño.",
    "Capacitación y oportunidades de crecimiento",
    "Lugar de trabajo: San Isidro.",
    "Horario: Lunes a sábado de 2:00 p.m. a 10:45 p.m. (45 minutos de refrigerio).",
    "Modalidad: Presencial.",
    "Contrato bajo régimen MYPE"
   ],
   "url": "https://www.bumeran.com.pe/empleos/asesora-de-ventas-san-isidro-adecco-1118386014.html"
  },
  {
   "id": 415,
   "puesto": "Supervisor Comercial Internacional",
   "empresa": "Talento Humano Perú",
   "cat": "Ventas",
   "min": 3000,
   "max": 3000,
   "moneda": "USD",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 92,
   "resumen": "Nuestro cliente, Agencia Internacional especializada en la comercialización de Seguros de Vida, Salud e Inversiones, nos ha encargado la búsqueda de un (1) Supervisor Comercial Internacional de Seguros de Vida y Salud.",
   "funciones": [
    "Diseñar e implementar estrategias para el cumplimiento de metas de ventas.",
    "Gestionar el desempeño de la fuerza comercial mediante indicadores (KPIs).",
    "Brindar acompañamiento y coaching comercial al equipo de ventas.",
    "Prospectar y desarrollar nuevas oportunidades de negocio.",
    "Mantener y fortalecer relaciones con clientes corporativos e individuales.",
    "Elaborar reportes comerciales y proyecciones de ventas para la dirección.",
    "Coordinar campañas comerciales y actividades de capacitación.",
    "Garantizar el cumplimiento de las políticas y lineamientos comerciales de la compañía.",
    "Promover la comercialización de productos internacionales de Vida, Salud e Inversiones."
   ],
   "requisitos": [
    "Profesional universitario en Administración, Marketing, Economía, Ingeniería Comercial o carreras afines.",
    "Experiencia mínima de tres (3) años en posiciones comerciales a fines de sector.",
    "Experiencia en el área comercial relacionada a Seguros de Vida, Seguros de Salud, AFPs, Seguros Internacionales o relacionados.",
    "Experiencia liderando equipos de ventas o fuerza comercial.",
    "Ms. Office, nivel intermedio. • Inglés, nivel intermedio."
   ],
   "beneficios": [
    "Remuneración: $ 3,000.00 dólares + Comisiones.",
    "Contrato Honorarios Profesionales con empresa extranjera (Locador de Servicios - USA).",
    "Vacaciones al año: 12 días hábiles.",
    "MODALIDAD DE TRABAJO Y HORARIO DE MIAMI",
    "Lunes a viernes de 9:00 a.m. a 6:00 p.m.",
    "Distrito en San Isidro.",
    "Trabajo presencial."
   ],
   "url": "https://www.bumeran.com.pe/empleos/supervisor-comercial-internacional--seguros-de-vida-y-salud-talento-humano-peru-1118386040.html"
  },
  {
   "id": 416,
   "puesto": "Supervisor(a) de Energías Renovables",
   "empresa": "Talento Humano Perú",
   "cat": "Tecnología",
   "min": 2500,
   "max": 2500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Piura",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-11-28",
   "restan": 63,
   "score": 92,
   "resumen": "Supervisar a pasantes estudiantes o profesionales extranjeros que trabajan con la ONG en campo.",
   "funciones": [
    "Supervisar a pasantes estudiantes o profesionales extranjeros que trabajan con la ONG en campo.",
    "Soporte del jefe inmediato, respecto al desarrollo de proyectos.",
    "Gestionar el desarrollo social comunitario relacionados a energías renovables."
   ],
   "requisitos": [
    "Bachiller en la carrera de Ingeniería Civil, Ingeniería Industrial, Ingeniería de Energía, Ingeniería en Energías Renovables, Ingeniería Mecánica, Ingeniería Eléctrica o carreras afines al puesto.",
    "Experiencia mínima de un (1) año en trabajos relacionados a energías renovables.",
    "Conocimiento técnico en energía solar.",
    "Conocimiento en energía eólica.",
    "Idioma inglés, nivel intermedio/avanzado."
   ],
   "beneficios": [
    "Ingreso a Planilla General, con todos los beneficios de acuerdo a Ley.",
    "Contrato de 1 año, renovable.",
    "Remuneración entre: S/. 2,500.00 soles a S/. 3,500.00 soles (De acuerdo a evaluación)",
    "Estadía, se brinda casa para empleados.",
    "Alimentación al 100%.",
    "EPS al 100% para el trabajador.",
    "Curso de inglés para nivelarse al nivel avanzado.",
    "Línea de carrera.",
    "Buen clima laboral, oficina en zona de playa.",
    "LUGAR Y HORARIO DE TRABAJO",
    "Lobitos, Talara, Piura.",
    "Lunes a viernes de 9:00 a.m. a 5:00 p.m. y sábados (eventualmente)"
   ],
   "url": "https://www.bumeran.com.pe/empleos/supervisora-de-energias-renovables-|-piura-talara-talento-humano-peru-1118386921.html"
  },
  {
   "id": 417,
   "puesto": "Operaria de Limpieza de Producción",
   "empresa": "CYL",
   "cat": "Ingeniería",
   "min": 1150,
   "max": 1150,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-09-29",
   "restan": 3,
   "score": 91,
   "resumen": "Limpiar y contar las piezas durante el proceso de producción.",
   "funciones": [
    "Limpiar y contar las piezas durante el proceso de producción.",
    "Brindar apoyo a los operarios de máquina.",
    "Mantener el orden y la limpieza del área de trabajo.",
    "Cumplir con las normas de calidad y seguridad.",
    "Desarrollar sus funciones dentro de planta."
   ],
   "requisitos": [
    "Secundaria completa",
    "Con o sin experiencia.",
    "DISPOSICIÓN PARA APRENDER Y ADAPTARSE EN EL ÁREA.",
    "Disponibilidad para realizar horas extras.",
    "Disponibilidad inmediata para laborar dentro de planta.",
    "Domingo día libre (ROTAN DE MANERA MENSUAL)",
    "Turno 1 Lunes a Viernes de 7 am a 4:15 pm y sábados de 7 am a 12:30 pm",
    "Turno 2 Lunes a Viernes de 9 am a 7:15 pm."
   ],
   "beneficios": [
    "Ingreso a planilla directo (todos los beneficios+SSALUD+ASIGNACION FAMILIAR+UTILIDADES)",
    "Bono de transporte+ bono por asistencia",
    "Estabilidad laboral",
    "Sueldo s/1150.00 aumenta al año s/1180. 00",
    "Alimentación un 70% cubierto"
   ],
   "url": "https://www.bumeran.com.pe/empleos/operaria-de-limpieza-de-produccion-ingreso-a-planillabonos-ate-cyl-1118387056.html"
  },
  {
   "id": 418,
   "puesto": "Técnico de Farmacia para atención al cliente",
   "empresa": "Confidencial",
   "cat": "Salud",
   "min": 1500,
   "max": 1500,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 90,
   "resumen": "Por encargado de nuestro cliente en el area de Farmacias nos encontramos en busqueda de un Técnico de Farmacia / Miraflores",
   "funciones": [
    "Dar seguimiento al paciente del canal asignado",
    "Gestionar, coordina de forma correcta la atención al paciente según lo indicado en su receta",
    "Absolver consultas e identificar las necesidades del los pacientes",
    "Informar y Coordinar con el cliente/paciente si hay observaciones en su solicitud",
    "Realizar otras funciones relacionadas al puesto.",
    "Tipo de trabajo",
    "Rango de horario 8am a 8pm dentro de ello habrían 2 turnos para poder rotar.",
    "Trabajo Lunes a sábado y domingo descansan",
    "Rotan cada turno pero de manera mensual ( quiere decir que les asignan un turno y estarán en ese turno 1 mes aproximadamente y luego rotan a otro de esos 3 turnos."
   ],
   "requisitos": [
    "Egresados y/o titulados de la carrera Técnico de Farmacia.",
    "Disponibilidad para laborar en turnos rotativos.",
    "Experiencia de 6 meses en boticas o Farmacias."
   ],
   "beneficios": [
    "Ingreso a planilla desde el primer día con todo los beneficios de ley al 100%.",
    "(CTS, gratificaciones, vacaciones, pago de Essalud, asignación familiar)",
    "Salario acorde al mercado",
    "Capacitaciones constantes y desarrollo de proyectos dentro del área.",
    "Salario S/1500",
    "Facilidades para adquirir productos de la empresa."
   ],
   "url": "https://www.bumeran.com.pe/empleos/tecnico-de-farmacia-para-atencion-al-cliente-cuidafarma-1118386056.html"
  },
  {
   "id": 419,
   "puesto": "Practicante de Soporte Técnico",
   "empresa": "BOLD ASCENT",
   "cat": "Prácticas",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 89,
   "resumen": "¡Únete a nuestro equipo de Soporte Técnico!",
   "funciones": [
    "Gestión de Software: Instalarás y configurarás programas de forma remota y presencial.",
    "Soporte al Usuario: Ayudarás a resolver problemas técnicos de nuestros clientes con la mejor actitud.",
    "Documentación: Apoyarás en la creación de guías y participarás en proyectos de mejora.",
    "Apoyo en Hardware: Brindarás soporte en el ensamblaje y mantenimiento preventivo de nuestros productos.",
    "Impacto Real: Participarás en proyectos con clientes internacionales desde el primer día."
   ],
   "requisitos": [
    "Educación: Estudiante (con disponibilidad de tiempo completo) o Egresado, ya sea de carrera Técnica o Universitaria en Sistemas, Informática, Computación, Redes o afines.",
    "Conocimientos: Manejo de Windows OS, instalación de software y nociones básicas/intermedias de ensamblaje de equipos de cómputo .",
    "Habilidades: Persona proactiva, con iniciativa y que disfrute el trabajo en equipo.",
    "Actitud: ¡Muchas ganas de crecer y aprender!"
   ],
   "beneficios": [
    "Línea de carrera: Posibilidad de ascender en corto plazo.",
    "Crecimiento salarial: Incrementos según tu desempeño y compromiso.",
    "Capacitación constante: uso de herramientas tecnológicas de vanguardia.",
    "Beneficios: Seguro FOLA.",
    "Ambiente: Un lugar dinámico, divertido y con excelente clima laboral."
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-de-soporte-tecnico-bold-ascent-1118386254.html"
  },
  {
   "id": 420,
   "puesto": "Labora de Lunes a Viernes FIJO! Atención al cliente",
   "empresa": "ManpowerGroup Perú",
   "cat": "Atención al Cliente",
   "min": 2100,
   "max": 2100,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 89,
   "resumen": "Por encargo de nuestro cliente líder, nos encontramos en búsqueda de talentos para cubrir la posición de:",
   "funciones": [
    "Resolver consultas en primera llamada",
    "Gestionar beneficios, reclamos y cuentas médicas",
    "Brindar soporte a clientes adultos estadounidenses",
    "Documentar interacciones con precisión y empatía"
   ],
   "requisitos": [
    "Secundaria completa",
    "6 meses en atención al cliente",
    "Inglés B2 – B2+ (indispensable)",
    "Manejo de Office (indispensable)",
    "100% presencial – San Borja"
   ],
   "beneficios": [
    "Sueldo: S/ 2,100",
    "Bono hasta 15%",
    "Movilidad: S/ 80",
    "Capacitación pagada: S/ 1,750 + movilidad",
    "Línea de carrera desde el mes 1",
    "Lunes a viernes (descanso fines de semana)",
    "Se pagan feriados peruanos (triple).",
    ".“En Manpower promovemos la inserción e inclusión laboral de personas con discapacidad\""
   ],
   "url": "https://www.bumeran.com.pe/empleos/labora-de-lunes-a-viernes-fijo-atencion-al-cliente-turno-manana-s.-2500-manpowergroup-peru-1118386965.html"
  },
  {
   "id": 421,
   "puesto": "Operador de Monitoreo Vehicular",
   "empresa": "Confidencial",
   "cat": "Otros",
   "min": 1200,
   "max": 1200,
   "moneda": "PEN",
   "modalidad": "Remoto",
   "ciudad": "Lima",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 88,
   "resumen": "CERTISAT , empresa especializada en soluciones de rastreo satelital y monitoreo vehicular, se encuentra en la búsqueda de un(a) Operador(a) de Monitoreo Vehicular para integrarse a nuestro equipo.",
   "funciones": [
    "Monitorear en tiempo real la ubicación y estado de las unidades vehiculares mediante la plataforma GPS.",
    "Recepcionar, analizar y gestionar alertas generadas por el sistema (exceso de velocidad, botón de pánico, desvíos de ruta, ingreso a zonas restringidas, desconexión de equipos, entre otros).",
    "Mantener comunicación permanente con conductores y clientes para la atención de incidencias.",
    "Activar y ejecutar los protocolos de seguridad establecidos según el tipo de evento.",
    "Registrar y reportar incidencias, novedades y eventos en el sistema de monitoreo.",
    "Coordinar con las áreas correspondientes para la atención oportuna de emergencias.",
    "Elaborar reportes de monitoreo y seguimiento de eventos.",
    "Brindar soporte telefónico a clientes respecto al servicio de monitoreo.",
    "Mantener actualizada la información de las unidades y clientes en la plataforma."
   ],
   "requisitos": [
    "Experiencia mínima de 1 año en call center , central de monitoreo GPS o monitoreo vehicular.",
    "Manejo de computadoras y herramientas de Microsoft Office a nivel básico.",
    "Capacidad para trabajar bajo presión y resolver incidencias.",
    "Buena comunicación verbal y escrita.",
    "Atención al detalle y alto sentido de responsabilidad.",
    "Disponibilidad para laborar en horario de 3:00 p.m. a 11:00 p.m.",
    "Disponibilidad para trabajar con descanso rotativo ."
   ],
   "beneficios": [
    "Lugar de trabajo: San Isidro – Modalidad híbrida (presencial y remoto).",
    "Horario: 3:00 p.m. a 11:00 p.m.",
    "Descanso: Rotativo.",
    "Sueldo básico de S/ 1,200 .",
    "Bonificaciones por cumplimiento de metas.",
    "Ingreso a planilla desde el primer día.",
    "Todos los beneficios de ley.",
    "Capacitación constante.",
    "Excelente clima laboral.",
    "Oportunidad de desarrollo profesional.",
    "Si deseas formar parte de una empresa en crecimiento y contribuir a la seguridad de nuestros clientes, postula y únete a CERTISAT."
   ],
   "url": "https://www.bumeran.com.pe/empleos/operador-de-monitoreo-vehicular--central-de-monitoreo-gps-1118386543.html"
  },
  {
   "id": 422,
   "puesto": "Jefe de Marketing",
   "empresa": "Confidencial",
   "cat": "Marketing",
   "min": 1800,
   "max": 1800,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Arequipa",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-11-28",
   "restan": 63,
   "score": 87,
   "resumen": "Personal a cargo: 2 colaboradores (Edición Audiovisual y Diseño/Contenido)",
   "funciones": [
    "Asimismo, liderar al equipo de marketing y coordinar de manera transversal con los jefes de categoría para asegurar la correcta ejecución de las campañas comerciales.",
    "Estrategia de Marketing",
    "Ejecutar el plan anual y mensual de marketing.",
    "Crear campañas comerciales orientadas a incrementar el tráfico de visitantes.",
    "Diseñar estrategias para potenciar las ventas de los comercios de la galería.",
    "Desarrollar campañas para fortalecer el posicionamiento de la galería como destino comercial.",
    "Implementar campañas para atraer nuevos arrendatarios a los espacios comerciales disponibles.",
    "Definir el calendario comercial de campañas, promociones y eventos.",
    "Marketing Digital",
    "Planificar campañas para Facebook, Instagram, TikTok y otras plataformas digitales.",
    "Gestionar campañas pagadas (Meta Ads, Google Ads y TikTok Ads).",
    "Analizar métricas digitales y optimizar campañas de manera permanente."
   ],
   "requisitos": [
    "Orientación a resultados.",
    "Trabajo colaborativo.",
    "Específicas del cargo",
    "Comunicación e influencia.",
    "Análisis de datos.",
    "Orientación al cliente."
   ],
   "beneficios": [
    "Sueldo básico: S/ 1,800.00",
    "Bono variable: Hasta S/ 1,500.00 mensuales , sujeto al cumplimiento de indicadores.",
    "Ingreso a planilla desde el primer día.",
    "Régimen General (100% de beneficios de ley).",
    "Seguro EsSalud.",
    "Asignación familiar (de corresponder)."
   ],
   "url": "https://www.bumeran.com.pe/empleos/jefe-de-marketing-1118356328.html"
  },
  {
   "id": 423,
   "puesto": "Practicante de Contabilidad",
   "empresa": "Confidencial",
   "cat": "Prácticas",
   "min": 565,
   "max": 565,
   "moneda": "PEN",
   "modalidad": "Presencial",
   "ciudad": "Arequipa",
   "fuente": "Bumeran",
   "dias": 57,
   "vence": "2026-10-29",
   "restan": 33,
   "score": 85,
   "resumen": "Convocatoria – Practicante de Contabilidad",
   "funciones": [
    "Apoyo en el registro de comprobantes de compras y ventas en el sistema contable.",
    "Registro y control de caja chica.",
    "Apoyo en el análisis de cuentas contables.",
    "Organización, clasificación y archivo de documentación contable.",
    "Apoyo en la elaboración de reportes e informes contables.",
    "Verificación de documentos (facturas, recibos, entre otros).",
    "Apoyo en conciliaciones bancarias.",
    "Apoyo en el control de cuentas por pagar y cobrar.",
    "Soporte en procesos tributarios (registro de compras, ventas, detracciones, entre otros).",
    "Otras funciones asignadas por el jefe inmediato."
   ],
   "requisitos": [
    "Estudiante de últimos ciclos o egresado de Contabilidad o carreras afines.",
    "Manejo de Microsoft Office (nivel intermedio).",
    "Responsabilidad y organización.",
    "Manejo de sistemas contables.",
    "Experiencia previa mínima de 6 meses."
   ],
   "beneficios": [
    "Convenio de prácticas preprofesionales o profesionales.",
    "Certificado de prácticas.",
    "Incentivo economico .",
    "Oportunidad de aprendizaje y desarrollo profesional.",
    "Buen clima laboral.",
    "Posibilidad de línea de carrera.",
    "Lunes a viernes: 8:00 a. m. – 6:30 p. m.",
    "Sábados: 8:00 a. m. – 1:00 p. m.",
    "Envia tu CV: seleccion.mindgroup (arroba) gmail.com"
   ],
   "url": "https://www.bumeran.com.pe/empleos/practicante-de-contabilidad-1118386534.html"
  }
 ]
};
